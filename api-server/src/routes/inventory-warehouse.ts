...
router.get("/inventory/dashboard", async (_req, res) => {
  try {
    const [warehouses, rawStock, finishedStock, movements] = await Promise.all([
      pool.query("SELECT COUNT(*) as count FROM warehouses WHERE is_active=true"),
      pool.query("SELECT COUNT(*) as count, COALESCE(SUM(total_value),0) as total_value FROM raw_material_stock"),
      pool.query("SELECT COUNT(*) as count FROM finished_goods_stock"),
      pool.query("SELECT COUNT(*) as count FROM stock_movements WHERE created_at > NOW() - INTERVAL '30 days'"),
    ]);
    res.json({
      totalWarehouses: parseInt(warehouses.rows[0].count),
      rawMaterialItems: parseInt(rawStock.rows[0].count),
      rawMaterialValue: parseInt(rawStock.rows[0].total_value),
      finishedGoodsItems: parseInt(finishedStock.rows[0].count),
      recentMovements: parseInt(movements.rows[0].count),
    });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
