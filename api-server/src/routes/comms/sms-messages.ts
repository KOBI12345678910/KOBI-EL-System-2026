// ============================================================
// comms.sms_messages — list / get / send / update / delete
// ============================================================
...
router.get("/", async (req: Request, res: Response) => {
  const q = parseQuery(ListSmsQuerySchema, req, res);
  if (!q) return;
  try {
    ...
    res.json(result);
  } catch (err) { sendDbError(res, err, "sms:list"); }
});
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const r = await db.execute(sql`
      select * from comms.sms_messages
      where id = ${id} and coalesce(is_deleted, false) = false
    `);
    const row = (r.rows ?? [])[0];
    if (!row) return res.status(404).json({ error: "הודעת SMS לא נמצאה" });
    res.json(row);
  } catch (err) { sendDbError(res, err, "sms:get"); }
});

router.post("/send", async (req, res) => {
  const d = parseBody(SendSmsSchema, req, res); if (!d) return;
  ...
  try {
    ...
    const r = await db.execute(sql`
      insert into comms.sms_messages (
        ...
        ${d.phone_number}, ${body}, 'outbound', 'queued',
  ...
  } catch (err) { sendDbError(res, err, "sms:send"); }
});

router.patch("/:id", async (req, res) => {
  const id = idFromParams(req, res); if (!id) return;
  const d = parseBody(UpdateSmsSchema, req, res); if (!d) return;
  try {
    const r = await db.execute(sql`
      update comms.sms_messages set
    ...
    if (!row) return res.status(404).json({ error: "הודעת SMS לא נמצאה" });
    res.json(row);
  } catch (err) { sendDbError(res, err, "sms:update"); }
});
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    const r = await db.execute(sql`
      update comms.sms_messages set is_deleted = true, updated_at = now()
      where id = ${id} and coalesce(is_deleted, false) = false
      returning id
    `);
    if ((r.rows ?? []).length === 0) return res.status(404).json({ error: "הודעת SMS לא נמצאה" });
    res.status(204).send();
  } catch (err) { sendDbError(res, err, "sms:delete"); }
});
