...
router.post("/bom-headers", async (req, res) => {
  ...
  } catch (err: any) { /* שגיאה ביצירת כותרת BOM */ console.error("POST /bom-headers error:", err.message); res.status(500).json({ error: "שגיאה ביצירת כותרת BOM" }); }
});
...
router.put("/bom-headers/:id", async (req, res) => {
  ...
  } catch (err: any) { /* שגיאה בעדכון כותרת BOM */ console.error("PUT /bom-headers error:", err.message); res.status(500).json({ error: "שגיאה בעדכון כותרת BOM" }); }
});

router.delete("/bom-headers/:id", async (req, res) => {
  try {
  /* מחיקת כותרת BOM כולל שורות - פעולה מורכבת */
  ...
  } catch (err: any) { /* שגיאה במחיקת כותרת BOM */ console.error("DELETE /bom-headers error:", err.message); res.status(500).json({ error: "שגיאה במחיקת כותרת BOM" }); }
});
...
router.post("/production-work-orders", async (req, res) => {
  ...
  } catch (err: any) { /* שגיאה ביצירת הזמנת עבודה */ console.error("POST /production-work-orders error:", err.message); res.status(500).json({ error: "שגיאה ביצירת הזמנת עבודה" }); }
});
...
router.put("/production-work-orders/:id", async (req, res) => {
  ...
  } catch (err: any) { /* שגיאה בעדכון הזמנת עבודה */ console.error("PUT /production-work-orders error:", err.message); res.status(500).json({ error: "שגיאה בעדכון הזמנת עבודה" }); }
});
...
router.delete("/production-plans/:id", async (req, res) => {
  try {
  /* מחיקת תוכנית ייצור כולל שורות - פעולה מורכבת */
  ...
  res.json({ success: true });
  } catch (err: any) { /* שגיאה במחיקת תוכנית ייצור */ console.error("DELETE /production-plans error:", err.message); res.status(500).json({ error: "שגיאה במחיקת תוכנית ייצור" }); }
});
...
router.post("/qc-inspections", async (req, res) => {
  ...
  } catch (err: any) { /* שגיאה ביצירת בדיקת איכות */ console.error("POST /qc-inspections error:", err.message); res.status(500).json({ error: "שגיאה ביצירת בדיקת איכות" }); }
});
...
router.delete("/machines/:id", async (req, res) => {
  try {
  /* מחיקת מכונה כולל רשומות תחזוקה - פעולה מורכבת */
  ...
  res.json({ success: true });
  } catch (err: any) { /* שגיאה במחיקת מכונה */ console.error("DELETE /machines error:", err.message); res.status(500).json({ error: "שגיאה במחיקת מכונה" }); }
});
...
router.post("/machine-maintenance", async (req, res) => {
  ...
  } catch (err: any) { /* שגיאה ביצירת רשומת תחזוקה */ console.error("POST /machine-maintenance error:", err.message); res.status(500).json({ error: "שגיאה ביצירת רשומת תחזוקה" }); }
});
...
router.delete("/qa-test-plans/:id", async (req, res) => {
  try {
  /* מחיקת תוכנית בדיקות כולל מקרי בדיקה - פעולה מורכבת */
  await q(`DELETE FROM qa_test_cases WHERE plan_id=${req.params.id}`);
  await q(`DELETE FROM qa_test_plans WHERE id=${req.params.id}`);
  res.json({ success: true });
  } catch (err: any) { /* שגיאה במחיקת תוכנית בדיקות */ console.error("DELETE /qa-test-plans error:", err.message); res.status(500).json({ error: "שגיאה במחיקת תוכנית בדיקות" }); }
});
