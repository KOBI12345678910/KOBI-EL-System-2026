...
router.post("/general-ledger", async (req, res) => {
  ...
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה ביצירת רשומת ספר חשבונות ראשי */ console.error("POST /general-ledger error:", err.message); res.status(500).json({ error: "שגיאה ביצירת רשומת ספר חשבונות ראשי" }); }
});
...
router.put("/general-ledger/:id", async (req, res) => {
  ...
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה בעדכון ספר חשבונות ראשי */ console.error("PUT /general-ledger error:", err.message); res.status(500).json({ error: "שגיאה בעדכון ספר חשבונות ראשי" }); }
});
...
router.post("/expense-reports", async (req, res) => {
  ...
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה ביצירת דוח הוצאות */ console.error("POST /expense-reports error:", err.message); res.status(500).json({ error: "שגיאה ביצירת דוח הוצאות" }); }
});
...
router.put("/expense-reports/:id", async (req, res) => {
  ...
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה בעדכון דוח הוצאות - כולל לוגיקת אישור והחזר */ console.error("PUT /expense-reports error:", err.message); res.status(500).json({ error: "שגיאה בעדכון דוח הוצאות" }); }
});
...
router.post("/expense-reports/:id/approve", async (req, res) => {
  ...
  const rows = await q(`SELECT * FROM expense_reports WHERE id=${id}`);
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה באישור דוח הוצאות */ console.error("POST /expense-reports/approve error:", err.message); res.status(500).json({ error: "שגיאה באישור דוח הוצאות" }); }
});
...
router.post("/expense-reports/:id/lines", async (req, res) => {
  ...
  res.json(allLines);
  } catch (err: any) { /* שגיאה בהוספת שורות לדוח הוצאות - כולל לולאה וחישוב סיכום */ console.error("POST /expense-reports/lines error:", err.message); res.status(500).json({ error: "שגיאה בהוספת שורות לדוח הוצאות" }); }
});
...
router.delete("/expense-report-lines/:lineId", async (req, res) => {
  ...
  res.json({ success: true });
  } catch (err: any) { /* שגיאה במחיקת שורת הוצאה - כולל עדכון סיכום דוח */ console.error("DELETE /expense-report-lines error:", err.message); res.status(500).json({ error: "שגיאה במחיקת שורת הוצאה" }); }
});
...
router.get("/executive-summary", async (req, res) => {
  ...
  const [revenueData, expensesData, cashData, arData, apData, prevRevenueData] = await Promise.all([
    q(`SELECT COALESCE(SUM(total_amount), 0) as total FROM accounts_receivable WHERE invoice_date >= ${dateFilter} AND status NOT IN ('cancelled')`),
    ...
    q(`SELECT
      ...
      COALESCE(SUM(balance_due) FILTER (WHERE due_date < CURRENT_DATE - 90), 0) as overdue_90
    FROM accounts_receivable WHERE status NOT IN ('paid','cancelled','written_off')`),
    q(`SELECT
      COALESCE(SUM(balance_due), 0) as total,
      COUNT(*) FILTER (WHERE status IN ('open','partial','overdue')) as open_count,
      COALESCE(SUM(balance_due) FILTER (WHERE due_date BETWEEN CURRENT_DATE AND CURRENT_DATE + 30), 0) as due_soon,
      COALESCE(SUM(balance_due) FILTER (WHERE due_date < CURRENT_DATE), 0) as overdue
    FROM accounts_payable WHERE status NOT IN ('paid','cancelled')`),
    q(`SELECT COALESCE(SUM(total_amount), 0) as total FROM accounts_receivable WHERE invoice_date >= ${dateFilter} - INTERVAL '1 ${period === "month" ? "month" : period === "quarter" ? "quarter" : "year"}' AND invoice_date < ${dateFilter} AND status NOT IN ('cancelled')`)
  ]);
  ...
  const topCustomers = await q(`SELECT customer_name as name, COALESCE(SUM(total_amount), 0) as revenue FROM accounts_receivable WHERE invoice_date >= ${dateFilter} AND status NOT IN ('cancelled') GROUP BY customer_name ORDER BY revenue DESC LIMIT 5`);
  const topSuppliers = await q(`SELECT supplier_name as name, COALESCE(SUM(total_amount), 0) as amount FROM accounts_payable WHERE invoice_date >= ${dateFilter} AND status NOT IN ('cancelled') GROUP BY supplier_name ORDER BY amount DESC LIMIT 5`);

  const cashInflows = await q(`SELECT COALESCE(SUM(amount), 0) as total FROM cash_flow WHERE flow_type='inflow' AND record_date >= ${dateFilter}`);
  const cashOutflows = await q(`SELECT COALESCE(SUM(amount), 0) as total FROM cash_flow WHERE flow_type='outflow' AND record_date >= ${dateFilter}`);
