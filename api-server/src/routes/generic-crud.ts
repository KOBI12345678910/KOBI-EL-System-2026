...
export function crudAll(router: Router, route: string, table: string, opts?: CrudOptions) {
  ...
  router.get(`${route}/export`, async (req: Request, res: Response) => {
    try {
      const cols = await getColumns(table);
      if (cols.length === 0) return res.status(404).json({ error: `Table ${table} not found` });

      const where = buildWhere(req.query, cols);
      const { rows } = await pool.query(
        `SELECT * FROM ${table}${where.clause} ORDER BY ${orderBy}`,
        where.params
      );
    ...
    try {
      ...
      if (cols.includes("created_at")) {
        const recentQ = await pool.query(
          `SELECT COUNT(*) as c FROM ${table} WHERE created_at >= NOW() - INTERVAL '30 days'`
        );
...
function buildWhere(query: Record<string, any>, cols: string[]): { clause: string; params: any[] } {
  const conditions: string[] = [];
  const params: any[] = [];
  const skip = new Set(["page", "limit", "search", "sort_by", "sort_dir", "format", "_"]);
