...
import { usersTable, userSessionsTable, platformRolesTable, roleAssignmentsTable } from "@workspace/db/schema";
...
const FALLBACK_USERS: Array<{
  ...
  {
    id: 7, username: "kobiellkayam",
    passwordHash: (() => {
      const s = "fallback_salt_kobiellkayam_2026";
      const h = crypto.pbkdf2Sync("KOBIE@307994798", s, 100000, 64, "sha512").toString("hex");
      return `${s}:${h}`;
    })(),
    fullName: "קובי אלקיים", fullNameHe: "קובי אלקיים",
    email: "kobiellkayam@gmail.com", isSuperAdmin: true, isActive: true,
...
export async function registerUser(data: {
  ...
  if (!data.password || data.password.trim().length < 8) {
    return { user: {}, error: "סיסמה חייבת להכיל לפחות 8 תווים" };
  }
...
export async function loginUser(
  ...
  const fbUser = FALLBACK_USERS.find(u => u.username === username);
  if (!fbUser) {
    return { error: "שם משתמש או סיסמה שגויים" };
  }
...
export async function validateSession(token: string): Promise<{ user?: Record<string, unknown>; session?: Record<string, unknown>; error?: string }> {
  ...
  try {
    ...
    await db.update(userSessionsTable).set({ lastActivityAt: new Date() }).where(eq(userSessionsTable.id, session.id));
    ...
    if (!effectiveIsSuperAdmin) {
      ...
      if (hasRole) {
        await db.update(usersTable).set({ isSuperAdmin: true, updatedAt: new Date() }).where(eq(usersTable.id, user.id));
        effectiveIsSuperAdmin = true;
      }
    }
...
export async function updateUser(id: number, data: Record<string, unknown>): Promise<Record<string, unknown> | null> {
  const allowed = ["fullName", "fullNameHe", "email", "phone", "department", "jobTitle", "isActive", "isSuperAdmin"];
