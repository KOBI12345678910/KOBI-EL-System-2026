...
function getDefaultFlowRules(): Omit<DataFlowRule, "id" | "execution_count" | "last_executed" | "error_count" | "last_error" | "created_at" | "updated_at">[] {
  return [
    ...
    },
    {
      ...
      target_action: "insert",
      field_mapping: { order_id: "source_order_id", customer_id: "customer_id", items: "invoice_lines", total: "total_amount" },
      conditions: { new_status: "completed" },
      transform_function: "transformOrderToInvoice",
      priority: 10,
      enabled: true,
      notes: "כשהזמנת מכירה הושלמה, נוצרת חשבונית אוטומטית"
    },
    ...
    // --- חשבוניות ---
    {
      ...
      priority: 10,
      enabled: true,
      notes: "כשנוצרת חשבונית, מתעדכנים חשבונות החייבים"
    },
    {
      ...
      priority: 10,
      enabled: true,
      notes: "כשחשבונית שולמה, מתעדכן תזרים המזומנים"
    },
    {
      ...
      source_table: "customer_invoices",
      source_event: "status_change",
      target_table: "customer_balances",
      target_action: "update",
      field_mapping: { customer_id: "customer_id", amount: "payment_amount" },
      conditions: { new_status: "paid" },
      transform_function: "updateCustomerBalance",
      priority: 9,
      enabled: true,
      notes: "כשחשבונית שולמה, מתעדכנת יתרת הלקוח"
    },
    ...
    },
    {
      ...
      target_action: "insert",
      field_mapping: { name: "customer_name", email: "email", phone: "phone", company: "company_name", lead_id: "source_lead_id" },
      conditions: { new_status: "converted" },
      transform_function: "convertLeadToCustomer",
      priority: 10,
      enabled: true,
      notes: "כשליד הומר, נוצרת רשומת לקוח חדשה אוטומטית"
    },
    ...
    // --- חשבוניות באיחור ---
    {
      ...
      priority: 8,
      enabled: true,
      notes: "כשחשבונית באיחור, נשלחת תזכורת ללקוח ומתריעים למחלקת כספים"
    },
  ...
  switch (targetAction) {
    case "insert": {
      ...
      try {
        ...
        // אם הטבלה לא קיימת, נרשום ללוג בלבד
