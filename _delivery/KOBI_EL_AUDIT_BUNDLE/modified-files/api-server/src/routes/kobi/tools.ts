...
async function apiTest(input: any): Promise<{ result: string }> {
  ...
  const url = `http://localhost:8080${path.startsWith("/") ? path : `/${path}`}`;
    ...
    case "order_status": {
      if (order_id) {
        const r = await pool.query(`
          ...
          WHERE so.id = $1
        `, [order_id]);
        if (r.rows.length === 0) return { result: `הזמנה ${order_id} לא נמצאה` };
        const o = r.rows[0];
...
async function workflowTrigger(input: { action: string; workflow_type?: string; entity_id?: number; data?: any }): Promise<{ result: string }> {
  ...
  switch (action) {
    case "order_to_invoice": {
      ...
      const order = await pool.query("SELECT * FROM sales_orders WHERE id = $1", [entity_id]);
    ...
    case "xlsx": {
      const ws = XLSX.utils.json_to_sheet(rows.map(r => {
        ...
        for (const h of headers) {
          const v = r[h];
          if (h.endsWith("_cents") && typeof v === "number") {
            clean[h] = v / 100;
          } else {
            clean[h] = v;
          }
