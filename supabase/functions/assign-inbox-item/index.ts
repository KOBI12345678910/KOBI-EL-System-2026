...
serve(async (req: Request) => {
  ...
  try {
    ...
    const assignedToUserId = Number(body.assigned_to_user_id);
    ...
    const { data, error } = await admin.schema("orchestration").from("universal_inbox")
      .update({ assigned_to_user_id: assignedToUserId, state: "assigned", updated_at: new Date().toISOString() })
      .eq("id", inboxId).select("*").single();
    if (error || !data) throw error ?? new Error("INBOX_ASSIGN_FAILED");
