...
## Phase 7 — Workforce Reconciliation

Batch: `B-BATCH-WORKFORCE-RECONCILE-01` — 2026-04-18

Scope: **backend reconciliation only**. The 53 existing pages under `erp-app/src/pages/hr/` remain the canonical UI surface; only 2 gap-fill pages under `erp-app/src/pages/workforce/` are added to close the registry completion gate (PayrollRun360, WageSlipsArchivePage). No duplicate `workforce/*List*Page.tsx` created. Employee360 left untouched.

| ID | Date | Phase | Type | Description | Evidence |
|---|---|---|---|---|---|
| B-WF001 | 2026-04-18 | 7 | schema | Migration 00053 — Workforce domain complete: ALTER 17 existing workforce.* tables with canonical audit columns (is_deleted, is_active, record_code, notes, metadata, created_by, updated_by); CREATE 2 missing tables (attendance_exceptions, benefits); CHECK constraints for 4 status lifecycles (payroll_run: draft/in_progress/approved/paid/closed; leave_request: submitted/approved/rejected/cancelled; wage_slip: draft/published/superseded; attendance_exception: pending/approved/rejected); indexes on employee_id/payroll_run_id/period/status hot paths; audit triggers on 8 PII-sensitive tables (employees, wage_slips, pension_records, payroll_runs, attendance_exceptions, benefits, leave_requests, payroll_entries); RLS enabled on 16 sensitive tables; PII markers via COMMENT ON COLUMN for national_id + bank_account_reference; seed 7 baseline pay_components + 7 leave_types. Idempotent. | `supabase/migrations/00053_workforce_domain_complete.sql` |
| B-WF002 | 2026-04-18 | 7 | schema | Migration 00054 — Workforce menu wiring (net-new only): 7 idempotent entries under "כוח אדם" for the newly-built surfaces (payroll-runs → PayrollRun360, wage-slips-archive, attendance-exceptions, payroll-exceptions, payroll-export-batches, pay-components, benefits-admin). Legacy 53 hr/* menu entries untouched. | `supabase/migrations/00054_workforce_menu_wiring.sql` |
| B-WF003 | 2026-04-18 | 7 | code | Zod schemas — 14 files + `_shared.ts` + `index.ts` barrel under `lib-client/api-zod/src/workforce/` (employees, employee-profiles, attendance, leave-requests, leave-types, payroll-runs, payroll-line-items, payroll-exceptions, wage-slips, pay-components, benefits, pension-records, shifts, payroll-export-batches). Hebrew-locale validators for PII (`HebrewNationalIdSchema`, `HebrewBankAccountSchema`, `HebrewPhoneSchema`, `HebrewEmailSchema`). Canonical status enums per CHECK constraint. Exported via new `./workforce` subpath in `package.json`. | `lib-client/api-zod/src/workforce/`, `lib-client/api-zod/package.json` |
| B-WF004 | 2026-04-18 | 7 | code | API route files — 15 files (`_helpers.ts`, 14 routers, `index.ts`) under `api-server/src/routes/workforce/`. Every router installs `authMiddleware` + `hrOrAdminMiddleware`; sensitive business endpoints use stricter role guards (`hrManagerMiddleware` on approve/close, `financeManagerMiddleware` on pay/export). Uses `_safe-list-helpers` — zero `sql.raw` with user input. `logAudit` on every state change. Business endpoints: `POST /payroll-runs/:id/{calculate,approve,pay,close}`, `POST /leave-requests/:id/{approve,reject}`, `POST /attendance/exceptions/:id/{approve,reject}`, `POST /wage-slips/:id/publish` (best-effort PDF via pdf-generator-engine), `POST /payroll-exceptions/:id/resolve`, `POST /payroll-export-batches` (finance manager), `GET /employees/:id/wage-slips-history` (self-service). Mounted at `/api/workforce` in `routes/index.ts`. | `api-server/src/routes/workforce/{*,index}.ts`, `api-server/src/routes/index.ts` |
