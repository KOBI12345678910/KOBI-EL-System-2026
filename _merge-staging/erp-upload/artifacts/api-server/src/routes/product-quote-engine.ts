...
router.get("/products/:id/costing", async (req: Request, res: Response) => {
  try {
    ...
    const overheadPercent = 0.15; // תקורה 15%
    const overhead = (totalMaterialCost + laborCost) * overheadPercent;
    const profitMarginPercent = 0.25; // רווח 25%
    const totalCost = totalMaterialCost + laborCost + overhead;
    const suggestedPrice = totalCost * (1 + profitMarginPercent);
...
router.post("/ai-quote-optimize/:quoteId", async (req: Request, res: Response) => {
  try {
    ...
    const vatRate = 0.17;
    const optimizedGrandTotal = optimizedBeforeVat * (1 + vatRate);
