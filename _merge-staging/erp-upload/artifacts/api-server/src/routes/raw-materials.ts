...
import { eq, ilike, or, desc, sql, and, lt, gt } from "drizzle-orm";
...
router.get("/raw-materials", async (req, res) => {
  try {
    const { search, category, subCategory, materialType, finish: finishFilter } = req.query;
  ...
  try {
    const result = await db.execute(
      sql`SELECT id, material_number, material_name, category, unit,
              ...
              END as stock_status
          FROM raw_materials
          WHERE (COALESCE(minimum_stock,0) > 0 OR COALESCE(reorder_point,0) > 0 OR COALESCE(safety_stock,0) > 0)
...
router.get("/raw-materials/:id", async (req, res) => {
  try {
    ...
    const [material] = await db.select().from(rawMaterialsTable).where(eq(rawMaterialsTable.id, id));
...
router.delete("/raw-materials/:id", async (req, res) => {
  try {
    const id = z.coerce.number().int().positive().parse(req.params.id);
    const [deleted] = await db.delete(rawMaterialsTable).where(eq(rawMaterialsTable.id, id)).returning();
