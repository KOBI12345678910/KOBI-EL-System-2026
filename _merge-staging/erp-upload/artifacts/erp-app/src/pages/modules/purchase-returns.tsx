...
import {
  ...
  CreditCard, ArrowLeftRight, Hash, ClipboardList, ShieldCheck, RefreshCw
} from "lucide-react";
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex gap-1 bg-gray-800/50 rounded-xl p-1 border border-gray-700/50">
            {tabs.map(t => (
              <button key={t.key} onClick={() => setViewMode(t.key)}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === t.key ? "bg-orange-600 text-white shadow-lg" : "text-muted-foreground hover:text-white hover:bg-gray-700/50"}`}>
          ...
          <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש לפי ספק, מספר החזרה, סיבה..."
              className="w-full bg-gray-800/60 border border-gray-700 rounded-lg pr-10 pl-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-orange-500/50 focus:outline-none" />
          </div>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            className="bg-gray-800/60 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-gray-300 focus:border-orange-500/50 focus:outline-none">
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4 sm:space-y-6">
              ...
              <div className="bg-gray-800/40 border border-gray-700/50 rounded-xl p-5">
                ...
                <div className="flex items-center justify-between overflow-x-auto pb-2">
                  {[
                    { step: "טיוטה", icon: FileText, color: "bg-gray-600" },
                    ...
                    return (
                      <div key={s.step} className="flex items-center flex-shrink-0">
                        ...
                        {i < arr.length - 1 && <div className="w-8 h-0.5 bg-gray-600 mx-2 mt-[-20px]" />}
                      </div>
                    );
                  })}
                </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filtered.slice(0, 15).map(ret => (
                        <tr key={ret.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                          ...
                          <td className="py-3 px-2 text-center">
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={() => setSelectedReturn(ret)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Eye size={14} className="text-muted-foreground" /></button>
                              <button onClick={() => openForm(ret)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Edit2 size={14} className="text-blue-400" /></button>
                              <button onClick={async () => { if (globalConfirm("למחוק החזרה זו?")) deleteMutation.mutate(ret.id); }} className="p-1.5 hover:bg-gray-700 rounded-lg"><Trash2 size={14} className="text-red-400" /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          {viewMode === "list" && (
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              <BulkActions bulk={bulk} actions={defaultBulkActions} entityName="החזרות" />
              {filtered.map(ret => {
                const Icon = reasonIcon(ret.reasonCategory);
                return (
                  <motion.div key={ret.id} layout className={`bg-gray-800/40 border border-gray-700/50 rounded-xl p-4 hover:border-orange-500/30 transition-all ${bulk.isSelected(ret.id) ? "ring-1 ring-primary/50 bg-primary/5" : ""}`}>
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div onClick={e => e.stopPropagation()}><BulkCheckbox bulk={bulk} id={ret.id} /></div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex gap-1">
                          <button onClick={() => setSelectedReturn(ret)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Eye size={15} className="text-muted-foreground" /></button>
                          <button onClick={() => openForm(ret)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Edit2 size={15} className="text-blue-400" /></button>
                          <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMutation.mutate(ret.id); }} className="p-1.5 hover:bg-gray-700 rounded-lg"><Trash2 size={15} className="text-red-400" /></button>
                        </div>
                      </div>
                    </div>
                  </motion.div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {returns.filter(r => r.creditNoteNumber || r.replacementRequested || r.status === "זוכה").map(ret => (
                      <tr key={ret.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => setSelectedReturn(null)} className="p-2 hover:bg-gray-800 rounded-lg"><X size={20} /></button>
                </div>

                <div className="flex border-b border-gray-700 mb-6">
                  {[{key:"details",label:"פרטים"},{key:"related",label:"רשומות קשורות"},{key:"docs",label:"מסמכים"},{key:"history",label:"היסטוריה"}].map(t => (
                    <button key={t.key} onClick={() => setDetailTab(t.key)} className={`px-4 py-2.5 text-sm font-medium border-b-2 ${detailTab === t.key ? "border-orange-500 text-orange-400" : "border-transparent text-muted-foreground hover:text-white"}`}>{t.label}</button>
                  ))}
                </div>

                {detailTab === "details" && (<>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6 text-sm">
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">ספק:</span> <span className="font-medium mr-2">{supplierMap[selectedReturn.supplierId]?.supplierName || `#${selectedReturn.supplierId}`}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">תאריך:</span> <span className="font-medium mr-2">{selectedReturn.returnDate}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">סטטוס:</span> <span className={`mr-2 px-2 py-0.5 rounded-full text-xs border ${statusColor(selectedReturn.status)}`}>{selectedReturn.status}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">סיבה:</span> <span className="font-medium mr-2">{selectedReturn.reasonCategory}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">החזיר:</span> <span className="font-medium mr-2">{selectedReturn.returnedBy || "—"}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">אישר:</span> <span className="font-medium mr-2">{selectedReturn.approvedBy || "—"}</span></div>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      ...
                      <select value={form.supplierId} onChange={e => setForm({ ...form, supplierId: e.target.value })}
                        className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-orange-500/50 focus:outline-none">
                  ...
                  <div>
                    ...
                    <textarea rows={2} value={form.reasonDetails} onChange={e => setForm({ ...form, reasonDetails: e.target.value })}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-orange-500/50 focus:outline-none resize-none" />
                  </div>
                        ...
                        <div key={i} className="bg-gray-900/60 rounded-lg p-3 border border-gray-700/30">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                            <input value={item.itemName} onChange={e => updateItem(i, "itemName", e.target.value)} placeholder="שם פריט *"
                              className="bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-xs focus:border-orange-500/50 focus:outline-none" />
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <label className="flex items-center gap-2 text-xs text-gray-300">
                          <input type="checkbox" checked={form.creditNoteReceived} onChange={e => setForm({ ...form, creditNoteReceived: e.target.checked })}
                            className="rounded border-gray-600" /> זיכוי התקבל
                        </label>
                      </div>
                    </div>
                    <div className="bg-gray-800/40 rounded-xl p-4 border border-gray-700/50">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <label className="flex items-center gap-2 text-xs text-gray-300">
                          <input type="checkbox" checked={form.replacementRequested} onChange={e => setForm({ ...form, replacementRequested: e.target.checked })}
                            className="rounded border-gray-600" /> בקשת החלפה
                        </label>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    ...
                    <button onClick={handleSubmit} disabled={saveMutation.isPending || !form.supplierId}
                      className="flex items-center gap-2 px-6 py-2.5 bg-orange-600 hover:bg-orange-500 disabled:bg-gray-700 disabled:text-muted-foreground rounded-lg font-medium transition-colors">
