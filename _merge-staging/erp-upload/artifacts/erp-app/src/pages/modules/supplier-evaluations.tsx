...
import {
  ...
  AlertTriangle, Eye, Calendar, Target, Zap, ThumbsUp, ThumbsDown, Minus
} from "lucide-react";
        ...
        <div className="flex gap-1 bg-gray-800/50 rounded-xl p-1 border border-gray-700/50 w-fit">
          {tabs.map(t => (
            <button key={t.key} onClick={() => setViewMode(t.key)}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === t.key ? "bg-amber-600 text-white shadow-lg" : "text-muted-foreground hover:text-white hover:bg-gray-700/50"}`}>
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש ספק או מעריך..."
              className="w-full bg-gray-800/60 border border-gray-700 rounded-lg pr-10 pl-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-amber-500/50 focus:outline-none" />
          </div>
          <select value={recFilter} onChange={e => setRecFilter(e.target.value)}
            className="bg-gray-800/60 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-gray-300 focus:border-amber-500/50 focus:outline-none">
                ...
                <div className="bg-gray-800/40 border border-gray-700/50 rounded-xl p-5">
                  ...
                  <div className="space-y-3">
                    {latestBySupplier.sort((a, b) => parseFloat(b.overallScore) - parseFloat(a.overallScore)).slice(0, 5).map((ev, i) => {
                      ...
                      return (
                        <div key={ev.id} className="flex items-center gap-3 p-3 bg-gray-900/50 rounded-lg border border-gray-700/30">
                          <span className={`w-7 h-7 flex items-center justify-center rounded-full text-xs font-bold ${i === 0 ? "bg-amber-500 text-foreground" : i === 1 ? "bg-gray-300 text-foreground" : i === 2 ? "bg-amber-700 text-white" : "bg-gray-700 text-gray-300"}`}>{i + 1}</span>
              ...
              <div className="bg-gray-800/40 border border-gray-700/50 rounded-xl p-5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  {RECOMMENDATIONS.map(rec => {
                    ...
                    return (
                      <div key={rec} className={`${colors[rec] || "bg-gray-700/30 border-gray-600/30 text-gray-300"} border rounded-xl p-4 text-center`}>
                          ...
                          <tr key={ev.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                            ...
                            <td className="py-3 px-2 text-center">
                              <div className="flex items-center justify-center gap-1">
                                <button onClick={() => setSelectedEval(ev)} className="p-1.5 hover:bg-gray-700 rounded-lg transition-colors"><Eye size={14} className="text-muted-foreground" /></button>
                                <button onClick={() => openForm(ev)} className="p-1.5 hover:bg-gray-700 rounded-lg transition-colors"><Edit2 size={14} className="text-blue-400" /></button>
                                <button onClick={async () => { if (globalConfirm("למחוק הערכה זו?")) deleteMutation.mutate(ev.id); }} className="p-1.5 hover:bg-gray-700 rounded-lg transition-colors"><Trash2 size={14} className="text-red-400" /></button>
                              </div>
                            </td>
                          </tr>
                        );
          ...
          {viewMode === "list" && (
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              ...
              {filtered.map(ev => {
                const os = parseFloat(ev.overallScore);
                return (
                  <motion.div key={ev.id} layout className={`bg-gray-800/40 border border-gray-700/50 rounded-xl p-4 hover:border-amber-500/30 transition-all ${bulk.isSelected(ev.id) ? "ring-1 ring-primary/50 bg-primary/5" : ""}`}>
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs border ${ev.recommendation === "מומלץ" ? "bg-green-500/20 border-green-500/30 text-green-400" : ev.recommendation === "להפסקה" ? "bg-red-500/20 border-red-500/30 text-red-400" : "bg-gray-700/50 border-gray-600/30 text-gray-300"}`}>{ev.recommendation}</span>
                        <div className="flex gap-1">
                          <button onClick={() => setSelectedEval(ev)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Eye size={15} className="text-muted-foreground" /></button>
                          <button onClick={() => openForm(ev)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Edit2 size={15} className="text-blue-400" /></button>
                          <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMutation.mutate(ev.id); }} className="p-1.5 hover:bg-gray-700 rounded-lg"><Trash2 size={15} className="text-red-400" /></button>
                        </div>
                      </div>
                    </div>
                  </motion.div>
              ...
              {latestBySupplier.length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-700/50">
                        <th className="text-right py-3 px-3 text-muted-foreground font-medium sticky right-0 bg-gray-900/80 backdrop-blur-sm z-10">ספק</th>
                        {SCORE_CATEGORIES.map(cat => (
                      ...
                      {latestBySupplier.sort((a, b) => parseFloat(b.overallScore) - parseFloat(a.overallScore)).map(ev => {
                        ...
                        return (
                          <tr key={ev.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                            <td className="py-3 px-3 font-medium sticky right-0 bg-gray-900/80 backdrop-blur-sm z-10">{supplierMap[ev.supplierId]?.supplierName || `ספק #${ev.supplierId}`}</td>
                            {SCORE_CATEGORIES.map(cat => {
                              const v = parseFloat((ev as any)[cat.key]);
          ...
          {viewMode === "history" && (
            <motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
              ...
              {Object.entries(supplierHistory).map(([sid, evals]) => {
                ...
                return (
                  <div key={sid} className="bg-gray-800/40 border border-gray-700/50 rounded-xl overflow-hidden">
                    <button onClick={() => setExpandedSupplier(isExpanded ? null : supplierId)}
                      className="w-full flex items-center justify-between p-4 hover:bg-gray-800/60 transition-colors text-right">
                      <div className="flex items-center gap-3">
                          ...
                          <div className="px-4 pb-4 space-y-3">
                            ...
                            <div className="flex gap-2 overflow-x-auto pb-2">
                              {evals.map((ev, i) => {
                                ...
                                return (
                                  <div key={ev.id} className="flex-shrink-0 bg-gray-900/60 border border-gray-700/30 rounded-lg p-3 min-w-[120px] text-center">
          ...
          {selectedEval && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setSelectedEval(null)}>
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                className="bg-gray-900 border border-gray-700 rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => setSelectedEval(null)} className="p-2 hover:bg-gray-800 rounded-lg"><X size={20} /></button>
                </div>

                <div className="flex border-b border-gray-700 mb-6">
                  {[{key:"details",label:"פרטים"},{key:"related",label:"רשומות קשורות"},{key:"docs",label:"מסמכים"},{key:"history",label:"היסטוריה"}].map(t => (
                    <button key={t.key} onClick={() => setDetailTab(t.key)} className={`px-4 py-2.5 text-sm font-medium border-b-2 ${detailTab === t.key ? "border-amber-500 text-amber-400" : "border-transparent text-muted-foreground hover:text-white"}`}>{t.label}</button>
                  ))}
                </div>

                {detailTab === "details" && (<>
                <div className="grid grid-cols-2 gap-3 mb-6 text-sm">
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">תאריך הערכה:</span> <span className="font-medium mr-2">{selectedEval.evaluationDate}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">מעריך:</span> <span className="font-medium mr-2">{selectedEval.evaluator || "—"}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">תקופה:</span> <span className="font-medium mr-2">{selectedEval.periodStart || "—"} — {selectedEval.periodEnd || "—"}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">המלצה:</span> <span className="font-medium mr-2">{selectedEval.recommendation}</span></div>
                </div>
                ...
                <div className="space-y-4 mb-6">
                  {SCORE_CATEGORIES.map(cat => {
                    ...
                    return (
                      <div key={cat.key} className="bg-gray-800/40 rounded-lg p-3">
        ...
        <AnimatePresence>
          {showForm && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                className="bg-gray-900 border border-gray-700 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-6">
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      ...
                      <select value={form.supplierId} onChange={e => setForm({ ...form, supplierId: e.target.value })}
                        className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-amber-500/50 focus:outline-none">
                      ...
                      {SCORE_CATEGORIES.map(cat => (
                        <div key={cat.key} className="flex items-center gap-3">
                          ...
                          <input value={form[cat.notesKey] || ""} onChange={e => setForm({ ...form, [cat.notesKey]: e.target.value })} placeholder={`הערות ${cat.label}...`}
                            className="flex-1 bg-gray-900/60 border border-gray-700/50 rounded-lg px-3 py-1.5 text-xs focus:border-amber-500/50 focus:outline-none" />
                        </div>
                      ))}
                    </div>
                  </div>
                      ...
                      {[
                        ...
                        <div key={f.key}>
                          ...
                          <input type={f.type} value={form[f.key] || ""} onChange={e => setForm({ ...form, [f.key]: e.target.value })}
                            className="w-full bg-gray-900/60 border border-gray-700/50 rounded-lg px-3 py-2 text-sm focus:border-amber-500/50 focus:outline-none" />
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea rows={3} value={form.generalNotes} onChange={e => setForm({ ...form, generalNotes: e.target.value })}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-amber-500/50 focus:outline-none resize-none" />
                  </div>

                  {/* Submit */}
                  <div className="flex justify-end gap-3 pt-2">
                    <button onClick={() => setShowForm(false)} className="px-5 py-2.5 border border-gray-600 rounded-lg text-gray-300 hover:bg-gray-800 transition-colors">ביטול</button>
                    <button onClick={handleSubmit} disabled={saveMutation.isPending || !form.supplierId}
                      className="flex items-center gap-2 px-6 py-2.5 bg-amber-600 hover:bg-amber-500 disabled:bg-gray-700 disabled:text-muted-foreground rounded-lg font-medium transition-colors">
