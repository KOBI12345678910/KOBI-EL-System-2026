...
import {
  ...
  Scale, TrendingUp, Percent, Hash, Users, ThumbsUp
} from "lucide-react";
      ...
      <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
          ...
          <div className="flex items-center gap-2">
            {comparisonGroups.length > 0 && (
              <select onChange={e => { if (e.target.value) openComparison(e.target.value); e.target.value = ""; }}
                className="px-3 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white focus:border-amber-500 focus:outline-none text-sm">
            ...
            <button onClick={() => { setForm({...emptyForm, quoteDate: new Date().toISOString().split("T")[0]}); setEditingId(null); setQuoteItems([]); setShowForm(true); }}
              className="flex items-center gap-2 px-5 py-2.5 bg-amber-600 hover:bg-amber-500 text-white rounded-xl font-medium transition-colors">
        ...
        {statusDistribution.length > 0 && (
          <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-4">
            ...
            <div className="flex items-center gap-1 h-6 rounded-full overflow-hidden bg-[#12141a]">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-0 sm:min-w-[200px]">
            ...
            <input type="text" placeholder="חיפוש לפי מספר הצעה או ספק..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pr-10 pl-4 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:border-amber-500 focus:outline-none" />
          </div>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            className="px-3 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white focus:border-amber-500 focus:outline-none">
            ...
            <table className="w-full text-right">
              ...
              <tbody>
                {filtered.map(q => {
                  ...
                  return (
                    <>
                      <tr key={q.id} className={`border-b border-gray-800/50 hover:bg-[#1e2128] cursor-pointer transition-colors ${isSelected(q.id) ? "bg-amber-500/10" : expired ? "bg-red-500/5" : q.isRecommended ? "bg-emerald-500/5" : ""}`}
                        ...
                        <td className="px-4 py-3 text-white text-sm">{getSupplierName(q.supplierId)}</td>
                        ...
                        <td className="px-4 py-3 text-white font-mono text-sm font-medium" dir="ltr">
                        ...
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => openEdit(q)} className="p-1.5 text-muted-foreground hover:text-blue-400 rounded-md"><Edit2 className="w-4 h-4" /></button>
                            ...
                            <button onClick={() => setDeleteConfirm(q.id)} className="p-1.5 text-muted-foreground hover:text-red-400 rounded-md"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                      {expandedRow === q.id && (
                              ...
                              <table className="w-full text-right text-sm">
                                ...
                                <tbody>{expandedItems[q.id].map(item => (
                                  <tr key={item.id} className="border-t border-gray-800/30">
                                    ...
                                    <td className="py-2 pr-2 text-white font-medium" dir="ltr">{cs(q.currency)}{parseFloat(item.totalPrice).toLocaleString()}</td>
                                  </tr>
                                ))}</tbody>
                              </table>
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => setSelectedQuote(null)} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
                </div>
              </div>
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">תאריך הצעה</p><p className="text-white font-medium mt-1">{selectedQuote.quoteDate ? new Date(selectedQuote.quoteDate).toLocaleDateString("he-IL") : "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">תוקף</p><p className="text-white font-medium mt-1">{selectedQuote.validityDate ? new Date(selectedQuote.validityDate).toLocaleDateString("he-IL") : "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">תנאי תשלום</p><p className="text-white font-medium mt-1">{selectedQuote.paymentTerms || "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">ימי אספקה</p><p className="text-white font-medium mt-1">{selectedQuote.deliveryDays ? `${selectedQuote.deliveryDays} ימים` : "—"}</p></div>
                </div>
                <div className="bg-gradient-to-br from-[#12141a] to-[#1a1d23] rounded-xl p-5 border border-gray-800">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
                    <div><p className="text-muted-foreground text-xs mb-1">לפני מע״מ</p><p className="text-white text-xl font-bold font-mono" dir="ltr">{cs(selectedQuote.currency)}{parseFloat(selectedQuote.totalBeforeTax || "0").toLocaleString()}</p></div>
                ...
                {selectedQuote.items && selectedQuote.items.length > 0 && (
                  <div>
                    <h3 className="text-lg font-semibold text-white mb-3">פריטים ({selectedQuote.items.length})</h3>
                    <table className="w-full text-right text-sm">
                      <thead><tr className="text-muted-foreground border-b border-gray-800">
                      ...
                      <tbody>{selectedQuote.items.map(item => (
                        <tr key={item.id} className="border-t border-gray-800/30">
                          ...
                          <td className="py-2 pr-2 text-muted-foreground">{item.discountPercent || "0"}%</td>
                          <td className="py-2 pr-2 text-muted-foreground">{item.taxPercent || "18"}%</td>
                          <td className="py-2 pr-2 text-white font-medium" dir="ltr">{cs(selectedQuote.currency)}{parseFloat(item.totalPrice).toLocaleString()}</td>
                        </tr>
                      ))}</tbody>
                    </table>
                  </div>
                ...
                {selectedQuote.notes && (
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs mb-1">הערות</p><p className="text-gray-300 text-sm">{selectedQuote.notes}</p></div>
                )}
                </>)}
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center pt-4 overflow-y-auto" onClick={closeForm}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-4xl mx-4 mb-10" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white">{editingId ? "עריכת הצעה" : "הצעת מחיר חדשה"}</h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div><label className="block text-xs text-muted-foreground mb-1">מספר הצעה *</label><input value={form.quoteNumber} onChange={e => setForm({...form, quoteNumber: e.target.value})} placeholder="QT-001" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-amber-500 focus:outline-none text-sm" /></div>
                  <div><label className="block text-xs text-muted-foreground mb-1">ספק *</label><select value={form.supplierId} onChange={e => setForm({...form, supplierId: e.target.value})} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-amber-500 focus:outline-none text-sm"><option value="">בחר ספק</option>{suppliers.map(s => <option key={s.id} value={s.id}>{s.supplierName} ({s.supplierNumber})</option>)}</select></div>
                  <div><label className="block text-xs text-muted-foreground mb-1">סטטוס</label><select value={form.status} onChange={e => setForm({...form, status: e.target.value})} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-amber-500 focus:outline-none text-sm">{STATUSES.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  <div><label className="block text-xs text-muted-foreground mb-1">תאריך הצעה</label><input type="date" value={form.quoteDate} onChange={e => setForm({...form, quoteDate: e.target.value})} dir="ltr" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-amber-500 focus:outline-none text-sm" /></div>
                  <div><label className="block text-xs text-muted-foreground mb-1">תוקף הצעה</label><input type="date" value={form.validityDate} onChange={e => setForm({...form, validityDate: e.target.value})} dir="ltr" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-amber-500 focus:outline-none text-sm" /></div>
                  <div><label className="block text-xs text-muted-foreground mb-1">תנאי תשלום</label><select value={form.paymentTerms} onChange={e => setForm({...form, paymentTerms: e.target.value})} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-amber-500 focus:outline-none text-sm"><option value="">בחר</option><option value="מיידי">מיידי</option><option value="שוטף+30">שוטף+30</option><option value="שוטף+60">שוטף+60</option><option value="שוטף+90">שוטף+90</option></select></div>
                  <div><label className="block text-xs text-muted-foreground mb-1">ימי אספקה</label><input type="number" value={form.deliveryDays} onChange={e => setForm({...form, deliveryDays: e.target.value})} placeholder="0" dir="ltr" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-amber-500 focus:outline-none text-sm" /></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div><label className="block text-xs text-muted-foreground mb-1">נוצר ע״י</label><input value={form.createdBy} onChange={e => setForm({...form, createdBy: e.target.value})} placeholder="שם" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-amber-500 focus:outline-none text-sm" /></div>
                </div>

                {!editingId && (
                  <div className="border border-gray-700 rounded-xl p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-semibold text-white flex items-center gap-2"><Package className="w-5 h-5 text-amber-400" />פריטים</h3>
                      ...
                      return (
                        <div key={idx} className="bg-[#12141a] border border-gray-800 rounded-lg p-3 space-y-3">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <div><label className="block text-xs text-muted-foreground mb-1">חומר</label><select value={item.materialId} onChange={e => updateItem(idx, "materialId", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-amber-500 focus:outline-none"><option value="">בחר</option>{materials.map(m => <option key={m.id} value={m.id}>{m.materialName}</option>)}</select></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">קוד</label><input value={item.itemCode} onChange={e => updateItem(idx, "itemCode", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm font-mono focus:border-amber-500 focus:outline-none" /></div>
                            <div className="sm:col-span-2"><label className="block text-xs text-muted-foreground mb-1">תיאור *</label><input value={item.itemDescription} onChange={e => updateItem(idx, "itemDescription", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-amber-500 focus:outline-none" /></div>
                          </div>
                          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                            <div><label className="block text-xs text-muted-foreground mb-1">כמות</label><input type="number" value={item.quantity} onChange={e => updateItem(idx, "quantity", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-amber-500 focus:outline-none" dir="ltr" /></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">יחידה</label><select value={item.unit} onChange={e => updateItem(idx, "unit", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-amber-500 focus:outline-none">{UNITS.map(u => <option key={u} value={u}>{u}</option>)}</select></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">מחיר</label><input type="number" step="0.01" value={item.unitPrice} onChange={e => updateItem(idx, "unitPrice", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-amber-500 focus:outline-none" dir="ltr" /></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">הנחה%</label><input type="number" step="0.1" value={item.discountPercent} onChange={e => updateItem(idx, "discountPercent", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-amber-500 focus:outline-none" dir="ltr" /></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">מע״מ%</label><input type="number" step="0.1" value={item.taxPercent} onChange={e => updateItem(idx, "taxPercent", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-amber-500 focus:outline-none" dir="ltr" /></div>
                            <div className="flex items-end"><p className="text-white font-mono text-sm font-bold pb-1.5" dir="ltr">₪{lc.total.toLocaleString()}</p></div>
                          </div>
                        </div>
                      );
                    })}
                    {quoteItems.length > 0 && (
                      <div className="bg-gradient-to-br from-[#12141a] to-[#1a1d23] rounded-lg p-4 border border-gray-700">
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
                          <div><p className="text-muted-foreground text-xs mb-1">לפני מע״מ</p><p className="text-white font-bold font-mono" dir="ltr">₪{itemsTotals.beforeTax.toLocaleString()}</p></div>
                ...
                <div><label className="block text-xs text-muted-foreground mb-1">הערות</label><textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-amber-500 focus:outline-none resize-none text-sm" rows={2} /></div>
                ...
                <div className="flex items-center gap-3 pt-2">
                  <button type="submit" disabled={createMut.isPending || updateMut.isPending} className="flex items-center gap-2 px-6 py-2.5 bg-amber-600 hover:bg-amber-500 disabled:bg-amber-800 text-white rounded-lg font-medium"><Save className="w-4 h-4" />{editingId ? "עדכן" : "שמור"}</button>
        ...
        {deleteConfirm !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
            <div className="bg-[#1a1d23] border border-gray-700 rounded-xl p-6 max-w-sm mx-4" dir="rtl">
              <h3 className="text-lg font-bold text-white mb-2">מחיקת הצעה</h3>
...
function ComparisonModal({ group, quotes, getSupplierName, getSupplierRating, onClose, onRecommend, loadItems }: {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
        className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-6xl mx-4 mb-10" onClick={e => e.stopPropagation()} dir="rtl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
          <div>
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
              ...
              <thead><tr className="border-b border-gray-700">
                ...
                {sorted.map(q => (
                  <th key={q.id} className="pb-3 px-3 text-center">
                    <div className={`rounded-xl p-3 ${q.isRecommended ? "bg-emerald-500/10 border border-emerald-500/30" : "bg-[#12141a]"}`}>
                      <p className="text-white font-bold">{getSupplierName(q.supplierId)}</p>
                  ...
                  {sorted.map(q => {
                    ...
                    return (
                      <td key={q.id} className="py-3 px-3 text-center">
                        <span className={`font-mono font-bold text-lg ${isCheapest ? "text-emerald-400" : "text-white"}`} dir="ltr">
                ...
                {loaded && allDescriptions.length > 0 && (
                  <>
                    ...
                    {allDescriptions.map(desc => (
                      <tr key={desc} className="border-b border-gray-800/30">
                  ...
                  {sorted.map(q => (
                    <td key={q.id} className="py-4 px-3 text-center">
                      <button onClick={() => onRecommend(q.id, !q.isRecommended)}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${q.isRecommended ? "bg-emerald-600 text-white" : "bg-gray-800 text-muted-foreground hover:bg-gray-700"}`}>
