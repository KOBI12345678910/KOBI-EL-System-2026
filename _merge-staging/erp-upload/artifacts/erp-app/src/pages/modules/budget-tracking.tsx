...
import {
  ...
  ArrowUpRight, ArrowDownRight, Building2, Hash
} from "lucide-react";
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <select value={yearFilter} onChange={e => setYearFilter(parseInt(e.target.value))}
              className="bg-gray-800/60 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-gray-300 focus:border-emerald-500/50 focus:outline-none">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex gap-1 bg-gray-800/50 rounded-xl p-1 border border-gray-700/50">
            {tabs.map(t => (
              <button key={t.key} onClick={() => setViewMode(t.key)}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === t.key ? "bg-emerald-600 text-white shadow-lg" : "text-muted-foreground hover:text-white hover:bg-gray-700/50"}`}>
          ...
          <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש תקציב..."
              className="w-full bg-gray-800/60 border border-gray-700 rounded-lg pr-10 pl-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-emerald-500/50 focus:outline-none" />
          </div>
                ...
                <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-5">
                  ...
                  <div className="space-y-2">
                    {alertBudgets.slice(0, 10).map(b => (
                      <div key={b.id} className="flex items-center justify-between bg-gray-900/60 rounded-lg p-3 border border-gray-700/30 cursor-pointer hover:border-red-500/30" onClick={() => setSelectedBudget(b)}>
                        <div className="flex items-center gap-3">
                          ...
                          <tr key={b.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                            ...
                            <td className="py-3 px-2 text-center">
                              <div className="flex items-center justify-center gap-1">
                                <button onClick={() => setSelectedBudget(b)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Eye size={14} className="text-muted-foreground" /></button>
                                <button onClick={() => openForm(b)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Edit2 size={14} className="text-blue-400" /></button>
                                <button onClick={async () => { if (globalConfirm("למחוק תקציב זה?")) deleteMutation.mutate(b.id); }} className="p-1.5 hover:bg-gray-700 rounded-lg"><Trash2 size={14} className="text-red-400" /></button>
                              </div>
                            </td>
                          </tr>
                        );
          ...
          {viewMode === "list" && (
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              ...
              {filtered.map(b => {
                ...
                const remaining = budgeted - actual - committed;
                return (
                  <motion.div key={b.id} layout className={`bg-gray-800/40 border border-gray-700/50 rounded-xl p-4 hover:border-emerald-500/30 transition-all ${bulk.isSelected(b.id) ? "ring-1 ring-primary/50 bg-primary/5" : ""}`}>
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-5">
                        ...
                        <div className="flex gap-1">
                          <button onClick={() => setSelectedBudget(b)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Eye size={15} className="text-muted-foreground" /></button>
                          <button onClick={() => openForm(b)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Edit2 size={15} className="text-blue-400" /></button>
                          <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMutation.mutate(b.id); }} className="p-1.5 hover:bg-gray-700 rounded-lg"><Trash2 size={15} className="text-red-400" /></button>
                        </div>
                      </div>
                    </div>
                  </motion.div>
            ...
            <motion.div key="departments" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {departmentSummary.map(dept => (
                  <div key={dept.name} className="bg-gray-800/40 border border-gray-700/50 rounded-xl p-5 hover:border-emerald-500/30 transition-all">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {alertBudgets.map(b => {
                      ...
                      return (
                        <tr key={b.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 cursor-pointer" onClick={() => setSelectedBudget(b)}>
                  ...
                  return (
                    <>
                      <div className="flex items-center justify-between mb-6">
                        ...
                        <button onClick={() => setSelectedBudget(null)} className="p-2 hover:bg-gray-800 rounded-lg"><X size={20} /></button>
                      </div>

                      <div className="flex border-b border-gray-700 mb-6">
                        {[{key:"details",label:"פרטים"},{key:"related",label:"רשומות קשורות"},{key:"docs",label:"מסמכים"},{key:"history",label:"היסטוריה"}].map(t => (
                          <button key={t.key} onClick={() => setDetailTab(t.key)} className={`px-4 py-2.5 text-sm font-medium border-b-2 ${detailTab === t.key ? "border-emerald-500 text-emerald-400" : "border-transparent text-muted-foreground hover:text-white"}`}>{t.label}</button>
                        ))}
                      </div>
                      ...
                      {/* Utilization Bar */}
                      <div className="mb-6">
                        <div className="relative h-6 bg-gray-700/50 rounded-full overflow-hidden mb-1">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4 text-sm">
                        <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">סטטוס:</span> <span className={`mr-2 px-2 py-0.5 rounded-full text-xs border ${statusColor(b.status)}`}>{b.status}</span></div>
                        <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">סוג:</span> <span className="font-medium mr-2">{b.budgetType}</span></div>
                        <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">קטגוריה:</span> <span className="font-medium mr-2">{b.category}</span></div>
                        <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">מחלקה:</span> <span className="font-medium mr-2">{b.department || "—"}</span></div>
                        <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">שנה:</span> <span className="font-medium mr-2">{b.fiscalYear}</span> {b.fiscalMonth && <span className="text-xs">({MONTHS[b.fiscalMonth - 1]})</span>}</div>
                        <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">עדיפות:</span> <span className={`font-medium mr-2 ${priorityColor(b.priority)}`}>{b.priority || "רגילה"}</span></div>
                      </div>
                      ...
                      {(b.approvedBy || b.responsiblePerson) && (
                        <div className="bg-gray-800/40 rounded-lg p-3 mb-4 text-sm">
        ...
        <AnimatePresence>
          {showForm && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                className="bg-gray-900 border border-gray-700 rounded-2xl max-w-3xl w-full max-h-[92vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-6">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className="md:col-span-2">
                      ...
                      <input value={form.budgetName} onChange={e => setForm({ ...form, budgetName: e.target.value })} placeholder="שם התקציב"
                        className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-emerald-500/50 focus:outline-none" />
                    </div>
                    <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div>
                        ...
                        <input type="number" value={form.budgetedAmount} onChange={e => setForm({ ...form, budgetedAmount: e.target.value })}
                          className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-emerald-500/50 focus:outline-none" />
                      </div>
                      <div>
                  ...
                  <div>
                    ...
                    <textarea rows={2} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-emerald-500/50 focus:outline-none resize-none" />
                  </div>
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    ...
                    <button onClick={handleSubmit} disabled={saveMutation.isPending || !form.budgetName || !form.budgetedAmount}
                      className="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-gray-700 disabled:text-muted-foreground rounded-lg font-medium transition-colors">
