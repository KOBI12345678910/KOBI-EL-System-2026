  ...
  return (
    <div className="space-y-6 p-6" dir="rtl">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={openAdd}
          className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white font-medium px-4 py-2 rounded-lg transition-colors text-sm"
        >
      ...
      <div className="relative max-w-sm">
        ...
        <input
          className="w-full bg-card border border-border rounded-lg py-2 pr-9 pl-3 text-sm text-white placeholder:text-muted-foreground focus:outline-none focus:border-primary/50"
            ...
            {filtered.map((dept) => {
              ...
              return (
                ...
                >
                      ...
                      {openMenu === dept.nameHe && (
                        <div className="absolute left-0 top-8 bg-popover border border-border rounded-lg shadow-xl z-20 min-w-[130px] py-1">
                          <button
                            ...
                            className="flex items-center gap-2 w-full px-3 py-2 text-sm hover:bg-muted text-white"
                          >
                  ...
                  <h3 className="font-semibold text-white text-base leading-tight">{dept.nameHe}</h3>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
                      ...
                      <span className="font-medium text-white">{dept.employeeCount}</span>
                      <span>עובדים</span>
                    </div>
                    <Link
            ...
            >
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-primary/50 resize-none"
              ...
              <div className="flex justify-end gap-2 p-5 border-t border-border">
                <button onClick={() => setShowForm(false)} className="px-4 py-2 text-sm text-muted-foreground hover:text-white transition-colors">
                  ביטול
                </button>
                <button
                  onClick={handleSave}
                  className="px-5 py-2 bg-green-600 hover:bg-green-500 text-white font-medium text-sm rounded-lg transition-colors"
                >
        ...
        {deleteConfirm && (
          ...
          >
            <motion.div
              ...
              exit={{ scale: 0.95 }}
            >
              <h2 className="text-lg font-semibold text-white mb-2">מחיקת מחלקה</h2>
              <p className="text-sm text-muted-foreground mb-5">
                האם אתה בטוח שברצונך למחוק את המחלקה <span className="text-white font-medium">"{deleteConfirm}"</span>?
              </p>
              <div className="flex justify-end gap-2">
                <button onClick={() => setDeleteConfirm(null)} className="px-4 py-2 text-sm text-muted-foreground hover:text-white">
                  ביטול
                </button>
                <button
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setViewDetail(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card rounded-2xl border border-border shadow-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center p-5 border-b border-border">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><Building2 className="w-5 h-5 text-primary" /> {viewDetail.nameHe}</h2>
              ...
              {detailTab === "details" && <div className="p-5 space-y-3">
                <div className="flex justify-between py-1.5 border-b border-border/20"><span className="text-sm text-muted-foreground">שם בעברית</span><span className="text-sm text-white font-medium">{viewDetail.nameHe}</span></div>
                <div className="flex justify-between py-1.5 border-b border-border/20"><span className="text-sm text-muted-foreground">שם באנגלית</span><span className="text-sm text-white font-medium">{viewDetail.nameEn}</span></div>
                <div className="flex justify-between py-1.5 border-b border-border/20"><span className="text-sm text-muted-foreground">תיאור</span><span className="text-sm text-white">{viewDetail.desc || "—"}</span></div>
                <div className="flex justify-between py-1.5 border-b border-border/20"><span className="text-sm text-muted-foreground">עובדים</span><span className="text-sm text-white font-bold">{viewDetail.employeeCount}</span></div>
              </div>}
