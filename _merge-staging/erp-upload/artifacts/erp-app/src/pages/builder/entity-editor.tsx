...
export default function EntityEditor() {
  ...
  const { data: allEntities = [] } = useQuery({
    queryKey: ["all-entities-for-relations"],
    queryFn: async () => {
      const modules = await authFetch(`${API}/platform/modules`).then(r => r.json());
      const allEnts: any[] = [];
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <Link href={`/builder/data/${entityId}`} className="flex items-center gap-2 px-4 py-2.5 bg-green-600 text-white rounded-xl font-medium hover:bg-green-700 transition-colors">
      ...
      {activeTab === "fields" && (
        <FieldsTab fields={entity.fields || []} onAdd={() => setShowAddField(true)} onEdit={setEditingField} onDelete={async (id) => { if (await globalConfirm("למחוק את השדה?")) deleteFieldMutation.mutate(id); }} onClone={async (id) => { if (await globalConfirm("לשכפל את השדה?")) cloneFieldMutation.mutate(id); }} onReorder={(items) => reorderFieldsMutation.mutate(items)} />
      )}
      {activeTab === "statuses" && (
      ...
      {activeTab === "relations" && (
        <RelationsTab relations={relations} entityId={entityId} allEntities={allEntities} onAdd={() => setShowAddRelation(true)} onEdit={setEditingRelation} onDelete={async (id) => { if (await globalConfirm("למחוק את הקשר?")) deleteRelationMutation.mutate(id); }} />
      )}
      {activeTab === "sub_tables" && (
        <SubTablesTab entityId={entityId} />
      )}
      {activeTab === "forms" && (
        <FormsTab forms={forms} onAdd={() => setShowAddForm(true)} onEdit={setEditingForm} onDelete={async (id) => { if (await globalConfirm("למחוק את הטופס?")) deleteFormMutation.mutate(id); }} onEditSections={(form) => setEditingSections(form)} />
      )}
      {activeTab === "views" && (
        <ViewsTab views={views} onAdd={() => setShowAddView(true)} onEdit={setEditingView} onDelete={async (id) => { if (await globalConfirm("למחוק את התצוגה?")) deleteViewMutation.mutate(id); }} onEditColumns={(view) => setEditingViewColumns(view)} />
      )}
      {activeTab === "details" && (
        <DetailsTab details={detailDefs} onAdd={() => setShowAddDetail(true)} onEdit={setEditingDetail} onDelete={async (id) => { if (await globalConfirm("למחוק את הגדרת דף הפרטים?")) deleteDetailMutation.mutate(id); }} onEditSections={(detail) => setEditingDetailSections(detail)} />
      )}
      {activeTab === "buttons" && (
      ...
      <tr key={field.id} className="border-b border-border/50 last:border-0 hover:bg-muted/20 transition-colors group">
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-1 justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button onClick={() => onDelete(field.id)} className="p-1.5 hover:bg-destructive/10 rounded-lg" title="מחק שדה"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
          </div>
        </td>
      </tr>
    );
          ...
          {statuses.map(status => {
            ...
            return (
              <div key={status.id} className="bg-card border border-border rounded-xl p-4 flex items-center justify-between">
                ...
                <button onClick={() => onDelete(status.id)} className="p-1.5 hover:bg-destructive/10 rounded-lg"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
              </div>
            );
          })}
        </div>
            ...
            <div key={rel.id} className="bg-card border border-border rounded-xl p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => onDelete(rel.id)} className="p-1.5 hover:bg-destructive/10 rounded-lg"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
                </div>
              </div>
            </div>
          ))}
                ...
                <>
                  ...
                  {(form.options as string[]).length > 0 && (
                    <div className="flex items-center justify-between pt-2">
                      ...
                      <button type="button" onClick={async () => { if (globalConfirm("למחוק את כל האפשרויות?")) setForm(f => ({ ...f, options: [] })); }}
              ...
              <div className="p-4 bg-pink-500/5 border border-pink-500/20 rounded-xl">
                ...
                <div className="mt-4 p-3 bg-muted/30 rounded-lg border border-border/50">
                  ...
                  <div className="space-y-1 text-xs font-mono text-muted-foreground">
                    ...
                    <p className="cursor-pointer hover:text-pink-400 transition-colors" onClick={() => setForm(f => ({ ...f, formulaExpression: "round({price} * {quantity} * 1.18)" }))}>{"• round({price} * {quantity} * 1.18)"} — סה"כ כולל מע"מ מעוגל</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div key={form.id} className="bg-card border border-border rounded-xl p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => onDelete(form.id)} className="p-1.5 hover:bg-destructive/10 rounded-lg" title="מחיקת טופס">
              ...
              <div key={view.id} className="bg-card border border-border rounded-xl p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => onDelete(view.id)} className="p-1.5 hover:bg-destructive/10 rounded-lg" title="מחיקת תצוגה">
                      <Trash2 className="w-3.5 h-3.5 text-destructive" />
                    </button>
                  </div>
                </div>
              ...
              <div key={detail.id} className="bg-card border border-border rounded-xl p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => onDelete(detail.id)} className="p-1.5 hover:bg-destructive/10 rounded-lg" title="מחיקת הגדרה">
                      <Trash2 className="w-3.5 h-3.5 text-destructive" />
                    </button>
                  </div>
                </div>
