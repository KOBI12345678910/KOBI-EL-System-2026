...
export default function DashboardBuilder() {
  ...
  const { data: entities = [] } = useQuery<ModuleEntity[]>({
    ...
    queryFn: async () => {
      const allEntities: ModuleEntity[] = [];
      for (const mod of modules) {
        const r = await authFetch(`${API}/platform/modules/${mod.id}/entities`);
        if (r.ok) {
          const ents = await r.json();
          if (Array.isArray(ents)) allEntities.push(...ents.map((e: any) => ({ ...e, moduleId: mod.id })));
        }
      }
      return allEntities;
    },
            ...
            {dashboards.map((dash, i) => (
              <motion.div key={dash.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-border/50">
                  ...
                  <button onClick={async (e) => { e.stopPropagation(); if (globalConfirm("למחוק את הדשבורד?")) deleteDashboardMutation.mutate(dash.id); }}
