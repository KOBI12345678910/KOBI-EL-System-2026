...
export default function ButtonsBuilderPage() {
  ...
  const { data: modules = [] } = useQuery({
    ...
    queryFn: () => authFetch(`${API}/platform/modules`).then(r => r.json()),
  });
                  ...
                  return (
                    <div key={btn.id} className="flex items-center gap-2 group">
                      ...
                      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMutation.mutate(btn.id); }} className="p-1 hover:bg-destructive/10 rounded"><Trash2 className="w-3 h-3 text-destructive" /></button>
                      </div>
                    </div>
                  );
                })}
              ...
              {buttons.map(btn => (
                <div key={btn.id} className={`flex items-center gap-3 px-4 py-3 bg-card border rounded-xl transition-colors ${btn.isActive ? "border-border" : "border-border/50 opacity-60"}`}>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMutation.mutate(btn.id); }} className="p-1.5 hover:bg-destructive/10 rounded-lg"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
                  </div>
                </div>
              ))}
            </div>
