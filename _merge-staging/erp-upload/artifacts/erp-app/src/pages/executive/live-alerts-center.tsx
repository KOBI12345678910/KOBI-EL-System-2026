...
import { authFetch } from "@/lib/utils";
    ...
    const STATUSES = ["פעיל", "אושר", "נדחה", "נפתר", "מושהה"] as const;
    const SC: Record<string, string> = { "פעיל": "bg-red-500/20 text-red-300", "אושר": "bg-yellow-500/20 text-yellow-300", "נדחה": "bg-gray-500/20 text-gray-300", "נפתר": "bg-green-500/20 text-green-300", "מושהה": "bg-blue-500/20 text-blue-300" };
    const CATS = ["ייצור","מלאי","פיננסי","בטיחות","איכות","תחזוקה"];
    const EMP = ["יוסי כהן","שרה לוי","דוד מזרחי","רחל אברהם","אלון גולדשטיין","מיכל ברק","עומר חדד","נועה פרידמן","איתן רוזנברג","תמר שלום"];
export default function LiveAlertsCenter() {
  ...
  const [search, setSearch] = useState("");
    ...
    try {
      setError(null);
      const res = await authFetch("/api/executive/live-alerts");
      if (res.ok) {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.title?.toLowerCase().includes(s) || r.category?.toLowerCase().includes(s) || r.severity?.toLowerCase().includes(s)); }
      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingDown className="h-6 w-6 text-red-400" />מרכז התראות חי</h1><p className="text-sm text-muted-foreground mt-1">ניטור התראות בזמן אמת — ייצור, מלאי, פיננסי ובטיחות</p></div>
            <div className="flex gap-2">
              ...
              <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2"><Plus className="h-4 w-4" />התראה חדשה</Button>
            </div>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"התראות פעילות","v":"18","c":"text-red-400","t":"+3","u":false},{"l":"קריטיות","v":"3","c":"text-orange-400","t":"-1","u":true},{"l":"נפתרו היום","v":"12","c":"text-green-400","t":"+5","u":true},{"l":"זמן תגובה","v":"8 דק","c":"text-cyan-400","t":"-2","u":true},{"l":"אוטומטיות","v":"85%","c":"text-blue-400","t":"+5%","u":true},{"l":"סהכ היום","v":"42","c":"text-purple-400","t":"+8","u":true}].map((k: any, i: number) => (
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("title")}>התראה<SI field="title" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("triggered")}>זמן<SI field="triggered" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("responseTime")}>תגובה (דק)<SI field="responseTime" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingDown className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין מרכז התראות חי"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "התראה חדשה ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />התראה חדשה</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 "><Badge className={`${row.severity==="קריטי"?"bg-red-500/20 text-red-300 animate-pulse":row.severity==="גבוה"?"bg-orange-500/20 text-orange-300":"bg-yellow-500/20 text-yellow-300"} border-0 text-xs`}>{row.severity}</Badge></td>
            <td className="p-3 text-muted-foreground text-xs">{row.source}</td>
            <td className="p-3 font-mono text-muted-foreground text-xs">{row.triggered}</td>
            <td className="p-3 font-mono text-cyan-400 text-center">{row.responseTime}</td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]"><h2 className="text-lg font-bold text-white">{editId ? "עריכה" : "התראה חדשה"}</h2><Button variant="ghost" size="sm" onClick={()=>{setShowCreate(false);setEditId(null)}}><X className="h-4 w-4" /></Button></div>
            <div className="p-4 space-y-6">
              <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי התראה</h3></div>
            <div className="grid grid-cols-3 gap-4">
            <div><Label className="text-muted-foreground text-xs">כותרת *</Label><Input type="text" placeholder="תיאור ההתראה" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">קטגוריה *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>ייצור</option><option>מלאי</option><option>פיננסי</option><option>בטיחות</option><option>איכות</option><option>תחזוקה</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">מקור</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>חיישן</option><option>מערכת</option><option>ידני</option><option>AI</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>פעיל</option><option>אושר</option><option>נדחה</option><option>נפתר</option><option>מושהה</option></select></div>
            </div>
          <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטים</h3></div>
            <div className="grid grid-cols-3 gap-4">
            <div className="col-span-3"><Label className="text-muted-foreground text-xs">תיאור מפורט</Label><textarea rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1 resize-none" placeholder="פירוט..." /></div>
            </div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">התראה</p><p className="text-white mt-1 font-medium">{dr.title}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">הופעלה</p><p className="text-white mt-1 font-medium">{dr.triggered}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">תגובה</p><p className="text-white mt-1 font-medium">{dr.responseTime} דקות</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">אושרה</p><p className="text-white mt-1 font-medium">{dr.acknowledged?"כן":"לא"}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">נפתרה</p><p className="text-white mt-1 font-medium">{dr.resolvedAt||"פתוחה"}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">כלל</p><p className="text-white mt-1 font-medium">{dr.rule}</p></div>
