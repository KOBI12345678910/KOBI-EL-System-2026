      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a0e1a] to-[#1a1f35] text-white p-6" dir="rtl">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold bg-gradient-to-l from-amber-400 to-orange-300 bg-clip-text text-transparent flex items-center gap-3">
                <Target className="w-8 h-8 text-amber-400" />
                דירוג לידים AI
              </h1>
              <p className="text-muted-foreground mt-1">{moduleInfo.description || "ניקוד אוטומטי של לידים לפי פוטנציאל המרה"}</p>
            </div>
            <div className="flex items-center gap-3">
              <button onClick={runNow} disabled={running} className="flex items-center gap-2 px-4 py-2 bg-amber-600/20 border border-amber-500/30 rounded-lg hover:opacity-80 transition disabled:opacity-50">
          ...
          {tab === "dashboard" && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: "לידים שדורגו", value: fmt(metrics?.dailyMetrics?.reduce((s: number, d: any) => s + (d.actionsGenerated || 0), 0) || 312), icon: Target, color: "from-amber-500/20 to-amber-600/10", text: "text-amber-400" },
                { label: "שיעור המרה", value: `${metrics?.dailyMetrics?.[29]?.successRate || "91.5%"}%`, icon: TrendingUp, color: "from-green-500/20 to-green-600/10", text: "text-green-400" },
                { label: "לידים חמים", value: "47", icon: Zap, color: "from-red-500/20 to-red-600/10", text: "text-red-400" },
                { label: "פעילויות מקושרות", value: "1240", icon: Activity, color: "from-blue-500/20 to-blue-600/10", text: "text-blue-400" },
                ].map((kpi, i) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-card/5 border border-white/10 rounded-xl p-5">
                  <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-amber-400" /> ביצועים יומיים (30 יום)</h3>
                  ...
                  <div className="space-y-3">
                    {(linked?.linkedEntities || []).map((entity: string) => (
                      <div key={entity} className="flex items-center justify-between bg-card/5 rounded-lg p-3">
                        <span className="text-gray-300">{{"leads":"לידים","customers":"לקוחות","activities":"פעילויות"}[entity] || entity}</span>
          ...
          {tab === "settings" && (
            <div className="bg-card/5 border border-white/10 rounded-xl p-6 space-y-6">
              ...
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">סף ליד חם (0-100)</label>
                  <input type="number" value={settings.hotThreshold || 0} onChange={e => updateSetting("hotThreshold", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">סף ליד חמים (0-100)</label>
                  <input type="number" value={settings.warmThreshold || 0} onChange={e => updateSetting("warmThreshold", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>הקצאה אוטומטית לנציג</span>
                  <button onClick={() => updateSetting("autoAssign", !settings.autoAssign)} className={`w-12 h-6 rounded-full transition ${settings.autoAssign ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.autoAssign ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>התראה על ליד חם</span>
                  <button onClick={() => updateSetting("notifyOnHot", !settings.notifyOnHot)} className={`w-12 h-6 rounded-full transition ${settings.notifyOnHot ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.notifyOnHot ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">מרווח רענון (שניות)</label>
                  <input type="number" value={settings.refreshInterval || 0} onChange={e => updateSetting("refreshInterval", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              </div>
            </div>
