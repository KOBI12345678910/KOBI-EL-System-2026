...
import { Sparkles, Settings, Zap, FileText, Shield, BarChart3, Loader2, RefreshCw, Play, CheckCircle2, Clock, AlertTriangle, TrendingUp, Users, ShoppingCart, Target, Brain, Activity, Calendar, ToggleLeft, ToggleRight } from "lucide-react";
...
export default function AISalesAssistantPage() {
  ...
  const runNow = async () => {
    setRunning(true);
    try {
      await fetch(`${API}/ai-ops/modules/${MOD}/run`, { method: "POST", headers, body: JSON.stringify({}) });
      await load();
    } catch {}
    setRunning(false);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0e1a] to-[#1a1f35] text-white p-6" dir="rtl">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold bg-gradient-to-l from-purple-400 to-pink-300 bg-clip-text text-transparent flex items-center gap-3">
              <Sparkles className="w-8 h-8 text-purple-400" />
              עוזר מכירות AI
            </h1>
            <p className="text-muted-foreground mt-1">{moduleInfo.description || "ניתוח שיחות מכירה, המלצות מוצרים, ותחזית סגירה"}</p>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={runNow} disabled={running} className="flex items-center gap-2 px-4 py-2 bg-purple-600/20 border border-purple-500/30 rounded-lg hover:bg-purple-600/30 transition disabled:opacity-50">
        ...
        {tab === "dashboard" && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: "המלצות שנוצרו", value: fmt(metrics?.dailyMetrics?.reduce((s: number, d: any) => s + (d.actionsGenerated || 0), 0) || 156), icon: Brain, color: "from-purple-500/20 to-purple-600/10", text: "text-purple-400" },
                { label: "שיעור הצלחה", value: `${(metrics?.dailyMetrics?.[29]?.successRate || 94.2)}%`, icon: Target, color: "from-green-500/20 to-green-600/10", text: "text-green-400" },
                { label: "לקוחות מנותחים", value: fmt(linked?.entityCounts?.customers || 234), icon: Users, color: "from-blue-500/20 to-blue-600/10", text: "text-blue-400" },
                { label: "הזמנות מקושרות", value: fmt(linked?.entityCounts?.sales_orders || 89), icon: ShoppingCart, color: "from-amber-500/20 to-amber-600/10", text: "text-amber-400" },
              ].map((kpi, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-card/5 border border-white/10 rounded-xl p-5">
                <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-purple-400" /> ביצועים יומיים (30 יום)</h3>
              ...
              <div className="bg-card/5 border border-white/10 rounded-xl p-5">
                ...
                <div className="space-y-3">
                  {(linked?.linkedEntities || []).map((entity: string, i: number) => (
                    <div key={entity} className="flex items-center justify-between bg-card/5 rounded-lg p-3">
                      <span className="text-gray-300">{entity === "customers" ? "לקוחות" : entity === "sales_orders" ? "הזמנות מכירה" : entity === "quotations" ? "הצעות מחיר" : entity === "leads" ? "לידים" : entity}</span>
          ...
          <div className="bg-card/5 border border-white/10 rounded-xl p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">טמפרטורה (יצירתיות)</label>
                  <input type="range" min="0" max="1" step="0.1" value={settings.temperature || 0.7}
                    onChange={e => updateSetting("temperature", parseFloat(e.target.value))}
                    className="w-full mt-2" />
                  <span className="text-sm text-blue-400">{settings.temperature || 0.7}</span>
                </div>
                <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">מקסימום טוקנים</label>
                  <input type="number" value={settings.maxTokens || 2000} onChange={e => updateSetting("maxTokens", parseInt(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>הצעות אוטומטיות</span>
                  <button onClick={() => updateSetting("autoSuggest", !settings.autoSuggest)} className={`w-12 h-6 rounded-full transition ${settings.autoSuggest ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.autoSuggest ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
                <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">סף ביטחון מינימלי (%)</label>
                  <input type="number" min="0" max="100" value={settings.minConfidence || 75}
                    onChange={e => updateSetting("minConfidence", parseInt(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
                <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>התראה על ניקוד גבוה</span>
                  <button onClick={() => updateSetting("notifyOnHighScore", !settings.notifyOnHighScore)} className={`w-12 h-6 rounded-full transition ${settings.notifyOnHighScore ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.notifyOnHighScore ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
                <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">מרווח רענון (שניות)</label>
                  <input type="number" value={settings.refreshInterval || 300}
