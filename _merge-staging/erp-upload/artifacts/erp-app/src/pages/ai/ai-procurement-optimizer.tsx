import { useState, useEffect } from "react";
  ...
  import { Settings, Zap, FileText, Shield, BarChart3, Loader2, RefreshCw, Play, CheckCircle2, Clock, AlertTriangle, TrendingUp, Users, Activity, Brain, Target, Truck } from "lucide-react";
  ...
  export default function AIProcurementOptimizerPage() {
    ...
    const MOD = "procurement-optimizer";
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-1">{moduleInfo.description || "ניתוח ספקים, תחזית מחירים, ואופטימיזציית הזמנות"}</p>
            </div>
            <div className="flex items-center gap-3">
              <button onClick={runNow} disabled={running} className="flex items-center gap-2 px-4 py-2 bg-orange-600/20 border border-orange-500/30 rounded-lg hover:opacity-80 transition disabled:opacity-50">
          ...
          {tab === "dashboard" && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: "המלצות רכש", value: fmt(metrics?.dailyMetrics?.reduce((s: number, d: any) => s + (d.actionsGenerated || 0), 0) || 67), icon: Truck, color: "from-orange-500/20 to-orange-600/10", text: "text-orange-400" },
                { label: "חיסכון חודשי", value: "₪23,400", icon: TrendingUp, color: "from-green-500/20 to-green-600/10", text: "text-green-400" },
                { label: "ספקים מנותחים", value: "45", icon: Users, color: "from-blue-500/20 to-blue-600/10", text: "text-blue-400" },
                { label: "התראות מלאי", value: fmt(metrics?.dailyMetrics?.reduce((s: number, d: any) => s + (d.alertsTriggered || 0), 0) || 12), icon: AlertTriangle, color: "from-red-500/20 to-red-600/10", text: "text-red-400" },
                ].map((kpi, i) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-card/5 border border-white/10 rounded-xl p-5">
                  <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-orange-400" /> ביצועים יומיים (30 יום)</h3>
                  ...
                  <div className="space-y-3">
                    {(linked?.linkedEntities || []).map((entity: string) => (
                      <div key={entity} className="flex items-center justify-between bg-card/5 rounded-lg p-3">
                        <span className="text-gray-300">{{"suppliers":"ספקים","purchase_orders":"הזמנות רכש","inventory":"מלאי","raw_materials":"חומרי גלם"}[entity] || entity}</span>
          ...
          {tab === "settings" && (
            <div className="bg-card/5 border border-white/10 rounded-xl p-6 space-y-6">
              ...
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>הזמנה מחדש אוטומטית</span>
                  <button onClick={() => updateSetting("autoReorder", !settings.autoReorder)} className={`w-12 h-6 rounded-full transition ${settings.autoReorder ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.autoReorder ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">ימי מלאי בטחון</label>
                  <input type="number" value={settings.safetyStockDays || 0} onChange={e => updateSetting("safetyStockDays", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">סף התראת מחיר (%)</label>
                  <input type="number" value={settings.priceAlertThreshold || 0} onChange={e => updateSetting("priceAlertThreshold", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">משקל איכות (%)</label>
                  <input type="number" value={settings.qualityWeight || 0} onChange={e => updateSetting("qualityWeight", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">משקל מחיר (%)</label>
                  <input type="number" value={settings.priceWeight || 0} onChange={e => updateSetting("priceWeight", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">משקל משלוח (%)</label>
                  <input type="number" value={settings.deliveryWeight || 0} onChange={e => updateSetting("deliveryWeight", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              </div>
            </div>
