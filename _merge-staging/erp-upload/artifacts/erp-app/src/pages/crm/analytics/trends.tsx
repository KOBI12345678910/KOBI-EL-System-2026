    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      ...
      {loading ? (
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    <td className="px-4 py-3 font-medium text-white">{r.period}</td>
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-5">
          <h3 className="font-bold mb-4 text-white">מצב לידים לפי קטגוריה</h3>
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center gap-4">
                ...
                <span className="text-sm text-white w-8 text-right font-bold">{row.count}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{viewDetail.period} — פירוט</h2>
