    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white" dir="rtl">
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span className="text-white">Lead Quality</span>
        </div>
              ...
              <div className="flex items-center gap-2">
                {(["all", "high", "medium", "low"] as const).map(q => (
                  <button
                    ...
                    className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${filterQuality === q ? "bg-primary border-primary text-white" : "border-slate-700 text-muted-foreground hover:text-white"}`}
                  >
              ...
              <div className="rounded-2xl border border-slate-700/50 bg-slate-800/40 overflow-hidden">
                ...
                <div className="divide-y divide-slate-700/30 max-h-[400px] overflow-y-auto">
                  {filteredLeads.map((lead, i) => (
                    ...
                    >
                      <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-slate-600 to-slate-700 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                        {(lead.name || "?")[0]}
                      </div>
                      ...
                      <div className="text-xl font-bold text-white flex-shrink-0">{lead.quality.overall}%</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="rounded-2xl border border-green-500/20 bg-gradient-to-br from-green-500/5 to-slate-800/40 overflow-hidden">
                  ...
                  <div className="flex border-b border-slate-700/50">
                    {[{key:"details",label:"פרטים"},{key:"related",label:"רשומות קשורות"},{key:"docs",label:"מסמכים"},{key:"history",label:"היסטוריה"}].map(t => (
                      <button key={t.key} onClick={() => setDetailTab(t.key)} className={`px-3 py-2 text-xs font-medium border-b-2 ${detailTab === t.key ? "border-green-400 text-green-400" : "border-transparent text-muted-foreground hover:text-white"}`}>{t.label}</button>
                    ))}
                  </div>
                  {detailTab === "details" && (
