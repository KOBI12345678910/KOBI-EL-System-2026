...
function safeArray(d: any): any[] {
  ...
  if (d && typeof d === "object") {
    return d.data || d.items || d.rows || d.employees || d.orders || d.expenses || [];
  }
  return [];
}
...
export default function OperationsControlCenter() {
  ...
  const { data: customers = [] } = useQuery<any[]>({
    ...
    queryFn: async () => {
      const r = await authFetch(`${API}/sales/customers`);
      return r.ok ? safeArray(await r.json()) : [];
    },
  });

  const { data: notifStats } = useQuery<any>({
  ...
  const woOnHold = workOrders.filter((w: any) => w.status === "on_hold" || w.status === "מושהה").length;
  ...
  const moduleRouteMap: Record<string, string> = {
    inventory: "/raw-materials", suppliers: "/suppliers", procurement: "/purchase-orders",
    documents: "/documents", finance: "/finance", "crm-advanced": "/crm",
    installers: "/hr/employees", customers: "/sales/customers", hr: "/hr/employees",
    production: "/work-orders", sales: "/sales/orders", projects: "/projects",
    "field-measurements": "/field-measurements", meetings: "/meetings",
  };
                ...
                <div key={cat.key} className="relative flex-1 min-w-[100px]">
                  ...
                  >
                    <div className={`relative w-10 h-10 rounded-xl flex items-center justify-center ${cat.bgColor} border ${cat.borderColor}`}>
                      ...
                      {count > 0 && (
                        <span className={`absolute -top-2 -left-2 min-w-[20px] h-5 flex items-center justify-center text-[11px] font-bold text-white rounded-full px-1 ${cat.color}`}>
                          {count > 99 ? "99+" : count}
                        </span>
                      )}
                    </div>
                      ...
                      >
                        <div className={`px-4 py-2.5 border-b border-border ${cat.bgColor}`}>
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`mr-auto text-xs font-bold px-2 py-0.5 rounded-full text-white ${cat.color}`}>{count}</span>
                          </div>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-card border rounded-2xl p-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm flex items-center gap-1.5"><Target className="w-3.5 h-3.5 text-amber-400" /> מתוכנן</span>
                    <span className="font-bold text-amber-400 text-lg">{woPlanned}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> הושלם</span>
                    <span className="font-bold text-emerald-400 text-lg">{woCompleted}</span>
                  </div>
                  {woOnHold > 0 && (
                    <div className="flex items-center justify-between">
                      <span className="text-sm flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5 text-red-400" /> מושהה</span>
                    ...
                    <tbody>
                      {workOrders.slice(0, 6).map((wo: any, i: number) => (
                        <tr key={i} className="border-b border-border/20 hover:bg-accent/30 cursor-pointer transition-colors"
                          ...
                          <td className="py-2 px-2 max-w-[180px] truncate">{wo.title || wo.description}</td>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden">
              ...
              <div className="divide-y divide-border max-h-64 overflow-y-auto">
                {modules.slice(0, 15).map((mod: any) => {
            ...
            <div className="bg-gradient-to-l from-blue-500/5 to-purple-500/5 border border-blue-500/20 rounded-2xl p-4">
              ...
              <div className="space-y-1">
                ...
                <NavButton label="הנהלת חשבונות" icon={Receipt} href="/accounting" color="hover:bg-rose-500/10 text-rose-400" onNavigate={navigate} />
