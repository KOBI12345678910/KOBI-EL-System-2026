...
import { FolderKanban, Plus, Search, Download, Edit2, Trash2, Eye, X, TrendingUp, DollarSign, Users, Clock, Filter } from "lucide-react";
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg sm:text-2xl font-bold text-white">דשבורד פרויקטים</h1>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => openForm()} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm"><Plus size={16} />פרויקט חדש</button>
        </div>
      </div>
      ...
      <div className="flex gap-3 items-center">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש פרויקטים..." className="w-full bg-gray-800 border border-gray-700 rounded-lg pr-10 pl-4 py-2 text-sm text-white" />
        </div>
          ...
          <tbody>
            {filtered.map((p: any) => (
              <tr key={p.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                ...
                <td className="p-3 text-white font-medium cursor-pointer hover:text-blue-400" onClick={() => { setViewDetail(p); setDetailTab("details"); }}>{p.name}</td>
                <td className="p-3 text-gray-300">{p.client || "-"}</td>
                <td className="p-3"><span className={`px-2 py-1 rounded-full text-xs border ${STATUS_COLORS[p.status] || ""}`}>{p.status}</span></td>
                ...
                <td className="p-3 text-center">
                  <div className="flex justify-center gap-1">
                    <button onClick={() => { setViewDetail(p); setDetailTab("details"); }} className="p-1 hover:bg-gray-700 rounded"><Eye size={14} className="text-muted-foreground" /></button>
                    <button onClick={() => openForm(p)} className="p-1 hover:bg-gray-700 rounded"><Edit2 size={14} className="text-blue-400" /></button>
                    <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMut.mutate(p.id); }} className="p-1 hover:bg-gray-700 rounded"><Trash2 size={14} className="text-red-400" /></button>
                  </div>
                </td>
              </tr>
            ))}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              ...
              {detailTab === "details" && (
                <div className="p-5 grid grid-cols-2 gap-4">
                  <div><span className="text-xs text-muted-foreground">שם</span><div className="text-sm text-white mt-0.5">{viewDetail.name}</div></div>
                  <div><span className="text-xs text-muted-foreground">לקוח</span><div className="text-sm text-white mt-0.5">{viewDetail.client || "—"}</div></div>
                  ...
                  <div><span className="text-xs text-muted-foreground">תקציב</span><div className="text-sm text-white mt-0.5">₪{parseFloat(viewDetail.budget || "0").toLocaleString()}</div></div>
                  <div><span className="text-xs text-muted-foreground">הוצאה</span><div className="text-sm text-white mt-0.5">₪{parseFloat(viewDetail.spent || "0").toLocaleString()}</div></div>
                  <div><span className="text-xs text-muted-foreground">תאריך התחלה</span><div className="text-sm text-white mt-0.5">{viewDetail.startDate || "—"}</div></div>
                  <div><span className="text-xs text-muted-foreground">תאריך סיום</span><div className="text-sm text-white mt-0.5">{viewDetail.endDate || "—"}</div></div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-white">{editing ? "עריכת פרויקט" : "פרויקט חדש"}</h2>
