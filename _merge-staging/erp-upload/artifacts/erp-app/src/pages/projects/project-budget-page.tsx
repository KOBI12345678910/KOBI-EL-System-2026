...
const statusMap: Record<string, { label: string; color: string }> = {
  ...
  over_budget: { label: "חריגה", color: "bg-red-500/20 text-red-400" },
...
export default function ProjectBudgetPage() {
  ...
  const { errors, validate, clearErrors } = useFormValidation<any>({
    project_name: { required: true, minLength: 2, message: "שם פרויקט חובה" },
  });
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API}/projects/budget`);
  ...
  const filtered = useMemo(() => {
    let data = items.filter(i =>
      ...
      (!search || [i.project_name, i.category, i.description, i.notes]
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const url = editing ? `${API}/projects/budget/${editing.id}` : `${API}/projects/budget`;
      await authFetch(url, {
  ...
  const remove = async (id: number) => {
    if (await globalConfirm("למחוק פריט תקציב? פעולה זו אינה ניתנת לביטול.")) {
      await authFetch(`${API}/projects/budget/${id}`, { method: "DELETE" });
      load();
    }
  };
  ...
  const kpis = [
    ...
    { label: "ניצול", value: `${totalBudget ? ((totalSpent / totalBudget) * 100).toFixed(0) : 0}%`, icon: BarChart3, color: "text-purple-400" },
    { label: "חריגות", value: fmt(overBudgetCount), icon: AlertTriangle, color: "text-orange-400" },
  ];
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">ניהול תקציבים, מעקב הוצאות וניצול</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <ExportDropdown
            data={filtered}
            headers={{ project_name: "פרויקט", category: "קטגוריה", budget: "תקציב", spent: "הוצאות", status: "סטטוס" }}
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש פרויקט, קטגוריה..."
      ...
      <BulkActions selectedIds={selectedIds} onClear={clear} entityName="תקציבי פרויקטים" actions={defaultBulkActions(selectedIds, clear, load, `${API}/projects/budgets`)} />
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => {
                  const b = r.budget || r.allocated || 0;
                  const s = r.spent || r.actual || 0;
                  return (
                    <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                  <td className="px-2 py-3"><BulkCheckbox checked={isSelected(r.id)} onChange={() => toggle(r.id)} /></td>
                      <td className="px-4 py-3 text-white font-medium">{r.project_name || "—"}</td>
                      <td className="px-4 py-3 text-muted-foreground">{r.category || "—"}</td>
                      <td className="px-4 py-3 text-emerald-400">₪{fmt(b)}</td>
                      <td className="px-4 py-3 text-red-400">₪{fmt(s)}</td>
                      <td className="px-4 py-3">
                        <span className={b - s < 0 ? "text-red-400 font-bold" : "text-white"}>₪{fmt(b - s)}</span>
                      </td>
                      <td className="px-4 py-3">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{viewDetail.project_name}</h2>
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                ...
                <DetailField label="יתרה" value={`₪${fmt((viewDetail.budget || viewDetail.allocated || 0) - (viewDetail.spent || viewDetail.actual || 0))}`} />
                <DetailField label="סטטוס">
                  <Badge className={statusMap[viewDetail.status]?.color}>{statusMap[viewDetail.status]?.label}</Badge>
                </DetailField>
                <div className="col-span-2"><DetailField label="הערות" value={viewDetail.notes || viewDetail.description} /></div>
              </div>
              )}
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת תקציב" : "פריט תקציב חדש"}</h2>
