...
function AllocationHeatCell({ value }: { value: number }) {
  const bg = value > 100 ? "bg-red-500/30 text-red-300" :
    ...
    value > 0 ? "bg-blue-500/10 text-blue-300" : "bg-gray-700/30 text-gray-500";
  return (
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-100">תכנון משאבים</h1>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-700/50">
              {loading ? (
                ...
                return (
                  <tr key={item.id || idx} className={`hover:bg-gray-800/30 transition-colors ${rowBg}`}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${alloc > 100 ? "bg-red-500" : alloc >= 80 ? "bg-yellow-500" : "bg-emerald-500"}`} />
                        <span className="text-gray-200 font-medium">{item.resource_name || "—"}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
