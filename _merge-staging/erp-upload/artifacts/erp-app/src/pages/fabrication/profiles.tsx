        ...
        <div className="flex gap-3 items-center">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder={"חיפוש פרופיל..."}
              className="w-full bg-gray-800/50 border border-gray-700 rounded-lg pr-10 pl-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={p.id} className="border-b border-gray-700/30 hover:bg-gray-700/20 cursor-pointer" onClick={() => { setDetail(p); setDetailTab("details"); }}>
                    ...
                    <td className="p-3 flex gap-1" onClick={e => e.stopPropagation()}>
                      <button onClick={() => openEdit(p)} className="p-1 hover:bg-gray-600 rounded"><Edit2 className="w-4 h-4" /></button>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
            <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowForm(false)} className="p-1 hover:bg-gray-700 rounded"><X className="w-5 h-5" /></button>
              </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={() => setShowForm(false)} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg">{"ביטול"}</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setDetail(null)}>
            <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setDetail(null)} className="p-1 hover:bg-gray-700 rounded"><X className="w-5 h-5" /></button>
              </div>
              <div className="flex gap-2 mb-4 border-b border-gray-700">
                {["details", "related", "attachments", "history"].map(tab => (
                  <button key={tab} onClick={() => setDetailTab(tab)}
                    className={`px-4 py-2 text-sm transition-colors ${detailTab === tab ? "text-blue-400 border-b-2 border-blue-400" : "text-muted-foreground hover:text-white"}`}>
              ...
              {detailTab === "details" && (
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {[
                    ...
                    <div key={i}><span className="text-muted-foreground">{label}: </span><span className="text-white">{value || "—"}</span></div>
                  ))}
