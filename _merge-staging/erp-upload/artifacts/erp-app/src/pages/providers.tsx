  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
        className="w-full max-w-lg bg-[#0d1117] border border-gray-800 rounded-2xl overflow-hidden" dir="rtl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800 bg-blue-600/5">
          <h3 className="font-bold text-white">{provider ? "עריכת ספק" : "הוספת ספק חדש"}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500" required />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">מזהה (Slug)</label>
              <input value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value })} dir="ltr" className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 text-left" required />
            </div>
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">תיאור</label>
            <input value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500" />
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">אתר (Website)</label>
            <input value={form.website} onChange={e => setForm({ ...form, website: e.target.value })} dir="ltr" className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 text-left" placeholder="https://..." />
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">כתובת API (Base URL)</label>
            <input value={form.apiBaseUrl} onChange={e => setForm({ ...form, apiBaseUrl: e.target.value })} dir="ltr" className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 text-left font-mono" placeholder="https://api.example.com/v1" />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-800">
            ...
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700 disabled:opacity-50">{saving ? "שומר..." : "שמור"}</button>
          </div>
        </form>
      </motion.div>
    </motion.div>
    ...
    <div className="min-h-screen bg-[#080a0f] text-white" dir="rtl">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-6">
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl font-black text-white tracking-tight">ספקי AI</h1>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setEditingProvider("new")} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700 transition-colors text-sm">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-[#0d1117] border border-gray-800 rounded-2xl"><Plug className="w-12 h-12 text-muted-foreground mx-auto mb-4" /><h3 className="text-white font-bold mb-2">אין ספקים</h3><p className="text-muted-foreground text-sm">הוסף ספק AI ראשון</p></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {providers.map((provider, i) => (
              <motion.div key={provider.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                className="bg-[#0d1117] border border-gray-800 rounded-2xl overflow-hidden hover:border-blue-500/30 transition-colors">
                <div className="px-4 py-3 border-b border-gray-800 bg-blue-500/5 flex items-center justify-between">
                  ...
                  <span className={`text-[10px] px-2 py-1 rounded-lg ${provider.isActive ? "bg-green-500/10 text-green-400" : "bg-gray-800 text-muted-foreground"}`}>
                ...
                <div className="px-4 py-3 space-y-2">
                  ...
                  {provider.apiBaseUrl && (
                    <div className="text-[10px] text-muted-foreground font-mono bg-gray-900/50 rounded-lg px-2 py-1.5 border border-gray-800 break-all" dir="ltr">{provider.apiBaseUrl}</div>
                  )}
                  {provider.website && (
