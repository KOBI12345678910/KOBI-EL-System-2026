...
import { authFetch } from "@/lib/utils";
    ...
    const STATUSES = ["רווחי", "שולי", "הפסדי", "בתהליך", "ללא נתון"] as const;
    const SC: Record<string, string> = { "רווחי": "bg-green-500/20 text-green-300", "שולי": "bg-yellow-500/20 text-yellow-300", "הפסדי": "bg-red-500/20 text-red-300", "בתהליך": "bg-blue-500/20 text-blue-300", "ללא נתון": "bg-gray-500/20 text-gray-300" };
...
export default function ProjectProfitability() {
  ...
  const [search, setSearch] = useState("");
    ...
    try {
      setError(null);
      const res = await authFetch("/api/finance-control/project-profitability");
      if (res.ok) {
      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingUp className="h-6 w-6 text-emerald-400" />רווחיות פרויקטים</h1><p className="text-sm text-muted-foreground mt-1">ניתוח רווחיות פרויקטים — הכנסות, עלויות ומרווח</p></div>
            <div className="flex gap-2">
              ...
              <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2"><Plus className="h-4 w-4" />פרויקט חדש</Button>
            </div>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"פרויקטים","v":"45","c":"text-emerald-400","t":"+3","u":true},{"l":"רווחיים","v":"35","c":"text-green-400","t":"+5","u":true},{"l":"הפסדיים","v":"4","c":"text-red-400","t":"-1","u":true},{"l":"מרווח ממוצע","v":"24%","c":"text-blue-400","t":"+2%","u":true},{"l":"הכנסות כולל","v":"₪8.5M","c":"text-cyan-400","t":"+15%","u":true},{"l":"ROI","v":"32%","c":"text-amber-400","t":"+4%","u":true}].map((k: any, i: number) => (
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("revenue")}>הכנסות<SI field="revenue" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("directCosts")}>עלויות<SI field="directCosts" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("netProfit")}>רווח נקי<SI field="netProfit" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("margin")}>מרווח<SI field="margin" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("completion")}>התקדמות<SI field="completion" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingUp className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין רווחיות פרויקטים"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "פרויקט חדש ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />פרויקט חדש</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 font-mono text-emerald-400">{fmt(row.revenue)}</td>
            <td className="p-3 font-mono text-red-400">{fmt(row.directCosts)}</td>
            <td className="p-3 font-mono text-green-400">{fmt(row.netProfit)}</td>
            <td className="p-3 font-mono text-cyan-400">{row.margin}%</td>
            <td className="p-3 "><div className="w-16 bg-gray-700 rounded-full h-2"><div className="bg-blue-500 rounded-full h-2" style={{width:`${row.completion}%`}} /></div></td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]"><h2 className="text-lg font-bold text-white">{editId ? "עריכה" : "פרויקט חדש"}</h2><Button variant="ghost" size="sm" onClick={()=>{setShowCreate(false);setEditId(null)}}><X className="h-4 w-4" /></Button></div>
            ...
            <div><Label className="text-muted-foreground text-xs">הכנסות</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">עלויות ישירות</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">עלויות עקיפות</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            ...
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>רווחי</option><option>שולי</option><option>הפסדי</option><option>בתהליך</option><option>ללא נתון</option></select></div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">הכנסות</p><p className="text-white mt-1 font-medium">{fmt(dr.revenue)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">עלויות ישירות</p><p className="text-white mt-1 font-medium">{fmt(dr.directCosts)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">עלויות עקיפות</p><p className="text-white mt-1 font-medium">{fmt(dr.indirectCosts)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">רווח גולמי</p><p className="text-white mt-1 font-medium">{fmt(dr.grossProfit)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">רווח נקי</p><p className="text-white mt-1 font-medium">{fmt(dr.netProfit)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מרווח</p><p className="text-white mt-1 font-medium">{dr.margin}%</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">התקדמות</p><p className="text-white mt-1 font-medium">{dr.completion}%</p></div>
