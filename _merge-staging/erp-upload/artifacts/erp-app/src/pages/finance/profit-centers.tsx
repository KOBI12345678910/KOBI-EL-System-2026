...
import {
  PieChart, Search, Plus, Edit2, Trash2, X, Save, AlertTriangle,
  ArrowUpDown, DollarSign, Hash, Eye, TrendingUp, TrendingDown, Target, Users
} from "lucide-react";
...
export default function ProfitCentersPage() {
  ...
  const [viewDetail, setViewDetail] = useState<ProfitCenter | null>(null);
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API}/finance-sap/profit-centers`);
      if (res.ok) setItems(safeArray(await res.json()));
      else setError("שגיאה בטעינת מרכזי רווח");
    } catch (e: any) {
  ...
  const filtered = useMemo(() => {
    ...
    return data;
  }, [items, search, filterStatus, filterDept, sortField, sortDir]);
  ...
  const openEdit = (r: ProfitCenter) => {
    setEditing(r);
    setForm({
      code: r.code, name: r.name, nameHe: r.name_he,
      department: r.department, manager: r.manager,
      budget: r.budget, revenueTarget: r.revenue_target,
      costTarget: r.cost_target, parentCode: r.parent_code,
  ...
  const save = async () => {
    setSaving(true);
    try {
      const url = editing ? `${API}/finance-sap/profit-centers/${editing.id}` : `${API}/finance-sap/profit-centers`;
      await authFetch(url, {
  ...
  const remove = async (id: number) => {
    if (await globalConfirm("למחוק מרכז רווח זה?")) {
      await authFetch(`${API}/finance-sap/profit-centers/${id}`, { method: "DELETE" });
      load();
    }
  };
  ...
  const kpis = [
    { label: "מרכזי רווח", value: fmt(items.length), icon: Hash, color: "text-blue-400" },
    { label: "תקציב כולל", value: fmtCurrency(totalBudget), icon: DollarSign, color: "text-amber-400" },
    { label: "בפועל כולל", value: fmtCurrency(totalActual), icon: TrendingUp, color: "text-green-400" },
    { label: "סטייה כוללת", value: fmtCurrency(totalVariance), icon: totalVariance > 0 ? TrendingUp : TrendingDown, color: totalVariance >= 0 ? "text-green-400" : "text-red-400" },
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">ניהול מרכזי רווח, תקציבים וביצועים</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <ExportDropdown
            data={filtered}
            headers={{ code: "קוד", name_he: "שם", department: "מחלקה", manager: "מנהל", budget: "תקציב", actual: "בפועל", variance_pct: "סטייה %", status: "סטטוס" }}
            filename="profit_centers"
          />
          <button onClick={openCreate} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2.5 rounded-xl hover:bg-primary/90 shadow-lg text-sm font-medium">
            <Plus className="w-4 h-4" /> מרכז רווח חדש
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש לפי קוד, שם, מחלקה, מנהל..." className="w-full pr-10 pl-4 py-2.5 bg-card border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
        </div>
      ...
      {loading ? (
        <div className="space-y-4"><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">{Array.from({length:5}).map((_,i)=><div key={i} className="bg-card border border-border/50 rounded-2xl p-4 animate-pulse"><div className="h-5 w-5 bg-muted/30 rounded mb-2" /><div className="h-6 w-20 bg-muted/30 rounded mb-1" /><div className="h-3 w-16 bg-muted/30 rounded" /></div>)}</div><div className="h-10 bg-muted/20 rounded-xl animate-pulse" /><div className="border border-border/50 rounded-2xl bg-card/50 overflow-hidden"><table className="w-full"><tbody>{Array.from({length:8}).map((_,i)=><tr key={i} className="border-b border-border/20"><td className="p-3" colSpan={99}><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-muted/30 rounded" /><div className="h-4 w-16 bg-muted/30 rounded" /><div className="h-4 w-32 bg-muted/30 rounded" /><div className="h-4 w-24 bg-muted/30 rounded" /><div className="h-4 w-20 bg-muted/30 rounded" /></div></td></tr>)}</tbody></table></div></div>
      ) : error ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">אין מרכזי רווח</p>
          <p className="text-sm mt-1">{search || filterStatus !== "all" || filterDept !== "all" ? "נסה לשנות את הסינון" : "לחץ על 'מרכז רווח חדש' כדי להתחיל"}</p>
        </div>
      ) : (<>
        <BulkActions selectedIds={selectedIds} onClear={clear} entityName="items" actions={defaultBulkActions(selectedIds, clear, load, `${API}/finance-sap/profit-centers`)} />
                ...
                {pagination.paginate(filtered).map(r => {
                  ...
                  return (
                    <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-4 py-3 text-white">{r.name_he || r.name}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-bold ${vpct >= 0 ? "text-green-400" : "text-red-400"}`}>
                          {vpct >= 0 ? "+" : ""}{fmtPct(vpct)}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1">
                          ...
                          <button onClick={async () => { if (await globalConfirm(`למחוק את '${r.code || r.id}'?`)) remove(r.id); }} className="p-1.5 hover:bg-muted rounded-lg"><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
                        </div>
                      </td>
                    </tr>
                  );
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewDetail(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{viewDetail.name_he || viewDetail.name}</h2>
              ...
              {detailTab === "details" ? (
                <div className="p-5 grid grid-cols-2 gap-4">
                  ...
                  <DetailField label="סטייה %">
                    <span className={`text-sm font-bold ${Number(viewDetail.variance_pct || 0) >= 0 ? "text-green-400" : "text-red-400"}`}>
                      {fmtPct(viewDetail.variance_pct)}
                    </span>
                  </DetailField>
                  <DetailField label="יעד הכנסות" value={fmtCurrency(viewDetail.revenue_target)} />
                  <DetailField label="הכנסות בפועל" value={fmtCurrency(viewDetail.revenue_actual)} />
                  <DetailField label="יעד עלויות" value={fmtCurrency(viewDetail.cost_target)} />
                  <DetailField label="עלויות בפועל" value={fmtCurrency(viewDetail.cost_actual)} />
                ...
                <div className="p-5"><RelatedRecords entityType="profit-centers" entityId={viewDetail?.id} /></div>
              ) : detailTab === "attachments" ? (
                <div className="p-5"><AttachmentsSection entityType="profit-centers" entityId={viewDetail?.id} /></div>
              ) : (
                <div className="p-5"><ActivityLog entityType="profit-centers" entityId={viewDetail?.id} /></div>
              )}
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת מרכז רווח" : "מרכז רווח חדש"}</h2>
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">שם (EN)</label>
                  <input value={form.name || ""} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" dir="ltr" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="number" value={form.revenueTarget || ""} onChange={e => setForm({ ...form, revenueTarget: Number(e.target.value) })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">יעד עלויות</label>
                  <input type="number" value={form.costTarget || ""} onChange={e => setForm({ ...form, costTarget: Number(e.target.value) })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" />
                </div>
                <div>
