...
import {
  ...
  TrendingDown, DollarSign, Target, Eye, X, Hash
} from "lucide-react";
...
const typeMap: Record<string, { label: string; color: string }> = {
  revenue: { label: "הכנסות", color: "bg-green-500/20 text-green-400" },
  expenses: { label: "הוצאות", color: "bg-red-500/20 text-red-400" },
  customers: { label: "לקוחות", color: "bg-blue-500/20 text-blue-400" },
  projects: { label: "פרויקטים", color: "bg-purple-500/20 text-purple-400" },
};
...
export default function AnalyticalReportsPage() {
  ...
  const [viewDetail, setViewDetail] = useState<ReportRecord | null>(null);
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API}/analytical-reports?year=${year}`);
      if (res.ok) {
        const d = await res.json();
        const revenueByMonth = d?.revenueByMonth || [];
        const expensesByCategory = d?.expensesByCategory || [];
        const topCustomers = d?.topCustomers || [];
        const projectProfitability = d?.projectProfitability || [];
        setSummary(d?.summary || {});
        const merged: ReportRecord[] = [
          ...revenueByMonth.map((m: any, i: number) => ({ id: i + 1, month: m.month || "", revenue: Number(m.revenue || 0), expenses: 0, profit: 0, margin: 0, category: "", customer_name: "", project_name: "", type: "revenue" })),
          ...expensesByCategory.map((c: any, i: number) => ({ id: 100 + i, month: "", revenue: 0, expenses: Number(c.total || 0), profit: 0, margin: 0, category: c.category || "כללי", customer_name: "", project_name: "", type: "expenses" })),
          ...topCustomers.map((c: any, i: number) => ({ id: 200 + i, month: "", revenue: Number(c.total_amount || 0), expenses: 0, profit: Number(c.balance_due || 0), margin: 0, category: "", customer_name: c.customer_name || "", project_name: "", type: "customers" })),
          ...projectProfitability.map((p: any, i: number) => ({ id: 300 + i, month: "", revenue: Number(p.actual_revenue || 0), expenses: Number(p.actual_cost || 0), profit: Number(p.actual_revenue || 0) - Number(p.actual_cost || 0), margin: Number(p.margin || 0), category: "", customer_name: "", project_name: p.project_name || "", type: "projects" })),
        ];
        setItems(merged);
      } else {
        setError("שגיאה בטעינת דוחות אנליטיים");
      }
    } catch (e: any) {
  ...
  const filtered = useMemo(() => {
    let data = items.filter(i =>
      ...
      (!search || [i.month, i.category, i.customer_name, i.project_name]
  ...
  const kpis = [
    ...
    { label: "הכנסות שנתיות", value: fmtCurrency(Number(summary.total_revenue || 0)), icon: TrendingUp, color: "text-green-400" },
    { label: "הוצאות שנתיות", value: fmtCurrency(Number(summary.total_expenses || 0)), icon: TrendingDown, color: "text-red-400" },
    { label: "רווח נקי", value: fmtCurrency(Number(summary.net_profit || 0)), icon: DollarSign, color: Number(summary.net_profit || 0) >= 0 ? "text-emerald-400" : "text-red-400" },
    { label: "מרווח", value: `${Number(summary.margin || 0).toFixed(1)}%`, icon: Target, color: "text-purple-400" },
  ];
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-2">
            <BarChart3 className="text-teal-400 w-6 h-6" /> דוחות אנליטיים
          </h1>
          <p className="text-sm text-muted-foreground mt-1">ניתוחים מתקדמים — הכנסות, הוצאות, לקוחות ופרויקטים</p>
        </div>
        <div className="flex gap-2 flex-wrap items-center">
          ...
          <ExportDropdown
            data={filtered}
            headers={{ type: "סוג", month: "חודש", category: "קטגוריה", revenue: "הכנסות", expenses: "הוצאות", profit: "רווח" }}
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש לפי חודש, קטגוריה, לקוח..."
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-white">{r.month || "—"}</td>
                    <td className="px-4 py-3 text-muted-foreground">{r.category || "—"}</td>
                    <td className="px-4 py-3 text-muted-foreground">{r.customer_name || r.project_name || "—"}</td>
                    <td className="px-4 py-3 text-green-400 font-bold">{Number(r.revenue) > 0 ? fmtCurrency(r.revenue) : "—"}</td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><BarChart3 className="w-5 h-5 text-teal-400" /> פרטי רשומה</h2>
