...
import { RotateCcw, Search, Plus, Edit2, Trash2, X, Save, Hash, CheckCircle2, Clock, AlertTriangle, ArrowUpDown, DollarSign, FileText, Loader2, TrendingDown, BarChart3 } from "lucide-react";
...
export default function CreditNotesPage() {
  ...
  const openCreate = () => { setEditing(null); setForm({ creditType: "credit", creditDate: new Date().toISOString().slice(0,10), reason: "return", status: "draft", currency: "ILS", vatRate: 18 }); setShowForm(true); };
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => printPage("זיכויים")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Printer size={16} /> הדפסה</button>
          <button onClick={() => sendByEmail("זיכויים - טכנו-כל עוזי", generateEmailBody("זיכויים", items, { credit_number: "מספר", customer_name: "לקוח", total_amount: "סכום", status: "סטטוס" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Send size={16} /> שליחה</button>
          <button onClick={openCreate} className="flex items-center gap-2 bg-rose-600 text-white px-3 py-2 rounded-lg hover:bg-rose-700 shadow-lg text-sm"><Plus size={16} /> זיכוי חדש</button>
        </div>
      </div>
          ...
          <tbody>
            ...
            pagination.paginate(filtered).map(r => (
              <tr key={r.id} className="border-b hover:bg-rose-50/30">
                ...
                <td className="px-3 py-2"><div className="flex gap-1"><button onClick={() => openEdit(r)} className="p-1 hover:bg-blue-500/10 rounded"><Edit2 size={14} /></button><button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.customer_name || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1 hover:bg-red-500/10 rounded text-red-500"><Trash2 size={14} /></button></div></td>
              </tr>
            ))}
          </tbody>
          {filtered.length > 0 && (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium mb-1">מע"מ %</label><input type="number" step="0.01" value={form.vatRate ?? 18} onChange={e => setForm(recalc({ ...form, vatRate: e.target.value }))} className="w-full border rounded-lg px-3 py-2" /></div>
                <div className="col-span-2 bg-gradient-to-l from-rose-50 to-blue-50 rounded-xl border border-rose-200 p-4 mt-1">
                  <div className="text-sm font-bold text-foreground mb-3">פירוט חישוב מע"מ ({form.vatRate ?? 18}%)</div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                    ...
                    <div className="bg-card rounded-lg p-2 border">
                      ...
                      <div className="font-bold text-amber-600">{form.vatRate ?? 18}%</div>
                    </div>
                    <div className="bg-card rounded-lg p-2 border">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={save} className="flex items-center gap-2 bg-rose-600 text-white px-6 py-2 rounded-lg hover:bg-rose-700"><Save size={16} /> {editing ? "עדכון" : "שמירה"}</button>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">זיכוי #{selectedItem.credit_number || selectedItem.id}</h2>
              ...
              {detailTab === "details" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    ...
                    <div><div className="text-xs text-muted-foreground mb-1">סיבה</div><div className="text-sm text-white">{reasonMap[selectedItem.reason] || selectedItem.reason || "-"}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">חשבונית מקורית</div><div className="text-sm text-white">{selectedItem.original_invoice_number || "-"}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">תאריך</div><div className="text-sm text-white">{selectedItem.credit_date}</div></div>
                  </div>
                </div>
              )}
