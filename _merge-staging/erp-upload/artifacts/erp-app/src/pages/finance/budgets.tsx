            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-white text-sm">{d.dept}</span>
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                <div><p className="text-[10px] text-muted-foreground">מתוקצב</p><p className="text-xs font-bold text-white">₪{fmtK(d.budgeted)}</p></div>
          ...
          {rows.map((row, idx) => {
            ...
            return (
              <tr key={idx} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                <td className="px-3 py-2 sticky right-0 bg-[#1e293b]">
                  <div className="font-medium text-white truncate max-w-[110px]" title={row.name}>{row.name}</div>
                ...
                {Array.from({ length: 12 }, (_, i) => {
                  ...
                  return (
                    <td key={i} className="px-2 py-1 text-center">
                      <div className="font-mono text-[10px] text-white">₪{fmtK(m.budgeted)}</div>
                ...
                <td className="px-3 py-2 text-right">
                  <div className="font-mono text-white font-bold text-[11px]">₪{fmtK(yearBudgeted)}</div>
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg sm:text-2xl font-bold text-white">תקציבים</h1>
          ...
          <Card className="bg-slate-800/50 border-slate-700">
            <CardContent className="p-4">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-emerald-900/30 rounded-lg p-3 border border-emerald-700/50"><div className="text-[10px] text-muted-foreground mb-1">מתוקצב: חודש נוכחי מול קודם</div><div className="text-lg font-bold text-white">{fmt(curM.budgeted)}</div><div className="text-xs text-muted-foreground">מול {fmt(prevM.budgeted)}</div><Arrow val={pctChange(curM.budgeted, prevM.budgeted)} /></div>
                ...
                <div className="bg-purple-900/30 rounded-lg p-3 border border-purple-700/50"><div className="text-[10px] text-muted-foreground mb-1">מתוקצב: רבעון נוכחי מול קודם</div><div className="text-lg font-bold text-white">{fmt(curQ.budgeted)}</div><div className="text-xs text-muted-foreground">מול {fmt(prevQ.budgeted)}</div><Arrow val={pctChange(curQ.budgeted, prevQ.budgeted)} /></div>
                <div className="bg-orange-900/30 rounded-lg p-3 border border-orange-700/50"><div className="text-[10px] text-muted-foreground mb-1">שורות: חודש נוכחי מול קודם</div><div className="text-lg font-bold text-white">{curM.count}</div><div className="text-xs text-muted-foreground">מול {prevM.count}</div><Arrow val={pctChange(curM.count, prevM.count)} /></div>
              </div>
            </CardContent>
          </Card>
        );
      ...
      <div className="flex gap-2 border-b border-slate-700">
        {([
          ...
          <button key={key} onClick={() => setViewTab(key)} className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium border-b-2 ${viewTab === key ? "border-amber-500 text-amber-400" : "border-transparent text-muted-foreground hover:text-white"}`}>
      ...
      {viewTab === "list" && (<>
        <BulkActions items={rows} selectedIds={selectedIds} onSelectionChange={setSelectedIds} actions={[
          defaultBulkActions.delete(async (ids) => { for (const id of ids) await authFetch(`${API}/finance/budgets/${id}`, { method: "DELETE" }); qc.invalidateQueries({ queryKey: ["budgets"] }); }),
                ...
                {rows.length === 0 ? (
                  ...
                  return (
                    <tr key={r.id} className="border-b border-slate-700/50 hover:bg-slate-700/30 cursor-pointer" onClick={() => { setSelectedItem(r); setDetailTab("details"); }}>
                      ...
                      <td className="p-3 text-white font-medium">{r.budget_name || "-"}</td>
                      ...
                      <td className="p-3 font-mono text-white">{fmt(budgeted)}</td>
                      <td className="p-3 font-mono text-white">{fmt(actual)}</td>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">תקציב: {selectedItem.budget_name}</h2>
              ...
              {detailTab === "details" && (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <div><div className="text-xs text-muted-foreground mb-1">שם</div><div className="text-sm text-white font-medium">{selectedItem.budget_name}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">תקופה</div><div className="text-sm text-white">{MONTHS[selectedItem.fiscal_month]} {selectedItem.fiscal_year}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">מחלקה</div><div className="text-sm text-white">{selectedItem.department || "-"}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">קטגוריה</div><div className="text-sm text-white">{selectedItem.category || "-"}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">מתוקצב</div><div className="text-sm text-white font-bold">{fmt(Number(selectedItem.budgeted_amount || 0))}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">בפועל</div><div className="text-sm text-white font-bold">{fmt(Number(selectedItem.actual_amount || 0))}</div></div>
                  </div>
                  {(() => {
