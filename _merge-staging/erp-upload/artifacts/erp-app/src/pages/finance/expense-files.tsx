              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-white text-xs truncate max-w-[200px]">{r.file_name || `מסמך #${r.id}`}</td>
                    <td className="px-4 py-3 text-muted-foreground">{r.vendor_name || r.approved_by || "—"}</td>
                    <td className="px-4 py-3 text-white font-bold">{fmtCurrency(r.amount)}</td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{viewDetail.file_name || `מסמך #${viewDetail.id}`}</h2>
