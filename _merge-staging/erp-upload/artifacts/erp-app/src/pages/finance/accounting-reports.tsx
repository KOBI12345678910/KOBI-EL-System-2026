...
const REPORT_SECTIONS = [
  ...
  },
  {
    ...
    reports: [
      ...
      { id: "export-hashbeshbet", label: "יצוא לחשבשבת", icon: FileSpreadsheet, href: "/finance/accounting-settings", desc: "ייצוא נתונים לתוכנת חשבשבת" },
        ...
        <table className="w-full text-sm">
          ...
          <tfoot>
            <tr className="bg-slate-800/50 font-bold">
              <td className="p-3 text-white">סה"כ</td>
              <td className="p-3 text-white">{items.length}</td>
      ...
      return (
        <table className="w-full text-sm">
          ...
          <tbody>
            {grouped.map(([name, g]) => (
              <tr key={name} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                <td className="p-3 text-white font-medium">{payLabels[name] || name}</td>
                <td className="p-3 text-slate-300">{g.items.length}</td>
                <td className="p-3 text-green-400 font-medium">{fmt(g.total)}</td>
              </tr>
            ))}
    ...
    if (reportId === "payments-awaiting-invoice" || reportId === "open-payment-requests") {
      ...
      return (
        <table className="w-full text-sm">
          ...
          <tbody>
            {pending.map((item: any) => (
              <tr key={item.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                <td className="p-3 text-white font-medium">{item.supplier_name || "—"}</td>
          ...
          <tfoot>
            <tr className="bg-slate-800/50 font-bold">
              <td className="p-3 text-white" colSpan={2}>סה"כ</td>
              ...
              <tbody>
                {dupes.map(([key, arr]) => (
                  <tr key={key} className="border-b border-slate-800/50 hover:bg-slate-800/30 bg-red-900/10">
                    <td className="p-3 text-white font-medium">{arr[0].supplier_name || "—"}</td>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {items.map((item: any) => {
            ...
            return (
              <tr key={item.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                <td className="p-3 text-white">{item.document_number || item.invoice_number || item.id}</td>
        ...
        <tfoot>
          <tr className="bg-slate-800/50 font-bold">
            <td className="p-3 text-white" colSpan={3}>סה"כ ({items.length} רשומות)</td>
            <td className="p-3 text-green-400">{fmt(items.reduce((s: number, i: any) => s + getAmount(i), 0))}</td>
            <td className="p-3"></td>
          </tr>
        </tfoot>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-white">{reportInfo?.label}</h2>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 bg-slate-800 rounded-lg px-3 py-1.5 border border-slate-700">
            ...
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="bg-transparent text-sm text-white border-0 outline-none" />
            <span className="text-xs text-muted-foreground">עד:</span>
            <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="bg-transparent text-sm text-white border-0 outline-none" />
          </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="relative w-72">
          ...
          <Input
            ...
            className="pr-9 bg-slate-800 border-slate-700 text-white placeholder:text-muted-foreground"
          />
        </div>
      </div>
              ...
              {section.reports.map((report) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <div className="text-white font-medium text-sm">{report.label}</div>
