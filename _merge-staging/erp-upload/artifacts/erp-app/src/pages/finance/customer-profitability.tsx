...
import { authFetch } from "@/lib/utils";
    ...
    const STATUSES = ["רווחי מאוד", "רווחי", "שולי", "הפסדי"] as const;
    const SC: Record<string, string> = { "רווחי מאוד": "bg-emerald-500/20 text-emerald-300", "רווחי": "bg-green-500/20 text-green-300", "שולי": "bg-yellow-500/20 text-yellow-300", "הפסדי": "bg-red-500/20 text-red-300" };
    const CATS = ["VIP","עסקי","פרטי","ממשלתי","קבלן"];
...
export default function CustomerProfitability() {
  ...
  const [search, setSearch] = useState("");
    ...
    try {
      setError(null);
      const res = await authFetch("/api/finance-control/customer-profitability");
      if (res.ok) {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.customerName?.toLowerCase().includes(s) || r.customerType?.toLowerCase().includes(s)); }
        if (statusFilter !== "all") d = d.filter(r => r.status === statusFilter);
      if (catFilter !== "all") d = d.filter(r => r.customerType === catFilter);
      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingUp className="h-6 w-6 text-emerald-400" />רווחיות לקוחות</h1><p className="text-sm text-muted-foreground mt-1">ניתוח רווחיות לפי לקוח — הכנסות, עלויות ומרווח</p></div>
            <div className="flex gap-2">
              ...
              <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2"><Plus className="h-4 w-4" />ניתוח חדש</Button>
            </div>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"לקוחות","v":"85","c":"text-emerald-400","t":"+5","u":true},{"l":"רווחיים","v":"68","c":"text-green-400","t":"+3","u":true},{"l":"הפסדיים","v":"8","c":"text-red-400","t":"-2","u":true},{"l":"רווחיות ממוצעת","v":"28%","c":"text-blue-400","t":"+2%","u":true},{"l":"Top 10","v":"65%","c":"text-cyan-400","t":"+3%","u":true},{"l":"CLV ממוצע","v":"₪180K","c":"text-amber-400","t":"+12%","u":true}].map((k: any, i: number) => (
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("customerType")}>סוג<SI field="customerType" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("grossProfit")}>רווח<SI field="grossProfit" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("orders")}>הזמנות<SI field="orders" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("clv")}>CLV<SI field="clv" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingUp className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין רווחיות לקוחות"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "ניתוח חדש ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />ניתוח חדש</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 text-muted-foreground">{row.customerType}</td>
            <td className="p-3 font-mono text-emerald-400">{fmt(row.revenue)}</td>
            <td className="p-3 font-mono text-green-400">{fmt(row.grossProfit)}</td>
            <td className="p-3 font-mono text-cyan-400">{row.margin}%</td>
            <td className="p-3 font-mono text-muted-foreground text-center">{row.orders}</td>
            <td className="p-3 font-mono text-amber-400">{fmt(row.clv)}</td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]"><h2 className="text-lg font-bold text-white">{editId ? "עריכה" : "ניתוח חדש"}</h2><Button variant="ghost" size="sm" onClick={()=>{setShowCreate(false);setEditId(null)}}><X className="h-4 w-4" /></Button></div>
            ...
            <div><Label className="text-muted-foreground text-xs">סוג *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>VIP</option><option>עסקי</option><option>פרטי</option><option>ממשלתי</option><option>קבלן</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">עלויות</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">תקופה</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>Q1 2026</option><option>Q2 2026</option><option>שנתי</option></select></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>רווחי מאוד</option><option>רווחי</option><option>שולי</option><option>הפסדי</option></select></div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">סוג</p><p className="text-white mt-1 font-medium">{dr.customerType}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">עלויות</p><p className="text-white mt-1 font-medium">{fmt(dr.cost)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">רווח</p><p className="text-white mt-1 font-medium">{fmt(dr.grossProfit)}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">הזמנות</p><p className="text-white mt-1 font-medium">{dr.orders}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">ממוצע הזמנה</p><p className="text-white mt-1 font-medium">{fmt(dr.avgOrderValue)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">CLV</p><p className="text-white mt-1 font-medium">{fmt(dr.clv)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">שימור</p><p className="text-white mt-1 font-medium">{dr.retention}%</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">הזמנה אחרונה</p><p className="text-white mt-1 font-medium">{new Date(dr.lastOrder).toLocaleDateString("he-IL")}</p></div>
