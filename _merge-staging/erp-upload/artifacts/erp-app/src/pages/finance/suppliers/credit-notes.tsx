...
export default function SupplierCreditNotesPage() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ creditDate: new Date().toISOString().slice(0, 10), reason: "overcharge", status: "draft", currency: "ILS", vatRate: 18 });
    setShowForm(true);
  };
        ...
        <div className="border border-border/50 rounded-2xl bg-card/50 overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm">
          ...
          <tbody>{pagination.paginate(filtered).map(r => (
            <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
              ...
              <td className="px-4 py-3 text-white">{r.supplier_name}</td>
              ...
              <td className="px-4 py-3"><div className="flex gap-1">
                ...
                <button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.supplier_name || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1.5 hover:bg-muted rounded-lg"><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
              </div></td>
            </tr>
          ))}</tbody>
        </table></div></div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewDetail(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><RotateCcw className="w-5 h-5 text-purple-400" /> זיכוי {viewDetail.credit_number}</h2>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת זיכוי ספק" : "זיכוי ספק חדש"}</h2>
              ...
              <div className="p-5 space-y-4"><div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">סכום (₪)</label><input type="number" step="0.01" value={form.subtotal || ""} onChange={e => { const sub = Number(e.target.value) || 0; const vr = Number(form.vatRate) || 18; const va = sub * (vr / 100); setForm({ ...form, subtotal: e.target.value, vatAmount: Math.round(va * 100) / 100, totalAmount: Math.round((sub + va) * 100) / 100 }); }} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">מע"מ %</label><input type="number" step="0.01" value={form.vatRate ?? 18} onChange={e => setForm({ ...form, vatRate: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">זיכוי ספק #{selectedItem.credit_note_number || selectedItem.id}</h2>
              ...
              {detailTab === "details" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <div><div className="text-xs text-muted-foreground mb-1">ספק</div><div className="text-sm text-white">{selectedItem.supplier_name || "-"}</div></div>
                    ...
                    <div><div className="text-xs text-muted-foreground mb-1">תאריך</div><div className="text-sm text-white">{selectedItem.credit_note_date || selectedItem.created_at}</div></div>
                    ...
                    <div><div className="text-xs text-muted-foreground mb-1">סיבה</div><div className="text-sm text-white">{selectedItem.reason || "-"}</div></div>
                  </div>
                </div>
              )}
