...
import { authFetch } from "@/lib/utils";
    ...
    const STATUSES = ["תחרותי", "סביר", "יקר", "יקר מאוד"] as const;
...
MOCK.forEach(r => { r.variance = Number(((r.unitPrice - r.marketPrice) / r.marketPrice * 100).toFixed(1)); });
  ...
  const [search, setSearch] = useState("");
    ...
    try {
      setError(null);
      const res = await authFetch("/api/finance-control/supplier-cost-analysis");
      if (res.ok) {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.supplier?.toLowerCase().includes(s) || r.category?.toLowerCase().includes(s) || r.material?.toLowerCase().includes(s)); }
      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingDown className="h-6 w-6 text-orange-400" />ניתוח עלויות ספקים</h1><p className="text-sm text-muted-foreground mt-1">השוואת עלויות ספקים, מגמות וחלופות</p></div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"ספקים","v":"42","c":"text-orange-400","t":"+3","u":true},{"l":"עלות כוללת","v":"₪6.8M","c":"text-red-400","t":"+5%","u":false},{"l":"חיסכון פוטנציאלי","v":"₪320K","c":"text-green-400","t":"+15%","u":true},{"l":"ממוצע מחיר","v":"-3.2%","c":"text-emerald-400","t":"-1%","u":true},{"l":"ספקים תחרותיים","v":"28","c":"text-blue-400","t":"+5","u":true},{"l":"חלופות","v":"3.5","c":"text-cyan-400","t":"+0.5","u":true}].map((k: any, i: number) => (
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("unitPrice")}>מחיר יחידה<SI field="unitPrice" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("marketPrice")}>מחיר שוק<SI field="marketPrice" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("totalSpend")}>סה"כ<SI field="totalSpend" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingDown className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין ניתוח עלויות ספקים"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "ניתוח חדש ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />ניתוח חדש</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 "><div className="font-medium text-white">{row.supplier}</div></td>
            <td className="p-3 text-muted-foreground">{row.category}</td>
            <td className="p-3 text-muted-foreground text-xs">{row.material}</td>
            <td className="p-3 font-mono text-muted-foreground">{fmt(row.unitPrice)}</td>
            <td className="p-3 font-mono text-cyan-400">{fmt(row.marketPrice)}</td>
            <td className="p-3 "><span className={`font-mono ${row.variance<=0?"text-green-400":"text-red-400"}`}>{row.variance}%</span></td>
            <td className="p-3 font-mono text-orange-400">{fmt(row.totalSpend)}</td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div><Label className="text-muted-foreground text-xs">קטגוריה *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>חומרי גלם</option><option>ציוד</option><option>שירותים</option><option>הובלה</option><option>קבלנות</option></select></div>
            <div><Label className="text-muted-foreground text-xs">חומר</Label><Input type="text" placeholder="שם החומר" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">מחיר יחידה</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">מחיר שוק</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>תחרותי</option><option>סביר</option><option>יקר</option><option>יקר מאוד</option></select></div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מחיר יחידה</p><p className="text-white mt-1 font-medium">{fmt(dr.unitPrice)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מחיר שוק</p><p className="text-white mt-1 font-medium">{fmt(dr.marketPrice)}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">סה"כ הוצאה</p><p className="text-white mt-1 font-medium">{fmt(dr.totalSpend)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">נפח</p><p className="text-white mt-1 font-medium">{dr.volume}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">איכות</p><p className="text-white mt-1 font-medium">{dr.qualityScore}%</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">אספקה</p><p className="text-white mt-1 font-medium">{dr.deliveryScore}%</p></div>
