  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
        className="w-full max-w-2xl bg-[#0d1117] border border-gray-800 rounded-2xl overflow-hidden max-h-[90vh] overflow-y-auto" dir="rtl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800 bg-purple-600/5">
          <h3 className="font-bold text-white">{template ? "עריכת תבנית" : "יצירת תבנית חדשה"}</h3>
          <button onClick={onClose} className="p-2 text-muted-foreground hover:text-white rounded-lg"><X className="w-4 h-4" /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">שם התבנית</label>
              <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-purple-500" required />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">מזהה (Slug)</label>
              <input value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value })} dir="ltr" className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-purple-500 text-left" required />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">קטגוריה</label>
              <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-purple-500">
            ...
            <div>
              ...
              <input value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-purple-500" />
            </div>
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">System Prompt</label>
            <textarea value={form.systemPrompt} onChange={e => setForm({ ...form, systemPrompt: e.target.value })} dir="ltr" rows={3} className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-purple-500 font-mono resize-none" />
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">User Prompt Template</label>
            <textarea value={form.promptTemplate} onChange={e => setForm({ ...form, promptTemplate: e.target.value })} dir="ltr" rows={4} className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-purple-500 font-mono resize-none" required />
          ...
          <div>
            ...
            <input value={form.variables} onChange={e => setForm({ ...form, variables: e.target.value })} dir="ltr" className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-purple-500 text-left font-mono" />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-800">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm text-muted-foreground hover:text-white rounded-xl">ביטול</button>
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm bg-purple-600 text-white rounded-xl hover:bg-purple-700 disabled:opacity-50">{saving ? "שומר..." : "שמור"}</button>
          </div>
        </form>
      </motion.div>
    </motion.div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-6">
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl font-black text-white tracking-tight">תבניות פרומפט</h1>
                <p className="text-muted-foreground text-xs">ספריית תבניות מוכנות לשימוש עם מנועי AI — טכנו-כל עוזי</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setEditingTemplate("new")} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-purple-600 text-white hover:bg-purple-700 transition-colors text-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="bg-[#0d1117] border border-gray-800 rounded-2xl p-4">
                <div className={`w-8 h-8 rounded-lg ${stat.bg} flex items-center justify-center mb-3`}><Icon className={`w-4 h-4 ${stat.color}`} /></div>
                <div className="font-black text-xl text-white mb-0.5">{stat.value}</div>
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="חיפוש תבנית..." value={search} onChange={e => setSearch(e.target.value)} className="bg-[#0d1117] border border-gray-800 rounded-xl pr-9 pl-4 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500/50 w-64" />
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-[#0d1117] border border-gray-800 rounded-2xl"><Code2 className="w-12 h-12 text-muted-foreground mx-auto mb-4" /><h3 className="text-white font-bold mb-2">אין תבניות</h3><p className="text-muted-foreground text-sm">צור תבנית פרומפט ראשונה</p></div>
        ) : (
            ...
            {filtered.map((tpl, i) => {
              ...
              return (
                <motion.div key={tpl.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                  className="bg-[#0d1117] border border-gray-800 rounded-2xl overflow-hidden cursor-pointer"
                        ...
                        <div className="px-4 py-3 space-y-3 bg-black/20">
                          {tpl.systemPrompt && (
                            <div>
                              ...
                              <p className="text-xs text-gray-300 bg-gray-900/50 rounded-lg px-3 py-2 border border-gray-800 font-mono whitespace-pre-wrap">{tpl.systemPrompt}</p>
                            </div>
                          )}
                          <div>
                            <span className="text-[10px] text-muted-foreground font-mono block mb-1">USER TEMPLATE</span>
                            <p className="text-xs text-gray-300 bg-gray-900/50 rounded-lg px-3 py-2 border border-gray-800 font-mono whitespace-pre-wrap">{tpl.promptTemplate}</p>
                          </div>
                          {tpl.variables && (
