...
import { Users, Plus, Edit2, Trash2, Shield, Search, UserPlus, Clock, Key, ChevronDown, Eye, Settings, Lock, Unlock, Check, X, UserCheck, Layers, FileText, Palette } from "lucide-react";
...
export default function UserManagementSection() {
  ...
  const { data: modules = [] } = useQuery<{ id: number; slug: string; name: string; nameHe: string | null; isActive: boolean }[]>({
    queryKey: ["platform-modules"],
    queryFn: async () => {
      const res = await fetch(`${API_BASE}/platform/modules`);
      if (!res.ok) return [];
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {!isSelf && (
                                <button
                                  ...
                                  title="מחק"
                                  onClick={async () => {
                                    if (globalConfirm(`למחוק את ${user.fullName || user.username}? פעולה זו לא ניתנת לביטול!`)) {
            ...
            {roles.map(role => {
              ...
              return (
                <Card key={role.id} className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold" style={{ backgroundColor: role.color || "#3b82f6" }}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {!(role as any).isSystem && (
                        <button className="p-1.5 hover:bg-red-500/10 rounded" onClick={async () => { if (globalConfirm(`למחוק את "${role.nameHe || role.name}"?`)) deleteRoleMutation.mutate(role.id); }}>
                  ...
                  <tr className="border-b border-border bg-muted/30">
                    ...
                    {roles.map(r => (
                      <th key={r.id} className="p-2 font-medium text-center min-w-[100px]">
                        <span className="inline-block px-2 py-0.5 rounded-full text-white" style={{ backgroundColor: r.color || "#3b82f6" }}>
                          {r.nameHe || r.name}
                        </span>
                      </th>
                    ))}
