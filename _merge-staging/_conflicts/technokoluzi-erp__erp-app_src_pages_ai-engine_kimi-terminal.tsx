...
function FileTreeNode({ item, depth, onSelect, expandedDirs, toggleDir }: { item: FileItem; depth: number; onSelect: (path: string) => void; expandedDirs: Set<string>; toggleDir: (path: string) => void }) {
  ...
  return (
    ...
    >
      ...
      <span className="truncate text-gray-300 group-hover:text-white">{item.name}</span>
    </button>
  );
}
...
const TERMINAL_QUICK_COMMANDS = [
  { label: "בריאות מערכת", cmd: "curl -s http://localhost:8080/api/health | node -e \"let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>console.log(JSON.stringify(JSON.parse(d),null,2)))\"" },
...
function DevTerminalPanel() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0d1117] font-mono text-xs">
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-gray-800 bg-[#0a0e14]">
      ...
      <div className="flex flex-wrap gap-1 px-2 py-1.5 border-b border-gray-800/50 bg-[#0a0e14]/50">
      ...
      <div className="flex items-center border-t border-gray-800 bg-[#0a0e14]">
        ...
        <input ref={inputRef}
          ...
          className="flex-1 bg-transparent text-gray-200 outline-none py-2 text-xs" placeholder="הקלד פקודה... (↑↓ היסטוריה, Ctrl+L ניקוי)"
        />
      </div>
    </div>
  );
            ...
            {(tables?.tables || []).map((t: DbTable) => (
              <button key={t.table_name} onClick={() => { setSelectedTable(t.table_name); setActiveView("schema"); }}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-300 group-hover:text-white">{t.table_name}</span>
                </div>
          ...
          <div className="flex flex-col h-full">
            <div className="p-3">
              ...
              <textarea value={sqlInput} onChange={e => setSqlInput(e.target.value)}
                ...
                className="w-full h-24 bg-[#0a0e14] text-green-300 font-mono text-xs p-3 rounded border border-gray-800 outline-none resize-none focus:border-cyan-500/50"
                placeholder="SELECT * FROM ... (Ctrl+Enter להרצה)" dir="ltr" />
              <div className="flex items-center gap-2 mt-2">
                <button onClick={runQuery} disabled={loading}
                  className="px-4 py-1.5 bg-emerald-600 text-white rounded text-xs hover:bg-emerald-500 disabled:opacity-50 flex items-center gap-1.5">
            ...
            {results && (
              <div className="flex-1 overflow-auto p-3 border-t border-gray-800">
                {results.error ? (
                    ...
                    {Array.isArray(results.rows) && results.rows.length > 0 && (
                      <table className="min-w-full text-[11px]" dir="ltr">
                        <thead><tr className="border-b border-gray-700">
                          {Object.keys(results.rows[0]).map(k => <th key={k} className="px-2 py-1.5 text-left text-muted-foreground font-medium bg-gray-800/30">{k}</th>)}
                        </tr></thead>
                        <tbody>
                          {results.rows.slice(0, 100).map((row: any, i: number) => (
                            <tr key={i} className="border-b border-gray-800/50 hover:bg-card/5">
        ...
        {activeView === "schema" && selectedTable && (
          <div className="overflow-y-auto h-full p-3">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-white font-medium text-sm">{selectedTable}</span>
              {schema?.rowCount && <span className="text-muted-foreground text-xs bg-gray-800 px-2 py-0.5 rounded">{schema.rowCount} rows</span>}
            </div>
  ...
  return (
    <div className="overflow-y-auto h-full p-4 space-y-4 text-xs">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold flex items-center gap-2"><Activity className="w-4 h-4 text-green-400" /> מוניטור מערכת v3.0</h3>
        <button onClick={() => { refetch(); refetchRoutes(); refetchDeep(); }} className="text-muted-foreground hover:text-white flex items-center gap-1 text-[10px]"><RefreshCw className="w-3 h-3" /> רענן הכל</button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3">
          ...
          <div className="text-white font-bold text-lg">{health?.database?.latencyMs || 0}<span className="text-xs font-normal text-muted-foreground">ms</span></div>
        ...
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3">
          ...
          <div className="text-white font-bold text-lg">{health?.server?.memoryMB || 0}<span className="text-xs font-normal text-muted-foreground">MB</span></div>
          <div className="h-1.5 bg-gray-800 rounded-full mt-1"><div className={`h-full rounded-full ${memPct > 80 ? "bg-red-500" : "bg-blue-500"}`} style={{ width: `${memPct}%` }} /></div>
        </div>
        <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3">
          ...
          <div className="text-white font-bold text-lg">{okRoutes}<span className="text-xs font-normal text-muted-foreground">/{totalRoutes}</span></div>
      ...
      {routeHealth?.results && (
        <div>
          ...
          <div className="space-y-0.5 bg-[#0a0e14] rounded-lg p-2 border border-gray-800">
            ...
            {health.topTables.slice(0, 10).map((t: any) => {
              ...
              return (
                <div key={t.table_name || t.relname} className="flex items-center gap-2 px-2 py-1">
                  ...
                  <div className="flex-1 h-1.5 bg-gray-800 rounded-full"><div className="h-full bg-cyan-500/60 rounded-full" style={{ width: `${(count / maxCount) * 100}%` }} /></div>
...
function DevQAPanel() {
  ...
  return (
    <div className="overflow-y-auto h-full p-4 space-y-4 text-xs">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold flex items-center gap-2"><FlaskConical className="w-4 h-4 text-amber-400" /> QA Dashboard</h3>
        <button onClick={runFullScan} disabled={scanning}
          className="px-4 py-1.5 bg-amber-600 hover:bg-amber-500 disabled:opacity-50 text-white rounded-lg flex items-center gap-1.5">
      ...
      {results?.summary && (
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 text-center">
            ...
            <div className="text-white font-bold text-xl">{results.summary.total}</div>
          </div>
          ...
          <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 text-center">
            ...
            <div className="text-white font-bold text-xl">{results.summary.avgLatency}<span className="text-xs font-normal">ms</span></div>
          </div>
      ...
      {Object.entries(categories).map(([cat, routes]) => {
        ...
        return (
          <div key={cat}>
            ...
            <div className="bg-[#0a0e14] rounded-lg border border-gray-800 overflow-hidden">
              {(routes as RouteHealth[]).map(r => (
                <div key={r.route} className={`flex items-center justify-between px-3 py-2 border-b border-gray-800/50 last:border-0 ${!r.ok ? "bg-red-500/5" : ""}`}>
                  <div className="flex items-center gap-2">
...
function DevVersionControlPanel() {
  ...
  return (
    <div className="flex h-full">
      <div className="w-72 border-l border-gray-800 overflow-y-auto bg-[#0d1117]">
        <div className="p-3 border-b border-gray-800">
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-white font-medium flex items-center gap-1.5"><GitBranch className="w-4 h-4 text-cyan-400" /> Version Control</h4>
            <button onClick={() => refetch()} className="text-muted-foreground hover:text-white"><RefreshCw className="w-3 h-3" /></button>
          </div>
          {gitData?.branches && (
            <div className="space-y-0.5">
              {gitData.branches.slice(0, 5).map((b: any) => (
                <div key={b.name} className="flex items-center gap-1.5 text-[10px] px-2 py-1 bg-gray-800/30 rounded">
        ...
        {diffFile ? (
          <div className="p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => { setDiffFile(null); setDiffContent(""); }} className="text-muted-foreground hover:text-white"><X className="w-3 h-3" /></button>
            </div>
            <pre className="text-[11px] font-mono whitespace-pre-wrap text-gray-300 bg-[#0d1117] rounded-lg p-3 border border-gray-800" dir="ltr">
              {diffContent.split("\n").map((line, i) => (
...
function DevPreviewPanel() {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-800 bg-[#0d1117]">
        <button onClick={() => iframeRef.current?.contentWindow?.location.reload()} className="text-muted-foreground hover:text-white"><RefreshCw className="w-3.5 h-3.5" /></button>
        <div className="flex-1 flex items-center bg-[#0a0e14] rounded border border-gray-700 px-2 gap-1">
          <Globe className="w-3 h-3 text-muted-foreground" />
          <input value={urlInput} onChange={e => setUrlInput(e.target.value)}
            onKeyDown={e => e.key === "Enter" && navigate()}
            className="flex-1 bg-transparent text-gray-300 text-xs py-1 outline-none font-mono" dir="ltr" />
        </div>
        <button onClick={navigate} className="px-3 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-500">Go</button>
      </div>
      <div className="flex items-center gap-1 px-3 py-1 border-b border-gray-800 bg-[#0a0e14] overflow-x-auto">
        {quickLinks.map(l => (
...
function DevDataFlowPanel() {
  ...
  return (
    <div className="overflow-y-auto h-full p-4 space-y-4 text-xs">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold flex items-center gap-2"><Network className="w-4 h-4 text-blue-400" /> זרימת נתונים בין מחלקות</h3>
        <button onClick={() => refetch()} className="text-muted-foreground hover:text-white"><RefreshCw className="w-3 h-3" /></button>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {Object.entries(stats).map(([dept, count]) => (
          <div key={dept} className="bg-gray-800/30 border border-gray-700/50 rounded-xl p-3 text-center">
            ...
            <div className="text-white font-bold">{count as number} חיבורים</div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2">
        ...
        {flows.map((f: any, i: number) => (
          <div key={i} className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${f.status === "active" ? "bg-[#0d1117] border-gray-800" : "bg-yellow-500/5 border-yellow-500/20"}`}>
            <div className={`px-2 py-1 rounded-lg bg-gradient-to-r ${deptColors[f.from] || "from-gray-500 to-gray-600"} text-white text-[11px] font-medium`}>
              {f.from}
            </div>
            <div className="flex items-center gap-1 text-muted-foreground">
              <ArrowRight className="w-4 h-4" />
            </div>
            <div className={`px-2 py-1 rounded-lg bg-gradient-to-r ${deptColors[f.to] || "from-gray-500 to-gray-600"} text-white text-[11px] font-medium`}>
              {f.to}
            </div>
...
אנא ספק את כל הקוד הנדרש עם הסברים.`;
  ...
  return (
    <div className="overflow-y-auto h-full p-4 space-y-4 text-xs">
      <h3 className="text-white font-semibold flex items-center gap-2"><Wand2 className="w-4 h-4 text-purple-400" /> בונה מודולים</h3>
      <div className="bg-[#0d1117] rounded-xl border border-gray-800 p-4 space-y-3">
        <div>
          ...
          <input value={moduleName} onChange={e => setModuleName(e.target.value)}
            className="w-full bg-[#0a0e14] text-white px-3 py-2 rounded-lg border border-gray-700 outline-none text-xs focus:border-purple-500/50" placeholder="לדוגמה: ניהול ספקים" />
        </div>
        <div>
          ...
          <textarea value={moduleDesc} onChange={e => setModuleDesc(e.target.value)}
            className="w-full h-20 bg-[#0a0e14] text-white px-3 py-2 rounded-lg border border-gray-700 outline-none resize-none text-xs focus:border-purple-500/50" placeholder="תאר את הפונקציונליות..." />
        </div>
        <div className="space-y-2">
          {[
            ...
            <label key={opt.id} className="flex items-center gap-3 text-gray-300 cursor-pointer p-2 hover:bg-card/5 rounded-lg">
              <input type="checkbox" checked={opt.checked} onChange={e => opt.set(e.target.checked)} className="rounded border-gray-600 w-4 h-4" />
              <div>
        ...
        <button onClick={generate} disabled={!moduleName.trim()}
          className="w-full py-2.5 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 disabled:opacity-50 text-white rounded-lg flex items-center justify-center gap-2 font-medium">
...
function DevBugScannerPanel({ onSendToChat }: { onSendToChat: (msg: string) => void }) {
  ...
  return (
    <div className="overflow-y-auto h-full p-4 space-y-4 text-xs">
      <h3 className="text-white font-semibold flex items-center gap-2"><Bug className="w-4 h-4 text-red-400" /> סורק באגים</h3>
      <button onClick={scan} disabled={scanning}
        className="w-full py-2.5 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 disabled:opacity-50 text-white rounded-lg flex items-center justify-center gap-2 font-medium">
      ...
      {scanResults && (
        <>
          <div className="flex items-center gap-3 p-3 bg-[#0d1117] rounded-xl border border-gray-800">
            <Gauge className="w-8 h-8 text-cyan-400" />
            <div>
              <div className="text-white font-bold text-lg">{passed}/{total} <span className="text-xs font-normal text-muted-foreground">תקינים</span></div>
              <div className="h-2 w-40 bg-gray-800 rounded-full mt-1">
                <div className={`h-full rounded-full ${passed === total ? "bg-green-500" : "bg-amber-500"}`} style={{ width: `${(passed / Math.max(total, 1)) * 100}%` }} />
              </div>
            </div>
          </div>

          <div className="space-y-0.5 bg-[#0a0e14] rounded-lg border border-gray-800 overflow-hidden">
            {scanResults.map(r => (
              <div key={r.route} className={`flex items-center gap-2 px-3 py-2 border-b border-gray-800/50 last:border-0 ${!r.ok ? "bg-red-500/5" : ""}`}>
                {r.ok ? <CheckCircle2 className="w-3.5 h-3.5 text-green-400" /> : <XCircle className="w-3.5 h-3.5 text-red-400" />}
          ...
          {scanResults.some(r => !r.ok) && (
            <button onClick={fixBugs}
              className="w-full py-2 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 text-white rounded-lg flex items-center justify-center gap-2 font-medium">
...
function DevDocsPanel() {
  ...
  return (
    <div className="overflow-y-auto h-full p-4 space-y-4 text-xs">
      <h3 className="text-white font-semibold flex items-center gap-2"><BookOpen className="w-4 h-4 text-blue-400" /> תיעוד מערכת</h3>
      ...
      <div className="bg-[#0d1117] rounded-xl border border-gray-800 p-4">
        ...
        <div className="space-y-0.5">
          {(data?.apiRoutes || []).map((route: string) => (
            <div key={route} className="flex items-center gap-2 px-3 py-1.5 bg-[#0a0e14] rounded font-mono text-[11px]">
      ...
      <div className="bg-[#0d1117] rounded-xl border border-gray-800 p-4">
        ...
        <div className="grid grid-cols-2 gap-1">
          {(data?.frontendPages || []).map((page: string) => (
            <div key={page} className="flex items-center gap-1.5 px-2 py-1 text-[10px] bg-[#0a0e14] rounded truncate">
...
export default function KimiTerminalPage() {
  ...
  return (
    <div className={`flex flex-col h-[calc(100vh-3.5rem)] bg-[#0b0f19] text-white ${isFullscreen ? "fixed inset-0 z-50 h-screen" : ""}`} dir="rtl">
      <div className="flex items-center border-b border-gray-800 bg-[#0a0e14] h-10 px-2 flex-shrink-0">
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-1.5 text-muted-foreground hover:text-white ml-1">
        ...
        <div className="flex items-center gap-1 mr-auto flex-shrink-0">
          <div className="relative">
            <button onClick={() => setModelOpen(!modelOpen)} className="flex items-center gap-1 px-2 py-1 rounded text-xs text-muted-foreground hover:text-white hover:bg-card/5">
              <Cpu className="w-3 h-3" /> {MODEL_LABELS[activeTab.model]?.split(" —")[0] || activeTab.model}
              <ChevronDown className="w-3 h-3" />
            </button>
            {modelOpen && (
              <div className="absolute left-0 top-full mt-1 bg-[#1c2333] border border-gray-700 rounded-lg shadow-xl z-50 min-w-[200px]">
          ...
          <button onClick={() => setIsFullscreen(!isFullscreen)} className="p-1.5 text-muted-foreground hover:text-white">
      ...
      <div className="flex-1 flex overflow-hidden">
        {sidebarOpen && (
          <div className="w-60 border-l border-gray-800 bg-[#0d1117] flex flex-col flex-shrink-0">
                ...
                <div className="p-2">
                  <div className="relative">
                    ...
                    <input value={agentSearch} onChange={e => setAgentSearch(e.target.value)}
                      className="w-full bg-[#161b22] text-xs text-white py-1.5 pr-7 pl-2 rounded border border-gray-700 outline-none" placeholder="חיפוש סוכן..." />
                  </div>
                </div>
                <div className="flex flex-wrap gap-1 px-2 pb-2">
                  {(agentsData?.categories || []).map(cat => (
                    <button key={cat} onClick={() => setSelectedCategory(selectedCategory === cat ? null : cat)}
                      className={`px-2 py-0.5 rounded-full text-[10px] border transition-colors ${selectedCategory === cat ? (CATEGORY_COLORS[cat]?.bg || "bg-gray-700") + " " + (CATEGORY_COLORS[cat]?.text || "text-white") + " " + (CATEGORY_COLORS[cat]?.border || "border-gray-600") : "bg-gray-800/50 text-muted-foreground border-gray-700 hover:text-gray-300"}`}>
                      {cat}
                    </button>
                  ))}
                </div>
                <div className="space-y-0.5 px-2">
                  {(agentsData?.agents || []).map(agent => (
                    <button key={agent.id} onClick={() => addTab(agent)}
                      ...
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm flex-shrink-0 ${CATEGORY_COLORS[agent.category]?.bg || "bg-gray-700"}`}>
                        {agent.icon || "🤖"}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="text-xs text-white truncate">{typeof agent.name === "string" ? agent.name : String(agent.name ?? "")}</div>
              ...
              <div className="flex-1 overflow-y-auto space-y-0.5 p-2">
                {(conversationsData?.conversations || []).map(conv => (
                  <button key={conv.id} onClick={() => loadConversation(conv)}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-xs text-white truncate">{typeof conv.title === "string" ? conv.title : String(conv.title ?? "")}</div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {activeWorkspace === "chat" && (
            <>
              <div className="flex items-center gap-0.5 px-2 py-1 border-b border-gray-800 bg-[#0d1117] overflow-x-auto">
                {tabs.map(t => (
                  <div key={t.id} onClick={() => setActiveTabId(t.id)}
                    className={`flex items-center gap-1 px-2.5 py-1 text-xs rounded cursor-pointer max-w-[150px] group ${t.id === activeTabId ? "bg-[#161b22] text-white border border-cyan-500/30" : "text-muted-foreground hover:text-gray-300 hover:bg-card/5"}`}>
              ...
              {allMessages.length > 0 && (
                <div className="flex items-center justify-between px-3 py-1 bg-[#0a0e14] border-b border-gray-800/50 text-[9px] text-muted-foreground">
                  <div className="flex items-center gap-3">
              ...
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
                  ...
                  <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
                    ...
                    {activeTab.agent && (
                      <div className="flex flex-col items-center gap-2 mt-2">
                        ...
                        <div className="text-sm text-white font-medium">{typeof activeTab.agent.name === "string" ? activeTab.agent.name : String(activeTab.agent.name ?? "")}</div>
                      ...
                      {hiddenCount > 0 && (
                        <div className="flex justify-center py-2">
                          <button
                            ...
                            className="text-xs text-muted-foreground bg-card/5 border border-gray-700 px-3 py-1.5 rounded-lg hover:bg-card/10 hover:text-white transition-colors"
                          >
                  ...
                  return (
                    <div key={globalIdx} className={`group flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
                        ...
                        {msg.role === "user" ? <span className="text-xs">👤</span> : msg.isExecution ? <Zap className="w-4 h-4 text-white" /> : <Bot className="w-4 h-4 text-white" />}
                      </div>
                      <div className={`relative max-w-[80%] rounded-xl px-4 py-2.5 text-sm ${
                        ...
                        {msg.isExecution && msg.actionResults ? (
                          ...
                          <div className="prose prose-invert prose-sm max-w-none prose-pre:bg-[#0a0e14] prose-pre:border prose-pre:border-gray-800">
                        ...
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute top-1 left-1 flex items-center gap-0.5">
                          ...
                          {!msg.isExecution && (
                            <button onClick={() => navigator.clipboard.writeText(msg.content)}
                              className="text-[9px] bg-gray-700/80 text-gray-300 px-1.5 py-0.5 rounded hover:bg-gray-600">
                          ...
                          <button onClick={() => setPinnedMessages(prev => { const n = new Set(prev); n.has(globalIdx) ? n.delete(globalIdx) : n.add(globalIdx); return n; })}
                            className={`text-[9px] px-1 py-0.5 rounded ${pinnedMessages.has(globalIdx) ? "bg-yellow-500/30 text-yellow-400" : "bg-gray-700/80 text-gray-300 hover:bg-gray-600"}`}>
                            📌
                          </button>
                        </div>
                ...
                {streaming && (
                  <div className="flex gap-3">
                    ...
                    <div className="max-w-[80%] bg-[#161b22] border border-gray-800 rounded-xl px-4 py-2.5 text-sm">
              ...
              <div className="border-t border-gray-800 p-3 bg-[#0d1117] relative">
                {showSlashMenu && (() => {
                  const cmds = [
                  ...
                  return cmds.length > 0 ? (
                    <div className="absolute bottom-full left-3 right-3 mb-1 bg-[#161b22] border border-gray-700 rounded-lg shadow-2xl max-h-[250px] overflow-y-auto z-50">
                      <div className="px-3 py-1.5 text-[10px] text-muted-foreground border-b border-gray-800">פקודות מהירות — הקלד / לחיפוש</div>
                      {cmds.map((c, ci) => (
                ...
                {activeTab.pendingImages.length > 0 && (
                  <div className="flex gap-2 px-2 py-1.5 bg-[#161b22] rounded-t-xl border border-b-0 border-gray-700 overflow-x-auto">
                    {activeTab.pendingImages.map((img, i) => (
                      <div key={i} className="relative group flex-shrink-0">
                        <img src={img} className="h-16 w-16 object-cover rounded-lg border border-gray-600" />
                        <button
                ...
                <div className={`flex items-center gap-2 bg-[#161b22] ${activeTab.pendingImages.length > 0 ? "rounded-b-xl border-t-0" : "rounded-xl"} border border-gray-700 px-3 py-1 focus-within:border-cyan-500/50`}>
                  <button
                  ...
                  <textarea ref={textareaRef} value={input} onChange={e => {
                    ...
                    className="flex-1 bg-transparent text-white text-sm outline-none resize-none min-h-[36px] max-h-[120px] py-1.5" placeholder="הקלד הודעה... (/ לפקודות מהירות, Ctrl+V להדבקת תמונה)"
          ...
          {activeWorkspace === "editor" && (
            <div className="flex flex-col h-full">
              {editorFile ? (
                <>
                  <div className="flex items-center justify-between px-3 py-2 bg-[#161b22] border-b border-gray-800 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={saveFile} disabled={!editorDirty}
                        className="px-3 py-1 bg-green-600 hover:bg-green-500 disabled:opacity-30 text-white rounded text-xs flex items-center gap-1">
                        שמור (Ctrl+S)
                      </button>
                      <button onClick={() => { const prompt = `נתח את הקוד הבא מ-${editorFile.path} והצע שיפורים:\n\n\`\`\`${editorFile.language}\n${editorContent.slice(0, 3000)}\n\`\`\``; sendMessage(prompt); }}
                        className="px-3 py-1 bg-purple-600/50 hover:bg-purple-600 text-white rounded text-xs flex items-center gap-1">
                        <Sparkles className="w-3 h-3" /> נתח עם AI
                      </button>
                      <button onClick={() => { setEditorFile(null); setEditorDirty(false); }} className="text-muted-foreground hover:text-white p-1"><X className="w-4 h-4" /></button>
                    </div>
                  </div>
                  <div className="flex-1">
          ...
          {activeWorkspace === "files" && (
            <div className="flex h-full">
              <div className="w-72 border-l border-gray-800 bg-[#0d1117]">
                ...
                {editorFile ? (
                  <div className="flex flex-col h-full w-full">
                    <div className="flex items-center justify-between px-3 py-2 bg-[#161b22] border-b border-gray-800">
                      ...
                      <button onClick={saveFile} disabled={!editorDirty}
                        className="px-3 py-1 bg-green-600 hover:bg-green-500 disabled:opacity-30 text-white rounded text-xs">שמור</button>
                    </div>
                    <div className="flex-1">
                      <Suspense fallback={<div className="flex items-center justify-center h-full"><Loader2 className="w-6 h-6 animate-spin text-cyan-400" /></div>}>
            ...
            <div className="flex-1 overflow-auto p-4 space-y-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-bold text-white">נחיל סוכנים — Multi-Agent Swarm</h2>
                </div>
              ...
              {!swarmPolling && (!swarmStatus || swarmStatus.status !== "running") && (
                <div className="space-y-3">
                  ...
                  {swarmTasks.map((t, i) => (
                    <div key={i} className="bg-[#161b22] border border-gray-800 rounded-lg p-3 space-y-2">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0">{i + 1}</div>
                        <input
                          value={t.agentName}
                          onChange={e => { const u = [...swarmTasks]; u[i] = { ...u[i], agentName: e.target.value }; setSwarmTasks(u); }}
                          className="bg-transparent text-sm text-white font-medium border-none outline-none flex-1"
                      ...
                      <textarea
                        ...
                        className="w-full bg-[#0d1117] text-sm text-gray-300 rounded px-3 py-2 border border-gray-700 resize-none focus:border-pink-500/50 focus:outline-none"
                        rows={2}
                      />
                    </div>
                  ))}
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 py-2.5 bg-gradient-to-r from-pink-600 to-purple-600 text-white rounded-lg font-bold text-sm hover:from-pink-500 hover:to-purple-500 disabled:opacity-30 flex items-center justify-center gap-2 transition-all">
                    ...
                    <button
                      ...
                      className="px-3 py-2 bg-gray-800 text-muted-foreground rounded-lg text-xs hover:bg-gray-700 hover:text-white">
                      תבנית מוכנה
                    </button>
                  </div>
                </div>
              ...
              {swarmStatus && swarmStatus.error && !swarmStatus.agents && (
                <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 flex items-center gap-3">
                  ...
                  <button onClick={() => setSwarmStatus(null)} className="mr-auto text-xs text-muted-foreground hover:text-white px-2 py-1 bg-gray-800 rounded">נסה שוב</button>
                </div>
              )}

              {swarmStatus && swarmStatus.agents && (
                <div className="space-y-3">
                  <div className="flex items-center gap-3 bg-[#161b22] border border-gray-800 rounded-lg p-3">
                    ...
                    {swarmStatus.status !== "running" && (
                      <button onClick={() => { setSwarmStatus(null); setSwarmSessionId(null); setSwarmExpanded(new Set()); }} className="text-xs text-muted-foreground hover:text-white px-2 py-1 bg-gray-800 rounded">
                        משימה חדשה
                      </button>
                    )}
                  </div>
                  ...
                  <div className="grid gap-2">
                    {(swarmStatus.agents || []).map((agent: any, idx: number) => (
                      <div key={idx} className="bg-[#161b22] border border-gray-800 rounded-lg overflow-hidden">
                        <div
                          ...
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0 ${
                            agent.status === "completed" ? "bg-green-600" : agent.status === "running" ? "bg-yellow-600 animate-pulse" : agent.status === "failed" ? "bg-red-600" : "bg-gray-700"
                          }`}>{idx + 1}</div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
