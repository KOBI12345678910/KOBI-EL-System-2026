...
export default function BuilderDashboard() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["platform-modules"] }); setShowCreate(false); },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["platform-modules"] }); setEditModule(null); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => authFetch(`${API}/platform/modules/${id}`, { method: "DELETE" }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["platform-modules"] }),
  });
                ...
                return (
                  <motion.div key={mod.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                    ...
                    <div className="flex items-center gap-2 pt-3 border-t border-border/50">
                      ...
                      <button onClick={async () => { if (globalConfirm("לשכפל את המודול?")) cloneMutation.mutate(mod.id); }} className="p-2 hover:bg-blue-500/10 rounded-lg transition-colors" title="שכפל מודול" disabled={cloneMutation.isPending}><Copy className="w-4 h-4 text-blue-400" /></button>
                      <button onClick={async () => { if (globalConfirm("למחוק את המודול?")) deleteMutation.mutate(mod.id); }} className="p-2 hover:bg-destructive/10 rounded-lg transition-colors" title="מחק"><Trash2 className="w-4 h-4 text-destructive" /></button>
                    </div>
                  </motion.div>
                );
              })}
