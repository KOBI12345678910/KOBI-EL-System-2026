    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 text-white p-6" dir="rtl">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
        ...
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder={"חיפוש..."} className="w-full bg-gray-800/50 border border-gray-700 rounded-lg pr-10 pl-4 py-2 text-sm" />
        </div>

        {tab === "finishes" && (
          <div className="bg-gray-800/30 border border-gray-700/50 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead><tr className="bg-gray-800/60 text-muted-foreground border-b border-gray-700">
                  ...
                  filteredFinishes.map((f: any) => (
                    <tr key={f.id} className="border-b border-gray-700/30 hover:bg-gray-700/20">
                      ...
                      <td className="p-3 flex gap-1">
                        <button onClick={() => { setForm({ id: f.id, finishName: f.finish_name, finishType: f.finish_type, costPerSqm: f.cost_per_sqm, status: f.status, notes: f.notes }); setShowForm(true); }} className="p-1 hover:bg-gray-600 rounded"><Edit2 className="w-4 h-4" /></button>
                ...
                {filteredColors.length === 0 ? <tr><td colSpan={7} className="p-8 text-center text-muted-foreground">{"לא נמצאו צבעים"}</td></tr> :
                  filteredColors.map((c: any) => (
                    <tr key={c.id} className="border-b border-gray-700/30 hover:bg-gray-700/20">
                      ...
                      <td className="p-3">{c.hex_value ? <div className="w-6 h-6 rounded border border-gray-600" style={{ backgroundColor: c.hex_value }} /> : "—"}</td>
                      <td className="p-3">{Number(c.surcharge_percent) > 0 ? `${c.surcharge_percent}%` : "—"}</td>
                      <td className="p-3 flex gap-1">
                        <button onClick={() => { setForm({ id: c.id, colorName: c.color_name, colorNameHe: c.color_name_he, hexValue: c.hex_value, surchargePercent: c.surcharge_percent, status: c.status, notes: c.notes }); setShowForm(true); }} className="p-1 hover:bg-gray-600 rounded"><Edit2 className="w-4 h-4" /></button>
        ...
        {showForm && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
            <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[{ key: "finishCode", label: "קוד" }, { key: "finishName", label: "שם" }, { key: "finishType", label: "סוג", type: "select", options: ["powder_coating", "anodizing", "pvdf", "wet_paint", "electrophoresis", "sublimation"] }, { key: "costPerSqm", label: "מחיר/מ\"ר", type: "number" }, { key: "status", label: "סטטוס", type: "select", options: ["active", "inactive"] }].map(f => (
                    <div key={f.key}><label className="block text-sm text-muted-foreground mb-1">{f.label}</label>
                      {f.type === "select" ? <select value={form[f.key] || ""} onChange={e => setForm({ ...form, [f.key]: e.target.value })} className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-sm">{f.options?.map(o => <option key={o} value={o}>{o}</option>)}</select>
                        : <input type={f.type || "text"} value={form[f.key] || ""} onChange={e => setForm({ ...form, [f.key]: e.target.value })} className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-sm" />}
                    </div>
                  ))}
                </div>
              ) : (
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={() => setShowForm(false)} className="px-4 py-2 bg-gray-700 rounded-lg">{"ביטול"}</button>
              </div>
            </div>
          </div>
        )}
