    ...
    <div className="min-h-screen" dir="rtl">
      <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex gap-1 bg-[#12141a] border border-gray-800 rounded-xl p-1">
            {tabs.map(t => (
              <button key={t.key} onClick={() => setViewMode(t.key)}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${viewMode === t.key ? "bg-indigo-600 text-white" : "text-muted-foreground hover:text-white"}`}>
        ...
        {viewMode === "dashboard" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {belowMin.length > 0 && (
              <div className="bg-[#1a1d23] border border-red-500/30 rounded-xl p-5">
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                  {belowMin.map(m => {
                    ...
                    return (
                      <div key={m.id} className="bg-[#12141a] rounded-lg p-3 flex items-center gap-4 cursor-pointer hover:bg-[#1e2128]" onClick={() => setSelectedMaterial(m)}>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2"><p className="text-white text-sm font-medium truncate">{m.materialName}</p><span className="text-muted-foreground text-xs font-mono">{m.materialNumber}</span></div>
            ...
            <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
              <h3 className="text-lg font-semibold text-white flex items-center gap-2 mb-4"><MapPin className="w-5 h-5 text-indigo-400" />מלאי לפי מחסן</h3>
                ...
                {warehouseCounts.map(([wh, data]) => {
                  ...
                  return (
                    <div key={wh} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-6 bg-[#12141a] rounded-full overflow-hidden relative">
            ...
            <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
              <h3 className="text-lg font-semibold text-white flex items-center gap-2 mb-4"><Layers className="w-5 h-5 text-amber-400" />ניתוח ABC — סיכום</h3>
                ...
                {[
                  ...
                  return (
                    <div key={a.cls} className={`border rounded-xl p-4 ${a.color}`}>
                      ...
                      <p className="text-white font-mono text-lg font-bold" dir="ltr">₪{Math.round(val).toLocaleString()}</p>
            ...
            <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
              <h3 className="text-lg font-semibold text-white flex items-center gap-2 mb-4"><BarChart3 className="w-5 h-5 text-cyan-400" />מלאי לפי קטגוריה</h3>
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {categoryCounts.map(([cat, data]) => {
                  ...
                  return (
                    <div key={cat} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-5 bg-[#12141a] rounded-full overflow-hidden">
            ...
            {recentTx.length > 0 && (
              <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5 lg:col-span-2">
                <h3 className="text-lg font-semibold text-white flex items-center gap-2 mb-4"><Activity className="w-5 h-5 text-emerald-400" />תנועות אחרונות</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-right text-sm">
                    ...
                    <tbody>{recentTx.slice(0, 15).map(tx => (
                      <tr key={tx.id} className="border-t border-gray-800/30">
                        <td className="py-2 pr-2 text-muted-foreground text-xs">{new Date(tx.createdAt).toLocaleDateString("he-IL")}</td>
                        <td className="py-2 pr-2"><span className="text-white text-sm">{getMaterialName(tx.materialId)}</span><span className="text-muted-foreground text-xs block font-mono">{getMaterialNumber(tx.materialId)}</span></td>
                        ...
                        <td className="py-2 pr-2 text-white font-mono">{tx.quantity}</td>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div className="relative flex-1 min-w-0 sm:min-w-[200px]">
                ...
                <input type="text" placeholder="חיפוש חומר..." value={search} onChange={e => setSearch(e.target.value)}
                  className="w-full pr-10 pl-4 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:border-indigo-500 focus:outline-none" />
              </div>
              <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="px-3 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white focus:border-indigo-500 focus:outline-none text-sm">
                <option value="all">כל הקטגוריות</option>
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <select value={warehouseFilter} onChange={e => setWarehouseFilter(e.target.value)} className="px-3 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white focus:border-indigo-500 focus:outline-none text-sm">
                <option value="all">כל המחסנים</option>
                {WAREHOUSES.map(w => <option key={w} value={w}>{w}</option>)}
              </select>
              {viewMode === "abc" && (
                <select value={abcFilter} onChange={e => setAbcFilter(e.target.value)} className="px-3 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white focus:border-indigo-500 focus:outline-none text-sm">
              ...
              <table className="w-full text-right">
                ...
                <tbody>
                  {filtered.map(m => {
                    ...
                    return (
                      <tr key={m.id} className={`border-b border-gray-800/50 hover:bg-[#1e2128] cursor-pointer transition-colors ${status.label === "אזל" || status.label === "מתחת למינימום" ? "bg-red-500/5" : ""}`}
                        onClick={() => setSelectedMaterial(m)}>
                        <td className="px-4 py-3 text-indigo-400 font-mono text-sm">{m.materialNumber}</td>
                        <td className="px-4 py-3 text-white text-sm">{m.materialName}</td>
                        <td className="px-4 py-3 text-muted-foreground text-sm">{m.category}</td>
                        <td className="px-4 py-3 text-white font-mono text-sm font-medium">{n(m.currentStock).toLocaleString()} <span className="text-muted-foreground text-xs">{m.unit}</span></td>
                        ...
                        <td className="px-4 py-3 text-white text-sm font-mono font-medium" dir="ltr">₪{Math.round(value).toLocaleString()}</td>
                        {viewMode === "abc" && (
              ...
              <table className="w-full text-right text-sm">
                ...
                <tbody>
                  {recentTx.filter(tx => {
                    ...
                    <tr key={tx.id} className="border-t border-gray-800/30 hover:bg-[#1e2128]">
                      <td className="px-4 py-3 text-muted-foreground text-xs">{new Date(tx.createdAt).toLocaleString("he-IL")}</td>
                      <td className="px-4 py-3"><span className="text-white">{getMaterialName(tx.materialId)}</span><span className="text-muted-foreground text-xs block font-mono">{getMaterialNumber(tx.materialId)}</span></td>
                      ...
                      <td className="px-4 py-3 text-white font-mono font-medium">{tx.quantity}</td>
        ...
        {selectedMaterial && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-3xl mx-4 mb-10" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <div>
                  <h2 className="text-xl font-bold text-white">{selectedMaterial.materialName}</h2>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setSelectedMaterial(null)} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
                </div>
              </div>
              ...
              <div className="p-6 space-y-5">
                ...
                {(() => {
                  ...
                  return (
                    <div className="bg-[#12141a] rounded-xl p-5 border border-gray-800">
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-muted-foreground text-sm">רמת מלאי</span>
                        <span className={`px-2 py-0.5 rounded-lg text-xs font-medium ${status.bg} ${status.color}`}>{status.label}</span>
                      </div>
                      <p className="text-4xl font-bold text-white font-mono mb-3">{n(selectedMaterial.currentStock).toLocaleString()} <span className="text-lg text-muted-foreground">{selectedMaterial.unit}</span></p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">מחיר תקן</p><p className="text-white font-medium mt-1 font-mono" dir="ltr">₪{n(selectedMaterial.standardPrice).toLocaleString()}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">שווי מלאי</p><p className="text-white font-medium mt-1 font-mono" dir="ltr">₪{Math.round(n(selectedMaterial.currentStock) * n(selectedMaterial.standardPrice)).toLocaleString()}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">מחסן</p><p className="text-white font-medium mt-1">{selectedMaterial.warehouseLocation || "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">ימי אספקה</p><p className="text-white font-medium mt-1">{selectedMaterial.leadTimeDays ? `${selectedMaterial.leadTimeDays} ימים` : "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">ספירה אחרונה</p><p className="text-white font-medium mt-1">{selectedMaterial.lastCountDate ? new Date(selectedMaterial.lastCountDate).toLocaleDateString("he-IL") : "לא נספר"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">כמות בספירה</p><p className="text-white font-medium mt-1">{selectedMaterial.lastCountQuantity || "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">קבלה אחרונה</p><p className="text-white font-medium mt-1">{selectedMaterial.lastReceiptDate ? new Date(selectedMaterial.lastReceiptDate).toLocaleDateString("he-IL") : "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">ניפוק אחרון</p><p className="text-white font-medium mt-1">{selectedMaterial.lastIssueDate ? new Date(selectedMaterial.lastIssueDate).toLocaleDateString("he-IL") : "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">שימוש שנתי</p><p className="text-white font-medium mt-1 font-mono" dir="ltr">₪{Math.round(n(selectedMaterial.annualUsageValue)).toLocaleString()}</p></div>
                </div>

                {selectedMaterial.description && (
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs mb-1">תיאור</p><p className="text-gray-300 text-sm">{selectedMaterial.description}</p></div>
                )}
                ...
                {(() => {
                  ...
                  return (
                    <div>
                      <h3 className="text-sm font-semibold text-white mb-2">תנועות אחרונות</h3>
                      <table className="w-full text-right text-sm">
                        <thead><tr className="text-muted-foreground border-b border-gray-800">
                        ...
                        <tbody>{matTx.map(tx => (
                          <tr key={tx.id} className="border-t border-gray-800/30">
                            ...
                            <td className="py-1.5 pr-2 text-white font-mono text-xs">{tx.quantity}</td>
