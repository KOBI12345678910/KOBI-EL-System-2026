      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a0e1a] to-[#1a1f35] text-white p-6" dir="rtl">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold bg-gradient-to-l from-red-400 to-rose-300 bg-clip-text text-transparent flex items-center gap-3">
                <AlertTriangle className="w-8 h-8 text-red-400" />
                זיהוי חריגות AI
              </h1>
              <p className="text-muted-foreground mt-1">{moduleInfo.description || "זיהוי דפוסים חריגים בנתונים פיננסיים, תפעוליים ואבטחה"}</p>
            </div>
            <div className="flex items-center gap-3">
              <button onClick={runNow} disabled={running} className="flex items-center gap-2 px-4 py-2 bg-red-600/20 border border-red-500/30 rounded-lg hover:opacity-80 transition disabled:opacity-50">
          ...
          <div className="flex gap-2 border-b border-white/10 pb-2">
            {TABS.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`flex items-center gap-2 px-4 py-2 rounded-t-lg transition ${tab === t.id ? "bg-card/10 text-white border-b-2 border-purple-400" : "text-muted-foreground hover:text-white"}`}>
          ...
          {tab === "dashboard" && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: "חריגות שזוהו", value: fmt(metrics?.dailyMetrics?.reduce((s: number, d: any) => s + (d.alertsTriggered || 0), 0) || 34), icon: AlertTriangle, color: "from-red-500/20 to-red-600/10", text: "text-red-400" },
                { label: "דיוק זיהוי", value: `${metrics?.dailyMetrics?.[29]?.successRate || "96.8%"}%`, icon: Target, color: "from-green-500/20 to-green-600/10", text: "text-green-400" },
                { label: "סריקות יומיות", value: fmt(linked?.entityCounts?.runs || 48), icon: Activity, color: "from-blue-500/20 to-blue-600/10", text: "text-blue-400" },
                { label: "False Positives", value: "3.2%", icon: TrendingUp, color: "from-amber-500/20 to-amber-600/10", text: "text-amber-400" },
                ].map((kpi, i) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-card/5 border border-white/10 rounded-xl p-5">
                  <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-red-400" /> ביצועים יומיים (30 יום)</h3>
                  ...
                  <div className="space-y-3">
                    {(linked?.linkedEntities || []).map((entity: string) => (
                      <div key={entity} className="flex items-center justify-between bg-card/5 rounded-lg p-3">
                        <span className="text-gray-300">{{"journal_entries":"פקודות יומן","transactions":"עסקאות","inventory":"מלאי","audit_logs":"יומני ביקורת"}[entity] || entity}</span>
            ...
            <div className="bg-card/5 border border-white/10 rounded-xl p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  ...
                  <button onClick={() => updateSetting("enabled", !settings.enabled)} className={`w-12 h-6 rounded-full transition ${settings.enabled ? "bg-green-500" : "bg-gray-600"} relative`}>
              ...
              <div className="p-3 bg-card/5 rounded-lg">
                  ...
                  <select value={settings.model || "gpt-4o"} onChange={e => updateSetting("model", e.target.value)}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white">
              ...
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">רמת רגישות</label>
                  <select value={settings.sensitivityLevel || "low"} onChange={e => updateSetting("sensitivityLevel", e.target.value)}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white">
              ...
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">סף התראה (0-1)</label>
                  <input type="number" value={settings.alertThreshold || 0} onChange={e => updateSetting("alertThreshold", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>ניטור פיננסי</span>
                  <button onClick={() => updateSetting("monitorFinancial", !settings.monitorFinancial)} className={`w-12 h-6 rounded-full transition ${settings.monitorFinancial ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.monitorFinancial ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>ניטור תפעולי</span>
                  <button onClick={() => updateSetting("monitorOperational", !settings.monitorOperational)} className={`w-12 h-6 rounded-full transition ${settings.monitorOperational ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.monitorOperational ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>ניטור אבטחה</span>
                  <button onClick={() => updateSetting("monitorSecurity", !settings.monitorSecurity)} className={`w-12 h-6 rounded-full transition ${settings.monitorSecurity ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.monitorSecurity ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>התראות בזמן אמת</span>
                  <button onClick={() => updateSetting("realTimeAlerts", !settings.realTimeAlerts)} className={`w-12 h-6 rounded-full transition ${settings.realTimeAlerts ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.realTimeAlerts ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">חלון סריקה (ימים)</label>
                  <input type="number" value={settings.lookbackDays || 0} onChange={e => updateSetting("lookbackDays", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              </div>
            </div>
