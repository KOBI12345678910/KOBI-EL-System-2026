...
async function analyzeBusinessData(): Promise<AlertData[]> {
  const alerts: AlertData[] = [];

  const overdueAP = await safeQuery(`
  ...
  const overdueAR = await safeQuery(`
    SELECT COUNT(*) as count, COALESCE(SUM(CAST(balance_due AS numeric)), 0) as total
  ...
  const pendingPO = await safeQuery(`
  ...
  const staleSuppliers = await safeQuery(`
    SELECT COUNT(*) as count
    FROM suppliers
