    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          ...
          <span className="mr-3 text-muted-foreground">טוען דוחות כספיים...</span>
        </div>
      ) : filtered.length === 0 ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewDetail(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><DollarSign className="w-5 h-5 text-green-400" />{viewDetail.label}</h2>
