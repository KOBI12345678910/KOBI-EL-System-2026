...
import { Briefcase, Search, Plus, Edit2, Trash2, X, Save, CheckCircle2, AlertTriangle, ArrowUpDown, Users, Target, TrendingUp, LayoutGrid, List, ChevronLeft, Eye, Star, Phone, Mail, UserCheck, UserX, FileText } from "lucide-react";
              ...
              {(byStage[stage.key] || []).map(c => (
                <motion.div key={c.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <div className="flex items-start justify-between gap-1 mb-1">
                    <span className="text-sm font-bold text-white leading-tight">{c.full_name}</span>
                    {c.rating > 0 && (
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div><div className="text-xs text-muted-foreground">משרה</div><div className="font-medium text-white">{candidate.position_applied || "—"}</div></div>
            <div><div className="text-xs text-muted-foreground">מחלקה</div><div className="text-white">{candidate.department || "—"}</div></div>
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
        ...
        <div className="flex gap-2">
          {mainTab === "candidates" ? (
            <>
              ...
              <button onClick={openCCreate} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2.5 rounded-xl hover:bg-blue-700 shadow-lg text-sm font-medium">
            ...
            <>
              ...
              <button onClick={openPCreate} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2.5 rounded-xl hover:bg-blue-700 shadow-lg text-sm font-medium">
      ...
      <div className="flex gap-1 bg-card border border-border/50 rounded-2xl p-1 w-fit">
        <button onClick={() => setMainTab("candidates")}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${mainTab === "candidates" ? "bg-blue-600 text-white shadow" : "text-muted-foreground hover:text-white"}`}>
          <Users className="w-4 h-4" /> מועמדים
        </button>
        <button onClick={() => setMainTab("positions")}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${mainTab === "positions" ? "bg-blue-600 text-white shadow" : "text-muted-foreground hover:text-white"}`}>
          ...
          <div className="flex gap-3 flex-wrap items-center">
            <div className="flex gap-1 bg-card border border-border/50 rounded-xl p-1">
              <button onClick={() => setCViewMode("kanban")}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm ${cViewMode === "kanban" ? "bg-blue-600 text-white" : "text-muted-foreground hover:text-white"}`}>
                <LayoutGrid className="w-3.5 h-3.5" /> קנבן
              </button>
              <button onClick={() => setCViewMode("list")}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm ${cViewMode === "list" ? "bg-blue-600 text-white" : "text-muted-foreground hover:text-white"}`}>
                      ...
                      return (
                        <tr key={c.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                          ...
                          <td className="px-4 py-3"><div className="flex gap-1">
                            ...
                            <button onClick={() => openCEdit(c)} className="p-1.5 hover:bg-muted rounded-lg"><Edit2 className="w-3.5 h-3.5 text-blue-400" /></button>
                    ...
                    : filteredPositions.map(r => {
                      ...
                      return (
                        <tr key={r.id} className={`border-b border-border/20 hover:bg-muted/20 transition-colors ${isOverdue ? "bg-red-500/5" : ""}`}>
                          ...
                          <td className="px-4 py-3 text-white font-medium whitespace-nowrap">{r.position_title}</td>
                          ...
                          <td className="px-4 py-3 font-bold text-white">{r.candidates_count || 0}</td>
                          ...
                          <td className="px-4 py-3"><div className="flex gap-1">
                            ...
                            <button onClick={() => openPEdit(r)} className="p-1.5 hover:bg-muted rounded-lg"><Edit2 className="w-3.5 h-3.5 text-blue-400" /></button>
                            <button onClick={async () => { if (await globalConfirm(`למחוק '${r.position_title}'?`)) { await authFetch(`${API}/recruitment/${r.id}`, { method: "DELETE" }); loadPositions(); } }} className="p-1.5 hover:bg-muted rounded-lg"><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
                          </div></td>
                        </tr>
                      );
                    })}
        ...
        {showCForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowCForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editCandidate ? "עריכת מועמד" : "מועמד חדש"}</h2>
              ...
              <div className="p-5 border-t border-border flex justify-end gap-2">
                ...
                <button onClick={saveCandidate} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700"><Save className="w-3.5 h-3.5 inline ml-1" /> {editCandidate ? "עדכון" : "שמירה"}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {showPForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowPForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editPosition ? "עריכת משרה" : "משרה חדשה"}</h2>
              ...
              <div className="p-5 border-t border-border flex justify-end gap-2">
                ...
                <button onClick={savePosition} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700"><Save className="w-3.5 h-3.5 inline ml-1" /> {editPosition ? "עדכון" : "שמירה"}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
