...
export default function OrderLifecyclePage() {
  const [data, setData] = useState<{ stages: Stage[]; timestamp: string } | null>(null);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await authFetch("/api/executive/order-lifecycle");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">מחזור חיי הזמנה — End-to-End</h1>
            ...
            return (
              <div key={stage.id} className="flex flex-col items-center">
                ...
                >
                  ...
                  <p className="text-sm font-medium text-white text-center">{stage.name}</p>
                  <p className={`text-2xl font-bold text-center mt-1 ${textColorMap[stage.color] || "text-white"}`}>
                    {stage.total}
                  </p>
              ...
              return rates.map((r, i) => (
                <div key={i} className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white w-8 text-left">{r.rate}%</span>
                  </div>
                </div>
              ));
            })()}
