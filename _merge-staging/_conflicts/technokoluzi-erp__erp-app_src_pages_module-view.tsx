...
export default function ModuleView() {
  ...
  const executeAction = async (action: any, record: any) => {
    ...
    switch (action.handlerType) {
      ...
      case "delete": if (record && globalConfirm("למחוק את הרשומה?")) deleteMutation.mutate(record.id); break;
    ...
    <div className="space-y-4">
      ...
      {selectedRows.size > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 bg-primary/5 border border-primary/20 rounded-xl">
          ...
          <button onClick={async () => { if (globalConfirm(`למחוק ${selectedRows.size} רשומות?`)) bulkDeleteMutation.mutate(Array.from(selectedRows)); }}
      ...
      {(() => {
        ...
        if (currentViewType === "cards" || currentViewType === "summary") return <SummaryCardsView records={records} fields={fields} statuses={statuses} entity={entity} activeView={activeView} entityId={eId} onViewRecord={setViewRecord} onEditRecord={setEditRecord} onDeleteRecord={(id) => deleteMutation.mutate(id)} />;
        return (
                      ...
                      <tr key={rec.id} className={`border-b border-border/50 last:border-0 transition-colors ${isSelected ? "bg-primary/5" : "hover:bg-muted/20"}`}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMutation.mutate(rec.id); }} className="p-1.5 hover:bg-destructive/10 rounded-lg">
