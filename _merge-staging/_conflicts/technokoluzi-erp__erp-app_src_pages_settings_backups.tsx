  ...
  return (
    <div className="p-6 space-y-4" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><Download className="h-6 w-6 text-teal-400" />גיבויים</h1><p className="text-sm text-muted-foreground mt-1">ניהול גיבויים, שחזור ותכנון</p></div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => load()} className="border-[#2a2a3e] text-gray-300 gap-1"><RefreshCw className="h-4 w-4" />רענן</Button>
          <Button onClick={() => triggerBackup("database")} disabled={triggering} variant="outline" size="sm" className="border-[#2a2a3e] text-cyan-300 gap-1">{triggering ? <Loader2 className="h-4 w-4 animate-spin" /> : <HardDrive className="h-4 w-4" />}גיבוי DB</Button>
      ...
      <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
        ...
        <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הסטטוסים</option><option value="completed">הושלם</option><option value="in_progress">בתהליך</option><option value="failed">נכשל</option><option value="pending">ממתין</option></select>
        <select value={typeFilter} onChange={e => { setTypeFilter(e.target.value); setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הסוגים</option><option value="full">מלא</option><option value="database">מסד נתונים</option><option value="configuration">תצורה</option><option value="incremental">אינקרמנטלי</option></select>
      ...
      <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-[#2a2a3e] bg-[#0a0a1a]/50">
        ...
        {pd.map(row => (
          <tr key={row.id} className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30 transition-colors">
            ...
            <td className="p-3"><Badge variant="outline" className="text-xs border-[#2a2a3e]">{TYPE_LABELS[row.backup_type] || row.backup_type}</Badge></td>
            <td className="p-3 font-mono text-white">{formatSize(Number(row.size_bytes))}</td>
      ...
      {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]"><div className="flex items-center gap-3"><h2 className="text-lg font-bold text-white">גיבוי #{dr.id}</h2><Badge className={`${SC[dr.status] || SC.pending} border-0`}>{STATUS_LABELS[dr.status] || dr.status}</Badge></div><Button variant="ghost" size="sm" onClick={() => setShowDetail(null)}><X className="h-4 w-4" /></Button></div>
        <div className="p-4">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">סוג</p><p className="text-white mt-1 font-medium">{TYPE_LABELS[dr.backup_type] || dr.backup_type}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">גודל</p><p className="text-white mt-1 font-medium">{formatSize(Number(dr.size_bytes))}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">משך</p><p className="text-white mt-1 font-medium">{formatDuration(Number(dr.duration_seconds))}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">הופעל ע&quot;י</p><p className="text-white mt-1 font-medium">{dr.triggered_by}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">תאריך יצירה</p><p className="text-white mt-1 font-medium">{new Date(dr.created_at).toLocaleString("he-IL")}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">התחלה</p><p className="text-white mt-1 font-medium">{new Date(dr.started_at).toLocaleString("he-IL")}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">סיום</p><p className="text-white mt-1 font-medium">{dr.completed_at ? new Date(dr.completed_at).toLocaleString("he-IL") : "—"}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">סטטוס</p><p className="text-white mt-1 font-medium">{STATUS_LABELS[dr.status] || dr.status}</p></div>
          </div>
          {dr.notes && <div className="bg-[#0a0a1a] rounded-lg p-3 mt-3"><p className="text-[11px] text-muted-foreground">הערות</p><p className="text-white mt-1">{dr.notes}</p></div>}
        </div>
        <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-end"><Button variant="outline" onClick={() => setShowDetail(null)} className="border-[#2a2a3e]">סגור</Button></div>
      </div></div>}
    </div>
  );
}
