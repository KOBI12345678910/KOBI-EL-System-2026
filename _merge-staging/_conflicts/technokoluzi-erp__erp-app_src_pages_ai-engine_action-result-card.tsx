...
function ExpandedContent({ r }: { r: ActionResult }) {
  ...
  if (r.result.rows && Array.isArray(r.result.rows) && r.result.rows.length > 0) {
    return (
      <div className="overflow-x-auto max-h-[250px] overflow-y-auto rounded border border-gray-800">
        <table className="w-full text-[10px] font-mono">
          <thead className="bg-gray-900/80 sticky top-0">
            <tr>{Object.keys(r.result.rows[0]).map((h, hi) => <th key={hi} className="px-2 py-1 text-right text-muted-foreground border-b border-gray-800 whitespace-nowrap">{h}</th>)}</tr>
          </thead>
          <tbody>
            {r.result.rows.slice(0, 50).map((row: any, ri: number) => (
              <tr key={ri} className={ri % 2 === 0 ? "bg-black/20" : "bg-black/10"}>
                {Object.values(row).map((v: any, vi) => <td key={vi} className="px-2 py-0.5 text-gray-300 border-b border-gray-800/50 whitespace-nowrap max-w-[200px] truncate">{String(v ?? "")}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
  ...
  if (r.result.fields && Array.isArray(r.result.fields) && r.result.fields.length > 0) {
    return (
      <div className="overflow-x-auto max-h-[250px] overflow-y-auto rounded border border-gray-800">
        <table className="w-full text-[10px] font-mono">
          <thead className="bg-gray-900/80 sticky top-0">
            <tr><th className="px-2 py-1 text-right text-muted-foreground border-b border-gray-800">שם</th><th className="px-2 py-1 text-right text-muted-foreground border-b border-gray-800">סוג</th><th className="px-2 py-1 text-right text-muted-foreground border-b border-gray-800">חובה</th></tr>
          </thead>
          <tbody>
            {r.result.fields.map((f: any, fi: number) => (
              <tr key={fi} className={fi % 2 === 0 ? "bg-black/20" : "bg-black/10"}>
                <td className="px-2 py-0.5 text-gray-300 border-b border-gray-800/50">{f.name || f.slug}</td>
                <td className="px-2 py-0.5 text-cyan-400 border-b border-gray-800/50">{f.type || f.fieldType}</td>
                <td className="px-2 py-0.5 border-b border-gray-800/50">{f.required ? "✓" : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  if (r.result.csv) {
    return (
      <div className="relative">
        ...
        <button onClick={() => navigator.clipboard.writeText(r.result.csv)} className="absolute top-1 left-1 text-[9px] bg-gray-700 text-gray-300 px-1.5 py-0.5 rounded hover:bg-gray-600">
    ...
    return (
      <div className="space-y-2 text-[10px]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-medium">{r.result.score.toUpperCase()}</span>
    ...
    return (
      <div className="space-y-2 text-[10px]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-medium">ציון כולל: {r.result.overallScore}%</span>
    ...
    return (
      <div className="space-y-2 text-[10px]">
        <div className="grid grid-cols-3 gap-2">
          {r.result.total !== undefined && <div className="bg-black/30 rounded p-1.5"><div className="text-muted-foreground">סה"כ</div><div className="text-white font-mono">{r.result.total}</div></div>}
          ...
          {r.result.min !== undefined && <div className="bg-black/30 rounded p-1.5"><div className="text-muted-foreground">מינימום</div><div className="text-white font-mono">{r.result.min}</div></div>}
          {r.result.max !== undefined && <div className="bg-black/30 rounded p-1.5"><div className="text-muted-foreground">מקסימום</div><div className="text-white font-mono">{r.result.max}</div></div>}
          {r.result.avg !== undefined && <div className="bg-black/30 rounded p-1.5"><div className="text-muted-foreground">ממוצע</div><div className="text-white font-mono">{r.result.avg}</div></div>}
        </div>
    ...
    return (
      <div className="overflow-x-auto max-h-[250px] overflow-y-auto rounded border border-gray-800">
        <table className="w-full text-[10px] font-mono">
          <thead className="bg-gray-900/80 sticky top-0">
            <tr>
              <th className="px-2 py-1 text-right text-muted-foreground border-b border-gray-800">פעולה</th>
              <th className="px-2 py-1 text-right text-muted-foreground border-b border-gray-800">טבלה</th>
              <th className="px-2 py-1 text-right text-muted-foreground border-b border-gray-800">רשומה</th>
              <th className="px-2 py-1 text-right text-muted-foreground border-b border-gray-800">זמן</th>
            </tr>
          </thead>
          <tbody>
            {r.result.entries.map((e: any, ei: number) => (
              <tr key={ei} className={ei % 2 === 0 ? "bg-black/20" : "bg-black/10"}>
                <td className="px-2 py-0.5 border-b border-gray-800/50">
                ...
                <td className="px-2 py-0.5 text-gray-300 border-b border-gray-800/50">{e.table_name}</td>
                <td className="px-2 py-0.5 text-muted-foreground border-b border-gray-800/50">#{e.record_id}</td>
                <td className="px-2 py-0.5 text-muted-foreground border-b border-gray-800/50">{new Date(e.created_at).toLocaleString("he-IL")}</td>
              </tr>
            ))}
          </tbody>
        </table>
            ...
            <div className="space-y-0.5">
              {r.result.outgoing.map((rel: any, ri: number) => (
                <div key={ri} className="flex items-center gap-2 bg-cyan-500/5 rounded px-2 py-0.5">
                  ...
                  <span className="text-white">{rel.fieldName || rel.slug}</span>
            ...
            <div className="space-y-0.5">
              {r.result.incoming.map((rel: any, ri: number) => (
                <div key={ri} className="flex items-center gap-2 bg-orange-500/5 rounded px-2 py-0.5">
                  ...
                  <span className="text-white">{rel.sourceName || `Entity#${rel.sourceEntityId}`}</span>
                  <span className="text-muted-foreground">→</span>
  ...
  return (
    <div className={`rounded-lg border p-2.5 transition-all ${r.success ? "border-emerald-500/30 bg-emerald-500/5 hover:border-emerald-500/50" : "border-red-500/30 bg-red-500/5 hover:border-red-500/50"}`}>
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setExpanded(!expanded)} className="text-muted-foreground hover:text-white p-0.5">
