...
import { authFetch } from "@/lib/utils";
    ...
    import { TrendingUp, Download, Plus, Search, Printer, Upload, MoreHorizontal, ChevronRight, ChevronLeft, ArrowUp, ArrowDown, X, Save, Eye, Edit2, Trash2, Copy, FileSpreadsheet, TrendingDown, ChevronsUpDown, Clock, CheckCircle2, Send, AlertCircle, Loader2, Package } from "lucide-react";

    const fmt = (v: number) => new Intl.NumberFormat("he-IL", { style: "currency", currency: "ILS", maximumFractionDigits: 0 }).format(v / 100);
    const STATUSES = ["תקין", "נמוך", "חסר", "עודף", "קפוא"] as const;
    const SC: Record<string, string> = { "תקין": "bg-green-500/20 text-green-300", "נמוך": "bg-yellow-500/20 text-yellow-300", "חסר": "bg-red-500/20 text-red-300", "עודף": "bg-blue-500/20 text-blue-300", "קפוא": "bg-gray-500/20 text-gray-300" };
    const CATS = ["חומרי גלם","מוצר מוגמר","חלפים","אריזה","כלי עבודה"];
...
export default function InventoryDashboard() {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.itemName?.toLowerCase().includes(s) || r.category?.toLowerCase().includes(s) || r.warehouse?.toLowerCase().includes(s)); }
      ...
      {error && <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-red-400"><AlertCircle className="h-4 w-4 flex-shrink-0" /><span className="text-sm">{error}</span><button onClick={() => setError(null)} className="mr-auto"><X className="h-4 w-4" /></button></div>}
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><TrendingUp className="h-6 w-6 text-teal-400" />לוח בקרת מלאי</h1><p className="text-sm text-muted-foreground mt-1">סקירת מלאי, רמות מינימום, הזמנות חוזרות והתראות</p></div>
            <div className="flex gap-2">
              ...
              <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2"><Plus className="h-4 w-4" />פריט חדש</Button>
            </div>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            {[
              { l: "פריטים במלאי", v: summary ? String(summary.totalItems) : "—", c: "text-teal-400", icon: <Package className="h-4 w-4 text-teal-400/60" /> },
              { l: "שווי מלאי כולל", v: summary ? new Intl.NumberFormat("he-IL", { style: "currency", currency: "ILS", maximumFractionDigits: 0 }).format(summary.totalInventoryValue / 100) : "—", c: "text-emerald-400", icon: <TrendingUp className="h-4 w-4 text-emerald-400/60" /> },
              { l: "מתחת למינימום", v: summary ? String(summary.belowMinimum) : "—", c: summary?.belowMinimum > 0 ? "text-red-400" : "text-green-400", icon: summary?.belowMinimum > 0 ? <TrendingDown className="h-4 w-4 text-red-400/60" /> : <TrendingUp className="h-4 w-4 text-green-400/60" /> },
              { l: "מחסנים פעילים", v: summary ? String(summary.totalWarehouses) : "—", c: "text-blue-400", icon: <Package className="h-4 w-4 text-blue-400/60" /> },
              { l: "תנועות (30 יום)", v: summary ? String(summary.recentMovements) : "—", c: "text-cyan-400", icon: <TrendingUp className="h-4 w-4 text-cyan-400/60" /> },
            ].map((k: any, i: number) => (
              <Card key={i} className="bg-card/80 border-border hover:border-border transition-colors"><CardContent className="p-4">{isLoading ? <div className="space-y-2 animate-pulse"><div className="h-3 w-16 bg-muted rounded" /><div className="h-6 w-20 bg-muted rounded" /><div className="h-3 w-12 bg-muted rounded" /></div> : <div className="flex items-start justify-between"><div><p className="text-[11px] text-muted-foreground">{k.l}</p><p className={`text-lg font-bold font-mono mt-1 ${k.c}`}>{k.v}</p></div><div className="mt-1">{k.icon}</div></div>}</CardContent></Card>
            ))}
          </div>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("currentQty")}>כמות<SI field="currentQty" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("minQty")}>מינימום<SI field="minQty" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("unitCost")}>עלות יחידה<SI field="unitCost" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("totalValue")}>שווי<SI field="totalValue" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-border/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-muted rounded" /><div className="h-4 w-16 bg-muted rounded" /><div className="h-4 w-32 bg-muted rounded" /><div className="h-4 w-24 bg-muted rounded" /><div className="h-4 w-20 bg-muted rounded" /><div className="h-4 w-16 bg-muted rounded" /><div className="h-4 w-28 bg-muted rounded" /><div className="h-4 w-12 bg-muted rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-muted-foreground" /> : <Package className="h-16 w-16 text-muted-foreground" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "אין פריטי מלאי — הוסף פריט ראשון"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "הוסף פריטי מלאי כדי לעקוב אחר הכמויות, הערכים וההתראות"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />פריט חדש</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 text-muted-foreground text-xs">{row.warehouse}</td>
            <td className="p-3 font-mono text-cyan-400 text-center">{row.currentQty}</td>
            <td className="p-3 font-mono text-muted-foreground text-center">{row.minQty}</td>
            <td className="p-3 font-mono text-muted-foreground">{fmt(row.unitCost)}</td>
            <td className="p-3 font-mono text-emerald-400">{fmt(row.totalValue)}</td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-card border border-border rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-border"><h2 className="text-lg font-bold text-foreground">{editId ? "עריכה" : "פריט חדש"}</h2><Button variant="ghost" size="sm" onClick={()=>{setShowCreate(false);setEditId(null)}}><X className="h-4 w-4" /></Button></div>
            <div className="p-4 space-y-6">
              <div className="border-b border-border pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי פריט</h3></div>
            <div className="grid grid-cols-3 gap-4">
            <div><Label className="text-muted-foreground text-xs">שם פריט *</Label><Input type="text" placeholder="שם הפריט" className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">קטגוריה *</Label><select className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1"><option>בחר...</option><option>חומרי גלם</option><option>מוצר מוגמר</option><option>חלפים</option><option>אריזה</option><option>כלי עבודה</option></select></div>
            <div><Label className="text-muted-foreground text-xs">מחסן</Label><select className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1"><option>בחר...</option><option>ראשי</option><option>חומרים</option><option>מוגמרים</option><option>חלפים</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">מינימום</Label><Input type="number" placeholder="0" className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">מקסימום</Label><Input type="number" placeholder="0" className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">יחידה</Label><select className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1"><option>בחר...</option><option>יח'</option><option>ק"ג</option><option>מ'</option><option>ליטר</option></select></div>
            <div><Label className="text-muted-foreground text-xs">עלות יחידה</Label><Input type="number" placeholder="0" className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">Lead Time (ימים)</Label><Input type="number" placeholder="0" className="bg-input border-border text-foreground mt-1" /></div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-card border border-border rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">כמות נוכחית</p><p className="text-foreground mt-1 font-medium">{dr.currentQty}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מינימום</p><p className="text-foreground mt-1 font-medium">{dr.minQty}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מקסימום</p><p className="text-foreground mt-1 font-medium">{dr.maxQty}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">עלות יחידה</p><p className="text-foreground mt-1 font-medium">{fmt(dr.unitCost)}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">שווי כולל</p><p className="text-foreground mt-1 font-medium">{fmt(dr.totalValue)}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">קבלה אחרונה</p><p className="text-foreground mt-1 font-medium">{new Date(dr.lastReceived).toLocaleDateString("he-IL")}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">Lead Time</p><p className="text-foreground mt-1 font-medium">{dr.leadTime} ימים</p></div>
