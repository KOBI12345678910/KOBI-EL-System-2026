...
function buildPasswordResetEmailContent(params: {
  ...
  const safeName = escapeHtml(params.fullName);
  const safeUsername = escapeHtml(params.username);
  const safePassword = escapeHtml(params.newPassword);
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:40px;">
            <h2 style="margin:0 0 16px;color:#1e293b;font-size:20px;">שלום ${safeName} 👋</h2>
            <p style="margin:0 0 24px;color:#475569;font-size:15px;line-height:1.7;">הסיסמה שלך אופסה בהצלחה. להלן פרטי הכניסה החדשים שלך:</p>
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:24px;">
              <tr>
                <td style="padding:16px 20px;border-bottom:1px solid #e2e8f0;">
                  <span style="color:#64748b;font-size:13px;">שם משתמש</span><br>
            ...
            <p style="margin:0;color:#94a3b8;font-size:13px;text-align:center;">מומלץ לשנות את הסיסמה לאחר הכניסה</p>
