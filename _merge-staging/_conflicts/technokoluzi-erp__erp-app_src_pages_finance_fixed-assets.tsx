...
import { Package, Search, Plus, Edit2, Trash2, X, Save, Hash, Calendar, CheckCircle2, DollarSign, ArrowUpDown, TrendingDown, MapPin, Wrench, AlertTriangle, Building2, Clock, BarChart3, Calculator, Layers , Loader2 } from "lucide-react";
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => printPage("רכוש קבוע")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Printer size={16} /> הדפסה</button>
          <button onClick={runDepreciation} className="flex items-center gap-1.5 bg-orange-600 text-white px-3 py-2 rounded-lg hover:bg-orange-700 text-sm"><Calculator size={16} /> חשב פחת</button>
          <button onClick={() => sendByEmail("רכוש קבוע - טכנו-כל עוזי", generateEmailBody("רכוש קבוע", items, { asset_number: "מספר", asset_name: "נכס", purchase_price: "עלות", current_value: "שווי", status: "סטטוס" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Send size={16} /> שליחה</button>
          <button onClick={openCreate} className="flex items-center gap-2 bg-teal-600 text-white px-3 py-2 rounded-lg hover:bg-teal-700 shadow-lg text-sm"><Plus size={16} /> נכס חדש</button>
        </div>
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button onClick={save} className="px-6 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 flex items-center gap-2"><Save size={16} /> {editing ? "עדכן" : "צור"}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">נכס: {selectedItem.asset_name} ({selectedItem.asset_number})</h2>
            ...
            <div className="p-6">
              {detailTab === "details" && (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <div><div className="text-xs text-muted-foreground mb-1">סוג נכס</div><div className="text-sm text-white">{assetTypeMap[selectedItem.asset_type] || selectedItem.asset_type}</div></div>
                  ...
                  <div><div className="text-xs text-muted-foreground mb-1">עלות רכישה</div><div className="text-sm text-white font-bold">₪{fmt(selectedItem.purchase_price || selectedItem.purchase_cost)}</div></div>
                  ...
                  <div><div className="text-xs text-muted-foreground mb-1">מיקום</div><div className="text-sm text-white">{selectedItem.location || "-"}</div></div>
                  <div><div className="text-xs text-muted-foreground mb-1">שיטת פחת</div><div className="text-sm text-white">{depMethodMap[selectedItem.depreciation_method] || selectedItem.depreciation_method || "-"}</div></div>
                  <div><div className="text-xs text-muted-foreground mb-1">סטטוס</div><div className="text-sm text-white">{statusMap[selectedItem.status]?.label || selectedItem.status}</div></div>
                </div>
              )}
