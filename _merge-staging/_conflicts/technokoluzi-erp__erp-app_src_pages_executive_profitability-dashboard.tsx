...
import { authFetch } from "@/lib/utils";
    ...
    const STATUSES = ["רווחי", "שולי", "הפסדי", "ללא נתון"] as const;
    const SC: Record<string, string> = { "רווחי": "bg-green-500/20 text-green-300", "שולי": "bg-yellow-500/20 text-yellow-300", "הפסדי": "bg-red-500/20 text-red-300", "ללא נתון": "bg-gray-500/20 text-gray-300" };
    const CATS = ["מוצר","לקוח","פרויקט","מחלקה","אזור"];
...
export default function ProfitabilityDashboard() {
  ...
  const [search, setSearch] = useState("");
    ...
    try {
      setError(null);
      const res = await authFetch("/api/executive/profitability");
      if (res.ok) {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.name?.toLowerCase().includes(s) || r.dimension?.toLowerCase().includes(s)); }
        if (statusFilter !== "all") d = d.filter(r => r.status === statusFilter);
      if (catFilter !== "all") d = d.filter(r => r.dimension === catFilter);
      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingUp className="h-6 w-6 text-emerald-400" />לוח רווחיות</h1><p className="text-sm text-muted-foreground mt-1">ניתוח רווחיות לפי מוצר, לקוח ופרויקט</p></div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"רווח גולמי","v":"33.5%","c":"text-emerald-400","t":"+2.1%","u":true},{"l":"רווח תפעולי","v":"18.2%","c":"text-green-400","t":"+1.5%","u":true},{"l":"רווח נקי","v":"12.8%","c":"text-blue-400","t":"+0.8%","u":true},{"l":"ROI","v":"22%","c":"text-cyan-400","t":"+3%","u":true},{"l":"EBITDA","v":"₪980K","c":"text-purple-400","t":"+12%","u":true},{"l":"מרווח תרומה","v":"45%","c":"text-amber-400","t":"+2%","u":true}].map((k: any, i: number) => (
          ...
          <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
            ...
            <select value={catFilter} onChange={e => { setCatFilter(e.target.value); setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הממדים</option>{CATS.map((o: string) => <option key={o} value={o}>{o}</option>)}</select>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("dimension")}>ממד<SI field="dimension" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("grossProfit")}>רווח גולמי<SI field="grossProfit" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("grossMargin")}>מרווח<SI field="grossMargin" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("roi")}>ROI<SI field="roi" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingUp className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין לוח רווחיות"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "ניתוח חדש ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />ניתוח חדש</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 text-muted-foreground">{row.dimension}</td>
            ...
            <td className="p-3 font-mono text-cyan-400">{row.grossMargin}%</td>
            <td className="p-3 font-mono text-blue-400">{fmt(row.netProfit)}</td>
            <td className="p-3 font-mono text-purple-400">{row.roi}%</td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div><Label className="text-muted-foreground text-xs">שם *</Label><Input type="text" placeholder="שם הפריט" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">ממד *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>מוצר</option><option>לקוח</option><option>פרויקט</option><option>מחלקה</option><option>אזור</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">עלות מכר</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">הוצאות תפעול</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">תקופה</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>Q1 2026</option><option>Q2 2026</option><option>Q3 2025</option><option>Q4 2025</option></select></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>רווחי</option><option>שולי</option><option>הפסדי</option><option>ללא נתון</option></select></div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">ממד</p><p className="text-white mt-1 font-medium">{dr.dimension}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">תקופה</p><p className="text-white mt-1 font-medium">{dr.period}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">עלות מכר</p><p className="text-white mt-1 font-medium">{fmt(dr.cogs)}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מרווח גולמי</p><p className="text-white mt-1 font-medium">{dr.grossMargin}%</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">הוצאות תפעול</p><p className="text-white mt-1 font-medium">{fmt(dr.opex)}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מרווח נקי</p><p className="text-white mt-1 font-medium">{dr.netMargin}%</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">ROI</p><p className="text-white mt-1 font-medium">{dr.roi}%</p></div>
