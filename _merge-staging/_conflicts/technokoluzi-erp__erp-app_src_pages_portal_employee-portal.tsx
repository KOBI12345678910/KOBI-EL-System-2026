...
export default function EmployeePortalPage() {
  ...
  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-blue-400">טוען...</div>;
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto">
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition ${activeTab === tab.id ? "bg-emerald-600 text-white" : "bg-slate-800/50 text-muted-foreground hover:text-white hover:bg-slate-700/50"}`}>
              {tab.label}
            </button>
          ))}
        </div>
            ...
            <form onSubmit={submitLeave} className="space-y-4">
              <div>
                ...
                <select value={leaveForm.leaveType} onChange={e => setLeaveForm(p => ({ ...p, leaveType: e.target.value }))}
                  className="w-full px-4 py-2.5 bg-slate-700/50 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-emerald-500">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-slate-300 mb-1">מתאריך</label>
                  <input type="date" value={leaveForm.startDate} onChange={e => setLeaveForm(p => ({ ...p, startDate: e.target.value }))}
                    className="w-full px-4 py-2.5 bg-slate-700/50 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-emerald-500" />
                </div>
                <div>
                  <label className="block text-sm text-slate-300 mb-1">עד תאריך</label>
              ...
              <div>
                ...
                <textarea value={leaveForm.reason} onChange={e => setLeaveForm(p => ({ ...p, reason: e.target.value }))}
                  className="w-full px-4 py-2.5 bg-slate-700/50 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-emerald-500" rows={2} />
              </div>
              <button type="submit" disabled={submitting} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-medium transition disabled:opacity-50">
        ...
        {activeTab === "reimbursement" && (
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6 max-w-lg">
            ...
            <form onSubmit={submitReimbursement} className="space-y-4">
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-1">סכום (ILS)</label>
