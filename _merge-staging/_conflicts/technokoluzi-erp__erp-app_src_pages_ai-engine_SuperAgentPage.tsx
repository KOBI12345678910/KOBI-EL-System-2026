...
export default function SuperAgentDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-gray-100 p-4 md:p-6" dir="rtl">
      <div className="max-w-7xl mx-auto">

        <div className="flex items-center gap-4 mb-6">
          <Link href="/ai-engine">
            <button className="p-2 rounded-lg bg-gray-800/60 hover:bg-gray-700/60 transition">
        ...
        <div className="flex gap-1 mb-6 bg-gray-800/30 rounded-xl p-1 border border-gray-700/40">
          {tabs.map(tab => (
            <button
              key={tab.id}
              ...
              {uniqueFlows.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {uniqueFlows.map(flow => {
                    ...
                    return (
                      <div key={flow} className={`bg-gradient-to-br ${meta?.color || "from-gray-700/20 to-gray-800/20"} border border-gray-700/30 rounded-xl p-4`}>
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-lg">{meta?.icon || "🔄"}</span>
                          <span className="text-sm font-medium text-gray-200">{meta?.label || flow}</span>
                        </div>
            ...
            <div className="bg-gray-800/30 border border-gray-700/40 rounded-xl p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {(modules.length > 0 ? modules : ["sales","finance","procurement","inventory","production","hr","projects","crm","marketing","import_export","maintenance","quality","documents","strategy","supply_chain","platform","portal","analytics","product_dev","pricing","calendar","integrations","ai_engine"]).map(m => (
                  <span key={m} className="px-3 py-1.5 bg-gray-700/30 border border-gray-600/30 rounded-lg text-xs text-gray-300 hover:border-cyan-500/40 transition cursor-default">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(FLOW_META).map(([key, flow]) => {
              ...
              return (
                <div
                  key={key}
                  className={`bg-gradient-to-br ${flow.color} border border-gray-700/30 rounded-xl p-5 hover:border-purple-500/40 transition cursor-pointer group`}
                  onClick={() => { setSelectedFlow(key); setFlowParams({}); setExecuteResult(null); setActiveTab("execute"); }}
                >
                  <div className="flex items-center gap-3 mb-3">
                    <span className="text-2xl">{flow.icon}</span>
                    <div>
                      <h3 className="font-semibold text-gray-100">{flow.label}</h3>
                  ...
                  <div className="text-xs text-gray-500">
                    <code className="bg-gray-800/50 px-2 py-0.5 rounded">{key}</code>
                  </div>
        ...
        {activeTab === "execute" && (
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="bg-gray-800/30 border border-gray-700/40 rounded-xl p-6">
              ...
              <div className="mb-4">
                ...
                <select
                  ...
                  className="w-full bg-gray-800/60 border border-gray-700/50 rounded-lg px-4 py-3 text-sm text-gray-200 focus:outline-none focus:border-purple-500/50 transition"
                >
                    ...
                    <div key={p.key}>
                      ...
                      {p.type === "select" ? (
                        <select
                          ...
                          className="w-full bg-gray-800/60 border border-gray-700/50 rounded-lg px-4 py-2.5 text-sm text-gray-200 focus:outline-none focus:border-purple-500/50"
                        >
                        ...
                        <input
                          ...
                          onChange={e => setFlowParams({ ...flowParams, [p.key]: e.target.value })}
                          placeholder={p.label}
                          className="w-full bg-gray-800/60 border border-gray-700/50 rounded-lg px-4 py-2.5 text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:border-purple-500/50"
                        />
                      )}
                    </div>
                  ))}
              ...
              <button
                ...
                className="w-full py-3 rounded-lg bg-gradient-to-l from-purple-600 to-violet-600 hover:from-purple-500 hover:to-violet-500 text-white font-medium flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed transition"
              >
                {executing ? (
                  <>
            ...
            {executeResult && (
              <div ref={resultRef} className="bg-gray-800/30 border border-gray-700/40 rounded-xl p-5">
                <h4 className="text-sm font-semibold text-gray-300 mb-3 flex items-center gap-2">
                  <FileText className="w-4 h-4 text-cyan-400" />
                  תוצאה
                </h4>
                <div className="bg-gray-900/50 rounded-lg p-4 text-sm text-gray-200 whitespace-pre-wrap leading-relaxed" dir="rtl">
                  {executeResult.split("\n").map((line, i) => {
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  onChange={() => {}}
                  className="w-full bg-gray-800/60 border border-gray-700/50 rounded-lg pr-10 pl-4 py-2.5 text-sm text-gray-200 placeholder-gray-500 focus:border-purple-500/50 focus:outline-none"
                />
              </div>
              <select
                value={filterFlow}
                onChange={e => setFilterFlow(e.target.value)}
                className="bg-gray-800/60 border border-gray-700/50 rounded-lg px-3 py-2.5 text-sm text-gray-300 focus:outline-none focus:border-purple-500/50"
              >
                  ...
                  {transactions.map(tx => {
                    ...
                    return (
                      <div key={tx.transaction_id}>
                        <div
                          className="flex items-center gap-3 px-5 py-3.5 hover:bg-gray-700/20 cursor-pointer transition"
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <span className="text-xs bg-gray-700/60 text-gray-300 px-2 py-0.5 rounded">
                        ...
                        {isExpanded && (
                          <div className="px-5 pb-4 bg-gray-900/30">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                              <div>
                                <p className="text-gray-500 text-xs mb-1">פרמטרים</p>
                                <pre className="text-xs text-gray-300 bg-gray-800/50 rounded-lg p-3 overflow-auto max-h-32 whitespace-pre-wrap" dir="ltr">
                                  {JSON.stringify(tx.params, null, 2)}
                                </pre>
                              </div>
                              <div>
                                <p className="text-gray-500 text-xs mb-1">תוצאה</p>
                                <div className="text-xs text-gray-300 bg-gray-800/50 rounded-lg p-3 overflow-auto max-h-32 whitespace-pre-wrap">
                                  {tx.error_message ? (
...
function TxRow({ tx }: { tx: Transaction }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-gray-800/30 rounded-lg px-4 py-2.5">
      {tx.status === "completed" ? (
      ...
      <span className="text-sm text-gray-200">{meta?.label || tx.flow_name}</span>
