...
function NodeCard({ node, isSelected, onClick, childCount }: { node: OrgNode; isSelected: boolean; onClick: () => void; childCount: number }) {
  return (
    <div
      onClick={onClick}
    ...
    >
      <div className="flex items-center gap-3">
        <div className="w-11 h-11 rounded-xl bg-primary/10 flex items-center justify-center text-sm font-bold text-primary shrink-0 relative">
          {node.name.charAt(0)}
          {childCount > 0 && (
            <span className="absolute -top-1 -left-1 w-4 h-4 rounded-full bg-blue-500 text-white text-[9px] flex items-center justify-center font-bold">
              {childCount}
            </span>
          )}
        </div>
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative">
        <NodeCard node={node} isSelected={selectedId === node.id} onClick={() => onSelect(node)} childCount={children.length} />
        {hasChildren && (
          <button
            ...
            <CardHeader className="pb-3">
              ...
              <div className="flex border-b border-border/50 mt-2">
                {[{key:"details",label:"פרטים"},{key:"related",label:"קשורות"},{key:"docs",label:"מסמכים"},{key:"history",label:"היסטוריה"}].map(t => (
                  <button key={t.key} onClick={() => setDetailTab(t.key)} className={`px-2 py-1.5 text-[10px] font-medium border-b-2 ${detailTab === t.key ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-white"}`}>{t.label}</button>
                ))}
              </div>
            </CardHeader>
              ...
              <div className="space-y-2">
                {selectedNode.employee_number && (
                  <div className="flex justify-between py-1.5 border-b border-border/20">
                    ...
                    <span className="text-sm text-white font-mono">{selectedNode.employee_number}</span>
                  </div>
                )}
                <div className="flex justify-between py-1.5 border-b border-border/20">
                ...
                {selectedNode.managerName && (
                  <div className="flex justify-between py-1.5 border-b border-border/20">
                    ...
                    <span className="text-sm text-white">{selectedNode.managerName}</span>
                  </div>
                )}
                <div className="flex justify-between py-1.5 border-b border-border/20">
