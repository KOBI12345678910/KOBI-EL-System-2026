...
import { Scale, Search, AlertTriangle, ArrowUpDown, Eye, X, TrendingUp, TrendingDown, BarChart3, Hash, Filter } from "lucide-react";
...
export default function TrialBalance() {
  const [items, setItems] = useState<TrialBalanceEntry[]>([]);
  ...
  const [viewDetail, setViewDetail] = useState<TrialBalanceEntry | null>(null);
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API}/trial-balance?period=${period}`);
      if (res.ok) {
        const d = await res.json();
        setItems(safeArray(d));
        setSummary(d?.summary || d?.totals || {});
      } else setError("שגיאה בטעינת מאזן בוחן");
  ...
  const totalDebit = items.reduce((s, i) => s + Number(i.period_debit || 0), 0);
  const totalCredit = items.reduce((s, i) => s + Number(i.period_credit || 0), 0);

  const kpis = [
    { label: "חשבונות", value: fmt(items.length), icon: Hash, color: "text-blue-400" },
    { label: "סה\"כ חובה", value: fmtCurrency(summary.totalDebit || totalDebit), icon: TrendingUp, color: "text-green-400" },
    { label: "סה\"כ זכות", value: fmtCurrency(summary.totalCredit || totalCredit), icon: TrendingDown, color: "text-red-400" },
    { label: "הפרש", value: fmtCurrency(summary.difference || (totalDebit - totalCredit)), icon: BarChart3, color: "text-amber-400" },
    { label: "מסוננים", value: fmt(filtered.length), icon: Filter, color: "text-cyan-400" },
  ];
  ...
  const columns = [
    ...
    { key: "period_debit", label: "חובה בתקופה" },
    { key: "period_credit", label: "זכות בתקופה" },
  ...
  return (
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-2">
            <Scale className="text-indigo-400 w-6 h-6" /> מאזן בוחן
          </h1>
          <p className="text-sm text-muted-foreground mt-1">סיכום יתרות חובה וזכות לכל החשבונות</p>
        </div>
        <ExportDropdown data={filtered} headers={{ account_number: "מספר", account_name: "שם", account_type: "סוג", period_debit: "חובה", period_credit: "זכות", current_balance: "יתרה" }} filename="trial_balance" />
      </div>
      ...
      <BulkActions selectedIds={selectedIds} onClear={clear} entityName="חשבונות" actions={defaultBulkActions(selectedIds, clear, load, `${API}/trial-balance`)} />
              ...
              <tbody>
                {pagination.paginate(filtered).map((r, idx) => (
                  <tr key={r.account_number || idx} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-white">{r.account_name_he || r.account_name}</td>
                    <td className="px-4 py-3">
                      <Badge className={`text-[10px] ${typeMap[r.account_type]?.color || "bg-muted/20 text-muted-foreground"}`}>{typeMap[r.account_type]?.label || r.account_type}</Badge>
                    </td>
                    <td className="px-4 py-3 text-blue-400 font-bold">{Number(r.period_debit) > 0 ? fmtCurrency(r.period_debit) : ""}</td>
                    <td className="px-4 py-3 text-red-400 font-bold">{Number(r.period_credit) > 0 ? fmtCurrency(r.period_credit) : ""}</td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><Scale className="w-5 h-5 text-indigo-400" /> חשבון {viewDetail.account_number}</h2>
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                ...
                <DetailField label="חובה בתקופה" value={fmtCurrency(viewDetail.period_debit)} />
                <DetailField label="זכות בתקופה" value={fmtCurrency(viewDetail.period_credit)} />
