    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0e1a] to-[#1a1f35] p-6" dir="rtl">
      <div className="max-w-[1600px] mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {[
              ...
              <motion.div
                ...
                className="bg-[#1e2337]/80 rounded-xl p-4 border border-gray-700/50"
              >
        ...
        <motion.div
          ...
          className="bg-[#1e2337]/80 rounded-xl p-6 border border-gray-700/50"
        >
                      ...
                      <motion.button
                        ...
                        className={`flex flex-col items-center p-3 rounded-xl border transition-all min-w-[90px] ${
                          isSelected
                            ...
                            : 'border-gray-700/50 bg-[#252a3e]/80 hover:border-gray-600'
                        }`}
                      >
                        <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${catColor} flex items-center justify-center mb-2`}>
                          <Icon className="w-5 h-5 text-white" />
                        </div>
                        <span className="text-white text-xs font-medium text-center leading-tight">{node.name}</span>
        ...
        {selectedNode && (() => {
          ...
          return (
            <motion.div
              ...
              className="bg-[#1e2337]/80 rounded-xl p-6 border border-blue-500/30"
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-xl font-bold text-white">{node.name} ({node.nameEn})</h3>
                  <p className="text-muted-foreground text-sm mt-1">{node.description}</p>
                  <div className="flex items-center gap-2 mt-2">
                    <span className={`inline-block px-2 py-0.5 rounded text-xs bg-gradient-to-r ${categoryColors[node.category]} text-white`}>
                      {node.module}
                    </span>
                    {nodeNavigationMap[node.id] && (
                      <button
                ...
                <button onClick={() => { setSelectedNode(null); setExpandedNode(null); }} className="text-muted-foreground hover:text-white">
                  <ChevronUp className="w-5 h-5" />
                </button>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                <div className="bg-[#252a3e] rounded-lg p-3">
                      ...
                      {inEdges.map(e => {
                        ...
                        return (
                          <div key={e.id} className="flex items-center gap-2 text-xs">
                            <span className="text-white font-medium">{fromNode?.name || e.from}</span>
                      ...
                      {outEdges.map(e => {
                        ...
                        return (
                          <div key={e.id} className="flex items-center gap-2 text-xs">
                            ...
                            <ArrowRight className="w-3 h-3 text-muted-foreground rotate-180" />
                            <span className="text-white font-medium">{toNode?.name || e.to}</span>
                            <span className="text-muted-foreground">({e.trigger})</span>
                          </div>
                        );
                      })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div
            ...
            className="bg-[#1e2337]/80 rounded-xl p-5 border border-gray-700/50"
          >
              ...
              return (
                <div
                  ...
                  className={`text-right p-3 rounded-lg border transition-all ${
                    isExpanded ? 'border-blue-400 bg-blue-600/10' : 'border-gray-700/50 bg-[#252a3e]/60 hover:border-gray-600'
                  }`}
                >
                  <button
                    onClick={() => {
        ...
        >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-muted-foreground border-b border-gray-700/50">
              ...
              <tbody>
                {edges.map(edge => {
                  ...
                  return (
                    <tr key={edge.id} className="border-b border-gray-800/50 hover:bg-[#252a3e]/50">
                      <td className="py-2 px-3 text-white">{fromNode?.name || edge.from}</td>
                      <td className="py-2 px-3 text-white">{toNode?.name || edge.to}</td>
