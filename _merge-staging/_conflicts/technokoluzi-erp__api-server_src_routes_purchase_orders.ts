...
router.get("/purchase-orders", async (_req, res) => {
  try {
    const result = await pool.query("SELECT * FROM purchase_orders ORDER BY id DESC LIMIT 200");
    res.json(result.rows);
  } catch (err: any) {
    // שגיאה בשליפת הזמנות רכש
    console.error("Error fetching purchase_orders:", err);
...
router.get("/purchase-orders/:id", async (req, res) => {
  try {
    ...
    // שגיאה בשליפת הזמנת רכש לפי מזהה
    console.error("Error fetching purchase_order by id:", err);
    res.status(500).json({ error: "Internal server error / שגיאת שרת פנימית", details: err?.message });
  }
});
...
router.post("/purchase-orders", async (req, res) => {
  try {
    ...
    const result = await pool.query(`INSERT INTO purchase_orders (${keys.join(", ")}) VALUES (${placeholders}) RETURNING *`, vals);
    res.status(201).json(result.rows[0]);
  } catch (err: any) {
    // שגיאה ביצירת הזמנת רכש
    console.error("Error creating purchase_order:", err);
    res.status(500).json({ error: "Internal server error / שגיאת שרת פנימית", details: err?.message });
  }
});
...
router.delete("/purchase-orders/:id", async (req, res) => {
  try {
    ...
    // שגיאה במחיקת הזמנת רכש
    console.error("Error deleting purchase_order:", err);
    res.status(500).json({ error: "Internal server error / שגיאת שרת פנימית", details: err?.message });
  }
});
