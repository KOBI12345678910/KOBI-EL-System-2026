...
import { authFetch } from "@/lib/utils";
    ...
    const STATUSES = ["הצליח", "נכשל", "בתהליך", "ממתין", "בוטל"] as const;
    const SC: Record<string, string> = { "הצליח": "bg-green-500/20 text-green-300", "נכשל": "bg-red-500/20 text-red-300", "בתהליך": "bg-yellow-500/20 text-yellow-300", "ממתין": "bg-blue-500/20 text-blue-300", "בוטל": "bg-gray-500/20 text-gray-300" };
    const CATS = ["ייבוא","ייצוא","גיבוי","שחזור"];
...
export default function ImportExportPage() {
  ...
  const [search, setSearch] = useState("");
    ...
    try {
      setError(null);
      const res = await authFetch("/api/platform/system-info");
      if (res.ok) {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.fileName?.toLowerCase().includes(s) || r.type?.toLowerCase().includes(s) || r.module?.toLowerCase().includes(s)); }
      ...
      {error && <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-red-400"><AlertCircle className="h-4 w-4 flex-shrink-0" /><span className="text-sm">{error}</span><button onClick={() => setError(null)} className="mr-auto"><X className="h-4 w-4" /></button></div>}
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingUp className="h-6 w-6 text-green-400" />ייבוא/ייצוא</h1><p className="text-sm text-muted-foreground mt-1">ייבוא וייצוא נתונים, גיבויים ושחזור</p></div>
            <div className="flex gap-2">
              ...
              <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2"><Plus className="h-4 w-4" />פעולה חדשה</Button>
            </div>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"פעולות היום","v":"12","c":"text-green-400","t":"+3","u":true},{"l":"הצליחו","v":"10","c":"text-emerald-400","t":"+2","u":true},{"l":"נכשלו","v":"1","c":"text-red-400","t":"-1","u":true},{"l":"רשומות עובדו","v":"8,450","c":"text-blue-400","t":"+1200","u":true},{"l":"גיבוי אחרון","v":"היום","c":"text-cyan-400","t":"","u":true},{"l":"נפח","v":"2.3GB","c":"text-purple-400","t":"+150MB","u":true}].map((k: any, i: number) => (
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("fileName")}>קובץ<SI field="fileName" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("module")}>מודול<SI field="module" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("format")}>פורמט<SI field="format" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("records")}>רשומות<SI field="records" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("size")}>נפח<SI field="size" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("initiatedBy")}>מבצע<SI field="initiatedBy" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingUp className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין ייבוא/ייצוא"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "פעולה חדשה ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />פעולה חדשה</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 "><div className="font-medium text-white">{row.fileName}</div></td>
            <td className="p-3 "><Badge className={`${row.type==="ייבוא"?"bg-blue-500/20 text-blue-300":"bg-green-500/20 text-green-300"} border-0 text-xs`}>{row.type}</Badge></td>
            <td className="p-3 text-muted-foreground text-xs">{row.module}</td>
            <td className="p-3 font-mono text-cyan-400 text-xs">{row.format}</td>
            <td className="p-3 font-mono text-muted-foreground text-center">{row.records}</td>
            <td className="p-3 font-mono text-muted-foreground text-xs">{row.size}</td>
            <td className="p-3 text-muted-foreground text-xs">{row.initiatedBy}</td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]"><h2 className="text-lg font-bold text-white">{editId ? "עריכה" : "פעולה חדשה"}</h2><Button variant="ghost" size="sm" onClick={()=>{setShowCreate(false);setEditId(null)}}><X className="h-4 w-4" /></Button></div>
            <div className="p-4 space-y-6">
              <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטים</h3></div>
            <div className="grid grid-cols-3 gap-4">
            <div><Label className="text-muted-foreground text-xs">סוג *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>ייבוא</option><option>ייצוא</option><option>גיבוי</option><option>שחזור</option></select></div>
            <div><Label className="text-muted-foreground text-xs">מודול *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>לקוחות</option><option>מוצרים</option><option>מלאי</option><option>הזמנות</option><option>ספקים</option><option>חשבונאות</option></select></div>
            <div><Label className="text-muted-foreground text-xs">פורמט</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>CSV</option><option>XLSX</option><option>JSON</option><option>XML</option></select></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>הצליח</option><option>נכשל</option><option>בתהליך</option><option>ממתין</option><option>בוטל</option></select></div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">קובץ</p><p className="text-white mt-1 font-medium">{dr.fileName}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מודול</p><p className="text-white mt-1 font-medium">{dr.module}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">פורמט</p><p className="text-white mt-1 font-medium">{dr.format}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">רשומות</p><p className="text-white mt-1 font-medium">{dr.records}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">נפח</p><p className="text-white mt-1 font-medium">{dr.size}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">משך</p><p className="text-white mt-1 font-medium">{dr.duration}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">התחלה</p><p className="text-white mt-1 font-medium">{dr.startedAt}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">סיום</p><p className="text-white mt-1 font-medium">{dr.completedAt||"בתהליך"}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מבצע</p><p className="text-white mt-1 font-medium">{dr.initiatedBy}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">שגיאות</p><p className="text-white mt-1 font-medium">{dr.errors}</p></div>
