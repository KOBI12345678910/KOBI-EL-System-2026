    ...
    <div className="space-y-4 sm:space-y-6" dir="rtl">
      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {tab === "plans" ? (
            <button onClick={openCreatePlan} className="flex items-center gap-2 bg-rose-600 text-white px-4 py-2 rounded-xl hover:bg-rose-700 shadow-lg text-sm font-medium">
              <Plus className="w-4 h-4" /> תוכנית חדשה
            </button>
          ) : tab === "enrollments" ? (
            <button onClick={openCreateEnrollment} className="flex items-center gap-2 bg-rose-600 text-white px-4 py-2 rounded-xl hover:bg-rose-700 shadow-lg text-sm font-medium">
              ...
              {filteredPlans.length === 0 ? (
                ...
                <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="font-medium text-white">{r.plan_name}</div>
                  ...
                  <td className="px-4 py-3 font-bold text-white">{fmtCurrency(r.employer_contribution)}</td>
                  <td className="px-4 py-3 text-muted-foreground">{fmtCurrency(r.employee_contribution)}</td>
                  <td className="px-4 py-3">
                    <span className="text-white">{r.current_participants || 0}</span>
            ...
            <tbody>
              {filteredEnrollments.length === 0 ? (
                ...
                <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3 font-bold text-white">{fmtCurrency(r.employer_cost)}</td>
            ...
            <div className="bg-card border border-border/50 rounded-2xl p-5">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b border-border/20">
                  ...
                  <span className="text-lg font-bold text-white">{fmtCurrency(totalEmployerCost)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-border/20">
                  <span className="text-sm text-muted-foreground">עלות עובדים כוללת</span>
                  <span className="text-lg font-bold text-white">{fmtCurrency(totalEmployeeCost)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-border/20">
                  ...
                  return (
                    <div key={type}>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-white font-medium">{data.count} תוכניות | {fmtCurrency(data.cost)}</span>
                      </div>
            ...
            <div className="bg-card border border-border/50 rounded-2xl p-5">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center py-2 border-b border-border/20">
                  ...
                  <span className="text-lg font-bold text-white">{plans.filter(p => p.is_mandatory).length}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-border/20">
              ...
              {detailTab === "details" && <div className="p-5 space-y-3">
                {Object.entries(viewDetail).filter(([k]) => !["id", "created_at", "updated_at"].includes(k)).map(([key, val]) => (
                  <div key={key} className="flex justify-between py-1.5 border-b border-border/20">
                    ...
                    <span className="text-sm text-white max-w-[60%] text-left truncate">{String(val ?? "—")}</span>
                  </div>
                ))}
              </div>}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card rounded-2xl border border-border shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={save} className="flex items-center gap-2 bg-rose-600 text-white px-6 py-2.5 rounded-xl hover:bg-rose-700 font-medium text-sm">
