      ...
      <div key={node.id}>
        <div className={`flex items-center gap-2 px-3 py-2 hover:bg-muted/30 rounded-lg transition-colors group`} style={{ paddingRight: `${depth * 20 + 12}px` }}>
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button onClick={async () => { if (globalConfirm("למחוק את הקטגוריה?")) deleteMutation.mutate(node.id); }} className="p-1 hover:bg-destructive/10 rounded"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
          </div>
        </div>
                ...
                <div key={def.id} className="bg-card border border-border rounded-xl p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={async () => { if (globalConfirm("למחוק את הגדרת הקטגוריה?")) deleteDefMutation.mutate(def.id); }} className="p-1.5 hover:bg-destructive/10 rounded-lg"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
                    </div>
                  </div>
                </div>
              ))}
