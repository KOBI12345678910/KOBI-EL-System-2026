...
import { authFetch } from "@/lib/utils";
  ...
  import { Clock, Download, Plus, Search, Printer, Upload, MoreHorizontal, ChevronRight, ChevronLeft, ArrowUp, ArrowDown, X, Save, Eye, Edit2, Trash2, Copy, FileSpreadsheet, TrendingUp, TrendingDown, ChevronsUpDown, CheckCircle2, Send, AlertCircle, Loader2 } from "lucide-react";

  const fmt = (v: number) => new Intl.NumberFormat("he-IL", { style: "currency", currency: "ILS", maximumFractionDigits: 0 }).format(v / 100);
  const STATUSES = ["מתוכנן", "מאושר", "בביצוע", "הושלם", "נדחה"] as const;
  const SC: Record<string, string> = { "מתוכנן": "bg-gray-500/20 text-gray-300", "מאושר": "bg-blue-500/20 text-blue-300", "בביצוע": "bg-yellow-500/20 text-yellow-300", "הושלם": "bg-green-500/20 text-green-300", "נדחה": "bg-red-500/20 text-red-300" };
  const EVENT_TYPES = ["התקנה","תחזוקה","ביקורת","מסירה","הדרכה","ישיבה","אירוע"];
  const EMP = ["יוסי כהן","שרה לוי","דוד מזרחי","רחל אברהם","אלון גולדשטיין","מיכל ברק","עומר חדד","נועה פרידמן","איתן רוזנברג","תמר שלום"];
export default function InstallationsCalendar() {
  ...
  const [search, setSearch] = useState("");
    ...
    const filtered = useMemo(() => {
      ...
      if (evtFilter !== "all") d = d.filter(r => r.eventType === evtFilter);
      return d;
    }, [search, statusFilter, evtFilter]);
    ...
    const af = [statusFilter !== "all", evtFilter !== "all"].filter(Boolean).length;
    ...
    return (
      ...
      {error && <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-red-400"><AlertCircle className="h-4 w-4 flex-shrink-0" /><span className="text-sm">{error}</span><button onClick={() => setError(null)} className="mr-auto"><X className="h-4 w-4" /></button></div>}
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><Clock className="h-6 w-6 text-indigo-400" />לוח זמנים התקנות</h1><p className="text-sm text-muted-foreground mt-1">תכנון ותזמון עבודות התקנה, תחזוקה וביקורות</p></div>
          <div className="flex gap-2">
            ...
            <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2"><Plus className="h-4 w-4" />אירוע חדש</Button>
          </div>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {[{"l":"אירועים","v":"52","c":"text-indigo-400","t":"+8","u":true},{"l":"השבוע","v":"12","c":"text-blue-400","t":"+3","u":true},{"l":"בביצוע","v":"8","c":"text-yellow-400","t":"+2","u":true},{"l":"הושלמו","v":"22","c":"text-green-400","t":"+5","u":true},{"l":"עדיפות גבוהה","v":"15","c":"text-red-400","t":"+3","u":false},{"l":"צוותים פעילים","v":"6","c":"text-purple-400","t":"+1","u":true}].map((k: any, i: number) => (
        ...
        <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
          ...
          <select value={evtFilter} onChange={e => { setEvtFilter(e.target.value); setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל סוגי האירועים</option>{EVENT_TYPES.map((o: string) => <option key={o} value={o}>{o}</option>)}</select>
          {af > 0 && <Button variant="ghost" size="sm" onClick={() => { setStatusFilter("all"); setEvtFilter("all"); setSearch(""); }} className="text-red-400 hover:text-red-300 gap-1"><X className="h-3 w-3" />נקה ({af})</Button>}
        ...
        <th className="p-3 text-right text-muted-foreground font-medium">סטטוס</th>
          ...
          {pd.map(row => (
            ...
            <td className="p-3 text-muted-foreground">{row.eventType}</td>
            ...
            <td className="p-3 text-muted-foreground text-xs">{row.duration}</td>
        ...
        {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]"><h2 className="text-lg font-bold text-white">{editId ? "עריכה" : "אירוע חדש"}</h2><Button variant="ghost" size="sm" onClick={()=>{setShowCreate(false);setEditId(null)}}><X className="h-4 w-4" /></Button></div>
          <div className="p-4 space-y-6">
            ...
            <div><Label className="text-muted-foreground text-xs">סוג *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>התקנה</option><option>תחזוקה</option><option>ביקורת</option><option>מסירה</option><option>הדרכה</option><option>ישיבה</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">משך</Label><Input type="text" placeholder="2 שעות" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">מיקום</Label><Input type="text" placeholder="מיקום" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">צוות</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>צוות A</option><option>צוות B</option><option>צוות C</option><option>צוות D</option><option>צוות E</option><option>צוות F</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>מתוכנן</option><option>מאושר</option><option>בביצוע</option><option>הושלם</option><option>נדחה</option></select></div>
            </div>
        ...
        {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">סוג</p><p className="text-white mt-1 font-medium">{dr.eventType}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">אחראי</p><p className="text-white mt-1 font-medium">{dr.assignedTo}</p></div>
