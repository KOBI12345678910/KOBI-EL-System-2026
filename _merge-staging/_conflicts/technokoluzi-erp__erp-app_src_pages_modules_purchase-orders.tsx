...
const emptyItem: OrderItemForm = {
  ...
  unitPrice: "0", discountPercent: "0", taxPercent: "18", deliveryDate: "", notes: ""
};
...
export default function PurchaseOrdersPage() {
  ...
  async function createFromPR(pr: PurchaseRequest) {
    ...
    const prItems: OrderItemForm[] = (prFull.items || []).map((item: any) => ({
      ...
      discountPercent: "0", taxPercent: "18",
      ...
      <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => {
              const msg = `רשימת הזמנות רכש - טכנו-כל עוזי\n${orders.slice(0, 10).map((o: PurchaseOrder) => `• ${o.orderNumber} | ${o.status} | ₪${parseFloat(o.totalAmount || "0").toLocaleString()}`).join("\n")}`;
              window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`, "_blank");
            }} className="flex items-center gap-2 px-3 py-2 bg-green-700 hover:bg-green-600 rounded-xl text-sm text-gray-200 transition-colors">
            ...
            <button onClick={() => sendByEmail("הזמנות רכש - טכנו-כל עוזי", generateEmailBody("הזמנות רכש", orders, { orderNumber: "מס׳ הזמנה", status: "סטטוס", totalAmount: "סכום ₪" }))}
              className="flex items-center gap-2 px-3 py-2 bg-gray-700 hover:bg-gray-600 rounded-xl text-sm text-gray-300 transition-colors">
            ...
            <button onClick={() => exportToWord("הזמנות רכש", orders, { orderNumber: "מס׳ הזמנה", status: "סטטוס", orderDate: "תאריך", expectedDelivery: "אספקה משוערת", totalAmount: "סכום ₪", currency: "מטבע" }, "purchase_orders")}
              className="flex items-center gap-2 px-3 py-2 bg-indigo-700 hover:bg-indigo-600 rounded-xl text-sm text-gray-200 transition-colors">
            ...
            {purchaseRequests.filter(pr => pr.status === "מאושר").length > 0 && (
              <div className="relative group">
                <button className="flex items-center gap-2 px-4 py-2.5 bg-emerald-700 hover:bg-emerald-600 text-white rounded-xl font-medium transition-colors text-sm">
                  <FileText className="w-4 h-4" />צור מ-PR מאושר
                  <ChevronDown className="w-4 h-4" />
                </button>
                <div className="absolute left-0 top-full mt-1 bg-[#1a1d23] border border-gray-700 rounded-xl shadow-xl z-20 min-w-[260px] hidden group-hover:block">
                  {purchaseRequests.filter(pr => pr.status === "מאושר").slice(0, 8).map(pr => (
                    <button key={pr.id} onClick={() => createFromPR(pr)}
                      className="w-full px-4 py-2.5 text-right text-sm text-white hover:bg-[#2a2d33] border-b border-gray-800/50 last:border-0 first:rounded-t-xl last:rounded-b-xl">
        ...
        {isLoading ? (
          ...
          <>
          <BulkActions bulk={bulk} actions={defaultBulkActions} entityName="הזמנות רכש" />
            ...
            <table className="w-full text-right">
              ...
              <tbody>
                {filtered.map(o => {
                  ...
                  return (
                    <>
                      <tr key={o.id} className={`border-b border-gray-800/50 hover:bg-[#1e2128] cursor-pointer transition-colors ${isOverdue ? "bg-red-500/5" : ""} ${bulk.isSelected(o.id) ? "bg-primary/5" : ""}`}
                        onClick={() => loadOrderItems(o.id)}>
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}><BulkCheckbox bulk={bulk} id={o.id} /></td>
                        ...
                        <td className="px-4 py-3 text-white text-sm">{getSupplierName(o.supplierId)}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            <div className={`w-2 h-2 rounded-full ${o.requestId ? "bg-emerald-400" : "bg-gray-600"}`} title={o.requestId ? "דרישה מקושרת" : "ללא דרישה"} />
                            <div className={`w-2 h-2 rounded-full ${hasReceipt ? "bg-emerald-400" : "bg-gray-600"}`} title={hasReceipt ? "יש קבלת סחורה" : "ללא קבלה"} />
                            <div className={`w-2 h-2 rounded-full ${o.totalAmount && parseFloat(o.totalAmount) > 0 ? "bg-emerald-400" : "bg-gray-600"}`} title="חשבונית" />
                          </div>
                        ...
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => openEdit(o)} className="p-1.5 text-muted-foreground hover:text-amber-400 rounded-md" title="ערוך"><Edit2 className="w-4 h-4" /></button>
                            <button onClick={() => setDeleteConfirm(o.id)} className="p-1.5 text-muted-foreground hover:text-red-400 rounded-md" title="מחק"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                      {expandedRow === o.id && (
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                                <div className="bg-[#1a1d23] rounded-lg p-3">
                                  ...
                                  <p className="text-white text-sm font-medium">{o.paymentTerms || "—"}</p>
                                </div>
                                <div className="bg-[#1a1d23] rounded-lg p-3">
                                  <p className="text-muted-foreground text-xs mb-1">שיטת משלוח</p>
                                  <p className="text-white text-sm font-medium">{o.shippingMethod || "—"}</p>
                                </div>
                                <div className="bg-[#1a1d23] rounded-lg p-3">
                                  <p className="text-muted-foreground text-xs mb-1">מטבע</p>
                                  <p className="text-white text-sm font-medium">{o.currency || "ILS"}</p>
                                </div>
                                <div className="bg-[#1a1d23] rounded-lg p-3">
                                  <p className="text-muted-foreground text-xs mb-1">אישר</p>
                                  <p className="text-white text-sm font-medium">{o.approvedBy || "—"}</p>
                                </div>
                                <div className="bg-[#1a1d23] rounded-lg p-3">
                                    ...
                                    {expandedItems[o.id].map((item: PurchaseOrderItem) => {
                                      ...
                                      return (
                                        <tr key={item.id} className="border-t border-gray-800/30">
                                          ...
                                          <td className="py-2 pr-2 text-white font-medium" dir="ltr">{cs}{parseFloat(item.totalPrice).toLocaleString()}</td>
                                          <td className="py-2 pr-2 text-muted-foreground">{item.receivedQuantity || "0"}</td>
                                          <td className="py-2 pr-2">
                                            <div className="flex items-center gap-2">
                                              <div className="w-16 h-1.5 bg-gray-700 rounded-full overflow-hidden">
                                                <div className={`h-full rounded-full ${pct >= 100 ? "bg-emerald-500" : pct > 0 ? "bg-amber-500" : "bg-gray-600"}`} style={{ width: `${Math.min(pct, 100)}%` }} />
                                              </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <div>
                  <h2 className="text-xl font-bold text-white">הזמנה {selectedOrder.orderNumber}</h2>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => setSelectedOrder(null)} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
                </div>
              </div>
              ...
              {detailTab === "details" && (<>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-[#12141a] rounded-xl p-4">
                    ...
                    <p className="text-white font-medium mt-1">{selectedOrder.orderDate ? new Date(selectedOrder.orderDate).toLocaleDateString("he-IL") : "—"}</p>
                  </div>
                  <div className="bg-[#12141a] rounded-xl p-4">
                    <p className="text-muted-foreground text-xs flex items-center gap-1"><Truck className="w-3 h-3" />אספקה צפויה</p>
                    <p className="text-white font-medium mt-1">{selectedOrder.expectedDelivery ? new Date(selectedOrder.expectedDelivery).toLocaleDateString("he-IL") : "—"}</p>
                  </div>
                  <div className="bg-[#12141a] rounded-xl p-4">
                    <p className="text-muted-foreground text-xs flex items-center gap-1"><CreditCard className="w-3 h-3" />תנאי תשלום</p>
                    <p className="text-white font-medium mt-1">{selectedOrder.paymentTerms || "—"}</p>
                  </div>
                  <div className="bg-[#12141a] rounded-xl p-4">
                    <p className="text-muted-foreground text-xs flex items-center gap-1"><Truck className="w-3 h-3" />שיטת משלוח</p>
                    <p className="text-white font-medium mt-1">{selectedOrder.shippingMethod || "—"}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-[#12141a] rounded-xl p-4">
                    ...
                    <p className="text-white font-medium mt-1 text-sm">{selectedOrder.shippingAddress || "—"}</p>
                  </div>
                  <div className="bg-[#12141a] rounded-xl p-4">
                    <p className="text-muted-foreground text-xs flex items-center gap-1"><User className="w-3 h-3" />נוצר ע״י</p>
                    <p className="text-white font-medium mt-1">{selectedOrder.createdBy || "—"}</p>
                  </div>
                  <div className="bg-[#12141a] rounded-xl p-4">
                    <p className="text-muted-foreground text-xs flex items-center gap-1"><CheckCircle2 className="w-3 h-3" />אישר</p>
                    <p className="text-white font-medium mt-1">{selectedOrder.approvedBy || "—"}</p>
                  ...
                  <div className="bg-[#12141a] rounded-xl p-4">
                    ...
                    <p className="text-white font-medium mt-1">{selectedOrder.currency || "ILS"}</p>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-[#12141a] to-[#1a1d23] rounded-xl p-5 border border-gray-800">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <div className="text-center">
                      ...
                      <p className="text-white text-xl font-bold font-mono" dir="ltr">
                        {getCurrencySymbol(selectedOrder.currency)}{parseFloat(selectedOrder.totalBeforeTax || "0").toLocaleString()}
                      </p>
                    </div>
                    <div className="text-center">
                      <p className="text-muted-foreground text-xs mb-1">מע״מ (18%)</p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className={`rounded-xl p-4 border-2 text-center ${selectedOrder.requestId ? "border-emerald-500/30 bg-emerald-500/5" : "border-gray-700 bg-[#12141a]"}`}>
                      <FileText className={`w-8 h-8 mx-auto mb-2 ${selectedOrder.requestId ? "text-emerald-400" : "text-muted-foreground"}`} />
                      <p className="text-sm font-medium text-white">דרישת רכש</p>
                    ...
                    <div className="rounded-xl p-4 border-2 border-blue-500/30 bg-blue-500/5 text-center">
                      ...
                      <p className="text-sm font-medium text-white">הזמנת רכש</p>
                      <p className="text-xs mt-1 text-blue-400">{selectedOrder.orderNumber}</p>
                    </div>
                    <div className={`rounded-xl p-4 border-2 text-center ${getMatchingReceipts(selectedOrder.id).length > 0 ? "border-emerald-500/30 bg-emerald-500/5" : "border-gray-700 bg-[#12141a]"}`}>
                      ...
                      <p className="text-sm font-medium text-white">קבלת סחורה</p>
                ...
                {selectedOrder.items && selectedOrder.items.length > 0 && (
                  <div>
                    <h3 className="text-lg font-semibold text-white mb-3">פריטים ({selectedOrder.items.length})</h3>
                ...
                {selectedOrder.notes && (
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs mb-1">הערות</p><p className="text-gray-300 text-sm">{selectedOrder.notes}</p></div>
                )}
              </>)}
              {detailTab === "related" && (
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center pt-6 overflow-y-auto" onClick={closeForm}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-4xl mx-4 mb-10" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white">{editingId ? "עריכת הזמנה" : "הזמנת רכש חדשה"}</h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    {orderItems.map((item, idx) => {
                      ...
                      return (
                        <div key={idx} className="bg-[#12141a] border border-gray-800 rounded-lg p-3 space-y-3">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <div>
                              ...
                              <select value={item.materialId} onChange={e => updateItem(idx, "materialId", e.target.value)}
                                className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-blue-500 focus:outline-none">
                            ...
                            <div>
                              ...
                              <input value={item.itemCode} onChange={e => updateItem(idx, "itemCode", e.target.value)} placeholder="MAT-001"
                                className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm font-mono placeholder-gray-600 focus:border-blue-500 focus:outline-none" />
                            </div>
                            <div className="sm:col-span-2">
                              ...
                              <input value={item.itemDescription} onChange={e => updateItem(idx, "itemDescription", e.target.value)} placeholder="תיאור הפריט"
                                className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm placeholder-gray-600 focus:border-blue-500 focus:outline-none" />
                            </div>
                          </div>
                          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                            <div>
                              ...
                              <input type="number" value={item.quantity} onChange={e => updateItem(idx, "quantity", e.target.value)}
                                className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-blue-500 focus:outline-none" dir="ltr" />
                            </div>
                            <div>
                          ...
                          <div className="flex items-center justify-between text-sm pt-1 border-t border-gray-800/30">
                            ...
                            <span className="text-muted-foreground">סה״כ שורה: <span className="text-white font-bold font-mono">₪{lc.total.toLocaleString()}</span></span>
                          </div>
                        </div>
                      );
                    })}
                    {orderItems.length > 0 && (
                      <div className="bg-gradient-to-br from-[#12141a] to-[#1a1d23] rounded-lg p-4 border border-gray-700 mt-3">
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
                          <div>
                            <p className="text-muted-foreground text-xs mb-1">לפני מע״מ</p>
                            <p className="text-white font-bold font-mono text-lg" dir="ltr">₪{itemsTotalBeforeTax.toLocaleString()}</p>
                          </div>
                          <div>
                ...
                <div>
                  ...
                  <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})}
                    className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-blue-500 focus:outline-none resize-none" rows={2} />
                </div>
        ...
        {deleteConfirm !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
            <div className="bg-[#1a1d23] border border-gray-700 rounded-xl p-6 max-w-sm mx-4" dir="rtl">
              <h3 className="text-lg font-bold text-white mb-2">מחיקת הזמנה</h3>
