  ...
  return (
    <div dir="rtl" className="space-y-4 sm:space-y-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white">הגדרות AI</h1>
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-base font-semibold text-white">Claude AI (Anthropic)</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-semibold text-white">סטטוס חיבור</h3>
          </div>
          {statusLoading ? (
        ...
        <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-semibold text-white">ספק ומודל</h3>
          </div>
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <span className="text-white font-mono text-sm bg-violet-600/20 px-3 py-1.5 rounded-lg">
        ...
        <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-semibold text-white">נתוני שימוש</h3>
          </div>
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-muted-foreground">סה"כ שיחות:</span>
              <span className="text-white font-medium">{status?.totalConversations || 0}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">סה"כ הודעות:</span>
              <span className="text-white font-medium">{status?.totalMessages || 0}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Input Tokens:</span>
              <span className="text-white font-medium">{(status?.totalInputTokens || 0).toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Output Tokens:</span>
              <span className="text-white font-medium">{(status?.totalOutputTokens || 0).toLocaleString()}</span>
            </div>
            <div className="flex justify-between border-t border-gray-700/50 pt-2">
              <span className="text-muted-foreground">סה"כ Tokens:</span>
              <span className="text-white font-medium">{((status?.totalInputTokens || 0) + (status?.totalOutputTokens || 0)).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="font-semibold text-white">הגדרת API — Claude</h3>
        </div>
        {status?.configured ? (
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-[#12141a] border border-gray-700 rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:border-violet-500 focus:outline-none font-mono"
                dir="ltr"
              />
              <button
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-violet-600 hover:bg-violet-500 disabled:bg-gray-700 disabled:text-muted-foreground text-white rounded-lg text-sm font-medium transition-colors"
            >
              {saveMut.isPending ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
      ...
      <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-semibold text-white">כלי Builder (Tool Use)</h3>
          </div>
          <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${status?.toolsEnabled ? "bg-emerald-500/20 text-emerald-400" : "bg-gray-700 text-muted-foreground"}`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          {(status?.availableTools || []).map((tool: string) => (
            <div key={tool} className="flex items-center gap-2 px-3 py-2 bg-[#12141a] border border-gray-700/50 rounded-lg">
      ...
      <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="font-semibold text-white">בדיקת חיבור — Claude</h3>
        </div>
        <p className="text-muted-foreground text-sm mb-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-violet-600 hover:bg-violet-500 disabled:bg-gray-700 text-white rounded-lg text-sm font-medium transition-colors"
        >
          {testMut.isPending ? (
            <>
              <Loader2 className="w-4 h-4 animate-spin" />
      ...
      <div className="flex items-center gap-2 mt-8 mb-1">
        ...
        <h2 className="text-base font-semibold text-white">Kimi / Moonshot AI</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-semibold text-white">סטטוס חיבור — Kimi</h3>
          </div>
          {kimiStatusLoading ? (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Loader2 className="w-4 h-4 animate-spin" />
        ...
        <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-semibold text-white">פרטי ספק — Moonshot</h3>
          </div>
          <div className="text-sm space-y-2">
            <div>
              <span className="text-muted-foreground text-xs block mb-0.5">Organization ID</span>
              <span className="font-mono text-xs text-gray-200 bg-[#12141a] px-2 py-1 rounded block break-all">
            ...
            <div>
              ...
              <span className="font-mono text-xs text-gray-200 bg-[#12141a] px-2 py-1 rounded block">
      ...
      <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="font-semibold text-white">בדיקת חיבור — Kimi</h3>
        </div>
        <p className="text-muted-foreground text-sm mb-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-cyan-600 hover:bg-cyan-500 disabled:bg-gray-700 disabled:text-muted-foreground text-white rounded-lg text-sm font-medium transition-colors"
          >
          ...
          <Link href="/ai-engine/kimi">
            <button className="flex items-center gap-2 px-4 py-2.5 bg-[#12141a] border border-gray-700 hover:border-cyan-500/50 text-white rounded-lg text-sm font-medium transition-colors">
              <Moon className="w-4 h-4 text-cyan-400" />
