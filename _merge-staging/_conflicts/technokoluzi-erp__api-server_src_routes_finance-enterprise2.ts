...
router.post("/cost-centers", async (req, res) => {
  ...
  } catch (err: any) { /* שגיאה ביצירת מרכז עלות */ console.error("POST /cost-centers error:", err.message); res.status(500).json({ error: "שגיאה ביצירת מרכז עלות" }); }
});
...
router.get("/customer-invoices", async (_req, res) => {
  res.json(await q(`SELECT * FROM customer_invoices ORDER BY invoice_date DESC, id DESC`));
});
...
router.post("/customer-invoices", async (req, res) => {
  ...
  await q(`INSERT INTO customer_invoices (invoice_number, invoice_type, invoice_date, due_date, customer_name, customer_address, customer_tax_id, contact_name, contact_phone, contact_email, status, currency, exchange_rate, subtotal, discount_pct, discount_amount, before_vat, vat_rate, vat_amount, total_amount, payment_terms, payment_method, reference_number, po_number, project_name, cost_center, salesperson, delivery_date, item_description, notes, created_by, created_by_name)
    VALUES ('${num}', '${d.invoiceType||'tax_invoice'}', '${d.invoiceDate || new Date().toISOString().slice(0,10)}', ${d.dueDate ? `'${d.dueDate}'` : 'NULL'}, ${s(d.customerName)}, ${s(d.customerAddress)}, ${s(d.customerTaxId)}, ${s(d.contactName)}, ${s(d.contactPhone)}, ${s(d.contactEmail)}, '${d.status||'draft'}', '${d.currency||'ILS'}', ${d.exchangeRate||1}, ${d.subtotal||0}, ${d.discountPct||0}, ${d.discountAmount||0}, ${d.beforeVat||0}, ${d.vatRate||18}, ${d.vatAmount||0}, ${d.totalAmount||0}, '${d.paymentTerms||'net_30'}', ${s(d.paymentMethod)}, ${s(d.referenceNumber)}, ${s(d.poNumber)}, ${s(d.projectName)}, ${s(d.costCenter)}, ${s(d.salesperson)}, ${d.deliveryDate ? `'${d.deliveryDate}'` : 'NULL'}, ${s(d.itemDescription)}, ${s(d.notes)}, ${user?.id||'NULL'}, ${s(user?.fullName)})`);
  res.json((await q(`SELECT * FROM customer_invoices WHERE invoice_number='${num}'`))[0]);
  } catch (err: any) { /* שגיאה ביצירת חשבונית לקוח */ console.error("POST /customer-invoices error:", err.message); res.status(500).json({ error: "שגיאה ביצירת חשבונית לקוח" }); }
});
...
router.put("/customer-invoices/:id", async (req, res) => {
  ...
  res.json(updated);
  } catch (err: any) { /* שגיאה בעדכון חשבונית לקוח - כולל זרימת נתונים מורכבת */ console.error("PUT /customer-invoices error:", err.message); res.status(500).json({ error: "שגיאה בעדכון חשבונית לקוח" }); }
});
...
router.post("/credit-notes", async (req, res) => {
  ...
  await q(`INSERT INTO credit_notes (credit_number, credit_type, credit_date, original_invoice_number, customer_name, customer_tax_id, reason, reason_description, status, currency, subtotal, vat_rate, vat_amount, total_amount, refund_method, notes, created_by, created_by_name)
    VALUES ('${num}', '${d.creditType||'credit'}', '${d.creditDate || new Date().toISOString().slice(0,10)}', ${s(d.originalInvoiceNumber)}, ${s(d.customerName)}, ${s(d.customerTaxId)}, '${d.reason||'return'}', ${s(d.reasonDescription)}, '${d.status||'draft'}', '${d.currency||'ILS'}', ${d.subtotal||0}, ${d.vatRate||18}, ${d.vatAmount||0}, ${d.totalAmount||0}, ${s(d.refundMethod)}, ${s(d.notes)}, ${user?.id||'NULL'}, ${s(user?.fullName)})`);
  res.json((await q(`SELECT * FROM credit_notes WHERE credit_number='${num}'`))[0]);
  } catch (err: any) { /* שגיאה ביצירת זיכוי */ console.error("POST /credit-notes error:", err.message); res.status(500).json({ error: "שגיאה ביצירת זיכוי" }); }
});
...
router.post("/customer-refunds", async (req, res) => {
  ...
  await q(`INSERT INTO customer_refunds (refund_number, refund_date, customer_name, customer_tax_id, original_invoice_number, reason, reason_description, status, currency, subtotal, vat_rate, vat_amount, total_amount, refund_method, notes, created_by, created_by_name)
    VALUES ('${num}', '${safeDate2(d.refundDate)}', ${s(d.customerName)}, ${s(d.customerTaxId)}, ${s(d.originalInvoiceNumber)}, '${reason}', ${s(d.reasonDescription)}, '${status}', '${currency}', ${safeNum2(d.subtotal)}, ${safeNum2(d.vatRate,18)}, ${safeNum2(d.vatAmount)}, ${safeNum2(d.totalAmount)}, '${refundMethod}', ${s(d.notes)}, ${user?.id||'NULL'}, ${s(user?.fullName)})`);
  res.json((await q(`SELECT * FROM customer_refunds WHERE refund_number='${num}'`))[0]);
  } catch (err: any) { /* שגיאה ביצירת החזר ללקוח */ console.error("POST /customer-refunds error:", err.message); res.status(500).json({ error: "שגיאה ביצירת החזר ללקוח" }); }
});
...
router.post("/customer-payments", async (req, res) => {
  ...
  } catch (err: any) { /* שגיאה ביצירת תשלום לקוח */ console.error("POST /customer-payments error:", err.message); res.status(500).json({ error: "שגיאה ביצירת תשלום לקוח" }); }
});
...
router.post("/supplier-invoices", async (req, res) => {
  ...
  await q(`INSERT INTO supplier_invoices (invoice_number, invoice_date, due_date, supplier_name, supplier_tax_id, status, currency, subtotal, vat_rate, vat_amount, total_amount, amount_paid, balance_due, payment_terms, item_description, notes, created_by, created_by_name)
    VALUES ('${num}', '${invoiceDate}', ${dueDate}, ${s(d.supplierName)}, ${s(d.supplierTaxId)}, '${status}', '${currency}', ${safeNum2(d.subtotal)}, ${safeNum2(d.vatRate,18)}, ${safeNum2(d.vatAmount)}, ${totalAmount}, ${amountPaid}, ${totalAmount - amountPaid}, '${paymentTerms}', ${s(d.itemDescription)}, ${s(d.notes)}, ${user?.id||'NULL'}, ${s(user?.fullName)})`);
  res.json((await q(`SELECT *, (total_amount - amount_paid) AS balance_due FROM supplier_invoices WHERE invoice_number='${num}'`))[0]);
  } catch (err: any) { /* שגיאה ביצירת חשבונית ספק */ console.error("POST /supplier-invoices error:", err.message); res.status(500).json({ error: "שגיאה ביצירת חשבונית ספק" }); }
});
...
router.post("/supplier-credit-notes", async (req, res) => {
  ...
  await q(`INSERT INTO supplier_credit_notes (credit_number, credit_date, supplier_name, supplier_tax_id, original_invoice_number, reason, reason_description, status, currency, subtotal, vat_rate, vat_amount, total_amount, notes, created_by, created_by_name)
    VALUES ('${num}', '${safeDate2(d.creditDate)}', ${s(d.supplierName)}, ${s(d.supplierTaxId)}, ${s(d.originalInvoiceNumber)}, '${reason}', ${s(d.reasonDescription)}, '${status}', '${currency}', ${safeNum2(d.subtotal)}, ${safeNum2(d.vatRate,18)}, ${safeNum2(d.vatAmount)}, ${safeNum2(d.totalAmount)}, ${s(d.notes)}, ${user?.id||'NULL'}, ${s(user?.fullName)})`);
  res.json((await q(`SELECT * FROM supplier_credit_notes WHERE credit_number='${num}'`))[0]);
  } catch (err: any) { /* שגיאה ביצירת זיכוי ספק */ console.error("POST /supplier-credit-notes error:", err.message); res.status(500).json({ error: "שגיאה ביצירת זיכוי ספק" }); }
});
...
router.post("/supplier-payments", async (req, res) => {
  ...
  } catch (err: any) { /* שגיאה ביצירת תשלום ספק */ console.error("POST /supplier-payments error:", err.message); res.status(500).json({ error: "שגיאה ביצירת תשלום ספק" }); }
});
