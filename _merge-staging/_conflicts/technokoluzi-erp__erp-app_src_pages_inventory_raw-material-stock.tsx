        ...
        {showForm && (
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <input className="bg-card/5 border border-white/10 rounded-lg p-2 text-white" placeholder="מזהה חומר" value={form.material_id} onChange={(e) => setForm({ ...form, material_id: e.target.value })} />
              <input className="bg-card/5 border border-white/10 rounded-lg p-2 text-white" placeholder="מזהה מחסן" value={form.warehouse_id} onChange={(e) => setForm({ ...form, warehouse_id: e.target.value })} />
              <input className="bg-card/5 border border-white/10 rounded-lg p-2 text-white" placeholder="קוד מיקום" value={form.location_code} onChange={(e) => setForm({ ...form, location_code: e.target.value })} />
              <input className="bg-card/5 border border-white/10 rounded-lg p-2 text-white" placeholder="מספר אצווה" value={form.batch_number} onChange={(e) => setForm({ ...form, batch_number: e.target.value })} />
              <input className="bg-card/5 border border-white/10 rounded-lg p-2 text-white" placeholder="מספר LOT" value={form.lot_number} onChange={(e) => setForm({ ...form, lot_number: e.target.value })} />
              <input className="bg-card/5 border border-white/10 rounded-lg p-2 text-white" placeholder="כמות" type="number" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} />
              <input className="bg-card/5 border border-white/10 rounded-lg p-2 text-white" placeholder="עלות ליחידה (₪)" type="number" value={form.unit_cost} onChange={(e) => setForm({ ...form, unit_cost: e.target.value })} />
              <input className="bg-card/5 border border-white/10 rounded-lg p-2 text-white" placeholder="תאריך קבלה" type="date" value={form.received_date} onChange={(e) => setForm({ ...form, received_date: e.target.value })} />
              <input className="bg-card/5 border border-white/10 rounded-lg p-2 text-white" placeholder="תאריך תפוגה" type="date" value={form.expiry_date} onChange={(e) => setForm({ ...form, expiry_date: e.target.value })} />
              <select className="bg-card/5 border border-white/10 rounded-lg p-2 text-white" value={form.quality_status} onChange={(e) => setForm({ ...form, quality_status: e.target.value })}>
                <option value="pending">ממתין לבדיקה</option>
                <option value="approved">מאושר</option>
                <option value="rejected">נדחה</option>
              </select>
              <textarea className="bg-card/5 border border-white/10 rounded-lg p-2 text-white md:col-span-2" placeholder="הערות" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
              <div className="flex gap-2">
                <button type="submit" className="bg-blue-600 hover:bg-blue-700 px-6 py-2 rounded-lg transition-colors">שמור</button>
                <button type="button" onClick={() => setShowForm(false)} className="bg-gray-600 hover:bg-gray-700 px-6 py-2 rounded-lg transition-colors">ביטול</button>
              </div>
            </form>
          </motion.div>
        )}
        ...
        <div className="bg-card/5 border border-white/10 rounded-xl p-4">
          <div className="flex items-center gap-4 mb-4">
            <div className="flex-1 relative">
              ...
              <input
                className="w-full bg-card/5 border border-white/10 rounded-lg pl-4 pr-10 py-2 text-white"
