  ...
  const renderTreeNode = (node: Account, depth: number = 0): JSX.Element => {
    ...
    return (
      <Fragment key={node.id}>
        <tr className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30 transition-colors cursor-pointer" onClick={() => setSelectedItem(node)}>
          <td className="px-3 py-2.5" style={{ paddingRight: `${12 + depth * 24}px` }}>
            <div className="flex items-center gap-2">
              {hasChildren ? (
                <button onClick={e => { e.stopPropagation(); toggleExpand(node.id); }} className="text-muted-foreground hover:text-white p-0.5">
          ...
          <td className="px-3 py-2.5 text-white text-sm font-medium">{node.account_name}</td>
          ...
          <td className="px-3 py-2.5"><Badge className={`${statusConfig[node.status]?.color || "bg-[#2a2a3e]"} border-0 text-[10px]`}>{statusConfig[node.status]?.label || node.status}</Badge></td>
          <td className="px-3 py-2.5 text-center" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-center gap-1">
              <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-white" onClick={() => setSelectedItem(node)}><Eye className="h-3.5 w-3.5" /></Button>
              ...
              {hasChildren ? null : <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-red-400" onClick={async () => { if (await globalConfirm(`למחוק את חשבון '${node.account_number} — ${node.account_name}'? פעולה זו אינה ניתנת לביטול.`)) remove(node.id); }}><Trash2 className="h-3.5 w-3.5" /></Button>}
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><FolderTree className="text-cyan-400" /> עץ חשבונות</h1>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="outline" onClick={() => printPage("עץ חשבונות")} className="border-[#2a2a3e] text-muted-foreground gap-1"><Printer className="h-4 w-4" />הדפסה</Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {pieData.length > 1 && (
            <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-4">
              <div className="flex items-center gap-2 mb-3 text-sm font-semibold text-white"><Layers className="h-4 w-4 text-cyan-400" /> חלוקת חשבונות לפי סוג</div>
          ...
          {balanceByType.length > 1 && (
            <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-4">
              <div className="flex items-center gap-2 mb-3 text-sm font-semibold text-white"><BarChart3 className="h-4 w-4 text-emerald-400" /> יתרות לפי סוג חשבון</div>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={balanceByType} layout="vertical">
                  ...
                  <Tooltip formatter={(v: number) => `₪${fmt(v)}`} contentStyle={{ backgroundColor: "#1a1a2e", border: "1px solid #2a2a3e", borderRadius: "8px" }} />
      ...
      <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]"><Search className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={e => { setSearch(e.target.value); pagination.setPage(1); }} placeholder="חיפוש לפי קוד חשבון, שם..." className="pr-9 bg-[#0a0a1a] border-[#2a2a3e] text-white" /></div>
        <select value={filterType} onChange={e => { setFilterType(e.target.value); pagination.setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הסוגים</option>{Object.entries(typeConfig).slice(0, 5).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select>
        ...
        <div className="flex bg-[#0a0a1a] border border-[#2a2a3e] rounded-md overflow-hidden">
          <button onClick={() => setViewMode("tree")} className={`px-3 py-2 text-xs ${viewMode === "tree" ? "bg-cyan-600 text-white" : "text-muted-foreground hover:text-white"}`}>עץ</button>
          <button onClick={() => setViewMode("flat")} className={`px-3 py-2 text-xs ${viewMode === "flat" ? "bg-cyan-600 text-white" : "text-muted-foreground hover:text-white"}`}>שטוח</button>
        </div>
        {viewMode === "tree" && <>
      ...
      {viewMode === "flat" && (
        <BulkActions items={filtered} selectedIds={selectedIds} onSelectionChange={setSelectedIds} actions={[
          defaultBulkActions.delete(async (ids) => { for (const id of ids) await authFetch(`${API}/chart-of-accounts/${id}`, { method: "DELETE", headers }); load(); }),
      ...
      <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-0">
        <div className="overflow-x-auto relative">
          {tableLoading && <div className="absolute inset-0 bg-[#0a0a1a]/60 backdrop-blur-[1px] flex items-center justify-center z-10"><div className="flex items-center gap-2 bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg px-4 py-2 shadow-lg"><Loader2 className="w-4 h-4 animate-spin text-cyan-400" /><span className="text-sm text-white">טוען עץ חשבונות...</span></div></div>}
          <table className="w-full text-sm">
            <thead><tr className="border-b border-[#2a2a3e] bg-[#0a0a1a]/50">
              ...
              {[{ key: "account_number", label: "קוד חשבון" }, { key: "account_name", label: "שם חשבון" }, { key: "account_type", label: "סוג" }, { key: "current_balance", label: "יתרה" }, { key: "status", label: "סטטוס" }].map(col => (
                <th key={col.key} className="px-3 py-3 text-right text-muted-foreground font-medium cursor-pointer hover:text-white" onClick={() => viewMode === "flat" && toggleSort(col.key)}><div className="flex items-center gap-1 text-xs">{col.label}{viewMode === "flat" && <ArrowUpDown className="h-3 w-3" />}</div></th>
              ))}
            ...
            <tbody>
              {!tableLoading && viewMode === "tree" && tree.length === 0 && filtered.length === 0 ? (
                <tr><td colSpan={7} className="p-16 text-center"><div className="flex flex-col items-center gap-4"><FolderTree className="h-12 w-12 text-[#2a2a3e]" /><p className="text-lg font-medium text-muted-foreground">עדיין אין חשבונות בעץ</p><p className="text-sm text-muted-foreground/60">צור חשבון ראשון</p><Button onClick={() => openCreate()} className="bg-cyan-600 hover:bg-cyan-700 gap-2 mt-2"><Plus className="h-4 w-4" />חשבון חדש</Button></div></td></tr>
              ) : viewMode === "tree" ? (
                tree.length > 0 ? tree.map(node => renderTreeNode(node)) : filtered.map(r => renderTreeNode(r))
              ) : (
                pagination.paginate(filtered).length === 0 ? (
                  <tr><td colSpan={7} className="p-16 text-center"><div className="flex flex-col items-center gap-4"><Search className="h-12 w-12 text-[#2a2a3e]" /><p className="text-lg font-medium text-muted-foreground">לא נמצאו תוצאות</p></div></td></tr>
                  ...
                  return (
                    <tr key={r.id} className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30 transition-colors cursor-pointer" onClick={() => setSelectedItem(r)}>
                      ...
                      <td className="px-3 py-2.5 text-white text-sm font-medium">{r.account_name}</td>
                      ...
                      <td className="px-3 py-2.5 text-center" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-red-400" onClick={async () => { if (await globalConfirm(`למחוק את חשבון '${r.account_number} — ${r.account_name}'? פעולה זו אינה ניתנת לביטול.`)) remove(r.id); }}><Trash2 className="h-3.5 w-3.5" /></Button>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת חשבון" : "חשבון חדש"}</h2>
                <Button variant="ghost" size="sm" onClick={() => setShowForm(false)}><X className="h-4 w-4" /></Button>
              </div>
              <div className="p-4 space-y-5">
                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-cyan-400">פרטי חשבון</h3></div>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div><Label className="text-muted-foreground text-xs">קוד חשבון *</Label><Input value={form.accountNumber || ""} onChange={e => setForm({ ...form, accountNumber: e.target.value })} placeholder="1000" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1 font-mono" disabled={!!editing} /></div>
                  <div className="col-span-2"><Label className="text-muted-foreground text-xs">שם חשבון *</Label><Input value={form.accountName || ""} onChange={e => setForm({ ...form, accountName: e.target.value })} placeholder="שם חשבון..." className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div><Label className="text-muted-foreground text-xs">סוג חשבון *</Label><select value={form.accountType || "expense"} onChange={e => setForm({ ...form, accountType: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1">{Object.entries(typeConfig).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select></div>
                  ...
                  <div><Label className="text-muted-foreground text-xs">מטבע</Label><select value={form.currency || "ILS"} onChange={e => setForm({ ...form, currency: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option value="ILS">₪ ILS</option><option value="USD">$ USD</option><option value="EUR">€ EUR</option></select></div>
                  <div className="flex items-end gap-3 pb-1">
                    <label className="flex items-center gap-2 text-sm text-white cursor-pointer"><input type="checkbox" checked={form.isGroup || false} onChange={e => setForm({ ...form, isGroup: e.target.checked })} className="rounded" />חשבון קבוצה</label>
                  </div>
                </div>

                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-cyan-400">היררכיה</h3></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><Label className="text-muted-foreground text-xs">חשבון אב</Label><select value={form.parentAccountId || ""} onChange={e => { const pid = e.target.value; const parent = items.find(i => i.id === Number(pid)); setForm({ ...form, parentAccountId: pid, parentAccountNumber: parent?.account_number || "" }); }} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option value="">ללא (שורש)</option>{items.filter(i => i.is_group && (!editing || i.id !== editing.id)).map(a => <option key={a.id} value={a.id}>{a.account_number} — {a.account_name}</option>)}</select></div>
                  <div><Label className="text-muted-foreground text-xs">יתרת פתיחה (₪)</Label><Input type="number" step={0.01} value={form.openingBalance || ""} onChange={e => setForm({ ...form, openingBalance: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1 font-mono" /></div>
                </div>

                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-cyan-400">מיסוי ומרכזי עלות</h3></div>
                <div className="grid grid-cols-3 gap-4">
                  <div><Label className="text-muted-foreground text-xs">קטגוריית מס</Label><Input value={form.taxCategory || ""} onChange={e => setForm({ ...form, taxCategory: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">שיעור מס (%)</Label><Input type="number" step={0.01} value={form.taxRate || ""} onChange={e => setForm({ ...form, taxRate: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1 font-mono" /></div>
                  <div><Label className="text-muted-foreground text-xs">מרכז עלות</Label><Input value={form.costCenter || ""} onChange={e => setForm({ ...form, costCenter: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                </div>

                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-cyan-400">תיאור</h3></div>
                <textarea value={form.description || ""} onChange={e => setForm({ ...form, description: e.target.value })} rows={2} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white resize-none" placeholder="תיאור החשבון..." />
                <textarea value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} rows={2} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white resize-none" placeholder="הערות..." />
              </div>
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
              <div className="flex items-center gap-3">
                <h2 className="text-lg font-bold text-white font-mono">{selectedItem.account_number}</h2>
                <h3 className="text-white font-medium">{selectedItem.account_name}</h3>
            ...
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={i} className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">{d.l}</p><p className={`mt-1 font-medium text-sm ${(d as any).c || "text-white"}`}>{d.v}</p></div>
                ))}
              </div>

              <div className="bg-[#0a0a1a] rounded-lg border border-[#2a2a3e] p-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  <div><p className="text-[11px] text-muted-foreground mb-1">יתרת פתיחה</p><p className="text-lg font-bold font-mono text-white">₪{fmt(selectedItem.opening_balance)}</p></div>
              ...
              {(selectedItem.tax_category || selectedItem.cost_center || selectedItem.department) && (
                <div className="grid grid-cols-3 gap-3">
                  {selectedItem.tax_category && <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">קטגוריית מס</p><p className="text-white mt-1 text-sm">{selectedItem.tax_category}</p></div>}
                  {selectedItem.cost_center && <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מרכז עלות</p><p className="text-white mt-1 text-sm">{selectedItem.cost_center}</p></div>}
                  {selectedItem.department && <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מחלקה</p><p className="text-white mt-1 text-sm">{selectedItem.department}</p></div>}
                </div>
              )}

              {selectedItem.description && <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground mb-1">תיאור</p><p className="text-sm text-white">{selectedItem.description}</p></div>}
            ...
            <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-end">
              <Button variant="outline" className="border-[#2a2a3e] gap-1" onClick={() => printPage(`חשבון ${selectedItem.account_number}`)}><Printer className="h-4 w-4" />הדפסה</Button>
