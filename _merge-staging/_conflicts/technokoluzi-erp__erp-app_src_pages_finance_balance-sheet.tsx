...
export default function BalanceSheetPage() {
  ...
  const getExportRows = () => {
    if (!data) return [];
    return [
      ...data.assets.map(a => ({ ...a, section: "נכסים" })),
      ...data.liabilities.map(l => ({ ...l, section: "התחייבויות" })),
      ...data.equity.map(e => ({ ...e, section: "הון עצמי" })),
    ];
  };
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 no-print">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 bg-slate-600 hover:bg-slate-700 text-white px-3 py-1.5 rounded-lg text-sm transition-colors"
            >
