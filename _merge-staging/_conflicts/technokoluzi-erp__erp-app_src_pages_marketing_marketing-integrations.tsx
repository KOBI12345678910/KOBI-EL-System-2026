    ...
    <div dir="rtl" className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg sm:text-2xl font-bold text-white">חיבורי פלטפורמות שיווק</h1>
          </div>
          <p className="text-muted-foreground text-sm mt-1 mr-10">חבר פלטפורמות פרסום דיגיטלי לסנכרון אוטומטי של נתוני קמפיינים</p>
        </div>
        <button onClick={loadConnections} className="flex items-center gap-2 px-3 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg text-sm">
          <RefreshCw size={14} className={loading ? "animate-spin" : ""} />רענון
        </button>
      </div>

      <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 text-sm text-muted-foreground">
      ...
      <div className="space-y-4">
        {PLATFORMS.map(platform => {
          const state = connections[platform.id];
          return (
            <div key={platform.id} className={`${state.connected ? platform.bgColor : "bg-gray-900 border-gray-800"} border rounded-xl overflow-hidden`}>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-white font-semibold">{platform.nameHe}</span>
                        ...
                        {state.connected ? (
                          ...
                          <span className="flex items-center gap-1 text-xs text-muted-foreground bg-gray-700 px-2 py-0.5 rounded-full">
                  ...
                  <div className="flex items-center gap-2">
                    {state.connected && (
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-xs disabled:opacity-50"
                      >
                      ...
                      <div className="flex gap-2">
                        ...
                        {state.connected && (
                          <button
                            ...
                            disabled={state.syncing}
                            className="flex items-center gap-2 px-4 py-2 bg-green-700 hover:bg-green-600 text-white rounded-lg text-sm disabled:opacity-50"
                          >
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="bg-gray-800 rounded-lg p-3 flex items-center justify-between">
              ...
              <span className="text-xs bg-gray-700 text-gray-300 px-2 py-0.5 rounded">{ep.label}</span>
            </div>
          ))}
        </div>
      </div>
