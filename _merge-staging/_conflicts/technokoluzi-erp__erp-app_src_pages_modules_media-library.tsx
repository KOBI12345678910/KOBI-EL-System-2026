        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-gray-700 hover:bg-gray-600 text-white rounded-lg font-medium text-sm"
            >
              <Folder className="w-4 h-4" /> אלבום חדש
            </button>
            <button
              onClick={() => setShowUpload(true)}
              className="flex items-center gap-2 px-5 py-2.5 bg-violet-600 hover:bg-violet-500 text-white rounded-lg font-medium text-sm"
            >
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-0 sm:min-w-[200px]">
            ...
            <input
              ...
              className="w-full pr-9 pl-4 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white placeholder-gray-500 text-sm focus:outline-none focus:border-violet-500"
            />
          </div>
          <select
            value={filterType}
            onChange={e => setFilterType(e.target.value)}
            className="px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white text-sm focus:outline-none"
          >
          ...
          <div className="flex gap-1 bg-[#1a1d23] border border-gray-700 rounded-lg p-1">
            <button onClick={() => setViewMode("grid")} className={`p-1.5 rounded ${viewMode === "grid" ? "bg-violet-600 text-white" : "text-muted-foreground hover:text-white"}`}>
              <Grid3x3 className="w-4 h-4" />
            </button>
            <button onClick={() => setViewMode("list")} className={`p-1.5 rounded ${viewMode === "list" ? "bg-violet-600 text-white" : "text-muted-foreground hover:text-white"}`}>
              <List className="w-4 h-4" />
            </button>
          </div>
          {selectedFiles.size > 0 && (
            <button onClick={deleteSelected} className="flex items-center gap-1.5 px-3 py-2 bg-red-600 hover:bg-red-500 text-white rounded-lg text-sm font-medium">
            ...
            <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-4">
              ...
              <div className="space-y-2">
                <button
                  ...
                  className={`w-full text-right px-3 py-2 rounded-lg text-sm transition-colors ${filterAlbum === "all" ? "bg-violet-600 text-white" : "text-muted-foreground hover:text-white hover:bg-gray-700"}`}
                >
                  <div className="flex items-center justify-between">
                    <span>כל הקבצים</span>
                    <span className="text-xs bg-gray-700 rounded-full px-2 py-0.5">{files.length}</span>
                  </div>
                </button>
                {albums.map(album => (
                  <button
                    ...
                    className={`w-full text-right px-3 py-2 rounded-lg text-sm transition-colors ${filterAlbum === album.name ? "bg-violet-600 text-white" : "text-muted-foreground hover:text-white hover:bg-gray-700"}`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="truncate">{album.name}</span>
                      <span className="text-xs bg-gray-700 rounded-full px-2 py-0.5 flex-shrink-0">{album.fileCount}</span>
                    </div>
                    {album.customer && (
                ...
                {filtered.map(file => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`bg-[#1a1d23] border rounded-xl overflow-hidden cursor-pointer group relative transition-all ${isSelected ? "border-violet-500" : "border-gray-800 hover:border-gray-600"}`}
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-black/70 flex items-center justify-center gap-2 py-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={e => { e.stopPropagation(); setSelectedFile(file); }} className="p-1 text-white hover:text-violet-400">
                          <Eye className="w-4 h-4" />
                        </button>
                        <button onClick={e => { e.stopPropagation(); deleteFile(file.id); }} className="p-1 text-white hover:text-red-400">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.map(file => {
                      ...
                      const isSelected = selectedFiles.has(file.id);
                      return (
                        <tr key={file.id} className={`border-b border-gray-800 hover:bg-gray-800/40 transition-colors ${isSelected ? "bg-violet-900/20" : ""}`}>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-1">
                              <button onClick={() => setSelectedFile(file)} className="p-1.5 text-muted-foreground hover:text-violet-400 hover:bg-gray-700 rounded">
                                <Eye className="w-4 h-4" />
                              </button>
                              <button onClick={() => deleteFile(file.id)} className="p-1.5 text-muted-foreground hover:text-red-400 hover:bg-gray-700 rounded">
                                <Trash2 className="w-4 h-4" />
                              </button>
                            </div>
                          </td>
      ...
      <AnimatePresence>
        {showUpload && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center" onClick={() => !uploading && setShowUpload(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-lg mx-4" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white">העלאת קבצים</h2>
                {!uploading && <button onClick={() => setShowUpload(false)} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>}
              </div>
              <div className="p-6 space-y-4">
                <div
                  className={`border-2 border-dashed rounded-xl p-10 text-center transition-colors ${dragOver ? "border-violet-500 bg-violet-900/20" : "border-gray-600 hover:border-gray-500"}`}
                ...
                >
                  ...
                  <p className="text-white font-medium mb-1">גרור קבצים לכאן</p>
        ...
        {showNewAlbum && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center" onClick={() => setShowNewAlbum(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-md mx-4" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white">אלבום חדש</h2>
                <button onClick={() => setShowNewAlbum(false)} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">שם האלבום *</label>
                  <input value={newAlbumForm.name} onChange={e => setNewAlbumForm({ ...newAlbumForm, name: e.target.value })} placeholder='למשל: פרויקט כהן - שערים' className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-violet-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">תיאור</label>
                  <input value={newAlbumForm.description} onChange={e => setNewAlbumForm({ ...newAlbumForm, description: e.target.value })} placeholder="תיאור קצר..." className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-violet-500" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1">פרויקט</label>
                    <input value={newAlbumForm.project} onChange={e => setNewAlbumForm({ ...newAlbumForm, project: e.target.value })} placeholder="שם פרויקט" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-violet-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1">לקוח</label>
                    <input value={newAlbumForm.customer} onChange={e => setNewAlbumForm({ ...newAlbumForm, customer: e.target.value })} placeholder="שם לקוח" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-violet-500" />
                  </div>
                </div>
                <div className="flex gap-3 pt-2">
                  <button onClick={createAlbum} className="flex-1 px-4 py-2.5 bg-violet-600 hover:bg-violet-500 text-white rounded-lg font-medium text-sm">
                    צור אלבום
                  </button>
                  <button onClick={() => setShowNewAlbum(false)} className="px-4 py-2.5 text-muted-foreground hover:text-white text-sm">ביטול</button>
                </div>
              </div>
            </motion.div>
          </motion.div>
      ...
      <AnimatePresence>
        {selectedFile && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setSelectedFile(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-2xl" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <h2 className="text-lg font-bold text-white truncate">{selectedFile.name}</h2>
                <button onClick={() => setSelectedFile(null)} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-white capitalize">{selectedFile.type === "image" ? "תמונה" : selectedFile.type === "video" ? "סרטון" : selectedFile.type === "document" ? "מסמך" : selectedFile.type}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">גודל</p>
                    <p className="text-white">{formatSize(selectedFile.size)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">תאריך העלאה</p>
                    <p className="text-white">{formatDate(selectedFile.uploadedAt)}</p>
                  </div>
                  ...
                  {selectedFile.album && (
                    <div>
                      ...
                      <p className="text-white">{selectedFile.album}</p>
                    </div>
                  )}
                  {selectedFile.project && (
                    <div>
                      <p className="text-muted-foreground">פרויקט</p>
                      <p className="text-white">{selectedFile.project}</p>
                    </div>
                  )}
                  {selectedFile.customer && (
                    <div>
                      <p className="text-muted-foreground">לקוח</p>
                      <p className="text-white">{selectedFile.customer}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  {selectedFile.url !== "#" && (
                    <a href={selectedFile.url} download={selectedFile.name} className="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-lg text-sm font-medium">
                      <Download className="w-4 h-4" /> הורדה
                    </a>
                  )}
                  <button onClick={() => { deleteFile(selectedFile.id); }} className="flex items-center gap-2 px-4 py-2 bg-red-700 hover:bg-red-600 text-white rounded-lg text-sm font-medium">
                    <Trash2 className="w-4 h-4" /> מחיקה
                  </button>
                  <button onClick={() => setSelectedFile(null)} className="px-4 py-2 text-muted-foreground hover:text-white text-sm">סגור</button>
                </div>
              </div>
            </motion.div>
          </motion.div>
