    ...
    return (
      <div className="p-6 space-y-4" dir="rtl">
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><TrendingUp className="h-6 w-6 text-blue-400" />דשבורד BI</h1><p className="text-sm text-muted-foreground mt-1">לוח בקרה אנליטי — מדדים, גרפים וניתוחים מתקדמים</p></div>
        ...
        <Card className="bg-card/60 border-border"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
          ...
          <select value={rptFilter} onChange={e => { setRptFilter(e.target.value); setPage(1); }} className="bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground"><option value="all">כל הסוגים</option>{REPORT_TYPES.map((o: string) => <option key={o} value={o}>{o}</option>)}</select>
        ...
        {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-card border border-border rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          ...
          <div className="p-4 space-y-6">
            <div className="border-b border-border pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי דשבורד</h3></div>
            ...
            <div><Label className="text-muted-foreground text-xs">סוג *</Label><select className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1"><option>בחר...</option><option>מכירות</option><option>כספים</option><option>ייצור</option><option>HR</option><option>לקוחות</option><option>מלאי</option><option>פרויקטים</option><option>כללי</option></select></div>
            <div><Label className="text-muted-foreground text-xs">מקור נתונים</Label><select className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1"><option>בחר...</option><option>PostgreSQL</option><option>API</option><option>Excel</option><option>CSV</option><option>Mixed</option></select></div>
            <div><Label className="text-muted-foreground text-xs">רענון</Label><select className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1"><option>בחר...</option><option>זמן אמת</option><option>כל שעה</option><option>יומי</option><option>שבועי</option></select></div>
            <div><Label className="text-muted-foreground text-xs">משותף</Label><div className="mt-2"><input type="checkbox" defaultChecked className="rounded" /><span className="text-sm text-foreground mr-2">כן</span></div></div>
            <div><Label className="text-muted-foreground text-xs">תזמון אימייל</Label><div className="mt-2"><input type="checkbox" defaultChecked className="rounded" /><span className="text-sm text-foreground mr-2">כן</span></div></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1"><option>בחר...</option><option>פעיל</option><option>טיוטה</option><option>משותף</option><option>ארכיון</option></select></div>
            </div>
          <div className="border-b border-border pb-2"><h3 className="text-sm font-semibold text-blue-400">הגדרות</h3></div>
            <div className="grid grid-cols-3 gap-4">
            <div className="col-span-3"><Label className="text-muted-foreground text-xs">תיאור</Label><textarea rows={3} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1 resize-none" placeholder="תיאור הדשבורד..." /></div>
            <div><Label className="text-muted-foreground text-xs">קבוצת הרשאה</Label><select className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1"><option>בחר...</option><option>הכל</option><option>הנהלה</option><option>מנהלים</option><option>כספים</option></select></div>
        ...
        {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-card border border-border rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">יוצר</p><p className="text-foreground mt-1 font-medium">{dr.creator}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">צופים</p><p className="text-foreground mt-1 font-medium">{dr.viewers}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">ווידג'טים</p><p className="text-foreground mt-1 font-medium">{dr.widgets}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">רענון</p><p className="text-foreground mt-1 font-medium">{dr.refreshRate}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מקור</p><p className="text-foreground mt-1 font-medium">{dr.dataSource}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">עדכון</p><p className="text-foreground mt-1 font-medium">{dr.lastUpdated}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">משותף</p><p className="text-foreground mt-1 font-medium">{dr.shared?"כן":"לא"}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מתוזמן</p><p className="text-foreground mt-1 font-medium">{dr.scheduled?"כן":"לא"}</p></div>
