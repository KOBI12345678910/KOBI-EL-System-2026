...
router.get("/work-orders", async (_req, res) => {
  res.json(await q(`SELECT * FROM work_orders ORDER BY due_date ASC NULLS LAST, id DESC`));
});
...
router.delete("/work-orders/:id", async (req, res) => {
  ...
  if (isNaN(id)) return res.status(400).json({ error: "מזהה לא חוקי" });
  try {
    const result = await db.execute(sql`DELETE FROM work_orders WHERE id = ${id} AND status = 'draft'`);
