    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
        ...
        <div className="p-6" dir="rtl">
          ...
          {/* Net amount */}
          <div className="bg-blue-600 text-white rounded-xl p-4 text-center mb-5">
        ...
        <div className="p-5 space-y-4" dir="rtl">
          {!result ? (<>
            ...
            <button onClick={runPayroll} disabled={running}
              className="w-full flex items-center justify-center gap-2 bg-emerald-600 text-white py-3 rounded-xl hover:bg-emerald-700 disabled:opacity-50 font-medium">
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => { onDone(); onClose(); }} className="w-full bg-emerald-600 text-white py-2.5 rounded-xl hover:bg-emerald-700 font-medium text-sm">
                סגור ורענן
              </button>
            </div>
          )}
      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setShowRunPayroll(true)}
            className="flex items-center gap-1.5 bg-emerald-600 text-white px-3 py-2 rounded-lg hover:bg-emerald-700 shadow text-sm font-medium">
          ...
          <button onClick={() => printPage("שכר")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Printer size={16} /> הדפסה</button>
          <button onClick={() => sendByEmail("שכר", generateEmailBody("שכר", items, { record_number: "מספר", employee_name: "עובד", gross_salary: "ברוטו", net_salary: "נטו", status: "סטטוס" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Send size={16} /> שליחה</button>
          <button onClick={openCreate} className="flex items-center gap-2 bg-blue-600 text-white px-3 py-2 rounded-lg hover:bg-blue-700 shadow-lg text-sm"><Plus size={16} /> תלוש חדש</button>
        </div>
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={save} className="flex items-center gap-2 bg-emerald-600 text-white px-6 py-2 rounded-lg hover:bg-emerald-700"><Save size={16} /> {editing ? "עדכון" : "שמירה"}</button>
