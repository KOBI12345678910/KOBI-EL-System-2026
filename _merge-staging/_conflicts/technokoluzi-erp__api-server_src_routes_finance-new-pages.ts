...
router.post("/finance/journal-transactions", async (req, res) => {
  ...
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה ביצירת תנועת יומן */ console.error("POST /finance/journal-transactions error:", err.message); res.status(500).json({ error: "שגיאה ביצירת תנועת יומן" }); }
});
...
router.post("/finance/audit-control", async (req, res) => {
  ...
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה ביצירת בקרת ביקורת */ console.error("POST /finance/audit-control error:", err.message); res.status(500).json({ error: "שגיאה ביצירת בקרת ביקורת" }); }
});
...
router.put("/finance/audit-control/:id", async (req, res) => {
  ...
  const rows = await q(`SELECT * FROM audit_controls WHERE id=${id}`);
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה בעדכון בקרת ביקורת */ console.error("PUT /finance/audit-control error:", err.message); res.status(500).json({ error: "שגיאה בעדכון בקרת ביקורת" }); }
});
...
router.post("/finance/annual-report", async (req, res) => {
  ...
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה ביצירת דוח שנתי */ console.error("POST /finance/annual-report error:", err.message); res.status(500).json({ error: "שגיאה ביצירת דוח שנתי" }); }
});
...
router.put("/finance/annual-report/:id", async (req, res) => {
  ...
  const rows = await q(`SELECT * FROM annual_reports WHERE id=${id}`);
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה בעדכון דוח שנתי */ console.error("PUT /finance/annual-report error:", err.message); res.status(500).json({ error: "שגיאה בעדכון דוח שנתי" }); }
});
...
router.post("/finance/accounting-inventory", async (req, res) => {
  ...
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה ביצירת פריט מלאי חשבונאי */ console.error("POST /finance/accounting-inventory error:", err.message); res.status(500).json({ error: "שגיאה ביצירת פריט מלאי חשבונאי" }); }
});
...
router.put("/finance/accounting-inventory/:id", async (req, res) => {
  ...
  const rows = await q(`SELECT * FROM accounting_inventory WHERE id=${id}`);
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה בעדכון מלאי חשבונאי */ console.error("PUT /finance/accounting-inventory error:", err.message); res.status(500).json({ error: "שגיאה בעדכון מלאי חשבונאי" }); }
});
...
router.post("/finance/depreciation-schedule", async (req, res) => {
  ...
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה ביצירת לוח פחת - כולל חישובים מורכבים */ console.error("POST /finance/depreciation-schedule error:", err.message); res.status(500).json({ error: "שגיאה ביצירת לוח פחת" }); }
});
...
router.post("/finance/loan-analysis", async (req, res) => {
  ...
  res.json(rows[0]);
  } catch (err: any) { /* שגיאה ביצירת ניתוח הלוואה - כולל חישוב תשלום חודשי */ console.error("POST /finance/loan-analysis error:", err.message); res.status(500).json({ error: "שגיאה ביצירת ניתוח הלוואה" }); }
});
