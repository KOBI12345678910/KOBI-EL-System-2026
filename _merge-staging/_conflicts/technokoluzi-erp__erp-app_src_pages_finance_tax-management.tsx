...
export default function TaxManagementPage() {
  ...
  const openCreate = () => { setEditing(null); setForm({ taxType: "vat", periodStart: new Date().toISOString().slice(0,10), periodEnd: new Date().toISOString().slice(0,10), taxRate: 18, status: "pending", filingStatus: "not_filed", currency: "ILS", taxAuthority: "רשות המסים" }); setShowForm(true); };
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => printPage("ניהול מיסים")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
            <Printer size={16} /> הדפסה
          </button>
          <button onClick={() => sendByEmail("ניהול מיסים - טכנו-כל עוזי", generateEmailBody("ניהול מיסים", records, { record_number: "מספר", tax_type: "סוג", tax_period: "תקופה", amount_due: "לתשלום", status: "סטטוס" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
            <Send size={16} /> שליחה
          </button>
          <button onClick={openCreate} className="flex items-center gap-2 bg-orange-600 text-white px-3 py-2 rounded-lg hover:bg-orange-700 shadow-lg text-sm">
                      ...
                      <tr className={`border-b hover:bg-orange-50/30 ${deadlineSoon ? 'bg-red-50/40' : ''}`}>
                        ...
                        <td className="px-3 py-2">
                          <div className="flex gap-1">
                            ...
                            <button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.tax_period || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1 hover:bg-red-500/10 rounded text-red-500" title="מחיקה"><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                      {isExpanded && (
          ...
          {deadlines.length === 0 ? (
            ...
            <div className="space-y-3">
              {deadlines.map((d, i) => {
                ...
                const urgencyBadge = isOverdue ? 'bg-red-600 text-white' : isUrgent ? 'bg-orange-500 text-white' : isSoon ? 'bg-yellow-500 text-white' : 'bg-muted text-foreground';
                return (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={save} className="flex items-center gap-2 bg-orange-600 text-white px-6 py-2 rounded-lg hover:bg-orange-700"><Save size={16} /> {editing ? "עדכון" : "שמירה"}</button>
        ...
        {showFilingModal && filingRecord && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowFilingModal(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={submitFiling} className="flex items-center gap-2 bg-indigo-600 text-white px-6 py-2 rounded-lg hover:bg-indigo-700"><FileText size={16} /> הגש דיווח</button>
