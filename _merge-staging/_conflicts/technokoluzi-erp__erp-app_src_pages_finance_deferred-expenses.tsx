...
import { TrendingDown, Search, Plus, Edit2, Trash2, X, Save, AlertTriangle, ArrowUpDown, DollarSign, Hash, Clock, Eye, Calendar } from "lucide-react";
...
interface DeferredExpense { id: number; description: string; vendor_name: string; total_amount: number; recognized_amount: number; remaining_amount: number; start_date: string; end_date: string; monthly_amount: number; category: string; status: string; account_number: string; notes: string; }
...
const categoryMap: Record<string, string> = { insurance: "ביטוח", rent: "שכירות", subscription: "מנוי", prepaid: "תשלום מראש", other: "אחר" };
...
export default function DeferredExpensesPage() {
  const [items, setItems] = useState<DeferredExpense[]>([]);
  ...
  const [editing, setEditing] = useState<DeferredExpense | null>(null);
  const [viewDetail, setViewDetail] = useState<DeferredExpense | null>(null);
  ...
  const load = async () => {
    ...
    try {
      const res = await authFetch(`${API}/deferred-expenses`);
  ...
  const filtered = useMemo(() => {
    let data = items.filter(i =>
      ...
      (!search || [i.description, i.vendor_name, i.account_number].some(f => f?.toLowerCase().includes(search.toLowerCase())))
    );
  ...
  const openCreate = () => { setEditing(null); setForm({ category: "insurance", status: "active", startDate: new Date().toISOString().slice(0, 10) }); setShowForm(true); };
  const openEdit = (r: DeferredExpense) => { setEditing(r); setForm({ description: r.description, vendorName: r.vendor_name, totalAmount: r.total_amount, startDate: r.start_date?.slice(0, 10), endDate: r.end_date?.slice(0, 10), category: r.category, status: r.status, accountNumber: r.account_number, notes: r.notes }); setShowForm(true); };

  const save = async () => {
    setSaving(true);
    try {
      const url = editing ? `${API}/deferred-expenses/${editing.id}` : `${API}/deferred-expenses`;
  ...
  const remove = async (id: number) => {
    if (await globalConfirm("למחוק הוצאה נדחית זו?")) { await authFetch(`${API}/deferred-expenses/${id}`, { method: "DELETE" }); load(); }
  };

  const totalAmount = items.reduce((s, i) => s + Number(i.total_amount || 0), 0);
  const recognized = items.reduce((s, i) => s + Number(i.recognized_amount || 0), 0);
  const remaining = items.reduce((s, i) => s + Number(i.remaining_amount || 0), 0);

  const kpis = [
    { label: "סה\"כ הוצאות נדחות", value: fmt(items.length), icon: Hash, color: "text-blue-400" },
    { label: "סכום כולל", value: fmtCurrency(totalAmount), icon: DollarSign, color: "text-red-400" },
    { label: "הוכר", value: fmtCurrency(recognized), icon: TrendingDown, color: "text-green-400" },
    { label: "יתרה", value: fmtCurrency(remaining), icon: Clock, color: "text-amber-400" },
    { label: "פעילות", value: fmt(items.filter(i => i.status === "active").length), icon: Calendar, color: "text-emerald-400" },
  ];
  ...
  const columns = [
    ...
    { key: "recognized_amount", label: "הוכר" },
    { key: "remaining_amount", label: "יתרה" },
  ...
  return (
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-2"><TrendingDown className="text-orange-400 w-6 h-6" /> הוצאות נדחות</h1>
          <p className="text-sm text-muted-foreground mt-1">ניהול הוצאות נדחות והכרה בהוצאות לאורך זמן</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <ExportDropdown data={filtered} headers={{ description: "תיאור", vendor_name: "ספק", total_amount: "סכום", recognized_amount: "הוכר", remaining_amount: "יתרה", status: "סטטוס" }} filename="deferred_expenses" />
          <button onClick={openCreate} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2.5 rounded-xl hover:bg-primary/90 shadow-lg text-sm font-medium"><Plus className="w-4 h-4" /> הוצאה חדשה</button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground"><TrendingDown className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>אין הוצאות נדחות</p></div>
      ) : (<>
        <BulkActions selectedIds={selectedIds} onClear={clear} entityName="items" actions={defaultBulkActions(selectedIds, clear, load, `${API}/deferred-expenses`)} />
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-muted-foreground">{r.vendor_name || "—"}</td>
                    <td className="px-4 py-3 text-white font-bold">{fmtCurrency(r.total_amount)}</td>
                    <td className="px-4 py-3 text-green-400">{fmtCurrency(r.recognized_amount)}</td>
                    <td className="px-4 py-3 text-amber-400">{fmtCurrency(r.remaining_amount)}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        ...
                        <button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.description || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1.5 hover:bg-muted rounded-lg"><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewDetail(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{viewDetail.description}</h2>
                ...
                <div className="flex gap-1 p-3 border-b border-border bg-muted/20">
                  {[
                    ...
                    <button key={tab.id} onClick={() => setDetailTab(tab.id)} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${detailTab === tab.id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-white"}`}>{tab.label}</button>
                  ))}
                </div>
              {detailTab === "details" ? (
                ...
                <DetailField label="הוכר" value={fmtCurrency(viewDetail.recognized_amount)} />
                <DetailField label="יתרה" value={fmtCurrency(viewDetail.remaining_amount)} />
                <DetailField label="סכום חודשי" value={fmtCurrency(viewDetail.monthly_amount)} />
                <DetailField label="תחילה" value={viewDetail.start_date?.slice(0, 10)} />
                ...
                <div className="p-5"><RelatedRecords entityType="deferred-expenses" entityId={viewDetail?.id} /></div>
                ) : detailTab === "attachments" ? (
                <div className="p-5"><AttachmentsSection entityType="deferred-expenses" entityId={viewDetail?.id} /></div>
                ) : (
                <div className="p-5"><ActivityLog entityType="deferred-expenses" entityId={viewDetail?.id} /></div>
                )}
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת הוצאה נדחית" : "הוצאה נדחית חדשה"}</h2>
                <button onClick={() => setShowForm(false)} className="p-1 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-5 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2"><label className="block text-sm font-medium text-muted-foreground mb-1.5">תיאור *</label><input value={form.description || ""} onChange={e => setForm({ ...form, description: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                  ...
                  <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">סכום כולל</label><input type="number" value={form.totalAmount || ""} onChange={e => setForm({ ...form, totalAmount: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                  <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">תאריך תחילה</label><input type="date" value={form.startDate || ""} onChange={e => setForm({ ...form, startDate: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
