    ...
    const [
      ...
      db.execute(sql`
        SELECT COUNT(*)::int as total,
          COUNT(*) FILTER (WHERE status = 'פעיל' OR status = 'חדש' OR status = 'active')::int as active,
          COUNT(*) FILTER (WHERE status = 'הושלם' OR status = 'completed')::int as completed,
          COALESCE(SUM(total), 0)::numeric as total_value,
          COALESCE(SUM(CASE WHEN created_at >= NOW() - INTERVAL '30 days' THEN total ELSE 0 END), 0)::numeric as monthly_value,
      ...
      db.execute(sql`
        SELECT COUNT(*)::int as total,
          COALESCE(SUM(total_amount), 0)::numeric as total_value,
          COALESCE(SUM(CASE WHEN status IN ('שולם', 'paid') THEN total_amount ELSE 0 END), 0)::numeric as paid_value,
          COUNT(*) FILTER (WHERE status IN ('שולם', 'paid'))::int as paid_count,
