...
import {
  ...
  AlertCircle, Loader2, FileCheck, ArrowLeftRight
} from "lucide-react";
...
export default function DocumentsPage() {
  ...
  const deleteFolder = async (id: number) => {
    if (!(await globalConfirm("למחוק תיקייה?"))) return;
        ...
        <header className="bg-card border-b px-6 py-3 flex items-center gap-3 shrink-0">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 text-sm font-medium shadow-sm"
            >
              <Plus size={16} /> חדש
            </button>
            <AnimatePresence>
                    ...
                    {filteredFolders.map(folder => (
                      ...
                      >
                        ...
                        <div className="opacity-0 group-hover:opacity-100 flex gap-1">
                          <button onClick={e => { e.stopPropagation(); setEditingFolder(folder); setEditFolderName(folder.name); }} className="p-1 hover:bg-muted rounded text-muted-foreground"><Edit2 size={14} /></button>
                          <button onClick={e => { e.stopPropagation(); deleteFolder(folder.id); }} className="p-1 hover:bg-red-500/10 rounded text-red-400"><Trash2 size={14} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    {filteredFolders.map(folder => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="opacity-0 group-hover:opacity-100 flex gap-1">
                            <button onClick={e => { e.stopPropagation(); setEditingFolder(folder); setEditFolderName(folder.name); }} className="p-1 hover:bg-muted/50 rounded"><Edit2 size={12} /></button>
                            <button onClick={e => { e.stopPropagation(); deleteFolder(folder.id); }} className="p-1 hover:bg-red-500/10 rounded text-red-400"><Trash2 size={12} /></button>
                          </div>
                        </div>
                    ...
                    {filteredFolders.map(folder => (
                      ...
                      >
                        ...
                        <div className="absolute top-2 left-2 opacity-0 group-hover:opacity-100 flex gap-0.5">
                          <button onClick={e => { e.stopPropagation(); setEditingFolder(folder); setEditFolderName(folder.name); }} className="p-1 bg-card/80 hover:bg-card rounded shadow text-muted-foreground"><Edit2 size={12} /></button>
                          <button onClick={e => { e.stopPropagation(); deleteFolder(folder.id); }} className="p-1 bg-card/80 hover:bg-card rounded shadow text-red-400"><Trash2 size={12} /></button>
                        </div>
                      </motion.div>
                    ))}
                  </div>
                    ...
                    {stats.recentFiles.map(file => (
                      ...
                      >
                        ...
                        <div className="mt-2 opacity-0 group-hover:opacity-100 flex justify-center gap-1">
                          ...
                          <button onClick={e => { e.stopPropagation(); deleteFile(file.id); }} className="p-1 bg-muted/50 hover:bg-red-500/10 rounded text-red-400"><Trash2 size={12} /></button>
                        </div>
                      </motion.div>
                    ))}
                  </div>
              ...
              {detailTab === "details" && (
                <div className="p-6 space-y-4">
                  ...
                  <div className="flex gap-2 pt-2">
                    <button onClick={() => downloadFile(selectedFile)} className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 text-sm"><Download size={14} /> הורד</button>
                  </div>
                </div>
              )}
              {detailTab === "related" && (
          ...
          {files.map(file => (
            <div key={file.id} className="flex items-center gap-4 px-4 py-2.5 hover:bg-muted/30 group">
              ...
              <div className="flex gap-1 w-28 opacity-0 group-hover:opacity-100">
                ...
                <button onClick={() => onDelete(file.id)} className="p-1 hover:bg-red-500/10 rounded text-red-400"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
          ...
          {files.map(file => (
            ...
            >
              ...
              <div className="absolute top-2 left-2 opacity-0 group-hover:opacity-100 flex gap-0.5">
                ...
                <button onClick={e => { e.stopPropagation(); onDelete(file.id); }} className="p-1 bg-card/80 hover:bg-card rounded shadow text-red-400"><Trash2 size={12} /></button>
              </div>
            </motion.div>
          ))}
        </div>
