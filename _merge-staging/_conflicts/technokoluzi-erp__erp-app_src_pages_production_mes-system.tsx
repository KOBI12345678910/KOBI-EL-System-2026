              ...
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie data={stageDistribution} cx="50%" cy="50%" outerRadius={90} dataKey="value" label={({ name }) => name}>
                    {stageDistribution.map((entry, index) => <Cell key={index} fill={entry.color} />)}
                  </Pie>
                  <Tooltip />
                </PieChart>
              </ResponsiveContainer>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {workstations.map((ws, i) => (
              <div key={i} className="bg-card border rounded-xl p-4 shadow-sm flex flex-wrap items-center justify-between gap-3">
