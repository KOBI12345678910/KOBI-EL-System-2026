...
function getAgentAnthropicConfigs(): AgentAnthropicConfig[] {
  ...
  if (process.env.ANTHROPIC_API_KEY) {
    configs.push({
      ...
      label: "Anthropic Direct (Sonnet Fallback)",
    });
  }
  if (configs.length === 0) {
    configs.push({ apiKey: "", baseUrl: "https://api.anthropic.com", model: "claude-opus-4-6", label: "None" });
  }
  return configs;
}
...
const BUSINESS_FLOWS: BusinessFlow[] = [
  ...
  },
  {
    ...
    execute: async (params) => {
      ...
      const ord = await pool.query(
        `INSERT INTO sales_orders (order_number, customer_id, customer_name, order_date, status, subtotal, total, payment_status, notes, created_at, updated_at)
         VALUES ($1,$2,$3,NOW(),$4,$5,$6,$7,$8,NOW(),NOW()) RETURNING id`,
        [orderNumber, customerId, custName, "draft", estimatedVal, estimatedVal, "pending", `מקור: ליד CRM #${lead_id}`]
      );
  ...
  },
  {
    ...
    execute: async (params) => {
      ...
      const ord = await pool.query(
        `INSERT INTO sales_orders (order_number, customer_id, customer_name, order_date, status, subtotal, total, payment_status, notes, created_at, updated_at)
         VALUES ($1,$2,$3,NOW(),$4,$5,$6,$7,$8,NOW(),NOW()) RETURNING id`,
        [orderNumber, customerId, custName, "draft", val, val, "pending", `מקור: הזדמנות CRM #${opportunity_id} — ${o.name || ""}`]
      );
  ...
  },
  {
    ...
    execute: async (params) => {
      ...
      const order = await pool.query("SELECT * FROM sales_orders WHERE id = $1", [order_id]);
      if (order.rows.length === 0) return { result: `❌ הזמנה ${order_id} לא נמצאה` };
