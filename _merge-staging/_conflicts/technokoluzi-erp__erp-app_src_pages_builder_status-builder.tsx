                ...
                <div key={status.id} className="bg-card border border-border rounded-xl p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={async () => { if (globalConfirm("למחוק את הסטטוס?")) deleteStatusMutation.mutate(status.id); }} className="p-1.5 hover:bg-destructive/10 rounded-lg"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
                    </div>
                  </div>
                </div>
              );
                ...
                {transitions.map(t => {
                  ...
                  return (
                    <div key={t.id} className="flex items-center gap-3 px-3 py-2 bg-muted/20 rounded-lg">
                      ...
                      <button onClick={async () => { if (globalConfirm("למחוק את המעבר?")) deleteTransitionMutation.mutate(t.id); }} className="p-1 hover:bg-destructive/10 rounded"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
                    </div>
                  );
                })}
              </div>
