      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><TrendingUp className="h-6 w-6 text-cyan-400" />דוח תפוקות</h1><p className="text-sm text-muted-foreground mt-1">מעקב תפוקות ייצור, כמויות ויחידות</p></div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"תפוקה חודשית","v":"12,450","c":"text-cyan-400","t":"+8%","u":true},{"l":"יעד","v":"13,000","c":"text-blue-400","t":"95.8%","u":true},{"l":"פריטים/שעה","v":"52","c":"text-green-400","t":"+4","u":true},{"l":"פסולת","v":"2.1%","c":"text-red-400","t":"-0.3%","u":true},{"l":"עמידה ביעד","v":"96%","c":"text-emerald-400","t":"+2%","u":true},{"l":"קווים פעילים","v":"5/6","c":"text-amber-400","t":"0","u":true}].map((k: any) => (
          ...
          <Card className="bg-card/60 border-border"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
            ...
            <select value={catFilter} onChange={e => { setCatFilter(e.target.value); setPage(1); }} className="bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground"><option value="all">כל התקופות</option>{CATS.map((o: string) => <option key={o} value={o}>{o}</option>)}</select>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-border/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-muted rounded" /><div className="h-4 w-16 bg-muted rounded" /><div className="h-4 w-32 bg-muted rounded" /><div className="h-4 w-24 bg-muted rounded" /><div className="h-4 w-20 bg-muted rounded" /><div className="h-4 w-16 bg-muted rounded" /><div className="h-4 w-28 bg-muted rounded" /><div className="h-4 w-12 bg-muted rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-muted-foreground" /> : <TrendingUp className="h-16 w-16 text-muted-foreground" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין דוח תפוקות"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "דוח חדש ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />דוח חדש</Button>}</div></td></tr> : pd.map(row => (
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-card border border-border rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div><Label className="text-muted-foreground text-xs">סוג תקופה *</Label><select className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1"><option>בחר...</option><option>יומי</option><option>שבועי</option><option>חודשי</option><option>רבעוני</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">מתוכנן</Label><Input type="number" placeholder="0" className="bg-input border-border text-foreground mt-1" /></div>
            ...
            <div><Label className="text-muted-foreground text-xs">פסולת (%)</Label><Input type="number" placeholder="0" className="bg-input border-border text-foreground mt-1" /></div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-card border border-border rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">תקופה</p><p className="text-foreground mt-1 font-medium">{dr.periodType}</p></div>
            ...
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מתוכנן</p><p className="text-foreground mt-1 font-medium">{dr.planned.toLocaleString()}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">בפועל</p><p className="text-foreground mt-1 font-medium">{dr.actual.toLocaleString()}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">עמידה ביעד</p><p className="text-foreground mt-1 font-medium">{dr.completionRate}%</p></div>
