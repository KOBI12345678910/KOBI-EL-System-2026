...
function GaugeIndicator({ label, value, unit }: { label: string; value: number; unit?: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 p-4 rounded-xl border border-gray-700/50 bg-gray-800/30">
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold text-gray-100">ניהול ערך נרכש (EVM)</h1>
      ...
      {loading ? (
        ...
        <>
              ...
              <div key={i} className={`rounded-xl bg-gradient-to-br ${k.color} p-4 border border-white/10`}>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xl font-bold text-white mt-1">{k.value}</div>
                  </div>
                  <k.icon className="w-6 h-6 text-white/30" />
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Summary Table */}
          <div className="rounded-xl border border-gray-700/50 bg-gray-800/30 p-5">
            <h2 className="text-lg font-semibold text-white mb-4">סיכום מדדים</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={i} className="p-3 rounded-lg bg-gray-700/30">
                  ...
                  <div className={`text-lg font-bold ${m.good !== undefined ? (m.good ? "text-emerald-400" : "text-red-400") : "text-white"}`}>
                    {m.value}
                  </div>
                </div>
              ))}
