  ...
  return (
    <div className="min-h-screen bg-[#0f1117]" dir="rtl">
      <div className="p-6 space-y-6">
        ...
        <div className="flex gap-1 bg-[#1a1d23] border border-gray-800 rounded-xl p-1 w-fit">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === tab.id ? "bg-cyan-600 text-white" : "text-muted-foreground hover:text-white"}`}
            >
        ...
        {activeTab === "export" && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-semibold text-white">ייצוא נתונים</h2>
              <button onClick={() => setShowExportForm(true)} className="flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-sm font-medium">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {exports.map(job => {
                    ...
                    return (
                      <tr key={job.id} className="border-b border-gray-800 hover:bg-gray-800/40 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                            {(job.status === "ready" || job.status === "failed") && (
                              <button onClick={() => runExport(job.id)} disabled={runningExport === job.id} className="p-1.5 text-cyan-400 hover:bg-gray-700 rounded" title="הרץ">
                                <Play className="w-3.5 h-3.5" />
                              </button>
                            )}
                            {job.status === "completed" && (
                              <button onClick={() => downloadExport(job)} className="p-1.5 text-emerald-400 hover:bg-gray-700 rounded" title="הורד">
                                <Download className="w-3.5 h-3.5" />
                              </button>
                            )}
                            <button onClick={() => setExports(prev => prev.filter(e => e.id !== job.id))} className="p-1.5 text-muted-foreground hover:text-red-400 hover:bg-gray-700 rounded" title="מחק">
        ...
        {activeTab === "scheduled" && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-semibold text-white">שליחות מתוזמנות</h2>
              <button onClick={() => setShowScheduledForm(true)} className="flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-sm font-medium">
              ...
              {scheduled.map(report => {
                ...
                return (
                  <motion.div
                    ...
                    className="bg-[#1a1d23] border border-gray-800 rounded-xl p-4 space-y-3"
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium transition-colors ${report.isActive ? "bg-emerald-900/50 text-emerald-400 border border-emerald-700/50" : "bg-gray-700 text-muted-foreground border border-gray-600"}`}
                      >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-[#12141a] rounded-lg p-2">
                        ...
                        <p className="text-white">{freqLabel(report.frequency)}</p>
                      </div>
                      <div className="bg-[#12141a] rounded-lg p-2">
                        <p className="text-muted-foreground">שעת שליחה</p>
                        <p className="text-white">{report.sendTime}</p>
                      </div>
                      <div className="bg-[#12141a] rounded-lg p-2">
                        <p className="text-muted-foreground">שליחה הבאה</p>
                        <p className="text-white">{report.nextSendAt}</p>
                      </div>
                      <div className="bg-[#12141a] rounded-lg p-2">
        ...
        {activeTab === "webhooks" && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => setShowWebhookForm(true)} className="flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-sm font-medium">
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h3 className="text-white font-medium">{hook.name}</h3>
                        <p className="text-muted-foreground text-xs font-mono truncate max-w-[250px]">{hook.url}</p>
                      </div>
                    </div>
                    <button
                      onClick={() => toggleWebhook(hook.id)}
                      className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium transition-colors ${hook.isActive ? "bg-emerald-900/50 text-emerald-400 border border-emerald-700/50" : "bg-gray-700 text-muted-foreground border border-gray-600"}`}
                    >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
                    <div className="bg-[#12141a] rounded-lg p-2">
                      ...
                      <p className="text-white">{hook.dataSource}</p>
                    </div>
                    <div className="bg-[#12141a] rounded-lg p-2">
                      <p className="text-muted-foreground">טריגר</p>
                      <p className="text-white">{triggerLabel(hook.trigger)}</p>
                    </div>
                    <div className="bg-[#12141a] rounded-lg p-2">
                      <p className="text-muted-foreground">הצלחות/כישלונות</p>
                      <p className="text-white"><span className="text-emerald-400">{hook.successCount}</span> / <span className="text-red-400">{hook.failureCount}</span></p>
                    </div>
                  </div>
                  ...
                  {Object.keys(hook.headers).length > 0 && (
                    <div className="bg-[#12141a] rounded-lg p-2 text-xs font-mono">
        ...
        {activeTab === "logs" && (
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <h2 className="text-lg font-semibold text-white">לוג שליחות</h2>
              <div className="relative flex-1 max-w-sm">
                <Search className="absolute right-3 top-2.5 text-muted-foreground w-4 h-4" />
                <input value={searchLog} onChange={e => setSearchLog(e.target.value)} placeholder="חיפוש בלוג..." className="w-full pr-9 pl-4 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white placeholder-gray-500 text-sm focus:outline-none" />
              </div>
            </div>
            <div className="bg-[#1a1d23] border border-gray-800 rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredLogs.map(log => (
                    <tr key={log.id} className="border-b border-gray-800 hover:bg-gray-800/30 transition-colors">
                      <td className="px-4 py-3 text-white font-medium">{log.name}</td>
                      <td className="px-4 py-3">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center" onClick={() => setShowExportForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-md mx-4" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white">ייצוא חדש</h2>
                <button onClick={() => setShowExportForm(false)} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">שם הייצוא *</label>
                  <input value={exportForm.name} onChange={e => setExportForm({ ...exportForm, name: e.target.value })} placeholder='למשל: לקוחות פעילים Q1' className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">מקור נתונים *</label>
                  <select value={exportForm.dataSource} onChange={e => setExportForm({ ...exportForm, dataSource: e.target.value })} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:outline-none focus:border-cyan-500">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {(["csv", "excel", "json"] as const).map(fmt => {
                      const FmtIcon = FORMAT_ICONS[fmt];
                      return (
                        <button key={fmt} onClick={() => setExportForm({ ...exportForm, format: fmt })} className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium flex-1 justify-center transition-all ${exportForm.format === fmt ? `${FORMAT_BG[fmt]} ${FORMAT_COLORS[fmt]} border-current` : "border-gray-700 text-muted-foreground hover:border-gray-500"}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="date" value={exportForm.dateFrom} onChange={e => setExportForm({ ...exportForm, dateFrom: e.target.value })} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:outline-none focus:border-cyan-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1">עד תאריך</label>
                    <input type="date" value={exportForm.dateTo} onChange={e => setExportForm({ ...exportForm, dateTo: e.target.value })} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:outline-none focus:border-cyan-500" />
                  </div>
                </div>
                <div className="flex gap-3 pt-2">
                  <button onClick={createExport} className="flex-1 px-4 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg font-medium text-sm">
                    צור ייצוא
                  </button>
                  <button onClick={() => setShowExportForm(false)} className="px-4 py-2.5 text-muted-foreground hover:text-white text-sm">ביטול</button>
                </div>
              </div>
            </motion.div>
          </motion.div>
      ...
      <AnimatePresence>
        {showScheduledForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center" onClick={() => setShowScheduledForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-md mx-4" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white">שליחה מתוזמנת חדשה</h2>
                <button onClick={() => setShowScheduledForm(false)} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">שם *</label>
                  <input value={scheduledForm.name} onChange={e => setScheduledForm({ ...scheduledForm, name: e.target.value })} placeholder="למשל: דוח שבועי לקוחות" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">מקור נתונים *</label>
                  <select value={scheduledForm.dataSource} onChange={e => setScheduledForm({ ...scheduledForm, dataSource: e.target.value })} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:outline-none focus:border-cyan-500">
                    <option value="">בחר מקור...</option>
                    {DATA_SOURCES.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={scheduledForm.format} onChange={e => setScheduledForm({ ...scheduledForm, format: e.target.value as "csv" | "excel" | "json" })} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:outline-none">
                  ...
                  <div>
                    ...
                    <select value={scheduledForm.frequency} onChange={e => setScheduledForm({ ...scheduledForm, frequency: e.target.value as "daily" | "weekly" | "monthly" })} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:outline-none">
                ...
                <div>
                  ...
                  <input type="time" value={scheduledForm.sendTime} onChange={e => setScheduledForm({ ...scheduledForm, sendTime: e.target.value })} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:outline-none focus:border-cyan-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">נמענים (מופרדים בפסיק) *</label>
                  <input value={scheduledForm.recipients} onChange={e => setScheduledForm({ ...scheduledForm, recipients: e.target.value })} placeholder="email1@co.com, email2@co.com" dir="ltr" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500 text-left" />
                </div>
                <div className="flex gap-3 pt-2">
                  <button onClick={createScheduled} className="flex-1 px-4 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg font-medium text-sm">
                    צור שליחה
                  </button>
                  <button onClick={() => setShowScheduledForm(false)} className="px-4 py-2.5 text-muted-foreground hover:text-white text-sm">ביטול</button>
                </div>
              </div>
            </motion.div>
          </motion.div>
      ...
      <AnimatePresence>
        {showWebhookForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center" onClick={() => setShowWebhookForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-md mx-4" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white">Webhook חדש</h2>
                <button onClick={() => setShowWebhookForm(false)} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">שם *</label>
                  <input value={webhookForm.name} onChange={e => setWebhookForm({ ...webhookForm, name: e.target.value })} placeholder="למשל: עדכון CRM" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">URL *</label>
                  <input value={webhookForm.url} onChange={e => setWebhookForm({ ...webhookForm, url: e.target.value })} placeholder="https://example.com/webhook" dir="ltr" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500 text-left" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1">מקור נתונים *</label>
                    <select value={webhookForm.dataSource} onChange={e => setWebhookForm({ ...webhookForm, dataSource: e.target.value })} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:outline-none">
                  ...
                  <div>
                    ...
                    <select value={webhookForm.trigger} onChange={e => setWebhookForm({ ...webhookForm, trigger: e.target.value as "on_create" | "on_update" | "on_delete" | "scheduled" })} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:outline-none">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input value={webhookForm.headerKey} onChange={e => setWebhookForm({ ...webhookForm, headerKey: e.target.value })} placeholder="Authorization" dir="ltr" className="flex-1 px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500 text-left text-sm" />
                    <input value={webhookForm.headerValue} onChange={e => setWebhookForm({ ...webhookForm, headerValue: e.target.value })} placeholder="Bearer ..." dir="ltr" className="flex-1 px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500 text-left text-sm" />
                  </div>
                </div>
                <div className="flex gap-3 pt-2">
                  <button onClick={createWebhook} className="flex-1 px-4 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg font-medium text-sm">
                    צור Webhook
                  </button>
                  <button onClick={() => setShowWebhookForm(false)} className="px-4 py-2.5 text-muted-foreground hover:text-white text-sm">ביטול</button>
                </div>
              </div>
            </motion.div>
          </motion.div>
