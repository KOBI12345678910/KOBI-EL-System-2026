...
interface DashboardData {
  ...
  alerts?: AlertItem[];
  recentActivity?: Array<{ action: string; entity_type: string; entity_id: string; details: string; created_at: string; user_id: string }>;
}
...
async function exportExcel(data: DashboardData | null) {
  if (!data) return;
  try {
    const res = await authFetch(`${API}/reports-center/executive-dashboard/export-excel`, {
      method: "POST",
    ...
    a.href = url;
    a.download = `executive-dashboard-${new Date().toISOString().slice(0, 10)}.xlsx`;
    a.click();
    URL.revokeObjectURL(url);
  } catch {
    alert("שגיאה ביצוא Excel. אנא נסה שוב.");
  }
}
...
function exportDashboardPDF(data: DashboardData | null) {
  ...
  const headers = { category: "קטגוריה", metric: "מדד", value: "ערך", change: "שינוי" };
  exportData("pdf", rows, headers, "דשבורד-מנהלים");
}
...
export default function KPIDashboard() {
  ...
  const { data, isLoading, refetch, dataUpdatedAt } = useQuery<DashboardData | null>({
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
        <div className="text-center">
          ...
          <p className="text-muted-foreground text-sm">טוען דשבורד מנהלים...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 pb-8" dir="rtl">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-3">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <div className="flex gap-1 mr-auto">
          <Button variant="outline" size="sm" className="h-8 text-xs border-slate-600" onClick={() => exportExcel(data)}>
            <FileSpreadsheet className="w-3.5 h-3.5 ml-1" /> Excel
          </Button>
          <Button variant="outline" size="sm" className="h-8 text-xs border-slate-600" onClick={() => exportDashboardCSV(data)}>
            <Download className="w-3.5 h-3.5 ml-1" /> CSV
          </Button>
          <Button variant="outline" size="sm" className="h-8 text-xs border-slate-600" onClick={() => exportDashboardPDF(data)}>
      ...
      <div data-report-content>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-4">
          <HealthScoreCard score={healthScore} kpis={kpis} production={production} alerts={alerts} inventory={inventory} />
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <Button variant="outline" size="sm" className="h-7 text-xs border-slate-600 mr-auto" onClick={() => navigate("/platform/data-flow-automations")}>
            <Zap className="w-3 h-3 ml-1 text-amber-400" /> הפעל אוטומציות
          </Button>
        </div>
                ...
                <CardContent>
                  <div className="h-56">
                    <ResponsiveContainer width="100%" height={224}>
                      <BarChart data={(() => { const all = charts?.salesTrend || []; const withData = all.filter(m => m.revenue > 0 || m.expenses > 0); return withData.length > 6 ? withData.slice(-6) : withData.length > 0 ? withData : all.slice(-6); })()} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                    ...
                    {recentActivity.slice(0, 5).map((a, i) => (
                      <div key={i} className="flex items-center gap-3 px-3 py-1.5 rounded-lg hover:bg-slate-800/50 text-xs transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {a.details && <span className="text-muted-foreground mr-2 truncate"> {typeof a.details === "string" ? a.details.substring(0, 40) : ""}</span>}
                        </div>
                    ...
                    <table className="w-full text-xs" dir="rtl">
                      <thead>
                        <tr className="border-b border-slate-700">
                          ...
                          <th className="text-center py-2 px-3 text-white font-medium">סה"כ</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr className="border-b border-slate-700/50 hover:bg-slate-800/30">
                          ...
                          <td className="py-2.5 px-3 text-center">
                            <span className="text-white font-bold">{fmt(invoiceAging.receivable.current.amount + invoiceAging.receivable.d0_30.amount + invoiceAging.receivable.d31_60.amount + invoiceAging.receivable.d61_90.amount + invoiceAging.receivable.d90_plus.amount)}</span>
                          </td>
                        </tr>
                        <tr className="hover:bg-slate-800/30">
                          ...
                          <td className="py-2.5 px-3 text-center">
                            <span className="text-white font-bold">{fmt(invoiceAging.payable.current.amount + invoiceAging.payable.d0_30.amount + invoiceAging.payable.d31_60.amount + invoiceAging.payable.d61_90.amount + invoiceAging.payable.d90_plus.amount)}</span>
                          </td>
                        </tr>
                      </tbody>
                    </table>
            ...
            {salesPipeline ? (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <Card className="bg-slate-900/50 border-slate-700/50">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-lg sm:text-2xl font-bold text-white">{salesPipeline.quotes.total}</span>
                ...
                <Card className="bg-slate-900/50 border-slate-700/50">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-lg sm:text-2xl font-bold text-white">{salesPipeline.orders.total}</span>
                      ...
                      {(salesPipeline.topCustomers || []).map((c: { name: string; order_count: number; total_value: number }, i: number) => (
                        <div key={i} className="flex items-center justify-between text-xs">
                          ...
                          <div className="text-left flex items-center gap-2 flex-shrink-0">
                            ...
                            <span className="text-white font-medium">{fmtCompact(Number(c.total_value))}</span>
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="space-y-2">
                      {(production.recentWorkOrders || []).map((wo: { wo_number: string; product_name: string; status: string; priority: string; produced: number; planned: number; due_date: string }, i: number) => (
                        <div key={i} className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-lg text-xs">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-white font-mono text-[10px]">{wo.wo_number}</span>
                      ...
                      {(inventory.lowStockItems || []).map((item, i) => {
                        ...
                        return (
                          <div key={i} className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-lg text-xs">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <span className="text-white font-mono text-[10px]">{item.material_number}</span>
                    ...
                    {projects.active > 0 && (
                      <div className="mb-4">
                        <div className="flex justify-between text-xs mb-1">
                          ...
                          <span className="text-white font-medium">{projects.avgCompletion}%</span>
                        </div>
                    ...
                    <div className="space-y-2">
                      {(projects.recentProjects || []).map((proj, i) => (
                        <div key={i} className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-lg text-xs">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-white font-mono text-[10px]">{proj.project_number}</span>
            ...
            <Card className="bg-slate-900/50 border-slate-700/50">
              ...
              <CardContent>
                <div className="space-y-1.5">
                  {recentActivity.map((a, i) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-800/50 text-xs transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="text-muted-foreground mx-1">—</span>
                        <span className="text-muted-foreground">{a.entity_type}</span>
                        {a.details && <span className="text-muted-foreground mr-2 truncate"> {typeof a.details === "string" ? a.details.substring(0, 50) : ""}</span>}
                      </div>
                      <span className="text-[10px] text-muted-foreground flex-shrink-0">
                        {new Date(a.created_at).toLocaleString("he-IL", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
                      </span>
                  ...
                  {recentActivity.length === 0 && (
                    <p className="text-sm text-muted-foreground text-center py-8">אין פעילות אחרונה</p>
                  )}
                </div>
              </CardContent>
            </Card>
...
function calculateHealthScore(kpis: Record<string, KpiMetric>, fs: DashboardData["financialSummary"] | undefined, production: ProductionData | null | undefined, alerts: AlertItem[]): number {
  let score = 70;
  if ((kpis.profitMargin?.value || 0) > 10) score += 10;
  else if ((kpis.profitMargin?.value || 0) > 0) score += 5;
  else score -= 10;
  if ((fs?.budgetVsActual?.utilization || 0) <= 80) score += 5;
  else if ((fs?.budgetVsActual?.utilization || 0) > 95) score -= 10;
  if ((fs?.accountsReceivable?.overdueCount || 0) === 0) score += 5;
  else score -= 5;
  if (production && production.efficiency >= 80) score += 5;
  score -= alerts.filter(a => a.severity === "high").length * 3;
  score -= alerts.filter(a => a.severity === "medium").length * 1;
  return Math.max(0, Math.min(100, score));
}

function HealthScoreCard({ score, kpis, production, alerts, inventory }: { score: number; kpis?: Record<string, KpiMetric>; production?: ProductionData | null; alerts?: AlertItem[]; inventory?: InventoryData | null }) {
  const color = score >= 80 ? "text-green-400" : score >= 60 ? "text-yellow-400" : "text-red-400";
  const bgColor = score >= 80 ? "from-green-600/20 to-green-600/5" : score >= 60 ? "from-yellow-600/20 to-yellow-600/5" : "from-red-600/20 to-red-600/5";
  const label = score >= 80 ? "מצוין" : score >= 60 ? "טוב" : "דורש תשומת לב";
  ...
  const indicators = [
    ...
    { label: "יעילות ייצור", status: (production?.efficiency || 0) >= 80 ? "good" : (production?.efficiency || 0) >= 50 ? "warn" : "bad", value: `${production?.efficiency || 0}%` },
...
function FinancialRow({ label, value, badge, badgeColor, valueColor, highlight, bold }: FinancialRowProps) {
  return (
    <div className={`flex items-center justify-between ${highlight ? "bg-slate-800/50 -mx-2 px-2 py-1.5 rounded-lg" : ""}`}>
      <div className="flex items-center gap-2">
        <span className={`text-xs ${bold ? "font-bold text-white" : "text-muted-foreground"}`}>{label}</span>
        {badge && (
          <Badge variant={badgeColor || "default"} className="text-[10px] h-4">{badge}</Badge>
        )}
      </div>
      <span className={`text-sm font-medium ${bold ? "font-bold" : ""} ${valueColor || "text-white"}`}>{value}</span>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
        <ActivityLog entityType="kpi" entityId="all" />
        <RelatedRecords entityType="kpi" entityId="all" />
      </div>
    </div>
  );
}