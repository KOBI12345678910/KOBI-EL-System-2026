...
import { Shield, Plus, Trash2, AlertTriangle, TrendingDown, Activity, Calculator } from "lucide-react";
                  ...
                  {exposures.map(e => {
                    ...
                    return (
                      <tr key={e.id} className="border-b border-slate-700/30 hover:bg-slate-800/30">
                        <td className="py-3 px-4 text-white font-mono">{e.currencyPair}</td>
                        ...
                        <td className="py-3 px-4 flex items-center gap-1">
                          ...
                          <Button size="sm" variant="ghost" className="text-red-400 hover:text-red-300 h-7 w-7 p-0" onClick={async () => { if (globalConfirm("למחוק?")) deleteExposureMutation.mutate(e.id); }}>
                ...
                <tbody>
                  {commodities.map(c => (
                    <tr key={c.id} className="border-b border-slate-700/30 hover:bg-slate-800/30">
                      <td className="py-3 px-4 text-white">{c.materialName}</td>
                      ...
                      <td className="py-3 px-4">
                        <Button size="sm" variant="ghost" className="text-red-400 hover:text-red-300 h-7 w-7 p-0" onClick={async () => { if (globalConfirm("למחוק?")) deleteCommodityMutation.mutate(c.id); }}>
                          <Trash2 className="w-3.5 h-3.5" />
                        </Button>
                      </td>
                    </tr>
        ...
        <DialogContent className="bg-slate-900 border-slate-700 max-w-lg" dir="rtl">
          ...
          {selectedExposure && (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">חשיפה: <span className="text-white font-medium">{fmt(parseFloat(selectedExposure.exposureAmount ?? "0"))}</span></p>
              {hedgingScenarios.map(s => (
                <div key={s.type} className="p-3 rounded-lg border border-slate-700 bg-slate-800/50">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-white font-medium">{s.type}</span>
