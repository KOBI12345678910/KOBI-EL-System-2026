...
router.get("/notifications/dashboard-stats", async (req, res) => {
  ...
  const [totalUnread] = await db.select({ count: sql<number>`count(*)::int` }).from(notificationsTable)
    .where(and(baseWhere, eq(notificationsTable.isRead, false)));
  const [totalCritical] = await db.select({ count: sql<number>`count(*)::int` }).from(notificationsTable)
  ...
  const moduleHealthStats = await Promise.all(
    moduleHealth.map(async ({ module, type }) => {
      const [res] = await db.select({ count: sql<number>`count(*)::int` }).from(notificationsTable)
