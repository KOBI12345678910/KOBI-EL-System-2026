...
export default function IntegrationBuilderPage() {
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg sm:text-2xl font-bold text-white">{selectedConnection.name}</h1>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={async () => { if (globalConfirm("למחוק endpoint זה?")) deleteEndpointMutation.mutate(ep.id); }}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={async () => { if (globalConfirm("למחוק webhook זה?")) deleteWebhookMutation.mutate(wh.id); }}
                        className="text-destructive hover:text-destructive"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                      </Button>
          ...
          <Card>
            <div className="p-5 border-b border-border/50 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-white">יומן סנכרון</h3>
              </div>
                ...
                <div key={log.id} className="p-4 hover:bg-muted/5 transition-colors">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className={`text-sm font-medium ${log.status === "completed" ? "text-emerald-400" : log.status === "failed" ? "text-red-400" : "text-white"}`}>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-3xl font-bold text-white">בונה אינטגרציות</h1>
            ...
            <div key={conn.id} onClick={() => setSelectedConnection(conn)} className="cursor-pointer"><Card className="flex flex-col hover:border-primary/30 transition-colors">
              <div className="p-5 flex items-start justify-between">
                ...
                <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                  <button onClick={() => openEdit(conn)} className="p-2 text-muted-foreground hover:bg-card/10 hover:text-white rounded-lg transition-colors">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={async () => {
                    if (globalConfirm("למחוק חיבור זה?")) deleteMutation.mutate(conn.id);
