...
// Canonical Node version: onyx-ai/src/modules/hr-autonomy-engine.ts
...
// ═══════════════════════════════════════════════════════════════════════════
// BROWSER SINGLETON + LOCALSTORAGE PERSISTENCE
// ═══════════════════════════════════════════════════════════════════════════

const HR_STORAGE_KEY = 'tk_hr_snapshot_v1';

export const HR = new HRManager();

/** demo data seed — idempotent */
export function seedHRDemoData(): void {
  if (HR.employees.getAll().length > 0) return;

  // משרה ראשונה
  const job1 = HR.recruitment.createOpening({
    title: 'רתך מוסמך',
    department: 'ייצור',
    description: 'רתך/ת מוסמך/ת לעבודה במפעל מתכת — שרטוטים, הרכבות, תיקונים.',
    requirements: ['תעודת רתכות בתוקף', '3+ שנות ניסיון', 'נכונות לשעות נוספות'],
    salaryRange: { min: 10000, max: 14000 },
    employmentType: 'full_time',
    urgent: true,
    createdBy: 'קורין',
  });
  HR.recruitment.addCandidate(job1.id, { name: 'יוסי כהן', phone: '+972501234567', source: 'פייסבוק', notes: 'מועמד מנוסה' });
  HR.recruitment.addCandidate(job1.id, { name: 'אחמד חסן', phone: '+972509876543', source: 'המלצה' });

  const job2 = HR.recruitment.createOpening({
    title: 'מתקין גגות',
    department: 'התקנות',
    description: 'התקנת גגות פח ומערכות אוורור באתרים.',
    requirements: ['רישיון עבודה בגובה', 'ניסיון בעבודות מתכת'],
    salaryRange: { min: 9000, max: 12500 },
    employmentType: 'full_time',
    createdBy: 'קורין',
  });
  HR.recruitment.addCandidate(job2.id, { name: 'דויד מזרחי', phone: '+972541112222', source: 'מודעה' });

  // עובד מדגם
  const emp1 = HR.employees.add({
    firstName: 'דימה', lastName: 'קוזלוב',
    idNumber: '311222333', phone: '+972501112233',
    email: 'dima@technokoluzi.com',
    department: 'ייצור', position: 'מנהל ייצור',
    manager: 'kobi', employmentType: 'full_time',
    startDate: '2020-01-15', baseSalary: 22000,
    skills: ['ריתוך', 'ניהול צוות', 'CAD'],
    bankDetails: { bankName: 'הפועלים', branch: '540', accountNumber: '123456' },
    emergencyContact: { name: 'אנה קוזלוב', phone: '+972521234567', relation: 'אשה' },
  });
  HR.confirmEmployee(emp1.id, 'kobi');

  const emp2 = HR.employees.add({
    firstName: 'איליה', lastName: 'רוזנברג',
    idNumber: '322333444', phone: '+972502223344',
    department: 'ייצור', position: 'רתך ראשי',
    manager: emp1.id, employmentType: 'full_time',
    startDate: '2023-06-01', baseSalary: 13500,
    skills: ['ריתוך TIG', 'ריתוך MIG', 'קריאת שרטוטים'],
  });
  HR.confirmEmployee(emp2.id, emp1.id);

  const emp3 = HR.employees.add({
    firstName: 'קורין', lastName: 'לוי',
    idNumber: '333444555', phone: '+972503334455',
    email: 'corin@technokoluzi.com',
    department: 'הנהלה', position: 'מנהלת משרד / HR',
    manager: 'kobi', employmentType: 'full_time',
    startDate: '2022-03-01', baseSalary: 15000,
    skills: ['ניהול משרד', 'גיוס', 'חשבונות'],
    bankDetails: { bankName: 'לאומי', branch: '912', accountNumber: '987654' },
  });
  HR.confirmEmployee(emp3.id, 'kobi');

  // חופשה ממתינה
  HR.leaves.requestLeave(emp2, {
    type: 'annual', startDate: '2026-05-10', endDate: '2026-05-14', days: 5,
    reason: 'חופשה משפחתית',
  });

  // הערכת ביצוע לדימה
  HR.performance.createReview(emp1, {
    period: 'Q1 2026', reviewerId: 'kobi', reviewerName: 'קובי',
    scores: { quality: 5, productivity: 5, teamwork: 4, reliability: 5, initiative: 4, safety: 5, overall: 0 },
    strengths: ['מנהיגות', 'מקצועיות', 'אמינות גבוהה'],
    improvements: ['שיתוף מידע על פעולות יומיות'],
    goals: [{ goal: 'הטמעת מערכת ERP חדשה', deadline: '2026-09-30' }],
    comments: 'עובד מצוין, עמוד תווך של המפעל',
  });
}

/** wire HR to data flow engine for incoming signals (employee events) */
export function wireHRToDataFlow(): void {
  // placeholder — future: listen to DataFlow for hr.* events
}

/** helper: get snapshot for UI without exposing maps */
export function getHRSnapshot() {
  return {
    dashboard: HR.getDashboard(),
    employees: HR.employees.getAll(),
    activeEmployees: HR.employees.getActive(),
    openings: HR.recruitment.getAllOpenings(),
    openOpenings: HR.recruitment.getOpenOpenings(),
    complianceIssues: HR.compliance.audit(HR.employees.getActive()),
    overdueOnboardings: HR.onOffboarding.getOverdueOnboardings(),
  };
}
