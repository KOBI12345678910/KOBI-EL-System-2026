...
export const notificationService = {
  ...
  async send(options: {
    ...
    if (recipientType === 'employee' && employeeId) {
      const { rows } = await query(`SELECT name, phone FROM employees WHERE id = $1`, [employeeId]);
      recipientName = rows[0]?.name || '';
      recipientPhone = rows[0]?.phone || '';
    } else if (recipientType === 'client' && clientId) {
      const { rows } = await query(`SELECT name, phone FROM clients WHERE id = $1`, [clientId]);
      recipientName = rows[0]?.name || '';
      recipientPhone = rows[0]?.phone || '';
    }
