...
export const aiCoordinator = {

  async scheduleMeasurement(project: any) {
    // מצא מודד פנוי
    const { rows: surveyors } = await query(`
      SELECT e.id, e.name,
  ...
  async scheduleInstallation(project: any) {
    // מצא מתקין פנוי
    const { rows: installers } = await query(`
      SELECT e.id, e.name,
    ...
    // דלג על שישי שבת
    if (installDate.getDay() === 5) installDate.setDate(installDate.getDate() + 2);
    if (installDate.getDay() === 6) installDate.setDate(installDate.getDate() + 1);
