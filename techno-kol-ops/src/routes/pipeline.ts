...
router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    const projects = await pipelineService.getAllProjects(req.query);
    res.json(projects);
  } catch (err: any) {
...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  try {
    const project = await pipelineService.getProject(req.params.id);
    res.json(project);
  } catch (err: any) {
...
router.post('/', async (req: AuthRequest, res: Response) => {
  try {
    const {
      client_id, order_id, title, description, address,
    ...
    await pipelineService.triggerNextStage(
...
router.put('/:id/advance', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await pipelineService.advanceStage(
      id, stage,
      req.user?.id || '',
      req.user?.role || 'employee',
      { notes, photos, signature, lat, lng }
    );
...
router.get('/approvals/mine', async (req: AuthRequest, res: Response) => {
  try {
    const { rows } = await query(`
...
router.get('/client/:token', async (req: any, res: Response) => {
  try {
    ...
    if (!rows[0]) return res.status(404).json({ error: 'Link expired or invalid' });
    res.json(rows[0]);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/client/:token/sign', async (req: any, res: Response) => {
  try {
    const { token } = req.params;
    ...
    if (!rows[0]) return res.status(400).json({ error: 'Already used or expired' });

    const { project_id } = rows[0];

    await query(`UPDATE projects SET contract_signed_at = NOW() WHERE id = $1`, [project_id]);
    ...
    await pipelineService.advanceStage(project_id, 'contract_sent', 'client', 'client', { notes: 'חתימה דיגיטלית' });
...
// POST לקוח מילא סקר
router.post('/client/:token/survey', async (req: any, res: Response) => {
  try {
    const { token } = req.params;
    ...
    if (!rows[0]) return res.status(400).json({ error: 'Invalid' });
    ...
    await query(`UPDATE projects SET survey_score = $2, survey_feedback = $3 WHERE id = $1`,
      [rows[0].project_id, avg, free_text]);
