...
router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (material) { sql += ` AND wo.material_primary = $${i++}`; params.push(material); }
    if (client_id) { sql += ` AND wo.client_id = $${i++}`; params.push(client_id); }
    if (from) { sql += ` AND wo.delivery_date >= $${i++}`; params.push(from); }
    if (to) { sql += ` AND wo.delivery_date <= $${i++}`; params.push(to); }
...
router.post('/', async (req: AuthRequest, res: Response) => {
  try {
    const {
      ...
      quantity, unit, price, cost_estimate, advance_paid, delivery_date,
      priority, notes
    } = req.body;
...
router.put('/:id/progress', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (progress === 100) newStatus = 'ready';
...
router.post('/:id/employees', async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
