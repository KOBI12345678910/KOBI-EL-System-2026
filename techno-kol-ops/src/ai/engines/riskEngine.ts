...
export const riskEngine = {
  ...
  async analyzeCompanyRisks() {
    ...
    if (totalRiskScore > 70) {
      broadcastToAll('HIGH_RISK_ALERT', { score: totalRiskScore });
    }
    ...
    return {
      ...
      hedging_recommendations: this.generateHedgingStrategies(
        concentrationRisk, cashRisk, operationalRisk
      )
    };
  },
  ...
  async clientConcentrationRisk() {
    ...
    const top3Pct = rows.slice(0, 3).reduce((s: number, r: any) => s + parseFloat(r.pct || '0'), 0);
  ...
  async materialMarketRisk() {
    ...
    const maxIncrease = Math.max(...rows.map((r: any) => {
  ...
  generateHedgingStrategies(concentration: any, cash: any, operational: any): string[] {
