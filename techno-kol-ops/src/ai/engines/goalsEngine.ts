...
export const goalsEngine = {

  async setCompanyOKRs(quarter: string, year: number) {
  ...
  async trackProgress(quarter: string, year: number) {
    ...
    // עדכן התקדמות
    okrs.objectives.forEach((obj: any) => {
      ...
      obj.progress = Math.round(
        obj.key_results.reduce((s: number, kr: any) => s + kr.progress, 0) / obj.key_results.length
      );
    });

    broadcastToAll('OKR_UPDATE', { quarter, year, okrs });
    return okrs;
  },

  async calculateActuals(): Promise<Record<string, number>> {
