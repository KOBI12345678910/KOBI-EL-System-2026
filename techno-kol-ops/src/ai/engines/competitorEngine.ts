...
export const competitorEngine = {
  ...
  // ניתוח תחרות מלא
  async analyzeCompetition(category: string, material: string) {
  ...
  generateRecommendations(priceGap: number, wl: any): string[] {
    const recs: string[] = [];
    if (priceGap > 15) recs.push('המחיר שלנו גבוה ב-15%+ מהשוק — שקול הורדה בפרוייקטים תחרותיים');
    if (parseInt(wl.lost_on_price || '0') > 3) recs.push(`${wl.lost_on_price} הפסדות בגלל מחיר — בנה חבילות ערך`);
    if (priceGap < 0) recs.push('אנחנו זולים מהשוק — יש מקום להעלות מחיר');
  ...
  async getPriceHistory(category: string, material: string, months: number = 12) {
