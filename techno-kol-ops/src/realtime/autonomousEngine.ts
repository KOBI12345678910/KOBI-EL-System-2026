...
async function broadcastLiveMap() {
  ...
  broadcastToAll('LIVE_MAP_UPDATE', {
    employees: rows,
...
async function checkOrderDelays() {
  ...
  for (const order of rows) {
    const daysLate = Math.floor(
      (Date.now() - new Date(order.delivery_date).getTime()) / (1000 * 60 * 60 * 24)
    );

    broadcastToAll('ORDER_DELAYED', {
      id: order.id,
...
async function analyzeProductivity() {
  ...
  broadcastToAll('PRODUCTIVITY_UPDATE', {
    employees: rows,
    timestamp: new Date().toISOString()
  });
}
...
async function checkOverduePayments() {
  ...
  for (const order of rows) {
    broadcastToAll('PAYMENT_OVERDUE', {
      order_id: order.id,
      client: order.client_name,
...
async function sendDailyBriefing() {
  const [ordersRes, tasksRes, materialsRes, attendanceRes] = await Promise.all([
    query(`SELECT COUNT(*) as count FROM work_orders WHERE status IN ('pending','production','finishing')`),
