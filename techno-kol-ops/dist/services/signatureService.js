...
exports.signatureService = {
    ...
    async sendDocument(documentId) {
        const [docRes, recipientsRes] = await Promise.all([
            (0, connection_1.query)(`SELECT d.*, p.title as project_title FROM documents d LEFT JOIN projects p ON d.project_id=p.id WHERE d.id=$1`, [documentId]),
            (0, connection_1.query)(`SELECT * FROM document_recipients WHERE document_id=$1 ORDER BY signing_order ASC`, [documentId])
        ]);
        ...
        // עדכן סטטוס מסמך
        await (0, connection_1.query)(`UPDATE documents SET status='sent', updated_at=NOW() WHERE id=$1`, [documentId]);
        await this.auditLog(null, documentId, 'sent', 'system', null, `נשלח ל-${firstGroup.length} נמענים`);
        (0, websocket_1.broadcastToAll)('DOCUMENT_SENT', {
    ...
    // ── שלח לנמען בודד
    async sendToRecipient(doc, recipient) {
        // צור טוקן
    ...
    `, [doc.id, recipient.id, token, expiresAt]);
        // עדכן סטטוס נמען
        await (0, connection_1.query)(`UPDATE document_recipients SET status='sent', sent_at=NOW() WHERE id=$1`, [recipient.id]);
    ...
    buildSigningMessage(doc, recipient, signUrl) {
    ...
    async getDocumentByToken(token) {
        const { rows: tokenRows } = await (0, connection_1.query)(`
    ...
    `, [token]);
        ...
        if (tokenData.recipient_status === 'sent') {
            await (0, connection_1.query)(`UPDATE document_recipients SET status='viewed', viewed_at=NOW() WHERE id=$1`, [tokenData.recipient_id]);
            await (0, connection_1.query)(`UPDATE documents SET status='viewed', updated_at=NOW() WHERE id=$1 AND status='sent'`, [tokenData.document_id]);
        ...
        // קבל חתימות קיימות
        const { rows: sigsRes } = await (0, connection_1.query)(`
    ...
    `, [tokenData.document_id]);
        // קבל כל הנמענים
        const { rows: allRecipients } = await (0, connection_1.query)(`
    ...
    `, [params.token]);
        ...
        try {
            await client.query('BEGIN');
            // חשב hash לאימות
            const { rows: docRows } = await client.query(`SELECT content FROM documents WHERE id=$1`, [tokenData.document_id]);
      ...
      `, [tokenData.recipient_id]);
            ...
            await this.auditLog(client, tokenData.document_id, 'signed', 'recipient', tokenData.recipient_id, `${params.signedName} חתם`);
            await client.query('COMMIT');
            return {
                success: true,
    ...
    async checkAllSigned(client, documentId) {
    ...
    async triggerNextSigner(client, documentId, currentOrder) {
        const { rows } = await client.query(`
    ...
    async onAllSigned(documentId) {
        // שלח עותק חתום לכולם
        const { rows: recipients } = await (0, connection_1.query)(`
      SELECT dr.*, s.signed_at, s.signed_name
      FROM document_recipients dr
      LEFT JOIN signatures s ON dr.id=s.recipient_id
      WHERE dr.document_id=$1
    `, [documentId]);
        const { rows: docRows } = await (0, connection_1.query)(`
    ...
    `, [documentId]);
        const doc = docRows[0];
        // התרע למערכת
        (0, websocket_1.broadcastToAll)('DOCUMENT_FULLY_SIGNED', {
            document_id: documentId,
            title: doc.title,
        ...
        if (doc.project_id && doc.type === 'contract_client') {
            const { pipelineService } = await Promise.resolve().then(() => __importStar(require('./pipeline')));
            await pipelineService.advanceStage(doc.project_id, 'contract_sent', 'system', 'system', { notes: 'חוזה נחתם דיגיטלית על ידי כל הצדדים' }).catch(() => { });
        }
    ...
    // דחיית מסמך
    // ══════════════════════════════════════
    async rejectDocument(token, reason) {
        const { rows: tokenRows } = await (0, connection_1.query)(`
    ...
    `, [t.recipient_id, reason]);
        await (0, connection_1.query)(`UPDATE document_tokens SET used_at=NOW() WHERE token=$1`, [token]);
        await (0, connection_1.query)(`UPDATE documents SET status='rejected', updated_at=NOW() WHERE id=$1`, [t.document_id]);
        await this.auditLog(null, t.document_id, 'rejected', 'recipient', t.recipient_id, `${t.recipient_name} דחה: ${reason}`);
        (0, websocket_1.broadcastToAll)('DOCUMENT_REJECTED', {
    ...
    async sendReminder(documentId) {
        const { rows: pending } = await (0, connection_1.query)(`
    ...
    `, [documentId]);
        ...
        `, [documentId, recipient.id, newToken]);
            ...
            await this.auditLog(null, documentId, 'reminded', 'system', null, `תזכורת נשלחה ל-${recipient.recipient_name}`);
        }
    ...
    // יצירת PDF מסמך חתום
    // ══════════════════════════════════════
    async generateSignedDocument(documentId) {
        const [docRes, sigsRes, recipientsRes] = await Promise.all([
            (0, connection_1.query)(`SELECT * FROM documents WHERE id=$1`, [documentId]),
      ...
      `, [documentId]),
            (0, connection_1.query)(`SELECT * FROM document_recipients WHERE document_id=$1 ORDER BY signing_order`, [documentId])
        ]);
        const doc = docRes.rows[0];
  ...
  <div class="validation-footer">
    ...
    async verifyDocument(documentId) {
      ...
      `, [documentId])
        ...
        if (!docRes.rows[0])
            return { valid: false, error: 'מסמך לא נמצא' };
        const doc = docRes.rows[0];
    ...
    // ── Audit Log
    async auditLog(client, documentId, action, actorType, actorId, description) {
        const q = client || { query: (sql, params) => (0, connection_1.query)(sql, params) };
        await q.query(`
