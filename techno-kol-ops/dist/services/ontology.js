...
exports.getFactorySnapshot = getFactorySnapshot;
const connection_1 = require("../db/connection");
async function getFactorySnapshot() {
    const [ordersResult, attendanceResult, materialsResult, alertsResult, revenueResult, eventsResult] = await Promise.all([
    ...
    const utilizationPct = Math.round((activeOrders.filter(o => o.status === 'production').length / Math.max(activeOrders.length, 1)) * 100);
    return {
        activeOrders,
        attendance: {
