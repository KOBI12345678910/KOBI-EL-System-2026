...
// ════════════════════════════════════
exports.whatsappBot = {
    async handleIncoming(from, body) {
        const phone = from.replace('whatsapp:', '').replace('+', '');
        const msg = body.trim().toLowerCase();
        // זהה מי שולח
        const { rows: empRows } = await (0, connection_1.query)(`SELECT id, name, role FROM employees WHERE REPLACE(phone, '-', '') = $1 AND is_active = true`, [phone.replace(/[-+]/g, '')]);
        const { rows: clientRows } = await (0, connection_1.query)(`SELECT id, name FROM clients WHERE REPLACE(phone, '-', '') = $1 AND is_active = true`, [phone.replace(/[-+]/g, '')]);
        // ── עובד שולח
        if (empRows.length > 0) {
            const emp = empRows[0];
            if (msg.includes('סיימתי') || msg.includes('הסתיים') || msg.includes('done')) {
                // מצא משימה פעילה
                const { rows: tasks } = await (0, connection_1.query)(`
          SELECT * FROM tasks
          WHERE employee_id = $1 AND status = 'in_progress'
          ORDER BY scheduled_date DESC LIMIT 1
        `, [emp.id]);
                if (tasks.length > 0) {
                    await (0, connection_1.query)(`UPDATE tasks SET status = 'done', completed_at = NOW() WHERE id = $1`, [tasks[0].id]);
            ...
            if (msg.includes('אישור') || msg.includes('אשרתי') || msg.includes('אשר')) {
                const { rows: approvals } = await (0, connection_1.query)(`
        ...
        `, [emp.id]);
                if (approvals.length > 0) {
                    ...
                    await pipeline_1.pipelineService.advanceStage(appr.project_id, appr.stage, emp.id, emp.role, {
            ...
            if (msg.includes('משימות') || msg.includes('היום') || msg === 'מה יש לי') {
                const { rows: todayTasks } = await (0, connection_1.query)(`
          SELECT title, address, status, scheduled_time
          FROM tasks WHERE employee_id = $1 AND scheduled_date = CURRENT_DATE
          ORDER BY scheduled_time
        `, [emp.id]);
                if (todayTasks.length === 0)
                    return `${emp.name}, אין משימות מתוכננות להיום 👍`;
                const list = todayTasks.map((t, i) => `${i + 1}. ${t.title}\n   📍 ${t.address}\n   ⏰ ${t.scheduled_time || 'ללא שעה'} | ${{ pending: '⏳ ממתין', on_way: '🚗 בדרך', in_progress: '🔧 בביצוע', done: '✅ הסתיים' }[t.status] || t.status}`).join('\n\n');
        ...
        if (clientRows.length > 0) {
            ...
            if (msg.includes('סטטוס') || msg.includes('הפרוייקט') || msg.includes('איפה')) {
                const { rows: projects } = await (0, connection_1.query)(`
        ...
        `, [client.id]);
                if (projects.length === 0)
                    return `שלום ${client.name}! אין פרוייקטים פעילים כרגע.`;
                const p = projects[0];
                const STAGE_HE = {
