...
router.get('/:id', async (req, res) => {
    try {
        ...
        const [empRes, attendanceRes, ordersRes] = await Promise.all([
            (0, connection_1.query)(`SELECT * FROM employees WHERE id = $1`, [id]),
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { rows } = await (0, connection_1.query)(`UPDATE employees SET ${setClause} WHERE id = $1 RETURNING *`, [id, ...values]);
        res.json(rows[0]);
    }
    catch (err) {
