...
exports.ProcurementBridgeClient = void 0;
exports.getDefaultProcurementClient = getDefaultProcurementClient;
// ── Constants ─────────────────────────────────────────────────
...
// ── Logger fallback ───────────────────────────────────────────
const log = {
    info: (...a) => console.log('[ops→procurement]', ...a),
    warn: (...a) => console.warn('[ops→procurement]', ...a),
    error: (...a) => console.error('[ops→procurement]', ...a),
    debug: (..._a) => { },
};
// ── Client ────────────────────────────────────────────────────
class ProcurementBridgeClient {
    constructor(opts = {}) {
    ...
    async getPurchaseOrders(filters) {
        ...
        const data = await this.request('GET', `/api/purchase-orders${qs ? '?' + qs : ''}`);
    ...
    async getSuppliers(filters) {
        ...
        if (filters?.limit)
            params.set('limit', String(filters.limit));
        const qs = params.toString();
        return this.request('GET', `/api/suppliers${qs ? '?' + qs : ''}`);
    }
    /** Fetch a single supplier by ID */
    async getSupplier(id) {
        return this.request('GET', `/api/suppliers/${encodeURIComponent(id)}`);
    }
...
exports.ProcurementBridgeClient = ProcurementBridgeClient;
// ── Singleton ─────────────────────────────────────────────────
let _defaultClient = null;
function getDefaultProcurementClient() {
    if (!_defaultClient) {
