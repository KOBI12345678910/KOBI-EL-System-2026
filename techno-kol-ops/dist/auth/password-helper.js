...
try {
    ...
    try {
        ...
        console.warn('[password-helper] WARNING: neither bcrypt nor bcryptjs is installed — ' +
...
function scryptHash(plain) {
    return new Promise((resolve, reject) => {
        ...
        crypto.scrypt(plain, salt, SCRYPT_KEYLEN, { N: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P }, (err, derived) => {
            ...
            resolve(`${SCRYPT_PREFIX}N=${SCRYPT_N},r=${SCRYPT_R},p=${SCRYPT_P}$${saltB64}$${hashB64}`);
        });
    });
}
function scryptVerify(plain, stored) {
    return new Promise((resolve) => {
        ...
        const paramMap = Object.fromEntries(params.split(',').map((kv) => kv.split('=')));
...
async function verifyPassword(plain, hash) {
    ...
    if (usingFallback) {
        ...
        // eslint-disable-next-line no-console
        console.warn('[password-helper] stored hash is bcrypt but bcryptjs is not installed — cannot verify');
        return false;
    }
    try {
