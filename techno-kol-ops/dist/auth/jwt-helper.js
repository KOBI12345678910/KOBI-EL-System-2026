...
try {
    ...
    console.warn('[jwt-helper] WARNING: jsonwebtoken not installed — falling back to ' +
...
function getValidatedSecret() {
    ...
    if (!secret || typeof secret !== 'string') {
        throw new Error('[jwt-helper] JWT_SECRET is missing. Refusing to sign/verify tokens. ' +
    ...
    if (KNOWN_WEAK_SECRETS.has(secret)) {
        throw new Error('[jwt-helper] JWT_SECRET matches a known weak / placeholder value. ' +
            'Rotate it immediately. Generate with: openssl rand -hex 32');
    }
    if (secret.length < MIN_SECRET_LENGTH) {
        throw new Error(`[jwt-helper] JWT_SECRET is too short (${secret.length} chars). ` +
