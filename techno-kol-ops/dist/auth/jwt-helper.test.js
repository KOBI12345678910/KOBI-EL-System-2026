...
test('sign + verify round-trip preserves claims', () => {
    ...
    const token = signToken({ id: 'u-123', username: 'kobi', role: 'admin' }, { expiresIn: '1h' });
        ...
        setTimeout(() => {
            try {
                ...
                try {
                    ...
                    threw = true;
                    assert.ok(/expired/i.test(err.message) || err.name === 'TokenExpiredError', `expected expiry error, got: ${err.name}: ${err.message}`);
                }
...
test('tampered token is rejected', () => {
    process.env.JWT_SECRET = STRONG_SECRET;
    const { signToken, verifyToken } = freshRequire();
    const token = signToken({ id: 'u-2', role: 'viewer' }, { expiresIn: '1h' });
    ...
    try {
        ...
        threw = true;
        assert.ok(/invalid|signature|malformed/i.test(err.message), `expected signature error, got: ${err.message}`);
    }
...
test('weak / placeholder secret throws on init', () => {
    ...
    assert.throws(() => helper.assertSecretOnStartup(), /weak|placeholder/i, 'assertSecretOnStartup should reject the committed placeholder');
});
// --------------------------------------------------------------------------
...
test('too-short secret throws on init', () => {
    ...
    const helper = freshRequire();
    assert.throws(() => helper.assertSecretOnStartup(), /too short|minimum/i, 'assertSecretOnStartup should reject a sub-32-char secret');
});
// --------------------------------------------------------------------------
...
test('missing secret throws on init', () => {
    ...
    const helper = freshRequire();
    assert.throws(() => helper.assertSecretOnStartup(), /missing/i, 'assertSecretOnStartup should reject a missing JWT_SECRET');
});
// --------------------------------------------------------------------------
...
test('missing payload throws a validation error', () => {
    ...
    assert.throws(() => signToken(undefined), /payload must be a non-null object/);
