...
exports.intelligenceEngine = {
    async getRealtimeKPIs() {
        const [orders, revenue, employees, materials] = await Promise.all([
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM work_orders WHERE status IN ('pending','production','finishing','ready')`),
            (0, connection_1.query)(`SELECT COALESCE(SUM(amount),0) as total FROM financial_transactions WHERE type IN ('income','advance') AND is_paid = true AND date >= date_trunc('month', CURRENT_DATE)`),
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM attendance WHERE date = CURRENT_DATE AND location IN ('factory','field')`),
    ...
    async generateQuote(data) {
        ...
        const { rows } = await (0, connection_1.query)(`SELECT AVG(cost_per_unit) as avg_cost FROM material_items WHERE category = $1 AND is_active = true`, [material]);
    ...
    async detectAnomalies() {
        const anomalies = [];
        const { rows: delayed } = await (0, connection_1.query)(`
    ...
    `);
        ...
        const { rows: critical } = await (0, connection_1.query)(`
    ...
    async forecastRevenue() {
      ...
      ORDER BY month ASC
    `);
        const avg = rows.length > 0 ? rows.reduce((s, r) => s + parseFloat(r.revenue || '0'), 0) / rows.length : 0;
    ...
    async cashFlowForecast(days = 30) {
        const { rows: inflow } = await (0, connection_1.query)(`
    ...
    `);
        const avgDaily = inflow.length > 0 ? inflow.reduce((s, r) => s + parseFloat(r.amount), 0) / inflow.length : 0;
