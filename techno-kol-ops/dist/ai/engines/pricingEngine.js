...
exports.projectPricingEngine = void 0;
...
exports.projectPricingEngine = {
    ...
    async calculateOptimalPrice(params) {
      ...
      `, [params.category, params.material]),
            // היסטוריית לקוח
            params.clientId ? (0, connection_1.query)(`
      ...
      `, [params.category])
        ...
        const avgPPU = hist.length > 0
            ? hist.reduce((s, h) => s + parseFloat(h.ppu || 0), 0) / hist.length
        ...
        if ([2, 3, 4].includes(month))
            multiplier *= 1.08; // אביב — עונת שיא
        if ([6, 7].includes(month))
            multiplier *= 0.95; // קיץ — ירידה
        // דחיפות
        if (params.urgency === 'urgent')
            multiplier *= 1.18;
        if (params.urgency === 'flexible')
            multiplier *= 0.95;
        // מורכבות
        ...
        if (params.competitorQuote) {
            ...
            if (competitorPPU < avgPPU)
                multiplier = Math.min(multiplier, 0.97);
        }
        // התקנה וצביעה
        let additions = 0;
        if (params.installationRequired)
            additions += params.quantity * 45;
        if (params.paintRequired)
            additions += params.quantity * 28;
    ...
    getMarketBaseline(category, material) {
        const baselines = {
    ...
    async trackPriceChange(category, material, newPrice, source) {
        await (0, connection_1.query)(`
      INSERT INTO market_prices (category, material, market_price, source, recorded_at)
      VALUES ($1,$2,$3,$4,NOW())
    `, [category, material, newPrice, source]);
        (0, websocket_1.broadcastToAll)('PRICE_UPDATE', { category, material, price: newPrice, source });
    }
};
