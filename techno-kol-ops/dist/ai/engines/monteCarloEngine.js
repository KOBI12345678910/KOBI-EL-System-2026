...
exports.monteCarloEngine = {
    ...
    simulateRevenue(params) {
        ...
        return {
            ...
            probability_above_target: (target) => Math.round(results.filter(r => r >= target).length / iterations * 100),
    ...
    simulateProject(params) {
        ...
        return {
            ...
            probability_of_target_margin: (targetMargin) => {
                const target = params.quoted_price * targetMargin;
                return Math.round(profits.filter(p => p >= target).length / iterations * 100);
            },
            expected_margin_pct: Math.round((profits.reduce((a, b) => a + b, 0) / iterations) / params.quoted_price * 100),
            delay_stats: {
