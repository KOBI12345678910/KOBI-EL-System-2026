...
exports.salesAgentEngine = void 0;
const connection_1 = require("../../db/connection");
// ════════════════════════════════════════════
// ENGINE 4: SALES AGENT ENGINE
// מנוע מודל לסוכני מכירות — יעדים + תמריצים
// ════════════════════════════════════════════
exports.salesAgentEngine = {
    // יעדים חודשיים לסוכן
    async setMonthlyTargets(agentId, month) {
    ...
    // מעקב פעילות יומית
    async trackActivity(agentId, date) {
        const { rows } = await (0, connection_1.query)(`
      SELECT
    ...
    `, [agentId, date]);
        ...
        return {
            date,
            activities: activity,
            score: Math.round(parseInt(activity.calls || '0') * 2 +
