...
exports.brainEngine = void 0;
const node_cron_1 = __importDefault(require("node-cron"));
const connection_1 = require("../db/connection");
const websocket_1 = require("../realtime/websocket");
const payrollEngine_1 = require("./engines/payrollEngine");
const goalsEngine_1 = require("./engines/goalsEngine");
const growthEngine_1 = require("./engines/growthEngine");
// ── STATE
let brainState = {
...
╚══════════════════════════════════════════╝`);
    ...
    // ════════════════════════════════════════════
    async perceive() {
        const [orders, projects, employees, materials, finance, alerts, gps, pipeline, leads] = await Promise.all([
            this.perceiveOrders(),
    ...
    async think(perception) {
        ...
        const { rows: churnRisk } = await (0, connection_1.query)(`
    ...
    `);
        ...
        const { rows: nearComplete } = await (0, connection_1.query)(`
    ...
    `);
        ...
        const utilizationPct = Math.round(parseInt(perception.orders?.in_production || '0') / 12 * 100);
        insights.push({
            type: 'CAPACITY',
    ...
    async solveAutomatically(problem) {
        switch (problem.type) {
            case 'SUPPLY_CHAIN':
                // הזמן חומרים אוטומטית
                const { rows: critItems } = await (0, connection_1.query)(`
        ...
        `);
            ...
            case 'FINANCIAL':
                if (problem.id.includes('aged_invoices')) {
                    const { rows: overdueClients } = await (0, connection_1.query)(`
          ...
          `);
            ...
            case 'SALES':
                if (problem.id.includes('untouched_leads')) {
                    const { rows: coldLeads } = await (0, connection_1.query)(`
          ...
          `);
                ...
                return null;
            case 'PIPELINE':
                const { rows: stuckProjects } = await (0, connection_1.query)(`
    ...
    async capitalizeOpportunity(opportunity) {
    ...
    async executeAction(action) {
        ...
        `, [
            ...
            case 'NOTIFY_AGENTS_COLD_LEADS':
                (0, websocket_1.broadcastToAll)('COLD_LEADS_ALERT', {
            ...
            case 'ESCALATE_PROJECT':
                (0, websocket_1.broadcastToAll)('PROJECT_ESCALATED', {
    ...
    // שדר לכל המערכת
    // ════════════════════════════════════════════
    async communicate(perception, thoughts, decisions, actions) {
        const summary = {
            timestamp: new Date().toISOString(),
            system_health: brainState.systemHealth,
            problems_detected: thoughts.problems.length,
            critical_problems: thoughts.problems.filter((p) => p.severity === 'critical').length,
        ...
        // שדר לכל המחוברים
        (0, websocket_1.broadcastToAll)('BRAIN_UPDATE', summary);
        // שמור דוח
        await (0, connection_1.query)(`
    ...
    async syncFlow(flow) {
        switch (`${flow.from}_${flow.to}`) {
            case 'work_orders_financial_transactions':
                ...
                const { rowCount } = await (0, connection_1.query)(`
        ...
        `);
            ...
            case 'material_movements_alerts':
                ...
                const { rowCount: alertCount } = await (0, connection_1.query)(`
          INSERT INTO alerts (type, severity, title, message, entity_type, entity_id)
          SELECT 'material_low',
    ...
    async loadGoals() {
        ...
        try {
            await goalsEngine_1.goalsEngine.setCompanyOKRs(quarter, now.getFullYear());
        }
        catch { }
    ...
    async buildDailyAgenda() {
        const [tasksToday, installationsToday, materialsToOrder, pendingApprovals] = await Promise.all([
            ...
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM projects WHERE installation_date=CURRENT_DATE`),
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM material_items WHERE qty<=min_threshold AND is_active=true`),
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM approvals WHERE status='pending' AND deadline < NOW()+INTERVAL '24 hours'`)
        ]);
        return {
    ...
    async getPriorities() {
        ...
        if (priorities.length === 0)
            priorities.push('✅ כל המערכות תקינות');
        return priorities;
    },
    // ════════════════════════════════════════════
    // SCHEDULER
    ...
    scheduleAllJobs() {
        ...
        node_cron_1.default.schedule('30 6 * * 0-5', async () => {
            await this.sendDailyBriefing();
        });
        // ── כל יום 12:00: דוח אמצע יום
        node_cron_1.default.schedule('0 12 * * 0-5', async () => {
            await this.sendMidDayReport();
        });
        // ── כל יום 17:00: סיכום יומי
        node_cron_1.default.schedule('0 17 * * 0-5', async () => {
    ...
    async generateManagerReport() {
        ...
        (0, websocket_1.broadcastToAll)('MANAGER_REPORT', {
            type: 'PERIODIC',
            timestamp: new Date().toISOString(),
            data: snapshot
        });
    ...
    async sendDailyBriefing() {
        const [revenue, tasks, orders, materials] = await Promise.all([
            (0, connection_1.query)(`SELECT COALESCE(SUM(amount),0) as mtd FROM financial_transactions WHERE type IN ('income','advance') AND is_paid=true AND date>=date_trunc('month',CURRENT_DATE)`),
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM tasks WHERE scheduled_date=CURRENT_DATE`),
            (0, connection_1.query)(`SELECT COUNT(*) as overdue FROM work_orders WHERE delivery_date<CURRENT_DATE AND status NOT IN ('delivered','cancelled')`),
            (0, connection_1.query)(`SELECT COUNT(*) as critical FROM material_items WHERE qty<=min_threshold*0.3 AND is_active=true`)
        ]);
        const briefing = {
        ...
        (0, websocket_1.broadcastToAll)('DAILY_BRIEFING', briefing);
        // WhatsApp לקובי
    ...
    async sendMidDayReport() {
        ...
        (0, websocket_1.broadcastToAll)('MIDDAY_REPORT', {
            type: 'MIDDAY',
            timestamp: new Date().toISOString(),
    ...
    async sendEndOfDayReport() {
        const [tasksCompleted, ordersAdvanced, alertsResolved] = await Promise.all([
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM tasks WHERE scheduled_date=CURRENT_DATE AND status='done'`),
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM pipeline_events WHERE DATE(created_at)=CURRENT_DATE`),
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM alerts WHERE DATE(resolved_at)=CURRENT_DATE`)
        ]);
        (0, websocket_1.broadcastToAll)('EOD_REPORT', {
            type: 'END_OF_DAY',
            date: new Date().toLocaleDateString('he-IL'),
    ...
    async sendWeeklyReport() {
      ...
      `)
        ...
        (0, websocket_1.broadcastToAll)('WEEKLY_REPORT', {
            type: 'WEEKLY',
            ...
            insights: await growthEngine_1.growthEngine.analyzeGrowthOpportunities()
        });
    },
    async runMonthlyClose() {
        ...
        await payrollEngine_1.payrollEngine.calculateMonthlyPayroll(month, year);
        const quarter = `Q${Math.ceil((now.getMonth() + 1) / 3)}`;
        await goalsEngine_1.goalsEngine.setCompanyOKRs(quarter, now.getFullYear());
        (0, websocket_1.broadcastToAll)('MONTHLY_CLOSE', {
            month, year,
    ...
    async syncGoals() {
        ...
        const atRisk = goals.filter((g) => g.progress_pct < (new Date().getDate() / 30 * 100) * 0.8);
        if (atRisk.length > 0) {
            (0, websocket_1.broadcastToAll)('GOALS_AT_RISK', {
                goals: atRisk,
                message: `${atRisk.length} יעדים בסיכון — נדרשת פעולה`
            });
        }
    },
    async handleBrainError(err) {
        await (0, connection_1.query)(`
      INSERT INTO alerts (type, severity, title, message, entity_type)
      VALUES ('brain_error', 'critical', 'שגיאה קריטית במנוע המרכזי', $1, 'system')
    `, [err?.message || 'Unknown error']);
        (0, websocket_1.broadcastToAll)('BRAIN_ERROR', {
            error: err?.message,
            timestamp: new Date().toISOString()
        });
    }
