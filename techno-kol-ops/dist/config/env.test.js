...
test('throws listing ALL missing required vars at once', () => {
    ...
    for (const required of ['PORT', 'ALLOWED_ORIGINS', 'SUPABASE_URL', 'SUPABASE_ANON_KEY']) {
        assert.ok(error.message.includes(required), `expected error message to list missing var ${required}, got:\n${error.message}`);
        assert.ok(Array.isArray(error.missing) && error.missing.includes(required), `expected error.missing array to include ${required}`);
    }
...
test('summary redacts secrets as ****', () => {
    ...
    assert.ok(!rendered.includes('super-secret-anon-key'), 'secret value leaked into summary output');
});
