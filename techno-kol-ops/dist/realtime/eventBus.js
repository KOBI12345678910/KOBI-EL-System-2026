...
function initEventBus() {
    ...
    // ════════════════════════════════════════════
    exports.eventBus.on('workorder:created', async (data) => {
        await logEvent('execution.workorder.created', data);
        console.log(`[DOMAIN-EVENT] execution.workorder.created | ` +
            `id=${data.entity_id} product=${data.product || 'N/A'} actor=${data.actor}`);
    });
    exports.eventBus.on('workorder:assigned', async (data) => {
        await logEvent('execution.workorder.assigned', data);
        console.log(`[DOMAIN-EVENT] execution.workorder.assigned | ` +
            `id=${data.entity_id} employee=${data.employee_id} role=${data.role_on_order || 'N/A'}`);
    });
    exports.eventBus.on('workorder:completed', async (data) => {
        await logEvent('execution.workorder.completed', data);
        (0, websocket_1.broadcastToAll)('WORKORDER_COMPLETED', data);
        console.log(`[DOMAIN-EVENT] execution.workorder.completed | ` +
            `id=${data.entity_id} --> Invoice creation should be triggered`);
    });
    exports.eventBus.on('project:created', async (data) => {
        await logEvent('execution.project.created', data);
        (0, websocket_1.broadcastToAll)('PROJECT_CREATED', data);
        console.log(`[DOMAIN-EVENT] execution.project.created | ` +
