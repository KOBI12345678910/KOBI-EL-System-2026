 ...
 *   │                                                                      │
 *   │  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌────────────┐       │
 *   │  │Chart of    │ │Double-Entry│ │  Cashflow  │ │  Budget    │       │
 *   │  │Accounts    │ │  Ledger    │ │ Forecaster │ │  Engine    │       │
 *   │  └─────┬──────┘ └─────┬──────┘ └─────┬──────┘ └─────┬──────┘       │
 *   │        └───────────────┼──────────────┼──────────────┘              │
 *   │                        ↓              ↓                              │
 *   │  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌────────────┐       │
 *   │  │ Receivables│ │  Payables  │ │Profitabilty│ │    Tax     │       │
 *   │  │ + Auto     │ │ + Auto Pay │ │  Analyzer  │ │  Engine    │       │
 *   │  │  Collect   │ │            │ │            │ │            │       │
 *   │  └─────┬──────┘ └─────┬──────┘ └─────┬──────┘ └─────┬──────┘       │
 *   │        └───────────────┼──────────────┼──────────────┘              │
 *   │                        ↓                                             │
 ...
 *   │  │  • Cash runway          • Expense optimization            │       │
 *   │  │  • Revenue prediction   • Risk scoring                    │       │
 *   │  │  • Auto-reconciliation  • Compliance monitoring           │       │
 *   │  └──────────────────────────────────────────────────────────┘       │
 *   └──────────────────────────────────────────────────────────────────────┘
 */
...
type EntityId = string; // חברה/פרויקט/מחלקה/לקוח/ספק

/** חשבון בעץ חשבונות */
interface Account {
  code: string;              // "1100" — מספר חשבון
  name: string;
  nameEn: string;
  type: AccountType;
  parentCode?: string;
  level: number;             // 1=ראשי, 2=משני, 3=פירוט
  currency: Currency;
  balance: number;
  isActive: boolean;
  isBankAccount: boolean;
  bankDetails?: { bankName: string; branch: string; accountNumber: string };
  /** ממוצע נע לזיהוי חריגות */
...
interface JournalEntry {
  ...
  /** שורות — חייב + זכות חייבים להשתוות */
  lines: JournalLine[];
  description: string;
...
interface JournalLine {
  ...
  description?: string;
  currency: Currency;
  exchangeRate?: number;     // שער חליפין אם לא ILS
}
...
interface Payment {
  ...
  reference?: string;        // מספר צ'ק/העברה
...
interface CashflowForecast {
  ...
  summary: {
    ...
    daysUntilNegative: number;  // runway
...
// ═══════════════════════════════════════════════════════════════════════════
// SECTION 2: DOUBLE-ENTRY LEDGER — הנהלת חשבונות כפולה
// ═══════════════════════════════════════════════════════════════════════════
...
class Ledger {
  ...
  /** רשום פקודת יומן — חובה שחייב = זכות */
  post(params: {
...
// ═══════════════════════════════════════════════════════════════════════════
// SECTION 3: INVOICE MANAGER — חשבוניות + גבייה + תשלום
// ═══════════════════════════════════════════════════════════════════════════
...
class InvoiceManager {
  ...
  issue(invoiceId: string, issuedBy: string): Invoice {
    ...
    if (inv.type === 'receivable') {
      ...
      // חייב: הוצאות + מע"מ תשומות, זכות: ספקים
...
// ═══════════════════════════════════════════════════════════════════════════
// SECTION 4: CASHFLOW FORECASTER — חיזוי תזרים
// ═══════════════════════════════════════════════════════════════════════════
...
class CashflowForecaster {
  ...
  forecast(daysAhead: number = 90): CashflowForecast {
    // יתרת פתיחה — סכום כל חשבונות בנק + קופה
    ...
    for (let d = 0; d < daysAhead; d++) {
      ...
      // Inflows — חשבוניות שצפויות להתקבל
      const inflowDetails: CashflowForecast['daily'][0]['inflowDetails'] = [];
      for (const inv of receivables) {
        if (inv.dueDate === dateStr) {
          // Probability based on client history and days overdue
      ...
      // Recurring expenses (estimated by day of month)
...
// ═══════════════════════════════════════════════════════════════════════════
// SECTION 5: BUDGET ENGINE — תקציב + מעקב חריגות
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// SECTION 6: FINANCIAL INTELLIGENCE — AI אנליטיקס
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// SECTION 7: FINANCIAL REPORTING — דוחות כספיים
// ═══════════════════════════════════════════════════════════════════════════
...
export class FinancialAutonomyEngine {
  ...
  printDashboard(): void {
    const d = this.getDashboard() as any;
    console.log(`
╔══════════════════════════════════════════════════════════════════════════╗
║   💰 ONYX FINANCIAL AUTONOMY ENGINE                                    ║
╠══════════════════════════════════════════════════════════════════════════╣
║
║   📅 החודש (${d.period.monthStart} — ${d.period.today}):
║      הכנסות: ₪${(d.monthly.revenue).toLocaleString()}
║      עלות מכר: ₪${(d.monthly.cogs).toLocaleString()}
║      רווח גולמי: ₪${(d.monthly.grossProfit).toLocaleString()} (${d.monthly.grossMargin}%)
║      הוצאות תפעול: ₪${(d.monthly.opex).toLocaleString()}
║      רווח נקי: ₪${(d.monthly.netProfit).toLocaleString()} (${d.monthly.netMargin}%)
║
║   📊 שנתי:
║      הכנסות: ₪${(d.yearly.revenue).toLocaleString()}
║      רווח נקי: ₪${(d.yearly.netProfit).toLocaleString()} (${d.yearly.netMargin}%)
║
║   🏦 מאזן:
║      נכסים: ₪${(d.balanceSheet.totalAssets).toLocaleString()}
║      התחייבויות: ₪${(d.balanceSheet.totalLiabilities).toLocaleString()}
║      הון: ₪${(d.balanceSheet.equity).toLocaleString()}
║      מאוזן: ${d.balanceSheet.balanced ? '✅' : '❌'}
║
║   💸 תזרים:
║      יתרה נוכחית: ₪${(d.cashflow.currentBalance).toLocaleString()}
║      סיכון: ${d.cashflow.risk === 'critical' ? '🔴' : d.cashflow.risk === 'high' ? '🟠' : d.cashflow.risk === 'medium' ? '🟡' : '🟢'} ${d.cashflow.risk}
║      runway: ${d.cashflow.daysUntilNegative} ימים
║
║   📥 חייבים: ₪${(d.receivables.total).toLocaleString()} (באיחור: ₪${(d.receivables.overdue).toLocaleString()})
║   📤 זכאים: ₪${(d.payables.total).toLocaleString()} (באיחור: ₪${(d.payables.overdue).toLocaleString()})
║
║   🧠 חריגות AI: ${d.anomalies.total} (${d.anomalies.critical} קריטיות)
║
╚══════════════════════════════════════════════════════════════════════════╝`);
  }
}
