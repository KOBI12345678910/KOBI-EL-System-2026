...
const aiCoordinator_1 = require("./aiCoordinator");
...
exports.pipelineService = {
    ...
    async getProject(projectId) {
        ...
        WHERE p.id = $1
      `, [projectId]),
            (0, connection_1.query)(`SELECT pe.*, e.name as employee_name FROM pipeline_events pe LEFT JOIN employees e ON pe.performed_by = e.id WHERE pe.project_id = $1 ORDER BY pe.created_at ASC`, [projectId]),
            (0, connection_1.query)(`SELECT * FROM approvals WHERE project_id = $1 ORDER BY created_at DESC`, [projectId]),
            (0, connection_1.query)(`SELECT * FROM pipeline_notifications WHERE project_id = $1 ORDER BY created_at DESC LIMIT 20`, [projectId])
        ]);
        if (!projectRes.rows[0])
            throw new Error('Project not found');
        const project = projectRes.rows[0];
        const stageConfig = STAGE_CONFIG[project.current_stage];
        const completedStages = eventsRes.rows.map((e) => e.stage);
        const pendingApproval = approvalsRes.rows.find((a) => a.status === 'pending');
        return {
            ...project,
    ...
    // ═══════════════════════════════════════
    async advanceStage(projectId, stage, performedBy, performedByRole, data = {}) {
      ...
      `, [projectId, stage]);
        ...
        `, [projectId, nextStage]);
            ...
            const project = (await (0, connection_1.query)(`SELECT p.*, c.name as client_name, c.phone as client_phone FROM projects p JOIN clients c ON p.client_id = c.id WHERE p.id = $1`, [projectId])).rows[0];
            await this.triggerNextStage(project, nextStage);
            // 5. broadcast
            (0, websocket_1.broadcastToAll)('PROJECT_STAGE_ADVANCED', {
    ...
    // ═══════════════════════════════════════
    async triggerNextStage(project, stage) {
        switch (stage) {
            case 'measurement_scheduled': {
                // AI מתאם מדידה — שולח לסוקר ולקוח
                const measureDate = await aiCoordinator_1.aiCoordinator.scheduleMeasurement(project);
                await (0, connection_1.query)(`UPDATE projects SET measurement_date = $2, surveyor_id = $3 WHERE id = $1`, [project.id, measureDate.datetime, measureDate.surveyorId]);
                await this.createApproval(project.id, 'measurement_scheduled', 'surveyor', measureDate.surveyorId);
                await notifications_1.notificationService.send({
            ...
            case 'contract_sent': {
                ...
                await (0, connection_1.query)(`UPDATE projects SET contract_sent_at = NOW() WHERE id = $1`, [project.id]);
                await notifications_1.notificationService.send({
                    projectId: project.id, channel: 'whatsapp',
                    recipientType: 'client', clientId: project.client_id,
            ...
            case 'materials_ordered': {
                await (0, connection_1.query)(`UPDATE projects SET materials_ordered_at = NOW() WHERE id = $1`, [project.id]);
                await notifications_1.notificationService.send({
            ...
            case 'production_done': {
                await (0, connection_1.query)(`UPDATE projects SET production_end_at = NOW() WHERE id = $1`, [project.id]);
                await notifications_1.notificationService.send({
                    projectId: project.id, channel: 'push',
            ...
            case 'returned_from_paint': {
                await (0, connection_1.query)(`UPDATE projects SET paint_returned_at = NOW() WHERE id = $1`, [project.id]);
                // AI מתאם התקנה
                const installDate = await aiCoordinator_1.aiCoordinator.scheduleInstallation(project);
        ...
        `, [project.id, installDate.date, installDate.time, installDate.installerId]);
            ...
            case 'installation_done': {
                await (0, connection_1.query)(`UPDATE projects SET installation_done_at = NOW() WHERE id = $1`, [project.id]);
                // שלח סקר ללקוח
            ...
            case 'project_closed': {
                await (0, connection_1.query)(`UPDATE projects SET closed_at = NOW() WHERE id = $1`, [project.id]);
                (0, websocket_1.broadcastToAll)('PROJECT_CLOSED', { projectId: project.id, title: project.title });
                break;
            }
        }
    },
    // ── יצור approval
    async createApproval(projectId, stage, fromRole, employeeId) {
    ...
    // ── יצור טוקן ללקוח
    async createClientToken(projectId, clientId, purpose, expiryDays) {
    ...
    // ── יצור לינק תשלום
    async createPaymentLink(project, token) {
    ...
    `;
        ...
        let i = 1;
        if (filters.stage) {
            sql += ` AND p.current_stage = $${i++}`;
            params.push(filters.stage);
        }
        if (filters.contractor_id) {
            sql += ` AND p.contractor_id = $${i++}`;
            params.push(filters.contractor_id);
        }
        if (filters.installer_id) {
            sql += ` AND p.installer_id = $${i++}`;
