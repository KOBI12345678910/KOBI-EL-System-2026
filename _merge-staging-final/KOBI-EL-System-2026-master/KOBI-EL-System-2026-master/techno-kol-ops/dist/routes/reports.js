...
router.get('/order/:id', async (req, res) => {
    try {
      ...
      `, [id]),
            (0, connection_1.query)(`SELECT * FROM order_events WHERE order_id = $1 ORDER BY created_at DESC`, [id]),
            (0, connection_1.query)(`SELECT * FROM financial_transactions WHERE order_id = $1 ORDER BY date DESC`, [id]),
        ]);
        if (!orderRes.rows[0])
