...
router.get('/:id', async (req, res) => {
    try {
        ...
        const [itemRes, movementsRes] = await Promise.all([
            (0, connection_1.query)(`SELECT mi.*, s.name as supplier_name FROM material_items mi LEFT JOIN suppliers s ON mi.supplier_id = s.id WHERE mi.id = $1`, [id]),
            (0, connection_1.query)(`SELECT mm.*, wo.product as order_product FROM material_movements mm LEFT JOIN work_orders wo ON mm.order_id = wo.id WHERE mm.item_id = $1 ORDER BY mm.created_at DESC LIMIT 20`, [id])
        ]);
...
router.post('/:id/receive', async (req, res) => {
    try {
        ...
        await (0, connection_1.query)(`UPDATE material_items SET qty = qty + $2, cost_per_unit = $3 WHERE id = $1`, [id, qty, cost_per_unit]);
    ...
    `, [id, qty, cost_per_unit, supplier_id, notes]);
        const { rows } = await (0, connection_1.query)(`SELECT * FROM material_items WHERE id = $1`, [id]);
        (0, websocket_1.broadcastToAll)('MATERIAL_RECEIVED', rows[0]);
        res.json(rows[0]);
    }
    catch (err) {
...
router.post('/:id/consume', async (req, res) => {
    try {
        ...
        const check = await (0, connection_1.query)(`SELECT qty FROM material_items WHERE id = $1`, [id]);
        if (check.rows[0].qty < qty) {
            return res.status(400).json({ error: 'Insufficient stock' });
        }
        await (0, connection_1.query)(`UPDATE material_items SET qty = qty - $2 WHERE id = $1`, [id, qty]);
        await (0, connection_1.query)(`
    ...
    `, [id, order_id, qty, employee_id, notes]);
        ...
        if (rows[0].qty <= rows[0].min_threshold) {
            (0, websocket_1.broadcastToAll)('MATERIAL_LOW', rows[0]);
        }
        res.json(rows[0]);
    }
    catch (err) {
