...
exports.aipEngine = void 0;
const connection_1 = require("../db/connection");
const ontologyEngine_1 = require("../ontology/ontologyEngine");
...
exports.aipEngine = {
    ...
    async answerRevenue(q) {
        ...
        if (isYTD) {
            period = `date_trunc('year', CURRENT_DATE)`;
            label = 'שנה נוכחית';
        }
        if (isLastMonth) {
    ...
    `);
        ...
        return {
            answer: `סטטוס הזמנות:\n${rows.map((r) => `${r.status}: ${r.count} הזמנות (₪${Math.round(parseFloat(r.value || '0')).toLocaleString('he-IL')})`).join('\n')}\n\nסה"כ: ${total} הזמנות פעילות`,
            data: rows,
            confidence: 95,
    ...
    `;
        if (isCritical)
            sql += ` AND mi.qty <= mi.min_threshold`;
        sql += ` ORDER BY (mi.qty/NULLIF(mi.min_threshold,0)) ASC LIMIT 15`;
        const { rows } = await (0, connection_1.query)(sql);
        const critical = rows.filter((r) => r.status === 'קריטי').length;
        const low = rows.filter((r) => r.status === 'נמוך').length;
        return {
            answer: isCritical
                ? `${rows.length} פריטים מתחת לסף מינימום:\n${rows.slice(0, 5).map((r) => `• ${r.name}: ${r.qty} ${r.unit} (${r.status})`).join('\n')}`
    ...
    `);
        const inFactory = rows.filter((r) => r.location === 'factory').length;
        const inField = rows.filter((r) => r.location === 'field').length;
        const sick = rows.filter((r) => r.location === 'sick').length;
        const absent = rows.filter((r) => ['absent', 'vacation'].includes(r.location)).length;
        if (isLocation) {
            const fieldWorkers = rows.filter((r) => r.location === 'field');
            return {
                answer: `עובדים בשטח עכשיו (${fieldWorkers.length}):\n${fieldWorkers.map((r) => `• ${r.name} — ${r.role}`).join('\n')}`,
                data: fieldWorkers,
                confidence: 95,
    ...
    `);
        return {
            answer: `10 לקוחות מובילים:\n${rows.slice(0, 5).map((r, i) => `${i + 1}. ${r.name}: ₪${Math.round(parseFloat(r.revenue)).toLocaleString('he-IL')} | ${r.orders} הזמנות`).join('\n')}`,
            data: rows,
            confidence: 97,
    ...
    `);
        return {
            answer: `פרוייקטים פעילים (${rows.length}):\n${rows.slice(0, 5).map((r) => `• ${r.project_number} | ${r.title}\n  שלב: ${r.current_stage?.replace(/_/g, ' ')} | יתרה: ₪${Math.round(parseFloat(r.balance_due)).toLocaleString('he-IL')}`).join('\n')}`,
            data: rows,
            confidence: 95,
    ...
    `);
        return {
            answer: `ביצועי עובדים (30 יום):\n${rows.slice(0, 5).map((r) => `${r.name}: ${r.tasks_done} משימות | ${Math.round(parseFloat(r.completion_rate || '0'))}% השלמה`).join('\n')}`,
            data: rows,
            confidence: 90,
    ...
    // ── שאלה כללית
    async answerGeneral(q) {
        const snapshot = await ontologyEngine_1.ontologyEngine.getDigitalTwin();
        return {
