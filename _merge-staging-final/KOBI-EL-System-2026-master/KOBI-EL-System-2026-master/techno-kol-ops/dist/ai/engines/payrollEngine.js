...
// ════════════════════════════════════════════
exports.payrollEngine = {
    // חישוב שכר חודשי מלא
    async calculateMonthlyPayroll(month, year) {
        const { rows: employees } = await (0, connection_1.query)(`
      SELECT e.*,
      ...
      GROUP BY e.id
    `, [parseInt(month), year]);
        const payrollRun = await Promise.all(employees.map(emp => this.calculateEmployeePayroll(emp, month, year)));
    ...
    `, [month, year, payrollRun.length, totalGross, totalNet, totalEmployerCost, JSON.stringify(payrollRun)]);
        (0, websocket_1.broadcastToAll)('PAYROLL_CALCULATED', {
    ...
    // חישוב שכר עובד בודד
    async calculateEmployeePayroll(emp, month, year) {
