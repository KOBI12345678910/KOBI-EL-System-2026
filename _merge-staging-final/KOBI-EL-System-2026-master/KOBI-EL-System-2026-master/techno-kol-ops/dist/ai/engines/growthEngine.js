...
exports.growthEngine = {
    ...
    `);
        ...
        return {
            ...
            peak_month: rows.reduce((max, r) => parseFloat(r.revenue || '0') > parseFloat(max.revenue || '0') ? r : max, rows[0] || {}),
    ...
    async estimateMarketSize() {
        const { rows: ourRevenue } = await (0, connection_1.query)(`
    ...
    identifyGrowthLevers(trend, products) {
        const levers = [];
        const highMarginProducts = products.filter((p) => p.margin_pct > 35);
