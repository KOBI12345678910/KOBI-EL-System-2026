...
exports.materialCostEngine = void 0;
...
exports.materialCostEngine = {
    ...
    `);
        ...
        if (risers.length > 0) {
            (0, websocket_1.broadcastToAll)('MATERIAL_PRICE_SPIKE', {
    ...
    recommendAction(item) {
        ...
        if (days < leadDays)
            return 'ORDER_NOW_URGENT';
        if (days < 14 && trend > 5)
            return 'ORDER_NOW_BEFORE_PRICE_RISE';
        if (trend > 10)
            return 'CONSIDER_BULK_ORDER';
        if (trend < -8)
            return 'WAIT_FOR_LOWER_PRICE';
        if (days > 90)
            return 'REDUCE_ORDER';
        return 'NORMAL';
    },
    calculateRisk(item) {
        ...
        if (trend > 20)
            risk += 35;
        if (parseInt(item.lead_days || '0') > 10)
            risk += 15;
        return Math.min(100, risk);
    },
    // ניתוח ספק vs ספק
    async compareSuppliers(itemName) {
        const { rows } = await (0, connection_1.query)(`
