...
exports.fraudEngine = {
    async runFullScan() {
        const [materialFraud, timeFraud, financialFraud, supplierFraud, gpsAnomalies] = await Promise.all([
        ...
        if (highRisk.length > 0) {
            (0, websocket_1.broadcastToAll)('FRAUD_ALERT', {
    ...
    `);
        ...
        // תנועות גדולות בלילה
        const { rows: nightMovements } = await (0, connection_1.query)(`
    ...
    `);
        ...
        // שעות נוספות חריגות
        const { rows: overtimeRows } = await (0, connection_1.query)(`
    ...
    async detectFinancialFraud() {
        ...
        const { rows: roundAmounts } = await (0, connection_1.query)(`
    ...
    `);
        ...
        // תשלום כפול
        const { rows: duplicates } = await (0, connection_1.query)(`
