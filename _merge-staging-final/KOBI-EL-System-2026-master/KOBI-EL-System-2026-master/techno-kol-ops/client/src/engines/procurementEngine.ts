 ...
 * ╚══════════════════════════════════════════════════════════════════════════╝
 *
 *  Browser port. Canonical Node version lives at
 *  onyx-ai/src/modules/procurement-hyperintelligence.ts
...
// Canonical Node version: onyx-ai/src/modules/procurement-hyperintelligence.ts
// ═══════════════════════════════════════════════════════════════════════════
const crypto = {
...
// ═══════════════════════════════════════════════════════════════════════════
// SINGLETON + DEMO SEED — browser-only helpers for UI consumption
// ═══════════════════════════════════════════════════════════════════════════

export const Procurement = new ProcurementHyperintelligence();

let _seeded = false;

export function seedProcurementDemoData(): void {
  // idempotent: רק פעם אחת בסשן
  if (_seeded) return;
  _seeded = true;

  // ───── ספקים לדוגמה ─────
  const s1 = Procurement.suppliers.registerSupplier({
    name: 'אלקטרו-מרכז בע"מ',
    country: 'IL',
    categories: ['electrical', 'cables', 'lighting'],
    contactEmail: 'sales@electro-merkaz.co.il',
    paymentTermsDays: 60,
    currency: 'ILS',
    creditRating: 'A',
    financialHealth: 88,
  ...
  const s2 = Procurement.suppliers.registerSupplier({
    name: 'Global Supply Co.',
    country: 'DE',
    categories: ['electrical', 'mechanical', 'tools'],
    contactEmail: 'orders@globalsupply.de',
    paymentTermsDays: 90,
    currency: 'EUR',
    creditRating: 'A',
    financialHealth: 92,
  ...
  const s3 = Procurement.suppliers.registerSupplier({
    name: 'חומרי בניין אברהם',
    country: 'IL',
    categories: ['construction', 'cement', 'steel'],
    contactEmail: 'avraham@building-mat.co.il',
    paymentTermsDays: 45,
    currency: 'ILS',
    creditRating: 'B',
    financialHealth: 70,
  ...
  const s4 = Procurement.suppliers.registerSupplier({
    name: 'Yellow Dragon Industries',
    country: 'CN',
    categories: ['electrical', 'tools', 'plastics'],
    contactEmail: 'export@yellowdragon.cn',
    paymentTermsDays: 30,
    currency: 'USD',
    creditRating: 'C',
    financialHealth: 55,
  ...
  // ───── מוצרים לדוגמה ─────
  const p1 = Procurement.demand.registerProduct({
    sku: 'CBL-3X2.5',
    name: 'כבל חשמל 3x2.5 ממ"ר',
    category: 'electrical',
    subcategory: 'cables',
    unit: 'מ\'',
    currentStock: 450,
    ...
    lastUnitPrice: 8.5,
    preferredSupplierIds: [s1.id, s2.id],
  });

  const p2 = Procurement.demand.registerProduct({
    sku: 'BRK-16A',
    name: 'מפסק פחת 16A',
    category: 'electrical',
    ...
    lastUnitPrice: 95,
    preferredSupplierIds: [s1.id, s4.id],
  });

  const p3 = Procurement.demand.registerProduct({
    sku: 'CMT-42.5',
    name: 'מלט 42.5 (שק 50 ק"ג)',
    ...
    lastUnitPrice: 32,
    preferredSupplierIds: [s3.id],
  });

  // חישוב EOQ + reorder point לכל מוצר
  Procurement.demand.recomputeAllEOQ();

  // ───── היסטוריית צריכה דמה ─────
  for (let d = 30; d >= 0; d--) {
    const at = new Date(Date.now() - d * 86400_000).toISOString();
    Procurement.demand.recordConsumption(p1.id, 30 + Math.floor(Math.random() * 10), at);
    Procurement.demand.recordConsumption(p2.id, 2 + Math.floor(Math.random() * 2), at);
    Procurement.demand.recordConsumption(p3.id, 6 + Math.floor(Math.random() * 3), at);
  }

  // ───── היסטוריית מחירים לספקים ─────
  Procurement.suppliers.recordPricePoint(s1.id, { productId: p1.id, unitPrice: 8.2, currency: 'ILS', recordedAt: new Date(Date.now() - 30 * 86400_000).toISOString(), source: 'po' });
  Procurement.suppliers.recordPricePoint(s1.id, { productId: p1.id, unitPrice: 8.5, currency: 'ILS', recordedAt: new Date().toISOString(), source: 'po' });
  Procurement.suppliers.recordPricePoint(s2.id, { productId: p1.id, unitPrice: 8.8, currency: 'ILS', recordedAt: new Date().toISOString(), source: 'quote' });

  // ───── חוזה פעיל ─────
  Procurement.contracts.createContract({
    contractNumber: 'CT-2026-001',
    supplierId: s1.id,
    startDate: new Date(Date.now() - 60 * 86400_000).toISOString(),
    endDate: new Date(Date.now() + 300 * 86400_000).toISOString(),
    currency: 'ILS',
    totalValue: 250000,
    terms: [
      { productId: p1.id, sku: p1.sku, unitPrice: 7.9, currency: 'ILS', minimumAnnualQuantity: 5000, leadTimeDays: 5 },
    ],
    slaOnTimePct: 0.95,
    slaDefectPct: 0.02,
    slaResponseHours: 6,
  ...
  // ───── Auto-reorder rules ─────
  Procurement.reorder.createRule({
    productId: p2.id,
    enabled: true,
    cooldownHours: DEFAULT_AUTO_REORDER_COOLDOWN_HOURS,
    approvalThreshold: DEFAULT_AUTO_REORDER_APPROVAL_THRESHOLD,
  ...
  const po1: PurchaseOrder = {
    ...
    status: 'received',
    lines: [
      { productId: p1.id, sku: p1.sku, name: p1.name, quantity: 500, unitPrice: 8.2, currency: 'ILS', lineTotal: 4100 },
    ],
    currency: 'ILS',
    subtotal: 4100,
    taxAmount: 697,
    ...
    updatedAt: new Date(Date.now() - 18 * 86400_000).toISOString(),
    source: 'manual',
  };
  Procurement.spend.recordPO(po1);
  ...
  const po2: PurchaseOrder = {
    ...
    status: 'received',
    lines: [
      { productId: p3.id, sku: p3.sku, name: p3.name, quantity: 200, unitPrice: 32, currency: 'ILS', lineTotal: 6400 },
    ],
    currency: 'ILS',
    subtotal: 6400,
    taxAmount: 1088,
  ...
  Procurement.spend.recordPO(po2);

  // ───── QC דמה ─────
  Procurement.quality.runCheck({
    purchaseOrderId: po1.id,
    productId: p1.id,
    quantityReceived: 500,
    quantityPassed: 495,
    quantityRejected: 5,
    defectReasons: ['אריזה פגומה'],
    inspectedBy: 'warehouse',
  });
  Procurement.quality.runCheck({
    ...
    quantityPassed: 188,
    quantityRejected: 12,
    defectReasons: ['שקים קרועים', 'לחות'],
    inspectedBy: 'warehouse',
  });

  // מבצעים recompute אחרון של כל הציונים
  Procurement.suppliers.recomputeAllScores();
}

export function getProcurementSnapshot(): ReturnType<ProcurementHyperintelligence['getDashboard']> & {
  suppliers: ReturnType<ProcurementHyperintelligence['getDashboard']>['suppliers'] & { list: Array<Supplier & { score?: SupplierScore }> };
  products: Product[];
  openSessions: NegotiationSession[];
  openAuctions: ReverseAuction[];
  activeBundles: BundlingCandidate[];
  pendingApprovals: ReturnType<AutoReorderEngine['listPending']>;
} {
  const dashboard = Procurement.getDashboard();
  const supplierList: Array<Supplier & { score?: SupplierScore }> = Array.from(Procurement.suppliers.suppliers.values()).map(s => ({
    ...s,
    score: Procurement.suppliers.getScore(s.id),
  }));
  return {
    ...dashboard,
    suppliers: { ...dashboard.suppliers, list: supplierList },
    products: Array.from(Procurement.demand.products.values()),
    openSessions: Procurement.negotiation.listOpen(),
    openAuctions: Procurement.auctions.listOpen(),
    activeBundles: Procurement.bundling.listActive(),
    pendingApprovals: Procurement.reorder.listPending(),
  };
}
