...
export const documentEngine = {
  ...
  // ── הצעת מחיר מלאה
  async generateQuote(orderId: string): Promise<string> {
...
<div class="footer">
  ...
  // ── חשבונית מס
  async generateInvoice(projectId: string, amount: number, invoiceType: 'advance' | 'final'): Promise<string> {
    const { rows } = await query(`
...
<div style="margin-top:32px;display:flex;justify-content:space-between;align-items:flex-end">
  ...
  // ── חוזה בסיסי
  async generateContract(projectId: string): Promise<string> {
    const { rows } = await query(`
