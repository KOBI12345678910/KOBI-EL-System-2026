...
export const supplyChainIntelligence = {
  ...
  async scoreSuppliers() {
    ...
    return rows.map((s: any) => {
      ...
      let tier: 'strategic' | 'preferred' | 'transactional' | 'probation';
      if (overall >= 80) tier = 'strategic';
      else if (overall >= 60) tier = 'preferred';
      else if (overall >= 40) tier = 'transactional';
      else tier = 'probation';
  ...
  async stockoutRisk() {
    ...
    const invByCategory = inventory.reduce((acc: any, r: any) => {
  ...
  async abcAnalysis() {
    ...
    const total = rows.reduce((s: number, r: any) => s + parseFloat(r.value || '0'), 0);
    let cumulative = 0;
  ...
  async inventoryTurnover() {
    const [avgStock, cogs] = await Promise.all([
      query(`SELECT COALESCE(AVG(qty * cost_per_unit), 0) AS avg_stock FROM material_items WHERE is_active = true`),
      query(`
  ...
  // ──────────────────────────────────────────────
  async deadStock(daysThreshold: number = 90) {
    const { rows } = await query(`
      SELECT
    ...
    const totalTiedUp = rows.reduce((s: number, r: any) => s + parseFloat(r.tied_up || '0'), 0);
