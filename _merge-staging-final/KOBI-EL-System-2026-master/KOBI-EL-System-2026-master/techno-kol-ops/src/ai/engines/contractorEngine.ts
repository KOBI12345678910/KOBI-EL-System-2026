...
export const contractorEngine = {

  // ניקוד קבלן מלא
  async scoreContractor(employeeId: string) {
  ...
  calculateBonus(score: number, orders: any): number {
  ...
  generateInsights(overall: number, completion: number, onTime: number, attendance: number): string[] {
    const insights: string[] = [];
    if (overall >= 85) insights.push('ביצועים מצוינים — מועמד להעלאת שכר');
    if (onTime < 70) insights.push('בעיית עמידה בזמנים — נדרש שיחה');
    if (attendance < 80) insights.push('אחוז נוכחות נמוך — בדוק מצב אישי');
    if (completion > 95) insights.push('אחוז השלמה גבוה — שקול תפקיד בכיר');
    if (overall < 50) insights.push('ביצועים נמוכים — תוכנית שיפור נדרשת');
    return insights;
  },

  // חבילת תגמול אטרקטיבית
  async designCompensationPackage(employeeId: string, currentSalary: number) {
    ...
    const packages = {
      ...
      total_estimated_monthly: Math.round(
        currentSalary +
