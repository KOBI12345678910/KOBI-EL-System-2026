...
export const recordingEngine = {
  ...
  analyzeText(transcript: string) {
    ...
    const sentimentScore = Math.round(
      50 + (positiveCount - negativeCount) * 8
    );
    ...
    if (agentTalkRatio > 0.65) recommendations.push('הסוכן מדבר יותר מדי — תן ללקוח לדבר');
    if (objections.length > 3) recommendations.push('הרבה התנגדויות — עבוד על טיפול בהתנגדויות');
    if (!hasNextStep) recommendations.push('לא הוגדר שלב הבא — תמיד סגור עם next step');
    if (sentimentScore < 40) recommendations.push('סנטימנט שלילי — בדוק מה הרחיק את הלקוח');
    if (sentimentScore > 70 && !hasNextStep) recommendations.push('לקוח חיובי אבל לא נסגר — החמצת הזדמנות');
  ...
  async agentCallSummary(agentId: string) {
