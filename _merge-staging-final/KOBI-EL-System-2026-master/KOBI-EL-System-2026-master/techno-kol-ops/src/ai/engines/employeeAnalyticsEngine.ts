...
export const employeeAnalyticsEngine = {

  async fullEmployeeAnalysis(employeeId: string) {
    const [profile, attendance, tasks, productivity, salary, risk] = await Promise.all([
      query(`SELECT * FROM employees WHERE id=$1`, [employeeId]),
  ...
  async attendanceAnalysis(empId: string) {
  ...
  async taskAnalysis(empId: string) {
    const { rows } = await query(`
      SELECT
  ...
  async productivityTrend(empId: string) {
    const { rows } = await query(`
    ...
    const values = rows.map((r: any) => parseInt(r.done_tasks));
  ...
  async salaryAnalysis(empId: string) {
    const { rows } = await query(`
    ...
    return {
      ...
      raise_recommendation: (years_employed: number) => {
        const yoe = years_employed;
        if (yoe >= 3 && percentile < 60) return { pct: 8, reason: 'ותק + מתחת לממוצע' };
        if (yoe >= 5 && percentile < 75) return { pct: 10, reason: 'ותק משמעותי' };
        return { pct: 3, reason: 'עדכון שוטף' };
      }
    };
  },

  async attritionRisk(empId: string) {
    const { rows } = await query(`
      SELECT e.*,
    ...
    if (prevTasks > 0 && recentTasks < prevTasks * 0.6) riskScore += 30;

    // ימי מחלה
    if (parseInt(e.recent_sick || '0') > 4) riskScore += 25;

    // שנות ותק — בשנתיים הראשונות סיכון גבוה
    const yearsEmployed = new Date().getFullYear() - new Date(e.start_date).getFullYear();
    if (yearsEmployed < 2) riskScore += 20;
