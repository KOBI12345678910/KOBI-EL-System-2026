...
// ── סכמת אונטולוגיה
export const ONTOLOGY_SCHEMA: Record<ObjectType, {
  table: string;
  display: string;
  preview: string[];
  links: { type: string; target: ObjectType; foreignKey: string }[];
}> = {
  WorkOrder: {
    table: 'work_orders',
    display: 'product',
...
// ── חישובי properties
async function computeProperties(type: ObjectType, obj: any): Promise<Record<string, any>> {
...
export const ontologyEngine = {
  ...
  async getObject(type: ObjectType, id: string) {
    const schema = ONTOLOGY_SCHEMA[type];
    if (!schema) throw new Error(`Unknown object type: ${type}`);

    const { rows } = await query(`SELECT * FROM ${schema.table} WHERE id=$1`, [id]);
    ...
    for (const link of schema.links) {
      try {
        const targetSchema = ONTOLOGY_SCHEMA[link.target];
        if (!targetSchema) continue;

        if (link.type === 'hasMany') {
          const { rows: linkedRows } = await query(
            `SELECT id, * FROM ${targetSchema.table} WHERE ${link.foreignKey}=$1 LIMIT 5`,
            [id]
          );
          ...
          const { rows: linkedRows } = await query(
            `SELECT id, * FROM ${targetSchema.table} WHERE id=$1`,
            [obj[link.foreignKey]]
          );
          if (linkedRows[0]) {
            links.push({
  ...
  // ── חיפוש גלובלי
  async globalSearch(q: string, types?: ObjectType[]) {
    const searchTypes = types || (Object.keys(ONTOLOGY_SCHEMA) as ObjectType[]);
    const results: any[] = [];
    ...
    for (const type of searchTypes) {
      ...
      try {
        const { rows } = await query(
          `SELECT * FROM ${schema.table} WHERE ${schema.display}::text ILIKE $1 LIMIT 5`,
          [`%${q}%`]
        );
