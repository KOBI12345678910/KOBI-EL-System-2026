...
# 🐞 תבנית דיווח באג אחידה — חובה לכל ממצא

> **לכל באג / כשל / אי-עקביות שאתה מגלה** — חייב להחזיר את 8 השדות האלה, בדיוק בפורמט הזה.
> ככה Release Readiness Agent (סוכן 19) יכול לאסוף מכל 20 הסוכנים בלי parsing ייחודי.

```
### BUG-<AGENT_ID>-<NNN>
- **כותרת:** <משפט אחד, ברור, חד>
- **תיאור:** <2–4 שורות — מה הבעיה, מה ההקשר העסקי>
- **שלבי שחזור:**
  1. <צעד 1>
  2. <צעד 2>
  3. <צעד 3>
- **תוצאה בפועל:** <מה קורה עכשיו>
- **תוצאה צפויה:** <מה היה צריך לקרות>
- **חומרה:** CRITICAL | HIGH | MEDIUM | LOW
- **מודול מושפע:** <service/path/file:line>
- **הצעת תיקון:** <קטע קוד / שורה / החלטה — קונקרטית, לא "לבדוק שוב">
- **Status:** OPEN
```

**כללי מילוי:**
- `AGENT_ID` = "QA01" עד "QA20" (לפי מספר הסוכן).
- `NNN` = מזהה רץ (001, 002, ...).
- `חומרה` — תישאר ב-4 ערכים בדיוק, אחרת Release Readiness לא יודע לספור.
- `Status` תמיד מתחיל ב-`OPEN` (אחר כך המפתח מעדכן ל-`RESOLVED` / `WONT-FIX` / `DUPLICATE`).
- אם באג חוצה מודולים — פצל ל-2 באגים נפרדים.

---

# 📤 תבנית סיכום סופי אחידה — חובה בסוף כל דו"ח

> **בסוף כל פרומפט** (אחרי כל הטבלאות והפרקים) — חייב להופיע הבלוק הזה בדיוק.
> זה מה ש-Release Readiness סורק כדי להוציא Go / No-Go.

```
## 📊 Final Summary — Agent-<N>-<NAME>
- **בדיקות בוצעו:** <int>
- **עברו:** <int>
- **נכשלו:** <int>
- **באגים קריטיים (CRITICAL):** <int>
- **באגים גבוהים (HIGH):** <int>
- **באגים בינוניים (MEDIUM):** <int>
- **באגים נמוכים (LOW):** <int>
- **פסק דין אישי:** READY | NEEDS-FIX | NOT-READY
- **נימוק בשורה אחת:** <…>
- **3 פעולות מיידיות מומלצות:**
  1. <…>
  2. <…>
  3. <…>
```
...
**חוק ברזל:**
- מי שלא מחזיר את שני הבלוקים האלה (Bug Template + Final Summary) — הדו"ח שלו נפסל ו-Release Readiness מתייחס אליו כ-`MISSING`.
- אסור להחליף שמות שדות. אסור לתרגם שדות. השמות בעברית כפי שהם.
...
2. חפש באגים באופן אגרסיבי — תנחה את עצמך שיש בעיה גם אם הכל נראה בסדר.
3. תעד כל כשל, גם אם קטן.
4. לכל בעיה תכתוב בדיוק לפי "🐞 תבנית דיווח באג אחידה" מראש הקובץ
   (8 שדות: כותרת, תיאור, שלבי שחזור, תוצאה בפועל, תוצאה צפויה, חומרה, מודול, הצעת תיקון).
   אל תמציא פורמט אחר. אל תתרגם שמות שדות.
5. בסוף העבודה תן סיכום בדיוק לפי "📤 תבנית סיכום סופי אחידה" מראש הקובץ
   (בדיקות בוצעו / עברו / נכשלו / באגים לפי חומרה / פסק דין / 3 פעולות מיידיות).
6. היה קשוח. המטרה היא למצוא את כל מה ששבור — לא לרצות את המפתח.
7. כבד את חוקי הברזל — לא מוחקים, לא משנים קבצים אלא אם נתבקשת במפורש,
   לא נוגע בנושא Palantir, לא חושף מידע פרטי.
8. אם לא מצאת באג באזור — אמור זאת במפורש ("0 findings in area X"),
   אל תשאיר סעיף ריק שגורם ל-Release Readiness לחשוב שהאזור לא נבדק.
```
...
**עודכן אחרון:** 2026-04-16
**מחבר:** KOBI-EL System 2026 — QA Framework
**חוק ברזל:** לא מוחקים, רק משדרגים ומגדלים.

**שדרוג 2026-04-16:**
- נוסף בלוק "🐞 תבנית דיווח באג אחידה" בראש הקובץ — 8 שדות חובה לכל ממצא (כותרת, תיאור, שלבי שחזור, תוצאה בפועל, תוצאה צפויה, חומרה, מודול, הצעת תיקון, Status).
- נוסף בלוק "📤 תבנית סיכום סופי אחידה" בראש הקובץ — כל סוכן מסיים בפורמט זהה כדי שסוכן 19 (Release Readiness) יוכל לאסוף בלי parsing ייחודי לכל סוכן.
- חוקי עבודה 4–5 עודכנו להפנות לתבניות האחידות במקום לשכפל אותן.
- נוסף חוק 8: אזור שנבדק ללא findings חייב להיכתב מפורשות "0 findings in area X" — אחרת סוכן 19 מסווג כ-MISSING.
