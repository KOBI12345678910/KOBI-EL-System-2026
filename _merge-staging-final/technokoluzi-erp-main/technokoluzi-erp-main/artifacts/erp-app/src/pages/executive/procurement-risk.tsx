...
import { authFetch } from "@/lib/utils";
    ...
    const STATUSES = ["פעיל", "מנוטר", "ממוגן", "מקובל", "סגור"] as const;
    const SC: Record<string, string> = { "פעיל": "bg-red-500/20 text-red-300", "מנוטר": "bg-yellow-500/20 text-yellow-300", "ממוגן": "bg-green-500/20 text-green-300", "מקובל": "bg-blue-500/20 text-blue-300", "סגור": "bg-gray-500/20 text-gray-300" };
    const CATS = ["ספק","אספקה","מחיר","איכות","גיאופוליטי","רגולטורי"];
...
export default function ProcurementRisk() {
  ...
  const [search, setSearch] = useState("");
    ...
    try {
      setError(null);
      const res = await authFetch("/api/executive/procurement-risk");
      if (res.ok) {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.title?.toLowerCase().includes(s) || r.riskType?.toLowerCase().includes(s) || r.supplier?.toLowerCase().includes(s)); }
      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingDown className="h-6 w-6 text-amber-400" />סיכוני רכש</h1><p className="text-sm text-muted-foreground mt-1">ניתוח סיכוני שרשרת אספקה, ספקים ותלויות</p></div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"סיכונים","v":"18","c":"text-amber-400","t":"+1","u":false},{"l":"קריטיים","v":"3","c":"text-red-400","t":"-1","u":true},{"l":"ספקים בסיכון","v":"5","c":"text-orange-400","t":"-2","u":true},{"l":"חשיפה","v":"₪850K","c":"text-purple-400","t":"-8%","u":true},{"l":"מגוון ספקים","v":"3.2","c":"text-blue-400","t":"+0.5","u":true},{"l":"ממוגנים","v":"78%","c":"text-green-400","t":"+5%","u":true}].map((k: any, i: number) => (
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("alternatives")}>חלופות<SI field="alternatives" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingDown className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין סיכוני רכש"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "סיכון חדש ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />סיכון חדש</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 text-muted-foreground text-xs">{row.supplier}</td>
            ...
            <td className="p-3 font-mono text-cyan-400 text-center">{row.alternatives}</td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div><Label className="text-muted-foreground text-xs">סוג *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>ספק</option><option>אספקה</option><option>מחיר</option><option>איכות</option><option>גיאופוליטי</option><option>רגולטורי</option></select></div>
            <div><Label className="text-muted-foreground text-xs">ספק</Label><Input type="text" placeholder="שם הספק" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            ...
            <div><Label className="text-muted-foreground text-xs">מיגון</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>ספק חלופי</option><option>מלאי ביטחון</option><option>חוזה ארוך</option><option>גידור</option><option>ביטוח</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>פעיל</option><option>מנוטר</option><option>ממוגן</option><option>מקובל</option><option>סגור</option></select></div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">ספק</p><p className="text-white mt-1 font-medium">{dr.supplier}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מיגון</p><p className="text-white mt-1 font-medium">{dr.mitigation}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">חלופות</p><p className="text-white mt-1 font-medium">{dr.alternatives}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">Lead Time</p><p className="text-white mt-1 font-medium">{dr.leadTime} ימים</p></div>
