...
import {
  ...
  MessageCircle, Send, Download, Printer, Users, ExternalLink
} from "lucide-react";
...
export default function ImportOrdersPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 text-white p-4 md:p-6" dir="rtl">
      <div className="max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            }} className="flex items-center gap-1.5 px-3 py-2 bg-green-700 hover:bg-green-600 rounded-lg text-sm text-gray-200 transition-colors">
            ...
            <button onClick={() => sendByEmail("הזמנות יבוא - טכנו-כל עוזי", generateEmailBody("הזמנות יבוא", orders, { orderNumber: "מס׳ הזמנה", supplierName: "ספק", countryOfOrigin: "ארץ", status: "סטטוס", totalValue: "שווי $" }))}
              className="flex items-center gap-1.5 px-3 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-sm text-gray-300 transition-colors">
              <Mail size={16} /> אימייל
            </button>
            <button onClick={() => exportToWord("הזמנות יבוא", orders, { orderNumber: "מס׳ הזמנה", supplierName: "ספק", countryOfOrigin: "ארץ מקור", incoterms: "Incoterms", status: "סטטוס", totalValue: "שווי $", estimatedArrival: "הגעה משוערת" }, "import_orders")}
              className="flex items-center gap-1.5 px-3 py-2 bg-indigo-700 hover:bg-indigo-600 rounded-lg text-sm text-gray-200 transition-colors">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex gap-1 bg-gray-800/50 rounded-xl p-1 border border-gray-700/50">
            {tabs.map(t => (
              <button key={t.key} onClick={() => setViewMode(t.key)}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === t.key ? "bg-cyan-600 text-white shadow-lg" : "text-muted-foreground hover:text-white hover:bg-gray-700/50"}`}>
          ...
          <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש הזמנה, ספק, ארץ..."
              className="w-full bg-gray-800/60 border border-gray-700 rounded-lg pr-10 pl-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-cyan-500/50 focus:outline-none" />
          </div>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            className="bg-gray-800/60 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-gray-300 focus:border-cyan-500/50 focus:outline-none">
        ...
        <AnimatePresence mode="wait">
                    ...
                    {["הוזמן", "בייצור", "נשלח", "בנמל", "בשחרור מכס", "התקבל"].map((step, i) => {
                      ...
                      return (
                        <div key={step} className="flex-1 text-center cursor-pointer" onClick={() => { setStatusFilter(step); setViewMode("list"); }}>
                          <div className={`w-10 h-10 mx-auto rounded-full flex items-center justify-center text-sm font-bold mb-1 hover:scale-110 transition-transform ${count > 0 ? "bg-cyan-500/30 text-cyan-400 border border-cyan-500/40" : "bg-gray-700/50 text-muted-foreground border border-gray-600/30"}`}>
                            {count}
                          </div>
                  ...
                  <div className="space-y-2">
                    {(() => {
                      ...
                      return Object.entries(countries).sort((a, b) => b[1] - a[1]).slice(0, 8).map(([country, count]) => (
                        <div key={country} className="flex items-center justify-between bg-gray-900/40 rounded-lg p-2 px-3">
                ...
                <div className="bg-gray-800/40 border border-gray-700/50 rounded-xl p-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {SHIPPING_METHODS.map(m => {
                      ...
                      return (
                        <div key={m.value} className="bg-gray-900/50 border border-gray-700/30 rounded-lg p-4 text-center">
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filtered.slice(0, 15).map(o => (
                        <tr key={o.id} onClick={() => openDetail(o)} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors cursor-pointer">
                          ...
                          <td className="py-3 px-2 text-center"><span className="px-2 py-0.5 rounded bg-gray-700 text-xs font-mono">{o.incoterms}</span></td>
                          ...
                          <td className="py-3 px-2 text-center"><span className={`px-2 py-1 rounded-full text-xs border ${statusColor(o.status)}`}>{o.status}</span></td>
                          <td className="py-3 px-2 text-center" onClick={e => e.stopPropagation()}>
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={() => openForm(o)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Edit2 size={14} className="text-blue-400" /></button>
                              <button onClick={async () => { if (globalConfirm("למחוק הזמנה זו?")) deleteMutation.mutate(o.id); }} className="p-1.5 hover:bg-gray-700 rounded-lg"><Trash2 size={14} className="text-red-400" /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          {viewMode === "list" && (
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              ...
              {filtered.map(o => (
                <motion.div key={o.id} layout onClick={() => openDetail(o)} className={`bg-gray-800/40 border border-gray-700/50 rounded-xl p-4 hover:border-cyan-500/30 hover:bg-gray-800/60 transition-all cursor-pointer ${bulk.isSelected(o.id) ? "ring-1 ring-primary/50 bg-primary/5" : ""}`}>
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`px-2 py-0.5 rounded-full text-xs border ${statusColor(o.status)}`}>{o.status}</span>
                          <span className="px-2 py-0.5 rounded bg-gray-700 text-xs font-mono">{o.incoterms}</span>
                        </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                        <button onClick={() => openDetail(o)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Eye size={15} className="text-muted-foreground" /></button>
                        <button onClick={() => openForm(o)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Edit2 size={15} className="text-blue-400" /></button>
                        <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMutation.mutate(o.id); }} className="p-1.5 hover:bg-gray-700 rounded-lg"><Trash2 size={15} className="text-red-400" /></button>
                      </div>
                    </div>
                  </div>
                </motion.div>
            ...
            <motion.div key="tracking" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
              ...
              <div className="space-y-3">
                {orders.filter(o => ["נשלח", "בנמל", "בשחרור מכס", "בייצור", "הוזמן"].includes(o.status)).sort((a, b) => {
                  ...
                  return (
                    <div key={o.id} className="bg-gray-800/40 border border-gray-700/50 rounded-xl p-4 hover:border-cyan-500/30 transition-all cursor-pointer" onClick={() => openDetail(o)}>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {orders.filter(o => o.status !== "בוטל").sort((a, b) => costSort ? parseFloat((b[costSort] as string | null) || "0") - parseFloat((a[costSort] as string | null) || "0") : 0).map(o => (
                      <tr key={o.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 cursor-pointer" onClick={() => openDetail(o)}>
          ...
          {selectedOrder && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                className="bg-gray-900 border border-gray-700 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => setSelectedOrder(null)} className="p-2 hover:bg-gray-800 rounded-lg"><X size={20} /></button>
                </div>

                <div className="flex border-b border-gray-700 mb-6">
                  {[{key:"details",label:"פרטים"},{key:"related",label:"רשומות קשורות"},{key:"docs",label:"מסמכים"},{key:"history",label:"היסטוריה"}].map(t => (
                    <button key={t.key} onClick={() => setDetailTab(t.key)} className={`px-4 py-2.5 text-sm font-medium border-b-2 ${detailTab === t.key ? "border-cyan-500 text-cyan-400" : "border-transparent text-muted-foreground hover:text-white"}`}>{t.label}</button>
                  ))}
                </div>

                {detailTab === "details" && (<>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6 text-sm">
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">ספק:</span> <span className="font-medium mr-2">{selectedOrder.supplierName || "—"}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">ארץ:</span> <span className="font-medium mr-2">{selectedOrder.countryOfOrigin || "—"}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">Incoterms:</span> <span className="font-mono font-bold mr-2">{selectedOrder.incoterms}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">מטבע:</span> <span className="mr-2">{selectedOrder.currency}</span> <span className="text-xs text-muted-foreground">שער: {selectedOrder.exchangeRate}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">משלוח:</span> <span className="mr-2 flex items-center gap-1 inline-flex">{shippingIcon(selectedOrder.shippingMethod)} {SHIPPING_METHODS.find(m => m.value === selectedOrder.shippingMethod)?.label}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">מכולה:</span> <span className="mr-2">{selectedOrder.containerType || "—"} x{selectedOrder.containerCount || 1}</span></div>
                </div>
          ...
          {showItemForm && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
                className="bg-gray-900 border border-gray-700 rounded-xl max-w-lg w-full p-5" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div><label className="text-xs text-muted-foreground block mb-1">שם פריט *</label>
                      <input value={itemForm.itemName} onChange={e => setItemForm({ ...itemForm, itemName: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground block mb-1">קוד HS</label>
                      <input value={itemForm.hsCode} onChange={e => setItemForm({ ...itemForm, hsCode: e.target.value })} placeholder="8501.10" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm font-mono focus:border-cyan-500/50 focus:outline-none" /></div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div><label className="text-xs text-muted-foreground block mb-1">כמות</label>
                      <input type="number" value={itemForm.quantity} onChange={e => setItemForm({ ...itemForm, quantity: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground block mb-1">מחיר</label>
                      <input type="number" value={itemForm.unitPrice} onChange={e => setItemForm({ ...itemForm, unitPrice: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground block mb-1">מכס %</label>
                      <input type="number" value={itemForm.customsDutyPct} onChange={e => setItemForm({ ...itemForm, customsDutyPct: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground block mb-1">משקל (kg)</label>
                      <input type="number" value={itemForm.weightKg} onChange={e => setItemForm({ ...itemForm, weightKg: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                  </div>
                  <div className="flex justify-end gap-3">
                    <button onClick={() => setShowItemForm(false)} className="px-4 py-2 border border-gray-600 rounded-lg text-sm">ביטול</button>
                    <button onClick={() => {
                    ...
                    }} disabled={!itemForm.itemName} className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 rounded-lg text-sm font-medium disabled:bg-gray-700">שמור</button>
                  </div>
                </div>
              </motion.div>
            </motion.div>
        ...
        <AnimatePresence>
          {showForm && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                className="bg-gray-900 border border-gray-700 rounded-2xl max-w-4xl w-full max-h-[92vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-bold">{editingOrder ? `עריכת ${editingOrder.orderNumber}` : "הזמנת יבוא חדשה"}</h3>
                  <button onClick={() => setShowForm(false)} className="p-2 hover:bg-gray-800 rounded-lg"><X size={20} /></button>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      ...
                      <input value={form.supplierName} onChange={e => setForm({ ...form, supplierName: e.target.value })}
                        className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" />
                    </div>
                    <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div>
                        ...
                        <select value={form.incoterms} onChange={e => setForm({ ...form, incoterms: e.target.value })}
                          className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm font-mono focus:border-cyan-500/50 focus:outline-none">
                      ...
                      <div>
                        ...
                        <select value={form.currency} onChange={e => setForm({ ...form, currency: e.target.value })}
                          className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div><label className="text-xs text-muted-foreground mb-1 block">שווי ($)</label>
                      <input type="number" value={form.totalValue} onChange={e => setForm({ ...form, totalValue: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground mb-1 block">שווי (₪)</label>
                      <input type="number" value={form.totalValueIls} onChange={e => setForm({ ...form, totalValueIls: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground mb-1 block">מכס %</label>
                      <input type="number" value={form.customsDutyPct} onChange={e => setForm({ ...form, customsDutyPct: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground mb-1 block">מכס משוער (₪)</label>
                      <input type="number" value={form.estimatedCustomsDuty} onChange={e => setForm({ ...form, estimatedCustomsDuty: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                  </div>
                  ...
                  <div className="bg-gray-800/40 rounded-xl p-4 border border-gray-700/50">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      ...
                      <div><label className="text-xs text-muted-foreground mb-1 block">נמל מוצא</label>
                        <input value={form.portOfOrigin} onChange={e => setForm({ ...form, portOfOrigin: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                      <div><label className="text-xs text-muted-foreground mb-1 block">נמל יעד</label>
                        <input value={form.portOfDestination} onChange={e => setForm({ ...form, portOfDestination: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
                      <div><label className="text-xs text-muted-foreground mb-1 block">תאריך יציאה</label>
                        <input type="date" value={form.estimatedDeparture} onChange={e => setForm({ ...form, estimatedDeparture: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                      <div><label className="text-xs text-muted-foreground mb-1 block">הגעה משוערת</label>
                        <input type="date" value={form.estimatedArrival} onChange={e => setForm({ ...form, estimatedArrival: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                      <div><label className="text-xs text-muted-foreground mb-1 block">הגעה בפועל</label>
                        <input type="date" value={form.actualArrival} onChange={e => setForm({ ...form, actualArrival: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-800/40 rounded-xl p-4 border border-gray-700/50">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div><label className="text-xs text-muted-foreground mb-1 block">חברת ביטוח</label>
                        <input value={form.insuranceCompany} onChange={e => setForm({ ...form, insuranceCompany: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                      <div><label className="text-xs text-muted-foreground mb-1 block">מספר פוליסה</label>
                        <input value={form.insurancePolicyNumber} onChange={e => setForm({ ...form, insurancePolicyNumber: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                      <div><label className="text-xs text-muted-foreground mb-1 block">סכום ביטוח ($)</label>
                        <input type="number" value={form.insuranceValue} onChange={e => setForm({ ...form, insuranceValue: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-800/40 rounded-xl p-4 border border-gray-700/50">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div><label className="text-xs text-muted-foreground mb-1 block">מספר L/C</label>
                        <input value={form.lcNumber} onChange={e => setForm({ ...form, lcNumber: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm font-mono focus:border-cyan-500/50 focus:outline-none" /></div>
                      <div><label className="text-xs text-muted-foreground mb-1 block">בנק</label>
                        <input value={form.lcBank} onChange={e => setForm({ ...form, lcBank: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                      <div><label className="text-xs text-muted-foreground mb-1 block">סכום ($)</label>
                        <input type="number" value={form.lcAmount} onChange={e => setForm({ ...form, lcAmount: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                      <div><label className="text-xs text-muted-foreground mb-1 block">תפוגה</label>
                        <input type="date" value={form.lcExpiryDate} onChange={e => setForm({ ...form, lcExpiryDate: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div><label className="text-xs text-muted-foreground mb-1 block">עלות הובלה ($)</label>
                      <input type="number" value={form.freightCost} onChange={e => setForm({ ...form, freightCost: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground mb-1 block">טיפול ($)</label>
                      <input type="number" value={form.handlingCost} onChange={e => setForm({ ...form, handlingCost: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground mb-1 block">עלויות אחרות ($)</label>
                      <input type="number" value={form.otherCosts} onChange={e => setForm({ ...form, otherCosts: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground mb-1 block">עלות נחיתה ($)</label>
                      <input type="number" value={form.totalLandedCost} onChange={e => setForm({ ...form, totalLandedCost: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                  </div>

                  {/* Agents */}
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    <div><label className="text-xs text-muted-foreground mb-1 block">עמיל מכס</label>
                      <input value={form.customsBroker} onChange={e => setForm({ ...form, customsBroker: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground mb-1 block">חברת שילוח</label>
                      <input value={form.forwardingAgent} onChange={e => setForm({ ...form, forwardingAgent: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                    <div><label className="text-xs text-muted-foreground mb-1 block">איש קשר</label>
                      <input value={form.contactPerson} onChange={e => setForm({ ...form, contactPerson: e.target.value })} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none" /></div>
                  </div>

                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">הערות</label>
                    <textarea rows={2} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-cyan-500/50 focus:outline-none resize-none" />
                  </div>

                  <div className="flex justify-end gap-3 pt-2">
                    <button onClick={() => setShowForm(false)} className="px-5 py-2.5 border border-gray-600 rounded-lg text-gray-300 hover:bg-gray-800">ביטול</button>
                    <button onClick={handleSubmit} disabled={saveMutation.isPending || !form.supplierName}
                      className="flex items-center gap-2 px-6 py-2.5 bg-cyan-600 hover:bg-cyan-500 disabled:bg-gray-700 disabled:text-muted-foreground rounded-lg font-medium">
        ...
        <AnimatePresence>
          {showSendModal && savedOrder && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                className="bg-gray-900 border border-gray-700 rounded-2xl max-w-3xl w-full max-h-[85vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()} dir="rtl">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <button onClick={() => setShowSendModal(false)} className="p-2 hover:bg-gray-800 rounded-lg"><X size={20} /></button>
                </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto">
                      {suppliersForSend.filter(s => s.status === "פעיל").slice(0, 20).map((s: LocalSupplier) => (
                        <label key={`local-${s.id}`} className="flex items-center gap-2 p-2 hover:bg-gray-800/40 rounded-lg cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto">
                      {foreignSuppliersForSend.filter(s => s.status === "פעיל" || s.status === "VIP").slice(0, 20).map((s: ForeignSupplier) => (
                        <label key={`foreign-${s.id}`} className="flex items-center gap-2 p-2 hover:bg-gray-800/40 rounded-lg cursor-pointer">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  }} className="flex flex-col items-center gap-2 p-4 bg-green-600 hover:bg-green-500 rounded-xl text-white text-sm font-medium transition-colors">
                  ...
                  }} className="flex flex-col items-center gap-2 p-4 bg-blue-600 hover:bg-blue-500 rounded-xl text-white text-sm font-medium transition-colors">
                    <Mail size={22} />
                    <span>אימייל</span>
                  </button>
                  ...
                  <button onClick={() => exportToWord(`הזמנת יבוא ${savedOrder.orderNumber}`, [savedOrder], { orderNumber: "מס׳ הזמנה", supplierName: "ספק", countryOfOrigin: "ארץ מקור", incoterms: "Incoterms", currency: "מטבע", totalValue: "שווי $", estimatedCustomsDuty: "מכס ₪", freightCost: "הובלה $", status: "סטטוס", estimatedArrival: "הגעה משוערת" }, `import_order_${savedOrder.orderNumber}`)}
                    className="flex flex-col items-center gap-2 p-4 bg-indigo-600 hover:bg-indigo-500 rounded-xl text-white text-sm font-medium transition-colors">
                    <FileText size={22} />
                    <span>Word (.doc)</span>
                  </button>
                </div>
                ...
                <div className="flex justify-end mt-5">
                  <button onClick={() => setShowSendModal(false)} className="px-5 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-sm">סגור</button>
                </div>
              </motion.div>
            </motion.div>
          )}
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40">
          <div className="flex items-center gap-2 bg-gray-800/95 backdrop-blur border border-gray-700/50 rounded-2xl px-4 py-2.5 shadow-2xl">
