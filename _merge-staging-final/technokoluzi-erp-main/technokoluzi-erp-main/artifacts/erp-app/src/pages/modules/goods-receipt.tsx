...
import {
  ...
  ClipboardCheck, AlertCircle, Hash, MapPin, Calendar, User
} from "lucide-react";
...
export default function GoodsReceiptPage() {
  ...
  const formValidation = useFormValidation({ receiptNumber: { required: true, message: "מספר קבלה נדרש" }, supplierId: { required: true, message: "ספק נדרש" } });
  ...
  function closeForm() { setShowForm(false); setEditingId(null); setForm(emptyForm); setReceiptItems([]); }
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.receiptNumber || !form.supplierId) return;
  ...
  function openEdit(r: GoodsReceipt) {
    ...
    setEditingId(r.id); setReceiptItems([]); setShowForm(true);
  }
    ...
    <div className="min-h-screen" dir="rtl">
      <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <button onClick={() => { setForm({...emptyForm, receiptDate: new Date().toISOString().split("T")[0]}); setEditingId(null); setReceiptItems([]); setShowForm(true); }}
            className="flex items-center gap-2 px-5 py-2.5 bg-teal-600 hover:bg-teal-500 text-white rounded-xl font-medium transition-colors">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-0 sm:min-w-[200px]">
            ...
            <input type="text" placeholder="חיפוש לפי מספר קבלה, ספק או תעודת משלוח..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pr-10 pl-4 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:border-teal-500 focus:outline-none" />
          </div>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            className="px-3 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white focus:border-teal-500 focus:outline-none">
        ...
        {isLoading ? (
          ...
          <>
          <BulkActions bulk={bulk} actions={defaultBulkActions} entityName="קבלות סחורה" />
          <div className="bg-[#1a1d23] border border-gray-800 rounded-xl overflow-hidden">
            ...
            <table className="w-full text-right">
              ...
              <tbody>
                {filtered.map(r => (
                  <>
                    <tr key={r.id} className={`border-b border-gray-800/50 hover:bg-[#1e2128] cursor-pointer transition-colors ${r.overallQuality && r.overallQuality !== "תקין" ? "bg-red-500/5" : ""} ${bulk.isSelected(r.id) ? "bg-primary/5" : ""}`}
                      onClick={() => loadReceiptItems(r.id)}>
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}><BulkCheckbox bulk={bulk} id={r.id} /></td>
                      ...
                      <td className="px-4 py-3 text-white text-sm">{getSupplierName(r.supplierId)}</td>
                      ...
                      <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={() => openEdit(r)} className="p-1.5 text-muted-foreground hover:text-amber-400 rounded-md"><Edit2 className="w-4 h-4" /></button>
                          <button onClick={() => setDeleteConfirm(r.id)} className="p-1.5 text-muted-foreground hover:text-red-400 rounded-md"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </td>
                    </tr>
                    {expandedRow === r.id && (
                      <tr key={`exp-${r.id}`}>
                        <td colSpan={11} className="px-4 py-4 bg-[#12141a]">
                          {expandedItems[r.id] && expandedItems[r.id].length > 0 ? (
                            <div className="space-y-2">
                              <div className="flex items-center gap-2 mb-2">
                                <ClipboardCheck className="w-4 h-4 text-teal-400" />
                                <span className="text-sm font-medium text-white">פריטים ({expandedItems[r.id].length})</span>
                              </div>
                                  ...
                                  {expandedItems[r.id].map((item: GoodsReceiptItem) => {
                                    ...
                                    return (
                                      <tr key={item.id} className="border-t border-gray-800/30">
                                        ...
                                        <td className="py-2 pr-2 text-white font-medium">{item.receivedQuantity}</td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <div>
                  <h2 className="text-xl font-bold text-white flex items-center gap-2">קבלה {selectedReceipt.receiptNumber}</h2>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => setSelectedReceipt(null)} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
                </div>
              </div>
                ...
                <div className="flex gap-1 px-6 pt-2">
                  {[
                    ...
                    <button key={tab.key} onClick={() => setDetailTab(tab.key)}
                      className={`px-4 py-2 text-sm font-medium rounded-t-lg ${detailTab === tab.key ? "bg-[#12141a] text-white border-t border-x border-gray-700" : "text-muted-foreground hover:text-white"}`}>
                      {tab.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="p-6 max-h-[65vh] overflow-y-auto">
                {detailTab === "info" && (
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">תאריך קבלה</p><p className="text-white font-medium mt-1">{selectedReceipt.receiptDate ? new Date(selectedReceipt.receiptDate).toLocaleDateString("he-IL") : "—"}</p></div>
                    <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">מקבל</p><p className="text-white font-medium mt-1">{selectedReceipt.receivedBy || "—"}</p></div>
                    <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">בודק</p><p className="text-white font-medium mt-1">{selectedReceipt.inspector || "—"}</p></div>
                    <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">מחסן</p><p className="text-white font-medium mt-1">{selectedReceipt.warehouseLocation || "—"}</p></div>
                    <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">תעודת משלוח</p><p className="text-white font-medium mt-1 font-mono">{selectedReceipt.deliveryNoteNumber || "—"}</p></div>
                    <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">מספר רכב</p><p className="text-white font-medium mt-1 font-mono">{selectedReceipt.vehicleNumber || "—"}</p></div>
                    <div className="bg-[#12141a] rounded-xl p-4 md:col-span-3"><p className="text-muted-foreground text-xs">איכות כללית</p>
                    ...
                    {selectedReceipt.notes && (
                      <div className="bg-[#12141a] rounded-xl p-4 md:col-span-3"><p className="text-muted-foreground text-xs mb-1">הערות</p><p className="text-gray-300 text-sm">{selectedReceipt.notes}</p></div>
                    )}
                  </div>
                )}
                ...
                {detailTab === "items" && selectedReceipt.items && selectedReceipt.items.length > 0 && (
                  <div className="space-y-3">
                    {selectedReceipt.items.map(item => {
                      const v = getVarianceInfo(item.expectedQuantity, item.receivedQuantity);
                      return (
                        <div key={item.id} className={`bg-[#12141a] rounded-xl p-4 border ${item.qualityStatus === "פגום" ? "border-red-500/30" : item.qualityStatus === "פגום חלקית" ? "border-amber-500/30" : "border-gray-800"}`}>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                            ...
                            <div><p className="text-muted-foreground text-xs">התקבל</p><p className="text-white font-medium">{item.receivedQuantity} {item.unit}</p></div>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-5xl mx-4 mb-10" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white">{editingId ? "עריכת קבלה" : "קבלת סחורה חדשה"}</h2>
                <button onClick={closeForm} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
                <div className="bg-[#12141a] border border-gray-800 rounded-xl p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div><label className="block text-xs text-muted-foreground mb-1">מספר קבלה *</label><input value={form.receiptNumber} onChange={e => setForm({...form, receiptNumber: e.target.value})} placeholder="GR-001" className="w-full px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-teal-500 focus:outline-none text-sm" /></div>
                    <div><label className="block text-xs text-muted-foreground mb-1">הזמנת רכש (PO)</label>
                      <select value={form.orderId} onChange={e => selectPurchaseOrder(e.target.value)}
                        className="w-full px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white focus:border-teal-500 focus:outline-none text-sm">
                        <option value="">ללא הזמנה</option>
                        {openPOs.map(o => <option key={o.id} value={o.id}>{o.orderNumber} — {getSupplierName(o.supplierId)}</option>)}
                      </select>
                    </div>
                    <div><label className="block text-xs text-muted-foreground mb-1">ספק *</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    <div><label className="block text-xs text-muted-foreground mb-1">תאריך קבלה</label><input type="date" value={form.receiptDate} onChange={e => setForm({...form, receiptDate: e.target.value})} dir="ltr" className="w-full px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white focus:border-teal-500 focus:outline-none text-sm" /></div>
                    <div><label className="block text-xs text-muted-foreground mb-1">סטטוס</label>
                      <select value={form.status} onChange={e => setForm({...form, status: e.target.value})} className="w-full px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white focus:border-teal-500 focus:outline-none text-sm">
                        {STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                    </div>
                    <div><label className="block text-xs text-muted-foreground mb-1">מקבל</label><input value={form.receivedBy} onChange={e => setForm({...form, receivedBy: e.target.value})} placeholder="שם מקבל" className="w-full px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-teal-500 focus:outline-none text-sm" /></div>
                ...
                <div className="bg-[#12141a] border border-gray-800 rounded-xl p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    <div><label className="block text-xs text-muted-foreground mb-1">תעודת משלוח</label><input value={form.deliveryNoteNumber} onChange={e => setForm({...form, deliveryNoteNumber: e.target.value})} placeholder="מספר ת. משלוח" className="w-full px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-teal-500 focus:outline-none text-sm font-mono" /></div>
                    <div><label className="block text-xs text-muted-foreground mb-1">מספר רכב</label><input value={form.vehicleNumber} onChange={e => setForm({...form, vehicleNumber: e.target.value})} placeholder="לוחית רישוי" className="w-full px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-teal-500 focus:outline-none text-sm font-mono" /></div>
                ...
                {!editingId && (
                  <div className="border border-gray-700 rounded-xl p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-semibold text-white flex items-center gap-2"><Package className="w-5 h-5 text-teal-400" />פריטים שנתקבלו</h3>
                      ...
                      return (
                        <div key={idx} className="bg-[#12141a] border border-gray-800 rounded-lg p-3 space-y-3">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <div><label className="block text-xs text-muted-foreground mb-1">חומר</label><select value={item.materialId} onChange={e => updateReceiptItem(idx, "materialId", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-teal-500 focus:outline-none"><option value="">בחר</option>{materials.map(m => <option key={m.id} value={m.id}>{m.materialName}</option>)}</select></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">קוד</label><input value={item.itemCode} onChange={e => updateReceiptItem(idx, "itemCode", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm font-mono focus:border-teal-500 focus:outline-none" /></div>
                            <div className="sm:col-span-2"><label className="block text-xs text-muted-foreground mb-1">תיאור *</label><input value={item.itemDescription} onChange={e => updateReceiptItem(idx, "itemDescription", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-teal-500 focus:outline-none" /></div>
                          </div>
                          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                            <div><label className="block text-xs text-muted-foreground mb-1">כמות צפויה</label><input type="number" value={item.expectedQuantity} onChange={e => updateReceiptItem(idx, "expectedQuantity", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-teal-500 focus:outline-none" dir="ltr" /></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">כמות שנתקבלה</label><input type="number" value={item.receivedQuantity} onChange={e => updateReceiptItem(idx, "receivedQuantity", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-teal-500 focus:outline-none" dir="ltr" /></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">יחידה</label><select value={item.unit} onChange={e => updateReceiptItem(idx, "unit", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-teal-500 focus:outline-none">{UNITS.map(u => <option key={u} value={u}>{u}</option>)}</select></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">בדיקת איכות</label><select value={item.qualityStatus} onChange={e => updateReceiptItem(idx, "qualityStatus", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-teal-500 focus:outline-none">{QUALITY_STATUSES.map(q => <option key={q} value={q}>{q}</option>)}</select></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">אצווה (Lot)</label><input value={item.lotNumber} onChange={e => updateReceiptItem(idx, "lotNumber", e.target.value)} placeholder="LOT-XXX" className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm font-mono placeholder-gray-600 focus:border-teal-500 focus:outline-none" /></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">סריאלי (S/N)</label><input value={item.serialNumber} onChange={e => updateReceiptItem(idx, "serialNumber", e.target.value)} placeholder="SN-XXX" className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm font-mono placeholder-gray-600 focus:border-teal-500 focus:outline-none" /></div>
                          </div>
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <div><label className="block text-xs text-muted-foreground mb-1">מיקום אחסון</label><select value={item.storageLocation} onChange={e => updateReceiptItem(idx, "storageLocation", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-teal-500 focus:outline-none"><option value="">בחר</option>{WAREHOUSES.map(w => <option key={w} value={w}>{w}</option>)}</select></div>
                            <div><label className="block text-xs text-muted-foreground mb-1">תוקף</label><input type="date" value={item.expiryDate} onChange={e => updateReceiptItem(idx, "expiryDate", e.target.value)} dir="ltr" className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-teal-500 focus:outline-none" /></div>
                            <div className="sm:col-span-2"><label className="block text-xs text-muted-foreground mb-1">הערות מצב</label><input value={item.conditionNotes} onChange={e => updateReceiptItem(idx, "conditionNotes", e.target.value)} placeholder="תיאור מצב פיזי" className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm placeholder-gray-600 focus:border-teal-500 focus:outline-none" /></div>
                          </div>
                        </div>
                      );
                    })}
                ...
                <div><label className="block text-xs text-muted-foreground mb-1">הערות כלליות</label><textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-teal-500 focus:outline-none resize-none text-sm" rows={2} /></div>
                ...
                <div className="flex items-center gap-3 pt-2">
                  <button type="submit" disabled={createMut.isPending || updateMut.isPending} className="flex items-center gap-2 px-6 py-2.5 bg-teal-600 hover:bg-teal-500 disabled:bg-teal-800 text-white rounded-lg font-medium"><Save className="w-4 h-4" />{editingId ? "עדכן" : "שמור"}</button>
        ...
        {deleteConfirm !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
            <div className="bg-[#1a1d23] border border-gray-700 rounded-xl p-6 max-w-sm mx-4" dir="rtl">
              <h3 className="text-lg font-bold text-white mb-2">מחיקת קבלה</h3>
