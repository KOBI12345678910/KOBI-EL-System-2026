...
import {
  ...
  Filter, TrendingUp, Package, AlertCircle, Flame, ArrowUpRight
} from "lucide-react";
...
export default function PurchaseRequestsPage() {
  ...
  const pending = requests.filter(r => r.status === "ממתין לאישור");
  const approved = requests.filter(r => r.status === "מאושר");
  const rejected = requests.filter(r => r.status === "נדחה");
  const urgent = requests.filter(r => r.priority === "דחוף" || r.priority === "גבוה");
  const drafts = requests.filter(r => r.status === "טיוטה");
  const completed = requests.filter(r => r.status === "בוצע");
  ...
  const overdue = requests.filter(r => {
    if (!r.neededBy || ["בוצע", "בוטל", "נדחה"].includes(r.status)) return false;
  ...
  function closeForm() { setShowForm(false); setEditingId(null); setForm(emptyForm); setItems([]); }
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.requestNumber || !form.title) return;
  ...
  function getStepIndex(status: string) {
    if (status === "נדחה" || status === "בוטל") return -1;
      ...
      <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-4">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2">
              {processSteps.map((step, i) => {
                const count = requests.filter(r => r.status === step.status).length;
                return (
                  <div key={i} className="flex items-center gap-2 flex-1 min-w-[100px]">
                    <div className={`flex-1 ${STATUS_COLORS[step.status]?.split(" ")[0] || "bg-muted/10"} border border-gray-700 rounded-xl p-3 text-center`}>
          ...
          {statusDistribution.length > 0 && (
            <div className="bg-[#1a1d23] border border-gray-800 rounded-xl p-4">
              ...
              <div className="flex items-center gap-1 h-8 rounded-full overflow-hidden bg-[#12141a]">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-0 sm:min-w-[200px]">
            ...
            <input type="text" placeholder="חיפוש לפי מספר, כותרת, מבקש או מחלקה..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pr-10 pl-4 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none" />
          </div>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            className="px-3 py-2.5 bg-[#1a1d23] border border-gray-700 rounded-xl text-white focus:border-blue-500 focus:outline-none">
        ...
        {isLoading ? (
          ...
          <div className="bg-[#1a1d23] border border-gray-800 rounded-xl overflow-hidden">
            <div className="px-4 py-2 border-b border-gray-800 bg-[#12141a] flex items-center justify-between">
            ...
            <table className="w-full text-right">
              ...
              <tbody>
                {filtered.map(r => {
                  ...
                  const isOverdue = days !== null && days < 0 && !["בוצע", "בוטל", "נדחה"].includes(r.status);
                  const expanded = expandedItems[r.id];
                  return (
                    <>
                      <tr key={r.id} className={`border-b border-gray-800/50 hover:bg-[#1e2128] cursor-pointer transition-colors ${isSelected(r.id) ? "bg-blue-500/10" : isOverdue ? "bg-red-500/5" : ""}`}
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-medium ${PRIORITY_COLORS[r.priority] || ""}`}>
                            {PRIORITY_ICONS[r.priority] && (() => { const Icon = PRIORITY_ICONS[r.priority]; return <Icon className="w-3 h-3" />; })()}
                            {r.priority}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          ...
                          {days !== null && !["בוצע", "בוטל", "נדחה"].includes(r.status) && (
                        ...
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => openEdit(r)} className="p-1.5 text-muted-foreground hover:text-amber-400 rounded-md" title="ערוך"><Edit2 className="w-4 h-4" /></button>
                            <button onClick={() => setDeleteConfirm(r.id)} className="p-1.5 text-muted-foreground hover:text-red-400 rounded-md" title="מחק"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                      {expandedRow === r.id && (
                        <tr key={`expanded-${r.id}`}>
                          <td colSpan={10} className="px-4 py-4 bg-[#12141a]">
                            <div className="space-y-3">
                              <div className="flex items-center gap-1 overflow-x-auto pb-2">
                                {processSteps.map((step, si) => {
                                  ...
                                  return (
                                    <div key={si} className="flex items-center gap-1">
                                      <div className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${current ? "bg-blue-600 text-white ring-2 ring-blue-400/30" : active ? "bg-emerald-500/20 text-emerald-400" : "bg-gray-800 text-muted-foreground"}`}>
                                        {step.label}
                                      </div>
                                ...
                                {(r.status === "נדחה" || r.status === "בוטל") && <span className="px-3 py-1.5 rounded-lg text-xs font-medium bg-red-500/20 text-red-400">{r.status}</span>}
                              </div>
                              ...
                              {expanded && expanded.items.length > 0 && (
                                <table className="w-full text-right text-sm">
                                  <thead><tr className="text-muted-foreground border-b border-gray-800">
                                  ...
                                  <tbody>{expanded.items.map((item: PurchaseRequestItem) => (
                                    <tr key={item.id} className="border-t border-gray-800/30">
                                      ...
                                      <td className="py-2 pr-2 text-white font-medium" dir="ltr">{item.estimatedPrice && item.quantity ? `₪${(parseFloat(item.estimatedPrice) * parseFloat(item.quantity)).toLocaleString()}` : "—"}</td>
        ...
        {selectedRequest && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-4xl mx-4 mb-10" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <div>
                  <h2 className="text-xl font-bold text-white">דרישת רכש {selectedRequest.requestNumber}</h2>
                  <p className="text-sm text-muted-foreground">{selectedRequest.title}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`px-3 py-1 rounded-lg text-sm font-medium ${STATUS_COLORS[selectedRequest.status] || ""}`}>{selectedRequest.status}</span>
                  <span className={`px-3 py-1 rounded-lg text-sm font-medium ${PRIORITY_COLORS[selectedRequest.priority] || ""}`}>{selectedRequest.priority}</span>
                  <button onClick={() => { setSelectedRequest(null); setShowApprovalForm(null); }} className="p-1 text-muted-foreground hover:text-white"><X className="w-5 h-5" /></button>
                </div>
              </div>
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="flex items-center gap-1 overflow-x-auto pb-2">
                  {processSteps.map((step, si) => {
                    ...
                    return (
                      <div key={si} className="flex items-center gap-1">
                        <div className={`px-3 py-2 rounded-lg text-xs font-medium ${current ? "bg-blue-600 text-white ring-2 ring-blue-400/30" : active ? "bg-emerald-500/20 text-emerald-400" : "bg-gray-800 text-muted-foreground"}`}>
                          {step.label}
                        </div>
                        {si < processSteps.length - 1 && <ArrowLeft className={`w-3 h-3 flex-shrink-0 ${active ? "text-emerald-500" : "text-foreground"}`} />}
                      </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">מבקש</p><p className="text-white font-medium">{selectedRequest.requesterName || "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">מחלקה</p><p className="text-white font-medium">{selectedRequest.department || "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">נדרש עד</p><p className="text-white font-medium">{selectedRequest.neededBy ? new Date(selectedRequest.neededBy).toLocaleDateString("he-IL") : "—"}</p></div>
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs">סכום משוער</p><p className="text-emerald-400 font-bold text-lg" dir="ltr">{selectedRequest.totalEstimated ? `₪${parseFloat(selectedRequest.totalEstimated).toLocaleString()}` : "—"}</p></div>
                </div>
                        ...
                        <div key={a.id} className={`rounded-xl p-4 border ${a.approvalStatus === "מאושר" ? "border-emerald-500/30 bg-emerald-500/5" : a.approvalStatus === "נדחה" ? "border-red-500/30 bg-red-500/5" : "border-amber-500/30 bg-amber-500/5"}`}>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-white font-medium">{a.approverName}</p>
                ...
                {(selectedRequest.status === "ממתין לאישור" || selectedRequest.status === "טיוטה") && (
                  <div>
                    {showApprovalForm === selectedRequest.id ? (
                      <div className="bg-[#12141a] rounded-xl p-4 border border-indigo-500/30">
                        <h4 className="text-sm font-semibold text-white mb-3">הוסף החלטת אישור</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          <input value={approvalForm.approverName} onChange={e => setApprovalForm({...approvalForm, approverName: e.target.value})} placeholder="שם המאשר" className="px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white text-sm placeholder-gray-500 focus:border-blue-500 focus:outline-none" />
                          <select value={approvalForm.approvalStatus} onChange={e => setApprovalForm({...approvalForm, approvalStatus: e.target.value})} className="px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white text-sm focus:border-blue-500 focus:outline-none">
                            <option value="מאושר">מאושר</option>
                            <option value="נדחה">נדחה</option>
                            <option value="ממתין">ממתין</option>
                          </select>
                          <input value={approvalForm.approvalLevel} onChange={e => setApprovalForm({...approvalForm, approvalLevel: e.target.value})} placeholder="רמת אישור" type="number" className="px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white text-sm placeholder-gray-500 focus:border-blue-500 focus:outline-none" dir="ltr" />
                        </div>
                        <textarea value={approvalForm.comments} onChange={e => setApprovalForm({...approvalForm, comments: e.target.value})} placeholder="הערות" className="w-full mt-2 px-3 py-2 bg-[#1a1d23] border border-gray-700 rounded-lg text-white text-sm placeholder-gray-500 focus:border-blue-500 focus:outline-none" rows={2} />
                        <div className="flex gap-2 mt-3">
                          ...
                          <button onClick={() => setShowApprovalForm(null)} className="px-4 py-2 text-muted-foreground hover:text-white text-sm">ביטול</button>
                        </div>
                      </div>
                    ) : (
                ...
                {selectedRequest.items && selectedRequest.items.length > 0 && (
                  <div>
                    ...
                    <div className="flex justify-end mt-2 text-sm border-t border-gray-700 pt-2">
                      <span className="text-muted-foreground">סה״כ משוער:</span>
                      <span className="text-white font-bold font-mono mr-2" dir="ltr">₪{selectedRequest.items.reduce((s, i) => s + (parseFloat(i.estimatedPrice || "0") * parseFloat(i.quantity || "0")), 0).toLocaleString()}</span>
                    </div>
                  </div>
                )}
                {selectedRequest.notes && (
                  <div className="bg-[#12141a] rounded-xl p-4"><p className="text-muted-foreground text-xs mb-1">הערות</p><p className="text-gray-300 text-sm">{selectedRequest.notes}</p></div>
                )}
                </>)}
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center pt-6 overflow-y-auto" onClick={closeForm}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1d23] border border-gray-700 rounded-2xl w-full max-w-3xl mx-4 mb-10" onClick={e => e.stopPropagation()} dir="rtl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white">{editingId ? "עריכת דרישה" : "דרישת רכש חדשה"}</h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-gray-300 mb-1">מספר דרישה *</label><input value={form.requestNumber} onChange={e => setForm({...form, requestNumber: e.target.value})} placeholder="REQ-001" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none" /></div>
                  <div><label className="block text-sm font-medium text-gray-300 mb-1">כותרת *</label><input value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="כותרת הדרישה" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none" /></div>
                  <div><label className="block text-sm font-medium text-gray-300 mb-1">שם מבקש</label><input value={form.requesterName} onChange={e => setForm({...form, requesterName: e.target.value})} placeholder="שם מלא" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none" /></div>
                  <div><label className="block text-sm font-medium text-gray-300 mb-1">מחלקה</label><select value={form.department} onChange={e => setForm({...form, department: e.target.value})} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-blue-500 focus:outline-none"><option value="">בחר מחלקה</option>{DEPARTMENTS.map(d => <option key={d} value={d}>{d}</option>)}</select></div>
                  <div><label className="block text-sm font-medium text-gray-300 mb-1">עדיפות</label><select value={form.priority} onChange={e => setForm({...form, priority: e.target.value})} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-blue-500 focus:outline-none">{PRIORITIES.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                  <div><label className="block text-sm font-medium text-gray-300 mb-1">סטטוס</label><select value={form.status} onChange={e => setForm({...form, status: e.target.value})} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-blue-500 focus:outline-none">{STATUSES.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                  <div><label className="block text-sm font-medium text-gray-300 mb-1">נדרש עד (תאריך אספקה)</label><input type="date" value={form.neededBy} onChange={e => setForm({...form, neededBy: e.target.value})} dir="ltr" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-blue-500 focus:outline-none" /></div>
                  <div><label className="block text-sm font-medium text-gray-300 mb-1">קוד תקציב</label><input value={form.budgetCode} onChange={e => setForm({...form, budgetCode: e.target.value})} placeholder="BUD-2026-XX" className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none" /></div>
                </div>
                ...
                {!editingId && (
                  <div className="border border-gray-700 rounded-xl p-4 space-y-3">
                    ...
                    {items.map((item, idx) => (
                      <div key={idx} className="bg-[#12141a] border border-gray-800 rounded-lg p-3 space-y-3">
                        <div className="flex items-center justify-between">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          <div>
                            ...
                            <select value={item.materialId} onChange={e => updateItem(idx, "materialId", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-blue-500 focus:outline-none">
                          ...
                          <div className="sm:col-span-2">
                            ...
                            <input value={item.itemDescription} onChange={e => updateItem(idx, "itemDescription", e.target.value)} placeholder="תיאור הפריט" className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm placeholder-gray-600 focus:border-blue-500 focus:outline-none" />
                          </div>
                          <div>
                            <label className="block text-xs text-muted-foreground mb-1">כמות</label>
                            <input type="number" value={item.quantity} onChange={e => updateItem(idx, "quantity", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-blue-500 focus:outline-none" dir="ltr" />
                          </div>
                          <div>
                            <label className="block text-xs text-muted-foreground mb-1">יחידה</label>
                            <select value={item.unit} onChange={e => updateItem(idx, "unit", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-blue-500 focus:outline-none">
                              {UNITS.map(u => <option key={u} value={u}>{u}</option>)}
                            </select>
                          </div>
                          <div>
                            <label className="block text-xs text-muted-foreground mb-1">מחיר משוער ₪</label>
                            <input type="number" step="0.01" value={item.estimatedPrice} onChange={e => updateItem(idx, "estimatedPrice", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-blue-500 focus:outline-none" dir="ltr" />
                          </div>
                          <div className="sm:col-span-3">
                            <label className="block text-xs text-muted-foreground mb-1">ספק מועדף</label>
                            <select value={item.preferredSupplierId} onChange={e => updateItem(idx, "preferredSupplierId", e.target.value)} className="w-full px-2 py-1.5 bg-[#1a1d23] border border-gray-700 rounded text-white text-sm focus:border-blue-500 focus:outline-none">
                        ...
                        <div className="text-left text-sm text-muted-foreground">
                          סה״כ שורה: <span className="text-white font-mono">₪{(parseFloat(item.quantity || "0") * parseFloat(item.estimatedPrice || "0")).toLocaleString()}</span>
                        </div>
                      </div>
                    ))}
                    {items.length > 0 && (
                      <div className="flex justify-end text-sm pt-1 border-t border-gray-700 mt-2 pt-3">
                        <span className="text-muted-foreground">סה״כ דרישה:</span>
                        <span className="text-white font-bold font-mono mr-2" dir="ltr">₪{itemsTotal.toLocaleString()}</span>
                      </div>
                    )}
                ...
                <div>
                  ...
                  <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} className="w-full px-3 py-2 bg-[#12141a] border border-gray-700 rounded-lg text-white focus:border-blue-500 focus:outline-none resize-none" rows={2} />
                </div>
                ...
                <div className="flex items-center gap-3 pt-2">
                  <button type="submit" disabled={createMut.isPending || updateMut.isPending} className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 text-white rounded-lg font-medium"><Save className="w-4 h-4" />{createMut.isPending || updateMut.isPending ? "שומר..." : editingId ? "עדכן" : "שמור"}</button>
        ...
        {deleteConfirm !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
            <div className="bg-[#1a1d23] border border-gray-700 rounded-xl p-6 max-w-sm mx-4" dir="rtl">
              <h3 className="text-lg font-bold text-white mb-2">מחיקת דרישה</h3>
