...
import { ClipboardList, Search, Plus, Edit2, Trash2, X, Save, CheckCircle2, Clock, AlertTriangle, ArrowUpDown, Factory, DollarSign, Play, Pause, CheckSquare, Loader2, Eye } from "lucide-react";
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => printPage("הוראות עבודה")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Printer size={16} /> הדפסה</button>
          <button onClick={() => sendByEmail("הוראות עבודה - טכנו-כל עוזי", generateEmailBody("הוראות עבודה", items, { order_number: "מספר", title: "כותרת", priority: "עדיפות", status: "סטטוס", due_date: "יעד" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Send size={16} /> שליחה</button>
          <button onClick={openCreate} className="flex items-center gap-2 bg-amber-600 text-white px-3 py-2 rounded-lg hover:bg-amber-700 shadow-lg text-sm"><Plus size={16} /> הוראה חדשה</button>
        </div>
      </div>
              ...
              <tr key={r.id} className={`border-b hover:bg-amber-50/30 ${isSelected(r.id) ? "bg-amber-50/50" : ""}`}>
                ...
                <td className="px-3 py-2">
                  <div className="flex gap-1 flex-wrap">
                    ...
                    <button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.title || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1 hover:bg-red-500/10 rounded text-red-500" title="מחיקה"><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={save} className="flex items-center gap-2 bg-amber-600 text-white px-6 py-2 rounded-lg hover:bg-amber-700"><Save size={16} /> {editing ? "עדכון" : "שמירה"}</button>
