    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      ...
      <div className="bg-card border border-border/50 rounded-2xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-white flex items-center gap-2"><Filter className="w-4 h-4 text-rose-400" /> תנאי סינון</h3>
        ...
        <div className="flex gap-3 items-center flex-wrap">
          ...
          {QUICK.map((q, i) => (
            <button key={i} onClick={() => { setFilters(q.filters); }} className="text-xs px-3 py-1.5 rounded-lg bg-muted text-muted-foreground border border-border hover:text-white">{q.label}</button>
          ))}
        </div>
      </div>
