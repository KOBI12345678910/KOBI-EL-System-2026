...
import { Mail, Search, Send, Paperclip, Star, Inbox, CheckCircle, Plus, RefreshCw, Filter, X, Database } from "lucide-react";
...
export default function EmailSyncPage() {
  ...
  const inboxCount = emails.filter(e => e.status === "incoming").length;
  const sentCount = emails.filter(e => e.status === "outgoing").length;
  ...
  const filtered = emails.filter(e => {
    ...
    if (activeTab === "inbox" && e.status !== "incoming") return false;
    if (activeTab === "sent" && e.status !== "outgoing") return false;
    ...
    <div className="p-6 space-y-4" dir="rtl">
      ...
      {integrationMissing && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg px-4 py-3 flex items-center gap-2 text-amber-400 text-sm">
          ...
          <span>
            תיבת הדואר אינה מחוברת. חבר חשבון <strong>Gmail</strong> או <strong>Outlook</strong> דרך תפריט <strong>אינטגרציות</strong> לצפייה בדוא"ל ולשליחה.
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="lg:col-span-3 space-y-3">
          ...
          <BulkActions items={filtered} selectedIds={selectedIds} onSelectionChange={setSelectedIds} actions={[
            defaultBulkActions.delete(async (ids) => { for (const id of ids) await authFetch("/api/email/" + id, { method: "DELETE", headers }); loadEmails(); }),
            defaultBulkActions.export(async (ids) => { const rows = filtered.filter(e => ids.has(e.id)); const csv = ["שולח,נושא,תאריך", ...rows.map(e => `${e.from},${e.subject},${e.date}`)].join("\n"); const b = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8" }); const a = document.createElement("a"); a.href = URL.createObjectURL(b); a.download = "emails.csv"; a.click(); }),
          ]} />
          <div className="border rounded-xl overflow-hidden">
            {loading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                {integrationMissing ? (
                  <>
                    <p className="text-sm font-medium">תיבת הדואר אינה מחוברת</p>
                    <p className="text-xs mt-1">חבר חשבון אימייל דרך אינטגרציות לצפייה בהודעות</p>
                  </>
                ) : (
              ...
              filtered.map(email => (
                <div key={email.id} onClick={() => setSelected(email === selected ? null : email)}
                  ...
                  {email.starred && <Star className="w-4 h-4 text-amber-400 fill-amber-400 flex-shrink-0" />}
                </div>
              ))
            )}
          </div>
            ...
            <div className="bg-card border rounded-xl p-5 space-y-4">
              ...
              <div className="flex border-b border-border/50 mt-4">
                {[{key:"details",label:"פרטים"},{key:"related",label:"רשומות קשורות"},{key:"docs",label:"מסמכים"},{key:"history",label:"היסטוריה"}].map(t => (
                  <button key={t.key} onClick={() => setDetailTab(t.key)} className={`px-4 py-2 text-xs font-medium border-b-2 ${detailTab === t.key ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-white"}`}>{t.label}</button>
                ))}
              </div>
              {detailTab === "details" && (
