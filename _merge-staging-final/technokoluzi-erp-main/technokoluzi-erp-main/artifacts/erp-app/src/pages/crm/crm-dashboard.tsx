        ...
        <div className="relative z-10">
          <div className="flex items-start justify-between">
            ...
            <div className="text-left">
              ...
              <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white text-xs px-3 py-1 rounded-full font-bold">ULTRA PRO</div>
            </div>
          </div>
      ...
      <div className="bg-card border rounded-xl p-5 space-y-4">
        ...
        <div className="flex border-b border-border/50">
          {[{key:"related",label:"רשומות קשורות"},{key:"docs",label:"מסמכים מצורפים"},{key:"history",label:"היסטוריית פעילות"}].map(t => (
            <button key={t.key} onClick={() => setDashDetailTab(t.key)} className={`px-4 py-2.5 text-sm font-medium border-b-2 ${dashDetailTab === t.key ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-white"}`}>{t.label}</button>
          ))}
        </div>
