                  ...
                  return (
                    <div key={btn.id} className={`flex items-center gap-3 px-4 py-3 hover:bg-muted/20 transition-colors group ${!btn.isActive ? "opacity-50" : ""}`}>
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={async () => { if (globalConfirm("למחוק את הכפתור?")) deleteMutation.mutate(btn.id); }} className="p-1 hover:bg-destructive/10 rounded"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="divide-y divide-border/50">
                {buttons.filter(b => !BUTTON_PLACEMENT_LABELS[b.placement]).map(btn => (
                  <div key={btn.id} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/20 transition-colors group">
                    ...
                    <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMutation.mutate(btn.id); }} className="p-1 hover:bg-destructive/10 rounded opacity-0 group-hover:opacity-100"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
                  </div>
                ))}
              </div>
            </div>
