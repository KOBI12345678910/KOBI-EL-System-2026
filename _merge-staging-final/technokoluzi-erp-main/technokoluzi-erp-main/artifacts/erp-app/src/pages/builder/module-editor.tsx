    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowPublishModal(true)} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-xl text-sm font-medium hover:bg-green-700 transition-colors">
              ...
              return (
                <motion.div key={entity.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-border/50">
                    ...
                    <button onClick={async () => { if (globalConfirm("לשכפל את הישות?")) cloneEntityMutation.mutate(entity.id); }} className="p-2 hover:bg-blue-500/10 rounded-lg transition-colors" title="שכפל ישות" disabled={cloneEntityMutation.isPending}><Copy className="w-4 h-4 text-blue-400" /></button>
                    <button onClick={async () => { if (globalConfirm("למחוק את הישות?")) deleteEntityMutation.mutate(entity.id); }}
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
        ...
        <div className="flex items-center gap-3 mt-6 pt-4 border-t border-border">
          <button onClick={() => {
              ...
              onSubmit(submitData);
            }} disabled={!form.name || !form.namePlural || !form.slug || isLoading}
