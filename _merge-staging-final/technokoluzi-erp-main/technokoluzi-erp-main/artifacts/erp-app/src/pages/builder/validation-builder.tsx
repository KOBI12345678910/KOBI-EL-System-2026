                  ...
                  return (
                    <div key={rule.id} className={`flex items-center gap-3 px-4 py-3 bg-card border rounded-xl ${rule.isActive ? "border-border" : "border-border/50 opacity-60"}`}>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMutation.mutate(rule.id); }} className="p-1.5 hover:bg-destructive/10 rounded-lg"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
                      </div>
                    </div>
                  );
                })}
