...
import { authFetch } from "@/lib/utils";
  ...
  const STATUSES = ["טיוטה", "בעבודה", "הושלם", "ארכיון"] as const;
  const SC: Record<string, string> = { "טיוטה": "bg-gray-500/20 text-gray-300", "בעבודה": "bg-blue-500/20 text-blue-300", "הושלם": "bg-green-500/20 text-green-300", "ארכיון": "bg-purple-500/20 text-purple-300" };
  const SEGMENTS = ["מתכת","אלומיניום","זכוכית","נדל\"ן","תעשייה","בנייה","ריהוט"];
...
export default function MarketAnalysis() {
  ...
  const [search, setSearch] = useState("");
    ...
    const filtered = useMemo(() => {
      let d = [...data];
      if (search) { const s = search.toLowerCase(); d = d.filter(r => r.title.includes(s)||r.id.toLowerCase().includes(s)||r.segment.includes(s)); }
      if (statusFilter !== "all") d = d.filter(r => r.status === statusFilter);
      if (segFilter !== "all") d = d.filter(r => r.segment === segFilter);
      return d;
    }, [search, statusFilter, segFilter]);
    ...
    const af = [statusFilter !== "all", segFilter !== "all"].filter(Boolean).length;
    ...
    return (
      ...
      {error && <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-red-400"><AlertCircle className="h-4 w-4 flex-shrink-0" /><span className="text-sm">{error}</span><button onClick={() => setError(null)} className="mr-auto"><X className="h-4 w-4" /></button></div>}
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingUp className="h-6 w-6 text-cyan-400" />ניתוח שוק</h1><p className="text-sm text-muted-foreground mt-1">מחקרי שוק, ניתוח מתחרים וזיהוי הזדמנויות</p></div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {[{"l":"מחקרים","v":"30","c":"text-cyan-400","t":"+5","u":true},{"l":"בעבודה","v":"8","c":"text-blue-400","t":"+3","u":true},{"l":"שוק כולל","v":"₪2.5B","c":"text-emerald-400","t":"+12%","u":true},{"l":"הזדמנויות","v":"45","c":"text-green-400","t":"+8","u":true},{"l":"מתחרים","v":"67","c":"text-orange-400","t":"+5","u":false},{"l":"איומים","v":"12","c":"text-red-400","t":"-2","u":true}].map((k: any, i: number) => (
        ...
        <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
          ...
          <select value={segFilter} onChange={e => { setSegFilter(e.target.value); setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הפלחים</option>{SEGMENTS.map((o: string) => <option key={o} value={o}>{o}</option>)}</select>
          {af > 0 && <Button variant="ghost" size="sm" onClick={() => { setStatusFilter("all"); setSegFilter("all"); setSearch(""); }} className="text-red-400 hover:text-red-300 gap-1"><X className="h-3 w-3" />נקה ({af})</Button>}
        ...
        <th className="p-3 text-right text-muted-foreground font-medium">פלח שוק</th>
        <th className="p-3 text-right text-muted-foreground font-medium">אנליסט</th>
        <th className="p-3 text-right text-muted-foreground font-medium">אזור</th>
        <th className="p-3 text-right text-muted-foreground font-medium">גודל שוק</th>
        <th className="p-3 text-right text-muted-foreground font-medium">צמיחה</th>
        <th className="p-3 text-right text-muted-foreground font-medium">מתחרים</th>
        <th className="p-3 text-right text-muted-foreground font-medium">הזדמנויות</th>
        <th className="p-3 text-right text-muted-foreground font-medium">סטטוס</th>
          ...
          {pd.map(row => (
            ...
            <td className="p-3 text-muted-foreground">{row.segment}</td>
            <td className="p-3 text-muted-foreground text-xs">{row.analyst}</td>
            <td className="p-3 text-muted-foreground text-xs">{row.region}</td>
            <td className="p-3 font-mono text-emerald-400">{row.marketSize}</td>
            <td className="p-3 "><span className={`font-mono text-xs ${row.growth.startsWith("-")?"text-red-400":"text-green-400"}`}>{row.growth}</span></td>
            <td className="p-3 text-center text-muted-foreground font-mono">{row.competitors}</td>
            <td className="p-3 text-center text-green-400 font-mono">{row.opportunities}</td>
        ...
        {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          ...
          <div className="p-4 space-y-6">
            <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי ניתוח</h3></div>
            <div className="grid grid-cols-3 gap-4">
            <div><Label className="text-muted-foreground text-xs">כותרת *</Label><Input type="text" placeholder="שם המחקר" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">פלח שוק *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>מתכת</option><option>אלומיניום</option><option>זכוכית</option><option>נדל"ן</option><option>תעשייה</option><option>בנייה</option><option>ריהוט</option></select></div>
            <div><Label className="text-muted-foreground text-xs">אנליסט *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option></select></div>
            <div><Label className="text-muted-foreground text-xs">אזור</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>מרכז</option><option>צפון</option><option>דרום</option><option>שרון</option><option>ירושלים</option><option>כל הארץ</option></select></div>
            <div><Label className="text-muted-foreground text-xs">גודל שוק</Label><Input type="text" placeholder="₪100M" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">שיעור צמיחה</Label><Input type="text" placeholder="5%" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">תאריך השלמה</Label><Input type="date" placeholder="" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>טיוטה</option><option>בעבודה</option><option>הושלם</option><option>ארכיון</option></select></div>
            </div>
          <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">ניתוח</h3></div>
            <div className="grid grid-cols-3 gap-4">
            <div className="col-span-3"><Label className="text-muted-foreground text-xs">סיכום</Label><textarea rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1 resize-none" placeholder="סיכום ממצאים..." /></div>
            <div className="col-span-3"><Label className="text-muted-foreground text-xs">הזדמנויות</Label><textarea rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1 resize-none" placeholder="הזדמנויות מרכזיות..." /></div>
            <div className="col-span-3"><Label className="text-muted-foreground text-xs">איומים</Label><textarea rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1 resize-none" placeholder="איומים..." /></div>
            <div className="col-span-3"><Label className="text-muted-foreground text-xs">המלצות</Label><textarea rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1 resize-none" placeholder="המלצות..." /></div>
        ...
        {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">פלח שוק</p><p className="text-white mt-1 font-medium">{dr.segment}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">אנליסט</p><p className="text-white mt-1 font-medium">{dr.analyst}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">אזור</p><p className="text-white mt-1 font-medium">{dr.region}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">גודל שוק</p><p className="text-white mt-1 font-medium">{dr.marketSize}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מתחרים</p><p className="text-white mt-1 font-medium">{String(dr.competitors)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">הזדמנויות</p><p className="text-white mt-1 font-medium">{String(dr.opportunities)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">איומים</p><p className="text-white mt-1 font-medium">{String(dr.threats)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">השלמה</p><p className="text-white mt-1 font-medium">{new Date(dr.completionDate).toLocaleDateString("he-IL")}</p></div>
