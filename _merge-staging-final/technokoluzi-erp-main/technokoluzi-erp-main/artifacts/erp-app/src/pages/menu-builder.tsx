  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-3xl font-bold text-white">בונה תפריט</h1>
          <p className="text-muted-foreground mt-1">התאמת הניווט בסרגל הצד — סדר, קבוצות, אייקונים ונראות לפי תפקיד</p>
        </div>
        <div className="flex items-center gap-2">
          <Button onClick={async () => { if (globalConfirm("ליצור פריטי תפריט אוטומטית מכל המודולים הפורסמים?")) autoGenerateMutation.mutate(); }} variant="outline" className="gap-2" disabled={autoGenerateMutation.isPending}>
                  ...
                  {items.map((item) => (
                    <div key={item.id} className={`flex items-center gap-3 px-4 py-3 transition-colors ${
                      ...
                      <div className="flex items-center gap-0.5">
                        ...
                        <button onClick={async () => { if (globalConfirm("למחוק פריט זה?")) deleteMutation.mutate(item.id); }} className="p-1 text-muted-foreground hover:text-destructive">
