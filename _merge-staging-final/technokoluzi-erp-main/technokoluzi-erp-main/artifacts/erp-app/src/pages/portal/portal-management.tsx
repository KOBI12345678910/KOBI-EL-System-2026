...
export default function PortalManagementPage() {
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-blue-400">טוען...</div>;

  return (
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white">ניהול פורטל חיצוני</h1>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition ${activeTab === tab.id ? "bg-blue-600 text-white" : "bg-slate-800/50 text-muted-foreground hover:text-white hover:bg-slate-700/50"}`}>
            {tab.label}
          </button>
        ))}
      </div>
          ...
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <input type="email" value={inviteForm.email} onChange={e => setInviteForm(p => ({ ...p, email: e.target.value }))}
                className="px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm" placeholder="אימייל" />
              ...
              <input type="text" value={inviteForm.linkedEntityId} onChange={e => setInviteForm(p => ({ ...p, linkedEntityId: e.target.value }))}
                className="px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm" placeholder="מזהה ישות (אופציונלי)" />
              <button onClick={createInvite} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition">שלח הזמנה</button>
            </div>
          </div>
          ...
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <input type="text" value={apiKeyForm.name} onChange={e => setApiKeyForm(p => ({ ...p, name: e.target.value }))}
                className="px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm" placeholder="שם המפתח" />
              ...
              <input type="text" value={apiKeyForm.ownerId} onChange={e => setApiKeyForm(p => ({ ...p, ownerId: e.target.value }))}
                className="px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm" placeholder="מזהה בעלים (אופציונלי)" />
              <button onClick={createApiKey} className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-medium transition">צור מפתח</button>
            </div>
          </div>
          ...
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <input type="text" value={webhookForm.name} onChange={e => setWebhookForm(p => ({ ...p, name: e.target.value }))}
                className="px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm" placeholder="שם" />
              <input type="url" value={webhookForm.url} onChange={e => setWebhookForm(p => ({ ...p, url: e.target.value }))}
                className="px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm" placeholder="URL" />
              <input type="text" value={webhookForm.events} onChange={e => setWebhookForm(p => ({ ...p, events: e.target.value }))}
                className="px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm" placeholder="אירועים (מופרד בפסיקים)" />
              <button onClick={createWebhook} className="px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded-lg text-sm font-medium transition">צור Webhook</button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <textarea value={sendForm.messageContent} onChange={e => setSendForm(p => ({ ...p, messageContent: e.target.value }))}
                className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white text-sm" rows={3} />
            </div>
          </div>
          <button onClick={sendDocument} className="mt-4 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition">
            שלח מסמך
          </button>
        </div>
      )}
