...
const statusConfig: Record<string, { label: string; color: string; next?: string[] }> = {
  draft: { label: "טיוטה", color: "bg-[#2a2a3e] text-muted-foreground", next: ["sent", "cancelled"] },
  ...
  cancelled: { label: "בוטל", color: "bg-[#2a2a3e] text-muted-foreground/60" },
};
...
export default function InvoicesPage() {
  ...
  const vatAmount = useMemo(() => Math.round(subtotal * (VAT_RATE / 100) * 100) / 100, [subtotal]);
  ...
  const kpis = [
    ...
    { label: "טיוטות", value: fmtInt(stats.drafts || items.filter(i => i.status === "draft").length), icon: Clock, color: "text-muted-foreground", bg: "from-[#2a2a3e]/50 to-[#2a2a3e]/20 border-[#2a2a3e]" },
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><FileText className="text-emerald-400" /> חשבוניות לקוחות</h1>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="outline" onClick={() => printPage("חשבוניות")} className="border-[#2a2a3e] text-muted-foreground gap-1"><Printer className="h-4 w-4" />הדפסה</Button>
          <Button variant="outline" onClick={() => sendByEmail("חשבוניות - טכנו-כל עוזי", generateEmailBody("חשבוניות", items, { invoice_number: "מספר", customer_name: "לקוח", total_amount: "סכום", status: "סטטוס" }))} className="border-[#2a2a3e] text-muted-foreground gap-1"><Send className="h-4 w-4" />שליחה</Button>
          <Button onClick={() => { window.print && window.print(); }} variant="outline" className="border-[#2a2a3e] text-purple-300 gap-1"><Download className="h-4 w-4" />PDF</Button>
      ...
      <div className="flex gap-2 border-b border-[#2a2a3e]">
        {([["invoices","חשבוניות", FileText], ["aging","דוח גיל חובות", Table]] as [string, string, any][]).map(([key, label, Icon]) => (
          <button key={key} onClick={() => setMainTab(key as any)} className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 ${mainTab === key ? "border-emerald-500 text-emerald-400" : "border-transparent text-muted-foreground hover:text-white"}`}>
        ...
        <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-white flex items-center gap-2"><Table className="w-4 h-4 text-amber-400" />דוח גיל חובות לפי לקוח</h3>
              <Button variant="outline" size="sm" className="border-[#2a2a3e] text-muted-foreground text-xs" onClick={loadAging}>רענן</Button>
            </div>
            {agingLoading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead><tr className="bg-[#0a0a1a]/50 border-b border-[#2a2a3e]">
                    ...
                    <th className="px-3 py-2.5 text-right text-white font-bold">סה״כ יתרה</th>
                  </tr></thead>
                  <tbody>
                    {agingData.map((row: any, i: number) => {
                      ...
                      return (
                        <tr key={i} className={`border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30 ${critical > 0 ? "bg-red-500/5" : ""}`}>
                          <td className="px-3 py-2 text-white font-medium">{row.customer_name}</td>
                          ...
                          <td className="px-3 py-2 font-mono font-bold text-white">₪{fmtInt(total)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                  <tfoot><tr className="bg-[#0a0a1a]/50 border-t-2 border-[#2a2a3e]">
                    <td className="px-3 py-2.5 text-white font-bold" colSpan={2}>סה״כ ({agingData.length} לקוחות)</td>
                    ...
                    <td className="px-3 py-2.5 font-mono text-white font-bold">₪{fmtInt(agingData.reduce((s: number, r: any) => s + Number(r.total_balance || 0), 0))}</td>
                  </tr></tfoot>
                </table>
              </div>
      ...
      {mainTab === "invoices" && items.length > 1 && (() => {
        ...
        return trendData.length > 1 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]">
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3 text-sm font-semibold text-white"><TrendingUp className="h-4 w-4 text-emerald-400" /> מגמת הכנסות חודשית</div>
            ...
            <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]">
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3 text-sm font-semibold text-white"><BarChart3 className="h-4 w-4 text-amber-400" /> מע״מ ויתרות</div>
                <ResponsiveContainer width="100%" height={200}>
      ...
      {mainTab === "invoices" && <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]"><Search className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={e => { setSearch(e.target.value); pagination.setPage(1); }} placeholder="חיפוש לפי מספר, לקוח, הזמנה..." className="pr-9 bg-[#0a0a1a] border-[#2a2a3e] text-white" /></div>
        <select value={filterStatus} onChange={e => { setFilterStatus(e.target.value); pagination.setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הסטטוסים</option>{Object.entries(statusConfig).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select>
        <select value={filterType} onChange={e => { setFilterType(e.target.value); pagination.setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הסוגים</option>{Object.entries(typeMap).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select>
      ...
      {mainTab === "invoices" && <BulkActions items={filtered} selectedIds={selectedIds} onSelectionChange={setSelectedIds} actions={[
        defaultBulkActions.delete(async (ids) => { for (const id of ids) await authFetch(`${API}/customer-invoices/${id}`, { method: "DELETE", headers }); load(); }),
        defaultBulkActions.export(async (ids) => { const selected = filtered.filter(i => ids.includes(String(i.id))); const csvRows = selected.map(i => [i.invoice_number, i.invoice_type, i.invoice_date?.slice(0, 10), i.customer_name, i.subtotal, i.total_amount, i.amount_paid, i.balance_due, i.status].join(",")); const csv = csvRows.join("\n"); const blob = new Blob([csv], { type: "text/csv;charset=utf-8" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "invoices.csv"; a.click(); }),
      ]} />}

      {mainTab === "invoices" && <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-0">
        <div className="overflow-x-auto relative">
          {tableLoading && <div className="absolute inset-0 bg-[#0a0a1a]/60 backdrop-blur-[1px] flex items-center justify-center z-10"><div className="flex items-center gap-2 bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg px-4 py-2 shadow-lg"><Loader2 className="w-4 h-4 animate-spin text-emerald-400" /><span className="text-sm text-white">טוען חשבוניות...</span></div></div>}
          <table className="w-full text-sm">
            <thead><tr className="border-b border-[#2a2a3e] bg-[#0a0a1a]/50">
              ...
              {[{ key: "invoice_number", label: "מספר" }, { key: "invoice_type", label: "סוג" }, { key: "invoice_date", label: "תאריך" }, { key: "customer_name", label: "לקוח" }, { key: "subtotal", label: "סכום" }, { key: "vat_amount", label: "מע״מ" }, { key: "total_amount", label: "סה״כ" }, { key: "balance_due", label: "יתרה" }, { key: "due_date", label: "לתשלום" }, { key: "status", label: "סטטוס" }].map(col => (
                <th key={col.key} className="px-3 py-3 text-right text-muted-foreground font-medium cursor-pointer hover:text-white" onClick={() => toggleSort(col.key)}><div className="flex items-center gap-1 text-xs">{col.label}<ArrowUpDown className="h-3 w-3" /></div></th>
              ))}
            ...
            <tbody>
              {!tableLoading && pagination.paginate(filtered).length === 0 ? (
                <tr><td colSpan={12} className="p-16 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-12 w-12 text-[#2a2a3e]" /> : <FileText className="h-12 w-12 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין חשבוניות"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "צור חשבונית ראשונה"}</p>{!(af > 0 || search) && <Button onClick={openCreate} className="bg-emerald-600 hover:bg-emerald-700 gap-2 mt-2"><Plus className="h-4 w-4" />חשבונית חדשה</Button>}</div></td></tr>
              ) : pagination.paginate(filtered).map(r => (
                <tr key={r.id} className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30 transition-colors cursor-pointer" onClick={() => setSelectedItem(r)}>
                  ...
                  <td className="px-3 py-2.5 text-white font-medium max-w-[150px] truncate">{r.customer_name}</td>
                  ...
                  <td className="px-3 py-2.5 font-mono text-xs font-bold text-white">₪{fmt(r.total_amount)}</td>
                  ...
                  <td className="px-3 py-2.5"><Badge className={`${statusConfig[r.status]?.color || "bg-[#2a2a3e]"} border-0 text-[10px]`}>{statusConfig[r.status]?.label || r.status}</Badge></td>
                  <td className="px-3 py-2.5 text-center" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-center gap-1">
                      <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-white" onClick={() => setSelectedItem(r)}><Eye className="h-3.5 w-3.5" /></Button>
                      ...
                      <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-red-400" onClick={async () => { if (await globalConfirm(`למחוק את חשבונית '${r.invoice_number}'? פעולה זו אינה ניתנת לביטול.`)) remove(r.id); }}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
            ...
            {filtered.length > 0 && (
              <tfoot><tr className="border-t-2 border-[#2a2a3e] bg-[#0a0a1a]/50 font-bold text-sm">
                <td className="px-3 py-3" /><td className="px-3 py-3 text-white" colSpan={4}>סה״כ ({filtered.length} חשבוניות)</td>
                <td className="px-3 py-3 font-mono text-xs text-white">₪{fmt(filtered.reduce((s, r) => s + Number(r.subtotal || 0), 0))}</td>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
                <div className="flex items-center gap-3">
                  <h2 className="text-lg font-bold text-white">{editing ? "עריכת חשבונית" : "חשבונית חדשה"}</h2>
              ...
              <div className="p-4 space-y-5">
                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי חשבונית</h3></div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div><Label className="text-muted-foreground text-xs">סוג מסמך *</Label><select value={form.invoiceType || "tax_invoice"} onChange={e => setForm({ ...form, invoiceType: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1">{Object.entries(typeMap).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></div>
                  <div><Label className="text-muted-foreground text-xs">מספר חשבונית</Label><Input value={form.invoiceNumber || ""} readOnly className="bg-[#0a0a1a] border-[#2a2a3e] text-emerald-400 font-mono mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">תאריך *</Label><Input type="date" value={form.invoiceDate || ""} onChange={e => setForm({ ...form, invoiceDate: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select value={form.status || "draft"} onChange={e => setForm({ ...form, status: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1">{Object.entries(statusConfig).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select></div>
                </div>

                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי לקוח</h3></div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="col-span-2 relative">
                    <Label className="text-muted-foreground text-xs">לקוח *</Label>
                    <Input value={customerSearch} onChange={e => { setCustomerSearch(e.target.value); setForm({ ...form, customerName: e.target.value }); setShowCustomerDropdown(true); }} onFocus={() => setShowCustomerDropdown(true)} placeholder="הקלד שם לקוח..." className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" />
                    {showCustomerDropdown && filteredCustomers.length > 0 && (
                      <div className="absolute z-20 top-full mt-1 w-full bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg shadow-xl max-h-48 overflow-y-auto">
                        {filteredCustomers.map((c, i) => (
                          <button key={i} onClick={() => { setForm({ ...form, customerName: c }); setCustomerSearch(c); setShowCustomerDropdown(false); }} className="w-full px-3 py-2 text-right text-sm text-white hover:bg-[#2a2a3e] transition-colors">{c}</button>
                        ))}
                      </div>
                    )}
                  </div>
                  <div><Label className="text-muted-foreground text-xs">ח.פ / ע.מ</Label><Input value={form.customerTaxId || ""} onChange={e => setForm({ ...form, customerTaxId: e.target.value })} placeholder="515123456" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">מס׳ הזמנה (PO)</Label><Input value={form.poNumber || ""} onChange={e => setForm({ ...form, poNumber: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                </div>

                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">תנאי תשלום</h3></div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div><Label className="text-muted-foreground text-xs">תנאי תשלום</Label><select value={form.paymentTerms || "net_30"} onChange={e => setForm({ ...form, paymentTerms: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1">{paymentTermsOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
                  <div><Label className="text-muted-foreground text-xs">לתשלום עד</Label><Input type="date" value={form.dueDate || ""} onChange={e => setForm({ ...form, dueDate: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">אמצעי תשלום</Label><select value={form.paymentMethod || ""} onChange={e => setForm({ ...form, paymentMethod: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option value="">בחר...</option><option value="bank_transfer">העברה בנקאית</option><option value="check">שיק</option><option value="cash">מזומן</option><option value="credit_card">כרטיס אשראי</option><option value="bit">ביט</option></select></div>
                  <div><Label className="text-muted-foreground text-xs">איש מכירות</Label><Input value={form.salesperson || ""} onChange={e => setForm({ ...form, salesperson: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                </div>

                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">שורות פריטים</h3></div>
                <div className="bg-[#0a0a1a] rounded-lg border border-[#2a2a3e] overflow-hidden">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-[#2a2a3e] bg-[#1a1a2e]/50">
                    ...
                    <tbody>
                      {lineItems.map((li, idx) => (
                        <tr key={li.id} className="border-b border-[#2a2a3e]/50">
                          <td className="px-3 py-2 text-muted-foreground text-xs">{idx + 1}</td>
                          <td className="px-3 py-1"><Input value={li.description} onChange={e => updateLineItem(li.id, "description", e.target.value)} placeholder="תיאור פריט..." className="bg-transparent border-0 text-white h-8 px-1 text-sm" /></td>
                          <td className="px-3 py-1"><Input type="number" min={0} step={1} value={li.quantity || ""} onChange={e => updateLineItem(li.id, "quantity", Number(e.target.value) || 0)} className="bg-transparent border-0 text-white h-8 px-1 text-sm font-mono text-center" /></td>
                          <td className="px-3 py-1"><Input type="number" min={0} step={0.01} value={li.unitPrice || ""} onChange={e => updateLineItem(li.id, "unitPrice", Number(e.target.value) || 0)} className="bg-transparent border-0 text-white h-8 px-1 text-sm font-mono text-center" /></td>
                          <td className="px-3 py-1"><Input type="number" min={0} max={100} step={0.5} value={li.discountPct || ""} onChange={e => updateLineItem(li.id, "discountPct", Number(e.target.value) || 0)} className="bg-transparent border-0 text-white h-8 px-1 text-sm font-mono text-center" /></td>
                ...
                <div className="flex justify-end">
                  <div className="w-full max-w-sm bg-[#0a0a1a] rounded-lg border border-[#2a2a3e] p-4 space-y-3">
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">סכום ביניים</span><span className="text-white font-mono">₪{fmt(subtotal)}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">מע״מ ({VAT_RATE}%)</span><span className="text-amber-400 font-mono">₪{fmt(vatAmount)}</span></div>
                    <hr className="border-[#2a2a3e]" />
                    <div className="flex justify-between text-lg font-bold"><span className="text-white">סה״כ לתשלום</span><span className="text-emerald-400 font-mono">₪{fmt(totalAmount)}</span></div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><Label className="text-muted-foreground text-xs">שולם (₪)</Label><Input type="number" step={0.01} value={form.amountPaid || ""} onChange={e => setForm({ ...form, amountPaid: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                </div>

                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">הערות</h3></div>
                <textarea value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white resize-none" placeholder="הערות לחשבונית..." />
              </div>

              <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-end">
                <Button variant="outline" onClick={() => setShowForm(false)} className="border-[#2a2a3e]">ביטול</Button>
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
              <div className="flex items-center gap-3">
                <h2 className="text-lg font-bold text-white">{selectedItem.invoice_number}</h2>
                <Badge className={`${statusConfig[selectedItem.status]?.color || "bg-[#2a2a3e]"} border-0`}>{statusConfig[selectedItem.status]?.label || selectedItem.status}</Badge>
              </div>
            ...
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={i} className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">{d.l}</p><p className="text-white mt-1 font-medium text-sm">{d.v}</p></div>
                ))}
              </div>
              ...
              <div className="flex justify-end">
                <div className="w-full max-w-xs bg-[#0a0a1a] rounded-lg border border-[#2a2a3e] p-4 space-y-2">
                  <div className="flex justify-between text-sm"><span className="text-muted-foreground">סכום ביניים</span><span className="text-white font-mono">₪{fmt(selectedItem.subtotal)}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-muted-foreground">מע״מ ({selectedItem.vat_rate || VAT_RATE}%)</span><span className="text-amber-400 font-mono">₪{fmt(selectedItem.vat_amount)}</span></div>
                  <hr className="border-[#2a2a3e]" />
                  <div className="flex justify-between font-bold"><span className="text-white">סה״כ</span><span className="text-emerald-400 font-mono">₪{fmt(selectedItem.total_amount)}</span></div>
              ...
              {selectedItem.notes && <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground mb-1">הערות</p><p className="text-sm text-white">{selectedItem.notes}</p></div>}
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {statusConfig[selectedItem.status]?.next?.map(nextStatus => (
                      <Button key={nextStatus} variant="outline" size="sm" className={`border-[#2a2a3e] gap-1 ${nextStatus === "paid" ? "text-emerald-400 border-emerald-500/30" : nextStatus === "cancelled" ? "text-red-400 border-red-500/30" : "text-blue-400 border-blue-500/30"}`} onClick={async () => {
            ...
            <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-end">
              <Button variant="outline" className="border-[#2a2a3e] gap-1" onClick={() => printPage(`חשבונית ${selectedItem.invoice_number}`)}><Printer className="h-4 w-4" />הדפסה</Button>
