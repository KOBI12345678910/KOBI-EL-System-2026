    ...
    <div className="space-y-4 sm:space-y-6" dir="rtl">
            ...
            {showExportMenu && (
              <div className="absolute left-0 top-full mt-1 bg-slate-800 border border-slate-600 rounded-lg shadow-xl z-50 min-w-0 sm:min-w-[200px]">
                <button onClick={() => handleExport("hashbeshbet")}
                  className="w-full text-right px-4 py-3 text-sm text-white hover:bg-slate-700 flex items-center gap-2 rounded-t-lg">
                  <FileSpreadsheet className="w-4 h-4 text-green-400" />
                  יצוא לחשבשבת
                </button>
                <button onClick={() => handleExport("unified")}
                  className="w-full text-right px-4 py-3 text-sm text-white hover:bg-slate-700 flex items-center gap-2 rounded-b-lg border-t border-slate-700">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowDownloadDialog(false)}>
          <div className="bg-slate-800 border border-slate-600 rounded-xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-slate-700">
              ...
              <button onClick={() => setShowDownloadDialog(false)} className="text-muted-foreground hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5 space-y-3">
              ...
              <button onClick={() => handleDownload("xlsx")}
                ...
                <div className="text-right">
                  <div className="text-white font-medium">XLSX (אקסל)</div>
              ...
              <button onClick={() => handleDownload("zip")}
                ...
                <div className="text-right">
                  <div className="text-white font-medium">ZIP (קבצי הוצאות)</div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="border-b border-slate-800/50">
                  <td className="p-3 text-white font-medium sticky right-0 bg-slate-900">הכנסות חייבות כולל מע"מ</td>
                  {monthsToShow.map(m => {
                    const val = Number(incomeData[m]?.taxable_with_vat) || 0;
                    return <td key={m} className="p-3 text-center text-white">{fmt(val)}</td>;
                  })}
                ...
                <tr className="border-b border-slate-800/50 bg-slate-800/20">
                  <td className="p-3 text-muted-foreground sticky right-0 bg-slate-900">18% מע"מ</td>
                  {monthsToShow.map(m => {
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="border-b border-slate-800/50">
                  <td className="p-3 text-white font-medium sticky right-0 bg-slate-900">הוצאות כולל מע"מ</td>
                  {monthsToShow.map(m => {
                ...
                <tr className="border-b border-slate-800/50 bg-slate-800/20">
                  <td className="p-3 text-muted-foreground sticky right-0 bg-slate-900">18% מע"מ תשומות</td>
                  {monthsToShow.map(m => {
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="border-b border-slate-800/50">
                  <td className="p-3 text-white font-medium sticky right-0 bg-slate-900">כמות קבצים</td>
                  {monthsToShow.map(m => {
      ...
      <Card className="bg-gradient-to-r from-green-900/20 to-red-900/20 border-slate-700/50">
        <CardContent className="p-5">
          <h3 className="text-base font-bold text-white mb-4">סיכום תקופה</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
        <Card className="bg-card border-border">
          <CardHeader><CardTitle className="text-sm text-white">רשומות קשורות</CardTitle></CardHeader>
          <CardContent><RelatedRecords entityType="income-expenses-report" entityId="dashboard" /></CardContent>
        </Card>
        <Card className="bg-card border-border">
          <CardHeader><CardTitle className="text-sm text-white">היסטוריה</CardTitle></CardHeader>
