      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {kpis.map((k, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="bg-[#111827] border border-white/10 rounded-xl p-4 flex items-center gap-4">
        ...
        <div className="flex gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש..." className="w-full pr-10 pl-4 py-2 bg-[#111827] border border-white/10 rounded-lg text-white" />
          </div>
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="px-3 py-2 bg-[#111827] border border-white/10 rounded-lg text-white">
        ...
        {selectedItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setSelectedItem(null)}>
            <div className="bg-[#1a1f35] border border-white/10 rounded-xl w-full max-w-2xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2 border-b border-white/10 pb-2">
                {["details", "history"].map(tab => (
                  <button key={tab} onClick={() => setDetailTab(tab)}
                    className={`px-3 py-1 rounded-lg text-sm ${detailTab === tab ? "bg-blue-600 text-white" : "text-muted-foreground hover:bg-card/5"}`}>
        ...
        {showForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
            <div className="bg-[#1a1f35] border border-white/10 rounded-xl w-full max-w-lg p-6 space-y-4">
              <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-sm text-muted-foreground">שם לקוח</label><input value={form.customer_name || ""} onChange={e => setForm({ ...form, customer_name: e.target.value })} className="w-full mt-1 px-3 py-2 bg-[#111827] border border-white/10 rounded-lg text-white" /></div>
                  <div><label className="text-sm text-muted-foreground">שם ספק</label><input value={form.supplier_name || ""} onChange={e => setForm({ ...form, supplier_name: e.target.value })} className="w-full mt-1 px-3 py-2 bg-[#111827] border border-white/10 rounded-lg text-white" /></div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-sm text-muted-foreground">סכום</label><input type="number" value={form.amount || 0} onChange={e => setForm({ ...form, amount: Number(e.target.value) })} className="w-full mt-1 px-3 py-2 bg-[#111827] border border-white/10 rounded-lg text-white" /></div>
                  <div><label className="text-sm text-muted-foreground">חשבונית מקור</label><input value={form.reference_invoice || ""} onChange={e => setForm({ ...form, reference_invoice: e.target.value })} className="w-full mt-1 px-3 py-2 bg-[#111827] border border-white/10 rounded-lg text-white" /></div>
                </div>
                <div><label className="text-sm text-muted-foreground">סיבה</label><textarea value={form.reason || ""} onChange={e => setForm({ ...form, reason: e.target.value })} className="w-full mt-1 px-3 py-2 bg-[#111827] border border-white/10 rounded-lg text-white" rows={2} /></div>
                <div><label className="text-sm text-muted-foreground">הערות</label><textarea value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} className="w-full mt-1 px-3 py-2 bg-[#111827] border border-white/10 rounded-lg text-white" rows={2} /></div>
              </div>
