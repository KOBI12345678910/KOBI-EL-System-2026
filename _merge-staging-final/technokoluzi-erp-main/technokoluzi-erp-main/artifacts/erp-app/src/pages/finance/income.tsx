      ...
      <div className="bg-card border border-border text-foreground rounded-xl shadow-2xl w-full max-w-4xl relative" dir="rtl">
        <div className="flex items-center justify-between p-4 border-b bg-muted/30 rounded-t-xl">
          <div className="flex items-center gap-3">
            ...
            <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white relative" onClick={() => setShowShareMenu(!showShareMenu)}>
        ...
        <div className="flex items-center gap-4 p-4 border-b">
          <button className="px-6 py-2 bg-blue-900 text-white rounded-lg font-medium">דיכוי (ביטול/תיקון) המסמך</button>
  ...
  return (
    <div className="space-y-4 sm:space-y-6" dir="rtl">
      ...
      <div className="flex gap-3 items-center flex-wrap">
        ...
        <div className="text-sm text-muted-foreground">
          הצגת פילטרים | סה"כ: <span className="text-white font-medium">{fmt(totalAmount)}</span> ({items.length})
        </div>
      </div>

      <BulkActions items={items} selectedIds={selectedIds} onSelectionChange={setSelectedIds} actions={[
        defaultBulkActions.delete(async (ids) => { for (const id of ids) await authFetch(`${API}/finance/income/${id}`, { method: "DELETE" }); qc.invalidateQueries({ queryKey: ["income-documents"] }); }),
                ...
                {items.map((item: any) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 cursor-pointer" onClick={() => setViewItem(item)}>
                      ...
                      <td className="p-3 text-white font-medium">{docLabel} / {item.document_number || item.id}</td>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">מסמך הכנסה #{selectedItem.document_number || selectedItem.id}</h2>
            ...
            <div className="p-6">
              {detailTab === "details" && (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  ...
                  <div><div className="text-xs text-muted-foreground mb-1">תאריך</div><div className="text-sm text-white">{fmtDate(selectedItem.invoice_date)}</div></div>
                  <div><div className="text-xs text-muted-foreground mb-1">סוג מסמך</div><div className="text-sm text-white">{selectedItem.document_type || "-"}</div></div>
                  ...
                  <div><div className="text-xs text-muted-foreground mb-1">סטטוס</div><div className="text-sm text-white">{statusLabels[selectedItem.status] || selectedItem.status}</div></div>
                </div>
              )}
