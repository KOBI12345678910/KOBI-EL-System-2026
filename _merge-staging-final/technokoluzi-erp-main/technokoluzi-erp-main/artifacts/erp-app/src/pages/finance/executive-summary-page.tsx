    ...
    <div className="bg-slate-800 border border-slate-600 rounded-lg shadow-xl p-3 text-sm" dir="rtl">
      ...
      {payload.map((p: any, i: number) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <span className="text-white font-bold">{fmtC(p.value)}</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          ...
          <div className="flex gap-1 border rounded-lg overflow-hidden">
            {[["month", "חודש"], ["quarter", "רבעון"], ["year", "שנה"]].map(([k, label]) => (
              <button key={k} onClick={() => setPeriod(k)}
                className={`px-3 py-2 text-sm transition-all ${period === k ? "bg-violet-600 text-white" : "hover:bg-muted/30"}`}>
                {label}
              </button>
            ))}
          </div>
          ...
          <button onClick={() => printPage("דוחות מנהלים")}
            className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
