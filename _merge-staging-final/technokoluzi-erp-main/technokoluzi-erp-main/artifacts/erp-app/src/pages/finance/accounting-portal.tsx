    ...
    <div className="border border-slate-700/40 rounded-lg overflow-hidden">
      <table className="w-full text-xs">
        ...
        <tfoot><tr className="bg-slate-800/40 border-t border-slate-600">
          <td colSpan={3} className="p-2 text-white font-bold flex items-center gap-2">
            {'סה"כ'}
            {isBalanced ? (
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-white flex items-center gap-2"><BookOpen className="w-4 h-4 text-emerald-400" />יומן — פקודות יומן (הנה״ח כפולה)</h3>
        <div className="flex items-center gap-2">
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-2">
          <KPICard label={'סה"כ פקודות'} value={String(stats.total || 0)} icon={BookOpen} color="text-white" />
      ...
      <div className="flex gap-2">
        ...
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-slate-800 border border-slate-700 rounded px-3 py-1.5 text-sm text-white">
              ...
              <table className="w-full text-xs">
                  ...
                  {filtered.slice(0, 100).map((e: any) => {
                    ...
                    return [
                      <tr key={e.id} className={`border-b border-slate-800/40 hover:bg-slate-800/20 cursor-pointer ${isExpanded ? "bg-slate-800/30" : ""}`}
                        ...
                        <td className="p-2 text-white truncate max-w-[200px]">{e.description}</td>
                        <td className="p-2">
                        ...
                        <td className="p-2 text-white font-mono font-bold text-left" dir="ltr">{fmtNum(amt)}</td>
                ...
                <tfoot>
                  <tr className="bg-slate-800/60 border-t-2 border-slate-600">
                    <td colSpan={5} className="p-2.5 text-white font-bold">{'סה"כ'} ({filtered.length} פקודות)</td>
                    ...
                    <td className="p-2.5 text-white font-mono font-bold text-left" dir="ltr">{fmtNum(totalDebit)}</td>
                    <td colSpan={3}>
      ...
      {selectedItem && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">פקודת יומן {selectedItem.entry_number || `#${selectedItem.id}`}</h2>
              ...
              {detailTab === "details" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <div><div className="text-xs text-muted-foreground mb-1">תאריך</div><div className="text-sm text-white">{fmtDate(selectedItem.entry_date)}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">תיאור</div><div className="text-sm text-white">{selectedItem.description}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">סכום</div><div className="text-sm text-white font-bold">{fmt(Number(selectedItem.amount || 0))}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">אסמכתא</div><div className="text-sm text-white">{selectedItem.reference || "-"}</div></div>
                  </div>
                </div>
              )}
...
function LedgerTab() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-white flex items-center gap-2"><FileSpreadsheet className="w-4 h-4 text-blue-400" />ספר ראשי — כרטסת חשבון</h3>
        <Button variant="outline" size="sm" className="border-slate-600" onClick={() => refetch()}><RefreshCw className="w-3.5 h-3.5" /></Button>
      </div>

      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-2">
          <KPICard label={'סה"כ תנועות'} value={String(stats.total || 0)} icon={FileSpreadsheet} color="text-white" />
      ...
      <div className="flex gap-1 bg-slate-800/50 rounded-lg p-0.5">
        {viewTabs.map(t => (
          <button key={t.id} onClick={() => setViewMode(t.id)}
            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded text-xs font-medium transition-colors ${viewMode === t.id ? "bg-blue-600 text-white" : "text-muted-foreground hover:text-white"}`}>
        ...
        <>
          <div className="flex gap-2 flex-wrap">
            <select value={selectedAccount} onChange={e => setSelectedAccount(e.target.value)}
              className="flex-1 min-w-[200px] bg-slate-800 border border-slate-700 rounded px-3 py-1.5 text-sm text-white">
          ...
          {selectedAccount && (
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-2 flex items-center gap-2">
              ...
              <span className="text-white text-sm font-mono">{selectedAccount}</span>
              <span className="text-muted-foreground text-xs">— {(accounts || []).find((a: any) => a.account_number === selectedAccount)?.account_name || ""}</span>
              <button onClick={() => setSelectedAccount("")} className="mr-auto text-muted-foreground hover:text-white"><X className="w-3.5 h-3.5" /></button>
            </div>
          )}
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                        ...
                        return filtered.slice(0, 200).map((e: any, i: number) => {
                          ...
                          return (
                            <tr key={e.id || i} className="border-b border-slate-800/40 hover:bg-slate-800/20">
                              ...
                              <td className="p-2 text-white">{e.account_name}</td>
                    ...
                    <tfoot><tr className="bg-slate-800/50 border-t-2 border-slate-600">
                      <td colSpan={6} className="p-2 text-white font-bold">{'סה"כ'} ({filtered.length} תנועות)</td>
            ...
            {!accountSummary || accountSummary.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                    ...
                    {accountSummary.map((a: any, i: number) => {
                      ...
                      return (
                        <tr key={a.account_number || i} className="border-b border-slate-800/40 hover:bg-slate-800/20 cursor-pointer"
                          ...
                          <td className="p-2 text-white font-medium">{a.account_name}</td>
                  ...
                  <tfoot><tr className="bg-slate-800/50 border-t-2 border-slate-600">
                    <td colSpan={4} className="p-2 text-white font-bold">{'סה"כ'} ({accountSummary.length} חשבונות)</td>
                    ...
                    <td className="p-2 text-white font-mono font-bold text-left" dir="ltr">{fmtNum(accountSummary.reduce((s: number, a: any) => s + Number(a.balance || 0), 0))}</td>
                    <td colSpan={2}></td>
                  </tr></tfoot>
                </table>
              </div>
                    ...
                    {periodSummary.map((p: any, i: number) => {
                      ...
                      return (
                        <tr key={i} className="border-b border-slate-800/40 hover:bg-slate-800/20">
                          <td className="p-2.5 text-white font-mono">{p.fiscal_year}</td>
...
function ChartOfAccountsTab() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-white flex items-center gap-2"><List className="w-4 h-4 text-cyan-400" />ספר חשבונות — מפתח חשבונות</h3>
        <div className="flex items-center gap-2">
        ...
        <Card className="bg-slate-800/50 border-emerald-500/30">
          ...
          <CardContent className="p-4 grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div><Label className="text-xs text-muted-foreground">סוג</Label>
              <select value={addForm.account_type} onChange={e => setAddForm({ ...addForm, account_type: e.target.value })} className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1.5 text-sm mt-1 text-white">
                {Object.entries(typeLabels).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
              </select>
            </div>
            <div><Label className="text-xs text-muted-foreground">צד רגיל</Label>
              <select value={addForm.normal_balance} onChange={e => setAddForm({ ...addForm, normal_balance: e.target.value })} className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1.5 text-sm mt-1 text-white">
          ...
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                        ...
                        return (
                          <tr key={a.id} className={`border-b border-slate-800/30 hover:bg-slate-800/20 ${isGroup ? "bg-slate-800/10" : ""}`}>
                            <td className="p-2 font-mono">
                              <span className={`${isGroup ? "text-white font-bold" : "text-cyan-400"}`}>
                                {a.account_number}
                              </span>
                            </td>
                            <td className="p-2">
                              <div style={{ paddingRight: `${indent * 16}px` }} className="flex items-center gap-1.5">
                                ...
                                <span className={isGroup ? "text-white font-semibold" : "text-slate-200"}>{a.account_name}</span>
                ...
                <tfoot>
                  <tr className="bg-slate-800/60 border-t-2 border-slate-600">
                    <td colSpan={6} className="p-2.5 text-white font-bold">{'סה"כ כל החשבונות'}</td>
    ...
    <Card className="bg-slate-900/50 border-slate-700/50">
      ...
      <CardContent className="p-0">
        <table className="w-full text-xs">
          ...
          <tfoot><tr className="bg-slate-800/50 border-t-2 border-slate-600">
            <td colSpan={3} className="p-2 text-white font-bold">{'סה"כ'} {title}</td>
...
function BalanceSheetTab() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-white flex items-center gap-2"><Scale className="w-4 h-4 text-blue-400" />מאזן — דו״ח מצב כספי</h3>
        <div className="flex items-center gap-2">
        ...
        <>
          ...
          <div className={`rounded-lg p-3 flex items-center justify-between ${isBalanced ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-yellow-500/10 border border-yellow-500/20"}`}>
            ...
            <div className="text-left" dir="ltr">
              <p className="text-lg font-mono font-bold text-white">{fmt(summary?.totalAssets || 0)}</p>
...
function ProfitLossTab() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-white flex items-center gap-2"><BarChart3 className="w-4 h-4 text-emerald-400" />דוח רווח והפסד</h3>
        <div className="flex items-center gap-2">
          <select value={fiscalYear} onChange={e => setFiscalYear(Number(e.target.value))}
            className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-white">
            {[currentYear, currentYear - 1, currentYear - 2].map(y => (
              ...
              <table className="w-full text-sm">
                <tbody>
                  <tr className="border-b border-slate-700/30">
                    <td className="py-2 text-white font-medium">{'סה"כ הכנסות'}</td>
                    <td className="py-2 text-emerald-400 font-mono font-bold text-left" dir="ltr">{fmt(totalRevenue)}</td>
                  </tr>
                  <tr className="border-b border-slate-700/30">
                    <td className="py-2 text-white font-medium">{'סה"כ הוצאות'}</td>
                  ...
                  <tr className="bg-slate-800/40">
                    <td className="py-2 text-white font-bold">{isProfit ? "רווח נקי" : "הפסד נקי"}</td>
...
function TrialBalanceTab() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-white flex items-center gap-2"><Scale className="w-4 h-4 text-amber-400" />מאזן בוחן</h3>
        <div className="flex items-center gap-2">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className={`rounded-lg p-3 flex items-center justify-between ${isBalanced ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-yellow-500/10 border border-yellow-500/20"}`}>
            ...
            <div className="text-left" dir="ltr">
              <p className="text-lg font-mono font-bold text-white">{fmt(totalDebit)}</p>
              <p className="text-[10px] text-muted-foreground">שנת {fiscalYear}</p>
            </div>
          </div>
          ...
          <Card className="bg-slate-900/50 border-slate-700/50">
            <CardContent className="p-0 overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tfoot><tr className="bg-slate-800/50 border-t-2 border-slate-600">
                  <td colSpan={4} className="p-2 text-white font-bold">{'סה"כ מאזן בוחן'}</td>
                  <td className="p-2 text-emerald-400 font-mono font-bold text-left" dir="ltr">{fmtNum(totalDebit)}</td>
                  <td className="p-2 text-red-400 font-mono font-bold text-left" dir="ltr">{fmtNum(totalCredit)}</td>
...
function ReportsTab() {
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-medium text-white flex items-center gap-2"><BarChart3 className="w-4 h-4 text-emerald-400" />דוחות כספיים</h3>
...
function VATTab() {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-medium text-white flex items-center gap-2"><Percent className="w-4 h-4 text-orange-400" />ניהול מע״מ ומיסים</h3>
...
function BankTab() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-white flex items-center gap-2"><Landmark className="w-4 h-4 text-blue-400" />חשבונות בנק</h3>
        <div className="flex gap-2">
      ...
      <div className="grid grid-cols-3 gap-3">
        <KPICard label="חשבונות" value={String((bankAccounts || []).length)} icon={Hash} color="text-white" />
              ...
              <tbody>
                {(bankAccounts || []).map((a: any) => (
                  <tr key={a.id} className="border-b border-slate-800/40 hover:bg-slate-800/20">
                    <td className="p-2.5 text-white font-medium">{a.bank_name}</td>
...
function SettingsTab() {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-medium text-white flex items-center gap-2"><Settings className="w-4 h-4 text-muted-foreground" />הגדרות הנהלת חשבונות</h3>
        ...
        <Card className="bg-slate-900/50 border-slate-700/50">
          ...
          <CardContent className="grid gap-3">
            ...
            <div>
              ...
              <select value={settings.fiscalYear} onChange={e => setSettings({ ...settings, fiscalYear: e.target.value })} className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm mt-1 text-white">
        ...
        {TABS.map(tab => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-md text-xs transition-colors whitespace-nowrap ${
              activeTab === tab.id ? "bg-slate-700 text-white font-medium" : "text-muted-foreground hover:text-white hover:bg-slate-700/50"
            }`}
          >
