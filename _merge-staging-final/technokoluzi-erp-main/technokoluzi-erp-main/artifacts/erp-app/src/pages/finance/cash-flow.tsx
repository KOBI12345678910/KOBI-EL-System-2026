...
function ForecastChart({ monthlyData, forecastData }: { monthlyData: any[]; forecastData: any[] }) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-[#1a1a2e]/80 border border-[#2a2a3e] rounded-xl p-4">
      ...
      <div className="bg-[#1a1a2e]/80 border border-[#2a2a3e] rounded-xl overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-[#0a0a1a]/50 border-b border-[#2a2a3e]"><tr>
            ...
            {forecastData.length === 0 ? (
              ...
              return (
                <tr key={i} className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/20">
                  <td className="px-3 py-2 font-medium text-white">{monthNames[month] || month} {year}</td>
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-2">
                      <div className="w-16 bg-[#2a2a3e] rounded-full h-2">
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-2"><DollarSign className="text-emerald-400" /> תזרים מזומנים</h1>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => printPage("תזרים מזומנים")} className="flex items-center gap-1.5 bg-[#2a2a3e] text-white px-3 py-2 rounded-lg hover:bg-[#3a3a4e] text-sm border border-[#3a3a4e]"><Printer size={16} /> הדפסה</button>
          <button onClick={openCreate} className="flex items-center gap-2 bg-emerald-600 text-white px-3 py-2 rounded-lg hover:bg-emerald-700 shadow-lg text-sm"><Plus size={16} /> רשומה חדשה</button>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
        {kpis.map((kpi, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-[#1a1a2e]/80 border border-[#2a2a3e] rounded-xl p-3">
      ...
      {records.length > 1 && (() => {
        ...
        return (
          <div className="bg-[#1a1a2e]/80 border border-[#2a2a3e] rounded-xl p-4">
            <div className="text-sm font-bold text-white mb-3 flex items-center gap-2">📊 השוואת תקופות</div>
      ...
      <div className="bg-[#1a1a2e]/80 border border-[#2a2a3e] rounded-xl p-4">
        <div className="text-sm font-bold text-white mb-2">תזרים נטו החודש</div>
        <div className="flex items-center gap-4">
          <div className="flex-1">
            ...
            <div className="h-4 bg-[#2a2a3e] rounded-full overflow-hidden flex">
      ...
      <div className="flex gap-2 border-b border-[#2a2a3e]">
        {([["records", "רשומות"], ["monthly", "תצוגה חודשית"], ["categories", "לפי קטגוריה"], ["forecast", "תחזית 12 חודשים"]] as [TabType, string][]).map(([key, label]) => (
          <button key={key} onClick={() => setTab(key)} className={`px-4 py-2 text-sm font-medium border-b-2 ${tab === key ? "border-emerald-500 text-emerald-400" : "border-transparent text-muted-foreground hover:text-white"}`}>{label}</button>
        ))}
      </div>
      ...
      {tab === "records" && (<>
        <div className="flex gap-3 flex-wrap">
          <div className="relative flex-1 min-w-0 sm:min-w-[200px]">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש..." className="w-full pr-10 pl-4 py-2 border border-[#2a2a3e] rounded-lg bg-[#0a0a1a] text-white" />
          </div>
          <select value={filterType} onChange={e => setFilterType(e.target.value)} className="border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white">
            <option value="all">הכל</option><option value="inflow">הכנסות</option><option value="outflow">הוצאות</option>
          </select>
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white">
            <option value="all">כל הסטטוסים</option>{Object.entries(statusMap).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
          </select>
        </div>
        <div className="bg-[#1a1a2e]/80 border border-[#2a2a3e] rounded-xl overflow-x-auto relative">
          {tableLoading && (
            <div className="absolute inset-0 bg-[#0a0a1a]/60 backdrop-blur-[1px] flex items-center justify-center z-10">
              <div className="flex items-center gap-2 bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg px-4 py-2 shadow-lg"><Loader2 className="w-4 h-4 animate-spin text-amber-400" /><span className="text-sm text-white">טוען נתונים...</span></div>
            </div>
          )}
          <table className="w-full text-sm">
            <thead className="bg-[#0a0a1a]/50 border-b border-[#2a2a3e]"><tr>
              {[
                ...
                <th key={col.key} className="px-2 py-3 text-right cursor-pointer hover:bg-[#2a2a3e]/50 text-xs text-muted-foreground" onClick={() => toggleSort(col.key)}>
            ...
            <tbody>
              ...
              pagination.paginate(filtered).map(r => (
                <tr key={r.id} className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30">
                  ...
                  <td className="px-2 py-2">
                    <div className="flex gap-1">
                      ...
                      <button onClick={async () => { if (await globalConfirm(`למחוק את '${r.category || r.id}'? פעולה זו אינה ניתנת לביטול.`)) remove(r.id); }} className="p-1 hover:bg-red-500/10 rounded text-muted-foreground hover:text-red-400"><Trash2 size={13} /></button>
                    </div>
                  </td>
                </tr>
              ))}
            ...
            {filtered.length > 0 && (
              <tfoot className="bg-[#0a0a1a]/50 border-t-2 border-[#2a2a3e] font-bold text-sm">
                <tr>
                  <td className="px-2 py-3 text-white" colSpan={3}>סה"כ ({filtered.length} שורות)</td>
          ...
          <div className="bg-[#1a1a2e]/80 border border-[#2a2a3e] rounded-xl p-4">
            ...
            <div className="space-y-3">
              {inflowCategories.map((c, i) => {
                ...
                return (
                  <div key={i}>
                    <div className="flex justify-between text-xs mb-1"><span className="font-medium text-white">{c.category}</span><span className="text-emerald-400 font-bold font-mono">₪{fmt(c.total)} ({pct.toFixed(1)}%)</span></div>
                    <div className="h-3 bg-[#2a2a3e] rounded-full overflow-hidden"><div className="bg-emerald-500 h-full rounded-full" style={{ width: `${pct}%` }} /></div>
                    <div className="text-[10px] text-muted-foreground mt-0.5">{c.count} רשומות | ממוצע: ₪{fmt(c.avg_amount)}</div>
                  </div>
                );
              })}
              <div className="border-t border-[#2a2a3e] pt-2 flex justify-between font-bold text-sm"><span className="text-white">סה"כ</span><span className="text-emerald-400 font-mono">₪{fmt(totalInflowCat)}</span></div>
            </div>}
          </div>
          <div className="bg-[#1a1a2e]/80 border border-[#2a2a3e] rounded-xl p-4">
            ...
            <div className="space-y-3">
              {outflowCategories.map((c, i) => {
                ...
                return (
                  <div key={i}>
                    <div className="flex justify-between text-xs mb-1"><span className="font-medium text-white">{c.category}</span><span className="text-red-400 font-bold font-mono">₪{fmt(c.total)} ({pct.toFixed(1)}%)</span></div>
                    <div className="h-3 bg-[#2a2a3e] rounded-full overflow-hidden"><div className="bg-red-500 h-full rounded-full" style={{ width: `${pct}%` }} /></div>
                    <div className="text-[10px] text-muted-foreground mt-0.5">{c.count} רשומות | ממוצע: ₪{fmt(c.avg_amount)}</div>
                  </div>
                );
              })}
              <div className="border-t border-[#2a2a3e] pt-2 flex justify-between font-bold text-sm"><span className="text-white">סה"כ</span><span className="text-red-400 font-mono">₪{fmt(totalOutflowCat)}</span></div>
            </div>}
          </div>
        </div>
      )}
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-[#1a1a2e] border border-[#2a2a3e] text-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div><label className="text-xs text-muted-foreground mb-1 block">תאריך <RequiredMark /></label><input type="date" value={form.recordDate || ""} onChange={e => setForm({ ...form, recordDate: e.target.value })} className="w-full border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white" /></div>
                <div><label className="text-xs text-muted-foreground mb-1 block">סוג תזרים</label>
                  <select value={form.flowType || "inflow"} onChange={e => setForm({ ...form, flowType: e.target.value })} className="w-full border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white">
                ...
                <div><label className="text-xs text-muted-foreground mb-1 block">קטגוריה</label>
                  <select value={form.category || ""} onChange={e => setForm({ ...form, category: e.target.value })} className="w-full border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white">
                ...
                <div className="col-span-2 md:col-span-3"><label className="text-xs text-muted-foreground mb-1 block">תיאור <RequiredMark /></label>
                  <input value={form.description || ""} onChange={e => setForm({ ...form, description: e.target.value })} className="w-full border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white" placeholder="תיאור הרשומה..." />
                ...
                <div><label className="text-xs text-muted-foreground mb-1 block">סכום (₪) <RequiredMark /></label>
                  <input type="number" min={0} value={form.amount || ""} onChange={e => setForm({ ...form, amount: e.target.value })} className="w-full border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white font-mono" placeholder="0.00" />
                ...
                <div><label className="text-xs text-muted-foreground mb-1 block">סטטוס</label>
                  <select value={form.status || "actual"} onChange={e => setForm({ ...form, status: e.target.value })} className="w-full border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white">
                    {Object.entries(statusMap).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                  </select></div>
                <div><label className="text-xs text-muted-foreground mb-1 block">לקוח</label><input value={form.customerName || ""} onChange={e => setForm({ ...form, customerName: e.target.value })} className="w-full border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white" /></div>
                <div><label className="text-xs text-muted-foreground mb-1 block">ספק</label><input value={form.supplierName || ""} onChange={e => setForm({ ...form, supplierName: e.target.value })} className="w-full border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white" /></div>
                <div><label className="text-xs text-muted-foreground mb-1 block">פרויקט</label><input value={form.projectName || ""} onChange={e => setForm({ ...form, projectName: e.target.value })} className="w-full border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white" /></div>
                <div className="col-span-2 md:col-span-3"><label className="text-xs text-muted-foreground mb-1 block">הערות</label><textarea value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} rows={2} className="w-full border border-[#2a2a3e] rounded-lg px-3 py-2 bg-[#0a0a1a] text-white resize-none" /></div>
              </div>
              <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-[#2a2a3e]">
                <button onClick={() => setShowForm(false)} className="px-4 py-2 border border-[#2a2a3e] rounded-lg text-sm text-white hover:bg-[#2a2a3e]">ביטול</button>
                <button onClick={save} disabled={actionLoading} className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700 flex items-center gap-2"><Save size={16} />{editing ? "עדכן" : "שמור"}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
