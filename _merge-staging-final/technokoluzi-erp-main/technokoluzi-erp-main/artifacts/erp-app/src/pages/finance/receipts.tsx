...
import {
  ...
  X, Save, Loader2, Printer, Send, TrendingUp, Hash, Eye, Edit2, Trash2
} from "lucide-react";
...
const statusConfig: Record<string, { label: string; color: string }> = {
  draft: { label: "טיוטה", color: "bg-[#2a2a3e] text-muted-foreground" },
...
export default function ReceiptsPage() {
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      vatAmount: (Number(form.amountReceived) * Number(form.vatRate || 18)) / (100 + Number(form.vatRate || 18)),
    };
    await executeSave(
  ...
  const kpis = [
    ...
    { label: "טיוטות", value: fmtInt(items.filter(r => r.status === "draft").length), icon: Clock, color: "text-muted-foreground", bg: "from-[#2a2a3e]/50 to-[#2a2a3e]/20 border-[#2a2a3e]" },
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Receipt className="text-emerald-400" /> קבלות</h1>
        ...
        <CardContent className="p-3">
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input value={search} onChange={e => { setSearch(e.target.value); pagination.setPage(1); }} placeholder="חיפוש לפי מספר קבלה, לקוח, חשבונית..." className="pr-9 bg-[#0a0a1a] border-[#2a2a3e] text-white" />
            </div>
            <select value={filterStatus} onChange={e => { setFilterStatus(e.target.value); pagination.setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white">
              <option value="all">כל הסטטוסים</option>
              {Object.entries(statusConfig).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
            </select>
            <select value={filterMethod} onChange={e => { setFilterMethod(e.target.value); pagination.setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white">
      ...
      <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]">
        <CardContent className="p-0">
          <div className="overflow-x-auto relative">
            {tableLoading && (
              <div className="absolute inset-0 bg-[#0a0a1a]/60 backdrop-blur-[1px] flex items-center justify-center z-10">
                <div className="flex items-center gap-2 bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg px-4 py-2 shadow-lg">
                  <Loader2 className="w-4 h-4 animate-spin text-emerald-400" /><span className="text-sm text-white">טוען קבלות...</span>
                </div>
              </div>
            )}
            <table className="w-full text-sm">
              <thead><tr className="border-b border-[#2a2a3e] bg-[#0a0a1a]/50">
                {[
                  ...
                  <th key={col.key} className="px-3 py-3 text-right text-muted-foreground font-medium cursor-pointer hover:text-white" onClick={() => toggleSort(col.key)}>
                ...
                {!tableLoading && pagination.paginate(filtered).length === 0 ? (
                  <tr><td colSpan={9} className="p-16 text-center">
                    <div className="flex flex-col items-center gap-4">
                      <Receipt className="h-12 w-12 text-[#2a2a3e]" />
                  ...
                  <tr key={r.id} className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30 transition-colors cursor-pointer" onClick={() => setSelectedItem(r)}>
                    ...
                    <td className="px-3 py-2.5">
                      <Badge className={`${statusConfig[r.status]?.color || "bg-[#2a2a3e]"} border-0 text-[10px]`}>
              ...
              {filtered.length > 0 && (
                <tfoot><tr className="border-t-2 border-[#2a2a3e] bg-[#0a0a1a]/50 font-bold text-sm">
                  <td className="px-3 py-3 text-white" colSpan={4}>סה״כ ({filtered.length} קבלות)</td>
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              <div className="p-4 space-y-5">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div><Label className="text-muted-foreground text-xs">מספר קבלה</Label><Input value={form.receiptNumber || ""} readOnly className="bg-[#0a0a1a] border-[#2a2a3e] text-emerald-400 font-mono mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">תאריך *</Label><Input type="date" value={form.receiptDate || ""} onChange={e => setForm({ ...form, receiptDate: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">סטטוס</Label>
                    <select value={form.status || "confirmed"} onChange={e => setForm({ ...form, status: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1">
                      {Object.entries(statusConfig).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                    </select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="relative">
                    ...
                    <Input value={customerSearch} onChange={e => { setCustomerSearch(e.target.value); setForm({ ...form, customerName: e.target.value }); setShowCustDropdown(true); }} onFocus={() => setShowCustDropdown(true)} placeholder="הקלד שם לקוח..." className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" />
                    {showCustDropdown && filteredCustomers.length > 0 && (
                      <div className="absolute z-20 top-full mt-1 w-full bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg shadow-xl max-h-36 overflow-y-auto">
                        {filteredCustomers.map((c, i) => (
                          <button key={i} onClick={() => { setForm({ ...form, customerName: c }); setCustomerSearch(c); setShowCustDropdown(false); }} className="w-full px-3 py-2 text-right text-sm text-white hover:bg-[#2a2a3e]">{c}</button>
                        ))}
                      </div>
                    )}
                  </div>
                  <div className="relative">
                    ...
                    <Input value={invoiceSearch} onChange={e => { setInvoiceSearch(e.target.value); setShowInvoiceDropdown(true); }} onFocus={() => setShowInvoiceDropdown(true)} placeholder="חיפוש חשבונית..." className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" />
                    {showInvoiceDropdown && filteredInvoices.length > 0 && (
                      <div className="absolute z-20 top-full mt-1 w-full bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg shadow-xl max-h-36 overflow-y-auto">
                        {filteredInvoices.map((inv: any, i: number) => (
                          ...
                          }} className="w-full px-3 py-2 text-right text-sm text-white hover:bg-[#2a2a3e] flex items-center justify-between">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div><Label className="text-muted-foreground text-xs">סכום שהתקבל (₪) *</Label>
                    <Input type="number" min={0} step={0.01} value={form.amountReceived || ""} onChange={e => setForm({ ...form, amountReceived: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-emerald-400 font-mono mt-1" placeholder="0.00" /></div>
                  <div><Label className="text-muted-foreground text-xs">יתרה לגבייה (₪)</Label>
                    <Input type="number" min={0} step={0.01} value={form.balanceRemaining || ""} onChange={e => setForm({ ...form, balanceRemaining: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-amber-400 font-mono mt-1" placeholder="0.00" /></div>
                  ...
                  <div><Label className="text-muted-foreground text-xs">מס׳ אסמכתא</Label><Input value={form.referenceNumber || ""} onChange={e => setForm({ ...form, referenceNumber: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" placeholder="שיק / עסקה / העברה" /></div>
                  <div><Label className="text-muted-foreground text-xs">מטבע</Label>
                    <select value={form.currency || "ILS"} onChange={e => setForm({ ...form, currency: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1">
                      <option value="ILS">₪ שקל (ILS)</option><option value="USD">$ דולר (USD)</option><option value="EUR">€ יורו (EUR)</option>
                    </select>
                  </div>
                  <div><Label className="text-muted-foreground text-xs">שיעור מע״מ (%)</Label><Input type="number" min={0} max={30} value={form.vatRate || 18} onChange={e => setForm({ ...form, vatRate: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                </div>

                {Number(form.amountReceived) > 0 && (
                  <div className="bg-[#0a0a1a] rounded-lg border border-[#2a2a3e] p-4 space-y-2 max-w-xs mr-auto">
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">סכום ברוטו</span><span className="font-mono text-white">₪{fmt(form.amountReceived)}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">מע״מ ({form.vatRate || 18}%)</span><span className="font-mono text-amber-400">₪{fmt((Number(form.amountReceived) * Number(form.vatRate || 18)) / (100 + Number(form.vatRate || 18)))}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">נטו</span><span className="font-mono text-emerald-400">₪{fmt(Number(form.amountReceived) - (Number(form.amountReceived) * Number(form.vatRate || 18)) / (100 + Number(form.vatRate || 18)))}</span></div>
                    {Number(form.balanceRemaining || 0) > 0 && (
                      <div className="flex justify-between text-sm border-t border-[#2a2a3e] pt-2"><span className="text-amber-400 font-bold">יתרה לגבייה</span><span className="font-mono text-amber-400">₪{fmt(form.balanceRemaining)}</span></div>
                    )}
                  </div>
                )}

                <div><Label className="text-muted-foreground text-xs">הערות</Label><textarea value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white resize-none mt-1" placeholder="הערות לקבלה..." /></div>
              </div>
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
              <div className="flex items-center gap-3">
                <h2 className="text-lg font-bold text-white">{selectedItem.receipt_number || `קבלה #${selectedItem.id}`}</h2>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="flex items-center gap-2 pt-2">
                <Button variant="outline" className="border-[#2a2a3e] gap-1" onClick={() => printPage(`קבלה ${selectedItem.receipt_number}`)}><Printer className="h-4 w-4" />הדפסה</Button>
