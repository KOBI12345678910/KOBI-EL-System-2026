    ...
    <div className="space-y-4" dir="rtl">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">AI Risk Intelligence Engine</h1>
          ...
          <div className="flex gap-2 mb-2">
            {[
              ...
              <Button key={f.key} size="sm" variant={filter === f.key ? "default" : "outline"}
                className={`text-[10px] h-6 px-2 ${filter === f.key ? "bg-blue-600 text-white" : "text-muted-foreground"}`}
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        {r.actions.map((a, j) => (
                          <div key={j} className={`rounded p-2 border ${a.status === "ready" ? "bg-green-500/5 border-green-500/20" : "bg-slate-800/30 border-slate-700/30"}`}>
                            ...
                            <div className="text-[10px] text-white">{a.step}</div>
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {RECOMMENDATIONS.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="border-b border-slate-800/40 hover:bg-slate-800/20">
                        ...
                        <td className="p-2 text-white text-[10px]">{r.title.substring(0, 35)}...</td>
                  ...
                  <tr className="bg-slate-800/30 border-t border-slate-700/50">
                    <td className="p-2 text-white font-bold" colSpan={2}>TOTAL PORTFOLIO</td>
          ...
          <Card className="bg-gradient-to-r from-yellow-500/10 to-emerald-500/10 border-yellow-500/20">
            <CardContent className="p-4">
              <div className="flex items-center gap-2 mb-2">
                <Brain className="w-5 h-5 text-yellow-400" />
                <span className="text-sm text-white font-medium">AI Executive Summary</span>
              </div>
