...
import {
  ...
  ClipboardCheck, UserCheck, ThumbsUp, ThumbsDown,
} from "lucide-react";
...
const statusConfig: Record<string, { label: string; color: string; next?: string[] }> = {
  draft: { label: "טיוטה", color: "bg-[#2a2a3e] text-muted-foreground", next: ["received"] },
...
export default function SupplierInvoicesPage() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ invoiceDate: new Date().toISOString().slice(0, 10), status: "draft", currency: "ILS", vatRate: VAT_RATE, paymentTerms: "net_30", supplierName: "", notes: "" });
  ...
  const openEdit = (r: SupplierInvoice) => {
    ...
    setForm({
      ...
      vatRate: r.vat_rate || VAT_RATE, amountPaid: r.amount_paid,
  ...
  const save = async () => {
    ...
    setSaving(true);
    const payload = {
      ...form, subtotal, beforeVat: subtotal, vatRate: VAT_RATE, vatAmount, totalAmount,
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Truck className="text-orange-400" /> חשבוניות ספקים</h1>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="outline" onClick={() => printPage("חשבוניות ספקים")} className="border-[#2a2a3e] text-muted-foreground gap-1"><Printer className="h-4 w-4" />הדפסה</Button>
      ...
      {items.length > 1 && (() => {
        ...
        return trendData.length > 1 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-4">
              <div className="flex items-center gap-2 mb-3 text-sm font-semibold text-white"><TrendingUp className="h-4 w-4 text-orange-400" /> מגמת הוצאות לספקים</div>
            ...
            <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-4">
              <div className="flex items-center gap-2 mb-3 text-sm font-semibold text-white"><BarChart3 className="h-4 w-4 text-red-400" /> שולם מול יתרה</div>
              <ResponsiveContainer width="100%" height={200}>
      ...
      <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]"><Search className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={e => { setSearch(e.target.value); pagination.setPage(1); }} placeholder="חיפוש לפי מספר, ספק, הזמנה..." className="pr-9 bg-[#0a0a1a] border-[#2a2a3e] text-white" /></div>
      ...
      <BulkActions items={filtered} selectedIds={selectedIds} onSelectionChange={setSelectedIds} actions={[
        defaultBulkActions.delete(async (ids) => { for (const id of ids) await authFetch(`${API}/supplier-invoices/${id}`, { method: "DELETE", headers }); load(); }),
      ...
      <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-0">
        <div className="overflow-x-auto relative">
          {tableLoading && <div className="absolute inset-0 bg-[#0a0a1a]/60 backdrop-blur-[1px] flex items-center justify-center z-10"><div className="flex items-center gap-2 bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg px-4 py-2 shadow-lg"><Loader2 className="w-4 h-4 animate-spin text-orange-400" /><span className="text-sm text-white">טוען חשבוניות ספקים...</span></div></div>}
          <table className="w-full text-sm">
            ...
            <tbody>
              {!tableLoading && pagination.paginate(filtered).length === 0 ? (
                <tr><td colSpan={12} className="p-16 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-12 w-12 text-[#2a2a3e]" /> : <Truck className="h-12 w-12 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין חשבוניות ספקים"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את הסינון" : "צור חשבונית ספק ראשונה"}</p>{!(af > 0 || search) && <Button onClick={openCreate} className="bg-orange-600 hover:bg-orange-700 gap-2 mt-2"><Plus className="h-4 w-4" />חשבונית ספק חדשה</Button>}</div></td></tr>
                ...
                <tr key={r.id} className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30 transition-colors cursor-pointer" onClick={() => setSelectedItem(r)}>
                  ...
                  <td className="px-3 py-2.5 text-white font-medium max-w-[140px] truncate">{r.supplier_name}</td>
                  ...
                  <td className="px-3 py-2.5 text-center" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-red-400" onClick={async () => { if (await globalConfirm(`למחוק את חשבונית ספק '${r.invoice_number}'? פעולה זו אינה ניתנת לביטול.`)) remove(r.id); }}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
                <div className="flex items-center gap-3">
                  <h2 className="text-lg font-bold text-white">{editing ? "עריכת חשבונית ספק" : "חשבונית ספק חדשה"}</h2>
              ...
              <div className="p-4 space-y-5">
                ...
                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי ספק</h3></div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="col-span-2 relative">
                    <Label className="text-muted-foreground text-xs">ספק *</Label>
                    <Input value={supplierSearch} onChange={e => { setSupplierSearch(e.target.value); setForm({ ...form, supplierName: e.target.value }); setShowSupplierDropdown(true); }} onFocus={() => setShowSupplierDropdown(true)} placeholder="הקלד שם ספק..." className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" />
                    {showSupplierDropdown && filteredSuppliers.length > 0 && (
                      <div className="absolute z-20 top-full mt-1 w-full bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg shadow-xl max-h-48 overflow-y-auto">
                        {filteredSuppliers.map((s, i) => (
                          <button key={i} onClick={() => { setForm({ ...form, supplierName: s }); setSupplierSearch(s); setShowSupplierDropdown(false); }} className="w-full px-3 py-2 text-right text-sm text-white hover:bg-[#2a2a3e] transition-colors">{s}</button>
                        ))}
                      </div>
                    )}
                  </div>
                  <div><Label className="text-muted-foreground text-xs">ח.פ / ע.מ ספק</Label><Input value={form.supplierTaxId || ""} onChange={e => setForm({ ...form, supplierTaxId: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">הזמנת רכש (PO)</Label><Input value={form.poNumber || ""} onChange={e => setForm({ ...form, poNumber: e.target.value })} placeholder="PO-2026-XXXX" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1 font-mono" /></div>
                </div>
                ...
                <div className="bg-[#0a0a1a] rounded-lg border border-[#2a2a3e] overflow-hidden">
                  ...
                  <div className="p-2 border-t border-[#2a2a3e]"><Button variant="ghost" size="sm" onClick={addLineItem} className="text-blue-400 hover:text-blue-300 gap-1 text-xs"><Plus className="h-3 w-3" />הוסף שורה</Button></div>
                </div>
                ...
                <div className="flex justify-end">
                  <div className="w-full max-w-sm bg-[#0a0a1a] rounded-lg border border-[#2a2a3e] p-4 space-y-3">
                    ...
                    <div className="flex justify-between text-lg font-bold"><span className="text-white">סה״כ לתשלום</span><span className="text-orange-400 font-mono">₪{fmt(totalAmount)}</span></div>
                ...
                <textarea value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white resize-none" placeholder="הערות לחשבונית ספק..." />
              </div>
              <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-end">
                ...
                <Button onClick={save} disabled={saving || !form.supplierName} className="bg-orange-600 hover:bg-orange-700 gap-1"><Save className="h-4 w-4" />{saving ? "שומר..." : editing ? "עדכן" : "שמור"}</Button>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 space-y-4">
              ...
              {selectedItem.po_number && (
                <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 flex items-center gap-3">
                  ...
                  <div><p className="text-sm text-white font-medium">מותאם להזמנת רכש {selectedItem.po_number}</p><p className="text-[11px] text-muted-foreground">חשבונית זו מקושרת להזמנת רכש</p></div>
                </div>
              )}
              ...
              <div className="flex justify-end">
                <div className="w-full max-w-xs bg-[#0a0a1a] rounded-lg border border-[#2a2a3e] p-4 space-y-2">
                  ...
                  <hr className="border-[#2a2a3e]" />
                  <div className="flex justify-between font-bold"><span className="text-white">סה״כ</span><span className="text-orange-400 font-mono">₪{fmt(selectedItem.total_amount)}</span></div>
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {statusConfig[selectedItem.status]?.next?.map(ns => (
                      <Button key={ns} variant="outline" size="sm" className={`border-[#2a2a3e] gap-1 ${ns === "approved" ? "text-emerald-400 border-emerald-500/30" : ns === "verified" ? "text-indigo-400 border-indigo-500/30" : ns === "paid" ? "text-green-400 border-green-500/30" : ns === "cancelled" || ns === "disputed" ? "text-red-400 border-red-500/30" : "text-amber-400 border-amber-500/30"}`} onClick={async () => {
            ...
            <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-end">
              <Button variant="outline" className="border-[#2a2a3e] gap-1" onClick={() => printPage(`חשבונית ספק ${selectedItem.invoice_number}`)}><Printer className="h-4 w-4" />הדפסה</Button>
