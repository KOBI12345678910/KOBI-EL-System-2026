        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">הוראת קבע: {selectedItem.customer_name}</h2>
              ...
              {detailTab === "details" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    ...
                    <div><div className="text-xs text-muted-foreground mb-1">תדירות</div><div className="text-sm text-white">{frequencyLabels[selectedItem.frequency] || selectedItem.frequency}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">תאריך התחלה</div><div className="text-sm text-white">{fmtDate(selectedItem.start_date)}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">תאריך סיום</div><div className="text-sm text-white">{fmtDate(selectedItem.end_date)}</div></div>
