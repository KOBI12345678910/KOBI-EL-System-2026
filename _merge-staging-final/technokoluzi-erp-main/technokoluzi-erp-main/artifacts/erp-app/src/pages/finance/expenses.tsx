...
function UploadZone({ onFileDrop }: { onFileDrop: (file: File) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-white font-medium mb-1">גררו קובץ לכאן</div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6" dir="rtl">
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="bg-slate-900 border-slate-700 max-w-lg" dir="rtl">
              ...
              <div className="grid gap-4">
                ...
                <Button onClick={() => createMutation.mutate(form)} disabled={!form.category || !form.description || !form.amount} className="bg-red-600 hover:bg-red-700 text-white">
      ...
      <div className="flex gap-3 items-center flex-wrap">
        ...
        <div className="text-sm text-muted-foreground">סה"כ: <span className="text-white font-medium">{fmt(totalAmount)}</span> ({items.length} רשומות)</div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">הוצאה #{selectedItem.receipt_number || selectedItem.id}</h2>
              ...
              {detailTab === "details" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <div><div className="text-xs text-muted-foreground mb-1">תאריך</div><div className="text-sm text-white">{fmtDate(selectedItem.expense_date)}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">ספק</div><div className="text-sm text-white">{selectedItem.vendor_name || "-"}</div></div>
                    ...
                    <div><div className="text-xs text-muted-foreground mb-1">הערות</div><div className="text-sm text-white">{selectedItem.notes || "-"}</div></div>
                  </div>
                </div>
              )}
