...
export default function GeneralLedgerPage() {
  ...
  const load = () => {
    ...
    Promise.all([
      authFetch(`${API}/general-ledger${params}`, { headers }).then(r => r.json()).then(d => setEntries(safeArray(d))),
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => printPage("ספר ראשי")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Printer size={16} /> הדפסה</button>
          <button onClick={() => sendByEmail("ספר ראשי", generateEmailBody("ספר ראשי", entries, { entry_number: "מספר", account_name: "חשבון", debit: "חובה", credit: "זכות" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Send size={16} /> שליחה</button>
          <button onClick={openCreate} className="flex items-center gap-2 bg-indigo-600 text-white px-3 py-2 rounded-lg hover:bg-indigo-700 shadow-lg text-sm"><Plus size={16} /> רשומה חדשה</button>
        </div>
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button onClick={save} className="flex items-center gap-2 bg-indigo-600 text-white px-6 py-2 rounded-lg hover:bg-indigo-700 shadow-sm"><Save size={16} /> {editing ? "עדכון" : "שמירה"}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">פקודת ספר ראשי #{selectedItem.entry_number || selectedItem.id}</h2>
            ...
            <div className="p-6">
              {detailTab === "details" && (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <div><div className="text-xs text-muted-foreground mb-1">תאריך</div><div className="text-sm text-white">{selectedItem.entry_date}</div></div>
                  ...
                  <div><div className="text-xs text-muted-foreground mb-1">חשבון</div><div className="text-sm text-white">{selectedItem.account_name || "-"}</div></div>
