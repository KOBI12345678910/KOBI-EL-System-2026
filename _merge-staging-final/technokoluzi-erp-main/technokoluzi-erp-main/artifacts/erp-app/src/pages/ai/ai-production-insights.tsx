import { useState, useEffect } from "react";
  ...
  import { Settings, Zap, FileText, Shield, BarChart3, Loader2, RefreshCw, Play, CheckCircle2, Clock, AlertTriangle, TrendingUp, Users, Activity, Brain, Target, Wrench } from "lucide-react";
  ...
  export default function AIProductionInsightsPage() {
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a0e1a] to-[#1a1f35] text-white p-6" dir="rtl">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold bg-gradient-to-l from-rose-400 to-pink-300 bg-clip-text text-transparent flex items-center gap-3">
                <Wrench className="w-8 h-8 text-rose-400" />
                תובנות ייצור AI
              </h1>
              <p className="text-muted-foreground mt-1">{moduleInfo.description || "ניתוח ביצועי ייצור, חיזוי תקלות, ואופטימיזציה"}</p>
            </div>
            <div className="flex items-center gap-3">
              <button onClick={runNow} disabled={running} className="flex items-center gap-2 px-4 py-2 bg-rose-600/20 border border-rose-500/30 rounded-lg hover:opacity-80 transition disabled:opacity-50">
          ...
          {tab === "dashboard" && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: "תובנות שהופקו", value: fmt(metrics?.dailyMetrics?.reduce((s: number, d: any) => s + (d.actionsGenerated || 0), 0) || 94), icon: Brain, color: "from-rose-500/20 to-rose-600/10", text: "text-rose-400" },
                { label: "יעילות ייצור", value: "87.3%", icon: TrendingUp, color: "from-green-500/20 to-green-600/10", text: "text-green-400" },
                { label: "חיזויי תחזוקה", value: "18", icon: AlertTriangle, color: "from-amber-500/20 to-amber-600/10", text: "text-amber-400" },
                { label: "הוראות עבודה", value: "156", icon: FileText, color: "from-blue-500/20 to-blue-600/10", text: "text-blue-400" },
                ].map((kpi, i) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-card/5 border border-white/10 rounded-xl p-5">
                  <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-rose-400" /> ביצועים יומיים (30 יום)</h3>
                  ...
                  <div className="space-y-3">
                    {(linked?.linkedEntities || []).map((entity: string) => (
                      <div key={entity} className="flex items-center justify-between bg-card/5 rounded-lg p-3">
                        <span className="text-gray-300">{{"work_orders":"הוראות עבודה","machines":"מכונות","products":"מוצרים","quality_reports":"דוחות איכות"}[entity] || entity}</span>
          ...
          {tab === "settings" && (
            <div className="bg-card/5 border border-white/10 rounded-xl p-6 space-y-6">
              ...
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>חיזוי תחזוקה</span>
                  <button onClick={() => updateSetting("maintenancePrediction", !settings.maintenancePrediction)} className={`w-12 h-6 rounded-full transition ${settings.maintenancePrediction ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.maintenancePrediction ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">סף איכות (%)</label>
                  <input type="number" value={settings.qualityThreshold || 0} onChange={e => updateSetting("qualityThreshold", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">יעד יעילות (%)</label>
                  <input type="number" value={settings.efficiencyTarget || 0} onChange={e => updateSetting("efficiencyTarget", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>התראה על השבתה</span>
                  <button onClick={() => updateSetting("alertOnDowntime", !settings.alertOnDowntime)} className={`w-12 h-6 rounded-full transition ${settings.alertOnDowntime ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.alertOnDowntime ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">חיזוי (ימים קדימה)</label>
                  <input type="number" value={settings.predictiveDays || 0} onChange={e => updateSetting("predictiveDays", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              </div>
            </div>
