import { useState, useEffect } from "react";
  ...
  import { Settings, Zap, FileText, Shield, BarChart3, Loader2, RefreshCw, Play, CheckCircle2, Clock, AlertTriangle, TrendingUp, Users, Activity, Brain, Target, Headphones } from "lucide-react";
  ...
  export default function AICustomerServiceProPage() {
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a0e1a] to-[#1a1f35] text-white p-6" dir="rtl">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold bg-gradient-to-l from-cyan-400 to-blue-300 bg-clip-text text-transparent flex items-center gap-3">
                <Headphones className="w-8 h-8 text-cyan-400" />
                שירות לקוחות AI
              </h1>
              <p className="text-muted-foreground mt-1">{moduleInfo.description || "מענה אוטומטי, ניתוב שיחות, וזיהוי רגשות"}</p>
            </div>
            <div className="flex items-center gap-3">
              <button onClick={runNow} disabled={running} className="flex items-center gap-2 px-4 py-2 bg-cyan-600/20 border border-cyan-500/30 rounded-lg hover:opacity-80 transition disabled:opacity-50">
          ...
          {tab === "dashboard" && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: "פניות שטופלו", value: fmt(metrics?.dailyMetrics?.reduce((s: number, d: any) => s + (d.actionsGenerated || 0), 0) || 523), icon: Users, color: "from-cyan-500/20 to-cyan-600/10", text: "text-cyan-400" },
                { label: "שיעור פתרון אוטו", value: "67%", icon: CheckCircle2, color: "from-green-500/20 to-green-600/10", text: "text-green-400" },
                { label: "זמן תגובה ממוצע", value: "45 שניות", icon: Clock, color: "from-blue-500/20 to-blue-600/10", text: "text-blue-400" },
                { label: "שביעות רצון", value: "4.6/5", icon: TrendingUp, color: "from-amber-500/20 to-amber-600/10", text: "text-amber-400" },
                ].map((kpi, i) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-card/5 border border-white/10 rounded-xl p-5">
                  <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-cyan-400" /> ביצועים יומיים (30 יום)</h3>
                  ...
                  <div className="space-y-3">
                    {(linked?.linkedEntities || []).map((entity: string) => (
                      <div key={entity} className="flex items-center justify-between bg-card/5 rounded-lg p-3">
                        <span className="text-gray-300">{{"customers":"לקוחות","tickets":"פניות","communications":"תקשורות"}[entity] || entity}</span>
          ...
          {tab === "settings" && (
            <div className="bg-card/5 border border-white/10 rounded-xl p-6 space-y-6">
              ...
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  ...
                  <button onClick={() => updateSetting("autoReply", !settings.autoReply)} className={`w-12 h-6 rounded-full transition ${settings.autoReply ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.autoReply ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">מקסימום תגובות אוטומטיות</label>
                  <input type="number" value={settings.maxAutoReplies || 0} onChange={e => updateSetting("maxAutoReplies", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>הסלמה בסנטימנט שלילי</span>
                  <button onClick={() => updateSetting("escalateOnNegativeSentiment", !settings.escalateOnNegativeSentiment)} className={`w-12 h-6 rounded-full transition ${settings.escalateOnNegativeSentiment ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.escalateOnNegativeSentiment ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">סף סנטימנט שלילי (-1 עד 0)</label>
                  <input type="number" value={settings.sentimentThreshold || 0} onChange={e => updateSetting("sentimentThreshold", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">יעד זמן תגובה (שניות)</label>
                  <input type="number" value={settings.responseTimeTarget || 0} onChange={e => updateSetting("responseTimeTarget", parseFloat(e.target.value))}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>שימוש בבסיס ידע</span>
                  <button onClick={() => updateSetting("knowledgeBase", !settings.knowledgeBase)} className={`w-12 h-6 rounded-full transition ${settings.knowledgeBase ? "bg-green-500" : "bg-gray-600"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.knowledgeBase ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              </div>
            </div>
