...
export default function Dashboard() {
  ...
  const { data: dashboards = [] } = useQuery<DashboardPage[]>({
  ...
  const { data: dynamicWidgets = [] } = useQuery<DashboardWidget[]>({
...
function DynamicDashboardView({ dashboard, widgets, navigate }: { dashboard: DashboardPage; widgets: DashboardWidget[]; navigate: (path: string) => void }) {
  const layoutWidgets = autoLayoutWidgets(widgets);
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl sm:text-3xl font-bold text-white tracking-tight">{dashboard.name}</h1>
          ...
          return (
            ...
            >
              <Card className="h-full p-4 overflow-hidden">
                ...
                <DashboardWidgetPreview type={widget.widgetType} config={widget.config} entityId={widget.entityId} />
              </Card>
            </motion.div>
          );
        })}
...
function DashboardWidgetPreview({ type, config, entityId }: { type: string; config: WidgetJsonb; entityId: number | null }) {
...
function StaticDashboard({ navigate }: { navigate: (path: string) => void }) {
  ...
  const { data: platformModules } = useQuery<PlatformModule[]>({
    queryKey: ["dashboard-platform-modules"],
    queryFn: async () => {
      const r = await authFetch(`${API}/platform/modules`, { headers: authHeaders() });
      if (!r.ok) return [];
      return r.json();
    },
    staleTime: STALE_5M,
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <div className="flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            <div>
              <h1 className="text-lg sm:text-2xl font-bold text-white tracking-tight">מעקב פעילות בזמן אמת</h1>
            </div>
          </div>
          <div className="flex items-center gap-5">
            <div className="flex items-center gap-2 text-sm">
              <span className="text-muted-foreground">סטטוס:</span>
              <span className="font-bold text-white">{overallStatus}%</span>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm text-white bg-card/10 hover:bg-card/15 rounded-xl transition-colors border border-white/10"
            >
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl sm:text-3xl font-bold text-white tracking-tight">טכנו-כל עוזי</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-card/20 text-white mb-2 inline-block">דחוף</span>
                <h4 className="text-white font-bold text-base mt-1">משימות באיחור</h4>
                <p className="text-white/70 text-xs mt-1">{overdueTaskCount} משימות שעברו את מועד היעד</p>
              </div>
              <div className="w-12 h-12 rounded-xl bg-card/20 flex items-center justify-center">
                <AlertTriangle className="w-6 h-6 text-white" />
              </div>
            </div>
            <button className="mt-3 flex items-center gap-1.5 text-xs font-medium text-white bg-card/20 hover:bg-card/30 px-3 py-1.5 rounded-lg transition-colors">
              טפל עכשיו
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <h4 className="text-white font-bold text-base">ערך צברת עסקאות</h4>
                <p className="text-white text-lg sm:text-2xl font-bold mt-1">₪{pipelineValue.toLocaleString("he-IL")}</p>
                <p className="text-white/70 text-xs mt-1">{pipelineDeals} עסקאות פתוחות</p>
              </div>
              <div className="w-12 h-12 rounded-xl bg-card/20 flex items-center justify-center">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <h4 className="text-white font-bold text-base">משימות שהושלמו היום</h4>
                <p className="text-white text-lg sm:text-2xl font-bold mt-1">{completedToday}</p>
                <p className="text-white/70 text-xs mt-1">{completedToday} משימות שהושלמו בהצלחה</p>
              </div>
              <div className="w-12 h-12 rounded-xl bg-card/20 flex items-center justify-center">
            ...
            <button
              className="mt-3 flex items-center gap-1.5 text-xs font-medium text-white bg-card/20 hover:bg-card/30 px-3 py-1.5 rounded-lg transition-colors"
          ...
          {[
            ...
            <motion.div key={`kpi-${i}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}>
              <Card className={`p-6 border ${kpi.border}`}>
                ...
                <p className="text-xl sm:text-3xl font-bold text-white mb-1">{kpi.value}</p>
            ...
            {dealTypes.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <RechartsPieChart>
                  ...
                  <Tooltip
                    contentStyle={{ background: "hsl(222,47%,13%)", border: "1px solid hsl(217,32%,17%)", borderRadius: "8px", fontSize: "12px" }}
          ...
          {quickActions.map((action, i) => (
            ...
            >
              ...
              >
                ...
                <span className="text-xs font-medium text-muted-foreground group-hover:text-white transition-colors">{action.label}</span>
              </button>
            </motion.div>
          ))}
        </div>
              ...
              {modules.slice(0, 9).map((mod, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground group-hover:text-white transition-colors text-center leading-tight line-clamp-2">{mod.name}</span>
                    {count !== undefined && (
                    ...
                    <table className="w-full text-xs">
                      <tbody>
                        <tr className="border-b border-border/30">
                          ...
                          <td className="px-3 py-2 text-left font-bold text-white">{s.total}</td>
                        </tr>
                        <tr className="border-b border-border/30">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
          <Card className="p-5 cursor-pointer hover:border-primary/50 transition-all" onClick={() => navigate("/product-catalog")}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-bold text-white">מוצרים</span>
            </div>
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between"><span className="text-muted-foreground">סה״כ</span><span className="font-bold text-white text-sm">{products.length}</span></div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.65 }}>
          <Card className="p-5 cursor-pointer hover:border-primary/50 transition-all" onClick={() => navigate("/hr/employees")}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-bold text-white">עובדים</span>
            </div>
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between"><span className="text-muted-foreground">סה״כ</span><span className="font-bold text-white text-sm">{Number(hr.total_employees || 0)}</span></div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}>
          <Card className="p-5 cursor-pointer hover:border-primary/50 transition-all" onClick={() => navigate("/sales/customers")}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-bold text-white">לקוחות</span>
            </div>
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between"><span className="text-muted-foreground">סה״כ</span><span className="font-bold text-white text-sm">{customers.length}</span></div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75 }}>
          <Card className="p-5 cursor-pointer hover:border-primary/50 transition-all" onClick={() => navigate("/crm/leads")}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-bold text-white">לידים</span>
            </div>
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between"><span className="text-muted-foreground">סה״כ לידים</span><span className="font-bold text-white text-sm">{Number(funnel.total_leads || 0)}</span></div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.65 }}>
        <Card className="overflow-hidden">
          <div className="px-5 py-4 border-b border-border/50 flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-white">סטטיסטיקות מחלקות</h3>
          </div>
                  ...
                  return (
                    <tr key={`dept-${i}`} className="border-b border-border/20 hover:bg-card/[0.02] transition-colors">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className="font-bold text-white text-xs">{dept.count}</span>
                      </td>
                      <td className="px-4 py-3 text-center">
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {lowStockMaterials.slice(0, 10).map(mat => (
                  <div key={mat.id} className="flex items-center justify-between p-3 rounded-xl bg-red-500/5 border border-red-500/20">
                    <div>
                      <span className="text-sm font-medium text-white">{mat.materialName}</span>
