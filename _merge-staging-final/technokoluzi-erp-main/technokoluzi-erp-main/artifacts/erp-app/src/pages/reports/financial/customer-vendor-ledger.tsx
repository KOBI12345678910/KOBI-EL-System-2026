      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="flex gap-1 bg-card border border-border/50 rounded-xl p-1">
          <button onClick={() => setEntityType("customer")}
            className={`px-4 py-2 rounded-lg text-sm transition-colors ${entityType === "customer" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-white"}`}>
            לקוחות
          </button>
          <button onClick={() => setEntityType("vendor")}
            className={`px-4 py-2 rounded-lg text-sm transition-colors ${entityType === "vendor" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-white"}`}>
            ספקים
          </button>
        </div>
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-white font-medium">{r.account_name || "—"}</td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><BookOpen className="w-5 h-5 text-indigo-400" /> {viewDetail.account_name}</h2>
