      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <Card className="bg-gradient-to-br from-green-500/10 to-green-600/5 border-green-500/20">
          <CardContent className="p-4">
            ...
            <p className="text-lg font-bold text-white">{fmt(liquidity.totalCash || 0)}</p>
        ...
        <Card className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border-blue-500/20">
          <CardContent className="p-4">
            ...
            <p className="text-lg font-bold text-white">{liquidity.currentRatio || 0}x</p>
        ...
        <Card className="bg-gradient-to-br from-red-500/10 to-red-600/5 border-red-500/20">
          <CardContent className="p-4">
            ...
            <p className="text-lg font-bold text-white">{overdueStats.overdue_count || 0}</p>
        ...
        <Card className="bg-gradient-to-br from-orange-500/10 to-orange-600/5 border-orange-500/20">
          <CardContent className="p-4">
            ...
            <p className="text-lg font-bold text-white">{fmt(Number(overdueStats.overdue_amount || 0))}</p>
        ...
        <Card className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 border-purple-500/20">
          <CardContent className="p-4">
            ...
            <p className="text-lg font-bold text-white">{fmt(totalFxExposure)}</p>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              {riskHeatMap.map((risk: any, idx: number) => {
                ...
                return (
                  <div key={idx} className={`p-4 rounded-xl border-2 ${rc.bg} border-opacity-30`} style={{ borderColor: HEATMAP_COLORS[risk.level] + "40" }}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <PieChart>
                    <Pie data={fxExposure} cx="50%" cy="50%" outerRadius={90} dataKey="exposure" nameKey="currency"
                      ...
                      {fxExposure.map((_: any, idx: number) => <Cell key={idx} fill={FX_COLORS[idx % FX_COLORS.length]} />)}
                    </Pie>
                  ...
                  {fxExposure.map((fx: any, idx: number) => (
                    <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-slate-800/30">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-white">{fx.currency}</span>
                        <span className="text-xs text-muted-foreground">({fx.docCount} מסמכים)</span>
                      </div>
                      <div className="text-left">
                        <span className="text-sm font-medium text-white">{fmt(fx.exposure)}</span>
                        {fx.currency !== "ILS" && (
            ...
            <CardContent>
              {customerConcentration.length > 0 ? (
                <div className="space-y-2">
                  {customerConcentration.slice(0, 10).map((c: any, idx: number) => (
                    <div key={idx} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-slate-300 truncate max-w-[150px]">{c.name || "—"}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-white font-medium">{fmt(c.total)}</span>
          ...
          <Card className="bg-slate-900/50 border-slate-700/50">
            ...
            <CardContent>
              {supplierConcentration.length > 0 ? (
                <div className="space-y-2">
                  {supplierConcentration.slice(0, 10).map((s: any, idx: number) => (
                    <div key={idx} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={agingBuckets}>
                  ...
                  <Bar dataKey="value" radius={[4, 4, 0, 0]} name="סכום">
                    {agingBuckets.map((entry, idx) => <Cell key={idx} fill={entry.color} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              <div className="space-y-3">
                {agingBuckets.map((bucket, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-slate-800/30">
                    ...
                    <span className="text-sm font-medium text-white">{fmt(bucket.value)}</span>
                  </div>
                ))}
                <div className="flex items-center justify-between p-3 rounded-lg border border-slate-700 font-bold">
                  ...
                  <span className="text-white">{fmt(agingBuckets.reduce((s, b) => s + b.value, 0))}</span>
                </div>
              </div>
            </div>
          </CardContent>
