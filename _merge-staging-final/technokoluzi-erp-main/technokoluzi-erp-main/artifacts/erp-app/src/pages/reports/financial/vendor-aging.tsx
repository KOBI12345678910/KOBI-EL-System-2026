...
import {
  Truck, Search, ArrowUpDown, AlertTriangle, DollarSign,
  Clock, Eye, X, Shield
} from "lucide-react";
...
export default function VendorAgingPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API}/aging-snapshots?type=payable`);
      if (res.ok) {
        const raw = safeArray(await res.json());
        const payables = raw.filter((i: any) => i.snapshot_type === "payable" || !i.snapshot_type);
        setItems(payables.map((r: any, i: number) => ({
          ...r, id: r.id || i + 1,
        ...
        setError("שגיאה בטעינת גיול ספקים");
      }
    } catch (e: any) {
  ...
  const kpis = [
    { label: "סה\"כ לתשלום", value: fmtCurrency(totalOutstanding), icon: DollarSign, color: "text-red-400" },
  ...
  const columns = [
    { key: "entity_name", label: "ספק" },
    { key: "total_outstanding", label: "סה\"כ לתשלום" },
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-2">
            <Truck className="text-red-400 w-6 h-6" /> גיול ספקים (AP Aging)
          </h1>
          <p className="text-sm text-muted-foreground mt-1">חובות לספקים לפי טווחי ימים — ניהול תשלומים</p>
        </div>
        <ExportDropdown
          data={filtered}
          headers={{ entity_name: "ספק", total_outstanding: "לתשלום", current_amount: "שוטף", days_1_30: "1-30", days_31_60: "31-60", days_61_90: "61-90", risk_level: "סיכון" }}
          filename="vendor_aging"
        />
      </div>
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש לפי שם ספק..."
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">אין נתוני גיול ספקים</p>
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-red-400 font-bold">{fmtCurrency(r.total_outstanding)}</td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><Truck className="w-5 h-5 text-red-400" /> {viewDetail.entity_name}</h2>
                <button onClick={() => setViewDetail(null)} className="p-1 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-5 grid grid-cols-2 gap-4">
                <DetailField label="ספק" value={viewDetail.entity_name} />
                <DetailField label={'סה"כ לתשלום'} value={fmtCurrency(viewDetail.total_outstanding)} />
