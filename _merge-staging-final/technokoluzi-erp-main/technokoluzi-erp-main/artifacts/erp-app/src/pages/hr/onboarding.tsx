          ...
          <tbody>
            {pagination.paginate(filtered).map(item => (
              <tr key={item.id} className="border-b border-border/50 hover:bg-muted/30">
                ...
                <td className="p-3"><span className={`px-2 py-1 rounded-full text-xs ${statusColor(item.status)}`}>{item.status}</span></td>
                <td className="p-3">
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(item)} className="p-1.5 hover:bg-muted rounded"><Edit2 size={14} /></button>
                    <button onClick={async()=>{if(await globalConfirm(`למחוק את '${item.employee_name || item.id}'? פעולה זו אינה ניתנת לביטול.`))remove(item.id)}} className="p-1.5 hover:bg-red-500/20 rounded text-red-400"><Trash2 size={14} /></button>
