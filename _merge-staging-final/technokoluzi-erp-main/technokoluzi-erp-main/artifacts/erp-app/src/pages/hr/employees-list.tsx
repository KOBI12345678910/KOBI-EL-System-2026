...
import { Users, Search, Plus, Edit2, Trash2, ArrowUpDown, Building2, Phone, Mail, UserCheck, UserX, Clock, Award, Hash, X, Eye, Briefcase, CreditCard, Heart, MapPin, Calendar, Loader2 } from "lucide-react";
...
export default function EmployeesListPage() {
  ...
  const formValidation = useFormValidation({
    full_name: [{ type: "required", message: "שם מלא נדרש" }],
    employee_id: [{ type: "required", message: "מספר עובד נדרש" }],
    id_number: [{ type: "required", message: "תעודת זהות נדרשת" }],
  });
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => printPage("עובדים")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Printer size={16} /> הדפסה</button>
          <button onClick={() => sendByEmail("עובדים - טכנו-כל עוזי", generateEmailBody("עובדים", exportData, { name: "שם", department: "מחלקה", job_title: "תפקיד", status: "סטטוס" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Send size={16} /> שליחה</button>
          <div className="flex border rounded-lg overflow-hidden">
            <button onClick={() => setViewMode("grid")} className={`px-3 py-2 text-sm ${viewMode === "grid" ? "bg-blue-600 text-white" : "bg-card"}`}>כרטיסים</button>
            <button onClick={() => setViewMode("table")} className={`px-3 py-2 text-sm ${viewMode === "table" ? "bg-blue-600 text-white" : "bg-card"}`}>טבלה</button>
          </div>
          <button onClick={openCreate} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 shadow-lg text-sm"><Plus size={16} /> עובד חדש</button>
        </div>
      </div>
      ...
      viewMode === "grid" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.length === 0 ? <div className="col-span-3 text-center py-16 text-muted-foreground">לא נמצאו עובדים</div> :
          filtered.map(emp => {
            const d = emp.data || {};
        ...
        <div className="bg-card rounded-xl shadow-sm border overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? <tr><td colSpan={11} className="text-center py-8 text-muted-foreground">לא נמצאו עובדים</td></tr> :
              filtered.map(emp => {
                const d = emp.data || {};
                const name = d.full_name || `${d.first_name || ""} ${d.last_name || ""}`.trim() || "ללא שם";
                return (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              <div className="sticky bottom-0 bg-card border-t px-6 py-4 flex justify-end gap-3 rounded-b-2xl">
                ...
                <button onClick={save} disabled={saving} className="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium shadow-lg disabled:opacity-50 flex items-center gap-2">{saving ? <Loader2 size={16} className="animate-spin" /> : null}{saving ? "שומר..." : editing ? "עדכון" : "שמירה"}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
