  ...
  return (
    <div>
      ...
      {item.isDir && isExpanded && children && (
        <div className="border-r border-gray-800/30 mr-2">
          {children.map(child => (
...
export default function KobiIDE() {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#0d1117] text-gray-200 overflow-hidden" dir="ltr">

      <div className="flex flex-1 overflow-hidden">

        <div className="w-12 bg-[#0a0e14] border-l border-gray-800 flex flex-col items-center py-2 gap-1 flex-shrink-0">
          <div className="w-8 h-8 rounded-lg bg-cyan-500/10 flex items-center justify-center mb-2">
            <Code2 className="w-4 h-4 text-cyan-400" />
          </div>

          <button onClick={() => toggleLeftPanel("files")} className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${leftPanel === "files" ? "bg-gray-800 text-cyan-400" : "text-gray-500 hover:text-gray-300 hover:bg-gray-800/50"}`} title="סייר קבצים">
            <FolderTree className="w-4 h-4" />
          </button>
          <button onClick={() => toggleLeftPanel("search")} className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${leftPanel === "search" ? "bg-gray-800 text-cyan-400" : "text-gray-500 hover:text-gray-300 hover:bg-gray-800/50"}`} title="חיפוש">
            <Search className="w-4 h-4" />
          </button>
          <button onClick={() => toggleLeftPanel("git")} className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors relative ${leftPanel === "git" ? "bg-gray-800 text-orange-400" : "text-gray-500 hover:text-gray-300 hover:bg-gray-800/50"}`} title="Git">
          ...
          }} className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${terminalVisible ? "bg-gray-800 text-green-400" : "text-gray-500 hover:text-gray-300 hover:bg-gray-800/50"}`} title="טרמינל">
            <Terminal className="w-4 h-4" />
          </button>
          <button onClick={() => toggleRightPanel("preview")} className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${rightPanel === "preview" ? "bg-gray-800 text-cyan-400" : "text-gray-500 hover:text-gray-300 hover:bg-gray-800/50"}`} title="תצוגה מקדימה">
            <Globe className="w-4 h-4" />
          </button>
          <button onClick={() => toggleRightPanel("chat")} className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${rightPanel === "chat" ? "bg-gray-800 text-purple-400" : "text-gray-500 hover:text-gray-300 hover:bg-gray-800/50"}`} title="קובי AI">
        ...
        {leftPanel && (<>
          <div className="flex flex-col border-l border-gray-800 bg-[#0d1117] flex-shrink-0" style={{ width: sidebarWidth }}>
            {leftPanel === "files" && (<>
            <div className="flex items-center gap-1 px-2 py-1.5 border-b border-gray-800/60">
              ...
              <button onClick={() => handleNewFile(currentPath)} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300" title="קובץ חדש">
                <FilePlus className="w-3 h-3" />
              </button>
              <button onClick={() => handleNewFolder(currentPath)} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300" title="תיקייה חדשה">
                <FolderPlus className="w-3 h-3" />
              </button>
              <button onClick={() => { setCurrentPath("."); setExpandedDirs(new Map()); loadRoot(); }} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300" title="רענן">
            ...
            <div className="px-2 py-1">
              <input
                ...
                placeholder="סנן..."
                className="w-full bg-[#161b22] border border-gray-800 rounded px-2 py-0.5 text-[11px] text-gray-300 placeholder:text-gray-600 focus:outline-none focus:border-cyan-500/40"
              />
            </div>
                  ...
                  {newItemMode && newItemMode.parentPath === currentPath && (
                    <div className="flex items-center gap-1 px-2 py-1">
                      ...
                      <input
                        ...
                        className="flex-1 bg-[#161b22] border border-cyan-500/50 rounded px-1.5 py-0.5 text-[11px] text-gray-200 focus:outline-none"
                      />
                    </div>
                  )}
            ...
            <div className="px-2 py-1.5 border-t border-gray-800/60 text-[9px] text-gray-600 flex items-center justify-between">
              ...
              <div className="flex flex-col flex-1">
                ...
                <div className="p-2">
                  <input
                    ...
                    className="w-full bg-[#161b22] border border-gray-800 rounded px-2 py-1.5 text-[11px] text-gray-300 placeholder:text-gray-600 focus:outline-none focus:border-cyan-500/40"
                    dir="rtl"
                  />
                </div>
                <div className="flex-1 overflow-y-auto px-2">
                  {searchResults.length > 0 && searchResults.map((r, i) => (
                    <button key={i} onClick={() => openFile(r.path)} className="w-full text-right px-2 py-1.5 text-[10px] text-gray-400 hover:text-gray-200 hover:bg-gray-800/30 rounded truncate" dir="ltr">
                      {r.path}
                    </button>
                  ))}
                </div>
            ...
            {leftPanel === "git" && (
              <div className="flex flex-col flex-1 overflow-hidden">
                <div className="flex items-center gap-2 px-2 py-1.5 border-b border-gray-800/60">
                  ...
                  <select
                    ...
                    className="text-[9px] text-gray-400 bg-[#0d1117] border border-gray-800 rounded px-1 py-0.5 font-mono focus:outline-none max-w-[100px]"
                    dir="ltr"
                  >
                    {gitBranches.map(b => (<option key={b} value={b}>{b}</option>))}
                  </select>
                  <div className="flex-1" />
                  <button onClick={loadGitStatus} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300">
                ...
                <div className="p-2 border-b border-gray-800/40">
                  <input
                    ...
                    className="w-full bg-[#161b22] border border-gray-800 rounded px-2 py-1 text-[10px] text-gray-300 placeholder:text-gray-600 focus:outline-none focus:border-orange-500/40 mb-1.5"
                    dir="rtl"
                  ...
                  <div className="flex gap-1 flex-wrap">
                    ...
                    <button onClick={() => gitAction("stash")} disabled={gitLoading} className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] bg-gray-700/50 text-gray-400 hover:bg-gray-700/80 disabled:opacity-30">
                ...
                {gitTab === "changes" && (
                  <div className="flex-1 overflow-y-auto">
                    {gitFiles.length === 0 ? (
                      ...
                      <div key={i} className="flex items-center gap-1 px-2 py-1 hover:bg-gray-800/30 group border-b border-gray-800/10">
                    ...
                    {gitDiffContent && (
                      <div className="border-t border-gray-800 max-h-48 overflow-y-auto bg-[#0a0e14]">
                        <div className="flex items-center px-2 py-0.5 bg-[#161b22] border-b border-gray-800/60 sticky top-0">
                ...
                {gitTab === "history" && (
                  <div className="flex-1 overflow-y-auto">
                    {gitCommits.map((c, i) => (
                      <div key={i} className="flex items-start gap-1.5 px-2 py-1.5 border-b border-gray-800/10 hover:bg-gray-800/20">
          ...
          <div className="w-1 bg-gray-800/30 hover:bg-cyan-500/30 cursor-col-resize flex-shrink-0 transition-colors" onMouseDown={e => startResize("left", e)} />
        </>)}
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
            ...
            <div className="flex items-center bg-[#161b22] border-b border-gray-800 overflow-x-auto flex-shrink-0">
              {openTabs.map((tab, idx) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-[11px] border-l border-gray-800/50 whitespace-nowrap group transition-colors ${
                    idx === activeTabIdx
                      ? "bg-[#0d1117] text-cyan-300 border-t-2 border-t-cyan-400"
                      : "text-gray-400 hover:text-gray-200 hover:bg-[#1c2333] border-t-2 border-t-transparent"
                  }`}
                >
                  ...
                  <button
                    ...
                    className="p-0.5 rounded opacity-0 group-hover:opacity-100 hover:bg-gray-700/50 text-gray-500 hover:text-white"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </button>
          ...
          <div className="flex-1 flex overflow-hidden">
            <div className="flex-1 overflow-hidden min-w-0">
              {activeTab ? (
                <Suspense fallback={<div className="flex items-center justify-center h-full bg-[#0d1117]"><Loader2 className="w-6 h-6 animate-spin text-cyan-400" /></div>}>
                  <MonacoEditor
                    height="100%"
                ...
                <div className="flex flex-col items-center justify-center h-full bg-[#0d1117] text-gray-500">
                  ...
                  <div className="flex flex-col gap-2 text-[11px] text-gray-600">
                    <div className="flex items-center gap-2"><span className="px-1.5 py-0.5 bg-gray-800 rounded font-mono">Ctrl+S</span> שמירה</div>
                    <div className="flex items-center gap-2"><span className="px-1.5 py-0.5 bg-gray-800 rounded font-mono">Ctrl+B</span> סייר קבצים</div>
                    <div className="flex items-center gap-2"><span className="px-1.5 py-0.5 bg-gray-800 rounded font-mono">Ctrl+`</span> טרמינל</div>
                    <div className="flex items-center gap-2"><span className="px-1.5 py-0.5 bg-gray-800 rounded font-mono">Ctrl+P</span> חיפוש</div>
                  </div>
                </div>
              )}
            </div>
          ...
          {terminalVisible && (
            <div className="w-full h-1 bg-gray-800/30 hover:bg-green-500/30 cursor-row-resize flex-shrink-0 transition-colors" onMouseDown={e => startResize("terminal", e)} />
          )}
          ...
          {terminalVisible && terminalTabs.length > 0 && (() => {
            ...
            return (
              <div className="border-t border-gray-800 bg-[#0a0e14] flex flex-col flex-shrink-0" style={{ height: terminalHeight }}>
                <div className="flex items-center border-b border-gray-800/60">
                  <div className="flex items-center flex-1 overflow-x-auto">
                    {terminalTabs.map((tt, idx) => (
                      <button
                        ...
                        className={`flex items-center gap-1 px-2.5 py-1 text-[10px] border-l border-gray-800/40 group whitespace-nowrap ${
                          idx === termIdx
                            ? "bg-[#0a0e14] text-emerald-300 border-t border-t-emerald-400"
                            : "bg-[#161b22] text-gray-500 hover:text-gray-300 border-t border-t-transparent"
                        }`}
                      >
                  ...
                  <div className="flex items-center gap-1 px-2">
                    <button onClick={createTerminalTab} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-emerald-300" title="טרמינל חדש">
                      <Plus className="w-3 h-3" />
                    </button>
                    <button onClick={() => clearTerminal(termIdx)} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300" title="נקה">
                      <Trash2 className="w-3 h-3" />
                    </button>
                    <button onClick={() => setTerminalVisible(false)} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 border-t border-gray-800/40">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent text-[12px] text-gray-200 placeholder:text-gray-600 focus:outline-none font-mono"
                  />
                  <button
                    onClick={() => runCommand(activeTerm.input, termIdx)}
                    disabled={activeTerm.running || !activeTerm.input.trim()}
                    className="p-1 rounded hover:bg-gray-700/50 text-emerald-400 disabled:opacity-30"
                  >
        ...
        {rightPanel && (
          <>
          <div className="w-1 bg-gray-800/30 hover:bg-purple-500/30 cursor-col-resize flex-shrink-0 transition-colors" onMouseDown={e => startResize("right", e)} />

          <div className="flex flex-col border-l border-gray-800 bg-[#0d1117] flex-shrink-0" style={{ width: rightPanelWidth }}>

            {rightPanel === "preview" && (
              <div className="flex flex-col flex-1 overflow-hidden">
                <div className="flex items-center gap-1 px-2 py-1 border-b border-gray-800/60 bg-[#161b22]">
                  <Globe className="w-3 h-3 text-cyan-400/60" />
                  <span className="text-[10px] text-gray-400">תצוגה מקדימה</span>
                  <div className="flex-1" />
                  <div className="flex items-center gap-0.5 bg-[#0d1117] rounded px-1 py-0.5">
                  ...
                  <button onClick={() => setPreviewKey(k => k + 1)} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300" title="רענן">
                    <RotateCw className="w-3 h-3" />
                  </button>
                  <button onClick={() => window.open(previewUrl, "_blank")} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300" title="פתח בחלון חדש">
                    <ExternalLink className="w-3 h-3" />
                  </button>
                  <button onClick={() => setRightPanel(null)} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300">
                    <X className="w-3 h-3" />
                  </button>
                </div>
                <div className="flex items-center gap-1 px-2 py-1 border-b border-gray-800/40 bg-[#161b22]">
                  <button onClick={() => { if (iframeRef.current) try { iframeRef.current.contentWindow?.history.back(); } catch {} }} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300">
                    <ArrowLeft className="w-3 h-3" />
                  </button>
                  <button onClick={() => { if (iframeRef.current) try { iframeRef.current.contentWindow?.history.forward(); } catch {} }} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300">
                    <ArrowRight className="w-3 h-3" />
                  </button>
                  <button onClick={() => { setPreviewUrl("/"); setPreviewUrlInput("/"); setPreviewKey(k => k + 1); }} className="p-0.5 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300">
                  ...
                  <input
                    ...
                    className="flex-1 bg-[#0d1117] border border-gray-800 rounded px-2 py-0.5 text-[11px] text-gray-300 font-mono focus:outline-none focus:border-cyan-500/40"
                    dir="ltr"
                  />
                </div>
                <div className="flex-1 overflow-hidden flex items-center justify-center bg-[#1a1f2e]">
                  <div className={`h-full transition-all duration-200 ${previewDevice === "mobile" ? "w-[375px] border-x border-gray-700" : previewDevice === "tablet" ? "w-[768px] border-x border-gray-700" : "w-full"}`}>
                    <iframe ref={iframeRef} key={previewKey} src={previewUrl} className="w-full h-full bg-[#1a1f2e]" sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-modals" title="תצוגה מקדימה" />
                  </div>
                </div>
                <div className="px-2 py-1 border-t border-gray-800/60 text-[9px] text-gray-600 flex items-center gap-2">
            ...
            {rightPanel === "chat" && (
              <div className="flex flex-col flex-1 overflow-hidden">
                <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-800/60 bg-[#161b22]">
                  ...
                  <div className="flex-1" />
                  <button onClick={() => { setChatMessages([]); setChatSessionId(null); }} className="p-1 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300" title="שיחה חדשה">
                    <Plus className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => setRightPanel(null)} className="p-1 rounded hover:bg-gray-700/50 text-gray-500 hover:text-gray-300">
                ...
                >
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center opacity-60">
                      ...
                      <div className="mt-4 space-y-1.5 w-full max-w-[250px]">
                        {["תקן את הבאג הזה", "הסבר את הקוד", "ערוך את הקובץ הפתוח", "נתח תמונה שאעלה", "שפר את הביצועים"].map(s => (
                          <button key={s} onClick={() => { setChatInput(s); }} className="w-full text-right px-3 py-1.5 rounded-lg border border-gray-800 text-[11px] text-gray-400 hover:text-gray-200 hover:border-purple-500/30 hover:bg-purple-500/5 transition-colors">
                            {s}
                          </button>
                        ))}
                      </div>
                  ...
                  {chatMessages.map((msg, i) => (
                    <div key={i} className={`flex gap-2 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                      ...
                      <div className="max-w-[85%] space-y-2">
                        <div className={`rounded-lg px-3 py-2 text-xs leading-relaxed ${msg.role === "user" ? "bg-cyan-500/10 text-gray-200 border border-cyan-500/20" : "bg-[#161b22] text-gray-300 border border-gray-800"}`}>
                          {msg.images && msg.images.length > 0 && (
                            <div className="flex flex-wrap gap-1.5 mb-2">
                              {msg.images.map((img, ii) => (
                                <div key={ii} className="relative group/img">
                                  <img src={img.preview_url} alt={img.name} className="w-20 h-20 object-cover rounded-md border border-gray-700/50 cursor-pointer hover:border-purple-400/50 transition-colors" onClick={() => window.open(img.preview_url, "_blank")} />
                        ...
                        {msg.diffs && msg.diffs.length > 0 && (
                          <div className="space-y-2">
                            {msg.diffs.map((diff, di) => (
                              <div key={di} className="rounded-lg border border-gray-800 overflow-hidden bg-[#0d1117]">
                                <div className="flex items-center gap-1.5 px-2 py-1 bg-[#161b22] border-b border-gray-800/60">
                                ...
                                <div className="overflow-x-auto max-h-48 overflow-y-auto" dir="ltr">
                                  {diff.lines.map((dl, dli) => (
                                    <div key={dli} className={`flex text-[10px] font-mono leading-5 ${dl.type === "add" ? "bg-green-500/10 text-green-300" : dl.type === "remove" ? "bg-red-500/10 text-red-300" : "text-gray-500"}`}>
                                      <span className="w-8 text-right px-1 text-gray-600 select-none flex-shrink-0 border-l border-gray-800/40">{dl.lineNum || ""}</span>
                  ...
                  {chatLoading && chatMessages.length > 0 && chatMessages[chatMessages.length - 1].role !== "assistant" && (
                    <div className="flex gap-2">
                      ...
                      <div className="bg-[#161b22] rounded-lg px-3 py-2 border border-gray-800">
                ...
                {activeTab && (
                  <div className="px-3 py-1 border-t border-gray-800/40 flex items-center gap-1.5" dir="rtl">
                    <Code2 className="w-3 h-3 text-gray-600" />
                    <span className="text-[9px] text-gray-600 truncate">הקשר: {activeTab.path}</span>
                  </div>
                )}

                <div className="p-2 border-t border-gray-800/60 bg-[#161b22]">
                  {chatImages.length > 0 && (
                  ...
                  <div className="flex items-end gap-1.5" dir="rtl">
                    <textarea
                      ...
                      rows={2}
                      className="flex-1 bg-[#0d1117] border border-gray-800 rounded-lg px-3 py-2 text-xs text-gray-300 placeholder:text-gray-600 focus:outline-none focus:border-purple-500/40 resize-none"
                      dir="rtl"
                    />
                    <div className="flex flex-col gap-1">
                      <button onClick={() => chatFileInputRef.current?.click()} className="p-2 rounded-lg bg-gray-800/50 text-gray-400 hover:text-purple-400 hover:bg-purple-500/10 transition-colors" title="העלה תמונה">
      ...
      <div className="h-6 bg-[#161b22] border-t border-gray-800 flex items-center px-3 text-[10px] text-gray-500 gap-4 flex-shrink-0">
        ...
        <button onClick={saveFile} disabled={!activeTab?.isDirty || saving} className="flex items-center gap-1 px-2 py-0.5 bg-emerald-600/80 hover:bg-emerald-500 disabled:opacity-30 text-gray-200 rounded text-[10px]">
      ...
      {ctxMenu && ctxMenu.item && (
        <div
          className="fixed z-50 bg-[#1c2333] border border-gray-700 rounded-lg shadow-2xl py-1 min-w-[180px]"
      ...
      {renameTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setRenameTarget(null)}>
          <div className="bg-[#1c2333] border border-gray-700 rounded-lg p-4 w-80" onClick={e => e.stopPropagation()}>
            ...
            <input
              ...
              className="w-full bg-[#0d1117] border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-cyan-500/50 mb-3"
            />
            <div className="flex gap-2 justify-end">
              <button onClick={() => setRenameTarget(null)} className="px-3 py-1 text-xs text-gray-400 hover:text-white rounded hover:bg-gray-700/50">ביטול</button>
              <button onClick={submitRename} className="px-3 py-1 text-xs bg-cyan-600 hover:bg-cyan-500 text-white rounded">שנה</button>
            </div>
          </div>
        </div>
      )}
