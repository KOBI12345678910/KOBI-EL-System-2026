...
import { authFetch } from "@/lib/utils";
  ...
  const STATUSES = ["תכנון", "ביצוע", "השלמה", "מסירה", "אחריות"] as const;
  const SC: Record<string, string> = { "תכנון": "bg-gray-500/20 text-gray-300", "ביצוע": "bg-blue-500/20 text-blue-300", "השלמה": "bg-yellow-500/20 text-yellow-300", "מסירה": "bg-green-500/20 text-green-300", "אחריות": "bg-purple-500/20 text-purple-300" };
  const BUILDINGS = ["בניין A","בניין B","בניין C","בניין D","משותף","תשתיות"];
  const EMP = ["יוסי כהן","שרה לוי","דוד מזרחי","רחל אברהם","אלון גולדשטיין","מיכל ברק","עומר חדד","נועה פרידמן","איתן רוזנברג","תמר שלום"];
export default function Kiryati10() {
  ...
  const [search, setSearch] = useState("");
    ...
    const filtered = useMemo(() => {
      let d = [...data];
      if (search) { const s = search.toLowerCase(); d = d.filter(r => r.task.includes(s)||r.id.toLowerCase().includes(s)||r.building.includes(s)); }
      if (statusFilter !== "all") d = d.filter(r => r.status === statusFilter);
      if (bldFilter !== "all") d = d.filter(r => r.building === bldFilter);
      return d;
    }, [search, statusFilter, bldFilter]);
    ...
    const af = [statusFilter !== "all", bldFilter !== "all"].filter(Boolean).length;
    ...
    return (
      ...
      {error && <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-red-400"><AlertCircle className="h-4 w-4 flex-shrink-0" /><span className="text-sm">{error}</span><button onClick={() => setError(null)} className="mr-auto"><X className="h-4 w-4" /></button></div>}
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingUp className="h-6 w-6 text-blue-400" />קריית י' — פרויקט דגל</h1><p className="text-sm text-muted-foreground mt-1">ניהול פרויקט קריית י' — בינוי, התקנות ומעקב</p></div>
          <div className="flex gap-2">
            ...
            <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2"><Plus className="h-4 w-4" />משימה חדש</Button>
          </div>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {[{"l":"משימות","v":"48","c":"text-blue-400","t":"+6","u":true},{"l":"בביצוע","v":"15","c":"text-cyan-400","t":"+4","u":true},{"l":"תקציב","v":"₪12M","c":"text-emerald-400","t":"+8%","u":true},{"l":"ניצול","v":"₪7.5M","c":"text-orange-400","t":"+15%","u":false},{"l":"התקדמות","v":"62%","c":"text-purple-400","t":"+8%","u":true},{"l":"מסירות","v":"8","c":"text-green-400","t":"+3","u":true}].map((k: any, i: number) => (
        ...
        <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
          ...
          <select value={bldFilter} onChange={e => { setBldFilter(e.target.value); setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הבניינים</option>{BUILDINGS.map((o: string) => <option key={o} value={o}>{o}</option>)}</select>
          {af > 0 && <Button variant="ghost" size="sm" onClick={() => { setStatusFilter("all"); setBldFilter("all"); setSearch(""); }} className="text-red-400 hover:text-red-300 gap-1"><X className="h-3 w-3" />נקה ({af})</Button>}
        ...
        <th className="p-3 text-right text-muted-foreground font-medium">משימה</th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium">בוצע</th>
        <th className="p-3 text-right text-muted-foreground font-medium">התקדמות</th>
        <th className="p-3 text-right text-muted-foreground font-medium">דדליין</th>
        <th className="p-3 text-right text-muted-foreground font-medium">סטטוס</th>
          ...
          {pd.map(row => (
            ...
            <td className="p-3 "><div className="font-medium text-white">{row.task}</div></td>
            <td className="p-3 text-muted-foreground">{row.building}</td>
            <td className="p-3 text-muted-foreground text-xs">{row.floor}</td>
            <td className="p-3 text-muted-foreground text-xs">{row.contractor}</td>
            <td className="p-3 font-mono text-white text-xs">{fmt(row.budget)}</td>
            <td className="p-3 font-mono text-orange-400 text-xs">{fmt(row.spent)}</td>
            <td className="p-3 "><div className="flex items-center gap-2"><div className="w-16 h-1.5 bg-[#2a2a3e] rounded-full overflow-hidden"><div className="h-full rounded-full bg-blue-500" style={{width:`${row.progress}%`}} /></div><span className="text-xs text-muted-foreground">{row.progress}%</span></div></td>
            <td className="p-3 text-muted-foreground text-xs">{new Date(row.deadline).toLocaleDateString("he-IL")}</td>
        ...
        {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]"><h2 className="text-lg font-bold text-white">{editId ? "עריכה" : "משימה חדש"}</h2><Button variant="ghost" size="sm" onClick={()=>{setShowCreate(false);setEditId(null)}}><X className="h-4 w-4" /></Button></div>
          <div className="p-4 space-y-6">
            <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי משימה</h3></div>
            <div className="grid grid-cols-3 gap-4">
            <div><Label className="text-muted-foreground text-xs">משימה *</Label><Input type="text" placeholder="תיאור" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">בניין *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>בניין A</option><option>בניין B</option><option>בניין C</option><option>בניין D</option><option>משותף</option><option>תשתיות</option></select></div>
            <div><Label className="text-muted-foreground text-xs">קומה</Label><Input type="text" placeholder="קומה 1" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">קבלן</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>מסגריית השרון</option><option>אלומיניום פלוס</option><option>זכוכית אופטימל</option><option>חשמל-טק</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">דדליין *</Label><Input type="date" placeholder="" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">עדיפות</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>גבוה</option><option>בינוני</option><option>נמוך</option></select></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>תכנון</option><option>ביצוע</option><option>השלמה</option><option>מסירה</option><option>אחריות</option></select></div>
            </div>
          <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">מעקב</h3></div>
            <div className="grid grid-cols-3 gap-4">
        ...
        {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">משימה</p><p className="text-white mt-1 font-medium">{dr.task}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">קבלן</p><p className="text-white mt-1 font-medium">{dr.contractor}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">בוצע</p><p className="text-white mt-1 font-medium">{fmt(dr.spent)}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">דדליין</p><p className="text-white mt-1 font-medium">{new Date(dr.deadline).toLocaleDateString("he-IL")}</p></div>
