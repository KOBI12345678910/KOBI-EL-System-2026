...
router.get("/finance/dashboard", async (_req: Request, res: Response) => {
  try {
    ...
    const useDynamic = apEntityId && arEntityId;

    if (useDynamic) {
      const hasRecords = await safeQuery(`SELECT 1 FROM entity_records WHERE entity_id = ${apEntityId} LIMIT 1`);
      if (hasRecords.length > 0) {
        const projEntityId = await getEntityIdBySlugFinance("finance-projects");
        const [apSummary, arSummary, expSummary, cashSummary, projSummary,
               recentAP, recentAR, recentExp, overdueAP, overdueAR, monthlyExp, bankList] = await Promise.all([
          dynamicEntitySummary("accounts-payable", "amount"),
          dynamicEntitySummary("accounts-receivable", "amount", " AND status != 'written_off'"),
          (async () => {
            if (!expEntityId) return {};
            const rows = await safeQuery(`SELECT
              COUNT(*) as total_expenses,
              COALESCE(SUM((data->>'amount')::numeric), 0) as total_amount,
              COALESCE(SUM((data->>'amount')::numeric) FILTER (WHERE data->>'expense_date' >= to_char(date_trunc('month', CURRENT_DATE), 'YYYY-MM-DD')), 0) as this_month,
              COALESCE(SUM((data->>'amount')::numeric) FILTER (WHERE data->>'expense_date' >= to_char(date_trunc('month', CURRENT_DATE) - INTERVAL '1 month', 'YYYY-MM-DD') AND data->>'expense_date' < to_char(date_trunc('month', CURRENT_DATE), 'YYYY-MM-DD')), 0) as last_month
            FROM entity_records WHERE entity_id = ${expEntityId} AND status NOT IN ('cancelled','rejected')`);
            return rows[0] || {};
          })(),
          (async () => {
            if (!bankEntityId) return {};
            const rows = await safeQuery(`SELECT
              COALESCE(SUM((data->>'current_balance')::numeric), 0) as total_cash
            FROM entity_records WHERE entity_id = ${bankEntityId} AND (data->>'is_active' = 'true' OR data->>'is_active' IS NULL)`);
            return rows[0] || {};
          })(),
          (async () => {
            if (!projEntityId) return {};
            const rows = await safeQuery(`SELECT
              COUNT(*) as total_projects,
              COUNT(*) FILTER (WHERE status = 'active') as active_projects,
              COALESCE(SUM((data->>'actual_revenue')::numeric), 0) as total_revenue,
              COALESCE(SUM((data->>'actual_cost')::numeric), 0) as total_cost,
              CASE WHEN SUM((data->>'actual_revenue')::numeric) > 0 THEN ROUND(((SUM((data->>'actual_revenue')::numeric) - SUM((data->>'actual_cost')::numeric)) / SUM((data->>'actual_revenue')::numeric) * 100)::numeric, 1) ELSE 0 END as avg_margin
            FROM entity_records WHERE entity_id = ${projEntityId}`);
            return rows[0] || {};
          })(),
          (async () => {
            if (!apEntityId) return [];
            return safeQuery(`SELECT id, data->>'invoice_number' as invoice_number, data->>'supplier_name' as supplier_name, (data->>'amount')::numeric as amount, (data->>'balance_due')::numeric as balance_due, data->>'due_date' as due_date, status FROM entity_records WHERE entity_id = ${apEntityId} AND status != 'cancelled' ORDER BY created_at DESC LIMIT 10`);
          })(),
          (async () => {
            if (!arEntityId) return [];
            return safeQuery(`SELECT id, data->>'invoice_number' as invoice_number, data->>'customer_name' as customer_name, (data->>'amount')::numeric as amount, (data->>'balance_due')::numeric as balance_due, data->>'due_date' as due_date, status FROM entity_records WHERE entity_id = ${arEntityId} AND status != 'cancelled' ORDER BY created_at DESC LIMIT 10`);
          })(),
          (async () => {
            if (!expEntityId) return [];
            return safeQuery(`SELECT id, data->>'category' as category, data->>'description' as description, (data->>'amount')::numeric as amount, data->>'expense_date' as expense_date, status FROM entity_records WHERE entity_id = ${expEntityId} AND status != 'cancelled' ORDER BY created_at DESC LIMIT 10`);
          })(),
          (async () => {
            if (!apEntityId) return [];
            return safeQuery(`SELECT id, data->>'invoice_number' as invoice_number, data->>'supplier_name' as supplier_name, (data->>'balance_due')::numeric as balance_due, data->>'due_date' as due_date FROM entity_records WHERE entity_id = ${apEntityId} AND (status = 'overdue' OR (status IN ('open','partial') AND (data->>'due_date')::date < CURRENT_DATE)) ORDER BY (data->>'due_date')::date ASC LIMIT 10`);
          })(),
          (async () => {
            if (!arEntityId) return [];
            return safeQuery(`SELECT id, data->>'invoice_number' as invoice_number, data->>'customer_name' as customer_name, (data->>'balance_due')::numeric as balance_due, data->>'due_date' as due_date FROM entity_records WHERE entity_id = ${arEntityId} AND (status = 'overdue' OR (status IN ('open','partial') AND (data->>'due_date')::date < CURRENT_DATE)) ORDER BY (data->>'due_date')::date ASC LIMIT 10`);
          })(),
          (async () => {
            if (!expEntityId) return [];
            return safeQuery(`SELECT
              to_char((data->>'expense_date')::date, 'YYYY-MM') as month,
              data->>'category' as category,
              SUM((data->>'amount')::numeric) as total
            FROM entity_records WHERE entity_id = ${expEntityId} AND status NOT IN ('cancelled','rejected') AND (data->>'expense_date')::date >= CURRENT_DATE - INTERVAL '6 months'
            GROUP BY month, category ORDER BY month DESC`);
          })(),
          (async () => {
            if (!bankEntityId) return [];
            return safeQuery(`SELECT id, data->>'bank_name' as bank_name, data->>'account_number' as account_number, data->>'account_type' as account_type, (data->>'current_balance')::numeric as current_balance, (data->>'available_balance')::numeric as available_balance, (data->>'credit_limit')::numeric as credit_limit FROM entity_records WHERE entity_id = ${bankEntityId} AND (data->>'is_active' = 'true' OR data->>'is_active' IS NULL) ORDER BY data->>'bank_name'`);
          })(),
        ]);
        res.json({
          ...
          recentPayables: recentAP,
          recentReceivables: recentAR,
    ...
    const [
      ...
      safeQuery(`SELECT 
        ...
        COALESCE(SUM(amount) FILTER (WHERE expense_date >= date_trunc('month', CURRENT_DATE) - INTERVAL '1 month' AND expense_date < date_trunc('month', CURRENT_DATE)),0) as last_month
      FROM expenses WHERE status != 'cancelled' AND status != 'rejected'`),
      safeQuery(`SELECT COALESCE(SUM(current_balance),0) as total_cash FROM bank_accounts WHERE is_active = true`),
      safeQuery(`SELECT 
        COUNT(*) as total_projects,
        COUNT(*) FILTER (WHERE status='active') as active_projects,
      ...
      FROM projects`),
      safeQuery(`SELECT id, invoice_number, supplier_name, amount, balance_due, due_date, status FROM accounts_payable WHERE status != 'cancelled' ORDER BY created_at DESC LIMIT 10`),
      safeQuery(`SELECT id, invoice_number, customer_name, amount, balance_due, due_date, status FROM accounts_receivable WHERE status != 'cancelled' ORDER BY created_at DESC LIMIT 10`),
      safeQuery(`SELECT id, expense_number, category, description, amount, expense_date, status FROM expenses WHERE status != 'cancelled' ORDER BY created_at DESC LIMIT 10`),
      safeQuery(`SELECT id, invoice_number, supplier_name, balance_due, due_date FROM accounts_payable WHERE status = 'overdue' OR (status IN ('open','partial') AND due_date < CURRENT_DATE) ORDER BY due_date ASC LIMIT 10`),
      safeQuery(`SELECT id, invoice_number, customer_name, balance_due, due_date FROM accounts_receivable WHERE status = 'overdue' OR (status IN ('open','partial') AND due_date < CURRENT_DATE) ORDER BY due_date ASC LIMIT 10`),
      safeQuery(`SELECT 
        TO_CHAR(expense_date, 'YYYY-MM') as month,
        category,
        SUM(amount) as total
      FROM expenses WHERE status NOT IN ('cancelled','rejected') AND expense_date >= CURRENT_DATE - INTERVAL '6 months'
      GROUP BY month, category ORDER BY month DESC`),
      safeQuery(`SELECT id, bank_name, account_number, account_type, current_balance, available_balance, credit_limit FROM bank_accounts WHERE is_active = true ORDER BY bank_name`),
    ]);

    res.json({
      accountsPayable: apSummary[0] || {},
      accountsReceivable: arSummary[0] || {},
