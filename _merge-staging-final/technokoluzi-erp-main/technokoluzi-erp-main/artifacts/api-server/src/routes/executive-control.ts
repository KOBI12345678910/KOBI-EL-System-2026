...
router.get("/executive/ceo-dashboard", async (_req: Request, res: Response) => {
  try {
    ...
    const [rev, exp, inv, po, cust, emp, leads, proj, wo, qc, bank, quot,
      ...
      q("SELECT COALESCE(SUM(total_amount),0) as total, COUNT(*) as cnt FROM sales_orders WHERE status NOT IN ('cancelled','draft')"),
      q("SELECT COALESCE(SUM(amount),0) as total, COUNT(*) as cnt FROM expenses"),
      q("SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE status='paid') as paid, COALESCE(SUM(total_amount),0) as amount, COALESCE(SUM(CASE WHEN status='paid' THEN total_amount ELSE 0 END),0) as paid_amount FROM customer_invoices"),
      q("SELECT COUNT(*) as total, COALESCE(SUM(total_amount),0) as amount FROM purchase_orders WHERE status NOT IN ('cancelled','draft')"),
      q("SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE status IN ('active','פעיל')) as active FROM customers"),
      q("SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE status IN ('active','פעיל')) as active FROM employees"),
      q("SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE status='converted') as converted FROM leads"),
      ...
      q("SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE status='completed') as completed, COUNT(*) FILTER(WHERE status='in_progress') as in_progress, COUNT(*) FILTER(WHERE status='planned') as planned FROM production_work_orders"),
      q("SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE result='pass') as passed FROM qc_inspections"),
      ...
      q("SELECT COALESCE(SUM(total_amount),0) as total FROM sales_orders WHERE TO_CHAR(created_at,'YYYY-MM')=$1 AND status NOT IN ('cancelled','draft')", [thisMonth]),
      q("SELECT COALESCE(SUM(total_amount),0) as total FROM sales_orders WHERE TO_CHAR(created_at,'YYYY-MM')=$1 AND status NOT IN ('cancelled','draft')", [lastMonthStr]),
      q("SELECT COALESCE(SUM(amount),0) as total FROM expenses WHERE TO_CHAR(created_at,'YYYY-MM')=$1", [thisMonth]),
      q("SELECT COALESCE(SUM(amount),0) as total FROM expenses WHERE TO_CHAR(created_at,'YYYY-MM')=$1", [lastMonthStr]),
      q(`SELECT TO_CHAR(created_at,'YYYY-MM') as month, COALESCE(SUM(total_amount),0) as total
         FROM sales_orders WHERE status NOT IN ('cancelled','draft') AND created_at > NOW()-INTERVAL '12 months'
         GROUP BY month ORDER BY month`),
      q("SELECT COALESCE(category,'אחר') as name, COALESCE(SUM(amount),0) as value FROM expenses GROUP BY category ORDER BY value DESC LIMIT 8"),
      q("SELECT status, COUNT(*) as cnt FROM production_work_orders GROUP BY status"),
      q("SELECT COUNT(*) as cnt, COALESCE(SUM(total_amount),0) as total FROM customer_invoices WHERE status NOT IN ('paid','cancelled') AND due_date < NOW()"),
      q("SELECT COUNT(*) as cnt FROM customers WHERE TO_CHAR(created_at,'YYYY-MM')=$1", [thisMonth]),
      q("SELECT COUNT(*) as cnt FROM customers WHERE TO_CHAR(created_at,'YYYY-MM')=$1", [lastMonthStr]),
      q("SELECT COUNT(*) as cnt FROM production_work_orders WHERE TO_CHAR(created_at,'YYYY-MM')=$1 AND status='completed'", [thisMonth]),
      q("SELECT COUNT(*) as cnt FROM production_work_orders WHERE TO_CHAR(created_at,'YYYY-MM')=$1 AND status='completed'", [lastMonthStr]),
      q(`SELECT TO_CHAR(created_at,'YYYY-MM') as month, COALESCE(SUM(amount),0) as total
         FROM expenses WHERE created_at > NOW()-INTERVAL '12 months'
         GROUP BY month ORDER BY month`),
  ...
  try {
    const [fin, ops, sales, hr, quality] = await Promise.all([
      q(`SELECT COALESCE(SUM(CASE WHEN t='r' THEN amt ELSE 0 END),0) as revenue, COALESCE(SUM(CASE WHEN t='e' THEN amt ELSE 0 END),0) as expenses,
         ...
         COALESCE((SELECT SUM(total_amount) FROM customer_invoices WHERE status !='paid' AND due_date < NOW()),0) as overdue_ar,
         COALESCE((SELECT SUM(total_amount) FROM supplier_invoices WHERE status !='paid' AND due_date < NOW()),0) as overdue_ap
         FROM (SELECT 'r' as t, total_amount as amt FROM sales_orders WHERE status NOT IN ('cancelled','draft')
               UNION ALL SELECT 'e', amount FROM expenses) x`),
      q(`SELECT COUNT(*) as total_wo, COUNT(*) FILTER(WHERE status='completed') as done_wo,
         COUNT(*) FILTER(WHERE status='delayed' OR (estimated_completion < NOW() AND status NOT IN ('completed','cancelled'))) as delayed_wo,
         (SELECT COUNT(*) FROM production_work_orders WHERE status IN ('in_progress','pending')) as active_wo
      ...
      q(`SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE status IN ('active','פעיל')) as active,
         ...
         FROM employees`),
      q(`SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE result='pass') as passed, COUNT(*) FILTER(WHERE result='fail') as failed FROM qc_inspections`),
    ]);
...
router.get("/executive/kpi-board", async (_req: Request, res: Response) => {
  try {
    ...
    const [revMonth, expMonth, ordMonth, woMonth, custMonth, arAging, invEff, leadConv] = await Promise.all([
      q("SELECT COALESCE(SUM(total_amount),0) as total FROM sales_orders WHERE TO_CHAR(created_at,'YYYY-MM')=$1 AND status NOT IN ('cancelled','draft')", [thisMonth]),
      q("SELECT COALESCE(SUM(amount),0) as total FROM expenses WHERE TO_CHAR(created_at,'YYYY-MM')=$1", [thisMonth]),
      q("SELECT COUNT(*) as cnt FROM sales_orders WHERE TO_CHAR(created_at,'YYYY-MM')=$1", [thisMonth]),
      q("SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE status='completed') as done FROM production_work_orders WHERE TO_CHAR(created_at,'YYYY-MM')=$1", [thisMonth]),
      q("SELECT COUNT(*) as new_cust FROM customers WHERE TO_CHAR(created_at,'YYYY-MM')=$1", [thisMonth]),
      q(`SELECT COUNT(*) FILTER(WHERE due_date < NOW()-INTERVAL '90 days' AND status!='paid') as over90,
         COUNT(*) FILTER(WHERE due_date < NOW()-INTERVAL '60 days' AND due_date >= NOW()-INTERVAL '90 days' AND status!='paid') as d60_90,
         COUNT(*) FILTER(WHERE due_date < NOW()-INTERVAL '30 days' AND due_date >= NOW()-INTERVAL '60 days' AND status!='paid') as d30_60,
         COUNT(*) FILTER(WHERE due_date < NOW() AND due_date >= NOW()-INTERVAL '30 days' AND status!='paid') as d0_30
         FROM customer_invoices`),
      q("SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE status='paid') as paid FROM customer_invoices WHERE TO_CHAR(created_at,'YYYY-MM')=$1", [thisMonth]),
      q("SELECT COUNT(*) as total, COUNT(*) FILTER(WHERE status='converted') as converted FROM leads WHERE TO_CHAR(created_at,'YYYY-MM')=$1", [thisMonth]),
    ]);
...
router.get("/executive/live-alerts", async (_req: Request, res: Response) => {
  try {
    ...
    const [overdueInv, lowStock, delayedWo, overdueAp, stalLeads, pendQuot, failedQc] = await Promise.all([
      q("SELECT id, customer_name, total_amount, due_date FROM customer_invoices WHERE status!='paid' AND due_date < NOW() ORDER BY due_date LIMIT 10"),
      q("SELECT id, name, quantity, minimum_quantity FROM raw_materials WHERE quantity < minimum_quantity ORDER BY (quantity::float/GREATEST(minimum_quantity,1)) LIMIT 10"),
      q("SELECT id, order_number, product_name, estimated_completion FROM production_work_orders WHERE status NOT IN ('completed','cancelled') AND estimated_completion < NOW() ORDER BY estimated_completion LIMIT 10"),
      q("SELECT id, supplier_name, total_amount, due_date FROM supplier_invoices WHERE status!='paid' AND due_date < NOW() ORDER BY due_date LIMIT 10"),
      q("SELECT id, name, company_name, created_at FROM leads WHERE status='new' AND created_at < NOW()-INTERVAL '14 days' ORDER BY created_at LIMIT 10"),
      ...
      q("SELECT id, product_name, inspection_date, result FROM qc_inspections WHERE result='fail' AND inspection_date > NOW()-INTERVAL '7 days' ORDER BY inspection_date DESC LIMIT 5"),
    ]);
...
router.get("/executive/financial-risk", async (_req: Request, res: Response) => {
  try {
    const [ar, ap, cashflow, concentration, budgets] = await Promise.all([
      ...
      q(`SELECT customer_name, COALESCE(SUM(total_amount),0) as total FROM customer_invoices WHERE status!='paid' GROUP BY customer_name ORDER BY total DESC LIMIT 5`),
...
router.get("/executive/operational-bottlenecks", async (_req: Request, res: Response) => {
  try {
    const [woStatus, woDelayed, longRunning, pendingPO, pendingApproval] = await Promise.all([
      q("SELECT status, COUNT(*) as cnt FROM production_work_orders GROUP BY status"),
      q("SELECT id, order_number, product_name, status, estimated_completion, created_at FROM production_work_orders WHERE status NOT IN ('completed','cancelled') AND estimated_completion < NOW() ORDER BY estimated_completion LIMIT 15"),
      q("SELECT id, order_number, product_name, status, created_at FROM production_work_orders WHERE status IN ('in_progress','pending') AND created_at < NOW()-INTERVAL '30 days' ORDER BY created_at LIMIT 10"),
...
router.get("/executive/delayed-projects", async (_req: Request, res: Response) => {
  try {
    const [projects, delayedSO, delayedWO] = await Promise.all([
      ...
      q(`SELECT id, order_number, product_name, status, quantity_ordered, estimated_completion
         FROM production_work_orders WHERE status NOT IN ('completed','cancelled') AND estimated_completion < NOW() ORDER BY estimated_completion LIMIT 15`),
    ]);
    res.json({
...
router.get("/executive/production-efficiency", async (_req: Request, res: Response) => {
  try {
    const [woStats, monthly, qcStats, machines] = await Promise.all([
      q(`SELECT status, COUNT(*) as cnt, COALESCE(SUM(quantity_ordered),0) as qty, COALESCE(SUM(quantity_produced),0) as produced
         FROM production_work_orders GROUP BY status`),
      q(`SELECT TO_CHAR(created_at,'YYYY-MM') as month, COUNT(*) as total, COUNT(*) FILTER(WHERE status='completed') as completed,
         COALESCE(SUM(quantity_ordered),0) as ordered, COALESCE(SUM(quantity_produced),0) as produced
         FROM production_work_orders WHERE created_at > NOW()-INTERVAL '6 months' GROUP BY month ORDER BY month`),
      q(`SELECT TO_CHAR(inspection_date,'YYYY-MM') as month, COUNT(*) as total, COUNT(*) FILTER(WHERE result='pass') as passed
         FROM qc_inspections WHERE inspection_date > NOW()-INTERVAL '6 months' GROUP BY month ORDER BY month`),
...
router.get("/executive/profitability", async (_req: Request, res: Response) => {
  try {
    const [rev, exp, byCustomer, byCategory, monthly] = await Promise.all([
      q("SELECT COALESCE(SUM(total_amount),0) as total FROM sales_orders WHERE status NOT IN ('cancelled','draft')"),
      q("SELECT COALESCE(SUM(amount),0) as total FROM expenses"),
      q(`SELECT c.name, COALESCE(SUM(so.total_amount),0) as revenue FROM customers c
         JOIN sales_orders so ON so.customer_id=c.id WHERE so.status NOT IN ('cancelled','draft')
         GROUP BY c.name ORDER BY revenue DESC LIMIT 10`),
      q("SELECT COALESCE(category,'אחר') as category, COALESCE(SUM(amount),0) as total FROM expenses GROUP BY category ORDER BY total DESC LIMIT 10"),
