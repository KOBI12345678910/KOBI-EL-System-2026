...
function createEntityRouter(config: EntityConfig): Router {
  const r = Router();
  const { table, entityName, hebrewName, searchFields, filterFields, sortFields, numberField, numberPrefix, statsQuery, hasIsActive = true } = config;
  ...
  r.put('/:id', async (req: Request, res: Response) => {
    try {
      ...
      const fields = { ...req.body, updated_at: new Date() };
