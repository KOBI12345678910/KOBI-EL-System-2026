...
async function nextNum(prefix: string, table: string, col: string) {
  const r = await q(`SELECT ${col} FROM ${table} ORDER BY id DESC LIMIT 1`);
  const last = r[0]?.[col] || `${prefix}0000`;
...
function crudRoutes(basePath: string, table: string, prefix: string, numCol: string, nameField: string, orderCol: string) {
  router.get(`/${basePath}`, async (_req, res) => {
    res.json(await q(`SELECT * FROM ${table} ORDER BY ${orderCol} DESC, id DESC`));
  });
  router.get(`/${basePath}/stats`, async (_req, res) => {
    const rows = await q(`SELECT COUNT(*) as total, COALESCE(SUM(total_amount),0) as total_amount,
      ...
      COUNT(*) FILTER (WHERE status='completed' OR status='approved' OR status='paid') as completed
      FROM ${table}`);
    res.json(rows[0] || {});
  });
  router.post(`/${basePath}`, async (req, res) => {
    ...
    if (table === "customer_refunds") {
      await q(`INSERT INTO customer_refunds (refund_number, refund_date, customer_name, customer_tax_id, invoice_number, reason, amount, vat_rate, vat_amount, total_amount, status, payment_method, notes, created_by, created_by_name)
        VALUES ('${num}', ${d.refundDate ? s(d.refundDate) : "'"+new Date().toISOString().slice(0,10)+"'"}, ${s(d.customerName)}, ${s(d.customerTaxId)}, ${s(d.invoiceNumber)}, ${s(d.reason)}, ${n(d.amount)}, ${n(d.vatRate)||18}, ${n(d.vatAmount)}, ${n(d.totalAmount)}, '${d.status||'pending'}', ${s(d.paymentMethod)}, ${s(d.notes)}, ${user?.id||'NULL'}, ${s(user?.fullName)})`);
      ...
      await q(`INSERT INTO supplier_invoices (invoice_number, invoice_type, invoice_date, due_date, supplier_name, supplier_tax_id, status, currency, subtotal, discount_amount, before_vat, vat_rate, vat_amount, total_amount, amount_paid, payment_terms, payment_method, po_number, item_description, notes, created_by, created_by_name)
        VALUES ('${num}', '${d.invoiceType||'tax_invoice'}', ${d.invoiceDate ? s(d.invoiceDate) : "'"+new Date().toISOString().slice(0,10)+"'"}, ${d.dueDate ? s(d.dueDate) : 'NULL'}, ${s(d.supplierName)}, ${s(d.supplierTaxId)}, '${d.status||'draft'}', '${d.currency||'ILS'}', ${n(d.subtotal)}, ${n(d.discountAmount)}, ${n(d.beforeVat)}, ${n(d.vatRate)||18}, ${n(d.vatAmount)}, ${n(d.totalAmount)}, ${n(d.amountPaid)}, '${d.paymentTerms||'net_30'}', ${s(d.paymentMethod)}, ${s(d.poNumber)}, ${s(d.itemDescription)}, ${s(d.notes)}, ${user?.id||'NULL'}, ${s(user?.fullName)})`);
      ...
      await q(`INSERT INTO supplier_credit_notes (credit_number, credit_date, supplier_name, supplier_tax_id, invoice_number, reason, amount, vat_rate, vat_amount, total_amount, status, notes, created_by, created_by_name)
        VALUES ('${num}', ${d.creditDate ? s(d.creditDate) : "'"+new Date().toISOString().slice(0,10)+"'"}, ${s(d.supplierName)}, ${s(d.supplierTaxId)}, ${s(d.invoiceNumber)}, ${s(d.reason)}, ${n(d.amount)}, ${n(d.vatRate)||18}, ${n(d.vatAmount)}, ${n(d.totalAmount)}, '${d.status||'draft'}', ${s(d.notes)}, ${user?.id||'NULL'}, ${s(user?.fullName)})`);
  ...
  router.put(`/${basePath}/:id`, async (req, res) => {
    ...
    if (sets.length > 1) {
      await q(`UPDATE ${table} SET ${sets.join(",")} WHERE id=${req.params.id}`);
    }
    res.json((await q(`SELECT * FROM ${table} WHERE id=${req.params.id}`))[0]);
  });
  router.delete(`/${basePath}/:id`, async (req, res) => {
    await q(`DELETE FROM ${table} WHERE id=${req.params.id}`);
