...
function extractQuotationContext(data: Record<string, any>): Record<string, any> {
  ...
  if (amount > 0) {
    ...
    enriched.vat_amount = Math.round(amount * 0.17 * 100) / 100;
    enriched.amount_with_vat = Math.round(amount * 1.17 * 100) / 100;
  }
...
function extractFinanceContext(data: Record<string, any>): Record<string, any> {
  ...
  if (amount > 0) {
    ...
    enriched.vat_component = Math.round(amount * 0.17 * 100) / 100;
    enriched.net_revenue = Math.round(amount / 1.17 * 100) / 100;
  }
