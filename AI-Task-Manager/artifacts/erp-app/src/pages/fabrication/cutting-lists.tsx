...
export default function CuttingListsPage() {
  ...
  const handleBulkDelete = async () => {
    await Promise.allSettled([...selected].map(id => authFetch(`/api/cutting-lists/${id}`, { method: "DELETE" })));
  ...
  return (
    <div className="p-6 space-y-4" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><FileSpreadsheet className="h-6 w-6 text-amber-400" />רשימות חיתוך</h1><p className="text-sm text-muted-foreground mt-1">ניהול רשימות חיתוך, אופטימיזציה ומעקב חומרים</p></div>
      ...
      <Card className="bg-card/60 border-border"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
        ...
        <select value={catFilter} onChange={e => { setCatFilter(e.target.value); setPage(1); }} className="bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground"><option value="all">כל החומרים</option>{CATS.map(o => <option key={o} value={o}>{o}</option>)}</select>
      ...
      <Card className="bg-card/80 border-border"><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border bg-background/50">
        ...
        {isLoading ? Array.from({ length: 8 }).map((_, sk) => (<tr key={sk} className="border-b border-border/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-muted rounded" /><div className="h-4 w-16 bg-muted rounded" /><div className="h-4 w-32 bg-muted rounded" /><div className="h-4 w-24 bg-muted rounded" /><div className="h-4 w-20 bg-muted rounded" /><div className="h-4 w-16 bg-muted rounded" /><div className="h-4 w-28 bg-muted rounded" /><div className="h-4 w-12 bg-muted rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4"><FileSpreadsheet className="h-16 w-16 text-muted-foreground" /><p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין רשימות חיתוך"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש" : "צור רשימה חדשה להתחיל"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />רשימה חדשה</Button>}</div></td></tr> : pd.map(row => (
          <tr key={row.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
            ...
            <td className="p-3"><div className="font-medium text-foreground">{row.material || "-"}</div></td>
      ...
      {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => { setShowCreate(false); setEditId(null); }}><div className="bg-card border border-border rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-border"><h2 className="text-lg font-bold text-foreground">{editId ? "עריכת רשימת חיתוך" : "רשימה חדשה"}</h2><Button variant="ghost" size="sm" onClick={() => { setShowCreate(false); setEditId(null); }}><X className="h-4 w-4" /></Button></div>
        <div className="p-4 space-y-6">
          <div className="border-b border-border pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי חיתוך</h3></div>
          <div className="grid grid-cols-3 gap-4">
            <div><Label className="text-muted-foreground text-xs">מספר חיתוך *</Label><Input value={form.cuttingNumber || ""} onChange={e => setForm({ ...form, cuttingNumber: e.target.value })} placeholder="CUT-0001" className="bg-input border-border text-foreground mt-1" /></div>
            ...
            <div><Label className="text-muted-foreground text-xs">חומר *</Label><select value={form.material || ""} onChange={e => setForm({ ...form, material: e.target.value })} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground mt-1"><option value="">בחר...</option>{CATS.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
            <div><Label className="text-muted-foreground text-xs">פרופיל</Label><Input value={form.profileName || ""} onChange={e => setForm({ ...form, profileName: e.target.value })} placeholder="פרופיל 200" className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">כמות</Label><Input type="number" value={form.quantity || ""} onChange={e => setForm({ ...form, quantity: Number(e.target.value) })} placeholder="0" className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">אורך חיתוך (מ״מ)</Label><Input type="number" value={form.cutLengthMm || ""} onChange={e => setForm({ ...form, cutLengthMm: Number(e.target.value) })} placeholder="0" className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">מפעיל</Label><Input value={form.operatorName || ""} onChange={e => setForm({ ...form, operatorName: e.target.value })} placeholder="שם" className="bg-input border-border text-foreground mt-1" /></div>
          ...
          <div><textarea rows={3} value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground resize-none" placeholder="הערות..." /></div>
        </div>
      ...
      {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setShowDetail(null)}><div className="bg-card border border-border rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-border"><div className="flex items-center gap-3"><h2 className="text-lg font-bold text-foreground">{dr.cutting_number || `#${dr.id}`}</h2><Badge className={`${SC[dr.status] || ""} border-0`}>{STATUS_HE[dr.status] || dr.status}</Badge></div><Button variant="ghost" size="sm" onClick={() => setShowDetail(null)}><X className="h-4 w-4" /></Button></div>
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מוצר</p><p className="text-foreground mt-1 font-medium">{dr.product_name || "-"}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">חומר</p><p className="text-foreground mt-1 font-medium">{dr.material || "-"}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">פרופיל</p><p className="text-foreground mt-1 font-medium">{dr.profile_name || "-"}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">כמות</p><p className="text-foreground mt-1 font-medium">{dr.quantity || 0}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">אורך (מ״מ)</p><p className="text-foreground mt-1 font-medium">{dr.cut_length_mm || 0}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">פסולת</p><p className="text-foreground mt-1 font-medium">{dr.waste_percent ? `${dr.waste_percent}%` : "-"}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מפעיל</p><p className="text-foreground mt-1 font-medium">{dr.operator_name || "-"}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">נוצר</p><p className="text-foreground mt-1 font-medium">{dr.created_at ? new Date(dr.created_at).toLocaleDateString("he-IL") : "-"}</p></div>
            <div className="bg-input rounded-lg p-3"><p className="text-[11px] text-muted-foreground">סטטוס</p><p className="text-foreground mt-1 font-medium">{STATUS_HE[dr.status] || dr.status}</p></div>
          </div>
          {dr.notes && <div><h3 className="text-sm font-semibold text-blue-400 border-b border-border pb-2">הערות</h3><p className="text-muted-foreground text-sm mt-2">{dr.notes}</p></div>}
        </div>
