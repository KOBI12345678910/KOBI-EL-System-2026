...
export default function TransportOrdersPage() {
  ...
  const handleBulkDelete = async () => {
    await Promise.allSettled([...selected].map(id => authFetch(`/api/transport-orders/${id}`, { method: "DELETE" })));
  ...
  return (
    <div className="p-6 space-y-4" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><Truck className="h-6 w-6 text-blue-400" />הזמנות הובלה</h1><p className="text-sm text-muted-foreground mt-1">ניהול הובלות, נהגים ומעקב משלוחים</p></div>
      ...
      <Card className="bg-card/80 border-border"><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border bg-background/50">
        ...
        {isLoading ? Array.from({ length: 6 }).map((_, sk) => (<tr key={sk} className="border-b border-border/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-muted rounded" /><div className="h-4 w-20 bg-muted rounded" /><div className="h-4 w-32 bg-muted rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4"><Truck className="h-16 w-16 text-muted-foreground" /><p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין הזמנות הובלה"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />הזמנה חדשה</Button>}</div></td></tr> : pd.map(row => (
      ...
      {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => { setShowCreate(false); setEditId(null); }}><div className="bg-card border border-border rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-border"><h2 className="text-lg font-bold text-foreground">{editId ? "עריכת הזמנת הובלה" : "הזמנה חדשה"}</h2><Button variant="ghost" size="sm" onClick={() => { setShowCreate(false); setEditId(null); }}><X className="h-4 w-4" /></Button></div>
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div><Label className="text-muted-foreground text-xs">מספר הובלה *</Label><Input value={form.transportNumber || ""} onChange={e => setForm({ ...form, transportNumber: e.target.value })} placeholder="TRN-0001" className="bg-input border-border text-foreground mt-1" /></div>
            ...
            <div><Label className="text-muted-foreground text-xs">כתובת יעד</Label><Input value={form.destinationAddress || ""} onChange={e => setForm({ ...form, destinationAddress: e.target.value })} className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">נהג</Label><Input value={form.driverName || ""} onChange={e => setForm({ ...form, driverName: e.target.value })} className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">רכב</Label><Input value={form.vehicle || ""} onChange={e => setForm({ ...form, vehicle: e.target.value })} placeholder="משאית 12T" className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">תאריך</Label><Input type="date" value={form.scheduledDate || ""} onChange={e => setForm({ ...form, scheduledDate: e.target.value })} className="bg-input border-border text-foreground mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">עלות (אגורות)</Label><Input type="number" value={form.transportCost || ""} onChange={e => setForm({ ...form, transportCost: Number(e.target.value) })} className="bg-input border-border text-foreground mt-1" /></div>
      ...
      {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setShowDetail(null)}><div className="bg-card border border-border rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-border"><div className="flex items-center gap-3"><h2 className="text-lg font-bold text-foreground">{dr.transport_number || `#${dr.id}`}</h2><Badge className={`${SC[dr.status] || ""} border-0`}>{STATUS_HE[dr.status] || dr.status}</Badge></div><Button variant="ghost" size="sm" onClick={() => setShowDetail(null)}><X className="h-4 w-4" /></Button></div>
