...
import {
  Target, Search, Plus, Edit2, Trash2, X, Save, Eye,
...
export default function GoalsPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API}/strategy/goals`);
  ...
  const filtered = useMemo(() => {
    let data = items.filter(i =>
      ...
      (!search || [i.title, i.owner, i.category, i.description, i.key_results]
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ status: "not_started", priority: "medium", progress: 0 });
    setShowForm(true);
  };

  const openEdit = (r: any) => {
    setEditing(r);
    setForm({ ...r, target_date: r.target_date?.slice(0, 10) });
    setShowForm(true);
  };
  ...
  const remove = async (id: number) => {
    if (await globalConfirm("למחוק יעד זה? פעולה זו אינה ניתנת לביטול.")) {
      await authFetch(`${API}/strategy/goals/${id}`, { method: "DELETE" });
      load();
    }
  };

  const avgProgress = items.length > 0
    ? (items.reduce((s, i) => s + (i.progress || 0), 0) / items.length).toFixed(0)
    : "0";

  const kpis = [
    { label: "סה\"כ יעדים", value: fmt(items.length), icon: Target, color: "text-blue-400" },
    { label: "בכיוון", value: fmt(items.filter(i => i.status === "on_track").length), icon: CheckCircle2, color: "text-green-400" },
    { label: "בסיכון", value: fmt(items.filter(i => i.status === "at_risk" || i.status === "behind").length), icon: AlertTriangle, color: "text-red-400" },
    { label: "הושגו", value: fmt(items.filter(i => i.status === "achieved").length), icon: TrendingUp, color: "text-emerald-400" },
    { label: "התקדמות ממוצעת", value: `${avgProgress}%`, icon: BarChart3, color: "text-purple-400" },
  ];
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-foreground flex items-center gap-2">
            ...
            ניהול יעדים אסטרטגיים
          </h1>
          <p className="text-sm text-muted-foreground mt-1">הגדרת יעדים אסטרטגיים, OKRs ומעקב התקדמות</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <ExportDropdown
            data={filtered}
            headers={{ title: "יעד", category: "קטגוריה", owner: "אחראי", progress: "התקדמות", priority: "עדיפות", status: "סטטוס" }}
            filename="strategic_goals"
          />
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש יעד, אחראי..."
      ...
      <BulkActions selectedIds={selectedIds} onClear={clear} entityName="יעדים" actions={defaultBulkActions(selectedIds, clear, load, `${API}/strategy/goals`)} />
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">אין יעדים</p>
          <p className="text-sm mt-1">{search || filterStatus !== "all" ? "נסה לשנות את הסינון" : "לחץ על 'יעד חדש' כדי להתחיל"}</p>
        </div>
      ) : (<>
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  ...
                  <td className="px-2 py-3"><BulkCheckbox checked={isSelected(r.id)} onChange={() => toggle(r.id)} /></td>
                    ...
                    <td className="px-4 py-3 text-muted-foreground text-xs">{r.target_date?.slice(0, 10) || "—"}</td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                <DetailField label="יעד" value={viewDetail.title} />
                ...
                <DetailField label="עדיפות">
                  <Badge className={priorityMap[viewDetail.priority]?.color}>{priorityMap[viewDetail.priority]?.label}</Badge>
                </DetailField>
                <DetailField label="סטטוס">
                  <Badge className={statusMap[viewDetail.status]?.color}>{statusMap[viewDetail.status]?.label}</Badge>
                </DetailField>
                <DetailField label="תוצאות מפתח" value={viewDetail.key_results} />
                <div className="col-span-2"><DetailField label="תיאור" value={viewDetail.description} /></div>
              </div>
              )}
              {detailTab === "related" && <div className="p-5"><RelatedRecords entityType="goal" entityId={viewDetail.id} /></div>}
              {detailTab === "docs" && <div className="p-5"><AttachmentsSection entityType="goal" entityId={viewDetail.id} /></div>}
              {detailTab === "history" && <div className="p-5"><ActivityLog entityType="goal" entityId={viewDetail.id} /></div>}
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-foreground">{editing ? "עריכת יעד" : "יעד חדש"}</h2>
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">תאריך יעד</label>
                ...
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">תוצאות מפתח (Key Results)</label>
                  <textarea value={form.key_results || ""} onChange={e => setForm({ ...form, key_results: e.target.value })}
