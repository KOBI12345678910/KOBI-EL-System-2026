...
import { Plus, Search, Download, FileSpreadsheet, Eye, Edit2, Trash2, ChevronRight, ChevronLeft, Filter, BarChart3, AlertCircle, CheckCircle2, Clock, Loader2 } from "lucide-react";

const STATUSES = ["תקין", "בשימוש", "דרוש תיקון", "פסול"] as const;
const SC: Record<string, string> = { "תקין": "bg-green-500/20 text-green-300", "בשימוש": "bg-blue-500/20 text-blue-300", "דרוש תיקון": "bg-yellow-500/20 text-yellow-300", "פסול": "bg-purple-500/20 text-purple-300" };
const COLS = [
    { key: "tool", label: "כלי" },
    { key: "type", label: "סוג" },
    { key: "uses", label: "שימושים" },
    { key: "maxUses", label: "מקסימום" },
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">ניהול כלים ותבניות</h1>
          <p className="text-sm text-muted-foreground mt-1">כלי חיתוך, תבניות, מכבשים</p>
        </div>
                    ...
                    <tr key={idx} className="border-b border-border/30 hover:bg-card/30 transition-colors">
                      {COLS.map(c => (
                        <td key={c.key} className="p-3 text-foreground">
                          {c.key === "status" ? <Badge className={SC[row[c.key]] || "bg-gray-500/20 text-gray-300"}>{row[c.key]}</Badge> : row[c.key]}
                        </td>
                      ))}
