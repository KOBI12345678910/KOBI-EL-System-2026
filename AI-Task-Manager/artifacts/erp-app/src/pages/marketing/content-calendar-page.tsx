...
import {
  CalendarDays, Plus, Search, Edit2, Trash2, X, Save, Eye, ArrowUpDown,
  AlertTriangle, FileText, Hash, Monitor, CheckCircle2, Clock, Tag, User
} from "lucide-react";
...
const typeMap: Record<string, string> = {
  blog: "בלוג", social: "רשתות", video: "וידאו", email: "אימייל",
  infographic: "אינפוגרפיקה", webinar: "וובינר", podcast: "פודקאסט", other: "אחר",
};
...
export default function ContentCalendarPage() {
  ...
  const load = async () => {
    ...
    try {
      const res = await authFetch(`${API}/marketing/content-calendar`);
  ...
  const filtered = useMemo(() => {
    let data = items.filter(i =>
      ...
      (filterType === "all" || i.content_type === filterType) &&
      (!search || [i.title, i.author, i.category, i.content_type, i.platform]
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ status: "draft", content_type: "blog", publish_date: new Date().toISOString().slice(0, 10) });
    setShowForm(true);
  };
  ...
  const save = async () => {
    ...
    if (!validate(form)) return;    setSaving(true);
    try {
      const url = editing ? `${API}/marketing/content-calendar/${editing.id}` : `${API}/marketing/content-calendar`;
  ...
  const remove = async (id: number) => {
    if (await globalConfirm("למחוק תוכן זה? פעולה זו אינה ניתנת לביטול.")) {
      await authFetch(`${API}/marketing/content-calendar/${id}`, { method: "DELETE" });
      load();
    }
  };

  const kpis = [
    { label: 'סה"כ תכנים', value: fmt(items.length), icon: FileText, color: "text-blue-400" },
    { label: "טיוטות", value: fmt(items.filter(i => i.status === "draft").length), icon: Clock, color: "text-muted-foreground" },
    { label: "מתוכננים", value: fmt(items.filter(i => i.status === "scheduled").length), icon: CalendarDays, color: "text-yellow-400" },
    { label: "פורסמו", value: fmt(items.filter(i => i.status === "published").length), icon: CheckCircle2, color: "text-green-400" },
  ];

  const columns = [
    { key: "title", label: "כותרת" },
    { key: "content_type", label: "סוג" },
    { key: "publish_date", label: "תאריך פרסום" },
    { key: "author", label: "כותב" },
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">ניהול תכנים שיווקיים, לוח זמנים ופרסומים</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <ExportDropdown
            data={filtered}
            headers={{ title: "כותרת", content_type: "סוג", publish_date: "תאריך", author: "כותב", platform: "פלטפורמה", category: "קטגוריה", status: "סטטוס" }}
            filename="content_calendar"
          />
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש לפי כותרת, כותב, קטגוריה..."
      ...
      <BulkActions selectedIds={selectedIds} onClear={clear} entityName="תכנים" actions={defaultBulkActions(selectedIds, clear, load, `${API}/marketing/content-calendar`)} />
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">אין תכנים</p>
          <p className="text-sm mt-1">{search || filterStatus !== "all" || filterType !== "all" ? "נסה לשנות את הסינון" : "לחץ על 'תוכן חדש' כדי להתחיל"}</p>
        </div>
      ) : (<>
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  ...
                  <td className="px-2 py-3"><BulkCheckbox checked={isSelected(r.id)} onChange={() => toggle(r.id)} /></td>
                    <td className="px-4 py-3 text-foreground font-medium max-w-[250px] truncate">{r.title || "—"}</td>
                    <td className="px-4 py-3">
                      <Badge className="text-[10px] bg-indigo-500/20 text-indigo-400">{typeMap[r.content_type] || r.content_type || "—"}</Badge>
                    </td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                ...
                <DetailField label="סוג תוכן" value={typeMap[viewDetail.content_type] || viewDetail.content_type} />
                <DetailField label="תאריך פרסום" value={viewDetail.publish_date?.slice(0, 10)} />
                <DetailField label="כותב" value={viewDetail.author} />
                ...
                <DetailField label="תגיות" value={viewDetail.tags} />
                <div className="col-span-2"><DetailField label="תיאור" value={viewDetail.description || viewDetail.content} /></div>
                <div className="col-span-2"><DetailField label="הערות" value={viewDetail.notes} /></div>
              </div>
              )}
                {detailTab === "related" && <div className="p-5"><RelatedRecords entityType="content-calendar" entityId={viewDetail.id} /></div>}
                {detailTab === "docs" && <div className="p-5"><AttachmentsSection entityType="content-calendar" entityId={viewDetail.id} /></div>}
                {detailTab === "history" && <div className="p-5"><ActivityLog entityType="content-calendar" entityId={viewDetail.id} /></div>}
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-foreground">{editing ? "עריכת תוכן" : "תוכן חדש"}</h2>
                <button onClick={() => setShowForm(false)} className="p-1 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-5 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-muted-foreground mb-1.5">כותרת <RequiredMark /></label>
                    <input value={form.title || ""} onChange={e => setForm({ ...form, title: e.target.value })}
                      className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" placeholder="כותרת התוכן" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-muted-foreground mb-1.5">סוג תוכן</label>
                    <select value={form.content_type || "blog"} onChange={e => setForm({ ...form, content_type: e.target.value })}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-muted-foreground mb-1.5">כותב</label>
                    <input value={form.author || ""} onChange={e => setForm({ ...form, author: e.target.value })}
                      className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" placeholder="שם הכותב" />
                  </div>
                  <div>
                    ...
                    <input value={form.platform || ""} onChange={e => setForm({ ...form, platform: e.target.value })}
                      className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" placeholder="פלטפורמת פרסום" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-muted-foreground mb-1.5">קטגוריה</label>
                    <input value={form.category || ""} onChange={e => setForm({ ...form, category: e.target.value })}
                      className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" placeholder="קטגוריה" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-muted-foreground mb-1.5">תגיות</label>
                    <input value={form.tags || ""} onChange={e => setForm({ ...form, tags: e.target.value })}
                      className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" placeholder="תגיות מופרדות בפסיק" />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-muted-foreground mb-1.5">תיאור</label>
                    <textarea value={form.description || ""} onChange={e => setForm({ ...form, description: e.target.value })} rows={3}
                      className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" placeholder="תיאור התוכן" />
                  </div>
                  <div className="col-span-2">
