import { useState, useEffect } from "react";
  ...
  export default function AIExecutiveInsightsPage() {
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a0e1a] to-[#1a1f35] text-foreground p-6" dir="rtl">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold bg-gradient-to-l from-blue-400 to-indigo-300 bg-clip-text text-transparent flex items-center gap-3">
                <Brain className="w-8 h-8 text-blue-400" />
                תובנות מנהלים AI
              </h1>
              <p className="text-muted-foreground mt-1">{moduleInfo.description || "סיכומי ביצוע, תחזיות, והמלצות אסטרטגיות"}</p>
            </div>
            <div className="flex items-center gap-3">
              <button onClick={runNow} disabled={running} className="flex items-center gap-2 px-4 py-2 bg-blue-600/20 border border-blue-500/30 rounded-lg hover:opacity-80 transition disabled:opacity-50">
          ...
          {tab === "dashboard" && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {(() => {
                  const totalActions = metrics?.dailyMetrics?.reduce((s: number, d: any) => s + (d.actionsGenerated || 0), 0) || 0;
                  const successLogs = logs.filter((l: any) => l.status === "success").length;
                  const totalLogs = logs.length;
                  const approvalRate = totalLogs > 0 ? Math.round((successLogs / totalLogs) * 100) : 0;
                  const kpisTracked = Array.isArray(linked?.kpis) ? linked.kpis.length : (linked?.kpiCount || 0);
                  const strategicRecs = Array.isArray(linked?.recommendations) ? linked.recommendations.length : (metrics?.recommendationsGenerated || logs.filter((l: any) => l.type === "recommendation").length || 0);
                  return [
                    { label: "דוחות שהופקו", value: totalActions > 0 ? fmt(totalActions) : "—", icon: FileText, color: "from-blue-500/20 to-blue-600/10", text: "text-blue-400" },
                    { label: "תחזיות שאושרו", value: approvalRate > 0 ? `${approvalRate}%` : "—", icon: CheckCircle2, color: "from-green-500/20 to-green-600/10", text: "text-green-400" },
                    { label: "KPIs במעקב", value: kpisTracked > 0 ? fmt(kpisTracked) : "—", icon: Target, color: "from-amber-500/20 to-amber-600/10", text: "text-amber-400" },
                    { label: "המלצות אסטרטגיות", value: strategicRecs > 0 ? fmt(strategicRecs) : "—", icon: Brain, color: "from-purple-500/20 to-purple-600/10", text: "text-purple-400" },
                  ];
                })().map((kpi, i) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-card/5 border border-white/10 rounded-xl p-5">
                  <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-blue-400" /> ביצועים יומיים (30 יום)</h3>
                  ...
                  <div className="space-y-3">
                    {(linked?.linkedEntities || []).map((entity: string) => (
                      <div key={entity} className="flex items-center justify-between bg-card/5 rounded-lg p-3">
                        <span className="text-gray-300">{{"reports":"דוחות","kpis":"מדדים","budgets":"תקציבים","projects":"פרויקטים"}[entity] || entity}</span>
          ...
          {tab === "settings" && (
            <div className="bg-card/5 border border-white/10 rounded-xl p-6 space-y-6">
              ...
              <div className="p-3 bg-card/5 rounded-lg">
                  <label className="text-sm text-muted-foreground">תדירות דוחות</label>
                  <select value={settings.reportFrequency || "daily"} onChange={e => updateSetting("reportFrequency", e.target.value)}
                    className="w-full mt-1 bg-card/10 border border-white/20 rounded-lg px-3 py-2 text-foreground">
              ...
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>כלול תחזיות</span>
                  <button onClick={() => updateSetting("includeForecasts", !settings.includeForecasts)} className={`w-12 h-6 rounded-full transition ${settings.includeForecasts ? "bg-green-500" : "bg-muted"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.includeForecasts ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>כלול השוואות</span>
                  <button onClick={() => updateSetting("includeBenchmarks", !settings.includeBenchmarks)} className={`w-12 h-6 rounded-full transition ${settings.includeBenchmarks ? "bg-green-500" : "bg-muted"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.includeBenchmarks ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              <div className="flex items-center justify-between p-3 bg-card/5 rounded-lg">
                  <span>התראות KPI</span>
                  <button onClick={() => updateSetting("kpiAlerts", !settings.kpiAlerts)} className={`w-12 h-6 rounded-full transition ${settings.kpiAlerts ? "bg-green-500" : "bg-muted"} relative`}>
                    <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full transition ${settings.kpiAlerts ? "right-0.5" : "left-0.5"}`} />
                  </button>
                </div>
              </div>
            </div>
