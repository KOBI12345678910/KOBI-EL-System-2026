...
import {
  Lightbulb, Search, Plus, Edit2, Trash2, X, Save, Eye,
  ArrowUpDown, AlertTriangle, CheckCircle2, Clock, TrendingUp, BarChart3
} from "lucide-react";
...
const statusMap: Record<string, { label: string; color: string }> = {
  "חדש": { label: "חדש", color: "bg-blue-500/20 text-blue-400" },
  "פתוח": { label: "פתוח", color: "bg-cyan-500/20 text-cyan-400" },
  "בטיפול": { label: "בטיפול", color: "bg-yellow-500/20 text-yellow-400" },
  "מאושר": { label: "מאושר", color: "bg-purple-500/20 text-purple-400" },
  "הושלם": { label: "הושלם", color: "bg-green-500/20 text-green-400" },
  "נדחה": { label: "נדחה", color: "bg-red-500/20 text-red-400" },
};

const priorityMap: Record<string, { label: string; color: string }> = {
  "נמוך": { label: "נמוך", color: "bg-muted/20 text-muted-foreground" },
  "בינוני": { label: "בינוני", color: "bg-blue-500/20 text-blue-400" },
  "גבוה": { label: "גבוה", color: "bg-orange-500/20 text-orange-400" },
  "קריטי": { label: "קריטי", color: "bg-red-500/20 text-red-400" },
};

const sourceOptions = ["לקוח", "צוות פנימי", "מנהל", "תמיכה", "מכירות", "אחר"];
...
export default function FeatureRequestsPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API}/product-dev/feature-requests`);
  ...
  const filtered = useMemo(() => {
    let data = items.filter(i =>
      ...
      (!search || [i.title, i.source, i.requester_name, i.assignee, i.category, i.description]
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ status: "חדש", priority: "בינוני" });
    setShowForm(true);
  };
  ...
  const openEdit = (r: any) => {
    ...
    setForm({
      ...
      requesterName: r.requester_name, priority: r.priority, status: r.status,
      votes: r.votes, category: r.category, assignee: r.assignee,
      targetVersion: r.target_version, estimatedEffort: r.estimated_effort, notes: r.notes,
    });
    setShowForm(true);
  };
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const url = editing ? `${API}/product-dev/feature-requests/${editing.id}` : `${API}/product-dev/feature-requests`;
      await authFetch(url, {
  ...
  const remove = async (id: number) => {
    if (await globalConfirm("למחוק בקשת פיצ'ר? פעולה זו אינה ניתנת לביטול.")) {
      await authFetch(`${API}/product-dev/feature-requests/${id}`, { method: "DELETE" });
      load();
    }
  };

  const totalVotes = items.reduce((s, i) => s + (i.votes || 0), 0);

  const kpis = [
    { label: "סה\"כ בקשות", value: fmt(items.length), icon: Lightbulb, color: "text-violet-400" },
    { label: "חדשות/פתוחות", value: fmt(items.filter(i => i.status === "חדש" || i.status === "פתוח").length), icon: TrendingUp, color: "text-blue-400" },
    { label: "בטיפול", value: fmt(items.filter(i => i.status === "בטיפול").length), icon: Clock, color: "text-yellow-400" },
    { label: "הושלמו", value: fmt(items.filter(i => i.status === "הושלם").length), icon: CheckCircle2, color: "text-green-400" },
    { label: "סה\"כ הצבעות", value: fmt(totalVotes), icon: BarChart3, color: "text-purple-400" },
  ];
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">ניהול בקשות שיפור ופיצ'רים חדשים מלקוחות וצוותים</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <ExportDropdown
            data={filtered}
            headers={{ title: "כותרת", source: "מקור", priority: "עדיפות", status: "סטטוס", votes: "הצבעות", assignee: "אחראי" }}
            filename="feature_requests"
          />
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש בקשה, מבקש, מקור..."
        ...
        <select value={filterPriority} onChange={e => setFilterPriority(e.target.value)}
          ...
          {Object.keys(priorityMap).map(k => <option key={k} value={k}>{k}</option>)}
        </select>
        <span className="text-sm text-muted-foreground">{filtered.length} תוצאות</span>
      </div>

      <BulkActions selectedIds={selectedIds} onClear={clear} entityName="בקשות פיצר" actions={defaultBulkActions(selectedIds, clear, load, `${API}/product-dev/feature-requests`)} />
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">אין בקשות פיצ'רים</p>
          <p className="text-sm mt-1">{search || filterStatus !== "all" ? "נסה לשנות את הסינון" : "לחץ על 'בקשה חדשה' כדי להתחיל"}</p>
        </div>
      ) : (<>
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  ...
                  <td className="px-2 py-3"><BulkCheckbox checked={isSelected(r.id)} onChange={() => toggle(r.id)} /></td>
                    ...
                    <td className="px-4 py-3 text-yellow-400 font-medium">{r.votes || 0}</td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                ...
                <DetailField label="הצבעות" value={String(viewDetail.votes || 0)} />
                <DetailField label="עדיפות">
                  <Badge className={priorityMap[viewDetail.priority]?.color}>{viewDetail.priority}</Badge>
                </DetailField>
                ...
                <DetailField label="אחראי" value={viewDetail.assignee} />
                <DetailField label="גרסה יעד" value={viewDetail.target_version} />
                <DetailField label="מאמץ משוער" value={viewDetail.estimated_effort} />
              ...
              {detailTab === "related" && <div className="p-5"><RelatedRecords entityType="feature-request" entityId={viewDetail.id} /></div>}
              {detailTab === "docs" && <div className="p-5"><AttachmentsSection entityType="feature-request" entityId={viewDetail.id} /></div>}
              {detailTab === "history" && <div className="p-5"><ActivityLog entityType="feature-request" entityId={viewDetail.id} /></div>}
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-foreground">{editing ? "עריכת בקשה" : "בקשת פיצ'ר חדשה"}</h2>
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">שם מבקש</label>
                ...
                <div>
                  ...
                  <select value={form.priority || "בינוני"} onChange={e => setForm({ ...form, priority: e.target.value })}
                    className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm">
                    {Object.keys(priorityMap).map(k => <option key={k} value={k}>{k}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select value={form.status || "חדש"} onChange={e => setForm({ ...form, status: e.target.value })}
                    className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm">
                    {Object.keys(statusMap).map(k => <option key={k} value={k}>{k}</option>)}
                  </select>
                </div>
