    ...
    <div className={`bg-card border border-border/50 rounded-2xl overflow-hidden ${className}`}>
      <div className="border-b border-border/50 flex overflow-x-auto scrollbar-hide">
        {safeTabs.map(tab => {
          ...
          return (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`flex items-center gap-2 px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${activeTab === tab.key ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-white hover:border-border"}`}>
              ...
              <tbody className="divide-y divide-border/30">
                {currentData.slice(0, 10).map((row: any, i: number) => (
                  <motion.tr key={row.id || i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }} className="hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => currentTab?.onRowClick?.(row)}>
                    {currentTab?.columns.map(col => (
                      <td key={col.key} className="px-4 py-2.5 text-white/80">{col.render ? col.render(row[col.key], row) : (row[col.key] != null && typeof row[col.key] === "object" ? JSON.stringify(row[col.key]) : (row[col.key] ?? "—"))}</td>
                    ))}
                    {currentTab?.badge && (() => {
