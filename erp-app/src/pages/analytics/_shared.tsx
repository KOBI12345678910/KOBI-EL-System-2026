// Shared helpers for analytics pages — mirrors pages/intelligence/_shared.tsx
import { useState } from "react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { authFetch } from "@/lib/utils";

export const API_BASE = "/api/analytics";
...
export function useList<T = any>(
  ...
  return useQuery({
    queryKey,
    queryFn: () =>
      apiGet<{ rows: T[]; limit: number; offset: number; total?: number }>(path, params),
  });
}
...
export function StatusBadge({ state }: { state?: string | null }) {
  ...
  const tone: Record<string, string> = {
    ...
    skipped: "bg-gray-300 text-gray-800",
    active: "bg-green-200 text-green-900",
    higher_is_better: "bg-emerald-100 text-emerald-800",
    lower_is_better: "bg-indigo-100 text-indigo-800",
    within_range: "bg-amber-100 text-amber-800",
    exact_match: "bg-purple-100 text-purple-800",
  };
...
export function Pagination({
  ...
  return (
    <div className="flex items-center gap-2 text-sm" dir="rtl">
      ...
      <button
        className="rounded border px-2 py-1 disabled:opacity-40"
        disabled={!!total && offset + limit >= total}
...
export function ErrorBox({ err }: { err: unknown }) {
  ...
  return (
    <div className="p-6 text-red-700 bg-red-50 rounded" dir="rtl">
      שגיאה: {msg}
    </div>
  );
}
