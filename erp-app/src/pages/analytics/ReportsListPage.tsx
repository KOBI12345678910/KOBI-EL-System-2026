import { Link } from "wouter";
import {
  useList,
  useSearchState,
  SearchBar,
  Pagination,
  StatusBadge,
  Loading,
  ErrorBox,
  EmptyBox,
  PageHeader,
  formatDate,
} from "./_shared";

interface Report {
  id: string;
  report_code: string;
  report_name: string;
  report_label_he: string | null;
  category: string | null;
  output_format: string;
  is_active: boolean;
  updated_at: string;
}

export default function ReportsListPage() {
  const s = useSearchState({ limit: 50 });
  const queryKey = ["analytics", "reports", s.q, s.state, s.offset];
  const { data, isLoading, error } = useList<Report>(queryKey, "/reports", {
    q: s.q || undefined,
    category: s.state || undefined,
    limit: s.limit,
    offset: s.offset,
  });

  return (
    <div dir="rtl" className="p-6 space-y-4">
      <PageHeader title="דוחות" />
      <div className="flex gap-2 items-center">
        <SearchBar
          value={s.q}
          onChange={(v) => {
            s.setQ(v);
            s.setOffset(0);
          }}
          placeholder="חיפוש לפי קוד / שם דוח…"
        />
        <input
          value={s.state}
          onChange={(e) => {
            s.setState(e.target.value);
            s.setOffset(0);
          }}
          placeholder="קטגוריה"
          className="rounded border px-2 py-2 text-sm"
        />
      </div>
      {isLoading && <Loading />}
      {error && <ErrorBox err={error} />}
      {data && data.rows.length === 0 && <EmptyBox label="אין דוחות להצגה" />}
      {data && data.rows.length > 0 && (
        <div className="overflow-auto rounded border">
          <table className="w-full text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="p-2 text-right">קוד</th>
                <th className="p-2 text-right">שם</th>
                <th className="p-2 text-right">תווית</th>
                <th className="p-2 text-right">קטגוריה</th>
                <th className="p-2 text-right">פורמט</th>
                <th className="p-2 text-right">סטטוס</th>
                <th className="p-2 text-right">עודכן</th>
                <th className="p-2 text-right">פעולות</th>
              </tr>
            </thead>
            <tbody>
              {data.rows.map((r) => (
                <tr key={r.id} className="border-t hover:bg-gray-50">
                  <td className="p-2 font-medium">{r.report_code}</td>
                  <td className="p-2">{r.report_name}</td>
                  <td className="p-2">{r.report_label_he ?? "—"}</td>
                  <td className="p-2">{r.category ?? "—"}</td>
                  <td className="p-2">{r.output_format}</td>
                  <td className="p-2">
                    <StatusBadge state={r.is_active ? "active" : "skipped"} />
                  </td>
                  <td className="p-2 text-gray-500">{formatDate(r.updated_at)}</td>
                  <td className="p-2">
                    <Link href={`/reports/${r.id}`} className="text-indigo-700 hover:underline">
                      פתח
                    </Link>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
      <Pagination offset={s.offset} limit={s.limit} total={data?.total} onChange={s.setOffset} />
    </div>
  );
}
