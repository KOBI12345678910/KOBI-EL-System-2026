...
import { Calendar, Search, Plus, Edit2, Trash2, X, Save, CheckCircle2, Clock, AlertTriangle, ArrowUpDown, Ban, Users, Eye, ChevronLeft, ChevronRight, BarChart3 } from "lucide-react";
      ...
      <div className="flex items-center gap-3">
        <div className="flex gap-1 bg-card border border-border/50 rounded-xl p-1">
          <button onClick={() => setViewMode("remaining")}
            className={`px-3 py-1.5 rounded-lg text-sm ${viewMode === "remaining" ? "bg-emerald-600 text-white" : "text-muted-foreground hover:text-white"}`}>
            יתרה
          </button>
          <button onClick={() => setViewMode("taken")}
            className={`px-3 py-1.5 rounded-lg text-sm ${viewMode === "taken" ? "bg-blue-600 text-white" : "text-muted-foreground hover:text-white"}`}>
            נוצלו
          </button>
        </div>
          ...
          <tbody>
            {employees.map(({ emp, types, totalTaken, totalRemaining }) => (
              <tr key={emp} className="border-b border-border/20 hover:bg-muted/10">
                <td className="px-4 py-2.5 font-medium text-white whitespace-nowrap sticky right-0 bg-card">{emp}</td>
                {shownLeaveTypes.map(t => {
                  const d = types[t];
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        ...
        <span className="font-bold text-white text-sm min-w-[140px] text-center">{HEBREW_MONTHS[calMonth]} {calYear}</span>
...
export default function LeaveManagementPage() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const url = editing ? `${API}/leave-requests/${editing.id}` : `${API}/leave-requests`;
  ...
  const quickApprove = async (r: LeaveRequest) => {
    await authFetch(`${API}/leave-requests/${r.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "approved" }) });
    load();
  };
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={openCreate} className="flex items-center gap-2 bg-teal-600 text-white px-4 py-2.5 rounded-xl hover:bg-teal-700 shadow-lg text-sm font-medium">
          ...
          <div className="flex flex-wrap gap-2">
            {items.filter(r => r.status === "pending").slice(0, 5).map(r => (
              <div key={r.id} className="flex items-center gap-2 bg-card border border-border/50 rounded-lg px-3 py-1.5">
                <span className="text-sm text-white">{r.employee_name}</span>
      ...
      <div className="flex gap-1 bg-card border border-border/50 rounded-xl p-1 w-fit">
        {[
          ...
          <button key={key} onClick={() => setMainTab(key as any)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${mainTab === key ? "bg-teal-600 text-white" : "text-muted-foreground hover:text-white"}`}>
      ...
      {mainTab === "calendar" ? (
        ...
        <div className="bg-card border border-border/50 rounded-2xl overflow-hidden">
          <div className="px-4 py-3 border-b border-border/50 flex items-center gap-2">
            ...
            <span className="font-semibold text-white text-sm">יתרות וניצול חופשה לפי עובד</span>
          </div>
          <LeaveBalancePanel />
        </div>
      ) : loading ? (
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-white font-medium whitespace-nowrap">{r.employee_name}</td>
                    ...
                    <td className="px-4 py-3 font-bold text-white">{r.total_days}</td>
                    ...
                    <td className="px-4 py-3"><div className="flex gap-1">
                      ...
                      <button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.employee_name || r.id}'?`)) remove(r.id)}} className="p-1.5 hover:bg-muted rounded-lg"><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewDetail(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><Calendar className="w-5 h-5 text-teal-400" /> בקשה {viewDetail.request_number}</h2>
              ...
              <div className="flex border-b border-border/50">
                {[{key:"details",label:"פרטים"},{key:"related",label:"רשומות קשורות"},{key:"docs",label:"מסמכים"},{key:"history",label:"היסטוריה"}].map(t => (
                  <button key={t.key} onClick={() => setDetailTab(t.key)} className={`px-4 py-2.5 text-sm font-medium border-b-2 ${detailTab === t.key ? "border-teal-500 text-teal-400" : "border-transparent text-muted-foreground hover:text-white"}`}>{t.label}</button>
                ))}
              </div>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת בקשה" : "בקשת חופשה חדשה"}</h2>
              ...
              <div className="p-5 border-t border-border flex justify-end gap-2">
                ...
                <button onClick={save} disabled={saving} className="px-6 py-2 bg-teal-600 text-white rounded-lg text-sm hover:bg-teal-700 disabled:opacity-50">
