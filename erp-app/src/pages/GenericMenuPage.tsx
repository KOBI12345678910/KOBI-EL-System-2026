...
/**
 * GenericMenuPage (erp-app / wouter edition)
 * ─────────────────────────────────────────────────────────────────────────
 ...
 *
 * Mirrors the techno-kol-ops version but uses wouter's hooks (`useLocation`,
 * `useParams`) instead of react-router-dom.
 *
...
export function GenericMenuPage() {
  ...
  const title =
    ...
    menuEntry?.label ||
    (entityHint
      ? `${entityHint.schema}.${entityHint.name}`
            ...
            return (
              <span key={`${href}-${idx}`}>
                ...
                {isLast ? (
                  ...
                  <Link href={href} style={{ color: "#5C7080", textDecoration: "none" }}>
                    {seg}
                  </Link>
                )}
              </span>
      ...
      <div style={cardStyle}>
        ...
        {Object.keys(params).length > 0 && (
          <div style={{ marginTop: 12, fontSize: 12, color: "#8F99A8" }}>
            <strong>פרמטרים:</strong> <span style={codeStyle}>{JSON.stringify(params)}</span>
          </div>
        )}
      </div>
    </div>
