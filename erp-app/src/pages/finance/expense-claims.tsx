...
import { Receipt, Search, Plus, Edit2, Trash2, X, Save, CheckCircle2, Clock, AlertTriangle, ArrowUpDown, DollarSign, Hash, Users, MapPin, Ban , Loader2 } from "lucide-react";
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => printPage("תביעות הוצאות")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Printer size={16} /> הדפסה</button>
          <button onClick={openCreate} className="flex items-center gap-2 bg-teal-600 text-white px-3 py-2 rounded-lg hover:bg-teal-700 shadow-lg text-sm"><Plus size={16} /> תביעה חדשה</button>
        </div>
      </div>
          ...
          <tbody>
            ...
            pagination.paginate(filtered).map(r => (
              <tr key={r.id} className="border-b hover:bg-teal-50/30">
                ...
                <td className="px-3 py-2"><div className="flex gap-1"><button onClick={() => openEdit(r)} className="p-1 hover:bg-blue-500/10 rounded"><Edit2 size={14} /></button><button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.employee_name || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1 hover:bg-red-500/10 rounded text-red-500"><Trash2 size={14} /></button></div></td>
              </tr>
            ))}
          </tbody>
          {filtered.length > 0 && (
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="col-span-2 bg-gradient-to-l from-teal-50 to-blue-50 rounded-xl border border-teal-200 p-4">
                  <div className="text-sm font-bold text-foreground mb-3">סיכום + מע"מ (18%)</div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                    ...
                    <div className="bg-teal-100 rounded-lg p-2 border border-teal-300">
                      ...
                      <div className="font-bold text-teal-800 text-lg">₪{fmt(Math.round((Number(form.totalClaimed) || 0) * 1.18 * 100) / 100)}</div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={save} className="flex items-center gap-2 bg-teal-600 text-white px-6 py-2 rounded-lg hover:bg-teal-700"><Save size={16} /> {editing ? "עדכון" : "שמירה"}</button>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">תביעת הוצאות #{selectedItem.claim_number || selectedItem.id}</h2>
              ...
              {detailTab === "details" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <div><div className="text-xs text-muted-foreground mb-1">עובד</div><div className="text-sm text-white">{selectedItem.employee_name || "-"}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">סכום</div><div className="text-sm text-white font-bold">₪{Number(selectedItem.total_amount || 0).toLocaleString()}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">תאריך</div><div className="text-sm text-white">{selectedItem.claim_date}</div></div>
