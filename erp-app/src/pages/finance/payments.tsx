    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg sm:text-2xl font-bold text-white">תשלומים</h1>
      ...
      <BulkActions items={rows} selectedIds={selectedIds} onSelectionChange={setSelectedIds} actions={[
        defaultBulkActions.delete(async (ids) => { for (const id of ids) await authFetch(`${API}/finance/payments/${id}`, { method: "DELETE" }); qc.invalidateQueries({ queryKey: ["payments"] }); }),
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                <tr key={r.id} className="border-b border-slate-700/50 hover:bg-slate-700/30 cursor-pointer" onClick={() => { setSelectedItem(r); setDetailTab("details"); }}>
                  ...
                  <td className="p-3 font-mono text-white">{fmt(r.amount)}</td>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">תשלום #{selectedItem.id}</h2>
              ...
              {detailTab === "details" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <div><div className="text-xs text-muted-foreground mb-1">תאריך</div><div className="text-sm text-white">{fmtDate(selectedItem.payment_date)}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">סוג</div><div className="text-sm text-white">{typeLabels[selectedItem.payment_type] || selectedItem.payment_type}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">אמצעי</div><div className="text-sm text-white">{methodLabels[selectedItem.payment_method] || selectedItem.payment_method}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">סכום</div><div className="text-sm text-white font-bold">{fmt(selectedItem.amount)}</div></div>
                    ...
                    <div><div className="text-xs text-muted-foreground mb-1">אסמכתא</div><div className="text-sm text-white">{selectedItem.reference_number || "-"}</div></div>
                  </div>
                </div>
              )}
