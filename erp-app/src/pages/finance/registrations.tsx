...
import {
  ClipboardList, Search, Plus, Edit2, Trash2, X, Save, AlertTriangle,
  ArrowUpDown, Hash, Eye, Clock, CheckCircle2, DollarSign
} from "lucide-react";
...
const statusMap: Record<string, { label: string; color: string }> = {
  ...
  renewal: { label: "בחידוש", color: "bg-blue-500/20 text-blue-400" },
};
...
export default function RegistrationsPage() {
  ...
  const [viewDetail, setViewDetail] = useState<RegItem | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      else setError("שגיאה בטעינת רישומים");
    } catch (e: any) {
  ...
  const filtered = useMemo(() => {
    let data = items.filter(i =>
      ...
      (!search || [i.registration_number, i.name, i.type, i.authority]
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ status: "pending", registrationDate: new Date().toISOString().slice(0, 10) });
    setShowForm(true);
  };
  ...
  const openEdit = (r: RegItem) => {
    ...
    setForm({
      ...
      name: r.name,
      type: r.type,
      registrationDate: r.registration_date?.slice(0, 10),
  ...
  const save = async () => {
    setSaving(true);
    try {
      const url = editing ? `${API}/registrations/${editing.id}` : `${API}/registrations`;
      await authFetch(url, {
  ...
  const remove = async (id: number) => {
    if (await globalConfirm("למחוק רישום זה?")) {
      await authFetch(`${API}/registrations/${id}`, { method: "DELETE" });
      load();
    }
  };

  const kpis = [
    { label: "רישומים", value: fmt(items.length), icon: Hash, color: "text-blue-400" },
    { label: "פעילים", value: fmt(items.filter(i => i.status === "active").length), icon: CheckCircle2, color: "text-green-400" },
    { label: "פגי תוקף", value: fmt(items.filter(i => i.status === "expired").length), icon: AlertTriangle, color: "text-red-400" },
    { label: "ממתינים", value: fmt(items.filter(i => i.status === "pending").length), icon: Clock, color: "text-yellow-400" },
    { label: "סה\"כ אגרות", value: fmtCurrency(items.reduce((s, i) => s + Number(i.fee || 0), 0)), icon: DollarSign, color: "text-amber-400" },
  ];

  const columns = [
    { key: "registration_number", label: "מספר" },
    { key: "name", label: "שם" },
    { key: "type", label: "סוג" },
    { key: "authority", label: "רשות" },
    { key: "registration_date", label: "תאריך רישום" },
    { key: "expiry_date", label: "תוקף" },
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">ניהול רישומים, רישיונות ואישורים</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <ExportDropdown
            data={filtered}
            headers={{
              registration_number: "מספר", name: "שם", type: "סוג",
              authority: "רשות", expiry_date: "תוקף", status: "סטטוס",
            }}
          ...
          >
            <Plus className="w-4 h-4" /> רישום חדש
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input
            ...
            placeholder="חיפוש לפי מספר, שם, סוג, רשות..."
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">אין רישומים</p>
          <p className="text-sm mt-1">
            {search || filterStatus !== "all" ? "נסה לשנות את הסינון" : "לחץ על 'רישום חדש' כדי להתחיל"}
          </p>
        </div>
      ) : (<>
        <BulkActions selectedIds={selectedIds} onClear={clear} entityName="items" actions={defaultBulkActions(selectedIds, clear, load, `${API}/registrations`)} />
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-teal-400 font-bold">{r.registration_number}</td>
                    <td className="px-4 py-3 text-white">{r.name}</td>
                    <td className="px-4 py-3 text-muted-foreground">{r.type}</td>
                    <td className="px-4 py-3 text-muted-foreground">{r.authority}</td>
                    <td className="px-4 py-3 text-muted-foreground text-xs">{r.registration_date?.slice(0, 10)}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        ...
                        <button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.name || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1.5 hover:bg-muted rounded-lg"><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewDetail(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              ...
              {detailTab === "details" ? (
                ...
                <DetailField label="מספר" value={viewDetail.registration_number} />
                <DetailField label="שם" value={viewDetail.name} />
                <DetailField label="סוג" value={viewDetail.type} />
                <DetailField label="רשות" value={viewDetail.authority} />
                <DetailField label="תאריך רישום" value={viewDetail.registration_date?.slice(0, 10)} />
                <DetailField label="תוקף" value={viewDetail.expiry_date?.slice(0, 10)} />
                <DetailField label="אגרה" value={fmtCurrency(viewDetail.fee)} />
                ...
                <div className="p-5"><RelatedRecords entityType="registrations" entityId={viewDetail?.id} /></div>
                ) : detailTab === "attachments" ? (
                <div className="p-5"><AttachmentsSection entityType="registrations" entityId={viewDetail?.id} /></div>
                ) : (
                <div className="p-5"><ActivityLog entityType="registrations" entityId={viewDetail?.id} /></div>
                )}
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת רישום" : "רישום חדש"}</h2>
                <button onClick={() => setShowForm(false)} className="p-1 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-5 grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">שם *</label>
                  <input value={form.name || ""} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">סוג</label>
                  <input value={form.type || ""} onChange={e => setForm({ ...form, type: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">רשות</label>
                  <input value={form.authority || ""} onChange={e => setForm({ ...form, authority: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">תאריך רישום</label>
                  <input type="date" value={form.registrationDate || ""} onChange={e => setForm({ ...form, registrationDate: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">תוקף</label>
                  <input type="date" value={form.expiryDate || ""} onChange={e => setForm({ ...form, expiryDate: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">אגרה</label>
                  <input type="number" value={form.fee || ""} onChange={e => setForm({ ...form, fee: Number(e.target.value) })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" />
                </div>
                <div>
