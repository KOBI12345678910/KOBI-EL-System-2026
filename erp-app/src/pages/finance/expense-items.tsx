              ...
              <tbody>
                {items.map((item: any) => (
                  <tr key={item.id || item.name} className="border-b border-slate-800/50 hover:bg-slate-800/30 group">
                    ...
                    <td className="p-3 text-white font-medium cursor-pointer hover:underline" onClick={() => { setItemDetailView(item); setDetailTab("details"); }}>{item.name}</td>
                    <td className="p-3">
                      <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="p-1.5 rounded hover:bg-slate-700 text-muted-foreground hover:text-white" title="עריכה">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button className="p-1.5 rounded hover:bg-slate-700 text-muted-foreground hover:text-white" title="צפייה">
        ...
        <DialogContent className="bg-slate-900 border-slate-700 max-w-md" dir="rtl">
          ...
          <div className="space-y-2">
            {DOC_TYPES.map(dt => (
              <button
                key={dt.id}
                className="w-full text-right p-4 rounded-lg border border-slate-700 hover:border-blue-500/50 hover:bg-slate-800/50 transition-all text-white font-medium"
                onClick={() => {
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full mt-1 p-2 rounded-md bg-slate-800 border border-slate-700 text-white"
              >
      ...
      <Dialog open={!!itemDetailView} onOpenChange={(open) => { if (!open) { setItemDetailView(null); setDetailTab("details"); } }}>
        <DialogContent className="bg-slate-900 border-slate-700 max-w-2xl" dir="rtl">
          ...
          {detailTab === "details" && itemDetailView && <div className="grid grid-cols-2 gap-4 p-4">
            <div><span className="text-muted-foreground block text-sm">שם פריט</span><span className="text-white font-medium">{itemDetailView.name}</span></div>
            <div><span className="text-muted-foreground block text-sm">מזהה</span><span className="text-white">{itemDetailView.id || "-"}</span></div>
            <div><span className="text-muted-foreground block text-sm">תיאור</span><span className="text-white">{itemDetailView.description || "-"}</span></div>
            <div><span className="text-muted-foreground block text-sm">סוג מסמך</span><span className="text-white">{itemDetailView.doc_type || "-"}</span></div>
          </div>}
