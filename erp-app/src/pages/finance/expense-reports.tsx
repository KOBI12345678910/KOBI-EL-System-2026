...
import { FileText, Search, Plus, Edit2, Trash2, X, Save, CheckCircle2, Clock, AlertTriangle, ArrowUpDown, DollarSign, Hash, Send as SendIcon, ThumbsUp, ThumbsDown, ChevronDown, ChevronUp, Layers, Users, Wallet , Loader2 } from "lucide-react";
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => printPage("דוחות הוצאות")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Printer size={16} /> הדפסה</button>
          <button onClick={openCreate} className="flex items-center gap-2 bg-amber-600 text-white px-3 py-2 rounded-lg hover:bg-amber-700 shadow-lg text-sm"><Plus size={16} /> דוח חדש</button>
        </div>
      </div>
                  ...
                  <tr key={`${r.id}-detail`}><td colSpan={11} className="p-0">
                    <div className="bg-amber-50/30 border-t border-amber-100 px-6 py-3">
                      ...
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <button onClick={() => openAddLine(r.id)} className="flex items-center gap-1 text-xs bg-amber-600 text-white px-2 py-1 rounded hover:bg-amber-700"><Plus size={12} /> הוסף שורה</button>
                      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button onClick={save} className="px-6 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 flex items-center gap-2"><Save size={16} /> {editing ? "עדכן" : "צור"}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowLineForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button onClick={saveLine} className="px-6 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 flex items-center gap-2"><Save size={16} /> הוסף</button>
              </div>
            </motion.div>
          </motion.div>
        )}
