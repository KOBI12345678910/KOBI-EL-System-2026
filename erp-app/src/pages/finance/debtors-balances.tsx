            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={customerSearch} onChange={e => setCustomerSearch(e.target.value)}
                  ...
                  className="w-full pr-9 pl-3 py-2 bg-slate-800 border border-slate-700 rounded-md text-sm text-white placeholder:text-muted-foreground" />
              </div>
            </div>
            <div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-700/50 bg-slate-800/30">
                    ...
                    <th className="p-3 text-center text-muted-foreground font-medium cursor-pointer hover:text-white" onClick={() => toggleSort("total_invoiced")}>
                      <span className="flex items-center justify-center gap-1">סה"כ חשבוניות <ArrowUpDown className="w-3 h-3" /></span>
                    </th>
                    <th className="p-3 text-center text-muted-foreground font-medium cursor-pointer hover:text-white" onClick={() => toggleSort("total_paid")}>
                      <span className="flex items-center justify-center gap-1">שולם <ArrowUpDown className="w-3 h-3" /></span>
                    </th>
                    <th className="p-3 text-center text-muted-foreground font-medium cursor-pointer hover:text-white" onClick={() => toggleSort("balance")}>
                    ...
                    return (
                      <>
                        <tr key={d.customer_id} className="border-b border-slate-800/50 hover:bg-slate-800/30 cursor-pointer"
                          ...
                          <td className="p-3 text-white font-medium">{d.customer_name || d.name}</td>
                                    ...
                                    <tbody>
                                      {(d.open_documents || []).map((doc: any, idx: number) => (
                                        <tr key={idx} className="border-b border-slate-700/50">
                                          <td className="p-2 text-white">{doc.doc_type} #{doc.doc_number}</td>
