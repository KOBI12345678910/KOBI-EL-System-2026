...
import {
  ...
  Clock, AlertTriangle, ArrowUpDown, DollarSign, Eye, Hash, Send as SendIcon
} from "lucide-react";
...
export default function CustomerInvoicesPage() {
  ...
  const recalc = (f: any) => {
    ...
    const vr = Number(f.vatRate) || 18, va = bv * (vr / 100);
    return { ...f, beforeVat: bv, vatAmount: Math.round(va * 100) / 100, totalAmount: Math.round((bv + va) * 100) / 100 };
  };

  const openCreate = () => { setEditing(null); setForm({ invoiceType: "tax_invoice", invoiceDate: new Date().toISOString().slice(0, 10), status: "draft", currency: "ILS", vatRate: 18, paymentTerms: "net_30" }); setShowForm(true); };
  ...
  return (
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-2"><FileText className="text-emerald-400 w-6 h-6" /> חשבוניות לקוחות</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {kpis.map((kpi, i) => (<motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-card border border-border/50 rounded-2xl p-4"><kpi.icon className={`${kpi.color} w-5 h-5 mb-2`} /><div className="text-xl font-bold text-white">{kpi.value}</div><div className="text-xs text-muted-foreground">{kpi.label}</div></motion.div>))}
      </div>
      ...
      ) : filtered.length === 0 ? (<div className="text-center py-16 text-muted-foreground"><FileText className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>אין חשבוניות</p></div>
      ) : (<>
        <div className="border border-border/50 rounded-2xl bg-card/50 overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm">
          ...
          <tbody>{pagination.paginate(filtered).map(r => (
            <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
              ...
              <td className="px-4 py-3 text-white max-w-[150px] truncate">{r.customer_name}</td>
              <td className="px-4 py-3 font-bold text-white">₪{fmt(r.total_amount)}</td>
              ...
              <td className="px-4 py-3"><div className="flex gap-1">
                ...
                <button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.customer_name || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1.5 hover:bg-muted rounded-lg"><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
              </div></td>
            </tr>
          ))}</tbody></table></div></div>
        ...
        {viewDetail && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewDetail(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center"><h2 className="text-lg font-bold text-white">חשבונית {viewDetail.invoice_number}</h2><button onClick={() => setViewDetail(null)} className="p-1 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button></div>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center"><h2 className="text-lg font-bold text-white">{editing ? "עריכת חשבונית" : "חשבונית לקוח חדשה"}</h2><button onClick={() => setShowForm(false)} className="p-1 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button></div>
              <div className="p-5 space-y-4"><div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">תאריך</label><input type="date" value={form.invoiceDate || ""} onChange={e => setForm({ ...form, invoiceDate: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">לתשלום עד</label><input type="date" value={form.dueDate || ""} onChange={e => setForm({ ...form, dueDate: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                ...
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">מוכר</label><input value={form.salesperson || ""} onChange={e => setForm({ ...form, salesperson: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">סכום (₪)</label><input type="number" step="0.01" value={form.subtotal || ""} onChange={e => setForm(recalc({ ...form, subtotal: e.target.value }))} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">הנחה (₪)</label><input type="number" step="0.01" value={form.discountAmount || ""} onChange={e => setForm(recalc({ ...form, discountAmount: e.target.value }))} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">מע"מ %</label><input type="number" value={form.vatRate ?? 18} onChange={e => setForm(recalc({ ...form, vatRate: e.target.value }))} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                ...
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">שולם (₪)</label><input type="number" step="0.01" value={form.amountPaid || ""} onChange={e => setForm({ ...form, amountPaid: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">מס' הזמנה</label><input value={form.poNumber || ""} onChange={e => setForm({ ...form, poNumber: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
              </div>
              <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">תיאור פריטים</label><textarea value={form.itemDescription || ""} onChange={e => setForm({ ...form, itemDescription: e.target.value })} rows={2} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">חשבונית לקוח #{selectedItem.invoice_number || selectedItem.id}</h2>
              ...
              {detailTab === "details" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <div><div className="text-xs text-muted-foreground mb-1">לקוח</div><div className="text-sm text-white">{selectedItem.customer_name || "-"}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">סכום</div><div className="text-sm text-white font-bold">₪{Number(selectedItem.total_amount || selectedItem.amount || 0).toLocaleString()}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">תאריך</div><div className="text-sm text-white">{selectedItem.invoice_date}</div></div>
                    <div><div className="text-xs text-muted-foreground mb-1">תאריך פירעון</div><div className="text-sm text-white">{selectedItem.due_date || "-"}</div></div>
