  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Receipt className="text-blue-400" /> קבלות</h1>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="outline" onClick={() => printPage("קבלות")} className="border-[#2a2a3e] text-muted-foreground gap-1"><Printer className="h-4 w-4" />הדפסה</Button>
          <Button variant="outline" onClick={() => sendByEmail("קבלות - טכנו-כל עוזי", generateEmailBody("קבלות", filtered, { payment_number: "מספר", customer_name: "לקוח", amount: "סכום", status: "סטטוס" }))} className="border-[#2a2a3e] text-muted-foreground gap-1"><Send className="h-4 w-4" />שליחה</Button>
      ...
      <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]"><Search className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={e => { setSearch(e.target.value); pagination.setPage(1); }} placeholder="חיפוש לפי מספר, לקוח, אסמכתא..." className="pr-9 bg-[#0a0a1a] border-[#2a2a3e] text-white" /></div>
        ...
        <select value={filterMethod} onChange={e => { setFilterMethod(e.target.value); pagination.setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל האמצעים</option>{methodOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>
      ...
      <BulkActions items={filtered} selectedIds={selectedIds} onSelectionChange={setSelectedIds} actions={[
        defaultBulkActions.delete(async (ids) => { for (const id of ids) await authFetch(`${API}/finance/customer-payments/${id}`, { method: "DELETE" }); load(); }),
      ...
      <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-0">
        <div className="overflow-x-auto relative">
          {loading && <div className="absolute inset-0 bg-[#0a0a1a]/60 backdrop-blur-[1px] flex items-center justify-center z-10"><div className="flex items-center gap-2 bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg px-4 py-2 shadow-lg"><Clock className="w-4 h-4 animate-spin text-blue-400" /><span className="text-sm text-white">טוען קבלות...</span></div></div>}
          <table className="w-full text-sm">
            ...
            <tbody>
              {!loading && pagination.paginate(filtered).length === 0 ? (
                <tr><td colSpan={10} className="p-16 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-12 w-12 text-[#2a2a3e]" /> : <Receipt className="h-12 w-12 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין קבלות"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "צור קבלה ראשונה"}</p>{!(af > 0 || search) && <Button onClick={openCreate} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />קבלה חדשה</Button>}</div></td></tr>
              ) : pagination.paginate(filtered).map(r => (
                <tr key={r.id} className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30 transition-colors cursor-pointer" onClick={() => setViewDetail(r)}>
                  ...
                  <td className="px-3 py-2.5 text-center" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-center gap-1">
                      <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-white" onClick={() => setViewDetail(r)}><Eye className="h-3.5 w-3.5" /></Button>
                      ...
                      <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-red-400" onClick={async () => { if (await globalConfirm(`למחוק את קבלה '${r.payment_number}'? פעולה זו אינה ניתנת לביטול.`)) remove(r.id); }}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
            ...
            {filtered.length > 0 && (
              <tfoot><tr className="border-t-2 border-[#2a2a3e] bg-[#0a0a1a]/50 font-bold text-sm">
                <td className="px-3 py-3" /><td className="px-3 py-3 text-white" colSpan={6}>סה״כ ({filtered.length} קבלות)</td>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת קבלה" : "קבלה חדשה"}</h2>
                <Button variant="ghost" size="sm" onClick={() => setShowForm(false)}><X className="h-4 w-4" /></Button>
              </div>
              <div className="p-4 space-y-5">
                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי קבלה</h3></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><Label className="text-muted-foreground text-xs">תאריך *</Label><Input type="date" value={form.paymentDate || ""} onChange={e => setForm({ ...form, paymentDate: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                ...
                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי תשלום</h3></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><Label className="text-muted-foreground text-xs">סכום (₪) *</Label><Input type="number" step={0.01} value={form.amount || ""} onChange={e => setForm({ ...form, amount: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1 font-mono" /></div>
                  <div>
                    <Label className="text-muted-foreground text-xs">אמצעי תשלום *</Label>
                    <select value={form.paymentMethod || "bank_transfer"} onChange={e => setForm({ ...form, paymentMethod: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1">
                      {methodOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                    </select>
                  </div>
                  <div><Label className="text-muted-foreground text-xs">חשבונית מקושרת</Label><Input value={form.invoiceNumber || ""} onChange={e => setForm({ ...form, invoiceNumber: e.target.value })} placeholder="INV-2026-XXXX" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1 font-mono" /></div>
                  <div><Label className="text-muted-foreground text-xs">מס׳ אסמכתא</Label><Input value={form.referenceNumber || ""} onChange={e => setForm({ ...form, referenceNumber: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                  {form.paymentMethod === "check" && (
                    <div><Label className="text-muted-foreground text-xs">מספר שיק</Label><Input value={form.checkNumber || ""} onChange={e => setForm({ ...form, checkNumber: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1 font-mono" /></div>
                  )}
                  <div className={form.paymentMethod === "check" ? "" : "col-span-2"}>
                    <Label className="text-muted-foreground text-xs">חשבון בנק</Label>
                    <select value={form.bankAccount || ""} onChange={e => setForm({ ...form, bankAccount: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1">
                ...
                {Number(form.amount) > 0 && (
                  <div className="bg-[#0a0a1a] rounded-lg border border-emerald-500/20 p-4 flex items-center justify-between">
                ...
                <textarea value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white resize-none" placeholder="הערות..." />
              </div>
          ...
          <div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
              <div className="flex items-center gap-3">
                <h2 className="text-lg font-bold text-white">קבלה {viewDetail.payment_number}</h2>
                <Badge className={`${statusConfig[viewDetail.status]?.color || "bg-[#2a2a3e]"} border-0`}>{statusConfig[viewDetail.status]?.label || viewDetail.status}</Badge>
              </div>
            ...
            <div className="p-4 space-y-4">
              ...
              {viewDetail.notes && <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground mb-1">הערות</p><p className="text-sm text-white">{viewDetail.notes}</p></div>}
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {statusConfig[viewDetail.status]?.next?.map(nextStatus => (
                      <Button key={nextStatus} variant="outline" size="sm" className={`border-[#2a2a3e] gap-1 ${nextStatus === "completed" ? "text-emerald-400 border-emerald-500/30" : nextStatus === "cancelled" ? "text-red-400 border-red-500/30" : "text-amber-400 border-amber-500/30"}`} onClick={async () => {
            ...
            <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-end">
              <Button variant="outline" className="border-[#2a2a3e] gap-1" onClick={() => printPage(`קבלה ${viewDetail.payment_number}`)}><Printer className="h-4 w-4" />הדפסה</Button>
