...
import {
  ...
  ArrowUpDown, DollarSign, Hash, Eye, TrendingUp, Boxes
} from "lucide-react";
...
export default function CustomerProductsPage() {
  const [items, setItems] = useState<CustProduct[]>([]);
  ...
  const [editing, setEditing] = useState<CustProduct | null>(null);
  const [viewDetail, setViewDetail] = useState<CustProduct | null>(null);
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API}/finance/customer-products`);
      if (res.ok) setItems(safeArray(await res.json()));
      else setError("שגיאה בטעינת מוצרי לקוחות");
    } catch (e: any) {
  ...
  const filtered = useMemo(() => {
    let data = items.filter(i =>
      ...
      (!search || [i.product_name, i.product_code, i.customer_name, i.category]
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ status: "active", discount: 0 });
    setShowForm(true);
  };

  const openEdit = (r: CustProduct) => {
    setEditing(r);
    setForm({
  ...
  const save = async () => {
    ...
    try {
      const url = editing
        ? `${API}/finance/customer-products/${editing.id}`
        : `${API}/finance/customer-products`;
      await authFetch(url, {
  ...
  const remove = async (id: number) => {
    if (await globalConfirm("למחוק מוצר לקוח זה? פעולה זו אינה ניתנת לביטול.")) {
      await authFetch(`${API}/finance/customer-products/${id}`, { method: "DELETE" });
      load();
    }
  };
  ...
  const kpis = [
    ...
    { label: "לקוחות", value: fmt(new Set(items.map(i => i.customer_code)).size), icon: Boxes, color: "text-purple-400" },
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">ניהול מוצרים ומחירונים ללקוחות</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <ExportDropdown
            data={filtered}
            headers={{
              product_code: "קוד", product_name: "מוצר", customer_name: "לקוח",
              price: "מחיר", quantity: "כמות", category: "קטגוריה", status: "סטטוס",
            }}
            filename="customer_products"
          />
          <button
            onClick={openCreate}
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input
            ...
            placeholder="חיפוש לפי שם, קוד, לקוח..."
      ...
      <BulkActions selectedIds={selectedIds} onClear={clear} entityName="מוצרי לקוחות" actions={defaultBulkActions(selectedIds, clear, load, `${API}/finance/customer-products`)} />
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">אין מוצרי לקוחות</p>
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-emerald-400 font-bold">{r.product_code}</td>
                    ...
                    <td className="px-4 py-3 text-green-400 font-bold">{fmtCurrency(r.price)}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        ...
                        <button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.product_name || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1.5 hover:bg-muted rounded-lg" title="מחיקה">
          ...
          >
            ...
            >
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                ...
                <DetailField label="מחיר" value={fmtCurrency(viewDetail.price)} />
                ...
                <DetailField label="הנחה" value={`${fmt(viewDetail.discount)}%`} />
              ...
              {detailTab === "related" && <div className="p-5"><RelatedRecords entityType="customer-product" entityId={viewDetail.id} /></div>}
              {detailTab === "attachments" && <div className="p-5"><AttachmentsSection entityType="customer-product" entityId={viewDetail.id} /></div>}
              {detailTab === "history" && <div className="p-5"><ActivityLog entityType="customer-product" entityId={viewDetail.id} /></div>}
              <div className="p-5 border-t border-border flex justify-end gap-2">
                ...
                <button onClick={() => setViewDetail(null)} className="px-4 py-2 bg-muted text-muted-foreground rounded-lg text-sm">
                  סגור
                </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            ...
            >
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת מוצר" : "מוצר לקוח חדש"}</h2>
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">מחיר</label>
                  <input
                    type="number"
                    value={form.price || ""}
                ...
                <div>
                  <label className="block text-sm font-medium text-muted-foreground mb-1.5">הנחה %</label>
                  <input
                    type="number"
                    min={0}
                    max={100}
              ...
              <div className="p-5 border-t border-border flex gap-3">
                <button
                  onClick={save}
                  disabled={saving}
                  className="flex items-center gap-2 bg-primary text-primary-foreground px-6 py-2.5 rounded-xl hover:bg-primary/90 text-sm disabled:opacity-50"
                >
                ...
                <button
                  onClick={() => setShowForm(false)}
                  className="px-6 py-2.5 bg-muted text-muted-foreground rounded-xl text-sm"
                >
                  ביטול
                </button>
              </div>
