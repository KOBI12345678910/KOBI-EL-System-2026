    ...
    <div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg p-3 shadow-xl text-right" dir="rtl">
      ...
      {payload.map((entry: any, i: number) => (
        <div key={i} className="flex items-center justify-between gap-4 text-xs">
          ...
          <span className="font-mono text-white">{fmtC(entry.value)}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><BarChart3 className="text-cyan-400" /> גיול חובות</h1>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="outline" onClick={() => printPage("גיול חובות")} className="border-[#2a2a3e] text-muted-foreground gap-1"><Printer className="h-4 w-4" />הדפסה</Button>
          ...
          return (
            <Card key={i} className={`bg-gradient-to-br ${b.bgClass}`}>
              <CardContent className="p-4">
                ...
                <div className="mt-2 bg-[#2a2a3e] rounded-full h-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold text-white flex items-center gap-2"><BarChart3 className="h-4 w-4 text-cyan-400" />פילוח גיול לפי ישות (טופ 10)</CardTitle></CardHeader>
        ...
        <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold text-white flex items-center gap-2"><Clock className="h-4 w-4 text-amber-400" />התפלגות לפי טווח זמן</CardTitle></CardHeader>
          <CardContent className="pt-0">
            <div className="h-[200px] flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(v: number) => fmtC(v)} contentStyle={{ backgroundColor: "#1a1a2e", border: "1px solid #2a2a3e", borderRadius: "8px", direction: "rtl" }} itemStyle={{ color: "#fff" }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
            <div className="grid grid-cols-2 gap-2 mt-2">
              {bucketTotals.map((b, i) => (
                <div key={i} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-white font-mono mr-auto">{fmtC(b.value)}</span>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]"><Search className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={e => { setSearch(e.target.value); pagination.setPage(1); }} placeholder="חיפוש לפי שם..." className="pr-9 bg-[#0a0a1a] border-[#2a2a3e] text-white" /></div>
        <select value={filterType} onChange={e => { setFilterType(e.target.value); pagination.setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הסוגים</option><option value="customer">לקוחות</option><option value="supplier">ספקים</option></select>
        <select value={filterRisk} onChange={e => { setFilterRisk(e.target.value); pagination.setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל רמות הסיכון</option>{Object.entries(riskConfig).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select>
      ...
      <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-0">
        <div className="overflow-x-auto relative">
          {loading && <div className="absolute inset-0 bg-[#0a0a1a]/60 backdrop-blur-[1px] flex items-center justify-center z-10"><div className="flex items-center gap-2 bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg px-4 py-2 shadow-lg"><Clock className="w-4 h-4 animate-spin text-cyan-400" /><span className="text-sm text-white">טוען נתוני גיול...</span></div></div>}
          <table className="w-full text-sm">
            <thead><tr className="border-b border-[#2a2a3e] bg-[#0a0a1a]/50">
              ...
              <th className="px-3 py-3 text-right text-muted-foreground font-medium text-xs cursor-pointer hover:text-white" onClick={() => toggleSort("entity_name")}><div className="flex items-center gap-1">שם<ArrowUpDown className="h-3 w-3" /></div></th>
              <th className="px-3 py-3 text-right text-muted-foreground font-medium text-xs">סוג</th>
              {BUCKETS.map(b => (
                <th key={b.key} className="px-3 py-3 text-right font-medium text-xs cursor-pointer hover:text-white" onClick={() => toggleSort(b.key)} style={{ color: b.color }}><div className="flex items-center gap-1">{b.label.split(" ")[0]}<ArrowUpDown className="h-3 w-3" /></div></th>
              ))}
              <th className="px-3 py-3 text-right text-muted-foreground font-medium text-xs cursor-pointer hover:text-white" onClick={() => toggleSort("total")}><div className="flex items-center gap-1">סה״כ<ArrowUpDown className="h-3 w-3" /></div></th>
            ...
            <tbody>
              {!loading && pagination.paginate(filtered).length === 0 ? (
                <tr><td colSpan={11} className="p-16 text-center"><div className="flex flex-col items-center gap-4"><BarChart3 className="h-12 w-12 text-[#2a2a3e]" /><p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "אין נתוני גיול"}</p></div></td></tr>
                ...
                return (
                  <tr key={r.id} className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30 transition-colors cursor-pointer" onClick={() => setViewDetail(r)}>
                    ...
                    <td className="px-3 py-2.5 text-white font-medium max-w-[150px] truncate">{r.entity_name}</td>
                    <td className="px-3 py-2.5"><Badge className={`${entityTypeMap[r.entity_type]?.color || "bg-[#2a2a3e]"} border-0 text-[10px]`}>{entityTypeMap[r.entity_type]?.label || r.entity_type}</Badge></td>
                    {BUCKETS.map(b => {
                    ...
                    <td className="px-3 py-2.5 font-mono text-sm text-white font-bold">{fmtC(r.total)}</td>
                    ...
                    <td className="px-3 py-2.5" onClick={e => e.stopPropagation()}>
                      <div className="flex h-3 rounded-full overflow-hidden bg-[#2a2a3e] min-w-[100px]">
                        {BUCKETS.map(b => {
                          const val = Number((r as any)[b.key] || 0);
                    ...
                    <td className="px-3 py-2.5 text-center" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-red-400" onClick={async () => { if (await globalConfirm(`למחוק את גיול '${r.entity_name}'? פעולה זו אינה ניתנת לביטול.`)) remove(r); }}><Trash2 className="h-3.5 w-3.5" /></Button>
                      </div>
                    </td>
                  </tr>
                );
            ...
            {filtered.length > 0 && (
              <tfoot><tr className="border-t-2 border-[#2a2a3e] bg-[#0a0a1a]/50 font-bold text-sm">
                <td className="px-3 py-3" /><td className="px-3 py-3 text-white" colSpan={2}>סה״כ ({filtered.length} ישויות)</td>
                {BUCKETS.map(b => {
                ...
                <td className="px-3 py-3 font-mono text-white">{fmtC(filtered.reduce((s, i) => s + Number(i.total || 0), 0))}</td>
                <td colSpan={3} />
              </tr></tfoot>
            )}
          </table>
          ...
          <div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
              <div className="flex items-center gap-3">
                <h2 className="text-lg font-bold text-white">{viewDetail.entity_name}</h2>
            ...
            <div className="p-4 space-y-4">
              <div className="bg-[#0a0a1a] rounded-lg border border-[#2a2a3e] p-4">
                <p className="text-sm font-semibold text-white mb-3">פירוט גיול</p>
                  ...
                  {BUCKETS.map(b => {
                    ...
                    return (
                      <div key={b.key}>
                        ...
                        <div className="bg-[#2a2a3e] rounded-full h-2">
                ...
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-[#2a2a3e]">
                  <span className="text-white font-semibold">סה״כ חוב</span>
                  <span className="text-xl font-bold font-mono text-white">{fmtC(viewDetail.total)}</span>
                </div>
              </div>
              ...
              <div className="bg-[#0a0a1a] rounded-lg p-4">
                <div className="flex h-6 rounded-lg overflow-hidden">
                  {BUCKETS.map(b => {
                    ...
                    const pct = Number(viewDetail.total) > 0 ? (val / Number(viewDetail.total)) * 100 : 0;
                    return pct > 0 ? (
                      <div key={b.key} className="h-full flex items-center justify-center text-[9px] font-bold text-white" style={{ width: `${pct}%`, backgroundColor: b.color, minWidth: pct > 5 ? undefined : "0" }}>
            ...
            <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-end">
              <Button variant="outline" className="border-[#2a2a3e] gap-1" onClick={() => printPage(`גיול — ${viewDetail.entity_name}`)}><Printer className="h-4 w-4" />הדפסה</Button>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת גיול" : "רשומת גיול חדשה"}</h2>
                <Button variant="ghost" size="sm" onClick={() => setShowForm(false)}><X className="h-4 w-4" /></Button>
              </div>
              <div className="p-4 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div><Label className="text-muted-foreground text-xs">שם *</Label><Input value={form.entity_name || ""} onChange={e => setForm({ ...form, entity_name: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">סוג *</Label><select value={form.entity_type || "customer"} onChange={e => setForm({ ...form, entity_type: e.target.value })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option value="customer">לקוח</option><option value="supplier">ספק</option></select></div>
                </div>
                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">סכומים לפי טווח</h3></div>
                <div className="grid grid-cols-2 gap-3">
                  {BUCKETS.map(b => (
                    <div key={b.key}><Label className="text-xs" style={{ color: b.color }}>{b.label} (₪)</Label><Input type="number" min={0} value={(form as any)[b.key] ?? ""} onChange={e => setForm({ ...form, [b.key]: Number(e.target.value) || 0 })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1 font-mono" /></div>
                  ))}
                </div>
                <div className="bg-[#0a0a1a] rounded-lg p-3 border border-[#2a2a3e] flex justify-between items-center">
                  <span className="text-muted-foreground text-sm">סה״כ חוב</span>
                  <span className="text-lg font-bold font-mono text-white">{fmtC(BUCKETS.reduce((s, b) => s + (Number((form as any)[b.key]) || 0), 0))}</span>
                </div>
                <div><Label className="text-muted-foreground text-xs">הערות</Label><textarea value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} rows={2} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1 resize-none" /></div>
              </div>
              <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-end">
