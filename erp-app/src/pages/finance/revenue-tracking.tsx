...
import { authFetch } from "@/lib/utils";
    ...
    const STATUSES = ["מעל יעד", "ביעד", "מתחת", "הרבה מתחת"] as const;
    const SC: Record<string, string> = { "מעל יעד": "bg-emerald-500/20 text-emerald-300", "ביעד": "bg-green-500/20 text-green-300", "מתחת": "bg-yellow-500/20 text-yellow-300", "הרבה מתחת": "bg-red-500/20 text-red-300" };
    const CATS = ["חודשי","רבעוני","שנתי","מוצר","אזור"];
...
export default function RevenueTracking() {
  ...
  const [search, setSearch] = useState("");
    ...
    try {
      setError(null);
      const res = await authFetch("/api/finance-control/revenue-tracking");
      if (res.ok) {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.item?.toLowerCase().includes(s) || r.dimension?.toLowerCase().includes(s) || r.period?.toLowerCase().includes(s)); }
      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingUp className="h-6 w-6 text-cyan-400" />מעקב הכנסות</h1><p className="text-sm text-muted-foreground mt-1">מעקב מגמות הכנסות, תחזיות ויעדים</p></div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"הכנסות YTD","v":"₪14.2M","c":"text-cyan-400","t":"+15%","u":true},{"l":"יעד שנתי","v":"₪20M","c":"text-blue-400","t":"71%","u":true},{"l":"צמיחה","v":"+18%","c":"text-green-400","t":"+3%","u":true},{"l":"תחזית","v":"₪19.5M","c":"text-emerald-400","t":"97.5%","u":true},{"l":"MRR","v":"₪320K","c":"text-purple-400","t":"+8%","u":true},{"l":"Pipeline","v":"₪5.2M","c":"text-amber-400","t":"+12%","u":true}].map((k: any, i: number) => (
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("item")}>פריט<SI field="item" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("target")}>יעד<SI field="target" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("actual")}>בפועל<SI field="actual" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("achievement")}>עמידה<SI field="achievement" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("growth")}>צמיחה<SI field="growth" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingUp className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין מעקב הכנסות"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "רשומה חדשה ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />רשומה חדשה</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 "><div className="font-medium text-white">{row.item}</div></td>
            <td className="p-3 text-muted-foreground">{row.dimension}</td>
            <td className="p-3 text-muted-foreground text-xs">{row.period}</td>
            <td className="p-3 font-mono text-muted-foreground">{fmt(row.target)}</td>
            <td className="p-3 font-mono text-emerald-400">{fmt(row.actual)}</td>
            <td className="p-3 "><div className="flex items-center gap-2"><div className="w-12 bg-gray-700 rounded-full h-2"><div className={`rounded-full h-2 ${row.achievement>=100?"bg-green-500":row.achievement>=80?"bg-yellow-500":"bg-red-500"}`} style={{width:`${Math.min(row.achievement,100)}%`}} /></div><span className="font-mono text-xs">{row.achievement}%</span></div></td>
            <td className="p-3 "><span className={`font-mono ${row.growthUp?"text-green-400":"text-red-400"}`}>{Number(row.growth)>0?"+":""}{row.growth}%</span></td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div><Label className="text-muted-foreground text-xs">ממד *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>חודשי</option><option>רבעוני</option><option>שנתי</option><option>מוצר</option><option>אזור</option></select></div>
            <div><Label className="text-muted-foreground text-xs">תקופה</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>ינואר</option><option>פברואר</option><option>מרץ</option><option>Q1</option><option>Q2</option><option>2026</option></select></div>
            <div><Label className="text-muted-foreground text-xs">יעד</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">בפועל</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">תחזית</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>מעל יעד</option><option>ביעד</option><option>מתחת</option><option>הרבה מתחת</option></select></div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">פריט</p><p className="text-white mt-1 font-medium">{dr.item}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">יעד</p><p className="text-white mt-1 font-medium">{fmt(dr.target)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">בפועל</p><p className="text-white mt-1 font-medium">{fmt(dr.actual)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">תחזית</p><p className="text-white mt-1 font-medium">{fmt(dr.forecast)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">עמידה</p><p className="text-white mt-1 font-medium">{dr.achievement}%</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">צמיחה</p><p className="text-white mt-1 font-medium">{dr.growth}%</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">Pipeline</p><p className="text-white mt-1 font-medium">{fmt(dr.pipeline)}</p></div>
