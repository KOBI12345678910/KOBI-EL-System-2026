...
export default function FinancialReportsPage() {
  ...
  const kpis = [
    { label: "סה\"כ נכסים", value: `₪${fmt(data.totalAssets)}`, icon: Building2, color: "from-blue-500 to-blue-600", textColor: "text-white" },
    { label: "סה\"כ התחייבויות", value: `₪${fmt(data.totalLiabilities)}`, icon: Receipt, color: "from-red-500 to-red-600", textColor: "text-white" },
    { label: "הון עצמי", value: `₪${fmt(data.totalEquity)}`, icon: Scale, color: "from-purple-500 to-purple-600", textColor: "text-white" },
    { label: "רווח נקי", value: `₪${fmt(data.netIncome)}`, icon: TrendingUp, color: Number(data.netIncome) >= 0 ? "from-green-500 to-green-600" : "from-red-500 to-red-600", textColor: "text-white" },
  ];
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={load} className="flex items-center gap-1.5 bg-indigo-600 text-white px-3 py-2 rounded-lg hover:bg-indigo-700 text-sm"><RefreshCw size={16} /> רענון</button>
          <button onClick={() => printPage("דוחות כספיים מאוחדים")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Printer size={16} /> הדפסה</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
        <div className="bg-card border border-border/50 rounded-2xl p-5">
          <h3 className="text-sm font-bold text-white mb-3">רשומות קשורות</h3>
          <RelatedRecords entityType="financial-reports" entityId="dashboard" />
        </div>
        <div className="bg-card border border-border/50 rounded-2xl p-5">
          <h3 className="text-sm font-bold text-white mb-3">היסטוריה</h3>
