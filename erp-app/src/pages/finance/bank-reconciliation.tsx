    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => printPage("התאמות בנק")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Printer size={16} /> הדפסה</button>
          <button onClick={() => sendByEmail("התאמות בנק - טכנו-כל עוזי", generateEmailBody("התאמות בנק", recs, { reconciliation_number: "מספר", bank_account_name: "חשבון", difference: "הפרש", status: "סטטוס" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm"><Send size={16} /> שליחה</button>
          <button onClick={openCreate} className="flex items-center gap-2 bg-blue-600 text-white px-3 py-2 rounded-lg hover:bg-blue-700 shadow-lg text-sm"><Plus size={16} /> התאמה חדשה</button>
        </div>
      </div>
                  ...
                  <tr className={`border-b hover:bg-blue-50/30 ${expandedId === r.id ? 'bg-blue-50/50' : ''}`}>
                    ...
                    <td className="px-2 py-2">
                      <div className="flex gap-1">
                        ...
                        <button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.reconciled_by_name || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1 hover:bg-red-500/10 rounded text-red-500"><Trash2 size={13} /></button>
                      </div>
                    </td>
                  </tr>
                  {expandedId === r.id && (
                    <tr><td colSpan={12} className="p-0">
                      <div className="bg-blue-50/30 border-t border-blue-100 px-6 py-4">
                        ...
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <button onClick={() => openAddItem(r.id)} className="flex items-center gap-1 text-xs bg-blue-600 text-white px-2 py-1 rounded hover:bg-blue-700"><Plus size={12} /> הוסף פריט</button>
                        </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={save} className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700"><Save size={16} /> {editing ? "עדכון" : "שמירה"}</button>
        ...
        {showItemForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onClick={() => setShowItemForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-card border border-border text-foreground rounded-2xl shadow-2xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={saveItem} className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700"><Save size={16} /> שמור פריט</button>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">התאמת בנק: {selectedItem.rec_number || `#${selectedItem.id}`}</h2>
            ...
            <div className="p-6">
              {detailTab === "details" && (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <div><div className="text-xs text-muted-foreground mb-1">תאריך דף חשבון</div><div className="text-sm text-white">{selectedItem.statement_date}</div></div>
                  <div><div className="text-xs text-muted-foreground mb-1">חשבון בנק</div><div className="text-sm text-white">{selectedItem.bank_account_name || "-"}</div></div>
                  <div><div className="text-xs text-muted-foreground mb-1">יתרת דף חשבון</div><div className="text-sm text-white font-bold">₪{Number(selectedItem.statement_balance || 0).toLocaleString()}</div></div>
                  <div><div className="text-xs text-muted-foreground mb-1">יתרת ספרים</div><div className="text-sm text-white font-bold">₪{Number(selectedItem.book_balance || 0).toLocaleString()}</div></div>
                  <div><div className="text-xs text-muted-foreground mb-1">הפרש</div><div className="text-sm text-white">{selectedItem.difference || "0"}</div></div>
                  <div><div className="text-xs text-muted-foreground mb-1">סטטוס</div><div className="text-sm text-white">{selectedItem.status}</div></div>
                </div>
              )}
