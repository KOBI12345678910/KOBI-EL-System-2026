...
import {
  BookOpen, Search, ArrowUpDown, AlertTriangle, Users,
  Building2, ArrowUpRight, ArrowDownRight, Eye, X, Hash
} from "lucide-react";
...
export default function EntityLedgerPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API}/entity-ledger`);
      if (res.ok) {
  ...
  const customerCount = new Set(items.filter(i => i.entity_type === "customer").map(i => i.entity_name)).size;
  const supplierCount = new Set(items.filter(i => i.entity_type === "supplier").map(i => i.entity_name)).size;

  const kpis = [
    { label: "סה\"כ תנועות", value: fmt(items.length), icon: Hash, color: "text-blue-400" },
    { label: "לקוחות", value: fmt(customerCount), icon: Users, color: "text-cyan-400" },
    { label: "ספקים", value: fmt(supplierCount), icon: Building2, color: "text-purple-400" },
    { label: "סה\"כ חיוב", value: fmtCurrency(totalDebit), icon: ArrowUpRight, color: "text-green-400" },
    { label: "סה\"כ זיכוי", value: fmtCurrency(totalCredit), icon: ArrowDownRight, color: "text-red-400" },
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-2">
            <BookOpen className="text-amber-400 w-6 h-6" /> כרטסת לקוחות / ספקים
          </h1>
          <p className="text-sm text-muted-foreground mt-1">הצגת תנועות פיננסיות לפי ישות — Entity Ledger</p>
        </div>
        <ExportDropdown
          data={filtered}
          headers={{ entity_name: "ישות", entity_type: "סוג", entry_date: "תאריך", description: "תיאור", debit: "חיוב", credit: "זיכוי", balance: "יתרה" }}
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש לפי שם, תיאור, אסמכתא..."
              ...
              <tbody>
                {pagination.paginate(filtered).map(r => (
                  <tr key={r.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-white font-medium">{r.entity_name || "—"}</td>
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  <BookOpen className="w-5 h-5 text-amber-400" /> תנועת כרטסת
                </h2>
              ...
              {detailTab === "details" && (
                <div className="p-5 grid grid-cols-2 gap-4">
                  ...
                  <DetailField label="חיוב" value={fmtCurrency(viewDetail.debit)} />
                  <DetailField label="זיכוי" value={fmtCurrency(viewDetail.credit)} />
