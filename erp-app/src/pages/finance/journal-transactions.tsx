...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  voided: { label: "מבוטל", color: "bg-[#2a2a3e] text-muted-foreground/60" },
};
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><BookOpen className="text-blue-400" /> יומן תנועות</h1>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="outline" onClick={() => printPage("יומן תנועות")} className="border-[#2a2a3e] text-muted-foreground gap-1"><Printer className="h-4 w-4" />הדפסה</Button>
        ...
        return trendData.length > 1 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-4">
              <div className="flex items-center gap-2 mb-3 text-sm font-semibold text-white"><BarChart3 className="h-4 w-4 text-blue-400" /> חיוב מול זיכוי לפי חודש</div>
            ...
            {pieData.length > 1 && (
              <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3 text-sm font-semibold text-white"><Columns className="h-4 w-4 text-purple-400" /> חלוקה לפי חשבון (Top 7)</div>
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
      ...
      <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]"><Search className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={e => { setSearch(e.target.value); pagination.setPage(1); }} placeholder="חיפוש לפי מספר, חשבון, תיאור, אסמכתא..." className="pr-9 bg-[#0a0a1a] border-[#2a2a3e] text-white" /></div>
        ...
        <select value={filterType} onChange={e => { setFilterType(e.target.value); pagination.setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הסוגים</option><option value="debit">חיוב</option><option value="credit">זיכוי</option></select>
      ...
      <BulkActions items={filtered} selectedIds={selectedIds} onSelectionChange={setSelectedIds} actions={[
        defaultBulkActions.delete(async (ids) => { for (const id of ids) await authFetch(`${API}/finance/journal-transactions/${id}`, { method: "DELETE", headers }); load(); }),
      ...
      <Card className="bg-[#1a1a2e]/80 border-[#2a2a3e]"><CardContent className="p-0">
        <div className="overflow-x-auto relative">
          {tableLoading && <div className="absolute inset-0 bg-[#0a0a1a]/60 backdrop-blur-[1px] flex items-center justify-center z-10"><div className="flex items-center gap-2 bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg px-4 py-2 shadow-lg"><Loader2 className="w-4 h-4 animate-spin text-blue-400" /><span className="text-sm text-white">טוען תנועות יומן...</span></div></div>}
          <table className="w-full text-sm">
            ...
            <tbody>
              {!tableLoading && pagination.paginate(filtered).length === 0 ? (
                <tr><td colSpan={11} className="p-16 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-12 w-12 text-[#2a2a3e]" /> : <BookOpen className="h-12 w-12 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין תנועות ביומן"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את הסינון" : "צור פקודת יומן ראשונה"}</p>{!(af > 0 || search) && <Button onClick={openCreate} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />פקודת יומן חדשה</Button>}</div></td></tr>
                ...
                return (
                  <tr key={r.id} className="border-b border-[#2a2a3e]/50 hover:bg-[#2a2a3e]/30 transition-colors cursor-pointer" onClick={() => setSelectedItem(r)}>
                    ...
                    <td className="px-3 py-2.5 text-white text-xs max-w-[120px] truncate">{r.account_name || "—"}</td>
                    ...
                    <td className="px-3 py-2.5 text-center" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-red-400" onClick={async () => { if (await globalConfirm(`למחוק את תנועת יומן '${r.transaction_number}'? פעולה זו אינה ניתנת לביטול.`)) remove(r.id); }}><Trash2 className="h-3.5 w-3.5" /></Button>
                      </div>
                    </td>
                  </tr>
                );
            ...
            {filtered.length > 0 && (
              <tfoot><tr className="border-t-2 border-[#2a2a3e] bg-[#0a0a1a]/50 font-bold text-sm">
                <td className="px-3 py-3" /><td className="px-3 py-3 text-white" colSpan={4}>סה״כ ({filtered.length} תנועות)</td>
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
                <div className="flex items-center gap-3">
                  <h2 className="text-lg font-bold text-white">{editing ? "עריכת פקודת יומן" : "פקודת יומן חדשה"}</h2>
              ...
              <div className="p-4 space-y-5">
                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי פקודה</h3></div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div><Label className="text-muted-foreground text-xs">תאריך *</Label><Input type="date" value={form.transactionDate || ""} onChange={e => setForm({ ...form, transactionDate: e.target.value })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">שנת כספים</Label><Input type="number" value={form.fiscalYear || ""} onChange={e => setForm({ ...form, fiscalYear: Number(e.target.value) })} className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1 font-mono" /></div>
                  <div><Label className="text-muted-foreground text-xs">תקופה</Label><select value={form.fiscalPeriod || 1} onChange={e => setForm({ ...form, fiscalPeriod: Number(e.target.value) })} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1">{Array.from({ length: 12 }, (_, i) => <option key={i + 1} value={i + 1}>{i + 1}</option>)}</select></div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><Label className="text-muted-foreground text-xs">תיאור כללי</Label><Input value={form.description || ""} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="תיאור הפקודה..." className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
                  <div><Label className="text-muted-foreground text-xs">אסמכתא</Label><Input value={form.reference || ""} onChange={e => setForm({ ...form, reference: e.target.value })} placeholder="מספר מסמך מקור..." className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1 font-mono" /></div>
                </div>

                <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">שורות חיוב/זיכוי</h3></div>
                <div className="bg-[#0a0a1a] rounded-lg border border-[#2a2a3e] overflow-visible">
                  <table className="w-full text-sm">
                        ...
                        <tr key={li.id} className="border-b border-[#2a2a3e]/50">
                          ...
                          <td className="px-3 py-1 relative">
                            ...
                            {activeLineId === li.id && filteredAccounts.length > 0 && (
                              <div className="absolute z-30 top-full mt-0 right-0 w-72 bg-[#1a1a2e] border border-[#2a2a3e] rounded-lg shadow-xl max-h-48 overflow-y-auto">
                                {filteredAccounts.map(a => (
                                  <button key={a.id} onClick={() => selectAccount(li.id, a)} className="w-full px-3 py-2 text-right text-sm hover:bg-[#2a2a3e] transition-colors flex items-center gap-2 border-b border-[#2a2a3e]/30">
                                    <span className="font-mono text-xs text-cyan-400 min-w-[60px]">{a.account_number}</span>
                                    <span className="text-white text-xs truncate flex-1">{a.account_name}</span>
                          ...
                          <td className="px-3 py-1"><Input value={li.accountName} onChange={e => updateLine(li.id, "accountName", e.target.value)} placeholder="שם חשבון..." className="bg-transparent border-0 text-white h-8 px-1 text-sm" /></td>
                          <td className="px-3 py-1"><Input value={li.description} onChange={e => updateLine(li.id, "description", e.target.value)} placeholder="תיאור..." className="bg-transparent border-0 text-white h-8 px-1 text-sm" /></td>
                ...
                <textarea value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white resize-none" placeholder="הערות לפקודת יומן..." />
              </div>
              <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-between">
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setSelectedItem(null)}>
          <div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]">
              <div className="flex items-center gap-3">
                <h2 className="text-lg font-bold text-white">{selectedItem.transaction_number}</h2>
            ...
            <div className="flex items-center gap-2 p-4 border-t border-[#2a2a3e] justify-end">
              <Button variant="outline" className="border-[#2a2a3e] gap-1" onClick={() => printPage(`פקודת יומן ${selectedItem.transaction_number}`)}><Printer className="h-4 w-4" />הדפסה</Button>
