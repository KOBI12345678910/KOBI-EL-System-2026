  ...
  return (
    <div className="relative">
      ...
      {open && filtered.length > 0 && (
        <div className="absolute z-50 top-full mt-1 w-64 bg-[#1a1a2e] border border-slate-700 rounded-lg shadow-xl max-h-48 overflow-y-auto">
          {filtered.map((a: any) => (
            <button key={a.id} type="button"
              className="w-full px-3 py-1.5 text-right text-xs text-white hover:bg-slate-700 flex items-center gap-2"
    ...
    <div className="bg-slate-950/50 border-t border-slate-700/30">
      <table className="w-full text-xs">
        ...
        <tbody>
          {lines.map((line: any, i: number) => (
            <tr key={line.id || i} className="border-b border-slate-800/30 hover:bg-slate-800/20">
              ...
              <td className="p-2 text-white">{line.account_name || "—"}</td>
        ...
        <tfoot><tr className="bg-slate-800/40 border-t border-slate-700">
          <td colSpan={3} className="p-2 text-xs font-bold text-white">סה״כ</td>
    ...
    <Card className="bg-slate-900/80 border-slate-600/50">
      <CardHeader className="pb-2 pt-3 px-4">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm text-white flex items-center gap-2"><Plus className="w-4 h-4 text-blue-400" />פקודת יומן חדשה — רישום כפול</CardTitle>
      ...
      <CardContent className="p-4 pt-0 space-y-4">
        ...
        <div className="bg-slate-950/40 rounded-lg border border-slate-700/50 overflow-hidden">
          <div className="bg-slate-800/40 border-b border-slate-700/50 px-3 py-2 flex items-center justify-between">
            <span className="text-xs font-semibold text-white">שורות פקודה (רישום כפול)</span>
    ...
    <div className="space-y-4 p-4 max-w-7xl mx-auto" dir="rtl">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
          className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-white h-8">
      ...
      <BulkActions items={filtered} selectedIds={selectedIds} onSelectionChange={setSelectedIds} actions={[
        defaultBulkActions.delete(async (ids) => { for (const id of ids) await authFetch(`${API}/journal-entries/${id}`, { method: "DELETE" }); refetch(); }),
          ...
          <CardContent className="p-0">
            <table className="w-full text-xs">
                  ...
                  return (
                    <React.Fragment key={entry.id}>
                      ...
                      >
                        ...
                        <td className="p-2.5 text-white font-medium truncate max-w-[200px]">{entry.description}</td>
                        ...
                        <td className="p-2.5 font-mono font-bold text-white text-left" dir="ltr">{fmt(Number(entry.amount || 0))}</td>
                        <td className="p-2.5">
                          ...
                          <td colSpan={10} className="p-0">
                            <div className="bg-slate-950/30 border-t border-b border-slate-700/30 p-3 space-y-2">
                              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
                                <div><span className="text-muted-foreground">סוג: </span><span className="text-white">{entry.entry_type || "standard"}</span></div>
                                <div><span className="text-muted-foreground">שנת כספים: </span><span className="text-white">{entry.fiscal_year || "—"}</span></div>
                                <div><span className="text-muted-foreground">תקופה: </span><span className="text-white">{entry.fiscal_period || "—"}</span></div>
                                <div><span className="text-muted-foreground">נוצר ע״י: </span><span className="text-white">{entry.created_by_name || "—"}</span></div>
              ...
              <tfoot><tr className="bg-slate-800/50 border-t-2 border-slate-600">
                <td colSpan={8} className="p-2.5 text-white font-bold">{'סה"כ'} {filtered.length} פקודות</td>
