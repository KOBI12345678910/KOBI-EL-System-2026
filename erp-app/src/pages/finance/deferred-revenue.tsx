...
import {
  TrendingUp, Search, Plus, Edit2, Trash2, X, Save, AlertTriangle,
  ArrowUpDown, DollarSign, Hash, Clock, Eye, Calendar
} from "lucide-react";
...
const categoryMap: Record<string, string> = {
  subscription: "מנוי", service: "שירות", license: "רישיון", project: "פרויקט", other: "אחר",
};
...
export default function DeferredRevenuePage() {
  ...
  const [editing, setEditing] = useState<DeferredRevenue | null>(null);
  const [viewDetail, setViewDetail] = useState<DeferredRevenue | null>(null);
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API}/deferred-revenue`);
  ...
  const filtered = useMemo(() => {
    let data = items.filter(i =>
      ...
      (!search || [i.description, i.customer_name].some(f => f?.toLowerCase().includes(search.toLowerCase())))
    );
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ category: "subscription", status: "active", startDate: new Date().toISOString().slice(0, 10) });
    setShowForm(true);
  };

  const openEdit = (r: DeferredRevenue) => {
    setEditing(r);
    setForm({
      description: r.description, customerName: r.customer_name, totalAmount: r.total_amount,
  ...
  const save = async () => {
    setSaving(true);
    try {
      const url = editing ? `${API}/deferred-revenue/${editing.id}` : `${API}/deferred-revenue`;
      await authFetch(url, {
  ...
  const remove = async (id: number) => {
    if (confirm("למחוק הכנסה נדחית זו?")) {
      await authFetch(`${API}/deferred-revenue/${id}`, { method: "DELETE" });
      load();
    }
  };

  const totalAmount = items.reduce((s, i) => s + Number(i.total_amount || 0), 0);
  const totalRecognized = items.reduce((s, i) => s + Number(i.recognized_amount || 0), 0);
  ...
  const kpis = [
    ...
    { label: "הוכר", value: fmtCurrency(totalRecognized), icon: TrendingUp, color: "text-emerald-400" },
    { label: "יתרה", value: fmtCurrency(totalRemaining), icon: Clock, color: "text-amber-400" },
    { label: "פעילות", value: fmt(items.filter(i => i.status === "active").length), icon: Calendar, color: "text-cyan-400" },
  ];
    ...
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-2">
            <TrendingUp className="text-green-400 w-6 h-6" /> הכנסות נדחות
          </h1>
          <p className="text-sm text-muted-foreground mt-1">ניהול הכנסות נדחות והכרה בהכנסות לאורך זמן</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <ExportDropdown
            data={filtered}
            headers={{ description: "תיאור", customer_name: "לקוח", total_amount: "סכום", recognized_amount: "הוכר", remaining_amount: "יתרה", status: "סטטוס" }}
            filename="deferred_revenue"
          />
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש לפי תיאור, לקוח..."
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">אין הכנסות נדחות</p>
          <p className="text-sm mt-1">{search || filterStatus !== "all" ? "נסה לשנות את הסינון" : "לחץ על 'הכנסה חדשה' להתחיל"}</p>
        </div>
      ) : (<>
        <BulkActions selectedIds={selectedIds} onClear={clear} entityName="items" actions={defaultBulkActions(selectedIds, clear, load, `${API}/deferred-revenue`)} />
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              {detailTab === "details" ? (
                ...
                <DetailField label="חודשי" value={fmtCurrency(viewDetail.monthly_amount)} />
                ...
                <div className="p-5"><RelatedRecords entityType="deferred-revenue" entityId={viewDetail?.id} /></div>
                ) : detailTab === "attachments" ? (
                <div className="p-5"><AttachmentsSection entityType="deferred-revenue" entityId={viewDetail?.id} /></div>
                ) : (
                <div className="p-5"><ActivityLog entityType="deferred-revenue" entityId={viewDetail?.id} /></div>
                )}
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכה" : "הכנסה נדחית חדשה"}</h2>
              ...
              <div className="p-5 grid grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">סכום</label><input type="number" value={form.totalAmount || ""} onChange={e => setForm({ ...form, totalAmount: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">תחילה</label><input type="date" value={form.startDate || ""} onChange={e => setForm({ ...form, startDate: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">סיום</label><input type="date" value={form.endDate || ""} onChange={e => setForm({ ...form, endDate: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm" /></div>
                <div><label className="block text-sm font-medium text-muted-foreground mb-1.5">קטגוריה</label><select value={form.category || "subscription"} onChange={e => setForm({ ...form, category: e.target.value })} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm">{Object.entries(categoryMap).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></div>
