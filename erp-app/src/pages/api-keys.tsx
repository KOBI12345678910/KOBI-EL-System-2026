  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800 bg-amber-600/5">
          <h3 className="font-bold text-white">{apiKey ? "עריכת מפתח" : "הוספת מפתח API חדש"}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            ...
            <select value={form.providerId} onChange={e => setForm({ ...form, providerId: Number(e.target.value) })} className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-amber-500">
              {providers.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">שם מזהה</label>
            <input value={form.keyName} onChange={e => setForm({ ...form, keyName: e.target.value })} className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-amber-500" placeholder="Production Key 1" required />
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">מפתח API</label>
            <input value={form.apiKey} onChange={e => setForm({ ...form, apiKey: e.target.value })} type="password" dir="ltr" className="w-full bg-[#1a1d23] border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-amber-500 text-left font-mono" placeholder="sk-..." required />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-800">
            ...
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm bg-amber-600 text-white rounded-xl hover:bg-amber-700 disabled:opacity-50">{saving ? "שומר..." : "שמור"}</button>
          </div>
        </form>
      </motion.div>
    </motion.div>
    ...
    <div className="min-h-screen bg-[#080a0f] text-white" dir="rtl">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-6">
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl font-black text-white tracking-tight">מפתחות API</h1>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setEditingKey("new")} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-600 text-white hover:bg-amber-700 transition-colors text-sm">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-[#0d1117] border border-gray-800 rounded-2xl"><Key className="w-12 h-12 text-muted-foreground mx-auto mb-4" /><h3 className="text-white font-bold mb-2">אין מפתחות API</h3><p className="text-muted-foreground text-sm">הוסף מפתח API ראשון לחיבור לספקי AI</p></div>
        ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {apiKeys.map(key => {
                    ...
                    return (
                      <tr key={key.id} className="border-b border-gray-800/50 hover:bg-card/[0.02] transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-xs bg-gray-900 px-2.5 py-1 rounded-lg border border-gray-800 text-muted-foreground">
                              {isRevealed ? key.apiKey : "••••••••••••••••••••"}
                            </span>
                            <button onClick={() => toggleReveal(key.id)} className="p-1 text-muted-foreground hover:text-white">
                        ...
                        <td className="px-4 py-3">
                          {key.isActive
                            ...
                            : <span className="text-[10px] px-2 py-1 rounded-lg bg-gray-800 text-muted-foreground">מושבת</span>}
                        </td>
