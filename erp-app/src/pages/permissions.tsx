...
export default function PermissionsPage() {
  ...
  const { data: allModules = [] } = useQuery<PlatformModule[]>({
    queryKey: ["platform-modules-full"],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-3xl font-bold text-white">הרשאות ותפקידים</h1>
                      ...
                      <Card key={role.id} className="flex flex-col overflow-hidden">
                        <div className="p-5 border-b border-border/50 bg-muted/10 flex items-center justify-between">
                          ...
                          <div className="flex gap-1">
                            ...
                            {!role.isSystem && (
                              <button onClick={async () => { if (globalConfirm("האם למחוק תפקיד זה?")) deleteRoleMutation.mutate(role.id); }} className="p-1.5 text-muted-foreground hover:text-destructive transition-colors">
                    ...
                    assignments.map((a) => {
                      ...
                      return (
                        <tr key={a.id} className="border-b border-border/30 hover:bg-card/[0.02] transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <button
                              onClick={async () => { if (globalConfirm("הסר שיוך זה?")) removeAssignmentMutation.mutate(a.id); }}
