// ============================================================
// Shared compact list shell for orchestration pages.
// Hebrew RTL. Mirrors the governance _GovernanceTable pattern.
// ============================================================
...
export function OrchestrationTable<T extends { id: number }>(p: Props<T>) {
  const [q, setQ] = useState("");
  const qs = useMemo(() => {
    const s = new URLSearchParams();
    if (q) s.set(p.searchParamName ?? "q", q);
    if (p.extraQuery) {
      for (const [k, v] of Object.entries(p.extraQuery)) {
        if (v !== undefined && v !== null && v !== "") s.set(k, String(v));
      }
    }
    s.set("limit", "200");
    return s.toString();
  }, [q, p.searchParamName, p.extraQuery]);
            ...
            <TableBody>
              {isLoading ? (
                ...
                rows.map((row) => (
                  <TableRow
                    key={row.id}
                    className={p.onRowClick ? "cursor-pointer hover:bg-muted/50" : ""}
...
export function OrchStatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    claimed: "bg-indigo-100 text-indigo-900",
    resolved: "bg-green-100 text-green-800",
    dismissed: "bg-gray-100 text-gray-800",
    sent: "bg-sky-100 text-sky-900",
    read: "bg-slate-200 text-slate-800",
...
export function formatHe(dt: string | null | undefined): string {
  if (!dt) return "—";
  try { return new Date(dt).toLocaleString("he-IL"); } catch { return dt; }
}
