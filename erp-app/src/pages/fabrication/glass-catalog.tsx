    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 text-white p-6" dir="rtl">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder={"חיפוש זכוכית..."} className="w-full bg-gray-800/50 border border-gray-700 rounded-lg pr-10 pl-4 py-2 text-sm" />
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={g.id} className="border-b border-gray-700/30 hover:bg-gray-700/20 cursor-pointer" onClick={() => { setDetail(g); setDetailTab("details"); }}>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={f.key}>
                    ...
                    {f.type === "select" ? (
                      <select value={form[f.key] || ""} onChange={e => setForm({ ...form, [f.key]: e.target.value })} className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-sm">{f.options?.map(o => <option key={o} value={o}>{o}</option>)}</select>
                    ) : (
