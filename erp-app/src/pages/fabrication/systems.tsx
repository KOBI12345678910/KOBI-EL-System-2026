    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 text-white p-6" dir="rtl">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder={"חיפוש מערכת..."} className="w-full bg-gray-800/50 border border-gray-700 rounded-lg pr-10 pl-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500" />
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={s.id} className="border-b border-gray-700/30 hover:bg-gray-700/20 cursor-pointer" onClick={() => { setDetail(s); setDetailTab("details"); }}>
                  ...
                  <td className="p-3 flex gap-1" onClick={e => e.stopPropagation()}>
                    <button onClick={() => openEdit(s)} className="p-1 hover:bg-gray-600 rounded"><Edit2 className="w-4 h-4" /></button>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={f.key}>
                    ...
                    {f.type === "select" ? (
                      <select value={form[f.key] || ""} onChange={e => setForm({ ...form, [f.key]: e.target.value })} className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-sm">
                ...
                <div className="col-span-2">
                  ...
                  <textarea value={form.description || ""} onChange={e => setForm({ ...form, description: e.target.value })} rows={2} className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-sm" />
                </div>
              </div>
        ...
        {detail && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setDetail(null)}>
            <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2 mb-4 border-b border-gray-700">
                {["details", "related", "attachments", "history"].map(tab => (
                  <button key={tab} onClick={() => setDetailTab(tab)} className={`px-4 py-2 text-sm ${detailTab === tab ? "text-indigo-400 border-b-2 border-indigo-400" : "text-muted-foreground hover:text-white"}`}>
