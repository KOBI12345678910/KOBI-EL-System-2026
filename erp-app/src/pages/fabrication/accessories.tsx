      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder={"חיפוש פרזול..."} className="w-full bg-gray-800/50 border border-gray-700 rounded-lg pr-10 pl-4 py-2 text-sm" />
          </div>
          <select value={filterCat} onChange={e => setFilterCat(e.target.value)} className="bg-gray-800/50 border border-gray-700 rounded-lg px-3 py-2 text-sm">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? <tr><td colSpan={8} className="p-8 text-center text-muted-foreground">{"לא נמצאו פריטים"}</td></tr> :
                filtered.map((i: any) => (
                  <tr key={i.id} className="border-b border-gray-700/30 hover:bg-gray-700/20 cursor-pointer" onClick={() => { setDetail(i); setDetailTab("details"); }}>
                    ...
                    <td className="p-3 flex gap-1" onClick={e => e.stopPropagation()}>
                      <button onClick={() => { setForm({ id: i.id, partName: i.part_name, category: i.category, costPerUnit: i.cost_per_unit, currentStock: i.current_stock, status: i.status, notes: i.notes }); setShowForm(true); }} className="p-1 hover:bg-gray-600 rounded"><Edit2 className="w-4 h-4" /></button>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[{ key: "partNumber", label: "מק\"ט" }, { key: "partName", label: "שם (EN)" }, { key: "partNameHe", label: "שם (HE)" }, { key: "category", label: "קטגוריה", type: "select", options: CATEGORIES }, { key: "brand", label: "מותג" }, { key: "costPerUnit", label: "מחיר", type: "number" }, { key: "currentStock", label: "מלאי", type: "number" }, { key: "minimumStock", label: "מלאי מינימום", type: "number" }, { key: "status", label: "סטטוס", type: "select", options: ["active", "inactive"] }].map(f => (
                  <div key={f.key}><label className="block text-sm text-muted-foreground mb-1">{f.label}</label>
                    {f.type === "select" ? <select value={form[f.key] || ""} onChange={e => setForm({ ...form, [f.key]: e.target.value })} className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-sm">{f.options?.map(o => <option key={o} value={o}>{catLabels[o] || o}</option>)}</select>
