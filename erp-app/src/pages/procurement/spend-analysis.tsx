...
function HorizontalBar({ label, value, max, color }: { label: string; value: number; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-sm text-gray-200 w-24 text-left font-medium">{fmtCurrency(value)}</span>
    </div>
  );
}
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} className="flex flex-col items-center flex-1 gap-1">
            ...
            <div className="w-full bg-gray-700/50 rounded-t flex-1 relative" style={{ minHeight: 4 }}>
              <div
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-100">ניתוח הוצאות רכש</h1>
          ...
          <div key={i} className={`rounded-xl bg-gradient-to-br ${k.color} p-4 border border-white/10`}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className={`${(k as any).isText ? "text-lg" : "text-2xl"} font-bold text-white mt-1 truncate`}>{k.value}</div>
              </div>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="relative">
                  ...
                  <input type="text" placeholder="חיפוש..." value={search} onChange={e => setSearch(e.target.value)}
                    className="pr-7 pl-2 py-1 rounded border border-gray-600 bg-gray-700/50 text-gray-200 text-xs w-32" />
                </div>
              </div>
                ...
                <tbody className="divide-y divide-gray-700/50">
                  {items.length === 0 ? (
                    ...
                    <tr key={item.id || idx} className="hover:bg-gray-800/30 transition-colors">
                      <td className="px-4 py-3 text-gray-200 font-medium">{item.vendor_name || "—"}</td>
                      ...
                      <td className="px-4 py-3">
                        <Badge className="bg-gray-500/20 text-gray-300 border border-gray-600 text-xs">
