...
import { authFetch } from "@/lib/utils";
    ...
    const STATUSES = ["מצוין", "טוב", "בינוני", "דרוש שיפור", "קריטי"] as const;
    const SC: Record<string, string> = { "מצוין": "bg-emerald-500/20 text-emerald-300", "טוב": "bg-green-500/20 text-green-300", "בינוני": "bg-yellow-500/20 text-yellow-300", "דרוש שיפור": "bg-orange-500/20 text-orange-300", "קריטי": "bg-red-500/20 text-red-300" };
    const CATS = ["פיננסי","תפעולי","אנושי","לקוחות","טכנולוגי"];
...
export default function CompanyHealth() {
  ...
  const [search, setSearch] = useState("");
    ...
    try {
      setError(null);
      const res = await authFetch("/api/executive/company-health");
      if (res.ok) {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.indicator?.toLowerCase().includes(s) || r.dimension?.toLowerCase().includes(s)); }
      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingUp className="h-6 w-6 text-green-400" />בריאות החברה</h1><p className="text-sm text-muted-foreground mt-1">מדדי בריאות ארגונית, יציבות פיננסית ותפעולית</p></div>
            <div className="flex gap-2">
              ...
              <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2"><Plus className="h-4 w-4" />מדד חדש</Button>
            </div>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"ציון כולל","v":"82/100","c":"text-green-400","t":"+5","u":true},{"l":"פיננסי","v":"85","c":"text-emerald-400","t":"+3","u":true},{"l":"תפעולי","v":"78","c":"text-blue-400","t":"+4","u":true},{"l":"משאב אנושי","v":"88","c":"text-cyan-400","t":"+2","u":true},{"l":"לקוחות","v":"76","c":"text-amber-400","t":"+6","u":true},{"l":"טכנולוגי","v":"82","c":"text-purple-400","t":"+1","u":true}].map((k: any, i: number) => (
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("indicator")}>מדד<SI field="indicator" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("weight")}>משקל<SI field="weight" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("benchmark")}>Benchmark<SI field="benchmark" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("trend")}>מגמה<SI field="trend" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("lastAssessed")}>הערכה<SI field="lastAssessed" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingUp className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין בריאות החברה"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "מדד חדש ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />מדד חדש</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 "><div className="font-medium text-white">{row.indicator}</div></td>
            <td className="p-3 text-muted-foreground">{row.dimension}</td>
            <td className="p-3 font-mono text-cyan-400"><div className="flex items-center gap-2"><div className="w-12 bg-gray-700 rounded-full h-2"><div className={`rounded-full h-2 ${row.score>=80?"bg-green-500":row.score>=60?"bg-yellow-500":"bg-red-500"}`} style={{width:`${row.score}%`}} /></div><span>{row.score}</span></div></td>
            <td className="p-3 font-mono text-muted-foreground text-center">{row.weight}%</td>
            <td className="p-3 font-mono text-muted-foreground text-center">{row.benchmark}</td>
            <td className="p-3 "><span className={`font-mono ${row.trendUp?"text-green-400":"text-red-400"}`}>{row.trend}</span></td>
            <td className="p-3 text-muted-foreground text-xs">{new Date(row.lastAssessed).toLocaleDateString("he-IL")}</td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2a2a3e]"><h2 className="text-lg font-bold text-white">{editId ? "עריכה" : "מדד חדש"}</h2><Button variant="ghost" size="sm" onClick={()=>{setShowCreate(false);setEditId(null)}}><X className="h-4 w-4" /></Button></div>
            <div className="p-4 space-y-6">
              <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי מדד</h3></div>
            <div className="grid grid-cols-3 gap-4">
            <div><Label className="text-muted-foreground text-xs">מדד *</Label><Input type="text" placeholder="שם המדד" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">ממד *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>פיננסי</option><option>תפעולי</option><option>אנושי</option><option>לקוחות</option><option>טכנולוגי</option></select></div>
            ...
            <div><Label className="text-muted-foreground text-xs">משקל (%)</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">Benchmark</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>מצוין</option><option>טוב</option><option>בינוני</option><option>דרוש שיפור</option><option>קריטי</option></select></div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מדד</p><p className="text-white mt-1 font-medium">{dr.indicator}</p></div>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">משקל</p><p className="text-white mt-1 font-medium">{dr.weight}%</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">Benchmark</p><p className="text-white mt-1 font-medium">{dr.benchmark}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מגמה</p><p className="text-white mt-1 font-medium">{dr.trend}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">הערכה אחרונה</p><p className="text-white mt-1 font-medium">{new Date(dr.lastAssessed).toLocaleDateString("he-IL")}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מעריך</p><p className="text-white mt-1 font-medium">{dr.assessor}</p></div>
