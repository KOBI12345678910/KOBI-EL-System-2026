...
import { authFetch } from "@/lib/utils";
    ...
    const STATUSES = ["נוכח", "חופשה", "מחלה", "חיצוני", "לא פעיל"] as const;
    const SC: Record<string, string> = { "נוכח": "bg-green-500/20 text-green-300", "חופשה": "bg-blue-500/20 text-blue-300", "מחלה": "bg-yellow-500/20 text-yellow-300", "חיצוני": "bg-purple-500/20 text-purple-300", "לא פעיל": "bg-gray-500/20 text-gray-300" };
    const CATS = ["ייצור","הנהלה","מכירות","תחזוקה","מחסן","הובלה"];
...
export default function WorkforceStatus() {
  ...
  const [search, setSearch] = useState("");
    ...
    try {
      setError(null);
      const res = await authFetch("/api/executive/workforce-status");
      if (res.ok) {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.name?.toLowerCase().includes(s) || r.department?.toLowerCase().includes(s) || r.role?.toLowerCase().includes(s)); }
      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingUp className="h-6 w-6 text-blue-400" />סטטוס כח אדם</h1><p className="text-sm text-muted-foreground mt-1">מעקב נוכחות, זמינות, הסמכות ועומסים</p></div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"סהכ עובדים","v":"198","c":"text-blue-400","t":"+3","u":true},{"l":"נוכחים","v":"182","c":"text-green-400","t":"+5","u":true},{"l":"חופשה","v":"8","c":"text-cyan-400","t":"+2","u":true},{"l":"מחלה","v":"5","c":"text-yellow-400","t":"-1","u":true},{"l":"נוכחות","v":"92%","c":"text-emerald-400","t":"+1%","u":true},{"l":"שעות נוספות","v":"245 שע","c":"text-amber-400","t":"+12%","u":false}].map((k: any, i: number) => (
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("role")}>תפקיד<SI field="role" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("shift")}>משמרת<SI field="shift" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("hoursToday")}>שעות היום<SI field="hoursToday" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("overtime")}>נוספות<SI field="overtime" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("utilization")}>ניצולת<SI field="utilization" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingUp className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין סטטוס כח אדם"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "רשומה חדשה ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />רשומה חדשה</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 text-muted-foreground text-xs">{row.shift}</td>
            <td className="p-3 font-mono text-cyan-400">{row.hoursToday}</td>
            <td className="p-3 font-mono text-amber-400">{row.overtime}</td>
            <td className="p-3 font-mono text-green-400">{row.utilization}%</td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div><Label className="text-muted-foreground text-xs">שם *</Label><Input type="text" placeholder="שם העובד" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            <div><Label className="text-muted-foreground text-xs">מחלקה *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>ייצור</option><option>הנהלה</option><option>מכירות</option><option>תחזוקה</option><option>מחסן</option><option>הובלה</option></select></div>
            <div><Label className="text-muted-foreground text-xs">תפקיד</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>מפעיל</option><option>טכנאי</option><option>מנהל</option><option>מכירות</option><option>מחסנאי</option><option>נהג</option><option>מהנדס</option><option>בודק</option></select></div>
            <div><Label className="text-muted-foreground text-xs">משמרת</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>בוקר</option><option>צהריים</option><option>לילה</option></select></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>נוכח</option><option>חופשה</option><option>מחלה</option><option>חיצוני</option><option>לא פעיל</option></select></div>
            </div>
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">תפקיד</p><p className="text-white mt-1 font-medium">{dr.role}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">משמרת</p><p className="text-white mt-1 font-medium">{dr.shift}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">כניסה</p><p className="text-white mt-1 font-medium">{dr.checkedIn}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">שעות היום</p><p className="text-white mt-1 font-medium">{dr.hoursToday} שע</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">נוספות</p><p className="text-white mt-1 font-medium">{dr.overtime} שע</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">ניצולת</p><p className="text-white mt-1 font-medium">{dr.utilization}%</p></div>
