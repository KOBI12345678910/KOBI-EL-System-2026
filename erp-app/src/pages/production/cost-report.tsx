...
import { authFetch } from "@/lib/utils";
    ...
    const CATS = ["חומרים","עבודה","תקורה","ייצור","תחזוקה"];
...
export default function CostReport() {
  ...
  useEffect(() => { load(); }, [load]);
      ...
      const filtered = useMemo(() => {
        let d = [...data];
        if (search) { const s = search.toLowerCase(); d = d.filter(r => r.id?.toLowerCase().includes(s) || r.title?.toLowerCase().includes(s) || r.department?.toLowerCase().includes(s) || r.category?.toLowerCase().includes(s)); }
      ...
      return (
        <div className="p-6 space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingDown className="h-6 w-6 text-orange-400" />דוח עלויות</h1><p className="text-sm text-muted-foreground mt-1">ניתוח עלויות ייצור, חומרים ועבודה</p></div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[{"l":"עלות כוללת","v":"₪2.4M","c":"text-orange-400","t":"+8%","u":false},{"l":"חומרים","v":"₪1.1M","c":"text-blue-400","t":"+5%","u":false},{"l":"עבודה","v":"₪680K","c":"text-cyan-400","t":"+3%","u":false},{"l":"תקורה","v":"₪420K","c":"text-purple-400","t":"-2%","u":true},{"l":"סטייה מתקציב","v":"+4.2%","c":"text-red-400","t":"+1%","u":false},{"l":"חיסכון החודש","v":"₪85K","c":"text-green-400","t":"+12%","u":true}].map((k: any, i: number) => (
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("totalCost")}>עלות<SI field="totalCost" /></button></th>
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("budgeted")}>תקציב<SI field="budgeted" /></button></th>
        ...
        <th className="p-3 text-right text-muted-foreground font-medium"><button className="flex items-center gap-1" onClick={()=>toggleSort("status")}>סטטוס<SI field="status" /></button></th>
            ...
            {isLoading ? Array.from({length:8}).map((_,sk) => (<tr key={sk} className="border-b border-[#2a2a3e]/50"><td colSpan={99} className="p-3"><div className="flex items-center gap-4 animate-pulse"><div className="h-4 w-4 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-32 bg-[#2a2a3e] rounded" /><div className="h-4 w-24 bg-[#2a2a3e] rounded" /><div className="h-4 w-20 bg-[#2a2a3e] rounded" /><div className="h-4 w-16 bg-[#2a2a3e] rounded" /><div className="h-4 w-28 bg-[#2a2a3e] rounded" /><div className="h-4 w-12 bg-[#2a2a3e] rounded" /></div></td></tr>)) : pd.length === 0 ? <tr><td colSpan={99} className="p-20 text-center"><div className="flex flex-col items-center gap-4">{(af > 0 || search) ? <Search className="h-16 w-16 text-[#2a2a3e]" /> : <TrendingDown className="h-16 w-16 text-[#2a2a3e]" />}<p className="text-lg font-medium text-muted-foreground">{(af > 0 || search) ? "לא נמצאו תוצאות" : "עדיין אין דוח עלויות"}</p><p className="text-sm text-muted-foreground/60">{(af > 0 || search) ? "נסה לשנות את מונחי החיפוש או הסינון" : "דוח חדש ותתחיל לעבוד"}</p>{!(af > 0 || search) && <Button onClick={() => setShowCreate(true)} className="bg-blue-600 hover:bg-blue-700 gap-2 mt-2"><Plus className="h-4 w-4" />דוח חדש</Button>}</div></td></tr> : pd.map(row => (
            ...
            <td className="p-3 font-mono text-emerald-400">{fmt(row.totalCost)}</td>
            <td className="p-3 font-mono text-muted-foreground">{fmt(row.budgeted)}</td>
            <td className="p-3 "><span className={`font-mono ${Number(row.variance)>0?"text-red-400":"text-green-400"}`}>{row.variance}%</span></td>
          ...
          {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div><Label className="text-muted-foreground text-xs">קטגוריה *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>חומרים</option><option>עבודה</option><option>תקורה</option><option>ייצור</option><option>תחזוקה</option></select></div>
            <div><Label className="text-muted-foreground text-xs">מחלקה</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>ייצור</option><option>ריתוך</option><option>חיתוך</option><option>ציפוי</option><option>הרכבה</option></select></div>
            <div><Label className="text-muted-foreground text-xs">תקופה</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>ינואר 2026</option><option>פברואר 2026</option><option>מרץ 2026</option><option>אפריל 2026</option></select></div>
            <div><Label className="text-muted-foreground text-xs">עלות כוללת</Label><Input type="number" placeholder="0" className="bg-[#0a0a1a] border-[#2a2a3e] text-white mt-1" /></div>
            ...
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>טיוטה</option><option>מאושר</option><option>סופי</option><option>ארכיון</option></select></div>
            </div>
          <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פירוט</h3></div>
            <div className="grid grid-cols-3 gap-4">
          ...
          {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">עלות</p><p className="text-white mt-1 font-medium">{fmt(dr.totalCost)}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">תקציב</p><p className="text-white mt-1 font-medium">{fmt(dr.budgeted)}</p></div>
