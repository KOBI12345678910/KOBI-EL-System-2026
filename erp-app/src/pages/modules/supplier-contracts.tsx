...
import {
  ...
  ChevronDown, ChevronUp, Bell, Target, Zap
} from "lucide-react";
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex gap-1 bg-gray-800/50 rounded-xl p-1 border border-gray-700/50">
            {tabs.map(t => (
              <button key={t.key} onClick={() => setViewMode(t.key)}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === t.key ? "bg-indigo-600 text-white shadow-lg" : "text-muted-foreground hover:text-white hover:bg-gray-700/50"}`}>
          ...
          <div className="relative flex-1 min-w-0 sm:min-w-[200px] max-w-md">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש חוזה, ספק..."
              className="w-full bg-gray-800/60 border border-gray-700 rounded-lg pr-10 pl-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-indigo-500/50 focus:outline-none" />
          </div>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            className="bg-gray-800/60 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-gray-300 focus:border-indigo-500/50 focus:outline-none">
                ...
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-5">
                  ...
                  <div className="space-y-2">
                    {expiringContracts.filter(c => (c.daysLeft ?? 999) <= 30).map(c => (
                      <div key={c.id} className="flex items-center justify-between bg-gray-900/60 rounded-lg p-3 border border-gray-700/30">
                        <div className="flex items-center gap-3">
                    ...
                    <tbody>
                      {filtered.slice(0, 15).map(c => (
                        <tr key={c.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                          ...
                          <td className="py-3 px-2 text-center">
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={() => setSelectedContract(c)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Eye size={14} className="text-muted-foreground" /></button>
                              <button onClick={() => openForm(c)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Edit2 size={14} className="text-blue-400" /></button>
                              <button onClick={async () => { if (globalConfirm("למחוק חוזה זה?")) deleteMutation.mutate(c.id); }} className="p-1.5 hover:bg-gray-700 rounded-lg"><Trash2 size={14} className="text-red-400" /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          {viewMode === "list" && (
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              ...
              {filtered.map(c => {
                ...
                return (
                  <motion.div key={c.id} layout className={`bg-gray-800/40 border border-gray-700/50 rounded-xl p-4 hover:border-indigo-500/30 transition-all ${bulk.isSelected(c.id) ? "ring-1 ring-primary/50 bg-primary/5" : ""}`}>
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex gap-1">
                          <button onClick={() => setSelectedContract(c)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Eye size={15} className="text-muted-foreground" /></button>
                          <button onClick={() => openForm(c)} className="p-1.5 hover:bg-gray-700 rounded-lg"><Edit2 size={15} className="text-blue-400" /></button>
                          <button onClick={async () => { if (globalConfirm("למחוק?")) deleteMutation.mutate(c.id); }} className="p-1.5 hover:bg-gray-700 rounded-lg"><Trash2 size={15} className="text-red-400" /></button>
                        </div>
                      </div>
                    </div>
                  </motion.div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {expiringContracts.map(c => (
                      <tr key={c.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 cursor-pointer" onClick={() => setSelectedContract(c)}>
            ...
            <motion.div key="sla" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
              ...
              <div className="space-y-3">
                {contracts.filter(c => c.slaResponseTime || c.slaResolutionTime || c.slaUptimePct || c.penaltyLateDelivery || c.penaltyQualityIssue || c.penaltySlaBreach).map(c => (
                  <div key={c.id} className="bg-gray-800/40 border border-gray-700/50 rounded-xl p-5">
          ...
          {selectedContract && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                className="bg-gray-900 border border-gray-700 rounded-2xl max-w-3xl w-full max-h-[85vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => setSelectedContract(null)} className="p-2 hover:bg-gray-800 rounded-lg"><X size={20} /></button>
                </div>

                <div className="flex border-b border-gray-700 mb-6">
                  {[{key:"details",label:"פרטים"},{key:"related",label:"רשומות קשורות"},{key:"docs",label:"מסמכים"},{key:"history",label:"היסטוריה"}].map(t => (
                    <button key={t.key} onClick={() => setDetailTab(t.key)} className={`px-4 py-2.5 text-sm font-medium border-b-2 ${detailTab === t.key ? "border-indigo-500 text-indigo-400" : "border-transparent text-muted-foreground hover:text-white"}`}>{t.label}</button>
                  ))}
                </div>

                {detailTab === "details" && (<>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6 text-sm">
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">ספק:</span> <span className="font-medium mr-2">{supplierMap[selectedContract.supplierId]?.supplierName || `#${selectedContract.supplierId}`}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">סוג:</span> <span className="font-medium mr-2">{selectedContract.contractType}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">סטטוס:</span> <span className={`mr-2 px-2 py-0.5 rounded-full text-xs border ${statusColor(selectedContract.status)}`}>{selectedContract.status}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">תחילה:</span> <span className="font-medium mr-2">{selectedContract.startDate}</span></div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">סיום:</span> <span className="font-medium mr-2">{selectedContract.endDate || "ללא"}</span> {selectedContract.endDate && expiryBadge(daysUntil(selectedContract.endDate))}</div>
                  <div className="bg-gray-800/50 rounded-lg p-3"><span className="text-muted-foreground">שווי:</span> <span className="font-bold mr-2 text-blue-400">₪{parseFloat(selectedContract.contractValue || "0").toLocaleString()}</span></div>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      ...
                      <select value={form.supplierId} onChange={e => setForm({ ...form, supplierId: e.target.value })}
                        className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-indigo-500/50 focus:outline-none">
                  ...
                  <div>
                    ...
                    <textarea rows={2} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:border-indigo-500/50 focus:outline-none resize-none" />
                  </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <label className="flex items-center gap-2 text-sm text-gray-300">
                        <input type="checkbox" checked={form.autoRenewal} onChange={e => setForm({ ...form, autoRenewal: e.target.checked })}
                          className="rounded border-gray-600" /> חידוש אוטומטי
                      </label>
                      <div>
                        ...
                        <input type="number" value={form.renewalPeriodMonths} onChange={e => setForm({ ...form, renewalPeriodMonths: e.target.value })}
                          className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-indigo-500/50 focus:outline-none" />
                      </div>
                      <div>
                  ...
                  <div className="bg-gray-800/40 rounded-xl p-4 border border-gray-700/50">
                    ...
                    <div className="mt-2">
                      ...
                      <textarea rows={2} value={form.slaDetails} onChange={e => setForm({ ...form, slaDetails: e.target.value })}
                        className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-indigo-500/50 focus:outline-none resize-none" />
                    </div>
                  </div>
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    ...
                    <button onClick={handleSubmit} disabled={saveMutation.isPending || !form.supplierId || !form.title}
                      className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:bg-gray-700 disabled:text-muted-foreground rounded-lg font-medium transition-colors">
