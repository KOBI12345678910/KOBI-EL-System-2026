...
import {
  ...
  Check, XCircle, Clock, DollarSign, Users, Edit2, Trash2, Save
} from "lucide-react";
...
export default function PurchaseApprovalsPage() {
  ...
  const remove = async (id: number) => {
    if (await globalConfirm("למחוק דרישת רכש זו?")) {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-2"><CheckSquare className="text-green-400 w-6 h-6" />אישורי רכש</h1>
      ...
      {loading ? (
        ...
        <BulkActions items={filtered} selectedIds={bulk.selectedIds} onSelectionChange={bulk.setSelectedIds} actions={[
          defaultBulkActions.delete(async (ids) => { for (const id of ids) { await authFetch(`${API}/purchase-requests/${id}`, { method: "DELETE" }); } load(); }),
          defaultBulkActions.statusChange("אישור מרובה", async (ids) => { for (const id of ids) { await authFetch(`${API}/purchase-requests/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "מאושר", approvedBy: "מנהל רכש" }) }); } load(); }),
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewDetail(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><CheckSquare className="w-5 h-5 text-green-400" />דרישה {viewDetail.request_number || `#${viewDetail.id}`}</h2>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold text-white">{editing ? "עריכת דרישה" : "דרישת רכש חדשה"}</h2>
