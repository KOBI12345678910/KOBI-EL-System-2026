...
import {
  ...
  TrendingUp, CreditCard, Landmark, Download, Printer, Send, MessageCircle
} from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            }} className="flex items-center gap-1.5 bg-green-700 text-white px-3 py-2 rounded-lg hover:bg-green-600 text-sm">
            ...
            <button onClick={() => exportToWord("ספקים בחו\"ל", suppliers, { supplierCode: "קוד ספק", companyName: "שם חברה", country: "ארץ", status: "סטטוס", phone: "טלפון", email: "אימייל" }, "foreign_suppliers")} className="flex items-center gap-1.5 bg-indigo-700 text-white px-3 py-2 rounded-lg hover:bg-indigo-600 text-sm">
              <FileText size={16} /> Word
            </button>
            <button onClick={() => printPage("ספקים בחו\"ל")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
              <Printer size={16} /> הדפסה
            </button>
            <button onClick={() => sendByEmail("ספקים בחו\"ל - טכנו-כל עוזי", generateEmailBody("ספקים בחו\"ל", suppliers, { supplierCode: "קוד ספק", companyName: "שם חברה", country: "ארץ", status: "סטטוס", phone: "טלפון", email: "אימייל" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
              <Send size={16} /> שליחה
            </button>
            <button onClick={openCreate} className="flex items-center gap-2 bg-indigo-600 text-white px-3 py-2 rounded-lg hover:bg-indigo-700 shadow-lg text-sm">
        ...
        <div className="flex gap-1 bg-card rounded-xl p-1 shadow-sm border border-slate-100">
          {TABS.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === tab.key ? "bg-indigo-600 text-white shadow-md" : "text-muted-foreground hover:bg-muted/30"}`}>
                      ...
                      return (
                        <motion.div key={s.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                          ...
                          <div className="flex gap-1 pt-2 border-t border-slate-100">
                            ...
                            <button onClick={() => openEdit(s)} className="flex-1 flex items-center justify-center gap-1 text-amber-600 hover:bg-amber-50 rounded-lg py-1.5 text-sm"><Edit2 size={14} /> עריכה</button>
                            <button onClick={async () => { if (globalConfirm("למחוק ספק זה?")) deleteMut.mutate(s.id); }} className="flex-1 flex items-center justify-center gap-1 text-red-600 hover:bg-red-50 rounded-lg py-1.5 text-sm"><Trash2 size={14} /> מחיקה</button>
                          </div>
                        </motion.div>
                      );
                    })}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="flex gap-3 p-4 border-t border-slate-100 sticky bottom-0 bg-card rounded-b-2xl">
                <button onClick={handleSave} disabled={createMut.isPending || updateMut.isPending}
                  className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 text-white py-2.5 rounded-lg hover:bg-indigo-700 disabled:opacity-50">
