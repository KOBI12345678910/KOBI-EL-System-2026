...
import {
  ...
  Globe, Building2, Hash, XCircle, RefreshCw, Lock, Unlock
} from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={() => printPage("תאימות ותעודות")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
              <Printer size={16} /> הדפסה
            </button>
            <button onClick={() => sendByEmail("תאימות ותעודות - טכנו-כל עוזי", generateEmailBody("תאימות ותעודות", certs, { certNumber: "מספר", certName: "שם", certType: "סוג", status: "סטטוס", expiryDate: "תפוגה" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
              <Send size={16} /> שליחה
            </button>
            <button onClick={openCreate} className="flex items-center gap-2 bg-purple-600 text-white px-3 py-2 rounded-lg hover:bg-purple-700 shadow-lg text-sm">
        ...
        <div className="flex gap-1 bg-card rounded-xl p-1 shadow-sm border border-slate-100">
          {TABS.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === tab.key ? "bg-purple-600 text-white shadow-md" : "text-muted-foreground hover:bg-muted/30"}`}>
                      ...
                      return (
                        <motion.div key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                          ...
                          <div className="flex gap-1 pt-2 border-t border-slate-100">
                            ...
                            <button onClick={() => openEdit(c)} className="flex-1 flex items-center justify-center gap-1 text-amber-600 hover:bg-amber-50 rounded-lg py-1.5 text-sm"><Edit2 size={14} /> עריכה</button>
                            <button onClick={async () => { if (globalConfirm("למחוק תעודה זו?")) deleteMut.mutate(c.id); }} className="flex-1 flex items-center justify-center gap-1 text-red-600 hover:bg-red-50 rounded-lg py-1.5 text-sm"><Trash2 size={14} /> מחיקה</button>
                          </div>
                        </motion.div>
                      );
                    })}
