...
import { Users, Plus, Trash2, Edit2, TrendingDown, AlertTriangle } from "lucide-react";
              ...
              >
                <CardContent className="p-4">
                  ...
                  <div className="mt-3 flex gap-2">
                    <Button size="sm" variant="ghost" className="text-muted-foreground hover:text-white h-7 px-2" onClick={(e) => { e.stopPropagation(); openEditCompetitor(c); }}>
                      <Edit2 className="w-3.5 h-3.5" />
                    </Button>
                    <Button size="sm" variant="ghost" className="text-red-400 hover:text-red-300 h-7 px-2" onClick={async (e) => { e.stopPropagation(); if (globalConfirm("למחוק מתחרה זה?")) deleteCompetitorMutation.mutate(c.id); }}>
                  ...
                  {prices.map(p => {
                    ...
                    return (
                      <tr key={p.id} className="border-b border-slate-700/30 hover:bg-slate-800/30">
                        <td className="py-3 px-4 text-white">{p.productCategory}</td>
                  ...
                  {(comparison?.alerts ?? []).map(a => {
                    ...
                    return (
                      <tr key={a.id} className="border-b border-slate-700/30 hover:bg-amber-500/5">
                        <td className="py-3 px-4 text-white">{a.competitor_name}</td>
