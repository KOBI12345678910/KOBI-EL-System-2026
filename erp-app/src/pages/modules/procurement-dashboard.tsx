    ...
    <div className="min-h-screen" dir="rtl">
      <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
          ...
          <div className="flex items-center gap-2">
            {(["week", "month", "quarter", "year"] as const).map(range => (
              <button key={range} onClick={() => setTimeRange(range)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${timeRange === range ? "bg-primary text-white" : "bg-[#1a1d23] text-muted-foreground hover:text-white border border-gray-700"}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {topKPIs.map((kpi, i) => (
            <div key={i} className={`bg-gradient-to-br ${kpi.bg} border border-gray-800 rounded-2xl p-5 relative overflow-hidden`}>
            ...
            {processFlow.map((step, i) => (
              <div key={i} className="flex items-center gap-2 flex-1 min-w-[140px]">
                <div className={`flex-1 ${step.bg} border-2 ${step.color} rounded-xl p-4 text-center`}>
                  ...
                  <p className="text-sm font-semibold text-white mt-1">{step.label}</p>
                  <div className="mt-2 flex items-center justify-center gap-3 text-xs">
                    <span className="text-muted-foreground">סה״כ: <span className="text-white font-bold">{step.count}</span></span>
        ...
        <div className="bg-[#1a1d23] border border-gray-800 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {quickActions.map(a => (
              <button key={a.label} onClick={() => navigate(a.href)}
                className={`flex flex-col items-center gap-2 p-4 ${a.color} text-white rounded-xl text-sm font-medium transition-all hover:scale-[1.02] hover:shadow-lg`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {operationalMetrics.map(s => (
            <button key={s.label} onClick={() => navigate(s.href)}
              className="bg-[#1a1d23] border border-gray-800 rounded-xl p-5 flex items-center gap-4 hover:border-gray-600 transition-all hover:shadow-lg text-right w-full group">
          ...
          <div className={`border rounded-2xl p-5 ${criticalStock.length > 0 ? "bg-red-950/20 border-red-800/50" : "bg-amber-950/20 border-amber-800/50"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {lowStockMaterials.slice(0, 8).map((m: any, i: number) => {
                ...
                return (
                  <div key={m.id || i} className={`flex items-center justify-between rounded-xl p-3 ${isCritical ? "bg-red-500/5 border border-red-800/30" : "bg-[#12141a] border border-gray-800/50"}`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-8 rounded-full ${isCritical ? "bg-red-500" : "bg-amber-500"}`} />
                      <div>
                        <span className="text-white font-medium text-sm">{m.materialName}</span>
          ...
          <div className="bg-[#1a1d23] border border-gray-800 rounded-2xl p-5">
            ...
            {requests.length === 0 ? (
              <div className="text-center py-8">
                ...
                <button onClick={() => navigate("/builder/data/13")} className="flex items-center gap-2 mx-auto px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm">
              ...
              <div className="space-y-2">
                {requests.slice(0, 6).map((r: any, i: number) => (
                  <div key={r.id || i} className="flex items-center justify-between bg-[#12141a] rounded-xl p-3 hover:bg-[#1e2128] transition-colors">
                    <div className="flex items-center gap-3">
          ...
          <div className="bg-[#1a1d23] border border-gray-800 rounded-2xl p-5">
            ...
            {orders.length === 0 ? (
              <div className="text-center py-8">
                ...
                <button onClick={() => navigate("/builder/data/15")} className="flex items-center gap-2 mx-auto px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-sm">
              ...
              <div className="space-y-2">
                {orders.slice(0, 6).map((o: any, i: number) => (
                  <div key={o.id || i} className="flex items-center justify-between bg-[#12141a] rounded-xl p-3 hover:bg-[#1e2128] transition-colors">
                    <div className="flex items-center gap-3">
        ...
        <div className="bg-[#1a1d23] border border-gray-800 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-[#12141a] rounded-xl p-4 text-center">
