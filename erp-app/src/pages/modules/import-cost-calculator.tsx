      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={() => printPage("מחשבון עלויות יבוא")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
              <Printer size={16} /> הדפסה
            </button>
            <button onClick={() => sendByEmail("מחשבון עלויות יבוא - טכנו-כל עוזי", generateEmailBody("מחשבון עלויות יבוא", calculations, { calcNumber: "מספר", productName: "מוצר", totalLandedCost: "עלות נחיתה", costPerUnit: "עלות ליחידה" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
              <Send size={16} /> שליחה
            </button>
            <button onClick={openCreate} className="flex items-center gap-2 bg-emerald-600 text-white px-3 py-2 rounded-lg hover:bg-emerald-700 shadow-lg text-sm">
          ...
          <>
            {activeTab === "calculator" && !showForm && (
              <div className="text-center py-16 bg-card rounded-xl border border-slate-100">
                ...
                <button onClick={openCreate} className="inline-flex items-center gap-2 bg-emerald-600 text-white px-6 py-3 rounded-lg hover:bg-emerald-700 text-lg">
                    ...
                    {filtered.map(item => (
                      <motion.div key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                        ...
                        <div className="flex gap-1 pt-2 border-t border-slate-100">
                          ...
                          <button onClick={async () => { if (globalConfirm("למחוק חישוב זה?")) deleteMut.mutate(item.id); }} className="flex-1 flex items-center justify-center gap-1 text-red-600 hover:bg-red-50 rounded-lg py-1.5 text-sm"><Trash2 size={14} /> מחיקה</button>
                        </div>
