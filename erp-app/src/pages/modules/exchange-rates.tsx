...
import {
  ...
  Building2, AlertTriangle, Clock, Minus, RefreshCw, FileText
} from "lucide-react";
...
export default function ExchangeRatesPage() {
  ...
  const handleDelete = async (type: string, id: number) => {
    if (!(await globalConfirm("למחוק רשומה זו?"))) return;
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            }} className="flex items-center gap-1.5 bg-slate-600 text-white px-3 py-2 rounded-lg hover:bg-slate-700 text-sm">
              <Download size={16} /> ייצוא
            </button>
            <button onClick={() => printPage("ניהול שערי חליפין")} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
              <Printer size={16} /> הדפסה
            </button>
            <button onClick={() => sendByEmail("שערי חליפין - טכנו-כל עוזי", generateEmailBody("שערי חליפין", rates, { currencyCode: "מטבע", rate: "שער", changePercent: "שינוי %", rateDate: "תאריך" }))} className="flex items-center gap-1.5 bg-muted text-white px-3 py-2 rounded-lg hover:bg-slate-600 text-sm">
              <Send size={16} /> שליחה
            </button>
            <button onClick={() => openCreate("rate")} className="flex items-center gap-1 bg-emerald-600 text-white px-3 py-2 rounded-lg hover:bg-emerald-700 text-sm shadow-lg">
              <Plus size={16} /> שער חדש
            </button>
            <button onClick={() => openCreate("hedge")} className="flex items-center gap-1 bg-blue-600 text-white px-3 py-2 rounded-lg hover:bg-blue-700 text-sm shadow-lg">
              <Plus size={16} /> חוזה גידור
            </button>
            <button onClick={() => openCreate("exposure")} className="flex items-center gap-1 bg-purple-600 text-white px-3 py-2 rounded-lg hover:bg-purple-700 text-sm shadow-lg">
              <Plus size={16} /> חשיפה
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-1 bg-card rounded-xl p-1 shadow-sm border border-slate-100">
          {TABS.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === tab.key ? "bg-emerald-600 text-white shadow-md" : "text-muted-foreground hover:bg-muted/30"}`}>
                        ...
                        <tr key={r.id} className="border-t border-slate-100 hover:bg-muted/30">
                          ...
                          <td className="p-3">
                            <div className="flex gap-1">
                              ...
                              <button onClick={() => handleDelete("rate", r.id)} className="p-1 text-red-600 hover:bg-red-50 rounded"><Trash2 size={14} /></button>
                            </div>
                          </td>
                        </tr>
                      );
                  ...
                  return (
                    <motion.div key={h.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      ...
                      <div className="flex gap-1 pt-2 border-t border-slate-100">
                        <button onClick={() => openEdit("hedge", h)} className="flex-1 flex items-center justify-center gap-1 text-amber-600 hover:bg-amber-50 rounded-lg py-1.5 text-sm"><Edit2 size={14} /> עריכה</button>
                        <button onClick={() => handleDelete("hedge", h.id)} className="flex-1 flex items-center justify-center gap-1 text-red-600 hover:bg-red-50 rounded-lg py-1.5 text-sm"><Trash2 size={14} /> מחיקה</button>
                      </div>
                    </motion.div>
                  );
                })}
                      ...
                      <tr key={e.id} className="border-t border-slate-100 hover:bg-muted/30">
                        ...
                        <td className="p-3">
                          <div className="flex gap-1">
                            ...
                            <button onClick={() => handleDelete("exposure", e.id)} className="p-1 text-red-600 hover:bg-red-50 rounded"><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="flex gap-3 p-4 border-t border-slate-100 sticky bottom-0 bg-card rounded-b-2xl">
                <button onClick={handleSave} disabled={createMut.isPending || updateMut.isPending}
                  className="flex-1 flex items-center justify-center gap-2 bg-emerald-600 text-white py-2.5 rounded-lg hover:bg-emerald-700 disabled:opacity-50">
