...
import {
  ...
  AlertTriangle, ArrowUpDown, XCircle, Eye, Wrench, BarChart3, Hash
} from "lucide-react";
...
export default function QualityControlPage() {
  ...
  const remove = async (id: number) => {
    if (await globalConfirm("למחוק בדיקת איכות?")) { await authFetch(`${API}/quality-inspections/${id}`, { method: "DELETE" }); load(); }
  };
  ...
  return (
    <div className="p-4 md:p-6 space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-2"><Shield className="text-green-400 w-6 h-6" />בקרת איכות</h1>
      ...
      {loading ? (
        ...
        <BulkActions items={filtered} selectedIds={bulk.selectedIds} onSelectionChange={bulk.setSelectedIds} actions={[
          defaultBulkActions.delete(async (ids) => { for (const id of ids) { await authFetch(`${API}/quality-inspections/${id}`, { method: "DELETE" }); } load(); }),
          ...
          <table className="w-full text-sm"><thead className="bg-muted/30 border-b border-border/50"><tr>
            ...
            {pagination.paginate(filtered).map(r => (
              <tr key={r.id} className={`border-b border-border/20 hover:bg-muted/20 transition-colors ${bulk.isSelected(r.id) ? "bg-primary/5" : ""}`}>
                ...
                <td className="px-4 py-3 text-white max-w-[150px] truncate">{r.product_name || "—"}</td>
                ...
                <td className="px-4 py-3"><div className="flex gap-1">
                  ...
                  <button onClick={async()=>{if(await globalConfirm(`למחוק את '${r.inspection_number || r.id}'? פעולה זו אינה ניתנת לביטול.`))remove(r.id)}} className="p-1.5 hover:bg-muted rounded-lg"><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
                </div></td>
              </tr>
            ))}
          </tbody></table>
        ...
        {viewDetail && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewDetail(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center"><h2 className="text-lg font-bold text-white flex items-center gap-2"><Shield className="w-5 h-5 text-green-400" />בדיקה {viewDetail.inspection_number}</h2><button onClick={() => setViewDetail(null)} className="p-1 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button></div>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-border flex justify-between items-center"><h2 className="text-lg font-bold text-white">{editing ? "עריכת בדיקה" : "בדיקת איכות חדשה"}</h2><button onClick={() => setShowForm(false)} className="p-1 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button></div>
