      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-6">
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl font-black text-white tracking-tight">המלצות AI</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          <div className="relative">
            ...
            <input type="text" placeholder="חיפוש המלצה..." value={search} onChange={e => setSearch(e.target.value)} className="bg-[#0d1117] border border-gray-800 rounded-xl pr-9 pl-4 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500/50 w-64" />
          </div>
          <div className="flex gap-1.5">
            {[
              ...
              <button key={f.key} onClick={() => setStatusFilter(f.key)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${statusFilter === f.key ? "bg-purple-600/20 text-purple-400 border border-purple-500/40" : "bg-[#0d1117] text-muted-foreground border border-gray-800 hover:border-gray-600"}`}>
                {f.label}
              </button>
            ))}
          </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-[#0d1117] border border-gray-800 rounded-2xl"><Lightbulb className="w-12 h-12 text-muted-foreground mx-auto mb-4" /><h3 className="text-white font-bold mb-2">אין המלצות</h3><p className="text-muted-foreground text-sm">שנה את הסינון או חכה להמלצות חדשות</p></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map((rec, i) => {
              ...
              return (
                <motion.div key={rec.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className={`bg-[#0d1117] border border-gray-800 rounded-2xl overflow-hidden ${rec.status !== "pending" ? "opacity-70" : ""}`}>
                  <div className={`flex items-center justify-between px-4 py-3 ${cat.bg} border-b border-gray-800`}>
                    <div className="flex items-center gap-2">
                      <CatIcon className={`w-4 h-4 ${cat.color}`} />
                      <span className={`text-xs font-bold ${cat.color}`}>{cat.label}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      <span>ביטחון: <span className="text-white font-bold">{confidence.toFixed(0)}%</span></span>
                    </div>
                  </div>
                  ...
                  <div className="px-4 py-3">
                    ...
                    {rec.recommendationType && (
                      <div className="mt-2 flex items-center gap-2 text-[10px] text-muted-foreground">
                        <span className="px-1.5 py-0.5 rounded bg-gray-800">{rec.recommendationType}</span>
                        {rec.entityType && <span className="px-1.5 py-0.5 rounded bg-gray-800">{rec.entityType}</span>}
                      </div>
                    )}
                  </div>
