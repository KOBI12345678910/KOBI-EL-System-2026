...
export default function ModelCatalogPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await authFetch("/api/executive/model-catalog");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">קטלוג מודלים — Data Dictionary</h1>
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-slate-800 border border-slate-700 rounded-lg pr-10 pl-4 py-2 text-sm text-white placeholder:text-muted-foreground focus:outline-none focus:border-blue-500"
          />
        </div>
          ...
          {tabs.map(tab => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs transition-colors ${
                activeTab === tab.id ? "bg-blue-600 text-white" : "text-muted-foreground hover:text-white"
              }`}
            >
                    ...
                    {modEntities.map(ent => (
                      <div key={ent.id} className="flex items-center gap-3 bg-slate-900/50 rounded-lg p-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-white">{ent.name}</p>
          ...
          <div className="space-y-2">
            {filteredEntities.map(ent => (
              <div key={ent.id} className="flex items-center gap-3 bg-slate-900/50 rounded-lg p-3 hover:bg-slate-900/80 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-white">{ent.name}</p>
          ...
          <div className="space-y-2">
            {filteredRoutes.map((route, i) => (
              <div key={i} className="flex items-center gap-3 bg-slate-900/50 rounded-lg p-3 hover:bg-slate-900/80 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-mono text-white">{route.path}</p>
