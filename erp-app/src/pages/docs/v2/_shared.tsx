// Shared helpers for docs v2 pages
...
export function StatusBadge({ state }: { state?: string | null }) {
  ...
  const tone: Record<string, string> = {
    ...
    partially_signed: "bg-amber-200 text-amber-900",
...
export function useSearchState(defaults: { q?: string; state?: string; limit?: number; category?: string } = {}) {
  const [q, setQ] = useState(defaults.q ?? "");
  const [state, setState] = useState(defaults.state ?? "");
  const [category, setCategory] = useState(defaults.category ?? "");
  const [offset, setOffset] = useState(0);
  const limit = defaults.limit ?? 50;
  return { q, setQ, state, setState, category, setCategory, offset, setOffset, limit };
}
