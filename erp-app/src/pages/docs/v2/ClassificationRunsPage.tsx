...
export default function ClassificationRunsPage() {
  const s = useSearchState({ limit: 50 });
  const { data, isLoading, error } = useList<any>(
    ["docs", "classification-runs", s.state, s.offset],
    "/classification-runs",
    { status: s.state || undefined, limit: s.limit, offset: s.offset },
  );
  return (
    <div dir="rtl" className="p-6 space-y-4">
      <h1 className="text-2xl font-semibold">ריצות סיווג</h1>
        ...
        <div className="overflow-auto rounded border">
          <table className="w-full text-sm">
            <thead className="bg-gray-100"><tr>
              ...
              <th className="p-2 text-right">מסווג</th>
              <th className="p-2 text-right">קטגוריה חזויה</th>
            ...
            <tbody>
              {data.rows.map((r: any) => (
                <tr key={r.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="p-2">{r.classifier_name ?? "—"}</td>
                  <td className="p-2">{r.predicted_category ?? "—"}</td>
