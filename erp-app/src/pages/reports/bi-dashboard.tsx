...
import { authFetch } from "@/lib/utils";
  ...
  const STATUSES = ["פעיל", "טיוטה", "משותף", "ארכיון"] as const;
  const SC: Record<string, string> = { "פעיל": "bg-green-500/20 text-green-300", "טיוטה": "bg-gray-500/20 text-gray-300", "משותף": "bg-blue-500/20 text-blue-300", "ארכיון": "bg-purple-500/20 text-purple-300" };
  const REPORT_TYPES = ["מכירות","כספים","ייצור","HR","לקוחות","מלאי","פרויקטים","כללי"];
...
export default function BIDashboard() {
  ...
  const [search, setSearch] = useState("");
    ...
    const filtered = useMemo(() => {
      let d = [...data];
      if (search) { const s = search.toLowerCase(); d = d.filter(r => r.title.includes(s)||r.id.toLowerCase().includes(s)||r.reportType.includes(s)); }
      if (statusFilter !== "all") d = d.filter(r => r.status === statusFilter);
      if (rptFilter !== "all") d = d.filter(r => r.reportType === rptFilter);
      return d;
    }, [search, statusFilter, rptFilter]);
    ...
    const af = [statusFilter !== "all", rptFilter !== "all"].filter(Boolean).length;
    const dr = showDetail ? data.find(r => r.id === showDetail) : null;

    return (
      <div className="p-6 space-y-4" dir="rtl">
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><TrendingUp className="h-6 w-6 text-blue-400" />דשבורד BI</h1><p className="text-sm text-muted-foreground mt-1">לוח בקרה אנליטי — מדדים, גרפים וניתוחים מתקדמים</p></div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {[{"l":"דשבורדים","v":"35","c":"text-blue-400","t":"+5","u":true},{"l":"פעילים","v":"20","c":"text-green-400","t":"+3","u":true},{"l":"צופים","v":"156","c":"text-purple-400","t":"+22","u":true},{"l":"ווידג'טים","v":"198","c":"text-cyan-400","t":"+15","u":true},{"l":"עדכון אחרון","v":"2 שעות","c":"text-amber-400","t":"-30m","u":true},{"l":"משותפים","v":"12","c":"text-indigo-400","t":"+2","u":true}].map((k: any, i: number) => (
        ...
        <Card className="bg-[#1a1a2e]/60 border-[#2a2a3e]"><CardContent className="p-3"><div className="flex flex-wrap items-center gap-3">
          ...
          <select value={rptFilter} onChange={e => { setRptFilter(e.target.value); setPage(1); }} className="bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white"><option value="all">כל הסוגים</option>{REPORT_TYPES.map((o: string) => <option key={o} value={o}>{o}</option>)}</select>
          {af > 0 && <Button variant="ghost" size="sm" onClick={() => { setStatusFilter("all"); setRptFilter("all"); setSearch(""); }} className="text-red-400 hover:text-red-300 gap-1"><X className="h-3 w-3" />נקה ({af})</Button>}
        ...
        <th className="p-3 text-right text-muted-foreground font-medium">יוצר</th>
        <th className="p-3 text-right text-muted-foreground font-medium">צופים</th>
        <th className="p-3 text-right text-muted-foreground font-medium">ווידג'טים</th>
        <th className="p-3 text-right text-muted-foreground font-medium">רענון</th>
        <th className="p-3 text-right text-muted-foreground font-medium">מקור נתונים</th>
        <th className="p-3 text-right text-muted-foreground font-medium">משותף</th>
        <th className="p-3 text-right text-muted-foreground font-medium">סטטוס</th>
          ...
          {pd.map(row => (
            ...
            <td className="p-3 text-muted-foreground text-xs">{row.creator}</td>
            <td className="p-3 text-center text-muted-foreground">{row.viewers}</td>
            <td className="p-3 text-center text-cyan-400 font-mono">{row.widgets}</td>
            <td className="p-3 text-muted-foreground text-xs">{row.refreshRate}</td>
            <td className="p-3 text-muted-foreground text-xs">{row.dataSource}</td>
            <td className="p-3 text-muted-foreground text-xs">{row.shared?"כן":"לא"}</td>
        ...
        {showCreate && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>{setShowCreate(false);setEditId(null)}}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          ...
          <div className="p-4 space-y-6">
            <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">פרטי דשבורד</h3></div>
            ...
            <div><Label className="text-muted-foreground text-xs">סוג *</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>מכירות</option><option>כספים</option><option>ייצור</option><option>HR</option><option>לקוחות</option><option>מלאי</option><option>פרויקטים</option><option>כללי</option></select></div>
            <div><Label className="text-muted-foreground text-xs">מקור נתונים</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>PostgreSQL</option><option>API</option><option>Excel</option><option>CSV</option><option>Mixed</option></select></div>
            <div><Label className="text-muted-foreground text-xs">רענון</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>זמן אמת</option><option>כל שעה</option><option>יומי</option><option>שבועי</option></select></div>
            <div><Label className="text-muted-foreground text-xs">משותף</Label><div className="mt-2"><input type="checkbox" defaultChecked className="rounded" /><span className="text-sm text-white mr-2">כן</span></div></div>
            <div><Label className="text-muted-foreground text-xs">תזמון אימייל</Label><div className="mt-2"><input type="checkbox" defaultChecked className="rounded" /><span className="text-sm text-white mr-2">כן</span></div></div>
            <div><Label className="text-muted-foreground text-xs">סטטוס</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>פעיל</option><option>טיוטה</option><option>משותף</option><option>ארכיון</option></select></div>
            </div>
          <div className="border-b border-[#2a2a3e] pb-2"><h3 className="text-sm font-semibold text-blue-400">הגדרות</h3></div>
            <div className="grid grid-cols-3 gap-4">
            <div className="col-span-3"><Label className="text-muted-foreground text-xs">תיאור</Label><textarea rows={3} className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1 resize-none" placeholder="תיאור הדשבורד..." /></div>
            <div><Label className="text-muted-foreground text-xs">קבוצת הרשאה</Label><select className="w-full bg-[#0a0a1a] border border-[#2a2a3e] rounded-md px-3 py-2 text-sm text-white mt-1"><option>בחר...</option><option>הכל</option><option>הנהלה</option><option>מנהלים</option><option>כספים</option></select></div>
        ...
        {dr && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={()=>setShowDetail(null)}><div className="bg-[#1a1a2e] border border-[#2a2a3e] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">יוצר</p><p className="text-white mt-1 font-medium">{dr.creator}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">צופים</p><p className="text-white mt-1 font-medium">{dr.viewers}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">ווידג'טים</p><p className="text-white mt-1 font-medium">{dr.widgets}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">רענון</p><p className="text-white mt-1 font-medium">{dr.refreshRate}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מקור</p><p className="text-white mt-1 font-medium">{dr.dataSource}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">עדכון</p><p className="text-white mt-1 font-medium">{dr.lastUpdated}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">משותף</p><p className="text-white mt-1 font-medium">{dr.shared?"כן":"לא"}</p></div>
            <div className="bg-[#0a0a1a] rounded-lg p-3"><p className="text-[11px] text-muted-foreground">מתוזמן</p><p className="text-white mt-1 font-medium">{dr.scheduled?"כן":"לא"}</p></div>
