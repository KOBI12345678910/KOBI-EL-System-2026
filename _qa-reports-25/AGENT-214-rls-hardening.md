...
`governance.current_tenant_id()` resolves the tenant in this order:

1. **JWT claim** `request.jwt.claim.tenant_id` - preferred path. The web/edge client puts the user's active tenant on the JWT at sign-in. Stable, no extra round-trip.
2. **Session GUC** `app.current_tenant_id` - for cron jobs and edge functions where there is no JWT but we can `SET LOCAL app.current_tenant_id = '...'` at the top of the transaction.
3. **Profile row** `governance.users_profile.tenant_id` - last-resort fallback for human sessions where the JWT was minted before the multi-tenant claim was added.

`governance.is_service_role()` checks `request.jwt.claim.role = 'service_role'`. Edge functions and the platform admin tooling use the service-role key, so this is the right escape hatch for system tables.

Both functions are `STABLE`, `SECURITY DEFINER`, granted to all roles. They never raise; missing claims simply yield NULL/false, which causes RLS to deny.

---

## What this migration does NOT do

Per AGENT-09 plan, this is migration M5/M78 (combined) of the 11-step roadmap. The other migrations remain prerequisites or follow-ups:

- **M1 (00072)** create the helper - merged INTO this migration so 00073 is self-contained.
- **M2 (00073-renumber)** add 167 missing FK indexes - separate migration; required before the `tenant_id` predicate becomes performant on large tables. **Do not run 00073 against production until 00075 indexes ship.**
- **M3 (00074)** add `tenant_id` columns to the 57 child/line-item tables that lack them. Until then, those tables fall back to admin-only in this migration (see step 8).
- **M4 (00075)** index the 29 `tenant_id` columns currently unindexed.
- **M77 (00077)** harden the schema-qualified domains (`commercial.*`, `finance.*`, `procurement.*`, etc.). 00073 only touches `public.*`.
- **M80 (00080)** fix `WITH CHECK NULL` policies on INSERT-only policies.
- **M81 (00081)** add `CHECK` constraints for status enums and non-negative money.
- **M82 (00082)** drop legacy duplicate tables (`inventory`, `invoices` singular).
...
- `C:\Users\kobi\OneDrive\kobi\המערכת 2026  KOBI EL\.claude\worktrees\objective-merkle-40ff93\supabase\migrations\00068_harden_rls_policies_always_true.sql`
- `C:\Users\kobi\OneDrive\kobi\המערכת 2026  KOBI EL\.claude\worktrees\objective-merkle-40ff93\supabase\migrations\00071_remove_dangerous_anon_read_policies.sql`
- `C:\Users\kobi\OneDrive\kobi\המערכת 2026  KOBI EL\.claude\worktrees\objective-merkle-40ff93\supabase\migrations\00004_rls_helper_functions_v2.sql`

## Output file

- `C:\Users\kobi\OneDrive\kobi\המערכת 2026  KOBI EL\.claude\worktrees\objective-merkle-40ff93\supabase\migrations\00073_rls_hardening.sql` (written separately, contains the SQL above verbatim)
